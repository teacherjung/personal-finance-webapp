...
//    工作流對抗驗證 2026-08-28 實測：替 `bank: ''` 的機構（清單 38 家裡的 36 家）補純裝飾別名
...
/** 「其他（自行輸入）」那一項的選項值。
 * ⚠️ 它是**下拉的哨兵值**，`resolveIssuerInput` 永遠不會把它當成發卡行存下去。
 * ⚠️ 誠實劃界（工作流 2026-08-28）：這**不等於** `card.issuer` 不可能等於這個字串——
 *    使用者在「其他」文字框裡真的打 `__other__`，或經 CRUD／備份匯入送這個字串，都存得進去。
 *    存進去之後它在每一個讀取端都與一般自由文字**完全同路**（實測 round-trip 原字不動），
 *    全 repo 沒有一處拿存下來的 `card.issuer` 去跟這個哨兵比，所以不是缺陷——但「永遠不會被存進」是假的。 */
