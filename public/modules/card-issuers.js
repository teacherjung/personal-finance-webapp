 ...
 *    要講準的是**方向**：這一步只能把 `ok` 變成 `unconfirmed`（認不出來），
 *    **永遠不能把身分指去別家**——所以改壞這裡的代價是多按幾次選卡，不是帳單歸錯卡。
 *
 ...
 * ⇒ 2026-09-02 起，**有代號的卡不會因為這裡被動手腳而被指去別家**——它們只會從 `ok` 掉到
 *   `unconfirmed`（認不出來、退成手選）。⚠️ 但**它們仍然會經過這裡**（`cardCode` 用 `issuersNamed`
 *   做確認），所以不可以說「走不到這裡」；擋住危害的是**方向單向**，不是「不經過」。
