...
// ⚠️ **只寫「會改變結果」的別名**（`test/card-issuers.test.js` 有一題機械檢查，加裝飾用的別名會轉紅）。
...
//   （＝我們真的有內建範本的），其餘一律走「請使用者自己選卡」。列進來只為了①清單挑得到
//   ②把「這個寫法被誰宣稱過」記下來，好算出歧義。
...
/** 「其他（自行輸入）」那一項的選項值。⚠️ 它**只活在表單裡**，永遠不會被存進 `card.issuer`。 */
...
export const CARD_ISSUERS = Object.freeze([
  ...
  // 「台新」是**唯一**要靠 aka 收的寫法：`OWN_ISSUERS` 的樣式要求行名後面跟佐證詞（`台新銀行`／`Taishin`／
  // `Richart` 都直接命中樣式），只有裸的短名對不上——而既有卡片與 repo 既有考題就是這樣填的。
 ...
 *
 * ## ⚠️ 誠實劃界：「原字保存」到底保證到哪裡（Codex #520 r2#3 修正）
 *
 * 我原本在這裡寫「這是這支唯一還會動到既有值的路徑」——**那句不準**。表單走完一圈之後，
 * 既有值會不會變，要看它落在哪一格（三種都要按下儲存才會發生）：
