 ...
 * ⚠️ **代號查表本身不過這裡**（`issuerById` 逐字相等——代號是我們自己發的機器值）；
 *    ⚠️ **但「顯示名確認了代號嗎」那一步會過這裡**（`cardCode` 走 `issuersNamed`）。
 *    第一版寫成「代號那條路一個字都不過這裡」，那是**錯的**（Codex #547 r6）。
 *    要講準的是**射程**：改壞這裡可以讓有代號的卡在 `ok` 與 `unconfirmed` **兩個方向都移動**
 *    （把別家名稱折成這一家 ⇒ 本來說不清楚的變成算數＝**放寬**；反過來則是認不出來）。
 *    ⚠️ 第一版寫「只能把 `ok` 變成 `unconfirmed`」是**過強**（Codex #547 r7）。
 *    真正站得住的只有這一句：**永遠不能把有代號的卡指去別家**——身分只會是那個代號自己那一家，
 *    或者判不出來。所以改壞這裡的代價是「認不認得出」變了，不是「認成誰」變了。
 *
 ...
 * ⇒ 2026-09-02 起，**有代號的卡不會因為這裡被動手腳而被指去別家**——最壞只是在
 *   `ok`／`unconfirmed` 之間移動（認不認得出變了，認成誰沒變）。⚠️ 但**它們仍然會經過這裡**
 *   （`cardCode` 用 `issuersNamed` 做確認），所以不可以說「走不到這裡」；
 *   擋住危害的是**身分只可能是代號那一家或空**，不是「不經過」、也不是「只往一個方向動」。
 *   而**沒有代號的卡照舊完整走這裡**（William 裁示「照舊自動＋提示升級」）——這一段今天仍然承重，
 *   在那條路上它照舊可以憑空授予身分。要整條關掉＝「只認代號」，那是另案。
 * @param {unknown} s
 */
 ...
 *
 * ・顯示名是空白 ⇒ `ok`（沒有相反證據）。
 *   ⚠️ **這一格在錢的路徑上是放寬，照實記**（Grok 複審後掃 2026-09-02）：一張**發卡行沒填**的卡
 *   在改動前一定落在 `unsureCards`、擋住**所有**銀行的「單卡自動歸」；替它寫上一個可解析的代號
 *   （只送 `issuerId`、不送 `issuer`——表單做不到，只有 API／備份匯入產得出來）之後，
 *   它會被當成「確定是代號那一家」而出局，於是別的銀行的單卡自動歸就成立了。
 *   **判定為可接受**：那是資料自己明確宣告的身分，不是程式猜的；而且方向與 J10 同一類
 *   （代號讓唯一性更容易成立、歸的是對的那張）。但它是**行為差異**，所以有考題釘著、不是默默發生。
 ...
 *    就被歸成台新，而全卷 37 題照樣綠。
 *    ⚠️ 改成 `unconfirmed` 之後**變成真的那一句，不是「不受顯示字串影響」**（顯示字串照樣會讓卡在
 *    `ok`／`unconfirmed` 之間移動）——變成真的是下面這句更窄、也更該記住的：
 *
 ...
 *
 * 判準（由上而下，第一個命中就回）：
 *   ①**代號可用**（`cardCode` 回 `ok`＝顯示名確認了它，或顯示名是空的）⇒ 預選那一項。
 *     ⚠️ 不是「查得到就預選」（那句 r2 之前寫過、已撤回）：說不清楚的卡要落回②，
 *     否則使用者按個儲存就把「畫面寫甲、代號寫乙」寫死了。
