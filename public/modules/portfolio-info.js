...
export const FX_INFO_HTML = `<p>匯率的來源依序是：<b>Yahoo 報價</b> → 抓不到就走<b>備援匯率 API</b>（兩個）→ 都抓不到就沿用<b>上次抓到的匯率</b> → 從來沒抓到過才用<b>預設值</b>（美元 32、英鎊 40.8、日圓 0.215）。開 app 時會自動抓一次（超過一小時才抓），投資頁「更新報價」可隨時再抓。</p>
