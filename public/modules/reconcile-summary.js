...
export function totalsCheckSentence(tc) {
  ...
  if (status === 'pass') {
    ...
    // ⚠️ **「有跑」≠「這幾型都罩到了」**（複審後掃＋r4 兩輪抓到的同一族）：這道只擋得住
    //    **會改動它比到的那幾欄**的錯——漏掉一筆收入不會改動支出合計、方向對調不會改動筆數。
    //    所以沒比到的欄要點名，「只比到筆數」那一格再多講一句（它連方向都罩不到）。
    //    ⚠️ 誠實殘餘（考題釘住，不寫進畫面免得吵）：金額 0 或小於容差 BAL_EPS 的列方向對調，
    //    兩側合計都不會變＝比到出入合計也看不到；那種列不影響金額，取捨照實記在這裡。
    ...
    return `合計交叉驗證：帳單印的${names.join('、')}與逐筆算出來的一致。`
      + (missing.length ? `⚠️ 帳單沒印${missing.join('、')}，這道只擋得住<b>會改動${names.join('、')}</b>的錯。` : '')
      + (sums ? '' : '<b>方向對調不會改變筆數</b>，所以「每個帳戶第一筆的方向讀反」這一型仍然沒有第二道把關，請自己核對收支方向。');
  }
