...
<details>
  ...
  <ul style="margin:8px 0 0;padding-left:18px;line-height:1.9">
    ...
    <li>讀出來的結果會**先回到畫面上讓你核對**；那份暫時放在伺服器的記憶體裡（程式重開就沒了），要等你按下匯入，才寫進這個 app 自己的資料裡。AI 那邊不會保留一份給你用。</li>
 ...
 * ⚠️ 第二段（**驗不到什麼**）必須留在畫面上、不可搬進 `<details>`：那是誠實劃界的白話版——
 * 強閘只驗數字扣不扣得起來，機構名／帳號／日期／摘要讀錯它一概驗不到。
 * ⚠️ 驗算**結論**只住 `reconcile-summary.js` 的 `gateSummaryHtml`（兩頁共用翻譯）；本徽章只講
 * 「誰讀的、驗不到什麼」，不重述驗了幾關。
 * @param {any} preview
 */
...
<div class="card" style="margin-bottom:12px;padding:12px 14px">
  ...
  <details style="margin-top:8px">
    ...
    <ul class="muted" style="margin:8px 0 0;padding-left:18px;font-size:12px;line-height:1.9">
      <li>一樣要通過驗算才准匯入：拿<b>台幣帳戶</b>的明細，把相鄰兩筆的餘額扣起來看接不接得上、最後一筆的餘額對不對得上概要那一欄。對不上就整份退回。</li>
      <li>⚠️ <b>這道驗算看不到的三件事</b>：①每個帳戶的<b>第一筆</b>沒有前一筆可比，金額與方向驗不到 ②<b>外幣</b>明細不在這道驗算裡（它們本來就不計入台幣收支；外幣帳戶的餘額仍會照帳單更新）③萬一金額和餘額一起被抄成剛好自洽的另一組數字，數學是平的、驗算就看不出來。所以下面兩張表還是請你自己看一眼。</li>
      <li>台幣帳戶如果整組都沒東西可驗（例如帳單上根本沒印每一筆的餘額），這一份也不收。這點比內建範本更嚴：內建範本驗不到會照舊放行，AI 讀的驗不到就整份退回。</li>
