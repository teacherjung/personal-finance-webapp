...
<div class="card" style="margin-bottom:12px;padding:12px 14px">
  ...
  <details style="margin-top:8px">
    ...
    <ul class="muted" style="margin:8px 0 0;padding-left:18px;font-size:12px;line-height:1.9">
      ...
      <li>⚠️ <b>這些驗算看不到的（不只這些）</b>：①<b>日期或店名讀錯</b>——金額全對、驗算照過，但那兩欄是「這筆有沒有匯過」的辨識依據，下次重匯同一份帳單時可能被當成新的一筆再記一次 ②帳單用了我們沒看過的繳款字眼＝繳款列被當成消費算進加總，通常整份被<b>誤擋</b>（不會誤收）③<b>兩筆正負剛好抵銷的交易一起漏抄</b>（例如 +100 的消費和 −100 的退款）——加總不變、等式照平，三道防線都看不到 ④摘要與明細<b>整組</b>被抄成帳單他處恰好印過、又剛好自洽的另一組數字——同樣看不到。所以上面那行「請確認」還是要請你自己看一眼。</li>
      <li>你現在看到的這一份，就是按下確認會寫進去的那一份——換卡重看也是同一份、不會再讀一次。這份預覽擺太久、或已經按過一次確認，就會失效；那時請重新上傳預覽一次。</li>
    </ul>
  </details>
</div>`;
}
