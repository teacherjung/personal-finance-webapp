...
<details>
  ...
  <ul style="margin:8px 0 0;padding-left:18px;line-height:1.9">
    ...
    ${card
    ? `<li>信用卡帳單走<b>單讀</b>：先讀一次，讀出來對不平才換更強的模型再讀一次（讀取費用一發起跳、最多兩發）。讀出來的每一筆會先跟帳單自己印的合計<b>逐筆驗算</b>，對不上就整份不收。匯入成功後會<b>再花一發</b>試著把這個版面學成規則卡——學成之後，下期同版面會<b>先免費試讀</b>（讀得動就不花錢、不外送；版面改版或讀不動時才會再用 AI）；沒學成會告訴你。</li>`
...
<div class="card" style="margin-bottom:12px;padding:12px 14px">
  ...
  <details style="margin-top:8px">
    <summary style="font-size:12px">AI 讀的，跟平常讀的差在哪？</summary>
    <ul class="muted" style="margin:8px 0 0;padding-left:18px;font-size:12px;line-height:1.9">
      <li>一樣要通過驗算才准匯入，卡片是兩道：①<b>應繳等式</b>——上期應繳 −已繳退款 ＋本期新增 ＋帳單上有名字的調整項（利息、年費…），要對得上本期應繳；②<b>逐筆加總</b>——明細加起來要對得上帳單自己印的摘要（退款折進「本期新增」或算進「已繳款退款」桶，兩種常見印法都認得；繳款列**有印出來時**另外對「已繳款」桶——只印在摘要的繳款對不了）。任何一道對不上就整份退回、分毫不差才收，比內建範本更嚴。帳單上有名字的調整項（利息、年費…）<b>會一併記進帳</b>（日期用該列印的日期；沒印用帳單期別的 1 號，連期別都沒讀到就用最近一筆明細的日期）——在下面清單裡照樣可勾選、可改分類。</li>
      <li>另有一道<b>接地檢查</b>：答案上的每個金額都要在帳單上找得到自己的位置，AI 自己算的、猜的數字進不來（日期與店名刻意不查——民國年換算後字面就不同了）。</li>
      <li>⚠️ <b>這些驗算看不到的（不只這些）</b>：①<b>日期或店名讀錯</b>——金額全對、驗算照過，但那兩欄是「這筆有沒有匯過」的辨識依據，下次重匯同一份帳單時可能被當成新的一筆再記一次 ②帳單用了我們沒看過的繳款字眼＝那筆會被當一般消費／退款算——多半讓驗算對不上而整份<b>誤擋</b>；但金額恰好與摘要對得齊時就變成⑤那型（會誤收、不是保證擋得住）③<b>兩筆正負剛好抵銷的交易一起漏抄</b>（例如 +100 的消費和 −100 的退款）——加總不變、等式照平，三道防線都看不到 ④摘要與明細<b>整組</b>被抄成帳單他處恰好印過、又剛好自洽的另一組數字——同樣看不到 ⑤<b>繳款和退款的身分抄錯</b>——負數列算哪種，是看 AI 抄回來的說明文字判定的。驗算擋得住其中一型（退款被抄成繳款、而退款原本折在「本期新增」裡＝金額立刻對不上）；<b>擋不住</b>另一型：繳款被抄成退款、金額又恰好等於「已繳款」合計（最常見的整筆繳卡款正是這樣）——算術上與真退款分不出來，會被當成消費抵減匯入。遇到這種形（明細沒有繳款列、卻有退款列）畫面會另外提醒；請看一眼負數的列：標「繳款」的要真的是繳款、大額退款要真的是退款 ⑥<b>「已繳款退款」桶裡的退款被漏抄</b>——繳款只印在摘要（明細沒有繳款列）時，那個合計沒有可對的列；桶裡若還有一筆退款、AI 漏抄了它，驗算全綠、帳本就少記那筆退款。所以上面那行「請確認」還是要請你自己看一眼。</li>
      <li>你現在看到的這一份，就是按下匯入會寫進去的內容——換卡重看也是同一份、不會再讀一次（重看用的是短效憑證，擺太久會過期；過期後請重新上傳預覽一次）。</li>
    </ul>
  </details>
</div>`;
}
