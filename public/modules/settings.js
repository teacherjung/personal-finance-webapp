 ...
 * 📌 完整的雲端語氣裁決仍待 William（含「雲端匯出**不含** IB 憑證與帳單密碼、還原後要重新輸入」
 *    那句怎麼講、要不要配 `.info-link` 詳解）——記在 `docs/文案審稿-雲端版的九句假話.md` 第 4 句那一段。
 *    ⚠️ 那一句**該補**（不講的話雲端使用者會以為「我有備份」），但它需要模式分流才寫得出來，
 *    不可以用一個含糊詞先蓋過去。
 * ⚠️ 誠實劃界：這句文案的**語意**沒有考題撐著（settings.js 的 DOM 路徑在 node 裡跑不起來）。
 *    有考題撐著的只有「不許出現宣稱內容完整的那幾個詞」這件事
 *    （`test/backup-export.test.js` 的〈文案｜備份卡的說明不可以宣稱「整包」〉）——那是字面黑名單，
 *    擋的是「下一個人（或我自己）把那個詞寫回去」，不是「這句話在兩種模式都成立」。
 */
