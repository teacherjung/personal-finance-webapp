...
import { cardLastFourText } from './card-last-four.js';   // 卡側末四碼只當顯示：{toString:null} 炸彈值炸不出＝不顯示（Codex #541 r4）
  ...
  openForm({
    ...
    fields: [
      { key: 'cardId', label: `${detail}，系統無法確定是哪張卡，請選：`, type: 'select',
        ...
        options: pick.map(c => { const lf = cardLastFourText(c.lastFour); return { value: c.id, label: c.name + (lf ? `（${lf}）` : '') }; }) }
    ],
...
function openStatementPreview(cardId, r, b64, cards, typedPw = '', onPage = () => true) {
  ...
  const cardOpts = () => cards.map(c => { const lf = cardLastFourText(c.lastFour); return `<option value="${esc(c.id)}" ${c.id === curCard ? 'selected' : ''}>${esc(c.name)}${lf ? `（${esc(lf)}）` : ''}</option>`; }).join('');
