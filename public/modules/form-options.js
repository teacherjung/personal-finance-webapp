// @ts-check
// 彈窗表單下拉選項的產生器（**零 DOM／零 API** 純模組，考題可以直接 import 跑行為級斷言）。
//（⚠️ 這裡原本寫「零依賴」，而本檔 import 了 `./html-escape.js`——#409 r6（2026-08-06）Codex 抓到的不實敘述。
//  「零依賴」在本 repo 有專門的意思：`theme.js`／`file-util.js` 那種**完全不 import 任何東西**的模組，
//  才可以放「頂層就會取用」的共用常數而不怕循環 import TDZ（AGENTS.md 地雷 2）。本檔不屬於那一類。）
//
...
// 對它們一點效果都沒有——除非那一處自己來呼叫本檔的函式。#409 自審抓到「分類」正是這一類，
// 而且其中兩處當時仍是同一個病（舊病，不是 #409 弄壞的）。現況逐處列名：
//
 ...
 *（#409 自審改過一次字：原本是「不在標準選項裡」——「標準」像術語，而且暗示「你的資料不標準」，
