...
function openBalanceAsOfInfo() {
  openInfo('餘額下面的日期是什麼？', `
    ...
    <p>寫著「上次 IB 同步…」的是<strong>券商同步來的現金帳戶</strong>（IBKR 不出這種對帳單，所以它們沒有對帳單可以匯）。</p>
    <p>那個日期是<strong>整次 IB 同步的時間，不是這一筆餘額的時間</strong>，兩個方向都可能對不上：如果那次同步沒拿到現金報表（IBKR 沒給、或幣別不支援），app 會<strong>保留原本的餘額不動</strong>、同步時間卻照樣往前走；反過來，你自己手動改過餘額的話，畫面上的數字會<strong>比這個日期新</strong>，而這裡看不出來。<strong>它只告訴你「上次連線是什麼時候」，不保證這個數字的新舊。</strong></p>
    <p>怎麼用它判斷：<strong>日期離今天越遠，這個餘額越可能已經跟實際數字對不上。</strong>對帳單類的帳戶，想更新就到「銀行收支」頁匯入一份最新的對帳單；<strong>IB 那幾個則是再跑一次 IB 同步</strong>（叫它們去匯對帳單是白跑一趟）。</p>
  `, { size: 'md' });
}
