...
function openBalanceAsOfInfo() {
  openInfo('餘額下面的日期是什麼？', `
    ...
    <p>寫著「上次 IB 同步…」的是<strong>券商同步來的現金帳戶</strong>：它們不靠對帳單，這種帳戶不需要、也沒有對帳單可以匯。</p>
    <p>那個日期是<strong>整次 IB 同步的時間，不是這一筆餘額的時間</strong>——差別很重要：如果那次同步沒拿到現金報表（IBKR 沒給、或幣別不支援），app 會<strong>保留原本的餘額不動</strong>，但同步時間照樣往前走。所以這個日期請當成<strong>上界</strong>：這個數字不會比它更新，但可能更舊。</p>
