...
// 上傳窗的文案（2026-08-12 William 指出原文「對帳單 PDF（台新綜合對帳單）」已過期——AI 路線存在的
// 理由正是不再侷限單一銀行）。⚠️ 誠實劃界：內建範本目前**確實只認得台新綜合對帳單**，所以不寫成
// 「支援各家銀行」；改成講清楚「認得什麼、認不得會怎樣」，使用者才知道那個 AI 同意窗為什麼會跳出來。
export const BANK_UPLOAD_FILE_LABEL = '對帳單 PDF';
export const BANK_UPLOAD_NOTICE = '選一份銀行寄給你的對帳單 PDF。目前<b>內建的讀取範本認得台新綜合對帳單</b>；換成其他銀行、或版面改版而認不出來時，會問你要不要交給 AI 讀一次（每一次都會先問過你）。';
