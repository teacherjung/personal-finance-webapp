...
// ⚠️ 這顆鈕要會說話的理由**不依賴任何一段指路文案**：畫面上有沒有一句話叫他先來按它，
//    都不改變「他按了、以為存好了、硬碟上那個 .json 其實是一則錯誤訊息」這件事。
//    指路文案的增減不是放寬下面三道關卡的理由。
//
...
// 3. **判準綁在「`settings` 與集合都放在頂層」這份契約上**。若 `/api/export` 的契約改成包一層
