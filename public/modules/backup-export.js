...
// 當初推動這一支的落點是**店名規則**那兩段文案（設定頁的卡片＋規則面板）：它們曾叫人「動手前先到
// 『資料與備份』按『匯出備份』存一份」（#410，2026-08-06 合併），自保路徑整條押在這顆鈕上，
// 所以「靜靜失敗」的後果被放大到「他在毫無安全網的狀態下按下不可逆的整批改名」。
// ⚠️ 那兩段指路**已隨 William 2026-08-08 的裁決一起移除**（拿掉「操作前自動備份」整層；
//    完整裁決見 `lib/services/backup.js` 檔中註解）——但這顆鈕要會說話的理由**不依賴那段指路**：
//    使用者仍然是靠它拿到唯一一份自己按得出來的備份。四處不可逆警告現行逐字＝
//    「儲存後沒有「復原」可以按」（釘在 `test/vault-and-backup-integrity.test.js` 的 ⭐ 裁決題）。
//
