...
export const XIRR_INFO_HTML = `
  ...
  <p><b>資料來源</b>：每月「記錄本月快照」的投入增量＝流出；IB 賣出的已實現損益逐筆按成交日計入（支援的外幣賣出缺 IBKR 匯率時以設定或預設匯率估算，不支援的幣別不計入；美元匯率本身是預設值時，進了現金流的賣出也算估算）；今日市值＝期末流入。口徑為台幣。</p>
  <p class="muted">限制：不含股息與利息（結果略為低估）；台股手動賣出的已實現損益未納入；快照為月頻、時點以月底近似；IB 交易紀錄僅涵蓋同步期間。外幣賣出缺 IBKR 匯率時以設定或預設匯率估算（標示「含匯率估算」）。歷史未滿 1 年時，年化會放大短期波動，僅供參考。</p>`;
...
/**
 * 頁首三張摘要卡。
 * @param {{total:number,totalCost:number,totalPnl:number,eqV:number,bondV:number,cashV:number,goldAll:number,allBase:number,leverage:number,netEquity:number,loanTwd:number,levCap:number,missingFx?:{currency:string,count:number,liabilities?:number}[],defaultFx?:{currency:string,count:number,rate:number}[]}} data
...
export function portfolioSummaryHtml(data, formatters) {
  const { total, totalCost, totalPnl, eqV, bondV, cashV, goldAll, allBase, leverage, netEquity, loanTwd, levCap, missingFx = [], defaultFx = [] } = data;
  ...
  </div>${fxNoteHtml(defaultFx, missingFx, formatters.escapeHtml)}`;
}

/**
 * 匯率註記（丙）：①用了預設匯率的部位＝照常計入、只提示（誤差通常不大）並告訴人怎麼抓即時匯率；
 * ②不支援的幣別＝無法換算、不計入，講清楚方向（負債被低估、淨值可能被高估）。幣別是資料值，一律經 escapeHtml。
 * @param {{currency:string, count:number, rate:number}[]} defaultFx
 * @param {{currency:string, count:number, liabilities?:number}[]} missingFx
 * @param {(value:any)=>string} escapeHtml
 */
export function fxNoteHtml(defaultFx, missingFx, escapeHtml) {
  let html = '';
  ...
  if (Array.isArray(missingFx) && missingFx.length) {
    ...
    const dir = liab > 0 ? `其中 ${liab} 筆是負債：負債被低估、淨值可能被高估。` : '';
    html += `<p class="${liab > 0 ? 'neg' : 'muted'} small" style="margin:-8px 0 16px">註：有 ${n} 筆部位的幣別（${curs}）系統不支援、無法換算，未計入上列金額；${dir}請把該帳戶／持股改成 TWD／USD／GBP／JPY。</p>`;
  }
  return html;
}
