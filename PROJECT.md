...
1. **重啟 → 03–06 四份對帳單逐份上傳＋確認匯入**（四份已用真檔驗證零阻擋；重複月不會重複入帳） — **William**
...
3. **個股研究頁（新主線，William 2026-07-24 指派 Codex）**：Codex 先交①衝突迴避方案②「個股研究頁施工計畫」（資料來源/畫面藍圖/評分方式/資料庫欄位/分階段 PR）給 William 裁決後才寫程式（**計畫已交＝#249 `docs/個股研究頁-施工計畫.md`，909 行，狀態：待 William 裁決**）。**衝突迴避基準**：用獨立新檔（前端新模組＋新路由檔）；注意既有 `public/modules/portfolio-research*.js` 與 `/api/research`（投組頁的研究筆記功能）——計畫需說明與它的關係（沿用/擴充/並存）；共用檔（app.js ROUTES、index.html 導覽、schema.js）只做最小增添，避免與 Claude 進行中的證券檔案衝突 — **Codex**
