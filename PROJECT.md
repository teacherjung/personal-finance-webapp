...
- **協作規矩凍結**（William 2026-09-11 裁「甲」、09-12 再裁三題）：本專案的協作規矩（AGENTS.md「協作流程」節、REVIEW-AND-MERGE.md、CLAUDE.md、COLLAB-MAP.md、PR 範本）**不再開修改它們的 PR**（加條、改字、整併都算）；可搬的新架構在 `teacherjung/ai-collab-kit`（私有倉庫）從零重建，成形後搬回本專案當第一個使用者、整份換掉。凍結的是「改規矩」不是「守規矩」：凍結期間一切照現行規矩執行；假綠、註解撐不住的保證、真的產線 bug 仍照「值不值得改」那三種情況修。新的協作教訓先記到 ai-collab-kit 的案例簿（同一個 GitHub 帳號，`gh` 讀得到），不在這裡加條文。
