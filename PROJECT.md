...
> 最後更新：2026-08-02（個股基本面 F5 完成；D3 瘦身後，本檔只留現在進行式）。
...
- **開著的工作**：#335 高風險複審剩四項 important 等 William 逐條裁決；#348 審查循環工具（Draft、待重工——分工文字改三模式＋rebase）；#342＋#346 堆疊組；#350 PDF 行程隔離；文件優化系列 D4（契約拆分，等 PR 隊列清空）／D5（工具政策）。
...
2. **#335 複審剩四項 important**（25MB 單一真相／OUTBOUND 絆索／tag 優先序／營收組成 vs 總額）逐條裁決。#341 的期間不對齊已依 William 裁決完成：合法值保留、payload 明示逐列期間、畫面逐列標示並警告不可橫向比較。主線範圍仍受 D0 定稿約束（`docs/個股基本面研究-施工計畫.md`，William 2026-07-28 五項裁決全採建議）：第一版只做美股 SEC 一般 US-GAAP 公司、八組研究問題、官方／衍生指標契約、六個 Chrome 式頁籤、F1–F6 分段 — **Codex 實作／William 裁決**
