...
- **協作規矩凍結**（William 2026-09-11 裁「甲」、09-12 再裁三題）：本專案的協作規矩（AGENTS.md「協作流程」節、REVIEW-AND-MERGE.md、CLAUDE.md、COLLAB-MAP.md、PR 範本）**不再開修改它們的 PR**（加條、改字、整併都算）；可搬的新架構在 `teacherjung/ai-collab-kit`（私有倉庫）從零重建，成形後搬回本專案當第一個使用者、整份換掉。**以下是 Claude 的操作化、不是他的原話**：凍結的是協作規則的演化、不是遵守——凍結期間一切照現行規矩執行，假綠、撐不住的保證、真的產線 bug 仍照 AGENTS「審查回饋處置」節「值不值得改」的判準修；新的協作流程教訓只當新架構的素材（記到 ai-collab-kit 的案例簿，同一個 GitHub 帳號、`gh` 讀得到），不在本專案形成新規矩；**技術慣例不在凍結範圍**（含「協作流程」節裡的技術驗證要求），仍照既有規定寫進 AGENTS.md。
