...
- **一步一 PR、三關全綠（test／typecheck／lint）、轉 ready 送審前對抗式自審**（開工第一步＝先開 Draft PR）；合併＝**William 裁決**，執行原則「**實作者不按自己的合併鍵**」＝由審查者執行（Codex↔Claude 對稱常設授權；William 隨時可自行執行或個案指定），一律走 審查與合併程序.md 合併五步驟（含堆疊閘；**決策與執行的完整規則在 AGENTS.md「協作流程」，本檔不另存副本**）；改後端要重啟。
