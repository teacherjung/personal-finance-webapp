...
1. **系統優化主線（Claude 實作、Codex 審核；已定稿＝`docs/系統優化-施工計畫.md`）**：階段一 U1 檔案轉碼器歸戶（施工中）→ U2 路由錯誤 helper → U3 彈窗外殼試點（securities 兩窗）→ U4 選配；階段二 大檔四刀（搬家不裝修＋前後端攤提公式對照考題）；階段三 考題與兩地圖更新；階段四（**等個股研究 P2 後**）本機 30 天滾動備份＋欄位長度分級上限。錯峰：階段一不碰 app.js；schema/types/crud.js 讓 P2 先行 — **Claude**
