...
測試現況：**755 題全綠**（node --test）＋ typecheck ＋ lint（三關＝pre-push 與 CI 門檻）。

## 待辦事項（依優先序）

1. **合併證券 r2 複審修正 PR**（五條全修＋A′ 落地：手續費幣別牆、A′ 文案、出清提醒指路、核心金額必填＋buy→out/sell→in 不變式、匯入時間本地化）＋**重啟** — **William**（實作 Claude ✅／複審 Codex）
2. **拿真實台新對帳單試匯入**（解析器目前只過合成考題與加密版面校準，真版面首戰）；有 blocker 訊息就原文回報 — **William**
