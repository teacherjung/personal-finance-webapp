...
1. **系統優化主線（Claude 實作、Codex 審核；定稿＝`docs/系統優化-施工計畫.md`）**：**階段一近收官**——U1 檔案轉碼器（#252 ✅）、U2 路由錯誤 helper（#253 ✅）、U3 外殼試點 securities 兩窗（#254 ✅）＋William 裁決擴大 11 窗＝設定頁 6 窗（#256 ✅，openRulePreview 刻意例外）＋信用卡 3 窗＋銀行收支 1 窗（**本 PR**；四窗原本皆無背景關閉＝外殼加 backdrop:false 原語意保留）、U4 月份選單（#255 ✅）。**下一步＝階段二大檔四刀**（搬家不裝修＋前後端攤提公式對照考題）；階段三 考題與兩地圖更新；階段四（**等個股研究 P2 後**）本機 30 天滾動備份＋欄位長度分級上限 — **Claude**
