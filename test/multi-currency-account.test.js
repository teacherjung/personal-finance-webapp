...
test('⚠️ 誠實殘餘｜配方**只被教過台幣區**時看不到外幣區＝那個混幣帳號會被當純台幣解出來（Codex #517 r4#1；**既有缺口、非本支引入**）', () => {
  ...
  //   ①②③ 三個修法方向都被實測打掉（漏放形狀 4–5 種、誤擋已實證且無退路、出生關治不到病），
  ...
  // ★裁示「甲」落地：看不到的那一列**被數到了**，但行為一格沒變（不拒解、不退 AI、不標疑似）
  assert.equal(p.unclaimedAccountRows, 1, '★配方沒認領的那一列帳號被計數（外幣區那列）');
  assert.equal(p.transactions.length, 1, '★行為不變：照舊解出 1 筆交易（計數不擋任何東西）');
  assert.equal(p.accounts.length, 1, '★行為不變：照舊只建 1 個帳戶');
...
test('⚠️ 誠實殘餘②｜**錨點吞併**：一個通用區段錨點同時命中台幣與外幣小標＝外幣區被當台幣區重新開張，每一列都「有被認領」⇒ 涵蓋率類的牆對它全盲（2026-08-26 實測，裁示「甲」照實釘住）', () => {
  // 為什麼這題必須存在：誠實殘餘①（上一題）講的是「配方**沒看到**」，本題講的是「配方**看到但歸錯**」。
  //   兩者都讓外幣以台幣入帳，但只有①會被「未認領的帳號列」這種判準量到——所以任何宣稱
  //   「數一數沒解到的列就補起來了」的修法，都會在本題這個形狀上發出假保證。
  ...
  assert.equal(p.unclaimedAccountRows, 0,
    '★★本題的重點：**一列都沒有「未認領」**——涵蓋率類的判準在這個形狀上完全看不到問題（宣稱「數沒解到的列就修好了」＝假保證）');
});
