...
test('明細｜右緣分不出欄時，界要用「中線」不是欄頭（大額支出不可翻成存入）', () => {
  // ⚠️ 註解：「金額右對齊，大額左緣會越過表頭左緣，用中線才不翻面」。
  // ⚠️ 考題設計（第一版是空包彈，Codex #408 r1 H① 抓到）：右緣必須**落出所有金額欄窗**才會走到
  //    中線退路。第一版用 x=290,w=6 ⇒ 右緣 296 正好落在支出欄 [272,331) ⇒ 主判斷就決定方向、
  //    退路那一行根本沒執行，把 mid 改成 xOut 照樣綠。
  //    這一版 x=290,w=110 ⇒ 右緣 400 落在所有欄窗之外（左緣 290 < xBal 396 ⇒ 也不是餘額欄）
  //    ⇒ 真的走退路；左緣 290 在 xOut(272) 與中線(301.5) **之間**：
  //      界＝中線 → out（正確）；界＝xOut → in（支出被算成收入）。
  const lines = [
    HEAD,
    DW(100, [[53, 0, '900200****3302'], [124, 0, '2026/06/13'], [177, 0, '媒體轉出'],
      [290, 110, '$1,615,555'], [400, 30, '$50,000']]),
  ];
  const txs = parseBankDetail(lines);
  assert.equal(txs.length, 1);
  assert.equal(txs[0].amount, 1615555);
  assert.equal(txs[0].direction, 'out',
    '左緣越過支出欄表頭、但還在中線左側＝仍是支出（界改成欄頭就會翻成收入）');
});

test('明細｜只有支票號碼、沒有交易金額的列 → 一筆都不可產生（票號不是金額）', () => {
  // ⚠️ 這一題對應一個**真 bug**（Codex #408 r1 H② 實測、本 PR 同時修掉）：
  //    主判斷（右緣分欄）有排除支票欄，但「右緣分不出欄」的退路原本從 xChk 起掃**未過濾的**格子
  //    ⇒ 只有票號的列會把票號當成金額 ⇒ 實測票號 0123456 變成**憑空的 123,456 元支出**。
  // ⚠️ 考題設計：fixture **刻意不放真正的金額**——第一版同時放了 8,500，於是即使刪掉排除，
  //    真金額照樣被選中、考題仍綠（那顆是空包彈）。
  // ⚠️ 誠實劃界（做突變驗證時發現的）：**主判斷裡那道支票欄 `continue` 其實是冗餘的**——
  //    支票欄窗 [xChk, xOut) 整段在 xOut 之下，那些格子本來就落不進「支出」或「存入」窗，
  //    刪掉它結果不變（本題照樣綠）。真正承重的是**退路**那道排除（今天補的那行）：
  //    刪掉它本題就會紅。主判斷那道留著當防禦性冗餘沒問題，但不要以為它是這裡的守衛。
  const lines = [
    HEAD,
    DW(100, [[53, 0, '900100****3301'], [124, 0, '2026/06/11'], [177, 0, '票據交換'],
      [244, 20, '0123456'], [400, 30, '$41,500']]),   // 票號＋餘額，沒有交易金額
  ];
  assert.deepEqual(parseBankDetail(lines), [],
    '這一列沒有交易金額，必須整列跳過——把票號當金額＝帳上憑空多一筆支出');
});

test('明細｜票號與真金額同列時要抓真金額（不可抓成票號）', () => {
  const lines = [
    HEAD,
    DW(100, [[53, 0, '900100****3301'], [124, 0, '2026/06/12'], [177, 0, '票據交換'],
      [244, 20, '0123456'], [300, 25, '$8,500'], [400, 30, '$33,000']]),
  ];
  const txs = parseBankDetail(lines);
  assert.equal(txs.length, 1);
