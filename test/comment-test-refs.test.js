 ...
 * ⚠️ 每個細節都是踩出來的：
 *   ・允許純空白關鍵字時，一個空白剛好命中而放行。
 *   ・**關鍵字**允許跨行時，兩段不相干的文字會被配成一個「合法」路標。
 *   ・`記號：「…」`（帶冒號）是很自然的寫法，不認它就等於整條路標靜靜消失。
 *   ・⚠️ **記號與左引號之間允許折一次行**（後面接 `//` 或 `*` 的續行）：中文長註解本來就會折，
 *     而不認它的後果最陰——那條路標**連「壞掉」都不算，是完全看不見**（#470 r6 實測：
 *     全樹 11 條路標裡有 2 條就是這樣折的，本閘一條都沒掃到）。
 *     ⚠️ 只允許**一次**換行、且續行必須帶註解符號：放寬成任意空白就會把記號和很遠的一個引號配起來。
 */
const REF_RE = () => new RegExp(`${MARKER}[:：]?[ \\t\\u3000]*(?:\\n[ \\t]*(?://|[*])[ \\t]*)?[「]([^」\\n]+)[」]`, 'g');
/** 開了引號但沒有合法收尾＝**壞掉的路標**（見 `scanRefs` 的理由）。 */
const OPEN_RE = () => new RegExp(`${MARKER}[:：]?[ \\t\\u3000]*(?:\\n[ \\t]*(?://|[*])[ \\t]*)?[「]`, 'g');
...
// 除了那顆還掃到別的＝真的有問題。
//
// ⚠️ **「掃的是真的那些檔案」守到什麼程度，逐條寫清楚**（#470 r2–r6 在這裡中刀四輪，
//    每補一層自我證明就被找到繞過那一層的突變）：
//   ・**誘餌**守「掃描有在跑、而且掃得到壞東西」。
//   ・題名關鍵字「來源真的是那些檔案」那題（獨立對照）守「內容各不相同、且真的來自磁碟」
//     ——擋得住「所有檔名配同一份內容」這一族。
//   ・**擋不住**「呼叫端把真實來源整個換掉、只傳誘餌」：那要靠函式自報處理了什麼，
//     而自報會被同一顆突變一起改掉（r5 實測，複驗者也不建議加回）。
//     ⚠️ 我上一版把這句寫成「**結構上**不可能證明」——**那是誇大**（#470 r6 複驗者指出
//     可用獨立對照）。改口：本檔沒有採用能擋住那一族的做法，理由是它的成本高於收益。
//   ⇒ 那一族由**外部**證據補：PR 內文記錄的突變測試（改壞實作 → 這幾題轉紅）。

test('⭐ 來源真的是那些檔案（獨立對照：內容各不相同、且讀得到磁碟上的特徵）', () => {
  // ⚠️ 這是 #470 r6 複驗者指出的做法：不必靠「函式自報」，用**與掃描無關的性質**去對照。
  //    擋得住的突變：把所有檔名配同一份內容（r5 實測那一顆）。
  const sources = realSources();
  assert.ok(sources.length > 50, `只列到 ${sources.length} 支考題檔，列舉大概壞了`);
  assert.equal(new Set(sources.map((x) => x.source)).size, sources.length,
    '有兩支以上的來源內容完全相同 ⇒ 讀檔那一段可能被換成「同一份內容配所有檔名」');
  // 讀得到磁碟上的特徵：本檔自己一定含得到記號的變數宣告那一行
  const self = sources.find((x) => x.name === 'test/comment-test-refs.test.js');
  assert.ok(self && self.source.includes("const MARKER = '"),
    '本檔自己的內容不在來源清單裡 ⇒ 讀檔那一段沒有真的讀磁碟');
});

test('⭐ 記號與左引號之間折行也要掃得到（中文長註解本來就會折）', () => {
  // ⚠️ #470 r6 實測：全樹 11 條路標裡有 2 條就是這樣折的，而上一版**一條都沒掃到**
  //    ——不是判錯，是連「這裡有個壞路標」都沒說。這是最陰的漏法。
  const wrapped = `test('折行目標題名', () => {});\n// 那一族由同檔題名關鍵字\n// 「折行目標題名」那題守。`;
  assert.deepEqual(scanRefs([{ name: 'f', source: wrapped }]), [], '折行的路標明明指得到卻被判違規');

  const wrappedBad = `test('毫不相干', () => {});\n// 由同檔題名關鍵字\n// 「這段字找不到」那題守。`;
  assert.equal(scanRefs([{ name: 'f', source: wrappedBad }]).length, 1,
    '折行的路標指不到東西卻沒被抓 ⇒ 這種寫法整族是本閘的盲區');

  // 反面：只允許折**一次**行、且續行要帶註解符號。隔了空行就**不算路標**（跟把記號當名詞
  // 提到同一類）——這是保守的那一邊：寧可不認，也不要把記號和很遠的一個引號配起來。
  // ⚠️ 代價寫在檔頭劃界：真的隔著空行寫路標的人，那條不會被檢查。
  const tooFar = `test('x', () => {});\n// ${MARKER}\n\n// 「隔了空行的東西」`;
  assert.deepEqual(scanRefs([{ name: 'f', source: tooFar }]), [],
    '隔了空行竟然被配成路標 ⇒ 判準太鬆，會把不相干的引號配進來');
});
