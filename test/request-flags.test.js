...
//
// 守兩層，射程各自寫清楚：
//   ①行為題：這一份守這些開關（整批傳播、還原自動判斷、清品牌規則、清空全部略過、預覽），各一題——
...
test('形狀題：lib/routes 底下這一族開關一律 === true／!== true 讀（解析器判定，註解裡寫什麼都不算數）', () => {
  ...
  // 用另一套機制（Node 自己的遞迴列目錄）算同一份集合來對帳：掃到的檔案少了一支時，
  // 逐名與反向對帳都看不出來（沒被讀到的檔本來就不會產生發現），這一行才是對得出差異的那道。
  // ⚠️ 這一行對帳的是「集合一不一樣」，不是「遞迴有沒有在遞」——路由目錄若沒有子目錄，
  //    兩邊都不會遞迴、也就都相等。遞迴本身由題名關鍵字「掃描集合的遞迴」那題用臨時目錄直接驗。
