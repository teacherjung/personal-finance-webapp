...
// ⚠️ 題名沿革（r11）：原本叫「新文案指到的那兩顆自救按鈕，必須真的在同一頁上」——William 把分類那兩段的
//    指路刪掉之後，那個題名就跟內容走散了（它現在守的是三件事，指路只是其中一件）。題名誇大在本專案算阻擋級，
//    所以一起改對。
test('畫面文案｜分類管理留著「無法復原」警告、不再承諾備份，而文案指到的自救出口必須真的在', () => {
  ...
  for (const btnId of ['manageCatsBtn', 'manageIncomeCatsBtn']) {
    ...
    // ⚠️ r11（William 2026-08-05 第二次裁決）：分類管理這兩段**不再指路**，只留「⚠️ 儲存後無法復原。」
    //    ——他的判斷是那一長串太囉嗦。所以這裡從「必須指到那三個名字」改成**必須留著那句警告**：
    //    警告是這兩段文案唯一的承重內容，被靜靜刪掉的話使用者就在毫無提示下按下不可逆的改名／刪除。
    //    （指路那一半改由下面的 POINTERS 那一題守——它以「哪些文案真的指路」為準，不寫死名單。）
    assert.match(stripTags(copy), /無法復原|不能復原|沒有「?復原/,
      `${btnId} 的說明文案必須留著「儲存後無法復原」這類警告——`
      + '分類改名會改寫全庫交易的分類、刪除會把交易改歸「其他／未分類」，兩者都沒有復原鈕，'
      + '而使用者是在按下去之前讀到這句話的');
  ...
  // ④ 指路面（r11 重寫）：**以「哪些文案真的指路」為準，不寫死名單。**
  //    r10 版寫死了三個名字（資料與備份／匯出備份／匯入備份），William 把分類那兩段的指路刪掉之後
  //    那份名單就跟畫面走散了——寫死清單自己會漂，這是本專案記過的教訓。
  //    改成：掃過設定頁與店名規則面板的**所有**文案，凡是被「」引用到的自救出口名稱，
  //    就必須真的有對應的 UI 元件、而且在整頁最後一個區塊底下。
  //    ⚠️ 反面守衛：至少要有一段文案在指路（否則整頁沒人告訴使用者怎麼自保，本題也會變空轉）。
  const panelSrc = readFileSync(join(ROOT, 'public/modules/settings-store-rules.js'), 'utf8');
  const allCopy = stripTags(src) + '\n' + stripTags(panelSrc);
  /** 自救出口的名字 → 在 settings.js 裡怎麼找到它的實體。 @type {[string, () => number, string][]} */
  const EXITS = [
    ['資料與備份', () => lastSection, '整頁最後一個區塊的標題'],
    ['匯出備份', () => src.indexOf('href="/api/export"'), '匯出按鈕'],
    ['匯入備份', () => src.indexOf('id="importBtn"'), '匯入按鈕'],
  ];
  let pointed = 0;
  for (const [name, find, what] of EXITS) {
    if (!allCopy.includes(`「${name}」`)) continue;          // 沒有文案指到它，就沒有要守的承諾
    pointed++;
    const at = find();
    assert.ok(at > 0, `有文案叫使用者按「${name}」，但${what}在檔案裡找不到——那句指路是假的`);
    assert.ok(at >= lastSection,
      `「${name}」必須在整頁最後一個區塊（「資料與備份」）底下——文案寫「到本頁最下面」`);
  }
  assert.ok(pointed >= 2,
    '整個設定頁與店名規則面板，竟然沒有兩段以上的文案告訴使用者自救出口在哪'
    + `（實際只找到 ${pointed} 個被引用的出口）——本題會因此變成空轉，`
    + '請先確認是不是有人把指路的句子整批刪掉了');
});
