...
//    一併把題名改對——題名若引用畫面未出現的逐字文案，讀題的人會被誤導去找一句不存在的話。
test('畫面文案｜分類管理只留「沒有復原可以按」的警告、不承諾備份，而文案叫人按的名字都要有那顆鈕', () => {
  ...
  assert.ok(pointingParas >= 1,
    ...
    + '釘死段數只會逼維護者為了過關去湊句子。掃到零段代表指路文案整批消失，'
...
test('畫面文案｜任何「系統會替你留一份」的說法都要交代哪一種模式才有（掃遍 public/）', () => {
  ...
  //    契約），其餘句子仍照本題的規矩自己交代（見 docs/hosted-copy-review-nine-false-claims.md 附註）。
...
// 而那正是刻意不要的東西。只寫在註解裡擋不住任何人，所以這裡把「不得出現」變成會轉紅的斷言。
// 使用者的救援手段另有其人（有哪些、各自保護到哪，單一真相＝lib/services/backup.js 檔頭的分工段）。
test('⭐ 裁決｜店名規則與開 app 自動整理這兩條路，不得長出「操作前自動備份」（刻意不做的，不是漏掉的）', () => {
  ...
  //    「儲存後無法復原」——使用者自己匯出的備份檔可以經 /api/import 整包匯回去，那句話比事實講得滿。
...
//    ⚠️ 寫這種「都有考題」的句子之前先逐支點過，判準＝**把那一支的備份整段刪掉，有沒有題轉紅**。
//       逐支檢查不可省：若③一條斷言也沒有，刪掉它的備份會全綠，而這段話仍宣稱它有考題
//       ＝誇大比缺口更糟。
//    ③④ 的**失敗路徑沒有考題**：要重現得先造出「還沒搬過家、且剛好 VACUUM 失敗」的庫，
//    而兩支都有 meta 鍵守著只跑一次；判斷投入產出不划算，列為**已知缺口**——
//    不寫「做不到」，因為本節那種長檔名手法同樣適用。
//    ⚠️ 把這幾份各自獨立的實作逐支列在這裡，是為了讓維護者收斂它們時，知道①②有安全網、③④沒有。
// ─────────────────────────────────────────────────────────────────────────────
