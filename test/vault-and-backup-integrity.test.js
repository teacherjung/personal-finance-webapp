...
//    這句指路的可信度曾**依賴匯出鈕會回報成功／失敗**——#417 已補上，而且 William 2026-08-08 拍板
//    把那句指路整句拿掉了（畫面只留「儲存後無法復原」）。以下敘述留作沿革。〔原文：由 `fix/export-backup-feedback` 提供；
...
test('畫面文案｜任何「系統會替你留一份」的說法都要交代哪一種模式才有（掃遍 public/；⚠️ 2026-08-08 起前端只拿得到匯出告知專用的那一個布林，其餘句子仍要自己交代）', () => {
  ...
  // ⚠️ **2026-08-08 兩處訂正**（#422 與 #417 各改了一件事，這一題的前提都變了）：
  //    ①那兩句話已經**整句不存在**——William 拍板拿掉「操作前自動備份」整層，畫面只寫
  //      「儲存後無法復原」，所以本題掃不到它們不是缺口（門檻已隨之調整，見下方空轉守衛）。
