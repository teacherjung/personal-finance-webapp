...
//      改成文案修正，不走備份——見下方誠實劃界。本節的**備份題**只守店名規則這一條路
//      （分類管理只守文案，見本節第二題）。）
//
...
//      而 saveTree／saveIncomeTree 的改名會改寫全庫交易的分類、刪除大類會把那些交易改歸**各自的退路**
//      ——支出＝「其他／未分類」（lib/services/categories.js:249 `conform`）、收入＝「其他／其他收入」
//      （同檔 :252 `conformIncome`，收入樹裡沒有「未分類」這個子類）。
...
//      ⚠️ 改後的文案沿革（**現況只有一句警告**，不要再引 r8／r9 那兩版）：
//        r8＝「改動前先重啟一次後端，每次啟動都會另存 data/store.db.bak」⇒ 自己又是一句假話
//            （settings.js 兩種模式共用、沒有分流，`lib/repo.js:252` 的 `backupNow` 在雲端一律回 false
//            ⇒ 那顆檔根本不存在；本機版也只是 best-effort，VACUUM 失敗只寫後端 console.warn）。
//        r9／r10＝改成指向兩種模式都有的「匯出備份／匯入備份」按鈕。
//        r11（**現況**）＝William 第二次裁決「太囉嗦」，分類管理兩段**只留一句警告**、不再指路。
//        r6（本輪）＝那句警告從「儲存後無法復原」改成「儲存後沒有『復原』可以按」
...
//    這句指路的可信度就懸在那裡。⚠️ 訂正：#417（本支）已補上——見 `public/modules/settings.js` 的
//    `exportBtn.onclick`（`ev.preventDefault()` → `runExport`，實作在 `public/modules/backup-export.js`）。
//    ⚠️ 至於那句指路本身：拿掉它是 **#422（另一支，尚未合併）** 的事——**在這棵樹上那兩段指路仍然
//    存在**（public/modules/settings.js:121 的店名規則卡片＋public/modules/settings-store-rules.js:100
//    的面板），本檔那顆空轉門檻 `pointingParas >= 2` 也還在要求兩段（兩者一致）。
//    ⚠️ 別把別支的改動寫成這棵樹的事實（r9／r11 各抓到一次，同一個病）。
// ─────────────────────────────────────────────────────────────────────────────

test('店名規則｜備份必須是「這次操作之前」的狀態（不是操作後、也不是永遠停在第一次）', async () => {
  // ⚠️ UI **原本**寫著「儲存前自動備份。」；r10 起改成 public/modules/settings-store-rules.js:100 的
  //    「別只靠系統的自動備份（只有<b>本機版</b>才有那一份，而且失敗了畫面不會講）」。啟動備份 .bak
  //    每個行程只寫一次，對「一天內改好幾次規則」毫無保護力＝空頭支票；這顆獨立的 pre-rules 備份
  //    才讓**現行文案「只有本機版才有那一份」那半句**是真的。
...
test('畫面文案｜分類管理只留「沒有復原可以按」的警告、不再承諾備份，而文案叫人按的名字都要有那顆鈕', () => {
  ...
  //    本題不重複、也不宣稱守到。
  //    ⚠️ 那句**指路本身在這棵樹上仍然存在**：拿掉它是 #422 的事，而 #422 尚未合併
  //    ——本題末尾的 `pointingParas >= 2` 也還在強制要求兩段指路，兩者一致。
  ...
  assert.ok(pointingParas >= 2,
    '整個設定頁與店名規則面板，竟然只有不到兩段文案在告訴使用者自救出口怎麼按'
    + `（實際找到 ${pointingParas} 段）——正常應有兩段：設定頁的「店名規則」卡片與店名規則編輯窗，`
    + '兩者都寫「動手前請先…按「匯出備份」存一份」。'
    + '分類管理那兩段自 r11 起刻意不指路（William 裁決：太囉嗦），所以不算在內。'
    + '掃不到就代表有人把指路的句子刪掉了，或 `<p>` 段落的抓法失效＝本題在空轉');
});
...
test('畫面文案｜任何「系統會替你留一份」的說法都要交代哪一種模式才有（掃遍 public/；⚠️ 2026-08-08 起前端只拿得到匯出告知專用的那一個布林，其餘句子仍要自己交代）', () => {
  ...
  //    ①那兩句話**在這棵樹上仍然存在**：William 拍板拿掉「操作前自動備份」整層是 **#422
  //      （另一支，尚未合併）** 的事；等它合併後畫面才會只寫
  //      「儲存後無法復原」——**屆時**本題掃不到它們才不是缺口（那支自己會調整門檻）。
  //      在本樹：兩段承諾仍在，本檔下方的空轉守衛也還在要求它們（兩者一致）。
  ...
  assert.ok(flagged.size >= 3,
    `前置：只掃到 ${flagged.size} 段「系統會替你留一份」的文案——`
    + '正常有四段：設定頁的店名規則卡片（還原檔＋.bak）、店名規則編輯窗（自動備份）、'
    + '每日備份警告 modules/backup-alert.js 兩段（回報失敗的那段放行，但仍算掃到）。'
    + '⚠️ 分類管理那兩段自 r11 起只剩一句不可逆警告、**不再提備份**，所以本來就掃不到（不是缺口）。'
    + '數字掉下來就代表觸發字或掃描範圍被改壞了＝這一題在空轉');
});
