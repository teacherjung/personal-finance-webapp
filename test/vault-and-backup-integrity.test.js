...
//
// ⚠️ **跨支依賴（合併順序看得出來）**：第二節那幾題守的文案，把使用者的自保押在設定頁
//    「資料與備份」的〈匯出備份〉那顆鈕上。而那顆鈕目前是純 `<a href="/api/export" download>`、
//    **沒有成功／失敗回饋**（雲端版 session 過期時，瀏覽器會安靜地存下一個內容是錯誤訊息的檔案，
//    使用者以為自己有備份了）。William 2026-08-05 裁決「不用文案打補丁，另開一支真的修」，
//    那一支就是**本支 #417**：鈕已改成「先驗再存」＋七句提示＋匯出前的告知窗（見 public/modules/backup-export.js）。
//    ⚠️ 所以上面那句「目前是純 `<a href>`、沒有回饋」講的是**改之前**的狀態，留著是沿革。
//    ⇒ 在它合併之前，「動手前先按〈匯出備份〉存一份」這句指路的**保護力有缺口**：
//      本檔只證明得出「那顆鈕在、名字對得上、按下去匯出的內容含分類樹」，
//      **證明不出**「使用者真的拿到一份完整的檔案」。這一點本檔任何一題都不宣稱守到。
//
...
// ⚠️ 同族的第三個缺口＝**跨支依賴，合併順序有意義**（#410 r6）：店名規則那兩段文案叫使用者
//    「動手前先按〈匯出備份〉存一份」，而那顆鈕是純 `<a href="/api/export" download>`、
//    **沒有成功／失敗回饋**（雲端版 session 過期時瀏覽器會安靜存下一個內容是錯誤訊息的檔案）。
//    這句指路的可信度曾**依賴匯出鈕會回報成功／失敗**——#417 已補上。
  //    ⚠️ 至於那句指路本身：拿掉它是 **#422（另一支，尚未合併）** 的事——
  //    **在這棵樹上那兩段指路仍然存在**，本檔 :585 一帶的空轉門檻也還在要求兩段（兩者一致）。
  //    ⚠️ 別把別支的改動寫成這棵樹的事實（r9／r11 各抓到一次，同一個病）。以下敘述留作沿革。〔原文：由 `fix/export-backup-feedback` 提供；
//    **在它合併之前，這句話的保護力有缺口**——本節證明得出「鈕在、名字對得上、匯出的內容含分類樹」，
//    證明不出「使用者真的拿到一份完整的檔案」。William 2026-08-05 裁決：不用文案打補丁、另開一支真的修，
//    所以那一輪**不改這句文案、也不碰那顆鈕**〕——#417（本支）已修好那顆鈕。
// ─────────────────────────────────────────────────────────────────────────────
...
test('畫面文案｜分類管理只留「沒有復原可以按」的警告、不再承諾備份，而文案叫人按的名字都要有那顆鈕', () => {
  ...
  //    ⚠️ 更關鍵的一塊仍在射程外（見檔頭的跨支依賴）：〈匯出備份〉那顆鈕**沒有成功／失敗回饋**，
  //    所以「照文案按下去就有一份備份」這件事本題證明不出來——那由 `fix/export-backup-feedback` 修，
  //    ⚠️ 2026-08-08 訂正：那一支就是本支（#417），鈕已修好。
  //    ⚠️ 那句**指路本身在這棵樹上仍然存在**：拿掉它是 #422 的事，而 #422 尚未合併
  //    ——本檔 :435 一帶的斷言也還在強制要求兩段指路，兩者一致。
  ...
  for (const btnId of ['manageCatsBtn', 'manageIncomeCatsBtn']) {
    ...
    for (const m of plain.matchAll(/自動備份/g)) {
      // ⚠️ 「被否定了嗎」交給 negatesClaim 這**唯一一份實作**判（下一題的掃描器共用同一顆，
  ...
  //      那幾段目前沒有指路句子；有人在那裡寫假指路，本題抓不到（文字掃描擋不完，見下一題的收手判準）。
...
test('匯出備份｜雲端版匯出的檔案裡必須有分類樹（店名規則文案叫人按的就是這顆按鈕）', () => {
  ...
  //    ⚠️ 也**不**宣稱「使用者真的拿到那份檔案」：那顆鈕是純 `<a download>`、沒有成功／失敗回饋
  //    （雲端版 session 過期時會安靜存下一個內容是錯誤訊息的檔案）。那一塊由
  //    #417（本支）修好了：三道關卡＋七句提示＋告知窗。以下沿革（見檔頭）。
  const trees = {
