...
//    **不**證明「使用者真的拿到一份完整的檔案」——那一塊在**本樹**由 `test/backup-export.test.js`
//    的行為題守著（先驗再存的關卡／落檔／出聲／接線真的走 public/modules/backup-export.js），
//    本檔不重複、也不宣稱守到。
//    〔沿革：那顆鈕原本是純 `<a href="/api/export" download>`、成功失敗都不出聲（雲端版 session
//     過期時，瀏覽器會安靜地存下一個內容是錯誤訊息的檔案，使用者以為自己有備份了）。
//     William 2026-08-05 裁決「不用文案打補丁，另開一支真的修」（落點：docs/文案審稿-雲端版的九句假話.md:205），
//     那一支就是本支 #417——見 `public/modules/settings.js` 的 `exportBtn.onclick`（`ev.preventDefault()` → `runExport`）。〕
//
...
test('寫入櫃檯｜**每一個**集合、**每一種**非陣列形狀都要 fail-loud（不是只有 transactions、也不是只有 {}）', () => {
  ...
  //       而 null/undefined 正是 JS 裡最可能誤傳的非陣列：服務層早退回傳 undefined、整包寫入自己拼物件。
  //       ⚠️ 攔截拓樸要講清楚（別把它講反）：`replaceCollection`（lib/repo.js:222）文件明寫「本函式不再
  //       驗證」，但它目前**唯一的呼叫端**（lib/routes/crud.js:120 `if (!Array.isArray(list)) … 400`，
  //       再往下傳的是本地新建的陣列）自己就擋掉非陣列 body，`/api/import` 也有 lib/routes/core.js:193-194
  //       的 400。所以本題守的**不是**那兩條路由，而是「繞過路由層直接寫入」那一類——在寫入管線裡
...
//
// ⚠️ 本節 2026-08-08 縮編過：原本前半是「店名規則的『儲存前自動備份』不是空頭支票」那五題
//    （守 `pre-rules`／`pre-normalize` 兩顆備份與 08-06 的「存不成就擋下＋問使用者」機制）。
//    William 裁決把那一整層拿掉之後，受測對象不存在了，那五題與它們的專用 helper 一起刪除
//    ——**不是被弱化，是它守的東西被拍板移除**（完整理由見 lib/services/backup.js 的裁決註解）。
//    留下來的是**文案面**這幾題，外加本節末尾那顆 ⭐ 裁決題（擋「好心把備份加回來」）。
...
//    ——William 把分類那兩段的指路刪掉之後題名就跟內容走散；r11 改成「留著『無法復原』警告」，
//    而 r6 把那句文案收窄成「沒有『復原』可以按」（見下方 ③），題名裡的引號又過期了。
//    題名誇大／過期在本專案算阻擋級，所以每次都一起改對。
test('畫面文案｜分類管理只留「沒有復原可以按」的警告、不再承諾備份，而文案叫人按的名字都要有那顆鈕', () => {
  // ⚠️ 為什麼這一題存在：William 拍板「分類管理不補備份、只改文案」之後，改出來的文案自己又是承諾，
  //    而**承諾要有考題撐著**（專案鐵則）。現在這一題守的是三件事＋一道反面守衛：
  ...
  //    William 審改措辭時不會被綁住。反過來，若有人把按鈕改名／搬走／刪掉，或把文案改回指向
  ...
  //    就有一份備份」由 test/backup-export.test.js 的 runExport 行為族守（本樹已把那顆鈕改成「先驗再存」
  //    ＋按下就出聲，見 `public/modules/settings.js` 的 `exportBtn.onclick` 與 `public/modules/backup-export.js` 的 `runExport`）——
  //    本題不重複、也不宣稱守到。
  //    ⚠️ 那句指路**已經不在了**（本支 #422）：William 2026-08-08 拿掉「操作前自動備份」整層，
  //    店名規則卡片與編輯窗那兩段「動手前先按〈匯出備份〉」連同備份承諾一起移除
  //    ——本題末尾的空轉門檻因此從 `>= 2` 降成 `>= 1`（現在唯一一段是編輯窗的「先按〈預覽影響〉」），
  //    兩者一致。〔沿革：這段註解在 main 上寫的是「#422 尚未合併、門檻還要求兩段」，rebase 後就過期了。〕
  ...
  // ③ 兩段分類管理文案：那句「沒有『復原』可以按」的警告不可被靜靜刪掉，而且**不可以再承諾任何備份、
  //    也不可以指向本機專屬的東西**（自 r11 起這兩段刻意**不指路**——見下方 r11 的裁決紀錄；指路面
  //    由 ④ 以「哪些文案真的在指路」為準守著，不寫死名單）
  ...
  assert.ok(pointingParas >= 1,
    ...
    + '⚠️ 這個門檻 2026-08-08 從「兩段」降成「一段」：William 裁決拿掉「操作前自動備份」整層之後，'
    + '設定頁店名規則卡片與編輯窗**都不再叫人先按〈匯出備份〉**（那句指路連同備份承諾一起移除）。'
    + '現在唯一一段是編輯窗的「先按〈預覽影響〉再儲存」。'
    + '掃不到就代表連那句也被刪了，或 `<p>` 段落的抓法失效');
});

// ⚠️ 題名沿革（#410 r6）：原本叫「分類管理｜文案叫人按的自救路徑…」——**分類管理的文案自 r11 起
//    已經完全不指路**（William 裁決只留一句警告），題名的主詞就過期了。指路的句子現在只剩店名規則
//    兩處，而它們指的正是這顆〈匯出備份〉；本題守的是「按下去匯出來的檔案裡有沒有分類樹」，
//    所以題名改以那顆按鈕為主詞。（改壞分類之後的唯一自救路徑仍然是這一條，射程沒有變。）
test('匯出備份｜雲端版匯出的檔案裡必須有分類樹（店名規則文案叫人按的就是這顆按鈕）', () => {
  ...
  //    本題只釘「匯出的檔案裡有分類樹」。
  //    ⚠️ 也**不**宣稱「使用者真的拿到那份檔案」——那是 public/modules/backup-export.js 誠實劃界第 6 點：
  //    `<a>` 這條路收不到落地回音。至於「按下去不出聲」那一塊，#417（本支）已修好：先驗再存的關卡
  //    ＋按下就出聲＋匯出前告知窗。
  //    〔改之前的沿革：那顆鈕是純 `<a download>`、沒有成功／失敗回饋，雲端版 session 過期時會安靜存下
  //    一個內容是錯誤訊息的檔案。〕
  const trees = {
...
test('畫面文案｜任何「系統會替你留一份」的說法都要交代哪一種模式才有（掃遍 public/；⚠️ 2026-08-08 起前端只拿得到匯出告知專用的那一個布林，其餘句子仍要自己交代）', () => {
  // ⚠️ 這一題關的是整族的門（#410 r9 複驗：「同一頁還有一句沒改的同族假話」）。r9 只改了分類管理那兩處，
  //    同一頁的店名規則卡片還寫著「（套用前會另存一份還原檔 data/store.db.pre-rules.bak）」、
  //    店名規則面板還寫著「儲存前自動備份。」——兩句在**雲端版都是假的**（`lib/repo.js` 的
  //    `backupNow` 在 HOSTED 一律回 false），而 `public/` 底下 grep `hosted` 零命中＝前端根本不知道
  //    自己跑在哪一種模式，同一份 HTML 兩邊共用。r10 把兩句都改成「交代模式」的寫法。
  // ⚠️ **2026-08-08 兩處訂正**（一件已在本樹生效、一件還沒——分清楚）：
  //    ①那兩句話**已經不在了**：William 拍板拿掉「操作前自動備份」整層＝本支 #422，
  //      畫面上現在只寫「儲存後沒有「復原」可以按」，不承諾任何自動還原檔。
  //      所以本題掃不到它們**不是缺口**，本檔下方的空轉守衛也隨之從「三段」降成「兩段」（兩者一致）。
  //      〔沿革：這段註解在 main 上寫的是「#422 尚未合併、兩段承諾仍在」，rebase 後就過期了。〕
  //    ②「`public/` grep hosted 零命中＝前端不知道模式」**也不再成立**：#417 加了唯讀端點
  //      `GET /api/mode`，匯出前的告知窗依模式講真話（見 docs/contracts/cloud-security.md
  //      「匯出前告知的模式分流」）。本題守的仍是「**沒有模式資訊的那些句子**不可以承諾備份」。
  // ⚠️ 為什麼是掃描題而不是逐處斷言：這一族已經被抓到四輪（r5 劃界、r8 文案自己又說謊、r9 漏了同頁兩處、
  //    r6 三個明確假承諾照樣通過），「列舉繞法補不完就要關門」（專案模式）。
  //
  ...
  //    不是窮舉；至於「把模式資訊送到前端」——#417 已經做了**最小版**（一個布林，只給匯出前
  //    那句告知用；不得擴張，見 cloud-security 契約），其餘句子仍照本題的規矩自己交代（見 docs/文案審稿-雲端版的九句假話.md 附註）。
  ...
  assert.ok(flagged.size >= 2,
    ...
    + '正常有兩段：每日備份警告 modules/backup-alert.js（回報失敗的那段放行，但仍算掃到）。'
    + '⚠️ 這個門檻 2026-08-08 從「三段」降成「兩段」：William 裁決拿掉「操作前自動備份」整層，'
    + '設定頁店名規則卡片（還原檔＋.bak）與編輯窗（自動備份）那兩段承諾**已整句移除**'
    + '（分類管理那兩段更早於 r11 就只剩不可逆警告）。所以掃不到不是缺口，是畫面上真的沒有那些承諾了。'
    + '數字再掉下來就代表觸發字或掃描範圍被改壞了＝這一題在空轉');
});

// ⭐ 這一題釘的是「**拿掉**」這個決定本身（William 2026-08-08 裁決）。
//
// 背景：2026-08-06 曾為「不可逆操作前的自動備份」做了一整層「存不成就擋下＋問使用者」
//（`backupBeforeIrreversible`、`needsConfirmation:'backup_failed'`、`proceedWithoutBackup` 旗標、
// 三處文案）。William 讀完完整拆解後決定**整層移除**：那層網會自己失敗，而為了誠實交代它，
// 一個單純的操作長出兩個確認框與三處說明。改成畫面只寫「儲存後沒有「復原」可以按」，不承諾任何自動還原檔。
//
// ⚠️ 沒有這一題的話，下一個看到「不可逆操作居然沒有備份」的人（人或 AI）會**好心加回來**，
// 而那正是被拍板拿掉的東西。所以這裡把「不得再出現」變成會轉紅的斷言，而不是只寫在註解裡。
// 使用者剩下的救援手段仍在：每日滾動備份 30 天（本檔第四節有題）、他自己按的「匯出備份」、啟動 .bak。
test('⭐ 裁決｜店名規則與開 app 自動整理這兩條路，不得再長出「操作前自動備份」（2026-08-08 拿掉的，不是漏掉的）', () => {
  const paths = ['lib/services/store-rules.js', 'lib/services/statement-import.js', 'lib/routes/statement.js'];
  for (const rel of paths) {
    const src = readFileSync(join(ROOT, rel), 'utf8');
    const code = stripJsComments(src);            // 註解裡講這段歷史是刻意的，不算違規
    assert.doesNotMatch(code, /backupBeforeIrreversible|backupNow\s*\(/,
      `${rel} 又出現「操作前自動備份」的呼叫——那一層是 William 2026-08-08 拍板拿掉的，`
      + '不是漏掉的（完整理由見 lib/services/backup.js 的裁決註解）。真的要加回來，先問過他。');
    assert.doesNotMatch(code, /backup_failed|proceedWithoutBackup/,
      `${rel} 又出現「備份沒做成」的確認旗標——同上，整層已移除。`);
  }
  // 前端那兩個呼叫端同樣不該回頭認那個 reason（認了就代表後端又長回來了）
  for (const rel of ['public/app.js', 'public/modules/settings-store-rules.js']) {
    ...
    assert.doesNotMatch(code, /backup_failed|proceedWithoutBackup|NO_BACKUP_CONFIRM/,
      `public 端的 ${rel} 又接起「備份沒做成」的確認——後端已經不會回這個 reason 了。`);
  }
  // 文案面：這兩處不可以再承諾自動還原檔（承諾了就會變成新的空頭支票）
  const cardSrc = readFileSync(join(ROOT, 'public/modules/settings.js'), 'utf8');
  const panelSrc = readFileSync(join(ROOT, 'public/modules/settings-store-rules.js'), 'utf8');
  // ⚠️ 警告句釘的是**四處統一之後**的那一句（William 2026-08-09 指派，同 ③ 對分類管理兩段的判準）：
  //    不可寫成無限定的「儲存後無法復原」——既有的匯出檔可以經 /api/import 整包匯回去，那句話比事實講得滿。
  //    這兩條同時是本題的**反面自我驗證**：掃描器讀錯檔或文案整段消失時，前置那條會先紅，本題不會靜靜空轉。
  ...
  for (const [rel, src] of [['settings.js', cardSrc], ['settings-store-rules.js', panelSrc]]) {
    ...
    assert.doesNotMatch(shown, /pre-rules|pre-normalize/,
      `${rel} 的畫面文案又點名了操作前備份檔——那個檔案已經不會產生了，寫出來就是假話。`);
    assert.ok(src.includes(WARN),
      `前置：${rel} 的「${WARN}」不見了——掃描器可能讀錯檔，或那句不可逆警告被刪了，本題會空轉`);
    assert.doesNotMatch(shown, /儲存後無法復原/,
      `${rel} 把警告寫回無限定的「儲存後無法復原」——匯出檔整包匯回去救得回來，`
      + `畫面撐得住的只有「${WARN}」這個 UI 事實（四處統一，William 2026-08-09 指派）。`);
  }
});
...
//    ⚠️ 這四份重複與那句 docstring 都是產線既有狀況（非本 PR 造成；**本支不碰 lib/store.js 的備份實作**
//    ——本支改動的正式程式是 public/modules/backup-export.js（新）／toast-timing.js（新）／settings.js／
//    app.js／styles.css 與 lib/routes/core.js 的 `GET /api/mode`，都不在這一節的射程內）；
