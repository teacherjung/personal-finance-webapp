...
test('畫面文案｜分類管理只留「沒有復原可以按」的警告、不再承諾備份，而文案叫人按的名字都要有那顆鈕', () => {
  ...
  for (const btnId of ['manageCatsBtn', 'manageIncomeCatsBtn']) {
    ...
    //       lib/repo.js:256 的 `snapshotTo` 在 HOSTED 直接丟錯；雲端的寫入走 store-pg，lib/store.js 的
...
// 一個單純的操作長出兩個確認框與三處說明。改成畫面只寫「儲存後無法復原」，不承諾任何自動還原檔。
//
...
test('⭐ 裁決｜店名規則與開 app 自動整理這兩條路，不得再長出「操作前自動備份」（2026-08-08 拿掉的，不是漏掉的）', () => {
  ...
  // 反面自我驗證（避免這一題變成「掃不到就過」的空轉）：確認掃描器真的讀得到內容
  assert.ok(cardSrc.includes('儲存後無法復原'), '前置：設定頁店名規則卡片的「儲存後無法復原」不見了——掃描器可能讀錯檔，本題會空轉');
  assert.ok(panelSrc.includes('儲存後無法復原'), '前置：店名規則編輯窗的「儲存後無法復原」不見了——同上');
});
