...
//    那一支是 `fix/export-backup-feedback`（本支不碰那顆鈕、也不改文案）。
...
//    這句指路的可信度因此**依賴匯出鈕會回報成功／失敗**，那由 `fix/export-backup-feedback` 提供；
...
test('畫面文案｜分類管理只留「沒有復原可以按」的警告、不再承諾備份，而文案叫人按的名字都要有那顆鈕', () => {
  ...
  for (const btnId of ['manageCatsBtn', 'manageIncomeCatsBtn']) {
    ...
    assert.match(noQuote, /沒有復原(可以按|按鈕|鈕)/,
    ...
    for (const m of plain.matchAll(/自動備份/g)) {
      // ⚠️ 窗口刻意只有 8 字、否定詞清單刻意**不含**「未」「不是」，r6 起也**不含裸字「沒」**：
      //    這段文案本身就有「其他／未分類」，而裸字「沒」會讓「沒問題，會自動備份。」這種
      //    明確假承諾被前面半句保下來（#410 r6 在下一題實測到的洞，同一個病這裡一起關）。
      const before = plain.slice(Math.max(0, (m.index ?? 0) - 8), m.index);
      assert.match(before, /不會|不做|沒有|沒做|不保證|別只靠|別依賴/,
  ...
  assert.ok(LABELS.includes('資料與備份'), '前置：抓不到區塊標題的字面（uiLabels 壞了）——④ 會整段變空轉');
...
test('匯出備份｜雲端版匯出的檔案裡必須有分類樹（店名規則文案叫人按的就是這顆按鈕）', () => {
  ...
  //    `fix/export-backup-feedback` 修，在它合併前，這條自救路徑的第一步就有缺口（見檔頭）。
  const trees = {
...
test('畫面文案｜任何「系統會替你留一份」的說法都要交代哪一種模式才有（掃遍 public/，前端拿不到模式資訊）', () => {
  ...
  //      (b) 否定詞清單拿掉裸字「沒」（洞①），仍**不含**「未」「不是」（畫面上到處都是「未分類」），
  //          而且否定詞要**貼著**那個說法（前 8 字）——否則隔壁半句的否定詞會替這一句作保。
  ...
  //    不是「所有可能的假承諾」。已知擋不住的三個例子：換一組完全不同的詞（「你的東西我們都留著」）、
  //    把「誰做的」與「留了一份什麼」**拆成兩句**（判準只看同一句）、
  //    或在 backup-alert.js 的句尾補一個「失敗」把承諾夾帶進來。目標是**讓下一個人踩到就紅**，
  //    不是窮舉；要更強的保證只能靠真的把模式資訊送到前端（見 docs/文案審稿-雲端版的九句假話.md 附註）。
  ...
  const NEG = /不會|不做|沒有|沒做|不保證|別只靠|別依賴/;                    // 否定句（＝沒有承諾）
  ...
  for (const rel of files) {
    ...
    for (const [si, seg] of sentences.entries()) {
      for (const m of seg.matchAll(SAFETY)) {
        ...
        const before = seg.slice(Math.max(0, idx - 8), idx);     // 否定詞要**貼著**這個說法，才算否定它
