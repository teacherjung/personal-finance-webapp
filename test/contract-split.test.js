...
//   ・**索引列整行＝契約算得出來的那一行**（#639 起）：第一格逐字等於該節的 `**改這裡**`，
//     第二格只有指路、label 是「契約：<領域名>・<節標題>」。索引因此不可能與契約說不同的話。
//     ⚠️ 它**不驗**契約內文本身對不對，也不驗那一行寫得好不好——只保證兩邊不會各說各話。
...
function assertHeadingForm(p, raw) {
  ...
  //      ⚠️ **兩種不一樣**（r3 #2、r4 #3 兩次點名，兩次都拿 GitHub API 實測）：
  //      link definition（`[x]: …`）GitHub **一個字都不顯示**；
  //      **footnote definition（`[^n]: …`）在被引用時會渲染出一個可見的註腳區**——那時它不是隱形的。
  //      ⚠️ **條件是「被引用」**（r5 #2 量到）：`有註腳[^n]` ＋ `[^n]: 可見註腳` ⇒ 有註腳區；
  //      **只有定義、沒有人引用** ⇒ 輸出是空的。所以這一族的正確說法是
  //      「**可能**藏進畫面看不到的東西」，不是「一定看得到」也不是「一定看不到」。
  //      這裡**統一禁用**，理由是「**可能**藏進畫面看不到的東西」，不是「一定看不到」。
  //      ⚠️ 原本的理由是「灌大內文、讓『索引摘要必須比內文短』那道比例檢查失效」——
  //      **那道比例門已於 #639 刪除**（索引改成整行由契約產出）。這一族現在擋的是
  //      **契約自己被塞進畫面看不見的東西**（隱形文字、假 anchor、零寬字元），
  //      那仍然是「契約說的跟畫面顯示的不一樣」。
  //    ②行**中**的 raw HTML：`可見文字 <a id="假anchor"></a><details>藏起來</details>`
  //      ——原本只擋行首 `<`。
  //    ③零寬與方向控制字元：畫面上不存在，原始碼卻有。
  //    三族的共同點是「**原始碼寫的跟畫面顯示的不一樣**」⇒ 一律禁止。
  //    ⚠️ 不要寫成「原始碼有、畫面沒有」——那對 footnote definition 不成立（它有畫面）。
  //    ⚠️ #639 前的理由是「長度算得到、會撐大比例門的分母」——**那道門已刪**；
  //    現在的理由是**契約說的跟畫面顯示的不一樣**（也讓表外副本題的逐字比對兩邊不是同一種文字）。
  ...
  assert.equal(refDef, -1,
    ...
    + '⚠️ 它**可能**在 GitHub 上完全不顯示：link definition 不顯示；註腳定義**被引用時**會渲染出註腳區、\n'
    + '   **沒被引用就不顯示**（#639 掃描與 r5 兩次實測）。所以這裡**統一禁用**：\n'
    + '   契約不可以「寫了一堆、畫面上不是那樣」。\n'
    + '   契約檔請直接寫行內連結。');
  // ⚠️ HTML entity 也算「畫面看不見、長度算得到」（Codex #384 r37）：
  //    `&ZeroWidthSpace;` 在原始碼是 18 個字元、在畫面上是 0 個 ⇒ 同一族「原始碼有、畫面沒有」。
...
/**
 * 索引行的硬上限。
 * ⚠️ #639 之後索引整行由契約產出，所以這一條擋的不再是「有人把摘要寫太長」，
 *    而是「契約那一節的 `**改這裡**` 那一行寫太長」——要縮就去縮契約。
 */
const MAX_INDEX_LEN = 600;
/**
 * 把一段 Markdown 正規化成**畫面上看得到的文字**（回傳的是文字，不是字數）。
 *
 * ⚠️ **現在誰在用**：只有「表外副本題」（#639 起；`visibleLen()` 與比例門已刪）。
 *
 * ⚠️ 當初存在的理由（歷史）：拿原始碼長度當分母會被「正常維護」灌大（Codex #384 r35）——
 ...
 *   ・**不是**「擋得住所有用不可見原始碼混進來的手法」——例如契約 body 裡的 HTML entity
 *     （`&ZeroWidthSpace;`）在這裡算成 18 個字，在畫面上是 0 個。
 *   ・**不是**「行內 code 只算內容」——`|_*~` 這些字元在 code 裡也會被一起刪掉。
 * 它能說的只有：**已知的幾種「原始碼有、畫面沒有」的東西會被剝掉**。要更準就得接真正的 Markdown 渲染器。
 * ⚠️ #639 之前這個「近似」承的是**比例門的分母**；那道門已刪，現在它承的是**表外副本題的比對基準**
 *   ——剝不乾淨的後果從「分母被撐大」變成「逐字比對的兩邊不是同一種文字」。
 * @param {string} md
 */
/** 「畫面看得到的字」正規化（#639 起只剩**表外副本題**在用；比例門與 `visibleLen()` 已刪）。 @param {string} md */
function visibleText(md) {
  return String(md)
    // ⚠️ 目的地可以含**平衡括號**（`…/report(section)?utm=…` 是正常網址，Codex #384 r37）：
    //    只吃到第一個 `)` 的話，後面的查詢參數會被當成可見文字（當年是撐大分母；
    //    現在是讓表外副本題把網址參數當成契約內文的一部分去比對）。
    .replace(/!\[[^\]]*\]\((?:[^()]|\([^()]*\))*\)/gu, '')       // 圖片：整個不算
...
/** 契約每一節的「改這裡」標籤——**索引列的第一格由它決定**（#639 起）。 */
const TRIGGER_LABEL = '**改這裡**：';
...
function indexRows() {
  ...
  lines.forEach((line, i) => {
    ...
    // ⚠️ **連結 label 只准 `契約：<領域名>・<節標題>`**（Codex #384 r35 立、#639 擴一格）：
    //    把整段契約原文塞進 label（`[契約：整段原文](…)`）畫面上完整顯示，
    //    而舊的摘要計算會把整個連結剝掉 ⇒ 索引長回原文而護欄全綠。
    //    #639 起 label 多了「・<節標題>」那一段——**它不是手寫的**：
    //    下面那一題（索引列整行由契約決定）會把整行逐字比對回契約，
    //    所以節標題塞不進別的東西。這裡只擋形狀，讓錯誤訊息停在近處。
    //    ⚠️ 這一條**只擋形狀，不擋內容**——內容由那一題負責。
    assert.match(m2[0], /^\[契約：[^\][()。，、；：！？\n]{1,12}・[^\][()\n]{1,40}\]\(/u,
      `${where} 的契約連結 label 不是 \`契約：<領域名>・<節標題>\` 的形式：${m2[0].slice(0, 70)}…\n`
      + '⚠️ label 是畫面上唯一顯示的文字——把內文塞進去，索引就長回原文了。\n'
      + '   節標題請**逐字**照契約那一節的標題，不要自己寫。');
...
function sectionsOf(file) {
  ...
  return heads.map((h, i) => {
    ...
    return {
      ...
      // 「改這裡」＝**行首**那個標籤、到換行為止，而且**整節剛好一條**（#639；Grok 複審後掃抓到）。
      //
      // ⚠️ 原本用 `text.indexOf(TRIGGER_LABEL)` 取第一次出現、不管在不在行首、也不管有幾條。
      //    掃描器實測兩種打法，兩種都讓「索引與契約說不同的話」而整卷仍 16/16 綠：
      //      改法 A：同一節再加第二條 `**改這裡**：`——`indexOf` 只看第一條，第二條完全隱形。
      //      改法 B：在真的那一條**前面**插「先寫一句 **改這裡**：假的觸發詞」——它不在行首，
      //              但 `indexOf` 照樣抓到，於是索引可以照抄那句假的。
      //    ⇒ 判準改成：**只認行首**、而且**數量必須剛好 1**；不是 1 就回空字串，
      //      由用它的那一題 fail-closed（「沒有那一行 ⇒ 索引列的第一格就無從產出」）。
      trigger: (() => {
        const hits = [...text.matchAll(/^\*\*改這裡\*\*：(.*)$/gmu)];
        return hits.length === 1 ? hits[0][1].trim() : '';
      })(),
      /** 行首的「改這裡」有幾條——用它把「0 條」與「2 條以上」分開報，錯誤訊息才講得清楚。 */
      triggerCount: [...text.matchAll(/^\*\*改這裡\*\*：/gmu)].length,
    };
  });
}
...
test('⭐ 拆分護欄｜索引列**整行由契約決定**（第一格＝改這裡、第二格只有指路，兩格都不准手寫）', () => {
  // ## 這一題取代了什麼（#639）
  //
  // 原本這裡守的是「摘要要比契約內文短 40%」——那是**妥協**：索引可以留一段人寫的濃縮，
  // 只要不要長到接近整段。實際狀況是 **88 列裡有 78 列真的留了摘要**，而量過之後
  // **沒有一段是契約內文的逐字複本**——它們是**人寫的第二份**，會各自漂。
  //
  // 比例門擋得住「長回整段」，擋不住「短短一句但講錯」。所以判準換成**逐字相等**：
  //   第一格 ＝ 契約那一節的 `**改這裡**` 那一行（逐字）
  //   第二格 ＝ 只有指路，連結文字是「契約：<領域>・<節標題>」
  // 兩格都從契約算得出來 ⇒ **索引不可能與契約說不同的話**。
  //
  // ⚠️ **代價寫明**：索引因此**大多不再告訴你「要注意什麼」**——那些話回到契約的「記得同步這裡」。
  //    ⚠️ **不是全部**：有些節的 `**改這裡**` 本身就帶著注意事項，那一部分仍留在索引第一格
  //    （Grok 複審後掃 #639 指出我原本寫「那句話只在契約裡」太滿）。
  //    換到的是「索引說的每一個字都來自契約」。裁示者 2026-09-23 選這一邊。
  //
  // ⚠️ 這一題**不驗**契約內文本身對不對，也不驗 `**改這裡**` 那一行寫得好不好——
  //    它只保證**兩邊不會各說各話**。
  for (const { line, file, anchor } of indexRows()) {
    ...
    assert.ok(s.trigger,
      `${file}#${anchor} 這一節**行首的**「${TRIGGER_LABEL}」有 ${s.triggerCount} 條（要剛好 1 條）⇒ 索引列的第一格無從產出。\n`
      + '⚠️ 0 條：那一節沒有這個標籤，或它沒有寫在行首（縮排、前面還有字都不算）。\n'
    ...
    assert.ok(line.length <= MAX_INDEX_LEN,
      ...
      + `⚠️ 索引已經是產出的 ⇒ 要縮就去縮 ${file} 那一節的「${TRIGGER_LABEL}」那一行。\n`
      + `實得：${line.slice(0, 120)}…`);
  }
});

test('⭐ 拆分護欄｜同步點清單那一節**整節的形狀**是固定的（不必懂 GFM，只要比對）', () => {
  // ## 這一題取代了什麼（#639 r6 的 F10 結論）
  //
  // 前三輪我一直在問「**這一行是不是 GFM 眼中的表格列**」——那要懂 GFM 怎麼解析，而我不懂：
  //   r4：表格列**不必以 `|` 開頭**，一行純文字也會變成一列
  //   r5：表格**裡面**的 `|---|---|` 不是表頭分隔線，是普通資料列
  //   r6：分隔線**也不必以 `|` 開頭**（`--- | ---` 合法）⇒ 整張表不會「活」起來
  // 三輪都是同一句話：「我沒想到 GFM 還能這樣寫」。**那是在重做一個 Markdown 剖析器**
  //（本倉庫 2026-08-03 結過：「要嘛真 parser 要嘛不走」；跑步機那條說「第二輪就該停」）。
  //
  // ⇒ **把問題反過來問**：不問「這一行是不是表格列」，改問「**這一節每一行是不是我預期的那一行**」。
  //   那一節的形狀本來就是固定的：說明文字 → 表頭 → 分隔線 → N 列索引（連續）→ 空行或節尾。
  //
  // ⚠️ **代價（刻意收緊，完整列出）**：那一節的職責就是那張索引表，所以——
  //   ・表頭與分隔線**一字不能改**（要改就得同時改這一題）
  //   ・**不能有第二張表**；前言與表後的散文**都不准出現 `|`**（行內程式碼、跳脫寫法裡的也算）
  //   ・N 列索引之間不准穿插空行、小標或說明
  //   ・表後面要接東西就得先空一行，而且那一行要**完全空**
  //   逐字固定的是「表頭＋分隔線＋N 列索引＋之後那一行」；前言與空行之後的散文**不在逐字射程內**
  //   ——這一句是射程，不是保證（#639 r7 F10 第③題）。
  // ⚠️ **但散文受的限制不只「不准含 `|`」**（#639 r8 #2 補）：下面抽索引列用的是 `LINK_RE`，
  //    所以**任何一行只要命中 `LINK_RE` 就會被抽進索引列**——把一個現有的契約連結寫進
  //    「參見 …」那種前言（就算完全不含 `|`），或寫進真空行之後的散文，都會讓下面①的
  //    連續 N 行比對對不上而轉紅。**不要把那兩塊當成「只要別打直線就自由」。**
  //    ⚠️ 實測（#639 r8 處置時自己跑的，帶對照組）：那兩發各讓 **4 題**一起紅（13 pass／4 fail）
  //    ——本題加上另外三題用到索引抽取的；同位置放**不含連結**的普通散文兩發都 17 pass ⇒
  //    紅的原因是連結，不是「多了一行」。
  //    ⚠️ **射程只到 inline 連結**（#640 r1 #2 抓到、我自己重驗）：`LINK_RE` 是行內 `[…](…)` 的形狀，
  ...
  // **表頭與分隔線自己也是固定形狀的一部分**——逐字釘死，不猜（#639 r7 補）。
  ...
  assert.equal(sec[hi + 1], SEPARATOR,
    ...
    + '⚠️ 這兩行是整節唯一兩個「含 `|` 但不是索引列」的豁免位置 ⇒ 不釘死就等於開一道後門。');

  const 索引列 = sec.filter((l) => LINK_RE.test(l));
  assert.ok(索引列.length >= 50, `那一節只找到 ${索引列.length} 列索引——抽取式壞了？`);

  // ①分隔線之後**連續**就是那 N 列索引：中間插進任何一行（含不帶 `|` 的純文字）都會對不上。
  // ⚠️ **它不檢查順序**——兩邊都是從同一節抽出來的，把某一列搬到別的位置仍然相等（實測仍綠）。
  //    那 N 列的排法是人排的、沒有獨立來源可比；而重排**不會讓索引說出跟契約不同的話**
  //    （每一列仍然逐字等於它自己那一節），所以這裡不擋它。
  const 實得 = sec.slice(hi + 2, hi + 2 + 索引列.length);
  assert.deepEqual(實得, 索引列,
    '同步點表的索引列**不是緊接在分隔線後面的連續 N 行** ⇒ 中間被插了東西。\n'
    + '⚠️ 插進去的那一行即使不含 `|`，GFM 照樣會把它渲染成表格的一列（第一格有字、第二格空）。');

  // ②第 N+1 行必須是空行或節尾
  const 之後 = sec[hi + 2 + 索引列.length];
  // ⚠️ 用 `=== ''` 不用 `.trim() === ''`：`trim()` 會把全形空白 U+3000、NBSP 也當成空的，
  //    但 GFM **不當**它們是空行 ⇒ 表格還活著，下一行純文字照樣變成一列（#639 r7 第 2 條實測）。
  //    這裡不去列舉「哪些字元算空白」（那又是在重做 GFM）——直接要求**完全空**。
  // ⚠️ **兩種紅法的原因不一樣，訊息不可以一概而論**（#639 r8 #1）：
  //    ・U+3000／NBSP ⇒ 表格**真的還活著**，後面那句話會被渲染成表格的一列（實測）。
  //    ・半形空白或 Tab ⇒ GFM 仍算空行、**表格會正常收尾**，後文渲染在表外的 `<p>`（審查者實測）。
  //      這種紅是**這裡的固定格式要求**擋的，不是渲染出錯——訊息裡不可以說成「表格還活著」。
  assert.ok(之後 === undefined || 之後 === '',
    `索引列之後那一行（第 ${start + hi + 2 + 索引列.length + 1} 行）不符合這裡要求的**完全空**：${JSON.stringify(之後)}\n`
    + '⚠️ 全形空白 U+3000 與 NBSP 會讓表格繼續活著、把後面那句話變成表格的一列；\n'
    + '   半形空白或 Tab 則表格會正常收尾，純粹是**這裡的格式要求**（本題不分辨哪些字元算空白）。\n'
    + '   要接著寫東西請先空一行——連一個空白都不要留。');

  // ③那一節其他任何一行都不准含 `|`（表頭與分隔線除外）
  const 多的 = sec
    .map((l, i) => ({ l, i }))
...
test('⭐ 拆分護欄｜用名字指到同步點清單的句子，那個名字必須真的在表上（#639）', () => {
  ...
  //
  // ⚠️ **射程（兩層，都要講）**：
  //   ①**抽取式只認緊接的寫法** `同步點清單「X」`。中間夾空白（`同步點清單 「X」`）
  //     或換行寫，這一題就**抽不到那一句**（審查者 r1 #2 實測：改成夾空白，16/16 仍全綠）。
  //     **不宣稱**偵測得到所有自然語言指路；要那個得寫半個 Markdown／中文剖析器，不划算。
  //     `refs.length >= 5` 是**數量絆線**，不是「抽取式整個壞掉」才紅：現有九句，
  //     **漏抽四句仍綠、漏抽五句才紅**——它不保證逐句都被抽到（審查者 r2 #1 實測）。
  //   ③**比對口徑是子字串**：拿抽到的名字對「第一格的 Markdown 原文」與「連結 label 裡的節標題」
  //     做 `includes()`，不是可見文字的完整相等。所以 `同步點清單「機密」`、甚至
  ...
  for (const { line } of rows) {
    const cells = line.replace(/\\\|/gu, '\uE000').split('|');
    可指的.add((cells[1] || '').trim());                                    // 第一格
    const t2 = /\[契約：[^・\]]+・([^\]]+)\]/u.exec(line);
    if (t2) 可指的.add(t2[1].trim());                                       // 連結 label 裡的節標題
  }
  const refs = [...md.matchAll(/同步點清單「([^」]+)」/gu)].map((m) => m[1]);
