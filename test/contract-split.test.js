...
test('⭐ 拆分護欄｜同步點清單那一節**整節的形狀**是固定的（不必懂 GFM，只要比對）', () => {
  ...
  //
  // ⚠️ **代價（刻意收緊）**：那一節**不能有第二張表**，表後面要接東西就得空一行、而且那些行不准含 `|`。
  //   那一節的職責就是那張索引表。
  ...
  // 表頭＝節內第一行含 `|`；分隔線＝它的下一行
  const hi = sec.findIndex((l) => l.includes('|'));
  assert.ok(hi > 0, '那一節裡找不到表頭 ⇒ 這一題自己壞了');
  assert.match(sec[hi + 1], /^ {0,3}\|?[ \t]*:?-+:?/u,
    `表頭（第 ${start + hi + 1} 行）的下一行不是分隔線：${JSON.stringify(sec[hi + 1])}`);
  ...
  assert.ok(之後 === undefined || 之後.trim() === '',
    `索引列之後那一行（第 ${start + hi + 2 + 索引列.length + 1} 行）不是空行：${JSON.stringify(之後)}\n`
    + '⚠️ 表格還活著，那一行會被渲染成表格的一列。要接著寫東西請先空一行。');
