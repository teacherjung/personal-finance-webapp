...
test('歸類｜子行程**提早死（未捕捉例外、code 1）** → 500，**絕不可以假裝成 400**', async () => {
  ...
  setPdfTimeoutForTest(60_000);   // 給得很寬：本題要看的是「它自己死」，不是逾時
  // ⚠️ 2026-09-05 流程體檢：改之前（main@ae66225 的版本）逾時 5s、上界 4s，機器忙時 spawn＋死掉的牆上時間可能超過 4s＝假紅。
  //    「沒等到逾時」由下面的 code 斷言證明；時間上界只是「不是靠逾時收回」的第二道證據。
  //    設定＝逾時 60s、上界 15s：壞路徑（等到逾時 timer 才 SIGKILL 收回）約 60s，是上界的 4 倍——判別餘裕由這兩個設定值決定
  //    （SIGKILL 失敗時 close 不會來、題會掛住而不是提早綠，所以這裡不寫「一定 ≥ 60s」）。
  //    誠實劃界：好路徑要多久是觀測值（本機幾十到幾百 ms）、記憶體吃緊時 spawn 可能拖到秒級，15s 是給它的空間，
  //    不是保證；真的超過 15s 這題會紅、那時該查的是機器不是判準。
  {
    const t0 = Date.now();
    const err = await errOf(parseStatement(normalPdf()));
    ...
    assert.ok(Date.now() - t0 < 15_000, '它是自己死的，不該等到逾時（逾時是 60s）');
  }
});
