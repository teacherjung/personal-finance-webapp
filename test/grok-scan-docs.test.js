...
//     「還在」＝不在 HTML 註解裡、也不在 **fenced code** 裡（fence 的判準照規格寫完整，
//     見 `visible()` 的四條）——但**縮排式程式碼區塊不涵蓋、也不打算涵蓋**，同見那裡的劃界。
//     ⚠️ 這句話本身改口過三次：r1 原寫「兩份檔案講法一致」（字串相同不等於講法一致），
//     r2 原寫「不是程式碼範例裡的字」（被 blockquote 裡的 fence 當場推翻），
//     r3 原寫「除縮排式以外都涵蓋」（被巢狀 fence 推翻）。現在寫的是它真正做得到的射程。
 ...
 * 要嘛照規格做完整、要嘛老實說看不出來）。所以 r4 起**不再逐形補**，改成把 Markdown
 * 對 fenced code 的判準**寫完整**——那是個**封閉集合，就下面四條，沒有第五條**：
