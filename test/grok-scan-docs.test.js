...
test('Grok 複審後掃｜時機：AGENTS 正本與 REVIEW 最短版把「通過」和「轉正式之前」綁在同一行；合併步驟前導寫「轉正式之前」；列出的現行指示不得出現三個舊時機字面', () => {
  ...
  // ⚠️ 射程（誰的支要掃）也釘一下（William 2026-09-09 裁「Grok 掃要對稱」）：此前三處條文明寫
  //    「Claude 實作的支」、指路句以「Codex 通過」暗示；改成對稱之後，只要有一處被**整段刪回**單邊、
  //    其餘不動，時機那幾題照樣全綠。
  //
  // ⚠️ **這是「某個詞還在不在」，不是「射程真的對稱」**（Grok #588 掃後照實補；Codex #588 r1 也實測過）。
  //    **擋得到**：那四處裡有一處把對稱字樣整段換成單邊寫法。
  //    **擋不到**（全部仍綠，實測）：①詞留著、後面再加例外（「都要掃，但某方的支除外」；
  //        例外寫在續行也一樣綠）②詞掛到別的名詞上（誰執行、誰發射、哪一輪），射程句被拿掉
  //      ③改**這四處以外**的地方——PR 範本的待填提示、AGENTS 常設授權的「小標已填」、
  //        `REVIEW-AND-MERGE.md` 實作模式那一步；**那三處才是人照著做的入口，都不在這道釘的射程裡**
  //      ④同列宣告本條停用存查（本檔開頭那段講的「不判斷規範效力」）。
  //    **會誤擋**：換成等義的別種說法（「無論」也紅）；把同一句折行、讓詞掉出擷取視窗也紅
  //      （⚠️ 2026-09-09 我一度把折行寫在「擋不到」那半邊——那是我轉述錯了，Codex #588 r3 四處逐一實測都是紅）。
  //      紅了就來改這裡，不要拿掉。
  //    ⇒ 不再往外補（本檔開頭「查字不查語意」那段是同一條界）：這一族的下限是複審的眼睛。
  //
  // ⚠️ **按名字挑，不按位置**（原本寫 `.slice(0, 4)`：有人在清單開頭插一項現行指示，
  //    釘就打到別的四格、真正承重的那格反而漏掉）；名字對不上＝紅，不默默少釘。
  const SCOPE_PINNED = ['AGENTS 複審後掃條', 'CLAUDE.md 入口句', 'REVIEW 最短可執行版開頭兩行', '合併步驟指路句'];
  for (const name of SCOPE_PINNED) {
    const hit = CURRENT_DIRECTIVES.find(([where]) => where === name);
    assert.ok(hit, `射程釘找不到「${name}」這一格——CURRENT_DIRECTIVES 的名字改了或那一格被刪了，`
      + '這道釘會靜靜少釘一處。改名字的人請一起改這裡。');
    assert.ok(hit[1].includes('不論'),
      `${name}沒有寫「不論」（實作者是誰都要掃）——2026-09-09 起 Grok 複審後掃對稱適用，`
        + '這一格被換成單邊寫法了（⚠️ 本斷言只認這個詞，擋不到「留著詞再加例外」——見上方註解）');
  }
    ...
    for (const old of OLD_TIMINGS) {
      assert.ok(
        ...
        `${where}出現舊時機「${old}」——只要新字串也還在，兩種相反答案就並存；`
          + '只讀到舊那半句的人會在合併前才掃，每修一條＝head 變＝重拿一張「通過」、整條複審鏈重走（那正是這次改時機要關掉的路）'
      );
    }
  }
});
