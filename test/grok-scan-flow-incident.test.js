...
test('runScan｜不注入 liveRoot 時，活金絲雀建在真的家目錄：掃描期間認得出本輪暗號那一個、掃完清掉', async (t) => {
  ...
  //    而且它的四個根寫死、沒有可注入的參數。⚠️ 2026-09-08 拆檔後**本檔沒有任何一題會走到真的沙箱金絲雀**
  //    （會走到的那一題在 `test/grok-scan-flow-credentials.test.js`）；本題只是讓**活金絲雀**落在真家目錄，
  //    沙箱金絲雀仍是注入的假的。上一版寫「本檔只有…那一題會走到它」，那是拆檔前整支大檔的成員關係（Grok #581 掃後 5）。
