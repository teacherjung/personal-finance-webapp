...
// 開關關閉回單讀階梯（那條路的完整行為卷在 test/ai-parse.test.js）／fail-open 判準／呼叫數釘 2·3。
...
test('仲裁｜兩讀金額不一致→Fable 與其中一份全欄一致＝採用那份、恰 3 發、dualRead=arbitrated', async () => {
  ...
  const spy = { calls: /** @type {string[]} */ ([]) };
  const good = answerOf();
  const bad = answerOf({ transactions: [{ ...good.transactions[0], amount: 100, balance: 4900 }, { ...good.transactions[1], amount: 600, balance: 5500, summary: '薪資入帳寫錯' }] });
...
test('一讀掛掉｜另一讀有效＝沒有兩份互證→仍走仲裁：Fable 一致＝採用；兩讀都掛＝服務錯優先照實丟', async () => {
  ...
  assert.equal(r.dualRead, 'arbitrated', '★單讀無互證＝Fable 補第二份獨立答案');
...
test('徽章｜dualRead=agree/arbitrated 各有一句、無 dualRead 不畫；句子只講事實不加保證', () => {
  ...
  assert.match(aiPreviewBadgeHtml({ ...base, dualRead: 'arbitrated' }), /三讀仲裁/);
  const plain = aiPreviewBadgeHtml(base);
  assert.doesNotMatch(plain, /雙讀一致|三讀仲裁/, '單讀＝不畫雙讀句');
  for (const h of [aiPreviewBadgeHtml({ ...base, dualRead: 'agree' }), aiPreviewBadgeHtml({ ...base, dualRead: 'arbitrated' })]) {
