 ...
 *     正式寫法都算）——設定檔裡對外接工具的拒絕規則只能寫**光名字**（帶括號的那種載入時會被跳過），
 *     而光名字的拒絕會讓那支工具**在規則存在的期間不可呼叫**（官方權限文件：「A bare tool name like
 *     `Bash` removes the tool from Claude's context entirely, so Claude never sees it.」2026-09-21 讀）
 *     ——蓋到的話**按不動它**，所以驗不到攔截器，不是「按下去量到平台」。⚠️ 不要把它寫成「定義一定看不到」：
 *     官方快取那一頁寫**工具搜尋啟用時、進行中那個請求的工具定義不變**，停用或不可用時才從下一次請求移除；
 *     而且這一題只驗清單、根本沒驗平台的可見性（看得見名字不等於防線失效）。**受管設定層更不可以放**：
 *     那一層要管理員密碼才改得掉，規則在的期間這顆鈕不可呼叫，要管理員改掉或刪掉那個檔才恢復；
...
test('⑥平台自己的拒絕清單沒有任何一條蓋到測試鈕（蓋到就量不到攔截器）', () => {
  ...
  for (const rule of claude.permissions?.deny ?? []) {
    assert.ok(!coversCanary(rule),
      `permissions.deny 的「${rule}」會蓋到測試鈕：光名字的拒絕會讓它在規則存在的期間不可呼叫＝按不動、驗不到攔截器`);
  }
});
