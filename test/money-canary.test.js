 ...
 *     而光名字的拒絕會把那支工具**整個從 AI 看得到的清單上拿掉**（官方權限文件：「A bare tool name like
 *     `Bash` removes the tool from Claude's context entirely, so Claude never sees it.」2026-09-21 讀）
 *     ——蓋到的話**根本沒有鈕可以按**，不是「按下去量到平台」。**受管設定層更不可以放**：那一層要管理員
 *     密碼才改得掉，規則在的期間這顆鈕不可呼叫，要管理員改掉或刪掉那個檔才恢復；
...
test('⑥平台自己的拒絕清單沒有任何一條蓋到測試鈕（蓋到就量不到攔截器）', () => {
  ...
  for (const rule of claude.permissions?.deny ?? []) {
    assert.ok(!coversCanary(rule),
      `permissions.deny 的「${rule}」會蓋到測試鈕：光名字的拒絕會把它整個從 AI 看得到的清單上拿掉＝根本沒有鈕可以按`);
  }
});
