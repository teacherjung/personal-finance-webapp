...
async function seedCard(id) {   // id 必帶且用 r11 前綴——seed.json 本來就有 c1/c2 等卡片 id，撞名會拿到 seed 的卡
...
test('r11#1｜偽造 stmtRef 被重建成標準格式；同筆消費再匯入會被去重（100 不會變 200）', async () => {
  await seedCard('r11a');
  const row = { date: '2026-01-10', amount: 100, desc: '測試店家', category: '生活', subcategory: '' };
  const r1 = await importRows('r11a', [{ ...row, stmtRef: '偽造|亂寫|x|y', store: '偽造店名（不是真實店名）' }]);
  ...
  const r2 = await importRows('r11a', [{ ...row, stmtRef: canonical }]);
  assert.equal(r2.imported, 0);
  assert.equal(r2.skipped, 1, '重建後的去重要擋住第二次匯入');
  const total = ((await repo.getDb()).transactions || []).filter(x => x.stmtRef === canonical)
    .reduce((s, x) => s + x.amount, 0);
  assert.equal(total, 100, '金額不可翻倍');
});

test('r11#1｜合法 |#N 序號段保留（同帳單兩杯一樣的咖啡）；重匯整份不多出來', async () => {
  await seedCard('r11b');
  ...
  const r1 = await importRows('r11b', rows);
  assert.equal(r1.imported, 2, '兩筆真消費都要進（|#2 序號合法保留）');
  const refs = ((await repo.getDb()).transactions || []).filter(t => t.stmtRef && t.stmtRef.startsWith(base)).map(t => t.stmtRef).sort();
  assert.deepEqual(refs, [base, `${base}|#2`]);
  const r2 = await importRows('r11b', rows);
  assert.equal(r2.imported, 0, '重匯同一份帳單＝同一組 stmtRef，一筆都不多');
});

test('r11#1｜非法尾段（|#1、|垃圾）剝除重建；壞日期/缺 desc 整列跳過而非 500 毒整批', async () => {
  await seedCard('r11c');
  const base = 'r11c|2026-01-12|70|尾段測試店';
  const r1 = await importRows('r11c', [
  ...
  const t = ((await repo.getDb()).transactions || []).find(x => x.stmtRef && x.stmtRef.startsWith(base));
...
test('r11#4｜同店整批改分類（applyCategoryToStore）同樣搬家不刪除', async () => {
  await seedCard('r11d');
  const orig = '整批測試店（南崁）';
  const key = storeKeyOf(orig);
  const db = await repo.getDb();
  ...
  await repo.saveDb(db);
  await applyCategoryToStore(key, '生活', '');
...
test('r11掃描｜renameStoreDisplay：找不到符合交易 → 404，且不種下隱形規則', async () => {
  await assert.rejects(() => renameStoreDisplay('根本不存在的帳單原文XYZ', '劫持名'),
    (/** @type {any} */ e) => e.status === 404, '要 404 不要 ok:true');
  assert.equal((await repo.getDb()).learnedCategories?.['根本不存在的帳單原文XYZ'], undefined, '學習表不可被種入');
});

test('r11掃描｜renameStoreDisplay 回歸：存在的原文照常改、冪等重存（changed=0）不誤擋', async () => {
  await seedCard('r11e');
  const orig = '改名回歸店';
  const db = await repo.getDb();
  ...
  await repo.saveDb(db);
  const r = await renameStoreDisplay(orig, '回歸新名');
  assert.equal(r.ok, true);
  assert.equal(r.changed, 1);
  const again = await renameStoreDisplay(orig, '回歸新名');   // 同值重存＝matched>0、changed=0，不可 404
  assert.equal(again.ok, true);
  assert.equal(again.changed, 0);
});

// ---- 對抗審查補洞（同輪抓到的修法缺陷，逐一鎖住）----
test('r11審查｜子集重匯保留序號：db 已有 base，單獨匯 base|#2 → 入庫且 ref 不被重編', async () => {
  await seedCard('r11f');
  const base = 'r11f|2026-01-20|55|子集咖啡';
  const r1 = await importRows('r11f', [{ date: '2026-01-20', amount: 55, desc: '子集咖啡', category: '飲食', subcategory: '', stmtRef: base }]);
  assert.equal(r1.imported, 1);
  // 使用者當初只勾了「第二杯」重匯：序號段必須原樣保留（整批重編會把它變回 base → 被去重吃掉，真消費消失）
  const r2 = await importRows('r11f', [{ date: '2026-01-20', amount: 55, desc: '子集咖啡', category: '飲食', subcategory: '', stmtRef: `${base}|#2` }]);
  assert.equal(r2.imported, 1, '只勾第二杯的子集重匯必須成功');
  const refs = ((await repo.getDb()).transactions || []).filter(t => t.stmtRef && t.stmtRef.startsWith(base)).map(t => t.stmtRef).sort();
  assert.deepEqual(refs, [base, `${base}|#2`], '序號段不可被伺服器端重編');
});

test('r11審查｜|#N 上限：|#10 合法保留、|#999999 超上限（>99）剝除重建', async () => {
  await seedCard('r11g');
  const base = 'r11g|2026-01-21|60|上限測試店';
  const r1 = await importRows('r11g', [
...
test('r11審查｜desc trim：前後補空白的變體收斂到同一指紋，不再重複入帳', async () => {
  await seedCard('r11h');
  const mk = (d) => ({ date: '2026-01-22', amount: 40, desc: d, category: '飲食', subcategory: '', stmtRef: `r11h|2026-01-22|40|${d}` });
  const r1 = await importRows('r11h', [mk('空白測試店')]);
  assert.equal(r1.imported, 1);
  const r2 = await importRows('r11h', [mk('空白測試店 '), mk(' 空白測試店 ')]);
  assert.equal(r2.imported, 0, 'trim 後同指紋 → 全部去重');
  assert.equal(r2.skipped, 2);
});

test('r11審查｜desc 含「|」整列拒收（守住「stmtRef 第 4 段起＝原文」不變式）', async () => {
  await seedCard('r11i');
  const r = await importRows('r11i', [{ date: '2026-01-23', amount: 30, desc: '怪店|#2', category: '生活', subcategory: '', stmtRef: 'r11i|2026-01-23|30|怪店|#2' }]);
...
test('r11審查｜原文＝鑰匙：applyCategoryToStore 不可把剛學的 entry 自己清掉', async () => {
  await seedCard('r11j');
  const orig = '自噬測試小館';
  const key = storeKeyOf(orig);
...
test('r11審查｜normalizeBranches 同語意：品牌層名無著落（交易全缺 stmtRef）＝保留，不無聲蒸發', async () => {
  const db = await repo.getDb();
  ...
  assert.equal((await repo.getDb()).learnedCategories['無著落小館']?.name, '我的無著落名',
