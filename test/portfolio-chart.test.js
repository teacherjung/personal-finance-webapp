...
test('投入折線圖｜台幣資料四捨五入並保留目前月份', () => {
  ...
  assert.equal(config.options.plugins.tooltip.callbacks.label({ dataset: { label: '市值' }, parsed: { y: 50001 } }), ' 市值: 5.0 萬');
});

test('投入折線圖｜美元計價先除匯率，再使用 K USD 提示', () => {
  const config = investmentChartConfig([{ month: '2026-06', cost: 32000, value: 40000 }], 64000, 80000, {
    viewCurrency: 'USD', usdRate: 32
  });
  ...
  assert.equal(config.options.plugins.tooltip.callbacks.label({ dataset: { label: '投入成本' }, parsed: { y: 2000 } }), ' 投入成本: 2.0 K USD');
});
