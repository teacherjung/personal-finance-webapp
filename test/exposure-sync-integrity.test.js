...
test('幣別曝險｜00719B 與 00720B 兩支台幣交易的美元債 ETF 都要歸到美元桶（含非正規形）', () => {
  // ⚠️ 逐字列出這兩支代號的是 **`docs/contracts/investment-sec.md` 的「台幣掛牌美債 ETF 清單」那一節**
  //    （另見台股報價那條）。`AGENTS.md` 的同步點清單那一列自 **#637** 起只剩一行索引＋連結。
  //    ⚠️ **歷史，只適用於當時**（#409 minor④）：本題那時寫「契約逐字列出」是**引錯檔案**——
  //    那時規則還住在 AGENTS 同步點清單、`docs/` 底下 grep 不到 00719B／00720B 任何一次。
  //    #637 把那一列整條搬進契約之後，**正確的來源就是上面那一節**，這段只留著解釋為什麼曾經改過。
