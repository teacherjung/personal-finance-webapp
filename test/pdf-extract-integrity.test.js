...
//    **突變實測**（把兩支抽取器的 `w` 一起換成
//    `Math.round(Math.abs(it.transform[0]) * it.str.trim().length * (ASCII ? 0.5 : 1))`——
//    `it.transform[0]` 就是那一段的字級）：本檔 **fail 11**（29 題）、全套 **fail 11**（1,516 題）；
//    銀行第 4、5 筆都從 in 翻成 out，證券 `grossAmount` 變 111、`feeDiscount` 變 `null`。
//
...
function bankStatementPdf({ splitHeader = '', detailMarker = '交易往來明細' } = {}) {
  ...
  return makeTextPdf([[
    ...
    // 第 5 筆（y=520）＝字寬邊界案例的**同字級**面，見上面 ⑤：金額用預設字級 10 印在 x=360，
    // 五個字元都是寬 1000 → 真實 w=50 → 右緣 410 落在「存入」欄；
    // 「字級 × 字數 × 0.5」=25 → 右緣 385 落回「支出」欄。帳戶 900100****3344 的唯一一列。
    ...detailRow([
      S(40, 520, '900100****3344'), S(140, 520, '2026/01/28'), S(240, 520, '轉入'),
      S(360, 520, '1,111'), S(510, 520, '$30,000'),
    ]),
    S(620, 420, '本頁小計說明'),   // 孤兒備註 C：離最近的第 5 筆 100 ＞ ORPHAN_MAX_DY(40) → 一段都不該黏上去
  ]]);
}
...
//    實測把 `pos` 改成**所有格子一律用右緣**（拿掉 `isAmt &&`）：本檔 fail 6、那一筆成交整筆消失（0 筆）
