...
//        第二筆成交的成交金額與折讓都落在欄界上。
//    **突變實測**（逐字重放審查者那一顆）：本檔 **fail 6**（30 題）、全套 **fail 6**（1,517 題）；
//    跨頁題的第 2 頁 w 變 `[0,0,0]`、銀行第 5 筆從 in 翻成 out（連帶兩題單分支對帳單題），
//    證券第 2 頁那一筆 `name` 變「台積電55,000」、`grossAmount` 變 111、`feeDiscount` 變 `null`。
//
...
for (const [name, extract] of /** @type {const} */ ([
  ...
  test(`${name}抽取器｜跨頁**每一頁**的文字、x、y、w 都要逐頁等於 fixture 指定的值`, async () => {
    // ⚠️ 縱向座標決定「孤兒備註黏到哪一筆交易」。兩頁都用同一個頁內 y=700，
    //    抽出來必須是 700 與 700-100000＝-99300；只驗「遞減」的話，人造的 -1、-2 也會過。
    // ⚠️ **r10 補的是 `w` 這一格**（r4 審查者退回的唯一一條 High）：上一版跨頁只斷言文字、x、y，
    //    而銀行與證券主入口的 fixture 全是單頁——於是兩支抽取器一起改成
    //    `w: pageBase === 0 ? Math.round(it.width || 0) : 0`（**只有第一頁保留字寬**）
    //    照樣全綠（實測修之前的樹：PDF 相關六個考題檔 139 題 `ℹ fail 0`）。
    //    `x + w` 是右對齊金額判欄的依據，所以那顆突變的實害是
    //    「多頁對帳單從第 2 頁起，金額無聲落進相鄰欄」。
    // ⚠️ 兩頁的字寬刻意**每一格都不一樣**（靠 `GLYPH_W` 讓 `1` 與 `.` 比其餘 ASCII 寬一倍）：
    //    第 1 頁 20／25／30、第 2 頁 15／20／25。所以「第 2 頁歸零」「照抄第 1 頁的 w」
    //    「字級 × 字數 × 0.5」（會說六格分別是 15／20／20／15／20／20）三種假值都對不上。
    const pdf = makeTextPdf([
      ROW(700, 'P1A', 'P1AA', 'P1.A'),
      ROW(700, 'P2A', 'P2AA', 'P2.A'),   // 第 2 頁刻意用同一個頁內 y
    ]);
    const lines = await extract(pdf, undefined);
    assert.deepEqual(lines.map((/** @type {any} */ l) => l.y), [700, 700 - PAGE_SHIFT],
      '跨頁 y 要是實際頁內 y 加上頁位移（第 2 頁 → -99300），不是任何一組「只要遞減就好」的數字');
    assert.deepEqual(lines.map((/** @type {any} */ l) => l.cells.map((/** @type {any} */ c) => c.s)),
      [['P1A', 'P1AA', 'P1.A'], ['P2A', 'P2AA', 'P2.A']],
    ...
    assert.deepEqual(lines.map((/** @type {any} */ l) => l.cells.map((/** @type {any} */ c) => c.w)),
      ...
      + '`P2A`＝500+500+500 → 15…）——第 2 頁起把 w 歸零、或拿第 1 頁的 w 套到後續頁，'
      + '這一格就對不上；那正是多頁對帳單「第 2 頁起金額靜靜漂進隔壁欄」的抽取層病根');
  });
}
...
/**
 * 一份合法的台新綜合對帳單 fixture（**兩頁**：概要區＋明細在第 1 頁、第 5 筆與一段孤兒備註在第 2 頁；
 * 金額右對齊、備註換行到相鄰列）。
 *
 * ⚠️ **下面每一項都是刻意「做成不好過」的**——每一項都對應一個**實際穿過去過**的繞法
 *    （r8 加了第四項、r9 加了第五項、r10 加了第六項，所以這裡不寫死項數）：
 *
 ...
 *
 * ⑥**第 5 筆搬到第 2 頁**（r10 加的，見檔頭⑪）：⑤ 那一格原本在第 1 頁，於是
 *   `w: pageBase === 0 ? Math.round(it.width || 0) : 0`（**只有第一頁保留字寬**）這顆突變
 *   ——r4 審查者退回的唯一一條 High——在主入口這一側完全踩不到（實測修之前的樹全綠）。
 *   現在它印在第 2 頁：w 一被第 2 頁歸零，右緣就退回左緣 360、落回支出欄，
 *   **收入被記成支出**（多頁對帳單真正的實害形狀）。這一頁刻意不重印表頭，
 *   走的就是「欄界沿用第 1 頁、明細一路掃到底」那條真實路徑。
 *
...
function bankStatementPdf({ splitHeader = '', detailMarker = '交易往來明細' } = {}) {
  ...
  return makeTextPdf([[
    ...
    // 第 4 筆（y=580）＝字寬邊界案例的**字級**面，見上面 ③：金額用字級 20 印在 x=345，
    ...
    // ── 第 2 頁（r10 加的，見上面 ⑥）──────────────────────────────────────────────
    // 第 5 筆（頁內 y=520 → 全域 y=520−100000）＝字寬邊界案例的**同字級**面，見上面 ⑤：
    // 金額用預設字級 10 印在 x=360，五個字元都是寬 1000 → 真實 w=50 → 右緣 410 落在「存入」欄；
    // 「字級 × 字數 × 0.5」=25 → 右緣 385 落回「支出」欄；**w 在第 2 頁被歸零**（r4 審查者那顆突變）
    // → 右緣退回左緣 360，也落回「支出」欄。帳戶 900100****3344 的唯一一列。
    // ⚠️ 這一頁**故意不重印明細表頭**：欄位 x 與 `inDetail` 都是第 1 頁建立的，
...
test('銀行主入口｜金額右緣剛好跨在支出／存入分界上 → 只有 PDF 的**真實字寬**判得對方向'
  + '（字級面＋同字級不同實寬面各一筆）', async () => {
  // ⚠️ 這一題釘的是 `w`。兩顆被實際用過的繞法各對應一筆：
  //    **第 4 筆（字級 20、x=345）** 對的是 Codex 審 `c2d7517` 那一輪的 `5 * it.str.trim().length`
  //      （完全不看字級）：真實 w=60 → 右緣 405 → 存入欄 [400,500) → in（正確）；
  //      估算 25 → 右緣 370 → 支出欄 → out。
  //    **第 5 筆（字級 10、x=360）** 對的是 Codex 審 `c41f8e0` 那一輪的
...
//  ⑧**跨頁只驗到「第 2 頁」，而且兩份主入口 fixture 的第 2 頁都刻意不重印表頭。**
//    - 頁數：本檔最多兩頁，`pageBase` 的位移只被「第 2 頁 ＝ −100000」這一格釘住；
//      把位移改成「只有第 2 頁推、第 3 頁起不推」之類的第三頁病，本檔不會轉紅。
...
//      本檔第 2 頁沿用第 1 頁的欄界，所以「第 2 頁表頭把欄位 x 讀歪」這種病本檔答不出來。
