...
const ROW = (y, a, b, c) => [S(ROW_X[0], y, a), S(ROW_X[1], y, b), S(ROW_X[2], y, c)];
...
for (const [name, extract] of /** @type {const} */ ([
  ...
  test(`${name}抽取器｜一列的文字與 x／w 必須逐格等於 fixture 指定的值`, async () => {
    ...
    assert.deepEqual(lines[0].cells.map((/** @type {any} */ c) => c.s), ['AAA', 'BBBB', 'CC'],
      '每一格的文字要照版面順序（x 升冪）排好');
    assert.deepEqual(lines[0].cells.map((/** @type {any} */ c) => c.x), [50, 200, 400],
      `x 要等於 fixture 指定的 ${JSON.stringify([...ROW_X])}——`
      + '座標一旦被換成人造值（歸零、或「第幾個字」這種遞增數列），支出／存入分欄與支票欄排除就整片失效');
  ...
  test(`${name}抽取器｜跨頁的 y 必須是「頁內 y 減去每頁 ${PAGE_SHIFT} 的位移」`, async () => {
    ...
    assert.deepEqual(lines.map((/** @type {any} */ l) => l.cells.map((/** @type {any} */ c) => c.s)),
      [['P1-A', 'P1-B', 'P1-C'], ['P2-A', 'P2-B', 'P2-C']], '兩頁的文字不可交叉或遺失');
    assert.deepEqual(lines.map((/** @type {any} */ l) => l.cells.map((/** @type {any} */ c) => c.x)),
      [[50, 200, 400], [50, 200, 400]], '兩頁的 x 都要照 fixture');
  });
}
...
/** 一份合法的台新綜合對帳單 fixture（概要區＋明細；金額右對齊、備註換行到下一列）。 */
function bankStatementPdf() {
  return makeTextPdf([[
    ...
    // 第 1 筆：金額右緣 315+25=340 落在「支出」欄 [300,400) → out；備註在同列
    S(40, 760, '900100****7788'), S(140, 760, '2026/01/05'), S(240, 760, '轉帳'),
    S(315, 760, '1,234'), S(510, 760, '$98,766'), S(620, 760, '轉出'),
    // 第 2 筆：金額右緣 445+25=470 落在「存入」欄 [400,500) → in；備註換行到下一列
    S(40, 740, '900100****7788'), S(140, 740, '2026/01/07'), S(240, 740, '薪轉'),
    S(445, 740, '5,000'), S(510, 740, '$103,766'),
    S(620, 730, '公司薪資'),   // 孤兒備註列：靠 y 距離（10 < 40）歸回第 2 筆
  ]]);
}

test('銀行主入口｜有效 PDF → 成功解析（不是一律 400），而且支出／存入靠 x 分欄、換行備註靠 y 歸位', async () => {
  ...
  assert.equal(r.transactions.length, 2, '明細要抽出兩筆');
  ...
  assert.equal(t2.note, '公司薪資', '換行到下一列的備註要靠 y 距離歸回第 2 筆（內轉／劃撥分箱全看備註）');
});
