...
//    型別守衛＝`帳戶概要區` AND `往來明細|交易往來`，共 4 題（兩個標記都沒有／只缺其中一個 ×2／
//    走 `交易往來` 那個分支不可誤擋）＋成功面那題順帶驗到「都在就放行」；
...
test('銀行主入口｜明細標記寫成「交易往來」（沒有「明細」二字）仍然算銀行對帳單，不可誤擋', async () => {
  // ⚠️ 守衛的第二個條件是 `往來明細|交易往來` 的**擇一**。上面兩題用的 fixture 都寫「交易往來明細」
  //    ——它同時命中兩個分支，所以把 `|交易往來` 整段刪掉，上面兩題照樣綠。這一題專釘那個分支：
  //    它一被刪，這份 fixture 就會被誤判成「不是銀行對帳單」而回 400（真帳單被擋在門外）。
  const alt = makeTextPdf([[
    S(40, 900, '台新銀行綜合對帳單'),
    S(40, 880, '新臺幣帳戶概要區'),
    S(40, 860, '交易往來'),
  ]]);
  const err = await parseBankStatement(alt, undefined).then(() => null, (/** @type {any} */ e) => e);
  assert.equal(err, null,
    `兩個標記都在（其中一個走「交易往來」那個分支）就不可以被型別守衛擋掉，實際：${err?.message}`);
});
