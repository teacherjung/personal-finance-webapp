...
//
// ⚠️ **r8 補的兩個「組合條件只釘住一角」**（自審逐條實測：刪掉那一角，全套照樣 fail 0）：
//    ⑧`isMultiMonthHeader` 是**兩條正則的擇一**，本檔只釘住「主入口拿它 throw 400」那條接線，
//      而接線題的 fixture（`115年1月~12月`）被第一條就吃掉了：第二條
//      `/年\d{1,2}月[連接號]/` 整條刪掉全套照樣綠。它唯一守得住的是「115年1月~116年3月」
//      這種兩邊都印年的跨年單。⇒ 兩條正則各自的 fixture 補在
//      `test/taishin-securities.test.js`（偵測器那一半的家），本檔仍只管接線。
//    ⑨銀行孤兒備註的收件條件是「y 夠近」**AND**「左緣落在備註欄」，r7 版只有前一半有 fixture 撐著
//      （`c.every(x => x.x >= xNote - 70)` 拿掉仍全綠）。⇒ `bankStatementPdf` 加了一列
//      低 x 的非交易文字，見該函式說明 ④（補上之後同一個拿掉，本檔 fail 1）。
//
 ...
 *
 * ④**一列「低 x 的非交易文字」（`本期小計`，x=40）貼在第 1 筆下方 10pt**（r8 加的）。
 *   孤兒備註的收件條件是**兩半**：`c.every(x => x.x >= xNote - 70)`（左緣要落在備註欄）
 *   AND 之後那道 `bd <= ORPHAN_MAX_DY`（y 要夠近）。r7 版只有第二半有 fixture 撐著——
 *   自審實測把前半整段拿掉，全套照樣 fail 0。拿掉之後真帳單明細區的小計列、頁尾文字
 *   會全部變成備註、黏到 40pt 內的交易上，而備註正是內轉／劃撥分箱的判準
 *   （`lib/bank-statement.js` 那句「漏收會把百萬劃撥誤當收入」的另一面：亂收也一樣會分錯箱）。
 *   這一列的 y 距離刻意做成「只要收件條件一鬆就一定黏得上」（10 ＜ `ORPHAN_MAX_DY`＝40），
 *   所以它問的純粹是 x 那一半。
 *
...
function bankStatementPdf({ splitHeader = '', detailMarker = '交易往來明細' } = {}) {
  ...
  return makeTextPdf([[
    ...
    // 明細區的小計列（x=40，遠離備註欄 620）：離第 1 筆只有 10 ＜ ORPHAN_MAX_DY(40)，
    // 所以擋住它的**只剩**「左緣要落在備註欄」那一半條件——見上面 ④。
    S(40, 750, '本期小計'),
...
test('銀行主入口｜有效 PDF → 成功解析（不是一律 400），而且支出／存入靠 x 分欄、'
  + '換行備註靠 y 與備註欄左緣（不是列序）歸位', async () => {
  ...
  //    ⑥（r8 加的）孤兒備註的收件條件是「y 夠近」**AND**「左緣落在備註欄」，r7 只有前者有考題撐著。
  ...
  // 先問「哪些列**算得上**備註」，再問「每段備註歸哪一筆」——收件條件是兩半，這是 x 那一半。
  assert.ok(!r.transactions.some(t => t.note.includes('本期小計')),
    '⑥孤兒備註的收件條件還有**左緣要落在備註欄**那一半：印在 x=40 的「本期小計」離第 1 筆只有 10'
    + '（＜ORPHAN_MAX_DY 40），y 那一半攔不住它——`c.every(x => x.x >= xNote - 70)` 一拿掉，'
    + '真帳單明細區的小計列與頁尾文字會全部變成備註黏上鄰近交易，而備註是內轉／劃撥分箱的判準');
...
//  ⑤孤兒備註的**兩個門檻值本身都沒有考題**，被釘住的只有「這一份 fixture 的實際距離兩邊分得開」：
//    y 上限（`ORPHAN_MAX_DY`）＝10 要收、100 不可收（實測改成 1e9 或 5 都會轉紅），
//    但 40 這個數字改成 30 或 80 這種「還是 10<cap<100」的值，本檔不會轉紅。
//    x 條件（`x >= xNote - 70`）同理：fixture 的孤兒備註在 x=620、非備註的小計列在 x=40，
//    `xNote`＝620 → 門檻 550，中間差得很開，所以 **`70` 這個寬容值換成 0 或 300 本檔照樣綠**
//    （整條拿掉才會轉紅——那正是 ④ 那一列在守的）。真帳單的備註若稍微左偏出備註欄，
//    要靠這個 70 收得回來，而它有多寬才對，本檔答不出來。
