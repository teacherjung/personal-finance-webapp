...
//      改成 `r = cell.x`，本檔 fail 3、第 4 與第 5 筆都從 in 翻成 out——第 4 筆左緣 345 在支出欄、
//      右緣 405 才在存入欄；第 5 筆左緣 360、右緣 410）。
...
//      （只含 `往來明細` ／只含 `交易往來`，各自附 fixture 自檢），並斷言真的抽出帳戶與每一筆交易。
//
...
//
// ⚠️ **r9 修的繞法⑩：`w` 還是可以完全不讀 PDF 字寬**（Codex 審 `c41f8e0` 那一輪退回的唯一一條 High）：
//    r8 版的 fixture 把**所有** ASCII 的字寬宣告成 500、所有中文宣告成 1000，於是兩支抽取器的
//    `it.width` 一起換成 `fontSize × 字數 × (ASCII ? 0.5 : 1)`（人工估算，完全不讀 PDF 的字寬）
//    ——本檔 29/29、全套 1,516 題照樣 fail 0。⑥那一版補的「兩種字級」與「一格中文」只驗到
//    「w 隨字級縮放」與「ASCII／中文分類」，**沒有一題驗到「同字級、同字數、不同 glyph 實寬」**。
//    ⇒ 這一版 `/Widths` 讓 `,` `.` `1` 寬 1000（其餘 ASCII 500）、中文的「小」寬 500（其餘中文 1000）
//      ——見 `GLYPH_W`（那張表是 fixture 自己編的，不是真字型的量測值），然後：
//      - 座標題多兩組**同字級、同字數、實寬不同**的對照：ASCII 三格 `148`/`111`/`AAA` → 20/30/15，
//        中文兩格 `存入金額`/`本期小計` → 40/35（那條估算會說 15/15/15 與 40/40）。
//      - **主入口的欄界上各放一格**（這才是實害那一半）：銀行第 5 筆的金額 `1,111`（字級 10、5 字、
//        實寬 50）印在 x=360——真右緣 410 落在存入欄，估算的 25 → 右緣 385 落回支出欄（收入記成支出）；
//        證券的折讓 `111`（字級 10、3 字、實寬 30）印在 x=275——真右緣 305 落在折讓欄，
//        估算的 15 → 右緣 290 落回成交金額欄（折讓變 null、成交金額變 111）。
//    **突變實測**（把兩支抽取器的 `w` 一起換成「字級 × 逐字 (ASCII 0.5 / 中文 1.0)」的人工估算——
//    `it.transform[0]` 就是那一段的字級）：**r10 重測**本檔 **fail 14**（30 題）、
//    全套 **fail 14**（1,517 題）；銀行第 4、5 筆都從 in 翻成 out，
//    證券 `grossAmount` 變 111、`feeDiscount` 變 `null`。
//    （r9 當時記的是 fail 11／29 題／1,516 題——那是 r10 加考題之前的數字。）
//
// ⚠️ **r10 修的繞法⑪：`w` 只在第一頁抽對就夠了**（獨立審查者 r4 退回的唯一一條 High）。
//    審查者把兩支抽取器改成 `w: pageBase === 0 ? Math.round(it.width || 0) : 0`
//    （＝**只有第一頁保留字寬、第二頁起一律 0**），他報 PDF／銀行／證券／隔離與上限相關的 125 題全綠。
//    **我逐字重放過**（修之前的樹）：PDF 相關六個考題檔共 **139 題、`ℹ fail 0`**
//    ——確認是完整假綠，不是模組崩潰型假紅。（那一輪我只量了這六個檔，沒量全套。）病根兩層：
//      - 本檔既有的跨頁考題只斷言第二頁的**文字、x、y**，`w` 沒人看；
//      - 銀行與證券主入口的 fixture **全是單頁**，所以端到端那一側根本踩不到第二頁。
//    實害：`x + w` 是右對齊金額判欄的唯一依據（銀行入／出帳＝`lib/bank-statement.js` 的
//    `r = cell.w ? cell.x + cell.w : cell.x`；證券成交金額／折讓＝`lib/taishin-securities.js` 的
//    `pos = isAmt && cell.w ? cell.x + cell.w : cell.x`）⇒ **多頁對帳單從第 2 頁起，
//    金額會無聲落進相鄰欄**，現金流方向與成交金額一起錯掉。
//    ⇒ r10 那一版三件事（**前兩件已被下面的⑫再擴一頁**，這裡留的是當時的處置）：
//      - 跨頁考題**逐頁斷言 w**，而且兩頁**每一格的字寬都不同**（第 1 頁 20／25／30、
//        第 2 頁 15／20／25），所以「第 2 頁歸零」「照抄第 1 頁」「字級 × 字數 × 0.5」都對不上。
//      - **銀行的第 5 筆搬到第 2 頁**（欄界上的 `1,111`，見 `bankStatementPdf` ⑥）——
//        端到端那一側從此踩得到第二頁。
//      - **證券多一份可選的第 2 頁**（`securitiesStatementPdf({ secondPage: true })`），
//        第二筆成交的成交金額與折讓都落在欄界上（這一份 r11 沒動，仍是兩頁）。
//    **突變實測**（逐字重放審查者那一顆；r11 換 fixture 後重測，數字沒變）：本檔 **fail 6**（30 題）、
//    全套 **fail 6**（1,517 題）；跨頁題第 2、3 頁的 w 變 `[0,0,0]`、銀行第 4、5 筆從 in 翻成 out
//    （連帶兩題單分支對帳單題），證券第 2 頁那一筆 `name` 變「台積電55,000」、
//    `grossAmount` 變 111、`feeDiscount` 變 `null`。
//
// ⚠️ **r11 修的繞法⑫：「只在前 N 頁抽對」這一族，r10 只補了 N=1**（獨立審查者實測退回）。
//    把 r4 那顆 High 逐字**往後挪一頁**——兩支抽取器一起改成
//    `w: pageBase > -200000 ? Math.round(it.width || 0) : 0`（＝第 1、2 頁保留 PDF 字寬、第 3 頁起歸零）
//    ——修之前的樹是**完整假綠**（審查者實測 PDF 相關六個考題檔 140 題 `ℹ fail 0`）。
//    病根：r10 的跨頁考題只有兩頁，銀行與證券主入口 fixture 也只到第 2 頁。
//    實害與 r4 那條 High 一模一樣、只是往後挪一頁：**三頁以上的對帳單從第 3 頁起**，
//    右對齊金額的右緣退回左緣、靜靜落進相鄰欄（銀行入／出帳翻面＝收入記成支出；
//    證券成交金額與折讓整組左移一欄），而且不報錯。真實銀行／證券對帳單三頁以上是常態。
//    ⇒ 這一版三件事：
//      - 跨頁考題**擴到三頁**、三頁每一格字寬都不同（20/25/30、15/20/25、35/40/45），逐頁斷言 w。
//      - 銀行主入口 fixture 也擴到三頁：**第 4 筆（欄界金額的字級面）放第 2 頁、
//        第 5 筆（欄界金額的同字級不同實寬面）放第 3 頁**——N=1 讓兩筆都翻面、N=2 讓第 5 筆翻面。
//      - 第 2 頁多一段「不該黏」的孤兒備註（見 `bankStatementPdf` ⑦），用來釘下面那一顆。
//    **突變實測**（逐字重放審查者那一顆 N=2）：本檔 **fail 5**（30 題）、全套 **fail 5**（1,517 題）；
//    跨頁題第 3 頁的 w 變 `[0,0,0]`、銀行第 5 筆從 in 翻成 out（連帶兩題單分支對帳單題）。
//
// ⚠️ **r11 補的繞法⑬：「跨頁 y 單調」也只驗到第 2 頁**（同一位審查者的第二顆）。
//    `pageBase -= 100000` 改成 `pageBase = -100000`（遞減改賦值，兩支一起）＝第 2 頁以後所有頁
//    共用同一個位移，程式自己註解保證的「跨頁 y 單調」就壞了——而修之前**全套 1,517 題 `ℹ fail 0`**。
//    實害（審查者拿三頁合成銀行對帳單實跑量到的）：第 3 頁的備註會落進第 2 頁某一筆的
//    `ORPHAN_MAX_DY`(40) 內而黏錯筆；證券側同理，第 3 頁的第二行會落進第 2 頁 pending 第一行的
//    `DY_MAX`(40) 內而錯併成一筆。備註是內轉／劃撥分箱的判準（`lib/bank-statement.js` 那句
//    「漏收會把百萬劃撥誤當收入」的另一面：亂收一樣分錯箱）。
//    ⇒ 兩處各釘一半：抽取層逐頁斷言「每翻一頁只推**一個**位移」（第 3 頁 ＝ 頁內 y − 2×位移）；
//      端到端靠第 2 頁那段「承前頁」——它跟第 3 頁第 5 筆的頁內 y 只差 20，位移一被共用就黏上去。
//    **突變實測**：全套 **fail 3**（1,517 題）——兩支抽取器的跨頁題，加上銀行的欄界金額題
//    （第 5 筆的 `note` 變「承前頁」）。
//
// ⚠️ **r11 放寬了一個假紅**（同一位審查者標出來的）：跨頁題原本寫死 `PAGE_SHIFT = 100_000`
//    逐格比對絕對 y，於是把位移量從 100000 換成 200000（對所有下游**完全等價**：契約只要求
//    「大於任何單頁 y 範圍」，`ORPHAN_MAX_DY`／`DY_MAX` 兩個 40pt 門檻怎麼比都一樣）也會紅（fail 2）。
//    現在位移量**由 fixture 自己量**（第 1→2 頁的落差），只斷言三件事：①第 1 頁不加位移
//    ②位移大於整頁高度 ③每翻一頁只推一個位移。實測 `pageBase -= 200000` 現在全套 `fail 0`；
//    非等量位移（`-= 100000 * i`）仍會紅，那是刻意的——理由與代價寫在檔尾誠實劃界⑧。
//
 ...
 *
 * ⚠️ **刻意做成「同一類字元也不等寬」**（r9 加的，見檔頭⑩）：上一版是「ASCII 一律 500、中文一律 1000」，
 *    於是 `fontSize × 字數 × (ASCII ? 0.5 : 1)` 這種**完全不讀 PDF 字寬**的人工估算每一格都剛好等於真值。
 *    現在 `GLYPH_W` 讓幾個 glyph 跟同類其他字不一樣，「同字級、同字數、實寬不同」才變成可測的。
 */
const W_ASCII = 500, W_CJK = 1000;
/**
 * 例外字寬：這幾個 glyph 刻意跟同類其他字**不同寬**（`,` `.` `1` 是其餘 ASCII 的兩倍、
 * 中文的「小」是其餘中文的一半）。
 *
 * ⚠️ 這張表是 fixture 自己編的，**不是任何真字型的量測值**（真比例字型的 `,` 反而比數字窄）。
 * 它唯一的用途是讓「抽取器有沒有真的讀 PDF 宣告的字寬」變成可測；差距刻意放大到兩倍／一半，
 * 是為了讓斷言裡的算術好對，不是在模擬台新的版面。
 */
const GLYPH_W = /** @type {Record<string, number>} */ ({ ',': 1000, '.': 1000, '1': 1000, '小': 500 });
/** 單一字元的字寬（千分之一 em）。 @param {string} ch */
const glyphW = (ch) => GLYPH_W[ch] ?? ((ch.codePointAt(0) || 0) > 0x7e ? W_CJK : W_ASCII);
/** 預設字級。ASCII 一個字 5pt、中文一個字 10pt（字寬 × 字級 ÷ 1000；`GLYPH_W` 那幾個字例外）。 */
const FONT_SIZE = 10;
/** 大字級（＝預設的兩倍）。**每一段文字都可以自己指定字級**，這是「w 不能用人造值頂替」的一半：
 *  同一段文字換個字級，抽出來的 `w` 必須跟著變，`5 × 字數` 這種只看字數的假值就對不上了。
 *  （另一半是「同字級也不等寬」——那要靠 `GLYPH_W`。） */
const FONT_SIZE_BIG = 20;
/** 小字級。同理，用在證券主入口的邊界案例（同一段文字改用小字級印，`w` 要跟著縮，
 *  右緣才落得進正確的欄位；只看字數的假值會把它算得太寬）。 */
 ...
 *
 * `MEDIA_H`（整頁高度）另有一個用途：跨頁題拿它當「頁位移必須大於整頁 y 範圍」的下界
 * ——見那一題的 y 斷言。
 */
const MEDIA_H = 1000;
const MEDIA_BOX = `[0 0 1000 ${MEDIA_H}]`;
...
function makeTextPdf(pages) {
  ...
  /** @type {Map<number, string>} 自訂字碼 → 非 ASCII 字元（回查字寬用） */
  const byCode = new Map([...glyphs].map(([ch, code]) => [code, ch]));
  /** @type {number[]} */
  const widths = [];
  for (let c = 32; c <= lastCode; c++) {
    const ch = c >= 0x80 ? byCode.get(c) : String.fromCharCode(c);
    widths.push(ch ? glyphW(ch) : W_CJK);   // 沒映射到的字碼（不會發生）給預設值，免 /Widths 少一格
  }
...
for (const [name, extract] of /** @type {const} */ ([
  ...
  test(`${name}抽取器｜文字與 x／w 必須逐格等於 fixture 指定的值（含**兩種字級、同字級不同實寬、中文**），且**照 x 升冪重排**`, async () => {
    // ⚠️ r1 版只驗「互異且遞增」，Codex 用「x＝該列已收字數+1」的人造遞增值就繞過去了。
    //    所以這裡改成**比對實際數字**：文字 ['AAA','BBBB','CC']、x [50,200,400]、
    //    w [15,20,10]（這幾個字的字寬都是預設 500 → 一個字 5pt）。
    //    Codex 那種「第幾個字」的人造數列同時對不上這三組。
    ...
    //    於是「w ＝ 字數 × 5」這種完全不讀 PDF 的假值剛好每一格都對。⇒ 讓同一段文字**以兩種字級進場**
    //    （第 2 列字級 20 → w 要翻倍）外加一格中文。
    // ⚠️ r9 版又被抓到同一個病的下一層（檔頭⑩）：兩種字級與中文只驗到「隨字級縮放」與
    //    「ASCII／中文分類」，所以 `fontSize × 字數 × (ASCII ? 0.5 : 1)` 這種人工估算仍然每一格都對。
    //    ⇒ 第 3 列是**同字級（10）、同字數（3 字）、實寬各不同**的三段文字（見 `GLYPH_W`）：
    //      '148'（`1` 寬 1000）→ 20／'111'（三個都是 `1`）→ 30／'AAA'（都是預設 500）→ 15；
    //      第 4 列是**同字數（4 字）的兩段中文**：'存入金額' → 40／'本期小計'（「小」寬 500）→ 35。
    //      那條估算會說這 5 格分別是 15／15／15／40／40，五格錯四格。
    const pdf = makeTextPdf([[
      ...ROW(700, 'AAA', 'BBBB', 'CC'),                    // 字級 10、字寬都是預設 → w＝字數 × 5
      ...ROW(650, 'AAA', 'BBBB', 'CC', FONT_SIZE_BIG),     // **同樣的文字**、字級 20 → w 整組翻倍
      ...ROW(600, '148', '111', 'AAA'),                    // **同字級、同字數、實寬不同** → 20／30／15
      S(50, 550, '存入金額'), S(200, 550, '本期小計'),        // 中文同字數不同實寬 → 40／35
    ]]);
    const lines = await extract(pdf, undefined);
    assert.equal(lines.length, 4, `應該抽出四列（實際 ${JSON.stringify(lines).slice(0, 400)}）`);
    assert.deepEqual(lines.map((/** @type {any} */ l) => l.y), [700, 650, 600, 550],
      'y 要等於 fixture 指定的值、且由上而下');
    assert.deepEqual(lines.map((/** @type {any} */ l) => l.cells.map((/** @type {any} */ c) => c.s)),
      [['AAA', 'BBBB', 'CC'], ['AAA', 'BBBB', 'CC'], ['148', '111', 'AAA'], ['存入金額', '本期小計']],
      '每一格的文字要照版面順序（x 升冪）排好——前三列是**逆序**寫進內容串流的（右→中→左），'
      + '抽取器少了依 x 排序就會回 [CC, BBBB, AAA]');
    assert.deepEqual(lines.map((/** @type {any} */ l) => l.cells.map((/** @type {any} */ c) => c.x)),
      [[50, 200, 400], [50, 200, 400], [50, 200, 400], [50, 200]],
    ...
    assert.deepEqual(lines.map((/** @type {any} */ l) => l.cells.map((/** @type {any} */ c) => c.w)),
      [[15, 20, 10], [30, 40, 20], [20, 30, 15], [40, 35]],
      'w 要是**PDF 裡逐字加總的實際字寬**（字寬 × 字級 ÷ 1000），不是任何一組只看字數的人造值：'
      + '①第 1 列與第 2 列是同樣的三段文字、只差字級（10 → 20），w 必須整組翻倍；'
      + '②第 3 列三格**同字級、同字數（3 字）**，w 卻是 20／30／15（`GLYPH_W` 讓 `1` 寬一倍）——'
      + '「字數 × 字級 × 0.5」這種估算會說三格都是 15；'
      + '③第 4 列兩格中文同樣 4 個字，w 是 40／35（「小」只有一半寬），估算會說兩格都是 40。'
      + 'w 是右對齊金額判欄的依據（右緣 ＝ x + w），假的 w 會讓金額靜靜漂到隔壁欄');
  });

  test(`${name}抽取器｜跨頁**每一頁**（三頁）的文字、x、w 都要逐頁等於 fixture 指定的值，`
    + `y 要是「頁內 y − 每頁一個大位移」`, async () => {
    // ⚠️ 縱向座標決定「孤兒備註黏到哪一筆交易」。三頁都用同一個頁內 y=700，
    //    所以「只要遞減就好」的人造值（-1、-2、-3）與「頁內 y 被丟掉、只剩位移」都對不上。
    // ⚠️ **r10 補的是 `w` 這一格**（r4 審查者退回的唯一一條 High）：r9 版跨頁只斷言文字、x、y，
    //    而銀行與證券主入口的 fixture 全是單頁——於是兩支抽取器一起改成
    //    `w: pageBase === 0 ? Math.round(it.width || 0) : 0`（**只有第一頁保留字寬**）
    //    照樣全綠。`x + w` 是右對齊金額判欄的依據，所以那顆突變的實害是
    //    「多頁對帳單從第 2 頁起，金額無聲落進相鄰欄」。
    // ⚠️ **r11 補的是「第三頁」**（獨立審查者實測，見檔頭⑫）：r10 只補到兩頁，於是同一族的
    //    `w: pageBase > -200000 ? Math.round(it.width || 0) : 0`（＝**前兩頁**保留字寬、第 3 頁起歸零）
    //    照樣全綠——把上面那顆 High 往後挪一頁就完全踩不到。三頁下來，「只有前 N 頁抽對」
    //    N=1 與 N=2 兩種寫法都對不上。
    // ⚠️ 三頁的字寬刻意**每一格都不一樣**（靠 `GLYPH_W` 讓 `1` 與 `.` 比其餘 ASCII 寬一倍）：
    //    第 1 頁 20／25／30、第 2 頁 15／20／25、第 3 頁 35／40／45。所以「某一頁起歸零」
    //    「照抄第 1 頁的 w」「字級 × 字數 × 0.5」（會說九格分別是 15／20／20、15／20／20、25／30／30）
    //    三種假值都對不上。
    const pdf = makeTextPdf([
      ROW(700, 'P1A', 'P1AA', 'P1.A'),
      ROW(700, 'P2A', 'P2AA', 'P2.A'),            // 第 2、3 頁刻意都用同一個頁內 y
      ROW(700, 'P3.1A', 'P3.1AA', 'P3.11A'),
    ]);
    const lines = await extract(pdf, undefined);
    assert.equal(lines.length, 3, `三頁各一列（實際 ${JSON.stringify(lines).slice(0, 400)}）`);
    // ── y：釘住抽取器**自己寫的契約**（`pageBase` 那兩行註解：「每頁往下推一個大位移，
    //    讓後頁 y 一定小於前頁」「大於任何單頁 y 範圍」），而**不釘位移量本身**。
    //    ⚠️ r10 版是寫死 `PAGE_SHIFT = 100_000` 逐格比對絕對值，於是把位移量從 100000 換成 200000
    //       這種對所有下游完全等價的改寫也會讓這題紅（自審實測 fail 2）＝假紅。位移量只要
    //       「大於整頁 y 範圍」，`ORPHAN_MAX_DY`／`DY_MAX` 那兩個 40pt 門檻怎麼比都一樣。
    //    所以改成：①第 1 頁不加位移（絕對值錨在這裡）②位移量由第 1→2 頁自己量出來、
    //    必須大於整頁高度 ③每翻一頁只推**一個**位移（第 3 頁 ＝ 700 − 2×位移）。
    //    ③ 這一條就是「第 2 頁以後所有頁共用同一個位移」（`pageBase = -100000` 這種
    //    遞減改賦值）會踩到的地方——那顆突變讓第 3 頁的備註黏到第 2 頁的交易上。
    const ys = lines.map((/** @type {any} */ l) => l.y);
    assert.equal(ys[0], 700, '第 1 頁不加位移：y 就是 fixture 指定的頁內實際值（人造 y 在這裡就對不上）');
    const shift = ys[0] - ys[1];
    assert.ok(shift > MEDIA_H,
      `頁位移（實測 ${shift}）必須大於整頁高度 ${MEDIA_H}——位移一小於單頁 y 範圍，`
      + '後頁的列就會落進前頁列的 ORPHAN_MAX_DY(40) 內，換行備註跨頁亂黏（備註是內轉／劃撥分箱的判準）');
    assert.deepEqual(ys, [700, 700 - shift, 700 - 2 * shift],
      '每翻一頁要往下推**一個**位移、頁內 y 原樣保留（第 3 頁 ＝ 頁內 700 − 2×位移）：'
      + '第 2 頁以後共用同一個位移（例如把 `pageBase -= 100000` 寫成 `pageBase = -100000`）的話，'
      + '第 3 頁與第 2 頁的落差會是 0，第 3 頁的備註就會黏到第 2 頁的交易上——而且不報錯');
    assert.deepEqual(lines.map((/** @type {any} */ l) => l.cells.map((/** @type {any} */ c) => c.s)),
      [['P1A', 'P1AA', 'P1.A'], ['P2A', 'P2AA', 'P2.A'], ['P3.1A', 'P3.1AA', 'P3.11A']],
      '三頁的文字不可交叉或遺失，而且各自照 x 升冪重排（fixture 三頁都是逆序寫進內容串流的）');
    assert.deepEqual(lines.map((/** @type {any} */ l) => l.cells.map((/** @type {any} */ c) => c.x)),
      [[50, 200, 400], [50, 200, 400], [50, 200, 400]], '三頁的 x 都要照 fixture，而且都是升冪');
    assert.deepEqual(lines.map((/** @type {any} */ l) => l.cells.map((/** @type {any} */ c) => c.w)),
      [[20, 25, 30], [15, 20, 25], [35, 40, 45]],
      '**每一頁**的 w 都要是該頁逐字加總的實際字寬（`P1A`＝500+1000+500 → 20、'
      + '`P2A`＝500+500+500 → 15、`P3.1A`＝500+500+1000+1000+500 → 35…）——'
      + '從第 2 頁或第 3 頁起把 w 歸零、或拿第 1 頁的 w 套到後續頁，這一格就對不上；'
      + '那正是多頁對帳單「某一頁起金額靜靜漂進隔壁欄」的抽取層病根');
  });
}
...
/**
 * 一份合法的台新綜合對帳單 fixture（**三頁**：概要區＋明細表頭＋前三筆在第 1 頁、第 4 筆與一段
 * 孤兒備註在第 2 頁、第 5 筆與一段孤兒備註在第 3 頁；金額右對齊、備註換行到相鄰列）。
 *
 * ⚠️ **下面每一項都是刻意「做成不好過」的**——每一項都對應一個**實際穿過去過**的繞法
 *    （r8 加了第四項、r9 加了第五項、r10 加了第六項、r11 把第六項擴到三頁並加了第七項，
 *    所以這裡不寫死項數）：
 *
 ...
 *
 * ②**五筆交易、三段孤兒備註**：A 在它那筆（第 2 筆）的**下方**、B 在它那筆（第 3 筆）的**上方**
 *   （`lib/bank-statement.js` 檔頭自己註明備註可能印在交易列上方）、C 離所有交易列都太遠（一筆都不該黏）。
 *   r2 版只有兩筆、孤兒備註剛好貼著最後一筆，所以「完全不看 y、一律黏到最後一筆」的寫法照樣全綠；
 *   這一版 A 與 B 歸的都**不是**最後一筆（最後一筆是第 5 筆）、也不是第一筆，那種寫法兩處都會答錯。
 *   y 距離是承重的：黏錯的話，真帳單上「第 5 筆的劃撥」會跑到「第 30 筆」，
 *   正是 `lib/bank-statement.js` 那句「漏收會把百萬劃撥誤當收入」。
 *   A、B 到「自己那筆」的 y 距離都是 10、到相鄰交易列都 ≥50（＞`ORPHAN_MAX_DY`＝40），
 *   所以「靠 y 最近」與「靠列序（第一筆／最後一筆／前一列／後一列）」四種寫法在這份 fixture 上答案全不同。
 *
 * ③**第 4 筆是「只有真實字寬 `w` 才判得對」的邊界案例之一：字級那一面**（Codex 審 `c2d7517`
 *   那一輪的繞法：把兩支抽取器的 `w` 都換成 `5 * it.str.trim().length`，完全不讀 PDF 的字寬，
 *   全套考題照樣綠）。它的金額 `2,468` 用**字級 20** 印在 x=345，左緣刻意擺在支出欄裡：
 *     真實 `w`＝60（`,` 寬 1000，見 `GLYPH_W`）→ 右緣 405 落在存入欄 [400,500) → in（正確）
 *     「字數 × 5」＝25（不看字級）        → 右緣 370 落回支出欄 [300,400) → out
 *     「字級 × 字數 × 0.5」＝50（看字級、不看字寬）→ 右緣 395 也落回支出欄 → out
 *   它同時是**自己那個帳戶（…1122）的唯一一列**，所以 running 餘額校正救不回來——方向錯就是錯。
 *
 ...
 *   所以它問的純粹是 x 那一半。
 *
 * ⑤**第 5 筆是 ③ 那個邊界案例的「同字級」那一面**（r9 加的，見檔頭⑩）：金額 `1,111` 用**預設字級 10**
 *   印在 x=360，五個字元（`1` 與 `,`）在 `GLYPH_W` 裡都是寬 1000 →
 *     真實 `w`＝50                   → 右緣 410 落在存入欄 → in（正確）
 *     `字級 × 字數 × 0.5`＝25（不讀字寬）→ 右緣 385 落回支出欄 → out（**收入被記成支出**）
 *   它跟第 1 筆的 `1,234`（同字級 10、同樣 5 個字元，實寬只有 35）是一組**同字級、同字數、
 *   不同實寬**的對照：只看字數的估算會說這兩格一樣寬，PDF 宣告的 `/Widths` 說不一樣。
 *   它屬於帳戶 …3344 的唯一一列，同樣沒有前一列餘額可以把方向校正回來。
 *
 * ⑥**兩個欄界金額分別擺在第 2 頁與第 3 頁**（r10 搬到第 2 頁、r11 擴到第 3 頁；見檔頭⑪⑫）：
 *   ③⑤ 那兩格原本都在第 1 頁，於是
 *   `w: pageBase === 0 ? Math.round(it.width || 0) : 0`（**只有第一頁保留字寬**）這顆突變
 *   ——r4 審查者退回的唯一一條 High——在主入口這一側完全踩不到。r10 把第 5 筆搬到第 2 頁補上了
 *   N=1；但同一族的 `pageBase > -200000 ? … : 0`（**前兩頁**保留字寬）又照樣全綠，所以 r11 把
 *   第 4 筆放第 2 頁、第 5 筆放第 3 頁：**N=1 與 N=2 兩種寫法各有一筆會從 in 翻成 out**
 *   （w 一歸零，右緣就退回左緣、落回支出欄＝**收入被記成支出**，多頁對帳單真正的實害形狀）。
 *   第 2、3 頁刻意都不重印表頭，走的就是「欄界沿用第 1 頁、明細一路掃到底」那條真實路徑。
 *
 * ⑦**第 2 頁有一段「不該黏」的孤兒備註「承前頁」**（r11 加的，見檔頭⑬）：它印在備註欄、頁內 y=500，
 *   正確行為下離同頁的第 4 筆（580）差 80、離第 3 頁的第 5 筆（頁內 520）差一整個頁位移，
 *   所以一筆都不該收。而「第 2 頁以後所有頁共用同一個位移」那顆突變
 *   （`pageBase -= 100000` 寫成 `pageBase = -100000`）會把它跟第 5 筆的距離算成 20 ≤ ORPHAN_MAX_DY(40)
 *   → 靜靜黏成第 5 筆的備註。備註是內轉／劃撥分箱的判準（`lib/bank-statement.js` 自己那句
 *   「漏收會把百萬劃撥誤當收入」的另一面：亂收一樣分錯箱），所以這是實害不是潔癖。
 *
...
function bankStatementPdf({ splitHeader = '', detailMarker = '交易往來明細' } = {}) {
  ...
  return makeTextPdf([[
    ...
    S(40, 835, '台幣活存'), S(200, 835, '900100****1122'), S(360, 835, '$50,000'),
    S(40, 825, '台幣活存'), S(200, 825, '900100****3344'), S(360, 825, '$30,000'),
    S(40, 815, '合計'),
    S(40, 800, detailMarker),
    // 明細表頭：欄位 x ＝ 支票號碼 200／支出金額 300／存入金額 400／帳戶餘額 500／備註 620
    ...H(200, '支票號碼'), ...H(300, '支出金額'), ...H(400, '存入金額'),
    ...H(500, '帳戶餘額'), ...H(620, '備註'),
    // 第 1 筆（y=760）：金額實寬 35（`1` 與 `,` 各寬 1000）→ 右緣 315+35=350 落在「支出」欄 [300,400)
    // → out；備註在同列，不是孤兒。它同時是 ⑤ 那組「同字級同字數不同實寬」對照的另一半。
    ...detailRow([
    ...
    S(40, 750, '本期小計'),
    // 第 2 筆（y=700）：金額右緣 445+30=475 落在「存入」欄 [400,500) → in；備註換行到**下方**
    ...detailRow([
    ...
    // ── 第 2 頁（r10 起；r11 把第 4 筆搬進來，見上面 ⑥）─────────────────────────────
    // 第 4 筆（頁內 y=580）＝字寬邊界案例的**字級**面，見上面 ③：金額用字級 20 印在 x=345，
    // 真實 w=60 → 右緣 405 落在「存入」欄；「字數 × 5」=25 → 370、「字級 × 字數 × 0.5」=50 → 395，
    // 兩種估算都落回「支出」欄。它是帳戶 900100****1122 的唯一一列，沒有前一列餘額可以校正方向。
    // ⚠️ 這一頁與第 3 頁都**故意不重印明細表頭**：欄位 x 與 `inDetail` 都是第 1 頁建立的，
    //    `parseBankDetail` 一路掃到底（它沒有頁的概念），所以這正是「跨頁沿用同一組欄界」的真實路徑。
    ...detailRow([
      S(40, 580, '900100****1122'), S(140, 580, '2026/01/25'), S(240, 580, '轉入'),
      S(345, 580, '2,468', FONT_SIZE_BIG), S(510, 580, '$50,000'),
    ]),
    // 孤兒備註 D「承前頁」（r11 加的，見上面 ⑦）：印在備註欄（x=620）、頁內 y=500。
    // 正確行為下它一段都不該黏：同頁的第 4 筆差 80（＞ORPHAN_MAX_DY 40），第 3 頁的第 5 筆差 10 萬量級。
    // 但「第 2 頁以後所有頁共用同一個位移」那顆突變（`pageBase -= 100000` 寫成 `pageBase = -100000`）
    // 會讓它跟第 3 頁的第 5 筆（頁內 520）只差 20 → 靜靜黏成第 5 筆的備註。
    S(620, 500, '承前頁'),
  ], [
    // ── 第 3 頁（r11 加的，見上面 ⑥）──────────────────────────────────────────────
    // 第 5 筆（頁內 y=520 → 全域 y ＝ 520 − 2×頁位移）＝字寬邊界案例的**同字級**面，見上面 ⑤：
    // 金額用預設字級 10 印在 x=360，五個字元都是寬 1000 → 真實 w=50 → 右緣 410 落在「存入」欄；
    // 「字級 × 字數 × 0.5」=25 → 右緣 385 落回「支出」欄；**w 從第 2 或第 3 頁起被歸零**
    // → 右緣退回左緣 360，也落回「支出」欄。帳戶 900100****3344 的唯一一列。
    ...detailRow([
      S(40, 520, '900100****3344'), S(140, 520, '2026/01/28'), S(240, 520, '轉入'),
      S(360, 520, '1,111'), S(510, 520, '$30,000'),
    ]),
    // 孤兒備註 C：跟第 5 筆同一頁、頁內距離 100 ＞ ORPHAN_MAX_DY(40) → 一段都不該黏上去
    //（跟前兩頁那四筆的距離是 10 萬量級，更不可能黏）
    S(620, 420, '本頁小計說明'),
  ]]);
}

/** 這份 fixture 抽出來的五筆交易（日期／方向／金額）——多處斷言共用。 */
...
test('銀行主入口｜有效 PDF → 成功解析（不是一律 400），而且支出／存入靠 x 分欄、'
  ...
  assert.deepEqual(r.accounts.map(a => [a.masked, a.balance, a.currency, a.label]),
    [['900100****7788', 100000, 'TWD', '台幣活存'], ['900100****1122', 50000, 'TWD', '台幣活存'],
      ['900100****3344', 30000, 'TWD', '台幣活存']],
    '概要區三個帳戶（末碼／餘額／幣別／標籤）');
  assert.equal(r.transactions.length, 5,
    '明細要抽出五筆——列內少了依 x 排序的話 `c[0]` 會變成備註／餘額格，整張靜靜抽出 0 筆');
...
test('銀行主入口｜金額右緣剛好跨在支出／存入分界上 → 只有 PDF 的**真實字寬**判得對方向'
  + '（字級面在第 2 頁、同字級不同實寬面在第 3 頁）', async () => {
  // ⚠️ 這一題釘的是 `w`。三顆被實際用過的繞法各對應一筆：
  //    **第 4 筆（第 2 頁、字級 20、x=345）** 對的是 Codex 審 `c2d7517` 那一輪的 `5 * it.str.trim().length`
  //      （完全不看字級）：真實 w=60 → 右緣 405 → 存入欄 [400,500) → in（正確）；
  //      估算 25 → 右緣 370 → 支出欄 → out。
  //    **第 5 筆（第 3 頁、字級 10、x=360）** 對的是 Codex 審 `c41f8e0` 那一輪的
  //      `fontSize × 字數 × (ASCII ? 0.5 : 1)`（看字級、但不看每個 glyph 的實寬）：
  //      真實 w=50（`1` 與 `,` 在 `GLYPH_W` 裡都寬 1000）→ 右緣 410 → 存入欄 → in（正確）；
  //      估算 25 → 右緣 385 → 支出欄 → out（**收入被記成支出**）。
  //      這條估算對第 4 筆也是錯的（50 → 395 → 支出欄），所以兩筆一起轉紅。
  //    **兩筆分別落在第 2 頁與第 3 頁**（r11）：所以「w 只在前 N 頁抽對」這一族，
  //      N=1 讓第 4、5 筆都翻面、N=2 讓第 5 筆翻面——把那顆 High 往後挪一頁不再踩不到。
  //    兩筆各自是自己帳戶（…1122／…3344）的唯一一列，`parseBankDetail` 的 running 餘額校正
  //    （同帳戶相鄰兩列餘額差）救不到它們——錯了就一路錯進現金流帳本。
  const r = await parseBankStatement(bankStatementPdf(), undefined);
  assert.deepEqual(r.transactions.map(t => [t.date, t.direction, t.amount]), BANK_TXNS.map(t => [...t]),
    '五筆的日期／方向／金額都要對——第 4 筆（2026-01-25、2,468）與第 5 筆（2026-01-28、1,111）的方向'
    + '完全靠「x + w 這個右緣」落在哪一欄判出來，w 一旦被人造值頂替（歸零、只看字數、'
    + '或「字級 × 字數 × 0.5」這種不讀 glyph 實寬的估算）它們就會從 in 翻成 out');
  const [t4, t5] = [r.transactions[3], r.transactions[4]];
  assert.equal(t4.acctMasked, '900100****1122', '第 4 筆屬於自己的帳戶（所以沒有前一列餘額可校正）');
  assert.equal(t4.balance, 50000);
  assert.equal(t4.note, '',
    '同一頁離它 80 的那段（承前頁）超過 ORPHAN_MAX_DY(40)，不可以黏上來');
  assert.equal(t5.acctMasked, '900100****3344', '第 5 筆屬於自己的帳戶（所以沒有前一列餘額可校正）');
  assert.equal(t5.balance, 30000);
  assert.equal(t5.note, '',
    '兩段都不可以黏上來：同頁離它 100 的「本頁小計說明」超過 ORPHAN_MAX_DY(40)；'
    + '**第 2 頁**的「承前頁」（頁內 y=500）跟它隔著一整個頁位移——'
    + '除非抽取器把第 2 頁以後的頁位移寫成同一個值（`pageBase = -100000`），'
    + '那時兩者的距離會被算成 20 而靜靜黏成這一筆的備註（備註是內轉／劃撥分箱的判準）');
});
 ...
 *
 *    ①**成交金額用小字級印**（`FONT_SIZE_SMALL`，x=215）——封「不看 glyph 實寬的 w」與「右緣規則被拿掉」：
 *        真實 w＝27（`104,000` 逐字加總 4500／1000 × 字級 6）→ 右緣 242 → 「成交金額」band ✔
 *        `字級 × 字數 × 0.5`＝21 → 右緣 236 → 落進「證券名稱」band ✘
 *        不看 w（pos＝左緣 215）→ 也落進「證券名稱」band ✘
 *        （`字數 × 5`＝35 → 右緣 250 仍在「成交金額」band，**這一格擋不掉它**，由 ② 擋。）
 *    ②**折讓金額的左緣落在前一欄**（預設字級 10、3 字，實寬 30——三個 `1` 在 `GLYPH_W` 裡各寬 1000）
 *      印在 x=275——封「右緣規則被整條拿掉」與「只看字數的 w」：
 *        真實右緣 305 → 「折讓」band ✔（左緣 275 還在「成交金額」band 裡）
 *        不看 w（pos＝左緣 275）→ 落回「成交金額」band ✘
 *        `字數 × 5` 或 `字級 × 字數 × 0.5` 都是 15 → 右緣 290 → 落回「成交金額」band ✘
 *      左緣飄進前一欄正是真對帳單的幾何：金額**右對齊**，左緣隨位數往左飄，飄過表頭左緣是常態。
 *      它同時跟同一列上游的手續費 `148`（同字級、同樣 3 個字，實寬只有 20）構成一組
 *      **同字級、同字數、不同實寬**的對照（r9 加的，見檔頭⑩）。
 *
 *    **實測後果**（三種假值各跑一次量的，都**不報錯**、只是金額靜靜錯掉）：
 *      w ← `5 * it.str.trim().length`：`grossAmount` 變 104000111、`feeDiscount` 變 `null`
 *        （折讓那格漂進成交金額欄、跟成交金額併成一格）；
 *      w ← `fontSize * 字數 * (ASCII ? 0.5 : 1)`：`grossAmount` 變 111、`feeDiscount` 變 `null`、
 *        `name` 變「元大台灣50104,000」（成交金額漂進證券名稱、折讓漂進成交金額）；
 *      `pos` ← `cell.x`：`grossAmount` 變 111、`feeDiscount` 變 `null`、`name` 同樣被黏上 104,000。
 *
 ...
 *
 * ⚠️ `secondPage`（r10 加的，見檔頭⑪）＝把**第二筆成交印到第 2 頁**，而且它的成交金額與折讓
 *    兩格都沿用 ② 那個「左緣落在前一欄、右緣才落回自己這一欄」的幾何。上面 ①② 兩格都在第 1 頁，所以
 *    `w: pageBase === 0 ? Math.round(it.width || 0) : 0`（**只有第一頁保留字寬**）
 *    ——r4 審查者退回的唯一一條 High——在證券主入口這一側完全踩不到（實測修之前的樹全綠）。
 *    第 2 頁**刻意不重印表頭**：`colsA`／`colsB` 只有遇到新表頭列或法規小字才會變，
 *    所以第 2 頁走的正是「沿用第 1 頁欄界」那條真實路徑。
 *    第 2 頁那一筆的數字彼此對得起來：500 股 × 110 ＝ 成交 55,000，
 *    應收付 55,037 ＝ 55,000 ＋ 手續費 148 − 折讓 111。
 *
 * @param {{sectionTitle?:string, netHeader?:string, docTitle?:string, secondPage?:boolean}} [opts]
 */
function securitiesStatementPdf({ sectionTitle = '成交明細', netHeader = '應收付金額', docTitle = '', secondPage = false } = {}) {
  /** 第 2 頁：一筆成交跨兩行，折讓的右緣才落回「折讓」band（見上面的 `secondPage` 說明）。 */
  const page2 = [
    S(40, 800, '115/01/20'), S(95, 800, '115/01/22'), S(150, 800, '2330'), S(240, 800, '500'),
    S(300, 800, '110.00'), S(360, 800, '148'), S(430, 800, '55,037'),
    S(40, 790, '現買'), S(150, 790, '台積電'),
    // 成交金額：左緣 230 在「證券名稱」band、實寬 35 → 真右緣 265 才落回「成交金額」band [240,300)
    // ⚠️ 左緣不可貼到 240：`55,000` 的右緣就會緊接著下一格 `111` 的左緣 275，
    //    **pdfjs 會把間距 <10pt 的相鄰文字併成同一個節點**（實測 gap 0 併成 `55,000111`、
    //    gap 5 併成 `55,000 111`、gap 10 才切開），fixture 想量的兩格就不存在了。
    S(230, 790, '55,000'),
    S(275, 790, '111'),      // 折讓：左緣 275 在「成交金額」band、真右緣 305 才落回「折讓」band
  ...
  return makeTextPdf([[
    ...
    // 一筆成交的第一行／第二行（應收付 104,037 ＝ 成交 104,000 ＋ 手續費 148 − 折讓 111；
    // 折讓 111 ＝ 手續費打二五折後退回來的部分（148 × 0.75），數字彼此對得起來，不是隨手填的）
    S(40, 800, '115/01/13'), S(95, 800, '115/01/15'), S(150, 800, '0050'), S(240, 800, '1,000'),
    S(300, 800, '104.00'), S(360, 800, '148'), S(430, 800, '104,037'),
    S(40, 790, '現買'), S(150, 790, '元大台灣50'),
    S(215, 790, '104,000', FONT_SIZE_SMALL),   // 成交金額：小字級，真右緣 242 才落進自己這一欄（見上面 ①）
    S(275, 790, '111'),   // 折讓：左緣 275 在「成交金額」band、真右緣 305 才落回「折讓」band（見上面 ②）
    S(360, 790, '0'), S(430, 790, 'TWD'),
  ], ...(secondPage ? [page2] : [])]);
}
...
test('證券主入口｜有效 PDF → 成功解析（不是一律 400），而且雙層表頭的欄位靠 x 各就各位', async () => {
  ...
  assert.deepEqual(
    ...
    ['2026-01-13', '2026-01-15', '0050', '元大台灣50', '現買',
      1000, 104, 104000, 148, 0, 104037, 'TWD']);
  assert.equal(t.feeDiscount, 111, '折讓欄要讀得到（它的左緣落在前一欄，右緣才落回自己這一欄）');
});
...
test('證券主入口｜金額的左緣飄進前一欄、右緣才落回自己這一欄 → 只有 PDF 的**真實字寬**判得對欄位', async () => {
  ...
  //    折讓「111」 x=275 實寬 30（三個 `1` 各寬 1000）→ 右緣 305 ∈ [300,360) ✔
  //      不看 w（pos=275）→ 落回成交金額欄 ✘；只看字數的估算（15）→ 右緣 290 → 也落回成交金額欄 ✘
  //      它跟同一列的手續費「148」（同字級、同 3 字、實寬只有 20）是一組同字級同字數不同實寬的對照。
  //    成交金額「104,000」小字級 x=215 實寬 27 → 右緣 242 ∈ [240,300) ✔
  //      `字級 × 字數 × 0.5`（21）→ 右緣 236 → 落進證券名稱欄 ✘；不看 w（215）→ 也落進證券名稱欄 ✘
  //  **兩格併存才把三種假值都封住**（實測見 `securitiesStatementPdf` 的「實測後果」）。
  const r = await parseTaishinSecuritiesPdf(securitiesStatementPdf(), undefined);
  const t = r.trades[0];
  assert.deepEqual([t.grossAmount, t.feeDiscount, t.name], [104000, 111, '元大台灣50'],
    '成交金額與折讓要各就各位：右緣規則一被拿掉（或 w 被人造值頂替），這兩格會併成一格、'
    + '另一格變 null，成交金額還會被黏到證券名稱後面——金額全錯而且完全不報錯');
});

test('證券主入口｜**第 2 頁**的欄界金額也要判得對（沿用第 1 頁欄界；字寬只保留第一頁 → 這一筆金額錯掉）', async () => {
  // ⚠️ r10 補的（r4 審查者退回的唯一一條 High）：上面那一題的兩格邊界案例都在第 1 頁，
  //    而本檔原本的跨頁考題只斷言文字與座標、沒斷言 w——於是兩支抽取器一起改成
  //    `w: pageBase === 0 ? Math.round(it.width || 0) : 0`（只有第一頁保留字寬），
  //    修之前的樹照樣全綠（實測 PDF 相關六個考題檔 139 題 `ℹ fail 0`）。
  //    真實害是「多頁對帳單從第 2 頁起金額靜靜漂進相鄰欄」，
  //    所以這一題把同一個欄界幾何搬到第 2 頁，量端到端的後果（不只抽取層那一格數字）。
  //  第 2 頁那一筆的幾何（下層欄界沿用第 1 頁的表頭：證券名稱 150／成交金額 240／折讓 300／證交稅 360）：
  //    成交金額「55,000」x=230、實寬 35 → 右緣 265 ∈ [240,300) → 成交金額欄 ✔
  //    折讓「111」   x=275、實寬 30 → 右緣 305 ∈ [300,360) → 折讓欄 ✔
  //    第 2 頁 w 歸零 → 兩格的右緣都退回左緣，整組往左移一欄
  //      ⇒ **實測**：`name` 變「台積電55,000」、`grossAmount` 變 111、`feeDiscount` 變 `null`。
  const r = await parseTaishinSecuritiesPdf(securitiesStatementPdf({ secondPage: true }), undefined);
  assert.equal(r.trades.length, 2, '第 2 頁那一筆也要收得到（跨頁沿用第 1 頁的雙層欄界）');
  const [t1, t2] = r.trades;
  assert.deepEqual([t1.grossAmount, t1.feeDiscount], [104000, 111], '第 1 頁那一筆不受影響（對照組）');
  assert.deepEqual(
    [t2.tradeDate, t2.settlementDate, t2.symbol, t2.name, t2.rawType,
      t2.quantity, t2.price, t2.grossAmount, t2.commission, t2.feeDiscount, t2.tax, t2.netSettlement],
    ['2026-01-20', '2026-01-22', '2330', '台積電', '現買', 500, 110, 55000, 148, 111, 0, 55037],
    '第 2 頁那一筆的證券名稱、成交金額、折讓要各就各位——字寬只在第一頁抽對（後續頁歸零）的話，'
    + '這兩格金額的右緣都退回左緣、整組往左移一欄：名稱變「台積電55,000」、成交金額變 111、折讓變 null，'
    + '而且完全不報錯（使用者拿到的是靜靜算錯的成交資料）');
});
...
for (const [col, harm] of /** @type {const} */ ([
  ['支出金額',
    '守衛改成合取後這一格的五筆數字碰巧還全對——所以它釘的是守衛本身：'
  ...
  ['存入金額',
    ...
    + '另兩個帳戶的第 4、5 筆碰巧沒判錯，所以錯的只有第一筆、整份預覽看起來完全正常'],
  ['帳戶餘額',
    '守衛改成合取後**五筆交易全部靜靜掉光（0 筆）**，而概要區的帳戶餘額照樣回得漂亮——'
    + '預覽看起來就像「這個月沒有交易」'],
])) {
...
// 下面兩題各用一份**只命中單一分支的完整對帳單**（帳戶概要區＋明細表頭＋五筆交易齊全），
...
for (const [marker, absent] of /** @type {const} */ ([
  ...
  test(`銀行主入口｜明細標記只寫「${marker}」（整份沒有「${absent}」）的完整對帳單 → 要放行並真的抽出帳戶與交易`, async () => {
    ...
    assert.deepEqual(r.accounts.map(a => [a.masked, a.balance]),
      [['900100****7788', 100000], ['900100****1122', 50000], ['900100****3344', 30000]],
      `走「${marker}」這個分支的真帳單要照樣抽出概要區三個帳戶`);
    assert.deepEqual(r.transactions.map(t => [t.date, t.direction, t.amount]), BANK_TXNS.map(t => [...t]),
      `走「${marker}」這個分支的真帳單要照樣抽出五筆交易——`
...
for (const { present, absent, opts } of /** @type {const} */ ([
  ...
  test(`證券主入口｜只命中「${present.join('＋')}」這一個分支的完整對帳單 → 要放行並真的抽出交易`, async () => {
    ...
    assert.deepEqual(
      ...
      [['2026-01-13', '0050', '現買', 1000, 104000, 104037]],
...
//    字級只驗了 6／10／20 三種，**不是**「任何字級都對」的證明。
//    字寬同理：`GLYPH_W` 是 fixture 自己宣告的 `/Widths`（`,` `.` `1` 與「小」刻意跟同類其他字不同寬），
//    所以本檔證得出的是「抽取器有沒有讀 PDF 宣告的字寬」，**證不出**「台新真單那套字型的比例落在哪」。
//    真字型的差距比這裡的兩倍／一半小得多，方向一樣（右對齊金額的右緣會漂）。
...
//  ⑤孤兒備註的**兩個門檻值本身沒有專門的考題**，被釘住的只有「這一份 fixture 的實際距離兩邊分得開」：
//    y 上限（`ORPHAN_MAX_DY`）＝10 要收、80 與 100 不可收（實測改成 1e9 或 5 都會轉紅；
//    r11 在第 2 頁加了「承前頁」之後，改成 **80** 也會轉紅——它離同頁第 4 筆剛好 80，實測本檔 fail 1），
//    但 40 這個數字改成 30 這種「還在 10 ＜ cap ＜ 80」的值，本檔照樣綠（實測 fail 0）。
...
//  ⑦**證券 `assignCols` 的「文字格用左緣」那一半，是被 fixture 的幾何撐住的，不是被一條專門的考題釘住。**
//    實測把 `pos` 改成**所有格子一律用右緣**（拿掉 `isAmt &&`）：本檔 fail 7、那一筆成交整筆消失（0 筆）
//    ——因為 `115/01/13`（x=40、實寬 65）的右緣 105 已經越過「交割日」欄的左緣 95。
//    但這是 r9 換掉字寬表之後才成立的**副作用**（r8 那版同一個突變全綠，當時同一格實寬只有 45），
//    沒有任何一題的名稱在講它、也沒有斷言直接量「文字格用的是左緣」。
//    真帳單的長證券名稱會不會溢出自己那一欄，本檔仍答不出來。
//  ⑧**跨頁驗到「第 3 頁」為止，而且後續頁都刻意不重印表頭。**
//    - 頁數：本檔最多三頁（抽取層跨頁題三頁、銀行主入口 fixture 三頁、證券主入口的可選第 2 頁）。
//      「只在前 N 頁抽對」這一族因此只封到 **N≤2**：把同一顆再往後挪成「前三頁抽對、第 4 頁起壞」，
//      本檔不會轉紅。這是有限頁數 fixture 的固有極限（下一個 N 永遠還在），不是改斷言能關掉的洞
//      ——r10 只補到兩頁時，N=2 那顆就是這樣穿過去的（見檔頭⑫）。
//    - **位移量本身刻意不釘**（r11 放寬假紅）：位移由 fixture 自己量出來，只驗「第 1 頁不加位移／
//      位移大於整頁高度／每頁只推一個位移」。所以 `pageBase -= 200000` 這種等價改寫不會紅（實測
//      全套 fail 0）；但把它改成逐頁遞增（`pageBase -= 100000 * i`）**會**紅（實測本檔 fail 2），
//      而那顆對下游其實也等價。留著紅是刻意的取捨：抽取器自己的註解寫「每頁往下推一個大位移」，
//      「一頁一個等量位移」當契約比較守得住「第 2 頁以後共用同一個位移」那顆真病（檔頭⑬）；
//      真要改成非等量位移時，得一起改這題的斷言。
//    - **後續頁的「頁內 y 原樣保留」在抽取層只驗到第 1 頁**：第 2、3 頁是靠「頁內 y − k×位移」
//      這條式子驗的，而位移是量出來的。真正把「後續頁的頁內 y 沒被壓平」釘住的是端到端那一側
//      （銀行第 2、3 頁各有一段孤兒備註，與同頁交易的距離要分得開）——實測
//      「第 2 頁起頁內 y 全歸零」那顆突變全套 fail 4，其中兩題是主入口的。
//    - **表頭重印那條路沒有考題**：兩支解析器自己都假設跨頁會重印表頭
//      （`lib/taishin-securities.js` 的註解寫「跨頁時下一頁表頭重印…會重建，續收」），
//      重印會讓 `parseBankDetail` 重跑一次欄位 x 抽取、讓 `colsA/colsB` 重建。
//      本檔的第 2、3 頁一律沿用第 1 頁的欄界，所以「後續頁的表頭把欄位 x 讀歪」這種病本檔答不出來。
//      （真台新對帳單是不是每頁都重印，我沒有真單可查證。）
//    - 證券的第 2 頁是**可選的**（`secondPage`）：型別守衛那組單分支考題與拒絕面各題仍是單頁，
//      「某個守衛只在第一頁生效」這類病不在本檔範圍。
