...
// 但**真正被照著執行的檔案是 `審查與合併程序.md`**，而那裡寫的是無條件的「一律 --squash --delete-branch」。
...
/**
 * 抓出 審查與合併程序.md 裡「合併由 Codex 代執行」那個 blockquote 區塊。
...
function mergeBlock(md) {
  ...
  assert.notEqual(start, -1, '審查與合併程序.md 找不到「合併由 Codex 代執行」的區塊——合併程序被搬走或改寫了，這道考題要跟著更新');
  let end = start;
...
test('合併程序：審查與合併程序 的合併步驟必須「在 fenced code 裡」跑堆疊閘腳本，且在 merge 之前', () => {
  const raw = mergeBlock(read('審查與合併程序.md'));
