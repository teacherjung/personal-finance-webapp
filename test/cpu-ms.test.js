...
// ⚠️ 真錶兩側都要釘（Grok #552 複審後掃・真錶對照組那一條）：假錶只證明算式，證明不了**這台機器的 `process.cpuUsage()` 真的在走**
 ...
 *  （Grok #552 複審後掃・還原檢查那一條：對區域變數比的話，任何一題漏還原，其他題都看不見）。 */
...
test('★真錶兩側：等待不算工作（`Atomics.wait` 60ms ⇒ CPU 接近 0）、固定的一段真工作必須量到明顯非 0', () => {
  ...
  //   給更快的機器與 Linux tick 級的計時解析度。真錶若是空操作，只有這句會紅（假錶只能證明算式，證明不了原生錶在走）——這正是它存在的理由。
  let x = 0;
