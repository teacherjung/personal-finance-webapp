...
test('cleanStore：帳單店名清理（涵蓋使用者指定的 14 條規則）', () => {
  const cases = [
    ['Apple Xinyi A第03/12期/TW', 'Apple（信義）第03/12期'],   // 定位碼被期數擠成的「A」併回品牌（2026-07-19）；顯示改（信義）＝鑰匙併進 Apple（使用者定 2026-07-27）
    ...
    ['聯信-%Arabica象A0145 TAIPEI', '%Arabica'],              // 2026-07-27 一步到位（自訂規則入內建）：「象」＝象山店截斷殘，一併收掉
    ...
    ['國外交易服務費', '國外交易服務費'],
    // 路易莎（使用者定 2026-07-27，更新 2026-07-18 舊裁決）：顯示只到品牌、不帶門市
    ['LOUISA COFFEA0145 NEW TA', '路易莎咖啡'],
  ];
...
test('身分鑰匙 storeKeyOf：品牌層（不含分店）＋加油站聚合（使用者定 2026-07-18）', () => {
  ...
  const cases = [
    ...
    ['LOUISA COFFEA0145 NEW TA', '路易莎咖啡', '路易莎咖啡'],   // 2026-07-27：顯示不再帶門市（舊裁決「全部＝林口文三門市」作廢）
    ...
    // 札伊卡 2026-07-27 改一步到位（使用者自訂顯示名入內建）：兩種寫法同收「札伊卡印度咖哩風味餐館」
    ['Zaika札伊卡印度咖哩風味A0145 TAIPEI', '札伊卡印度咖哩風味餐館', '札伊卡印度咖哩風味餐館'],
    ['Zaika札伊卡A0145 TAIPEI', '札伊卡印度咖哩風味餐館', '札伊卡印度咖哩風味餐館'],
    ...
    // 2026-07-27：顯示改 Apple（信義）＝stripBranch 後鑰匙與 APPLE.COM 訂閱同一把 Apple（使用者定「所有含 Apple 皆為 Apple」）
    ['Apple Xinyi A2716 Taipei', 'Apple（信義）', 'Apple'],
    ['Apple Xinyi A第03/12期/TW', 'Apple（信義）第03/12期', 'Apple'],
    ['Apple Xinyi A第07/12期/TW', 'Apple（信義）第07/12期', 'Apple'],
    ...
    ['威秀影城信義分A0145 TAIPEI', '威秀影城（信義）', '威秀影城'],   // 2026-07-27：「分」＝分公司截斷殘（COMPANY_TAIL），不再進分店名
    ...
    ['蘋果電腦-台灣-ECA0145 Taipei', 'Apple', 'Apple'],   // 2026-07-27：蘋果電腦併入 Apple（使用者定「所有含 Apple 或蘋果電腦皆為 Apple」）
