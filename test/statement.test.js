...
test('cleanStore：帳單店名清理（涵蓋使用者指定的 14 條規則）', () => {
  const cases = [
    ...
    ['柑園加油站有限公司林口二站TAIPEI', '林口二加油站'],   // 加油站顯示名（使用者定 2026-07-25）：分店當主體
...
test('身分鑰匙 storeKeyOf：品牌層（不含分店）＋加油站聚合（使用者定 2026-07-18）', () => {
  ...
  const cases = [
    ...
    // 加油站（使用者定 2026-07-25，全品牌統一）：顯示名＝「分店加油站」（品牌不進顯示名；
    // 已是「…加油站」或認不出分店＝原樣不動）；鑰匙照舊一律「加油站」（2026-07-18：所有加油站算同一件事）。
    // 頭兩筆＝使用者拍板當天給的驗收例（泰山／新店）。
    ['中油-泰山站(D2158)TAIPEI', '泰山加油站', '加油站'],
    ['統一精工-新店站TAIPEI', '新店加油站', '加油站'],
    ['中油-林口工三站A0145 TAIPEI', '林口工三加油站', '加油站'],
    ['台亞林口中山站', '林口中山加油站', '加油站'],
    ['車容坊加油站-文二站A0145 TAIPEI', '文二加油站', '加油站'],
    ['柑園加油站有限公司林口二站TAIPEI', '林口二加油站', '加油站'],
    ['世新加油站A0145 TAIPEI', '世新加油站', '加油站'],
    ['台塑-新店站(D9901)TAIPEI', '新店加油站', '加油站'],   // 台塑窄關鍵字（2026-07-25；D9901＝合成代碼）
    // 無分隔符無括號（Codex 複審 2026-07-26）：品牌前綴切；切不出分店＝「品牌加油站」，不留原樣
    ['中油A123 TAIPEI', '中油加油站', '加油站'],
    ['統一精工', '統一精工加油站', '加油站'],
    ['台塑石油股份有限公司', '台塑加油站', '加油站'],
    ['台塑石油新店站', '新店加油站', '加油站'],
    // 設施／服務詞不是分店名 → 用品牌（使用者定 2026-07-26）
    ['中油自助洗車站A1', '中油加油站', '加油站'],
...
test('加油站顯示名（使用者定 2026-07-25）：台塑判準＋新格式冪等＋顯示名回推鑰匙', () => {
  ...
  // 新格式跑第二遍不再變（規則指紋自動整理會重複執行，不冪等＝每次開 app 名字都在漂）
  assert.equal(cleanStore('泰山加油站'), '泰山加油站');
  // 手動記帳（無帳單原文）只有顯示名可回推——新格式含「加油站」字樣，鑰匙照樣聚合
  assert.equal(storeKeyOfName('泰山加油站'), '加油站');
  assert.equal(storeKeyOfName('新店加油站'), '加油站');
});
