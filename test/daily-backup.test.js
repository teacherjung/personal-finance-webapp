// 每日滾動備份（階段四 A）考題。裁決指定五條（docs/system-optimization-plan.md:43）：
...
test('裁決①：同日第二次呼叫不重複建立（lastBackupDate＋檔案都在＝跳過）', async () => {
  await reset();
  const r1 = await dailyBackupIfDue('2026-07-27');
  assert.equal(r1.created, true);
  assert.ok(existsSync(join(BDIR, backupFileName('2026-07-27'))));
  const r2 = await dailyBackupIfDue('2026-07-27');
  assert.equal(r2.ran, false, '同日第二次＝什麼都不做');
  assert.equal(readdirSync(BDIR).filter(n => n.startsWith('store-')).length, 1, '只有一顆');
});

test('裁決①補：記過日期但備份檔被手動刪掉 → 補做（不因記錄還在就永遠不補）', async () => {
  await reset();
  await dailyBackupIfDue('2026-07-27');
  rmSync(join(BDIR, backupFileName('2026-07-27')));
  const r = await dailyBackupIfDue('2026-07-27');
  assert.equal(r.created, true, '檔案不在就補一顆');
});

test('裁決②：超過 30 天正確清理；30 天內與不認得的檔一律不動', async () => {
  await reset();
...
test('裁決③＋失敗行為：備份失敗 → 正式庫不受影響、failStreak 累積、不寫 lastBackupDate（今天會重試）', async () => {
  await reset();
  ...
  const s1 = /** @type {any} */ ((await getDb()).settings);
  ...
  rmSync(BDIR);
  const r3 = await dailyBackupIfDue('2026-07-27');
  assert.equal(r3.created, true, '修好後同日重試成功');
  assert.equal(r3.failStreak, 0);
  assert.equal(/** @type {any} */ ((await getDb()).settings).backupFailStreak, 0, '成功歸零');
  assert.equal(/** @type {any} */ ((await getDb()).settings).lastBackupDate, '2026-07-27');
});

test('裁決④：備份檔可重新開啟＝真的 SQLite、kv 內容與正式庫一致', async () => {
  await reset();
  const r = await dailyBackupIfDue('2026-07-27');
  assert.equal(r.created, true);
...
test('裁決⑤：清理失敗不誤刪正式檔、也不影響今天剛建好的備份', async () => {
  await reset();
  mkdirSync(BDIR, { recursive: true });
