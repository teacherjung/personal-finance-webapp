...
test('自訂名「加油站」逐字照登：匯入不覆蓋、整理不動、不跳確認閘', async () => {
  const db = await getDb();
  ...
  await saveDb(db);
  await importRows('c1', [row('中油-金山站(D2159)TAIPEI')]);
  const t = (await getDb()).transactions.find(x => x.stmtRef.includes('金山'));
  assert.equal(t?.note, '加油站', '自訂名逐字，勝過新自動格式');
  // 整理（dryRun 預覽）：自訂名不得被改寫、不得出現在 learnedNameChanges（嚇人確認閘的依據）
  const prev = await normalizeBranches(true);
...
test('整理：舊格式（中油（泰山站））收斂到新格式，且整理冪等', async () => {
  const db = await getDb();
  db.cards = [{ id: 'c1', name: '測試卡', type: 'credit', lastFour: '9001' }];
  db.learnedCategories = {};
  ...
  await saveDb(db);
  const prev = await normalizeBranches(true);   // 預覽
  assert.deepEqual(prev.changes.map(c => [c.before, c.after]), [['中油（泰山站）', '加油站（泰山）']]);
  await normalizeBranches(false);               // 套用
  assert.equal((await getDb()).transactions[0].note, '加油站（泰山）');
  const again = await normalizeBranches(true);  // 再跑一次＝什麼都不該再變（冪等）
