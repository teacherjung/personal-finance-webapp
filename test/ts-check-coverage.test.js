...
// 型別檢查那一關有沒有看某支檔，取決於它在 TypeScript 算出來的檔案集合裡、且檔頭有 TypeScript 認得的指令——沒人數就會漂。
...
test('型別檢查射程：TypeScript 對 jsconfig 算出來的每一支 .js（lib／public／scripts／test-doubles／prototype 遞迴＋server.js）都有 TypeScript 認得的 // @ts-check（逐檔 opt-in 的另一半：沒人數就會漂）', () => {
  ...
  // include 被拿掉、遞迴 glob 換成單一檔、另加 exclude，都會在這裡紅（Grok #598 掃：前兩種舊版假綠）。
  ...
  for (const dir of ['lib/', 'public/', 'scripts/', 'test-doubles/', 'prototype/']) {
    ...
    assert.deepEqual(dropped, [], `這些檔在磁碟上、卻不在 TypeScript 對 jsconfig 算出來的集合裡（include 被縮、或加了 exclude）——三關的型別檢查不會看它們`);
  }
