...
// 所以分成兩種題，射程各自寫清楚：
//
// - **①「答案仍然正確」**（`injectDirtyGitEnv`）：注入下面**整組** `DIRTY_GIT_ENV`，
//   但照上表，真正驅動這一族題的**只有 `GIT_DIR`**（唯一四種形狀通吃的那個，也正是事故原兇）；
//   其餘幾個在這一種題裡**不產生影響力**，它們是為了題②才放進同一組的。
//   ⚠️ 誠實劃界：所以①是**代理指標**——它擋不住「退化成只刪 `GIT_DIR` 的列名版」。
//   （⚠️ 別在呼叫端的註解裡寫成「注了第二家族所以列名版會被抓到」：那句話是假的，
//     #463 r2 複審在六處註解裡抓到同一句——它是我第一版錯探針留下的殘骸。）
