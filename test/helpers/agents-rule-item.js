...
// 給要證明「承重句在**它自己那一條**裡」的文件考題用（誰在用＝grep `ruleItemRange`，這裡不列、列了會漂），
...
//   ——⚠️ **但那不一定造成早切**：只有 **tab 前面的連續空格數 < 內容欄**時才會切。內容欄是 4 的話，
//   三格空白＋tab 會切，四格空白＋tab 不會（#585 r11 實測）。
...
//   （例如圍欄與行首 raw HTML 由 `test/contract-split.test.js` 的兩道禁令接，Setext 則沒有）。
