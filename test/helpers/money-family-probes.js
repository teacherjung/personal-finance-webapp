 ...
 * ⚠️ 共用＝**共同失效點**（Codex #536 r3–r5 連四輪同族實證：刪詞、等量替換、
 * 實例 iterator、原型 iterator——值層面的釘每補一形就有下一形）。William 2026-09-01
 * 裁示改**位元組釘**：兩張考卷各釘一份本檔**檔案位元組**的 sha256——本檔任何一個
 * 字元的改動（含註解、含任何寫法花樣）都讓兩張考卷轉紅，必須同步重算兩顆釘＝
 * diff 必然橫跨三個檔案，審查者看得見。這是「動這個檔案」這個方向的機械終點。
 * 誠實劃界：釘防的是「只動本檔」的一切形狀；動考卷本身、或連兩顆釘一起改的人
 * 釘防不了——沒有任何 repo 內考題能防「把考題連題目一起改掉」的人，那層靠審查制度。
