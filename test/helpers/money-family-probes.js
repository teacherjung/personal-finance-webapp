 ...
 * ⚠️ 共用＝**共同失效點**（Codex #536 r3 H1 突變實證：從這裡刪掉 submit、
 * 兩側 command 同步弱化，兩張考卷都綠但 submit_order 實際放行）。所以兩張考卷
 * 各釘一份**helper 外的字面身分雜湊**（禁止面六張清單的 sha256）——改這裡的題目
 * 必須同時改兩張考卷裡的釘＝diff 必然橫跨三個檔案，審查者看得見。
 * 誠實劃界：釘防的是「單點改弱」與「小 diff 偷渡」；連兩顆釘一起改的人釘防不了，
 * 那一層靠審查制度（沒有任何 repo 內考題能防「把考題連題目一起改掉」的人）。
