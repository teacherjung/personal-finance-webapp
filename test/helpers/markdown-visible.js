...
//
// 這一份原本住在舊的 grok-scan-docs 考題裡，2026-09-10 搬出來共用（#591：文件題被實測用 HTML 註解與圍欄繞過，
// 兩處各寫一份就是兩份會漂的複本）。2026-09-17 切換日那兩支文件題退役後，本檔一度被刪、又在同一支救回：
// 固定小標「### 複審後掃」的接手題要的正是這把尺（Codex #611 r2／r3：用 indexOf 或只認欄首三反引號的正則都留活口）。
// 誰在用＝grep visible(。**判準與射程一字未改**，逐字保留在下面。
//
