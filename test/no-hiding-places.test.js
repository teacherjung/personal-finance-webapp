...
const ALLOWED_GITIGNORE = [
  ...
  // 測試被硬砍（SIGKILL）時會殘留，這兩行是為了**不讓殘骸被 git add 收進去**。
  // ⚠️ **必須綁死這兩個位置**（Grok 複審後掃 #2）：不帶開頭斜線的 `_xlsx_guard_probes/`
  //    **每一層都生效**（`scripts/`、`test/`、`docs/`、倉庫根都算）。後果＝在那些位置放一支
  //    **不引入 xlsx** 的檔，`git status` 看不到、ESLint 也 0 錯 ⇒ **一個真的藏身處**（實測過）。
  //    這正是本考題存在的理由，我加它的時候自己開了一個。
  // ⚠️ **不可以同時加進 ESLint 的 `ignores`**——忽略會讓探針**失去受測資格**：那一題要求
  // 26 種寫法**每一種都至少被抓到一次**，忽略之後 finding 消失 ⇒ **那一題會紅**
  // （報 `「靜態」這種寫法沒有被擋下` ——引文逐字取自 `test/xlsx-isolate.test.js` 的斷言原文）。
  // ⚠️ 我原本在這裡寫「忽略掉就永遠全綠」——**那是反話**，#640 r1 實測是 exit 1。
  //    決定沒變（不要忽略），錯的是理由。實測：在暫存副本把 `**/_xlsx_guard_probes/**`
...
test('.gitignore 的忽略樣式跟宣告清單一字不差', () => {
  ...
  assert.deepEqual(actual, ALLOWED_GITIGNORE,
    ...
    + '   還是「我們自己寫了但沒在用的程式碼」？\n'
    + '   後者的規則是**直接刪**（AGENTS.md「驗證與三關」節的「沒用到的程式直接刪」那一條），\n'
