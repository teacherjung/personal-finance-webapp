...
const CASES = [
  ...
  { id: 'A8', name: '期末與概要對不上（漏讀期末幾筆）', lines: linesFullNoTotals, expect: 'caught', defense: 'reconcile',
    ...
    proves: (/** @type {any} */ a) => a.transactions.length === 2 && a.totals.txCount === null && a.totals.totalOut === null && a.totals.totalIn === null && a.transactions[1].balance === 4700,
    why: '末筆 4,700 對不上概要 5,500＝reconcile 擋。版面用**無合計版**（r3#1：印著合計卻填 null＝把「漏讀合計」也混進注入、null 就不是誠實缺席）＋三欄全 null 逐欄驗——合計真不上膛、孤立的才是概要閘。' },
