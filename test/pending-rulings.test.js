...
test('⭐ 撤回的規則綁回正本：AGENTS 那一顆寫的三種理由與兩句固定字樣，工具要逐字認得', () => {
  ...
  // ⭐ 字眼絆線（**是絆線不是閘**）：那一句裡不該出現「這是舊的、已經不用了」這種話——
  //    正本就該只寫現行規則，沿革要寫請寫在別的段落。它擋得住 r3／r4 那兩種最便宜的寫法，
  //    **擋不住換句話說**（例如「先前版本」「不再採用」）。誤叫的代價很低：把沿革搬出這一句就好。
  // ⚠️ 詞表只收「這是歷史」那一類，**不收一般的禁止語**（#582 r5 Medium）：`請勿使用`／`不再使用`
  //    在現行規則裡是正常的（例：「括號裡要寫具體依據，請勿使用空泛理由」），收進來就是誤擋。
  for (const word of ['已停用', '舊寫法', '沿革', '舊版']) {
    assert.ok(!rule.includes(word),
      `撤回那一句裡出現「${word}」——正本那一句只該寫現行規則，沿革請寫在別的段落。`
      + '（這是絆線不是閘：它擋得住最常見的寫法，擋不住換句話說；「哪一段才是現行值」機器判不出來，那一半歸複審。）');
  }
