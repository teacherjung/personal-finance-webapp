...
test('⭐ 撤回的規則綁回正本：AGENTS 那一顆寫的三種理由與兩句固定字樣，工具要逐字認得', () => {
  ...
  //    ⇒ 那一半歸**複審的眼睛**：正本改一個字一定出現在 diff 裡，由複審辨識（切換日前那一整節曾被逐字釘住、改字必改釘本；
  //      切換日起只釘錨點與樣板，沒有機器逼人改釘本）。這不是缺口沒補，是分工。
...
test('⭐ 「沒有時限」也要在規則正本裡：現行那句話在，工具才有依據（切換日起正本＝RULES I2，附則只指條號與落點）', () => {
  // 2026-09-17 切換日前這題釘的是 AGENTS 那一節逐字「沒有時限、也沒有逾時預設——問了就等他」；切換後通則住 RULES I2（附則不複述，K1）。
  assert.match(readDoc('RULES.md'), /^- I2 沒有時限：問了就等/mu, 'RULES I2「沒有時限：問了就等」不見了——時限被改回去了，工具與這題要一起改');
  const section = reviewSectionOf(readDoc('AGENTS.md'));
  assert.equal(section.split('沒有時限＝RULES I2').length - 1, 1,
    '「審查回饋處置」那一節裡找不到「沒有時限＝RULES I2」那句指路（要剛好一處）——附則自己又長出一份時限規則、或指路被搬走了');
  assert.doesNotMatch(section, /逾時預設[^（]*照|時限內沒回/u, '「審查回饋處置」那一節又出現逾時預設的操作指示');
  // ⚠️ **這題只證「那句話在」**：它證不到「這一節沒有別的句子還在講逾時」——那一半沒有機器在看，
  //    靠的是複審的眼睛（正本改一個字就在 diff 裡；切換日前那一整節曾被逐字釘住，切換日起只釘錨點與樣板）。
 ...
 * 舊合併手冊與 CLAUDE.md 的零次釘（切換日 2026-09-17 隨舊手冊一起拿掉；CLAUDE.md 現在只指 RULES I1〜I3）。⚠️ 那兩顆當時守的也只是**那兩支檔案裡的字面尾巴**——
...
test('⭐ 這支不是閘：合併程序住的三個地方（AGENTS 附則整節、settings.json 登記的閘、tools/merge.js）都不得把它接進來', () => {
  // 2026-09-17 切換日前這題掃的是整份合併手冊（REVIEW-AND-MERGE.md，已刪）。合併程序現在分住三處：
  //   AGENTS.md「本專案協作附則」（合併段與自動守門段都在那一節）、settings.json 的 gates（合併指令只跑登記在這裡的閘）、
  //   tools/merge.js（合併指令本體）。三處都掃；「不在閘名單裡」那種寫法對這支永遠不會紅（它不自報閘名），所以直接掃字面。
  // ⚠️ **起點不要再釘了**：r11 釘在第一個 `> 1.`（前面插一行就繞過）、r12 改釘在前言那一行
  //   （前言**之前**插一行又繞過）。每釘一次就多一個「前面」。所以附則掃的是**從它的 H2 到檔尾整節**、
  //   不是「合併段」那幾行——這支不該出現在合併程序的任何位置，這句話才是封閉的。
  // ⚠️ 附則裡唯一准提到它的地方＝「審查回饋處置」那顆（它講的正是這支要讀的留痕形狀，開頭就寫明這支是舊工具）：
  //   准的只有**那一行**、而且只准提名字，不准寫成叫人跑它的指令。別的行提到＝紅。
  //   代價：哪天真的想在合併段提到它（例如寫「這支**不是**合併步驟的一環」），就要回來改這題。
  //   那個代價是對的：那正是「把它寫進合併程序」這件事該有的摩擦。
  const agents = readFileSync(join(ROOT, 'AGENTS.md'), 'utf8');
  const at = agents.indexOf('\n## 本專案協作附則');
  assert.ok(at >= 0, '找不到 AGENTS.md「本專案協作附則」那一節——這題變空包彈');
  const appendix = agents.slice(at + 1).split('\n');
  for (const [i, line] of appendix.entries()) {
    if (!/pending-rulings/.test(line)) continue;
    assert.match(line, /^\*\*審查回饋處置（/,
      `AGENTS.md 附則第 ${i + 1} 行提到這支＝它變成流程的一環（pre-push 那種「非零就擋」的地方會讓退出碼 2 變成擋人）：${line.slice(0, 60)}`);
    assert.doesNotMatch(line, /node\s+scripts\/pending-rulings/,
      '「審查回饋處置」那顆只准提名字，不准寫成叫人跑它的指令');
  }
  ...
  for (const g of gates) {
    assert.doesNotMatch([g.command, ...(g.args ?? [])].join(' '), /pending-rulings/,
      `settings.json 登記的閘「${g.name}」叫了這支＝合併指令會跑它、退出碼 2 變成擋人`);
  }
  assert.doesNotMatch(readFileSync(join(ROOT, 'tools/merge.js'), 'utf8'), /pending-rulings/,
    'tools/merge.js 提到這支＝合併指令把它接進來了');
});
