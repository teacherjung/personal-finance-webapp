...
test('★r6#1b 回歸護欄：幣別同名店家去了兩次、甚至佔多數列，都不得害正確的那一支被選掉', () => {
  // 判準的三個條件都承重：①是幣別碼 ②至少兩列 ③**覆蓋一半以上**。
  // 少了③，「同一家店去兩次」就會被當成幣別欄 ⇒ 正確的那一支被扣分 ⇒ 選錯。
  const f = [
    ['信用卡帳單'],
...
test('★掃#2 documenting test：認不出機構＋兩支都抓到列 ⇒ **可能交出錯的那一支**（已知缺口，不是已修好）', () => {
  ...
  assert.ok(r.transactions.every((x) => /^[A-Z]{3}$/.test(x.desc)),
    '★現況：交出來的是幣別代碼當店名那一支。**若這行開始紅**，代表有人做出了版面錨點——請一起更新本題與 lib/statement.js 的劃界');
});
