...
test('★r3#2 卡片發卡行的比對＝同一組樣式（香港富邦不得算成台北富邦）', async () => {
  ...
  // ★短名（Codex #518 r8：base 的 `includes` 認得，本支一度認不得＝功能退化）
  assert.equal(issuerBank('台新'), '台新', '★卡片欄位填短名也要認得——這是使用者自由文字，詞彙與帳單抬頭不同');
  assert.equal(issuerBank('富邦'), '富邦');
  // ★但只接受**完全相等**：放寬成 startsWith／includes 就會把香港富邦收進來
  assert.equal(issuerBank('富邦銀行'), '', '★不是完全相等、也不是台北富邦 ⇒ 認不出（安全方向）');
...
test('★r6#1 回歸護欄：幣別碼與店名的集合**本來就相交**，「HUF」是真品牌', () => {
  ...
  // ⚠️ 這題同時釘住「同一個值要重複出現」那一層——我曾以「沒有考題分辨得出來」為由拿掉它，
  //    **那個理由是錯的**：這份樣本就分辨得出來。「現有考題沒涵蓋」≠「不可證明」。
  const f = [
    ['信用卡帳單'],
    ['HUF'],
    ['115/07/03', '115/07/05', '100'],
...
test('★r6#1b 回歸護欄：幣別同名店家去兩次（2/5 列）不得害正確的那一支被選掉', () => {
  ...
  //    在這份樣本上不會扣分、這題不會紅。真正打死 r7 那版的是下面 r6#1 的 `HUF／星巴克／HUF`（2/3）。
  //    兩題要一起看。
  const f = [
    ['信用卡帳單'],
