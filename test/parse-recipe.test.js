...
test('引擎｜退路判向與跨界收緊：左緣過中線判 out 照舊；長溢出格（r12#1 起）＝歧義拒解', () => {
  const noIgn = { ...recipeA(), detail: { ...recipeA().detail, headerIgnore: [], headerNote: null } };
  const shell = [
    ...linesA().slice(0, 9),
    L(120, [[75, '帳號'], [135, '日期'], [272, '提領金額'], [331, '存進金額'], [396, '結存餘額']]),
  ];
  // 右緣 250+10=260 < 提領欄 272 ＝分不出；左緣 250 < 中線 (272+331)/2 → out（退路照舊）
  const p = parseWithRecipe([
    ...shell,
    L(100, [[53, 0, '900100****3301'], [124, 0, '2026/06/11'], [177, 0, '合成甲'], [250, 10, '$1,234']]),
  ], noIgn);
  assert.deepEqual(p.transactions.map(t => [t.direction, t.amount]), [['out', 1234]]);
  // 模板的「長溢出格進退路判 in」（右緣 ≥ xBal）在配方引擎收緊為歧義拒解（r12#1：
  // acct-date 的漂移餘額會被這條退路撈成反向金額）——退路判 in 從此結構性不可達，
  // 判向公式保留只為對稱、承重的是 out 半邊
