...
test('引擎｜期間錨點行只有一個日期＝參考日 null（同 AI 規則 1a「寧可 null」：填錯會拿舊蓋新）', () => {
  const lines = linesB();
  lines[0] = L(300, [[47, '帳務期間'], [200, '115/05/31']]);
...
test('引擎｜右側忽略欄（#408 H² 同型、預審①）：序號欄在存入欄右邊也要擋——兩條路都是', () => {
  ...
  const doc = (rows) => [L(300, [[47, '合成帳戶總覽區'], [452, '結算基準日:2026/06/30']]),
...
test('引擎｜寬格左緣在忽略/備註區間、右緣跨進金額窗（r5#1）：主路也要用左緣那把尺', () => {
  // 形一：流水號格 [200,272) 起、寬 80 ⇒ 右緣 290 跨進提領窗——0067890 不可變支出
  const p1 = parseWithRecipe([
    ...linesA().slice(0, 10),
    L(100, [[53, 0, '900100****3301'], [124, 0, '2026/06/11'], [210, 80, '0067890']]),
    L(90, [[53, 0, '900100****3301'], [124, 0, '2026/06/12'], [349, 40, '$500'], [418, 0, '$1,730']]),
  ], recipeA());
  assert.equal(p1.transactions.length, 1, '★寬序號格右緣跨進支出窗也不算——起點在忽略欄就是忽略欄的格');
  // 形二：備註格 [310,331) 起、寬 25 ⇒ 右緣 337 跨進存進窗——777 不可變收入
  const rec = { ...recipeA(), detail: { rowIdent: 'acct-date', headerOut: '提領金額', headerIn: '存進金額', headerBalance: '結存餘額', headerNote: '附記', headerIgnore: [] } };
  const p2 = parseWithRecipe([
    L(300, [[47, '合成帳戶總覽區'], [452, '結算基準日:2026/06/30']]),
    L(280, [[50, '甲種活存'], [150, '900100****3301'], [473, '$1,230']]),
  ...
  assert.deepEqual(p2.transactions.map(t => [t.direction, t.amount]), [['out', 500]],
    '★777 左緣在備註欄＝備註，右緣跨進存進窗也不可變收入');
});
