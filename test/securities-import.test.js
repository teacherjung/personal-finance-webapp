...
test('預覽：分幣別摘要、去重標記、無 blocker；遮罩 label 不含帳號原文', async () => {
  const p = buildSecuritiesPreview(await getDb(), parsed([BUY, SELL]));
...
test('fail-closed：未知類別/彙總不符/缺帳戶/金額爆表/無法辨識 → blockers 列清楚，apply 一律 400', async () => {
...
test('匯入：入庫含 id/批次/時間、無 null 欄、無帳號原文；重匯冪等 0 新增；同日同額兩筆都活', async () => {
  const db = await getDb();
  const r1 = applySecuritiesImport(db, parsed([BUY, SELL, { ...BUY }]));   // BUY 兩筆一模一樣＝真交易
  await saveDb(db);
  assert.equal(r1.imported, 3);
  assert.equal((await getDb()).securityTrades.length, 3, '同日同代號同額的兩筆真交易都要入庫');
  ...
  const again = applySecuritiesImport(await getDb(), parsed([BUY, SELL, { ...BUY }]));
  assert.equal(again.imported, 0, '重匯同一份＝0 新增');
  assert.equal(again.skippedDup, 3);
});

test('匯入紀錄＋整批刪除：台新批次可刪；IB 批次擋 400；不誤刪別的批次', async () => {
  const db = await getDb();
  ...
  await saveDb(db);
  const batches = listSecuritiesBatches(await getDb());
  assert.ok(batches.length >= 2);
  await assert.rejects(() => deleteSecuritiesBatch('ib-sync-x'), /不可整批刪除/);
  const tsBatch = batches.find(b => b.source === 'taishin');
  const before = (await getDb()).securityTrades.length;
  const del = await deleteSecuritiesBatch(tsBatch.batchId);
  assert.equal(del.deleted, 3);
  assert.equal((await getDb()).securityTrades.length, before - 3, '只刪該台新批次');
  assert.ok((await getDb()).securityTrades.some(r => r.id === 'ib1'), 'IB 列還在');
  await assert.rejects(() => deleteSecuritiesBatch('沒這批'), /找不到/);
});
...
test('schema 守門＋備份往返：合法列過櫃檯；缺 tradeDate/sourceRef 整筆濾除；匯入驗證接受合法列', async () => {
  ...
  const cleaned = sanitizeDbForWrite({ ...structuredClone(await getDb()), securityTrades: [good, noDate, noRef] }, { mode: 'strip' });
...
test('幣別牆：不支援幣別（EUR）在預覽就 fail-closed（否則會在寫入櫃檯被拒、毒死整批）', async () => {
  const p = buildSecuritiesPreview(await getDb(), parsed([{ ...BUY, currency: 'EUR' }]));
  assert.match(p.blockers.join(), /幣別不在系統支援範圍.*EUR/);
});

test('自審根治｜補印插入不漏記（服務層端到端）：先匯 [X,Y]，補發單 [X,Z,Y] → 只新增 Z、Y 不重複', async () => {
  const db = await getDb();
  ...
  const rows = (await getDb()).securityTrades.filter(r => r.symbol === '0056');
  assert.equal(rows.length, 3, '共三筆（X/Y/Z 各一）');
  assert.deepEqual(rows.map(r => r.name).sort(), ['X', 'Y', 'Z'], 'X/Y/Z 各一、無重複無漏');
});

test('自審 #6｜核心金額（價/成交額/應收付）缺席或非有限 → blocker（藍圖 §七）', async () => {
  const noPrice = buildSecuritiesPreview(await getDb(), parsed([{ ...BUY, price: null }]));
  assert.match(noPrice.blockers.join(), /核心金額讀不到/);
  const noNet = buildSecuritiesPreview(await getDb(), parsed([{ ...BUY, netSettlement: null }]));
...
test('Codex S2r1#5｜備份驗證牆＝完整合約：缺 side/currency/source 的殘缺列擋下', async () => {
  ...
  const cleaned = sanitizeDbForWrite({ ...structuredClone(await getDb()), securityTrades: [good, { ...good, id: 'nb', side: undefined }] }, { mode: 'strip' });
