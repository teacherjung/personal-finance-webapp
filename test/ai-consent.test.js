...
test('E｜同意窗內文：四件事都講到、危險句一句都不准出現、檔名要跳脫', () => {
  ...
  assert.doesNotMatch(html, /不會上傳|不會外送|已限速|保證正確|免費/,
    '⚠️ 這幾句在 AI 路線都是假的（帳單內文真的會送出去；「已限速」語焉不詳——C1 的護欄是發數上限、要講就講具體數；閘不保證讀對）');
...
test('批二｜aiConsentBodyHtml 的 card 變體：單讀說明取代雙讀、送出內容講卡片欄位；bank 預設一字不動', async () => {
  ...
  assert.ok(card.includes('先免費試讀'), '★r7#3：「免費」是條件式——規則卡只保證下期先試，讀不動照樣退 AI（無條件宣稱＝成本/隱私說明不實）');
  assert.ok(!card.includes('不再外送；'), '無條件的「不再外送」句型不得回來');
...
test('批二 r1#5｜卡片版 AI 徽章：只講卡片真的跑過的閘，不可借銀行的餘額鏈／仲裁文案', () => {
  ...
  assert.match(html, /繳款和退款的身分抄錯/, '★r8#1 家族的殘餘盲點要就地揭露');
  assert.match(html, /恰好等於「已繳款」合計/, '★r1#1 改口：整筆繳款被抄成退款＝算術不可區分——擋得住哪型、擋不住哪型都要講');
  assert.match(html, /桶裡的退款被漏抄/, '★r12#1 的盲點要就地揭露：繳款只列摘要時桶驗不了、桶裡的退款漏抄看不到');
  assert.match(html, /有印出來時/, '★r12#1 改口：「繳款列另外對桶」是有條件的——無條件宣稱＝畫面過度保證');
  assert.match(html, /會一併記進帳/, '★裁示②：具名調整（利息/年費）記帳＝要就地講清楚（含日期怎麼給）');
  assert.match(html, /分毫不差才收/, '★裁示①：容差 0 之後照實講嚴格程度');
  assert.doesNotMatch(html, /1 元容差|只拿來驗算/, '★舊揭露（容差縫隙/不入帳）已被裁示關上——留著＝畫面說謊');
