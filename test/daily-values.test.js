...
test('日線：同一天重跑只留一行，且值更新成最新（同日覆寫）', () => {
  seed(1000);
  const first = recordDailyValue();
...
test('日線：記錄了投組成本/市值與當日匯率（差異引擎與事後回推要用）', () => {
  seed(1000);
