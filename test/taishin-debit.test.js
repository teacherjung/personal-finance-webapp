...
test('★認得版面卻收不到東西＝throw，不可靜靜匯入 0 筆回報成功', () => {
  // 形狀一：定位得到表頭、但一列交易都沒有。
  assert.throws(() => parseTaishinDebit([...headerLines(), HEAD]),
    (/** @type {any} */ e) => e.code === 'bank_unrecognized' && /一列交易都讀不出來/.test(e.message));
  // 形狀二：整份根本沒有表頭列（辨識靠整份文字會說認得——欄名散在說明文字裡也算）。
  const noHead = [...headerLines(),
    L(400, [[137, '以下為日期、摘要、支出、存入、餘額、備註的說明', 200]]),   // 欄名都在、但是一句話，不是表頭
    row(390, '2026/01/02', 'CD轉出', '2,884', '0', '318,491'),
  ];
  assert.equal(isTaishinDebitStatement(noHead), true, '★辨識仍會說認得（六個欄名在整份文字裡都在）');
  assert.throws(() => parseTaishinDebit(noHead),
    (/** @type {any} */ e) => e.code === 'bank_unrecognized' && /定位不到/.test(e.message),
    '★所以要有第二道：認得版面卻定位不到表頭＝照實喊認不得，不可回 0 筆成功');
});

// ---------- 表頭被抽字拆成相鄰兩列（Stage 3） ----------
// #492 r1#1 實測：PDF 抽字有時把六欄表頭拆成兩列，整份就落到 AI（花錢）。
// 相鄰兩列**都是純碎片**（每格剛好是欄名）、y 夠近、合起來六欄到齊且 x 順序對 ⇒ 當一列表頭。
// 表頭是「錢不被算兩次」的界線：程式證明不了六個字是 D 區的，只能要求字與順序都對。兩列合併的信任面
// 比整列大（兩列各三個欄名更容易在別區湊出來），所以合併只當**退路**：整份有任何整列表頭就不合併。
/** 拆成兩列的表頭（前三欄一列、後三欄一列）。 @param {number} y1 @param {number} y2 @param {number} dx x 整體位移（跨頁欄界不同時用） */
const HEAD_SPLIT = (y1 = 400, y2 = 396, dx = 0) => [
  L(y1, [[143 + dx, '日期', 13], [191 + dx, '摘要', 13], [239 + dx, '支出', 13]]),
  L(y2, [[291 + dx, '存入', 13], [342 + dx, '餘額', 13], [415 + dx, '備註', 13]]),
];
/** 欄界整體右移 dx 的明細列（跨頁第二頁的版面）。 */
const rowShifted = (/** @type {number} */ dx, /** @type {number} */ y, /** @type {string} */ date, /** @type {string} */ summary, /** @type {string} */ out, /** @type {string} */ inn, /** @type {string} */ bal) => L(y, [
  [131 + dx, date, 37], [185 + dx, summary, 27], [251 + dx, out, 17], [312 + dx, inn, 11], [346 + dx, bal, 25],
]);

test('表頭拆成相鄰兩列：合起來當一列表頭，欄位定位照樣正確（y 差 4 與 5 都是真檔量到「同一列被拆開」的距離）', () => {
  for (const dy of [4, 5]) {
    const lines = [...headerLines(), ...trapLines(), ...HEAD_SPLIT(400, 400 - dy),
      row(390, '2026/01/02', 'CD轉出', '2,884', '0', '318,491', '榮先生'),
      row(380, '2026/01/05', '轉帳存入', '0', '10,000', '328,491'),
    ];
    const r = parseTaishinDebit(lines);
    assert.equal(r.transactions.length, 2, `dy=${dy}：兩列明細都讀到`);
    assert.deepEqual(r.transactions.map((t) => [t.direction, t.amount, t.balance, t.note]),
      [['out', 2884, 318491, '榮先生'], ['in', 10000, 328491, '']],
      `dy=${dy}：★支出／存入／餘額／備註四欄的界線來自合併後的表頭——任一欄 x 沒接對，方向或餘額就會錯`);
    assert.equal(r.accounts[0].balance, 328491, '期末餘額＝最後一列');
    assert.ok(!r.transactions.some((t) => t.amount === 305 || t.amount === 500), '★表頭之前的 A／C 區列不可變成交易');
  }
});

test('整份沒有整列表頭、拆分表頭跨頁重印且第二頁欄界不同：要重新定位才讀得對', () => {
  // 第二頁欄界整體右移 40：若第二頁的拆分表頭沒被認出來、沿用第一頁的欄界，摘要格會落進舊的支出欄
  // ＝讀不出數字 ⇒ fail-closed 丟 bank_unrecognized（不是方向翻面）；重新定位才讀得到第二筆。
  const lines = [...headerLines(), ...HEAD_SPLIT(400, 396),
    row(390, '2026/01/02', 'CD轉出', '2,884', '0', '318,491'),
    ...HEAD_SPLIT(300, 296, 40),
    rowShifted(40, 290, '2026/01/05', '轉帳存入', '0', '10,000', '328,491'),
  ];
  const r = parseTaishinDebit(lines);
  assert.deepEqual(r.transactions.map((t) => [t.direction, t.amount, t.balance]), [['out', 2884, 318491], ['in', 10000, 328491]],
    '★第二頁那筆必須照第二頁的欄界讀');
});

test('★整份有整列表頭＝碎片合併整個關閉：A 區兩列湊出六欄也不會提早開閘（Codex #498 r2#1 的版面）', () => {
  // A 區在 y=400／396 分兩列印出六個欄名（順序也對）、其後 y=390 有一列交易形狀的 305 元，真表頭在 y=300。
  // 整列判準會等到 y=300 才收；合併若開著就會在 y=396 開閘、把 305 收進來＝錢被算兩次。
  const lines = [...headerLines(), ...HEAD_SPLIT(400, 396),
    row(390, '2026/01/28', '刷卡消費', '305', '0', '318,186', 'APPLE.COM/BILL'),   // 表頭之前的 A 區列
    L(300, [[143, '日期', 13], [191, '摘要', 13], [239, '支出', 13], [291, '存入', 13], [342, '餘額', 13], [415, '備註', 13]]),
    row(290, '2026/01/02', 'CD轉出', '2,884', '0', '318,491'),
  ];
  const r = parseTaishinDebit(lines);
  assert.deepEqual(r.transactions.map((t) => t.amount), [2884], '★只有真表頭之後那筆；305 不可入帳');
});

test('★湊不成表頭的形狀：沒有任何表頭時一律喊認不得（寧可退回 AI，不猜界線）', () => {
  const tx = row(390, '2026/01/02', 'CD轉出', '2,884', '0', '318,491');
  const isUnrecog = (/** @type {any} */ e) => e.code === 'bank_unrecognized' && /定位不到/.test(e.message);
  // 兩列隔太遠（y 差 20＝真檔量到「真的隔一列」的距離）
  assert.throws(() => parseTaishinDebit([...headerLines(), ...HEAD_SPLIT(400, 380), tx]), isUnrecog, '★隔太遠＝不是同一列被拆開');
  // 拆成三列（只看相鄰兩列；第一片會被第二片換掉、第二第三片合起來缺欄）
  assert.throws(() => parseTaishinDebit([...headerLines(),
    L(400, [[143, '日期', 13], [191, '摘要', 13]]),
    L(397, [[239, '支出', 13], [291, '存入', 13]]),
    L(394, [[342, '餘額', 13], [415, '備註', 13]]), tx]), isUnrecog, '★三列＝不收（本支只放寬到相鄰兩列）');
  // 合起來仍缺一欄
  assert.throws(() => parseTaishinDebit([...headerLines(),
    L(400, [[143, '日期', 13], [191, '摘要', 13], [239, '支出', 13]]),
    L(396, [[291, '存入', 13], [342, '餘額', 13]]), tx]), isUnrecog, '★五欄＝不是表頭');
  // 碎片裡混了別的字（說明句帶著欄名）
  assert.throws(() => parseTaishinDebit([...headerLines(),
    L(400, [[143, '日期', 13], [191, '摘要', 13], [239, '支出', 13]]),
    L(396, [[291, '存入', 13], [342, '餘額', 13], [415, '備註', 13], [460, '（單位：元）', 40]]), tx]), isUnrecog,
    '★每一格都要剛好是欄名——含別的字就不是碎片（寬鬆判準會把含欄名的說明列當成表頭）');
  // 兩片之間夾了一列別的東西＝不相鄰
  assert.throws(() => parseTaishinDebit([...headerLines(),
    L(400, [[143, '日期', 13], [191, '摘要', 13], [239, '支出', 13]]),
    L(398, [[137, '（以下為本期明細）', 80]]),
    L(396, [[291, '存入', 13], [342, '餘額', 13], [415, '備註', 13]]), tx]), isUnrecog, '★中間夾別的列＝不相鄰');
  // 六個欄名都在、但 x 順序不對（整列與兩列都一樣不算）
  assert.throws(() => parseTaishinDebit([...headerLines(),
    L(400, [[143, '備註', 13], [191, '餘額', 13], [239, '存入', 13], [291, '支出', 13], [342, '摘要', 13], [415, '日期', 13]]), tx]), isUnrecog, '★整列：欄名順序反了＝不是表頭');
  assert.throws(() => parseTaishinDebit([...headerLines(),
    L(400, [[143, '日期', 13], [191, '摘要', 13], [239, '備註', 13]]),
    L(396, [[291, '存入', 13], [342, '餘額', 13], [415, '支出', 13]]), tx]), isUnrecog, '★兩列：合起來六欄但順序不對＝不是表頭');
});

test('已在明細裡遇到湊不成表頭的碎片：明細照上一個表頭的欄界續讀；碎片不可被當跨行備註黏到交易上', () => {
  // 整份沒有整列表頭（合併開著）；第一頁拆分表頭正常，第二頁重印被拆開卻湊不齊（第二片隔太遠）：
  // 碎片本身不是交易列、也不是備註；後面的列照舊讀。
  // 其中一片刻意只有一格「備註」落在備註欄（x=415 > xBal）且 y 貼近下一筆交易——它**符合**跨行備註
  // 歸位的幾何條件，碎片分支若不攔下它就會被黏進交易的備註（Codex #498 r2#2 指出原測資守不住這件事）。
  const lines = [...headerLines(), ...HEAD_SPLIT(400, 396),
    row(390, '2026/01/02', 'CD轉出', '2,884', '0', '318,491'),
    L(300, [[143, '日期', 13], [191, '摘要', 13], [239, '支出', 13], [291, '存入', 13], [342, '餘額', 13]]),
    L(275, [[415, '備註', 13]]),   // 與上一片隔 25＝湊不成；與下一筆交易隔 5＝在跨行備註的歸位距離內
    row(270, '2026/01/05', '轉帳存入', '0', '10,000', '328,491'),
  ];
  const r = parseTaishinDebit(lines);
  assert.deepEqual(r.transactions.map((t) => [t.direction, t.amount]), [['out', 2884], ['in', 10000]]);
  assert.equal(r.transactions[1].note, '', '★碎片列不可被當成跨行備註黏到交易上');
});
