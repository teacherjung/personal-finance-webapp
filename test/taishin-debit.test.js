...
// ---------- Stage 3（William 2026-08-20 排定）：表頭被抽字拆成相鄰兩列 ----------
// #492 r1#1 實測：PDF 抽字有時把六欄表頭拆成兩列。以前這種整份都喊認不得 ⇒ 落到 AI（花錢）。
// 現在：相鄰兩列**都是純碎片**（每格剛好是欄名）、y 夠近、合起來六欄到齊 ⇒ 當一列表頭。
// 三個條件缺一不可——表頭是「錢不被算兩次」的界線，猜錯的代價是把 A/B/C 區收進來。
/** 拆成兩列的表頭（前三欄一列、後三欄一列；y 差 4＝真檔量到「同一列被拆開」的距離）。 */
const HEAD_SPLIT = (/** @type {number} */ y1 = 400, /** @type {number} */ y2 = 396) => [
  L(y1, [[143, '日期', 13], [191, '摘要', 13], [239, '支出', 13]]),
  L(y2, [[291, '存入', 13], [342, '餘額', 13], [415, '備註', 13]]),
];

test('Stage 3｜表頭拆成相鄰兩列：合起來當一列表頭，欄位定位照樣正確', () => {
  const lines = [...headerLines(), ...trapLines(), ...HEAD_SPLIT(),
    row(390, '2026/01/02', 'CD轉出', '2,884', '0', '318,491', '榮先生'),
    row(380, '2026/01/05', '轉帳存入', '0', '10,000', '328,491'),
  ];
  const r = parseTaishinDebit(lines);
  assert.equal(r.transactions.length, 2, '★兩列明細都讀到（以前這份整份喊認不得）');
  assert.deepEqual(r.transactions.map((t) => [t.direction, t.amount, t.balance, t.note]),
    [['out', 2884, 318491, '榮先生'], ['in', 10000, 328491, '']],
    '★支出／存入／餘額／備註四欄的界線來自合併後的表頭——任一欄 x 沒接對，方向或餘額就會錯');
  assert.equal(r.accounts[0].balance, 328491, '期末餘額＝最後一列');
  // 陷阱區仍擋得住：合併後的表頭一樣是「錢不被算兩次」的界線
  assert.ok(!r.transactions.some((t) => t.amount === 305 || t.amount === 500), '★表頭之前的 A／C 區列不可變成交易');
});

test('Stage 3｜跨頁重印的表頭也被拆兩列：不中斷、續讀', () => {
  const lines = [...headerLines(), HEAD,
    row(390, '2026/01/02', 'CD轉出', '2,884', '0', '318,491'),
    ...HEAD_SPLIT(300, 296),                                        // 第二頁：表頭重印、而且被拆開
    row(290, '2026/01/05', '轉帳存入', '0', '10,000', '328,491'),
  ];
  const r = parseTaishinDebit(lines);
  assert.equal(r.transactions.length, 2);
  assert.equal(r.transactions[1].amount, 10000);
});

test('★Stage 3｜湊不成表頭的四種形狀一律照舊喊認不得（寧可退回 AI，不猜界線）', () => {
  ...
  // ②拆成三列（只看相鄰兩列；三列的第一片會被第二片換掉、第二第三片合起來缺欄）
