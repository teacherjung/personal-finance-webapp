...
// 表頭是「錢不被算兩次」的界線：程式證明不了六個字是 D 區的，只能要求字與順序都對——整列表頭同一個前提。
...
test('跨頁重印的表頭也被拆兩列、而且第二頁欄界不同：要重新定位才讀得對', () => {
  // 第二頁欄界整體右移 40：若第二頁的拆分表頭沒被認出來、沿用第一頁的欄界，金額會落錯欄（方向翻面）
  const lines = [...headerLines(), HEAD,
    row(390, '2026/01/02', 'CD轉出', '2,884', '0', '318,491'),
...
test('已在明細裡遇到湊不成表頭的碎片：明細照上一個表頭的欄界續讀（不新增 fail-closed，跟整列表頭時代一樣）', () => {
  // 跨頁重印被拆開卻湊不齊（第二片隔太遠）：碎片本身不是交易列、也不是備註；後面的列照舊讀
  const lines = [...headerLines(), HEAD,
    row(390, '2026/01/02', 'CD轉出', '2,884', '0', '318,491'),
    L(300, [[143, '日期', 13], [191, '摘要', 13], [239, '支出', 13]]),
    L(280, [[291, '存入', 13], [342, '餘額', 13], [415, '備註', 13]]),
