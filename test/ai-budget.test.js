...
test('take｜單張上限：到頂那發 throw ai_budget_exceeded、訊息含上限與下一步；被擋那發不佔每日名額', async () => {
  ...
  await assert.rejects(b.take(), (/** @type {any} */ e) =>
    ...
    '★白話＋含數字＋含下一步');
  assert.equal(m.box.settings.aiUsage.n, 2, '★被擋那發沒佔每日名額（單張先擋）');
...
test('take｜單日上限：跨兩份帳單合計數；到頂 throw、db 計數不動；換日自動恢復', async () => {
  ...
  assert.deepEqual(m.box.settings.aiUsage, { date: '2026-08-26', n: 3 }, '★被擋那發沒寫進 db');
...
test('take｜嚴格序列化：邊界上兩發並發只放行一發（雙讀 Promise.all 不會擠過同一個名額）', async () => {
