...
async function reset({ usdTwd = 30, signals = {}, insightState = {}, dailyValues = [], accounts = [], netWorthTarget = null } = {}) {
...
test('D3 首次執行：無書籤 → 全當持續中、不標 🆕、sinceLast=null；並寫下書籤', async () => {
  await reset({ usdTwd: 33, insightState: {} });   // usdTwd 33 ≥ fxHigh 32 → 當前有 fx-usd-high 提醒
  const r = await run();
  ...
  // 書籤已寫下（讀取＝看過了）
  const bm = (await getDb()).insightState;
...
test('D3 🆕 新出現：書籤沒有的 key 現在出現 → 標為 new', async () => {
  await reset({ usdTwd: 33, insightState: bmSeed({ reminders: [] }) });   // 書籤空提醒、但有 lastSeenAt（非首次）
  const r = await run();
...
test('D3 ✓ 已解除：書籤有、現在沒有 → cleared（帶 title 供「已解除：…」顯示）', async () => {
  await reset({ usdTwd: 30, insightState: bmSeed({ reminders: [{ key: 'fx-usd-high', title: '美元/台幣 33 已達 32 以上', module: '匯率', level: 'info' }] }) });
...
test('D3 跳檔：中股檔位 0→1 記為跳檔；未輸入(null)→有值 不算跳檔', async () => {
  await reset({ usdTwd: 30, signals: { china: 11 }, insightState: bmSeed({ tiers: { us: null, china: 0, japan: null, korea: null, taiwan: null } }) });
  ...
  await reset({ usdTwd: 30, signals: { japan: 1.25 }, insightState: bmSeed({ tiers: { us: null, china: null, japan: null, korea: null, taiwan: null } }) });
  const r2 = await run();
...
test('D3 平靜：提醒與檔位都沒變、今日 Δ 微小 → calm=true', async () => {
  ...
  await reset({ usdTwd: 33, insightState: bm, dailyValues: [{ date: '2026-07-21', netWorth: 1000 }, { date: '2026-07-22', netWorth: 1001 }] });   // 今日 +0.1% < 0.3%
  const r = await run();
...
test('D3 非平靜：今日 Δ 超過門檻（即使無新事件）→ calm=false', async () => {
  ...
  await reset({ usdTwd: 33, insightState: bm, dailyValues: [{ date: '2026-07-21', netWorth: 1000 }, { date: '2026-07-22', netWorth: 1050 }] });   // 今日 +5% ≥ 0.3%
  const r = await run();
...
test('D3 固定窗：今日＝最近兩既有日、本週＝約 7 天前既有日；不足不顯示', async () => {
  await reset({ usdTwd: 30, dailyValues: [
  ...
  await reset({ usdTwd: 30, dailyValues: [{ date: '2026-07-22', netWorth: 100 }] });   // 只有一天
  const r2 = await run();
...
test('D3 自上次 Δ：現值 − 書籤上次值', async () => {
  await reset({ usdTwd: 30, accounts: [{ id: 'a', type: 'cash', class: '現金', currency: 'TWD', balance: 1100 }],
...
test('D3 同日第二次開：第一次讀吸收 🆕，第二次讀無新事件（平靜）', async () => {
  await reset({ usdTwd: 33, insightState: bmSeed({ reminders: [] }) });
...
test('D3 自審#2/#5：殘缺書籤（缺 reminders 陣列／缺 lastSeenAt）→ 退回首次執行，不洪水標 🆕', async () => {
  await reset({ usdTwd: 33, insightState: { lastSeenAt: '2026-07-15T00:00:00Z', netWorth: 1000, tiers: { us: null, china: null, japan: null, korea: null, taiwan: null } } });
  const r1 = await run();
  assert.equal(r1.firstRun, true, '缺 reminders 陣列＝當首次執行');
  assert.equal(r1.reminders.new.length, 0, '不洪水標 🆕');

  await reset({ usdTwd: 33, insightState: { reminders: [], netWorth: 1000 } });   // 缺 lastSeenAt
  const r2 = await run();
...
test('D3 自審#3：基期 0 但有變動 → 不平靜（500k 從 0 起跳，pct 算不出也不可當平靜）', async () => {
  await reset({ usdTwd: 30, insightState: bmSeed({ reminders: [] }), dailyValues: [{ date: '2026-07-21', netWorth: 0 }, { date: '2026-07-22', netWorth: 500000 }] });
  const r = await run();
...
test('D3 自審#4：淨值為負時 Δ% 用 abs(基期)：-1000→-500（變好）＝+50%，不是 -50%', async () => {
  await reset({ usdTwd: 30, dailyValues: [{ date: '2026-07-21', netWorth: -1000 }, { date: '2026-07-22', netWorth: -500 }] });
  const r = await run();
...
test('D3 自審 r14#3：殘缺書籤缺 netWorth → sinceLast.netWorth = null（不謊報「自上次 +全部淨值」）', async () => {
  await reset({ usdTwd: 30, accounts: [{ id: 'a', type: 'cash', class: '現金', currency: 'TWD', balance: 500000 }],
