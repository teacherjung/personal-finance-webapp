 ...
 *   ③整張家族矩陣（`test/helpers/money-family-probes.js`，位元組由另一支考題釘）餵複本裡那份清單的行程內 `decide()`：
 *     該擋的都擋、該放的都放；白名單集合＝探針的 MONEY_SERVER_ALLOW（JSON 逐字集合，多列少列都紅）；
 *     白名單誤放建單工具時家族網仍擋（雙保險）；
 *   ④髒 GIT_* 環境下答案不變（指令用 `env -i` 起 node，環境本來就清空；套件 tests/guard-copy.test.js 另有 envonly 題）。
 *
 * ⚠️ 誠實劃界——這支考題證明不了的事：
 ...
 *     兩者若不同（forbidden 改了還沒重抽），只有重抽＋重按信任那條人的流程會補上。
 */
...
function withCopy(fn) {
  ...
  try {
    ...
    const { copyDir, group, failure } = build({ from: 'HEAD', to: join(scratch, 'copy'), root: src, allowTemp: true });
...
test('③整張家族矩陣餵複本裡那份清單：該擋的都擋、該放的都放；白名單集合＝探針清單；誤放建單工具時家族網仍擋', () => withCopy(({ copyForbidden }) => {
  ...
  // 白名單是精確集合：JSON 陣列逐字比（比舊制的 python AST 抽值更緊——沒有綁定形式可以繞）
  assert.deepEqual([...copyForbidden.allowlist].sort(), [...MONEY_SERVER_ALLOW].sort(), 'forbidden.allowlist 與探針清單不是同一個集合——多列＝悄悄放行，少列＝誤攔');
  // 雙保險：白名單誤放建單工具，家族網仍擋（名單內不在白名單那一層放行、照樣走家族網）
  const bad = { ...copyForbidden, allowlist: [...copyForbidden.allowlist, 'create_order_instruction'] };
  assert.equal(decide(`${MONEY_SERVER}create_order_instruction`, bad).deny, true, '白名單誤放建單工具時，家族網必須兜底');
}));

test('④髒 GIT_* 環境下答案不變（指令用 env -i 起 node；擋的照擋、放的照放）', () => withCopy(({ command }) => {
  const restore = injectDirtyGitEnv();
  try {
    denies(hook(command, asTool(CANARY)), '髒環境下測試鈕仍要被擋');
    allows(hook(command, asTool(HARMLESS)), '髒環境下無害工具仍要放行');
  } finally { restore(); }
}));
