...
test('★逾時是行為不是形狀：睡著的子行程在 50ms 就被 SIGKILL 收掉', () => {
  ...
  // ⚠️ 2026-09-05 流程體檢：改之前（main@ae66225 的版本）睡 1s、上界 900ms，機器忙時 spawn＋SIGKILL 的牆上時間也可能過 900ms＝假紅。
  //    設定＝睡 30s、上界 5s：壞路徑（沒逾時＝子行程睡滿才結束）約 30s，是上界的 6 倍——判別餘裕由這兩個設定值決定。
