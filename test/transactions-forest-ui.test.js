...
test('信用卡費接線（字面釘＝第二道網；帳本判準的**行為**在 test/ledger-split-behavior.test.js）：退款退路、月份、匯入、查帳與店家入口全部保留', () => {
  ...
  assert.match(source, /data-kind="count"[\s\S]*?<div class="stat sm">\$\{rows\.length\}<\/div>/,
    '本月筆數必須直接使用含退款的 rows.length');
