...
test('⭐ 逾時預設那一顆：**整節逐字**（從「審查回饋處置」那行到界線表整張表的末尾，一行不差）＋**整條祖先標題鏈**逐字——節內插一句「以下作廢」、往界線表插一列作廢、改擁有標題自己的後半、或在鏈上任何一段插標題把整節收進作廢小節，都會紅', () => {
  ...
  assert.doesNotMatch(section, /^ *\[[^\]]+\]:\s/mu, '這一節裡出現 Markdown 參考定義——那在畫面上不顯示，藏得下一句否定'
    + '（#585 r3 Medium：原本只收行首 0〜3 格，而清單項內容至少縮排四格＝正好躲過，所以不限縮排。'
    + '⚠️ 代價：這裡一律拒收＝比規格嚴。**部分**「長得像參考定義」的行其實會渲染成可見的程式碼文字'
...
//    ⚠️ 這份清單**跟著實作走**：r4 改成逐行釘之後，r2／r3 時代列在「擋不到」的好幾種已經會紅了
//    （第 12 條標題行尾補一句歷史說明、內文包進歷史巢狀清單——r5 實測兩種都由逐行比對接住）。
//    舊敘述留著會讓下一個人把「已經修好的」當成「已接受的豁免」（#585 r5 Low）。
//
//    **擋得到**：整條被搬走或被切進別的小節；承重句被抽掉或搬去隔壁那一條；指路離開本節；
//      以及**任何改動到第 12 條自己那幾行的寫法**（含把字藏進參考定義、腳註、連結 title、
//      圖片 alt、行中 HTML——那些都會讓那幾行對不上）。
//
//    **擋不到（r5 實測，這是現行的真限制）**：
//    ・**這一題讀的是原始檔的文字，證不了那些字在畫面上看得見。** 逐字相同的一行，可以被
//      **前後行**改變解析上下文而整行不顯示——r5 的反例是在引言裡用一個跨行的
//      `[ui]: /unused "` … `"` 把指路那一行包成 title：那一行一個字都沒改、在本節裡也剛好一次，
//      渲染出來卻找不到它。同理，把承重正文改寫成 `[補充](/unused "三句話")` 並**同步更新**
//      `RULE_12`，兩邊逐行相符而畫面上只剩一個「補充」連結。
//    ・要真正關掉它，只有一條路：**實際渲染一次、對渲染後的可見文字斷言**。本 repo 沒有
//      Markdown 引擎，為一道文件保存題引進一個新的 npm 相依不划算；而退而求其次的做法
//      （把「UI 現行慣例」第 1〜4 點也逐行釘住）會把一個**設計上就要一直改**的章節鎖死，
//      本身就是新的假保證。
//      ⚠️ **照實補一條我原本沒寫的路**（#585 r6 指出）：`gh api markdown` 渲染得了，而且
//      **不必新增任何 npm 套件**——複審時拿它驗是好用的（r5／r6 就是這樣抓到反例的）。
//      沒有把它做成自動關卡，是因為那會讓一道本機文件考題**改成依賴網路與 GitHub 服務**
//      （離線、限流、服務中斷都會變成假紅）。⇒ **記為待辦，不在本支做**；
//      理由是「範圍與成本」，不是「做不到」。
//    ・**從條外開一個會把整條吞掉的區塊**（程式碼圍欄、`<div>` 這類區塊標籤，開標在這一條前面）：
//      這一條的每一行都沒被動到，白名單與條界都不會叫——**本題接不到**。
//      ⚠️ **接住它的是 `test/contract-split.test.js` 的兩道明確禁令**：AGENTS.md 裡不得出現
//      code fence、不得出現行首 raw HTML（它在讀檔那一步就拒收，不管區塊有沒有閉合）。
//      也就是說**那一族確實有專門的防線，只是不在本題裡**——#585 r8 讀了失敗堆疊確認。
//      ⚠️ 這句話我寫錯過兩次：先寫成「由既有 HTML 禁令接」（結果對、理由錯），再寫成
//      「順帶接住、不是誰設計的」（也錯）。要引用請以 `contract-split` 那兩道 assert 為準。
//    ・**Setext 造的「歷史紀錄」標題**：判準只認 ATX（見 markdown-heading 檔頭）；Setext 用的是
//      底線、不是行首 HTML 也不是圍欄，所以上面那兩道禁令也接不到——**這一種沒有東西接**。
//    ・在**別條**（例如第 11 條）末尾補一句散文說「以下僅供歷史參考」。
//      寫了「作廢／廢止」字面的由題名關鍵字「作廢字眼絆線」那題接；其餘靠複審的眼睛。
//
//    ⚠️ **威脅模型**：本題主要防**誤刪與誤搬**——用戶是未來的自己人，不是會去改考題的攻擊者
//    （同 markdown-heading 檔頭）。⚠️ **但不可以由此推出「沒擋到的都只會是蓄意」**（#585 r6 Low）：
//    上面「擋不到」那幾種裡，**Setext 標題**與**在別條末尾補一句歷史說明**，正是一般整理文件會用的
//    寫法——新增歷史段落時不小心把後面的規則一起涵蓋進去，恰恰就是本題想降低的事故。
//    ⇒ 照實說：**未覆蓋的形式仍然可能出現在正常編修中，那份殘餘風險交給複審的眼睛辨識。**
...
test('⭐ 鐵則 12「必須懂的概念要在網頁上就地白話解釋」要留在正式鐵則區、承重句在它自己那一條裡（搬成沿革要紅；William 2026-07-22 定、2026-09-08 獨立）', () => {
  ...
  assert.equal(range12.hits, 1,
    `AGENTS.md **整份檔案**裡的頂層清單記號「12.」出現 ${range12.hits} 次（要剛好 1 次）——`
  ...
  // ⚠️ **代價（刻意接受，照實列）**：以後**動到這一條的任何一個字元**——改字、改縮排、
  //    在條內補一段說明——都會紅，要同時更新下面的 `RULE_12`＝**一定會有人看一眼**。
