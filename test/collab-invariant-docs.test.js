...
//      上一版寫「三種都擋不住」，說過頭）。**Grok #596 掃後再補兩類**（都不碰被釘的行、也不在旁邊加東西）：
//      ④**行中開啟、離這幾行很遠才關閉的 raw HTML 容器**（`<span>`／`<div>`）——全卷那道禁令只禁**行首** raw HTML，
//        而本檔為了寫 `🤖 <角色>` 這種格式刻意不禁行中 `<`；⑤**跨檔覆寫效力**——畫面上那句仍在，但合併手冊或 CLAUDE.md
//        叫按鍵的人做相反的事，本題不讀那兩份。**真正在看這五種的是複審的眼睛。**
