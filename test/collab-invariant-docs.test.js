...
test('工作區方案（實作常設／審查拋棄）：白名單句庫＋出現次數（改任何一份複本都會紅）＋第 6 題「問法與逾時預設」的承重句', () => {
  ...
  const PINS = [
    ...
    ['AGENTS.md', '**「做」含照上方常設授權合進 main**', 1],   // William 2026-09-06 裁「含」：不可縮回「合併等他」
    ['AGENTS.md', '**判準**與**架構都在射程內**', 1],   // William 2026-09-06 裁「適用」
    ['AGENTS.md', '放寬考題等他，其餘判準與架構照逾時預設', 1],   // 判準裡的 carve-out：弱化 CI 考題仍屬③
...
test('⭐ 第 6 題正本在正式位置：「問法與逾時預設」那顆要在「審查回饋處置」節裡、「界線表」之前，而且承重句都在那顆裡（逐字搬到沿革節＝規則降成沿革，要紅；#577 r3）', () => {
  ...
  for (const line of [
    ...
    '**「做」含照上方常設授權合進 main**', '**判準**與**架構都在射程內**', '放寬考題等他，其餘判準與架構照逾時預設',   // William 2026-09-06 兩題的裁示與 carve-out
  ]) {
