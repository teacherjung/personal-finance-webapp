...
//    ⇒ 換一條軸：**不看結構，只數字眼**。規則書裡「作廢／廢止」這兩個詞現在各出現幾次是固定的，
//    多一次就叫一聲，訊息請人去看那個字是不是指著「問法與逾時預設」那一節。
test('⭐ 作廢字眼絆線：規則書裡「作廢／廢止」的次數是固定的，多一次就要有人看一眼', () => {
  // ⚠️ 這是**絆線，不是閘**：
  //   ・它**不判斷**那個字指的是哪一節——判不出來，也不該在這裡長出第二份判斷。
  //   ・它**擋不住換句話說**：實測「本節僅為歷史紀錄」「已不再適用」都不含這兩個詞，照樣全綠。
  //   ・它**擋不住純 `<blockquote>`／`<details>` 包起來**（那兩種可以一個作廢字都不寫）。
  //   以上三項是**照實列的射程邊界**，不是漏掉。它買到的是：最常見、最便宜的那一刀會出聲。
  ...
  for (const [word, expected] of [['作廢', 11], ['廢止', 5]]) {
    ...
    assert.equal(got, expected,
      `AGENTS.md 裡「${word}」出現 ${got} 次（原本 ${expected} 次）——`
...
test('⭐ 逾時預設那一顆：**整節逐字**（從「審查回饋處置」那行到界線表整張表的末尾，一行不差）＋**整條祖先標題鏈**逐字——節內插一句「以下作廢」、往界線表插一列作廢、改擁有標題自己的後半、或在鏈上任何一段插標題把整節收進作廢小節，都會紅', () => {
  ...
  //     不是祖先容器，所以連換 Markdown 解析器也擋不到）②`<details><summary>已作廢</summary>` 把整節收摺
  //     ③`<blockquote>` 把整節包起來 ④跨行的 raw HTML 標題 ⑤離本節更遠的散文。
