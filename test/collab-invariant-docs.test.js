...
test('⭐ 逾時預設的射程：三段承重條文要**逐字整段**在正本那顆裡（連左邊的錨一起比，所以「不得解讀為…」「並非…」這種原地否定會把整段打斷、要紅）', () => {
  // 為什麼不是數子字串出現幾次（題名關鍵字「工作區方案（實作常設／審查拋棄）」那題的做法）：
  // 在原句前面加一個否定詞，子字串照樣出現一次、位置也沒變，語意卻整個反過來（`Codex #578 r1` 實證）。
  // 所以這裡比的是**封閉區段的等值**：從左邊那個固定的錨開始、到句子結束，整段逐字相同才算。
  // 黑名單式的「不准出現『並非』」擋不住換字，所以不用那條路。
  // 歸屬：①②是 William 2026-09-06 的裁示（原話在 AGENTS 與 #578 的 ⚖️ 留言）；③是 Claude 為了讓「判準」與
  // 「③閘變鬆」兩條規則不打架而加的劃界，不是他的話。
  const agents = read('AGENTS.md');
  ...
  const CLAUSES = [
    // ① William 的裁示：「先做」含合進 main
    '**時限內他沒回＝照我建議的預設先做、留痕、他隨時可翻案**（**「做」含照上方常設授權合進 main**——William 2026-09-06 裁，原話逐字「「先做」含不含合併：含」，落點＝#578 裡他的留言；問這一題的 ❓ 在 #577）',
    // ② William 的裁示：判準與架構都在射程內
    '。⚠️ 上面 08-14 三條例外裡的**判準**與**架構都在射程內**——William 2026-09-06 裁，原話逐字「判準與架構適用」，落點＝#578 裡他的留言；問這一題的 ❓ 在 #577。',
    // ③ Claude 的劃界：判準裡「弱化會進三關／CI 的考題通過條件」仍屬③（收緊不受限）
    '⚠️ **但判準裡有一塊仍屬③、永遠等他**：弱化會進三關／CI 的考題通過條件（界線表把「考題的通過條件」算在判準裡，③把「弱化」那一面留給他）——**放寬考題等他，其餘判準與架構照逾時預設**。',
  ];
  for (const clause of CLAUSES) {
    assert.equal(block.split(clause).length - 1, 1,
      `這一段承重條文不是逐字整段在那顆裡（或不只一次）：\n  ${clause.slice(0, 60)}…\n`
      + '⚠️ 在它前面加否定詞、或把中間拆開，都會讓這題紅——要改條文，先來改這裡（變更必經考題）。');
  }
});
