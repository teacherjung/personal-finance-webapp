...
//    所以**改寫那個列名這題會紅**——Codex #597 r3 實測：第 9 列改名仍綠、第 10 列改名就紅。要改名＝兩份一起改。
...
//    （規則書該段已照實劃界，本題不冒充補上）。
test('⭐ 規則書說固定維度有那兩列，程序那份的那一節裡就要真的有（兩份不可單邊消失）', () => {
  const MARK = '🧭 自評';
  // ⚠️ **2026-09-11 起守兩列**：原本只守「七輪自評」那一列；同一天的「第三輪自問表」也在 AGENTS 裡
  ...
  assert.ok(hit10.length > 0,
    ...
    + '但 AGENTS.md 的第三輪自問表正在告訴讀者「固定維度新增一列」。\n'
