...
test('⭐ 逾時預設那一顆：**整節逐字**（從「審查回饋處置」那行到界線表整張表的末尾，一行不差）＋**整條祖先標題鏈**逐字——節內插一句「以下作廢」、往界線表插一列作廢、改擁有標題自己的後半、或在鏈上任何一段插標題把整節收進作廢小節，都會紅', () => {
  ...
  //   →鎖整節二十三行（#578 r6 兩刀：界線表那行與它的表格之間插一句可見的粗體「以上整節已作廢」；本節前面插 `#### 以下整節已作廢`）
  //   →鎖到表頭分隔行、且只逐字比對直接的擁有標題（#578 r7 三刀：**改擁有標題自己的後半**、在擁有標題**上面**插一個 H2、往表格**插一列**寫作廢）。
  // 前七輪每一次修法都只把邊界往外推一格，於是下一刀就站到新邊界外面。這裡改成兩件**各自封閉**的事：
  ...
  //     （標題＝ATX／Setext／raw HTML `<h1>`～`<h6>` 三種，那是 GitHub 會渲染成標題的完整集合）。
  //     這一項是封閉的：任何「用標題把本節收進作廢小節」的寫法都得插在鏈的某一段裡，而每一段都查了。
  // ⚠️ 誠實劃界：**非標題的散文**看不到（例如整份 AGENTS.md 開頭寫一句「以下全部作廢」）——
  //   那是這一族的射程邊界，不是它宣稱守得住的東西。文件層級的整份作廢要靠人在 diff 裡看見，不是靠字串釘擋。
  ...
  // ⚠️ 這一段是**封閉**的論證，不是再往外推一格：任何「能讓本節在畫面上落進作廢小節」的新標題，
  //   一定得插在這條鏈的某一段裡（不然它管不到本節），而每一段都被查了。H1 還要求是全檔第一個標題。
  // ⚠️ 射程邊界（照實寫）：**非標題的散文**不在這裡的射程內——鏈完整、30 行逐字相符，
  //   仍然擋不住有人在別處寫一句「以上全部作廢」。那一類靠人在 diff 裡看見，這道考題不宣稱擋得住。
  // GitHub 真的會渲染成「標題」的寫法只有三種：ATX（`# …`）、Setext（段落下一行畫 = 或 -），
  //   以及 raw HTML 的 `<h1>`～`<h6>`。前七輪只認前兩種，於是 `前文 <h4>以下整節已作廢</h4>` 這種
  //   **寫在行中間**的 HTML 標題整條鏈都查不到（#578 r8 High①；GitHub 官方 API 實測輸出 `<p>前文 </p><h4>…</h4>`，
  //   而全域那道 raw HTML 護欄刻意只擋行首）。這三種是完整集合，補齊之後這條鏈的封閉論證才站得住。
  ...
  const headingAt = (/** @type {string[]} */ arr, /** @type {number} */ i) => {
    ...
    const noCode = arr[i].replace(/`[^`]*`/g, '');
    const html = [...noCode.matchAll(/<h([1-6])\b[^>]*>/gi)].map((x) => Number(x[1]));
  ...
  at.forEach((idx, k) => {
    ...
    for (let i = idx + 1; i < next; i += 1) {
      ...
      assert.ok(level === 0 || level > forbidUpTo,
        ...
        + '上面 30 行一字未動、鏈上每一行也都逐字相符，照樣能在畫面上把本節收進作廢的小節');
    }
  });
