...
test('工作區方案（實作常設／審查拋棄）：白名單句庫＋出現次數（改任何一份複本都會紅）＋第 6 題「問法與逾時預設」的承重句', () => {
  ...
  const PINS = [
    ...
    // 審查模型＝William 的裁示（2026-09-07）：三處＝發射指令那一行、「審查模型＝」那一條、
    // 以及**他的原話逐字**那一句。⚠️ 哪天換模型：前兩處改、**原話那一處不可以改**（改了就是竄改他說過的話），
    // 所以那時這個數字會變成 1，本考題會紅——那正是它要的摩擦。
    // ⚠️ 舊名 `gpt-5.6-sol` 在本檔仍有兩處，那是 #453 的歷史現場字串（同一位審查者被拆成兩個身分的實例），
    // **不是活指令、不要跟著改**。
...
test('⭐ 逾時預設那一顆：**整節逐字**（從「審查回饋處置」那行到界線表整張表的末尾，一行不差）＋**整條祖先標題鏈**逐字——節內插一句「以下作廢」、往界線表插一列作廢、改擁有標題自己的後半、或在鏈上任何一段插標題把整節收進作廢小節，都會紅', () => {
  ...
  //        而 #578 那幾輪的 High **全部**出在那兩種上，沒有一條跟 ATX 有關）
