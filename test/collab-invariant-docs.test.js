...
//    「幽靈閘兜底」那一題原本寄居在協作規矩路由表的考題檔裡，但它守的是**閘會不會跑**、
...
//    ・**兩題都只認 `MERGE_GATE` 這個名字**：改用別的 export 名開一道閘，兩邊都看不到。
//    ・「合併步驟裡實際會跑的那一組」怎麼認（只認 bash fence 裡逐字相符的標準指令行、
...
test('⭐ scripts/ 底下每一支提到 MERGE_GATE 的 js/mjs/cjs，都要真的出現在合併步驟裡（幽靈閘兜底）', () => {
  ...
  //    但只要有一邊搬家或改層數，列檔就可能回空陣列，而**下面那個迴圈一個斷言都不會執行＝靜靜全綠**。
  //    搬家前兩個動作共用同一個根，沒有這條分叉。⇒ fail-closed：空集合、或列不到「合併步驟
  //    實際會跑的那幾支」，都當作列檔壞了。
  ...
  for (const g of running) {
    assert.ok(files.includes(g),
      `合併步驟裡實際會跑 ${g}，但 \`scriptFiles()\` 沒有列到它。`
