...
test('⭐ 逾時預設那一顆：**整節逐字**（從「審查回饋處置」那行到界線表整張表的末尾，一行不差）＋**整條祖先標題鏈**逐字——節內插一句「以下作廢」、往界線表插一列作廢、改擁有標題自己的後半、或在鏈上任何一段插標題把整節收進作廢小節，都會紅', () => {
  ...
  assert.doesNotMatch(section, /^ *\[[^\]]+\]:\s/mu, '這一節裡出現 Markdown 參考定義——那在畫面上不顯示，藏得下一句否定'
    ...
    + '⚠️ 代價：縮排四格以上的「長得像參考定義」其實會渲染成可見的程式碼文字，這裡一律拒收＝比規格嚴。'
...
//      理由是「範圍與成本」，不是「做不到」。
//    ・**從條外開一個會把整條吞掉的區塊**（開了不關的程式碼圍欄、`<div>` 這類區塊標籤，開標在這一條
//      前面、收標在後面）：這一條的每一行都沒被動到，白名單與條界都不會叫——**本題接不到**。
//      ⚠️ 實測它現在會紅，但**不是本題擋的，也不是誰為它設計的**：`test/contract-split.test.js`
//      那一組在讀 AGENTS.md 的索引列時會被沒關的區塊弄壞而失敗＝**順帶接住**。
//      所以別把它當成穩定的保證——那一組哪天換讀法，這一族就沒有人接了。（Grok #585 掃後點名；
//      牠當時判斷「行首 HTML 沒有人接」，實測是交叉組會紅，但接住的理由跟牠想的不同。）
//    ・**Setext 造的「歷史紀錄」標題**：判準只認 ATX（見 markdown-heading 檔頭），**沒有東西接**。
//      行首 raw HTML 標題同上一點，由 `contract-split` 那一組順帶接住（r2／r5／Grok 三次實測交叉組會紅）。
