...
test('⭐ 逾時預設的射程：那兩行承重條文要**整行逐字相同、而且從第 0 欄開始**（行首加「以下只是錯誤範例」這種前綴、或整段包進 HTML 註解，都會紅）', () => {
  // 三版都被打穿過，寫進來當沿革：
  //   ・數子字串出現幾次 → 在原句前面加「不得解讀為」，次數不變、語意反過來（`Codex #578 r1`）。
  //   ・比含左錨的一小段 → 把否定詞放到那一小段外面就繞過（`Codex #578 r2`）。
  //   ・用 indexOf 找行中的子字串再 slice → **同一行左邊的前綴被切掉看不到**，行首加「以下兩行只是錯誤範例」照樣全綠（`Codex #578 r3`）。
  // 所以這一版：①用**實體行**逐行等值找起點（不 trim，所以第一行必須從第 0 欄的 `-` 開始、第二行的縮排也逐字固定）
  //   ②兩行必須相鄰、且落在「審查回饋處置」與「界線表」之間 ③那一段裡不得有 HTML 註解。
  // 代價是正常改字也要回來改這裡——對這兩行刻意當成安全契約的文字，那正是要的（變更必經考題）。
  // 歸屬：William 2026-09-06 的兩項裁示（「先做」含合進 main、判準與架構都在射程內）寫在這兩行裡；
  //   「弱化會進三關／CI 的考題通過條件仍屬③」那一句是 Claude 為了讓兩條規則不打架而加的劃界，不是他的話。
  ...
  const lineIndex = (/** @type {string} */ canon) => {
    const hits = lines.map((l, i) => (l === canon ? i : -1)).filter((i) => i >= 0);
    assert.equal(hits.length, 1,
      `這一整行承重條文不是逐字相同（或不只一行）：\n  ${canon.slice(0, 60)}…\n`
      + '⚠️ 行首加前綴、行內插字、改落點編號都會讓這題紅——要改條文，先來改這裡（變更必經考題）。');
    return hits[0];
  };
  const first = lineIndex(CANONICAL[0]);
  const second = lineIndex(CANONICAL[1]);
  assert.equal(second, first + 1, '那兩行要相鄰——中間插東西＝把裁示與它的原話拆開');
  const ruleStart = lines.findIndex((l) => l.startsWith('**審查回饋處置（'));
  const guardStart = lines.findIndex((l, i) => i > second && l.startsWith('**界線表（'));
  assert.ok(ruleStart >= 0 && ruleStart < first, '那兩行要在「審查回饋處置」節裡（不是被搬到別節）');
  assert.ok(guardStart > second, '那兩行後面要接著「界線表」（同一節的下一顆）');
  assert.doesNotMatch(lines.slice(ruleStart, guardStart).join('\n'), /<!--|-->/,
    '這一節裡出現 HTML 註解——規則不可以被藏成不可見內容（畫面上消失、字串還在）');
});
