...
test('⭐ 逾時預設的射程：那兩行承重條文要**整行逐字相同**（在錨外面加否定詞、或把整段包進 HTML 註解藏起來，都會紅）', () => {
  // 為什麼不是數子字串出現幾次、也不是只比一小段（前兩版被 `Codex #578 r1／r2` 各打穿一次）：
  //   ・只數次數 → 在原句前面加「不得解讀為」，子字串照樣一次、語意反過來。
  //   ・只比一小段（含左錨）→ 把否定詞放到那一小段的**外面**就繞過去了。
  //   ・整段原封不動包進 `<!-- -->` → 字串還在、畫面上規則消失。
  // 所以這裡①先拒絕整顆裡出現 HTML 註解（規則區塊沒有理由藏東西）②再要求承載規則的那兩**整行**逐字相同。
  ...
  assert.ok(blockStart >= 0 && blockEnd > blockStart, '找不到「問法與逾時預設」那顆');
  const block = agents.slice(blockStart, blockEnd);
  assert.doesNotMatch(block, /<!--|-->/, '那顆裡出現 HTML 註解——規則區塊不可以藏不可見內容（整段包進註解＝畫面上規則消失、字串還在）');
  ...
  for (const line of CANONICAL) {
    assert.equal(lines.filter((l) => l === line).length, 1,
      ...
      + '⚠️ 在行內任何位置加否定詞、插字、改落點編號都會讓這題紅——要改條文，先來改這裡（變更必經考題）。');
  }
});
