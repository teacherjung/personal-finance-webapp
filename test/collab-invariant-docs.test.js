...
test('⭐ 逾時預設那一顆：**整顆五行逐字、依序、相鄰**，後面緊接空行與「界線表」（加一行「以上僅供說明」、插參考定義、加前綴、包註解，全都會紅）', () => {
  // 五版都被打穿過，寫進來當沿革（每一版都以為關起來了）：
  //   ・數子字串出現幾次 → 在原句前面加否定詞，次數不變、語意反過來（`Codex #578 r1`）。
  //   ・比含左錨的一小段 → 把否定詞放到那一小段外面（`Codex #578 r2`）。
  //   ・用 indexOf 找行中的子字串再切 → 同一行左邊的前綴被切掉看不到（`Codex #578 r3`）。
  //   ・只鎖兩行、只要求相鄰 → 在後面補一行「以上兩句僅供說明，合併仍等他」；或用 Markdown 參考定義
  //     `[x]: # (否定句)`（GitHub 上不顯示、也不是 HTML 註解）（`Grok #578 掃`）。
  // 這一版鎖**整顆**：五行逐字、依序、相鄰，後面緊接一個空行與「界線表」——任何插進來的句子都會把它撐開。
  // 代價是這一顆的任何一個字改動都要回來改這裡；這一顆是安全契約，那正是要的（變更必經考題）。
  // 歸屬：William 2026-09-06 的兩項裁示（「先做」含合進 main、判準與架構都在射程內；落點＝#578 裡他的留言）
  //   寫在前兩行；「弱化會進三關／CI 的考題通過條件仍屬③」那一句在正本裡已自報是 Claude 的劃界、不是他的話。
  ...
  const first = lines.findIndex((l) => l === CANONICAL[0]);
  assert.ok(first >= 0, `這一顆的第一行不是逐字相同（或不在第 0 欄）：\n  ${CANONICAL[0].slice(0, 60)}…`);
  CANONICAL.forEach((canon, k) => {
    assert.equal(lines.filter((l) => l === canon).length, 1,
      `第 ${k + 1} 行不是逐字相同、或出現不只一次：\n  ${canon.slice(0, 60)}…\n`
      + '⚠️ 行首加前綴、行內插字、改落點編號都會讓這題紅——要改條文，先來改這裡（變更必經考題）。');
    assert.equal(lines[first + k], canon, `第 ${k + 1} 行不在它該在的位置——中間被插了東西？`);
  });
  const after = first + CANONICAL.length;
  assert.equal(lines[after], '', '這一顆後面要緊接一個空行——多出來的句子會被塞在這裡');
  assert.ok(lines[after + 1].startsWith('**界線表（'), '空行後面要緊接「界線表」那一顆（同一節的下一顆）');
  const ruleStart = lines.findIndex((l) => l.startsWith('**審查回饋處置（'));
  assert.ok(ruleStart >= 0 && ruleStart < first, '這一顆要在「審查回饋處置」節裡（不是被搬到別節）');
  const section = lines.slice(ruleStart, after + 1).join('\n');
