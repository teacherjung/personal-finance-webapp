...
//      理由是「範圍與成本」，不是「做不到」。
//    ・**Setext 與行首 raw HTML 造的「歷史紀錄」標題**：判準只認 ATX（見 markdown-heading 檔頭）；
//      行首 raw HTML 由既有的 HTML 禁令接（r2／r5 實測交叉組會紅），Setext 沒有東西接。
...
test('⭐ 鐵則 12「必須懂的概念要在網頁上就地白話解釋」要留在正式鐵則區、承重句在它自己那一條裡（搬成沿革要紅；William 2026-07-22 定、2026-09-08 獨立）', () => {
  ...
  while (got.length && got[got.length - 1].trim() === '') got.pop();
