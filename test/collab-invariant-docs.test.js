...
test('⭐ 逾時預設那一顆：**整節逐字**（從「審查回饋處置」那行到界線表的表頭分隔行，一行不差）＋本節前面不得插標題——節內插一句「以下作廢」、把界線表那行後半改成作廢宣告、在那行與表格之間補一句作廢、或用一個新小節把整節吃掉，都會紅', () => {
  // 這一族被連續打穿過好幾輪，形狀一次比一次外面：數子字串→比一小段→indexOf 切掉左前綴→只鎖五行（末尾補一句、參考定義）
  //   →鎖五行＋前後緊鄰（節內更前面插一顆「以下作廢」的同層項目、或把界線表那行的後半改成「以上五行作廢」）
  //   →只鎖到界線表那一行為止（#578 r6 的兩刀：在界線表那行與它的表格之間插一句畫面可見的粗體「以上整節已作廢」；
  //      或在本節前面插一個 `#### 以下整節已作廢`，讓整節落進那個新的小節）。
  // 每一次修法都只把邊界往外推一格，於是下一刀就站到新邊界外面。所以這裡釘的是**一段連續的字**加**一個結構條件**：
  //   ①從「**審查回饋處置（」那一行到界線表的表頭分隔行為止，逐行等值、依序、無增無減
  //     （界線表那行與表格之間塞不進東西——中間那一行空行本身也在釘的範圍裡）；
  //   ②本節與它的擁有標題「### 三方協作框架（」之間，不得出現任何形狀的標題（ATX 含前導空白、Setext 底線）。
  // ⚠️ 誠實劃界：離本節更遠的散文（例如整份 AGENTS.md 開頭寫一句「以下全部作廢」）本考題看不到——
  //   那是這一族的射程邊界，不是它宣稱守得住的東西。文件層級的整份作廢要靠人看見、靠 PR 審查擋，不是靠字串釘擋。
  ...
  // 節外前置作廢：本節前面插一個 `#### 以下整節已作廢`，上面 26 行一字未動、整節卻在畫面上落進新的小節。
  // CommonMark 的標題不只「行首 # 加空白」：ATX 可有 0～3 個前導空白、井號後可接空白／tab／行尾；Setext 是段落下一行的 = 或 - 底線。
  const headingAt = (/** @type {string[]} */ arr, /** @type {number} */ i) =>
    /^ {0,3}#{1,6}(?:[ \t]|$)/.test(arr[i])
    || (i > 0 && /^ {0,3}(?:=+|-+)[ \t]*$/.test(arr[i]) && arr[i - 1].trim() !== '');
  const OWNER = '### 三方協作框架（';
  const owner = lines.findIndex((l) => l.startsWith(OWNER));
  assert.ok(owner >= 0, `找不到本節的擁有標題「${OWNER}」`);
  assert.ok(owner < first, '本節的擁有標題跑到本節後面去了＝本節已經不屬於它');
  const gap = lines.slice(owner + 1, first);
  const bad = gap.findIndex((_, i) => headingAt(gap, i));
  assert.equal(bad, -1,
    `擁有標題「三方協作框架」與本節之間插了一個標題（第 ${owner + 2 + bad} 行：「${String(gap[bad]).slice(0, 40)}」）`
    + '——那會把整節切進一個新的小節，上面 26 行一字未動也能在畫面上宣告整節作廢');
