...
test('⭐ 逾時預設那一顆：**整節逐字**（從「審查回饋處置」那行到「界線表」那行，一行不差）——在節內任何位置插一句「以下作廢」、或改界線表那行的後半宣告作廢，都會紅', () => {
  // 這一族被連續打穿過好幾輪，形狀一次比一次外面：數子字串→比一小段→indexOf 切掉左前綴→只鎖五行（末尾補一句、參考定義）
  //   →鎖五行＋前後緊鄰（在節內更前面插一顆「以下作廢」的同層項目、或把界線表那行的後半改成「以上五行作廢」）。
  // 每一次修法都只把邊界往外推一格，於是下一刀就站到新邊界外面。所以這一版直接鎖**整節逐字**：
  // 從「**審查回饋處置（」那一行到「**界線表（」那一行，逐行等值、依序、無增無減。
  // ⚠️ 誠實劃界：這一節**以外**的文字（例如在整節前面另寫一句「以下整節作廢」）本考題看不到——
  //   那是這一族的射程邊界，不是它宣稱守得住的東西。同族再往外推的形狀進待辦，不再往外追（追不完）。
  // 代價：這一節的任何一個字改動都要回來改這裡。這一節是安全契約（誰拍板／值不值得改／怎麼問與沒回怎麼辦），那正是要的。
  // 歸屬：William 2026-09-06 的兩項裁示（「先做」含合進 main、判準與架構都在射程內；落點＝#578 裡他的留言）
  //   寫在「問法與逾時預設」那一顆的前兩行；「弱化會進三關／CI 的考題通過條件仍屬③」那一句在正本裡已自報是 Claude 的劃界。
  ...
  assert.notEqual(lines[first + SECTION.length], SECTION[SECTION.length - 1], '界線表那一行後面又冒出一行一樣的？');
