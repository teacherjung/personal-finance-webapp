...
//      Markdown 引擎，為一道文件保存題引進一個新相依不划算；而退而求其次的做法
//      （把「UI 現行慣例」第 1〜4 點也逐行釘住）會把一個**設計上就要一直改**的章節鎖死，
//      本身就是新的假保證。⇒ **記為待辦，不在本支做**。
...
//
//    ⚠️ **本題防的是「整理文件時手滑，把它跟空樁一起收掉」**——用戶是未來的自己人，不是攻擊者
//    （同 markdown-heading 檔頭的威脅模型）。上面那些「擋不到」都需要**刻意**寫出合法但反常的
//    語法，不是手滑做得出來的。本題也**不**證明任何人真的照做（那要靠複審與 William 驗收）。
