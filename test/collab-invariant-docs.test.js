...
//    核對義務拿掉。
// ⚠️ **第三輪自問那半不一樣：它認的是固定的列名片段**「第三輪自問的降級理由」（那一列沒有自己的記號可認），
//    所以**改寫那個列名這題會紅**——Codex #597 r3 實測：第 9 列改名仍綠、第 10 列改名就紅（2026-09-11 兩列併成一列兩款後，兩個片段在同一列裡，判準不變）。要改名＝兩份一起改。
//    （上一版這段註解寫「判準只認一個記號、不認列名」，擴成兩列之後對第 10 列是假話，r3 抓到。）
...
//    （規則書該段已照實劃界，本題不冒充補上）。
test('⭐ 規則書說固定維度有那一列兩款，程序那份的那一節裡就要真的有（兩份不可單邊消失）', () => {
  const MARK = '🧭 自評';
  // ⚠️ **2026-09-11 起守兩個片段**（同日稍後兩列併成一列兩款，兩個片段落在同一列——判準不變）：原本只守「七輪自評」那一列；同一天的「第三輪自問表」也在 AGENTS 裡
  //    宣稱「`REVIEW-AND-MERGE.md`「固定維度」新增一列」——那句話同樣會因為那一列被刪而變成假話
  //    （Grok #597 複審後掃指出：不加那一列，審查者跑完下限也不會被要求去核降級理由）。
  for (const [claim, why] of /** @type {[string, string][]} */ ([
    ['「固定維度」那節有一條', '自評款（第二款）'],
    ['「固定維度」那一列的第一款', '第三輪自問表（第一款）'],
  ])) {
  ...
  // 第三輪自問表那一列：只認「以 | 開頭且提到這個維度名」——同上，只看有沒有，不判內容
  ...
  assert.ok(hit10.length > 0,
    ...
    + '但 AGENTS.md 的第三輪自問表（第一款）正在告訴讀者「固定維度那一列的第一款」。\n'
