 ...
 *     ⚠️ **2026-09-24 起這個補救的射程變窄**（#641 r2 #1）：範圍＝「**兩張樣式表**」那一段（正本在
 *     `tools/forbidden-tools.js`，這裡不重述）。加前綴名單只救得到那一段裡 `patterns` 那一列；
 *     動詞名詞那兩道 ⇒ `obtain_create_order` 就算把 `obtain` 加進名單**仍然會被擋**（實測：
 *     主幹加了就放行、本版仍拒絕）。命中家族網的誤攔要照裁示流程處理，加前綴名單救不了。
 *     ②跨域名詞撞名（delete_stock_photo 型）仍會誤攔（trademark／sharepoint 型已用
 *     名詞邊界修掉）；③名字說謊的工具（叫 get_ 卻會動錢）——⚠️ **2026-09-24 起這句要收窄**
 *     （Grok 複審後掃 #1）：名字裡**同時**有唯讀前綴與動詞名詞的那一類**現在擋得住**
 *     （`view_create_order`／`get_place_order`／`get_create_order_status` 實測皆拒絕）。
 *     ⚠️ **仍然免疫不了的不只「名字裡根本沒有動作詞」那一類**（r4 #2 抓到我把它講太小）：
 *     `download_transfer_funds`、`read_withdraw_cash` **名字裡就帶著動作詞**、而且現在仍然放行
 *     （它們只命中「兩張樣式表」那一段裡 `patterns` 那一列）。所以免疫不了的是
 *     「①名字裡沒有動作詞的」＋「②只命中那一列又帶唯讀前綴的」兩類——
