...
test('⭐ CANON 各份：恰一行符合 POINTER_LINE、索引小於首個 ^## 、且含 POINTER_WORDS', () => {
  ...
  //
  // ⚠️ **這一題斷言了什麼**（William 2026-09-03 裁定的停損點；照實列，不多說）：
  //   ① `CANON` 列的每一份文件，各有**恰好一行**符合 `POINTER_LINE`
  //   ② 那一行的索引 **小於**該檔第一個 `^## ` 的索引（找不到 `^## ` 就 fail-closed）
  //   ③ 那一行含 `POINTER_WORDS` 的**每一個**詞
  //
  //   **沒有斷言其他任何事。** 特別是：**沒有**檢查那一行渲染之後長什麼樣、
  //   讀者看不看得到、點不點得到、那句話的意思對不對。
  //
  //   ⚠️ **這裡刻意不維護「哪些寫法擋得到／擋不到」的清單。**
  //   `Codex #548` 的 r10、r11、r12、r13 連續四輪，抓到的都不是護欄有洞，
  //   而是**我寫的那份清單與實作對不上**（把擋得到的寫成擋不到、漏列、
  //   還編了一個「共通點」當通則，而它既非必要也非充分條件）。
  ...
  //   ⚠️ 這裡不再補一句「所以它防的是什麼」——`Codex #548 r14` 指出我寫的那句
  //   （「防自然的腐爛，例如節被改名」）**也不成立**：把 `AGENTS.md` 的首個 H2 改個名，
  //   這一題照樣全綠（它只看有沒有 `^## ` 與索引，不看節名）。上面那三條就是全部。
  //
