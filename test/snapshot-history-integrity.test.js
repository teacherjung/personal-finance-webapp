...
test('日線匯率｜留底的匯率要與同一列五個金額欄實際用的同一個：沒設定時走同一個預設、使用者設過就要用設定值', async (t) => {
  ...
  // ⚠️ 兩個 case 缺一不可（Codex r2）：只跑 `settings: {}` 的話，「正確值」「預設常數」
  //    「留底」三者剛好都是同一個數，斷言分不出寫進去的數字是從哪個來源來的——把寫入改成寫死那個數
  ...
  // ①完全沒有 usdTwd：走「預設值」那條路，兩邊必須是同一個預設（fx-rates.js 的常數；William 2026-09-04 裁 31）
  const { FX_DEFAULT_TWD } = await import('../public/modules/fx-rates.js');
  await check({}, FX_DEFAULT_TWD.USD, '設定裡沒有 usdTwd');
