...
test('P1b-3 攔截率｜每一個盲點都要真的出現在畫面上，而且不可寫死件數', () => {
  ...
  //    這件事也只在金額大於容差時成立（amount 0 的殘餘另有考題「誠實殘餘：金額 0 的列方向對調」）。
  const APPROVED_BLIND_SPOTS = "⚠️ <b>這道驗算看不到的（不只這些）</b>——不過現在多了兩道幫手（<b>接地檢查</b>＝答案上的每個<b>金額、餘額、合計</b>數字都要在帳單上找得到（日期刻意不查——帳單印的日期格式（例如民國年）轉成標準寫法後字面就不同了，逐字對不起來）；<b>合計交叉驗證</b>＝帳單自己印的筆數與出入合計要對得上——⚠️ <b>這一道不是每份帳單都跑得起來</b>：帳單<b>同時有台幣與外幣</b>時整道關閉（合計涵蓋哪一段判不出來，硬比會誤擋正確答案）、帳單沒印合計時也沒得對；<b>而且它比到哪幾欄，就只擋得住會改動那幾欄的錯</b>——<b>這一份到底跑了什麼，寫在下面「帳單數學驗算」那一段</b>），哪幾種擋得住、什麼情況仍看不到，寫在各條後面：①每個帳戶的<b>第一筆</b>——驗算是拿它的<b>餘額</b>去比下一筆，它的<b>金額與方向</b>都沒有被驗到（方向讀反＝<b>收入被記成支出</b>）；<b>整筆漏掉也一樣</b>→<b>金額</b>抄錯：錯的數字不在帳單上＝接地擋、合計那道比到了<b>同一個方向</b>的合計（抄錯的是支出，就要比到支出合計）＝合計也擋；<b>整筆漏掉</b>：比到<b>筆數</b>、或比到<b>那一筆方向</b>的合計＝擋得住（漏掉一筆收入時，只比支出合計是看不到的）；<b>方向讀反</b>：要比到<b>出入合計</b>（對調會讓兩邊同時變，任一邊都抓得到；<b>金額 0 的列除外</b>）——只比到筆數＝仍看不到（方向對調不會改變筆數） ②<b>外幣</b>明細（本來就不計入台幣收支；外幣帳戶餘額仍會照帳單更新）→金額抄錯、錯的數字不在帳單上＝會被接地擋下 ③<b>這期沒有往來的帳戶</b>（只出現在概要、沒有明細可驗，但餘額仍會更新或新建帳戶）→概要餘額抄錯、錯的數字不在帳單上＝會被接地擋下 ④金額和餘額<b>一起</b>被抄成剛好自洽的另一組數字→湊出來的數字不在帳單上＝接地擋下；<b>恰好都是帳單他處印過的數字、而且合計那道沒跑（沒印、或混幣關閉）或合計也被一起抄成自洽＝仍看不到</b> ⑤某筆金額抄錯、<b>而且同一筆的餘額是空白</b>→同④ ⑥一筆支出和一筆收入被<b>併成一筆淨額</b>（餘額照樣接得上，但收入與支出的總額都錯了）→淨額數字不在帳單上、或<b>合計那道有跑</b>＝擋下；兩個條件都不成立＝仍看不到 ⑦<b>整個帳戶被漏讀</b>（沒讀到的東西沒有數字可以驗）→比到<b>筆數</b>、或比到那個帳戶動到的<b>那一側</b>合計＝擋下；沒跑（帳單沒印、或台幣外幣混合）＝仍看不到 ⑧台幣與外幣<b>互相認錯</b>——認成外幣＝那個帳戶的交易<b>一筆都不會進帳</b>；認成台幣＝<b>外幣數字被當成台幣入帳</b>。兩種畫面都會說驗算通過，<b>三道防線都看不到＝最重的已知盲點</b> ⑨<b>摘要或日期讀錯</b>——金額是對的，但那兩欄是「這筆有沒有匯過」的辨識依據，<b>下次重匯同一份帳單時會被當成新的一筆再記一次</b>；開著「雙讀」時兩個 AI 讀得不一樣會被比對器抓到，<b>兩個都錯得一模一樣才會漏過去</b>。<br>⚠️ <b>這份清單不保證完整</b>——⑤～⑧是 2026-08-13 覆審時才發現的，往後可能還有。所以下面兩張表還是請你自己看一眼，尤其是上面那張「帳戶餘額」。";
