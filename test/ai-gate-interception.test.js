...
// ⚠️ P2-2a（2026-08-15）起，正式 AI 路線在閘之前另有兩道不在本份射程的防線（接地檢查＋合計欄
//    交叉驗證）——本份量的是**閘**的條件攔截率，數字與 §八 的互扣不受那兩道影響、也不涵蓋它們。
...
const CASES = [
  ...
  // ── D 類：**金額與方向全對**，但驗算根本蓋不到那個帳戶 ⇒ ★6 保守拒收 ──
  // ⚠️ **不可寫成「金額全對＝帳本不會出錯」**（r13 指正，與 r12 同型）：餘額欄本身也是
  //    去重鍵 `bankRef` 的一部分——清空餘額 ⇒ 指紋變了 ⇒ 先匯錯版再匯正確版會**重複入帳**。
  //    它留在 D 類是因為**擋它的機制**是覆蓋政策（problems 空的），不是因為它無害。
  ...
  { id: 'D1', 類: 'D', base: corpusTwoAccounts, name: '某個台幣帳戶的餘額欄全空（該帳戶零驗證）', expect: 'caught',
    ...
    why: '★6 逐帳戶覆蓋：有任何一個受驗帳戶一道擋下型都沒吃到就拒收（level 是全檔旗標、擋不住搭便車）。'
      + '⚠️ **它也會破壞去重鍵**（r13）：餘額是 bankRef 的一部分，清空餘額＝指紋變了；'
      + '正式路線會被 ★6 擋下（所以不是生產漏洞），但**分類敘述不可寫成「帳本不會出錯」**。'
...
test('P1b-3 攔截率｜每一個盲點都要真的出現在畫面上，而且不可寫死件數', () => {
  ...
  //
  //   **守**＝這十二型盲點的文字**真的在徽章的輸出裡**（逐型比對，刪掉任一條就紅）。
  ...
  //   不該有人順手改掉而沒人看見），不是誤紅。
  // ⚠️ **2026-08-19 改版（William 指出：混幣時合計交叉驗證整個關掉、畫面卻說它擋得住——他自己的帳單正是混幣；轉述非逐字）**：
  //    ①⑥⑦的「帳單有印合計＝擋得住」改成有條件的講法（那道對混幣帳單整道跳過）；開場補「這一道不是每份帳單
  //    都跑得起來」與指路——**這一份到底跑了沒有**由後端裁決（totalsCheck）帶到預覽窗「帳單數學驗算」那一段。
  //    ⚠️ **同日第二次改口（複審後掃抓到）**：①原本寫「合計那道這次有跑＝擋得住」，但「有跑」可能
  //    只比到筆數——實測首筆 in→out 對調後筆數 3→3 一模一樣、只有出入合計會不符。所以①拆成三句：
  //    金額抄錯／整筆漏掉／方向讀反各自靠哪一欄，方向讀反明寫「只比到筆數＝仍看不到」。
  //    ⚠️ **第三次改口（Codex r4#1）**：上一版寫「比到出入合計就擋」仍然太寬——他用正式管線實測
  //    「首筆收入整筆漏掉、只比 totalOut」＝PASS。**同方向**才是那把刀，⑦同理；方向對調兩側同時變
  //    這件事也只在金額大於容差時成立（殘餘另有考題「誠實殘餘：金額 0 或不大於容差」）。
  //    ⚠️ **第四次改口＝William 2026-08-19 裁範圍**（這一族連三輪被中刀，每中一刀就多掛一句但書）：
  //    **收回誇大、不再加註**——講法一律改成「要比到 X 才擋得住」（必要條件），不再宣稱「比到了就擋得住」；
  //    開場自己講出真正的邊界（兩邊都出自同一份 AI 答案，整組一起抄錯看不出來）。容差殘餘不進畫面。
  //    ⚠️ **第五次改口（Codex r6#1）＝收回主詞**：畫面不可把 AI 回傳的數字稱作「帳單自己印的」——
  //    管線只能證明「AI 交回這個數字、而且它在原文某處出現過」，證明不了那是帳單印在合計欄的值
  //    （他實測：把自洽錯值散放在原文別處、連合計標籤都沒有，全管線照樣 pass）。⑥同時從充分改成必要。
  const APPROVED_BLIND_SPOTS = "⚠️ <b>這道驗算看不到的（不只這些）</b>——不過現在多了兩道幫手（<b>接地檢查</b>＝答案上的每個<b>金額、餘額、合計</b>數字都要在帳單上找得到（日期刻意不查——帳單印的日期格式（例如民國年）轉成標準寫法後字面就不同了，逐字對不起來）；<b>合計交叉驗證</b>＝AI 抄回來的筆數與出入合計，要跟它自己逐筆算的對得上——⚠️ <b>這一道不是每份帳單都跑得起來</b>：帳單<b>同時有台幣與外幣</b>時整道關閉（合計涵蓋哪一段判不出來，硬比會誤擋正確答案）、沒讀到合計欄時也沒得對；<b>而且它比的是「AI 抄的合計」與「AI 自己逐筆算的」——兩邊都出自同一份答案，整組一起抄錯它看不出來</b>（那一型見④）；<b>這一份到底比到了什麼，寫在下面「帳單數學驗算」那一段</b>），哪幾種擋得住、什麼情況仍看不到，寫在各條後面：①每個帳戶的<b>第一筆</b>——驗算是拿它的<b>餘額</b>去比下一筆，它的<b>金額與方向</b>都沒有被驗到（方向讀反＝<b>收入被記成支出</b>）；<b>整筆漏掉也一樣</b>→<b>金額</b>抄錯：錯的數字不在帳單上＝接地擋；合計那道<b>要比到「同一個方向」的合計</b>才擋得住（抄錯的是支出，就要比到支出合計）；<b>整筆漏掉</b>：要比到<b>筆數</b>或<b>那一筆方向</b>的合計（漏掉一筆收入時，只比支出合計是看不到的）；<b>方向讀反</b>：要比到<b>出入合計</b>——只比到筆數＝仍看不到（方向對調不會改變筆數） ②<b>外幣</b>明細（本來就不計入台幣收支；外幣帳戶餘額仍會照帳單更新）→金額抄錯、錯的數字不在帳單上＝會被接地擋下 ③<b>這期沒有往來的帳戶</b>（只出現在概要、沒有明細可驗，但餘額仍會更新或新建帳戶）→概要餘額抄錯、錯的數字不在帳單上＝會被接地擋下 ④金額和餘額<b>一起</b>被抄成剛好自洽的另一組數字→湊出來的數字不在帳單上＝接地擋下；<b>恰好都是帳單他處印過的數字、而且合計那道沒跑（沒讀到、或混幣關閉）或合計也被一起抄成自洽＝仍看不到</b> ⑤某筆金額抄錯、<b>而且同一筆的餘額是空白</b>→同④ ⑥一筆支出和一筆收入被<b>併成一筆淨額</b>（餘額照樣接得上，但收入與支出的總額都錯了）→要嘛淨額數字不在帳單上（接地擋）、要嘛合計那道比得到（併成一筆會讓<b>筆數與兩側合計都變</b>）才擋得住；兩個都不成立＝仍看不到 ⑦<b>整個帳戶被漏讀</b>（沒讀到的東西沒有數字可以驗）→要比到<b>筆數</b>、或那個帳戶動到的<b>那一側</b>合計才擋得住；沒跑（沒讀到合計、或台幣外幣混合）＝仍看不到 ⑧台幣與外幣<b>互相認錯</b>——認成外幣＝那個帳戶的交易<b>一筆都不會進帳</b>；認成台幣＝<b>外幣數字被當成台幣入帳</b>。兩種畫面都會說驗算通過，<b>三道防線都看不到＝最重的已知盲點</b> ⑨<b>摘要或日期讀錯</b>——金額是對的，但那兩欄是「這筆有沒有匯過」的辨識依據，<b>下次重匯同一份帳單時會被當成新的一筆再記一次</b>；開著「雙讀」時兩個 AI 讀得不一樣會被比對器抓到，<b>兩個都錯得一模一樣才會漏過去</b>。<br>⚠️ <b>這份清單不保證完整</b>——⑤～⑧是 2026-08-13 覆審時才發現的，往後可能還有。所以下面兩張表還是請你自己看一眼，尤其是上面那張「帳戶餘額」。";
...
test('P1b-3 攔截率｜計畫 §八 寫的數字＝這份考題實際量到的（兩邊互扣，改一邊另一邊就紅）', () => {
  ...
  // ★r13：D 類的**餘額欄也在去重鍵裡**——寫成「金額全對＝帳本不會出錯」會讓讀者以為它無害，
  //   但先匯錯版再匯正確版就會重複入帳（端到端考題釘住）。它留在 D 類是因為**擋它的機制**
  //   是覆蓋政策，不是因為無害。這句不見了，讀者就會誤讀分類。
  assert.match(plan, /別把它讀成「帳本不會出錯」|也會破壞去重鍵|餘額欄也是去重鍵/,
    '★§八 的 D 類要寫明它也會破壞去重鍵（不然「金額全對」會被讀成「帳本不會出錯」）');
