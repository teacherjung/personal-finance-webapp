...
> **適用檔案清單＝[README.md](README.md)「前端功能」節的責任檔案清單（單一真相，本頁首不重複維護一份會走散的副本）**——命中就必讀本檔。
