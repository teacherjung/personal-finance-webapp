...
- **送出之後**再開下一窗（密碼窗送出→等 preview→開預覽窗）→ 用 `onSubmit(out, ctx)` 拿到的 `ctx.owns.handoff()`。⚠️ 這裡**不可**用 `owns()` 或 `watchModalRoot()`：正常交接本來就會蓋兩次章（自己 `close` 時 `release`、下一窗 `claim`），用那兩個判會把**正常流程整個誤擋**。`handoff()` 分辨的是「動這一格的是不是我自己」——還是我的、或就是我自己 `release` 之後那個章＝放行；章比那更新＝**別人接管了**（使用者關掉我、改開別的窗），舊 continuation 不可以蓋掉它。判準寫在 `modal-ownership.js`（純邏輯有考題），呼叫端一律組成 `canOpenNext = () => onPage() && ctx.owns.handoff()` 再交給 `openWhenOnPage`。
