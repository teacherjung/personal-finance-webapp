...
**記得同步這裡**：**共用彈窗契約**（modal-shell.js 的邊界）：只共用**尺寸、標題列、關閉按鈕、背景與基本關閉行為**；送出、預覽、返回、非同步狀態與重畫流程**由各功能自行負責**。**#modal-root 世代擁有權（r6→r7）**：全站表單/彈窗共用同一格 `#modal-root`——表單 `onSubmit` 有 await，回來時可能 (1) 已切頁、或 (2) 期間開了新彈窗，舊的成功 continuation 若無條件 `close()` 會清掉**後開的**彈窗、毀掉未存輸入；舊的失敗會在新頁報過期錯誤。`owns()` 用**兩個判準**界定「還擁有這一格」：**世代章**（每次 `claimModalRoot()` 蓋新章；`releaseModalRoot()` 撤銷）＋**路由序號**（claim 當下記住 `currentRouteSeq()`，切頁後前進即作廢——修「單純切頁、沒有新彈窗」的漏）。規則：**任何直接把 `modal-bg` 外殼寫進 `#modal-root` 的開窗點都要 `claimModalRoot()`**（中央外殼 `app.js` 的 `openForm`／`openInfo`、`modal-shell.js` 的 `openModalShell` 內部已 claim；手刻的 `assets.js` 再平衡／目標配置、`settings-store-rules.js` 規則預覽也各自 claim）；**關窗即 `releaseModalRoot()`**（三個中央外殼的 `close` 都做）；`openForm` 的 async `onSubmit` 只在 `owns()` 為真時才 `close()`／`toast`。純邏輯在 `public/modules/modal-ownership.js`（`makeModalOwnership`，零依賴可測），考題＝`test/modal-ownership.test.js`（世代／release／切頁行為題＋接線形狀題＋**關門題**：掃 `public/` 所有直接寫 `modal-bg` 的開窗點，claim 次數必須 ≥ 開窗次數，防未來漏網）。
