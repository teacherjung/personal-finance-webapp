...
> ⚠️ **本支的選取母集合＝AGENTS 同步點表裡「還沒拆出去」的那 10 列**，不是整張 88 列。同一個判準套到**已經拆走、只剩索引的那 78 列**，也會命中 **17 列**跨領域（第 3、5、6、7、10、15、17、22、23、25、27、35、38、44、72、77、81 列）——⚠️ **這個 17 是兩種寫法的聯集**：只比對**完整路徑**會得到 10 列（#633 r1 的清單）、只比對**裸檔名**會得到 8 列（複審後掃補指出的那一族），**兩種各自都不完整**，合起來才是 17。先前這裡寫 10 列＝照單接受了單一方法的結果，已更正——**它們不在本支**（已各自住在領域契約裡），要不要重新分配是另一件事、記成待辦（#633 r1）。
