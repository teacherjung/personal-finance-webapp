...
> **內文＝AGENTS.md 原文逐字照搬**，轉換只有一類：表格列解框成「改這裡／記得同步這裡」兩段（跟前五份同樣的做法）；對照表在變更說明。
...
**記得同步這裡**：checklist：使用者可寫→補 **`lib/schema.js` 的 `WRITABLE_FIELDS`**（漏加會被默默剝掉、console 有警告）；數值/布林/枚舉/陣列→同補 **`FIELD_SCHEMA`**＋`lib/types.js` typedef；**服務層擁有的衍生欄位絕不進 CRUD 白名單**。原則、三道寫入閘門、逐集合歸屬與「PUT 挾帶假值」病史＝下方「**欄位所有權**」節（單一真相）。測試種帳單假資料走 repo 直寫（`server.test.js seedTx`），不可為了種資料把白名單加回去
