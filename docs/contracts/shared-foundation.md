...
> ⚠️ **本支的選取母集合＝AGENTS 同步點表裡「還沒拆出去」的那 10 列**，不是整張 88 列。同一個判準套到**已經拆走、只剩索引的那 78 列**，**用兩種抽取方式量到 17 列**跨領域（第 3、5、6、7、10、15、17、22、23、25、27、35、38、44、72、77、81 列）。⚠️ **這 17 不是完整清單**：那兩種是「完整路徑」與「裸檔名」；**已知至少還有第三種寫法抓不到——省略副檔名的模組名**（例如第 74 列的「schema」「types」、第 11 列的 `store.save()`），**以及第四種——只寫常數／識別字、由對應契約指出檔案**（例如第 13 列只寫 `REQUIRED_FIELDS／ROW_RULES`，對應契約才點名 `lib/schema.js`）。它們都符合入場條件①卻不在這 17 列裡。⚠️ **補上這兩種也不等於完整**——這裡只列「已知抓不到的寫法」，不宣稱窮舉。⚠️ 這個數字改過兩次：先寫 10（照單接受單一方法的結果）、再寫 17（兩種的聯集、但當成完整結果講），**現在只說「這兩種方式量到 17」**——**它們不在本支**（已各自住在領域契約裡），要不要重新分配是另一件事、記成待辦（#633 r1）。
