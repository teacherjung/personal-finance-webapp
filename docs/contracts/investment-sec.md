...
| 用在哪 | `HEAVY_ROUTES`＝上傳路由（`STATEMENT_FILE_POST_ROUTES`）＋列匯入（`STATEMENT_ROWS_POST_ROUTES`）＋`/api/import`＋`/api/ib/sync` | SEC 官方基本面 refresh（**刻意不掛 HTTP 層**——它有 `refreshInFlight` 去重，掛上去會連「不花錢的第二個」也擋掉） |
...
因為安靜退出會讓父行程只看到「沒有 stdout」。兩處要一起看。
⚠️ 2026-08-29 更正：那個計時器原本寫的理由是「pdfjs 卡死在解壓」，**不成立**——
已知的卡住是 `parse-limits.js` 取消串流沒帶理由造成的，已在源頭修掉；
計時器留著是**保險**（防未知），有考題釘住的是父行程那側的歸類：安靜退出／提早死一律 500。）
...
**記得同步這裡**：**資產換算（帳戶餘額／持股市值→台幣）**：**資產換算**（淨資產／持股市值／槓桿／日線／洞察書籤）的匯率表**唯一實作**＝`public/modules/fx-rates.js`（`resolveFxTable`／`fxFor`／`fxUsageTracker`；後端 `lib/derive.js`、日線寫入口（見 frontend-features 契約）與前端 `portfolio-calculations.js`／`portfolio-model.js` 都 import 同一份；結構考題釘住這四處 import）。**每一處換算都必須吃到設定裡的匯率**：哨兵行為題 `test/fx-sentinel.test.js` 餵兩組哨兵（落在每個預設值的上方 12345／23456／34567 與下方 2.5／3.75／0.125，`Math.max`／`Math.min` 夾制兩組至少一組會紅），三個檔 17 處換算逐點突變都會紅（William 2026-09-04 裁示：主網是行為題，結構題只當第二道網、不再為新形狀加輪）。交易損益／XIRR 的 `tradePnlBase` 與 IB 現金流的 `ib-sync fxToBase` 也走同一份匯率表（丙-2）：沒抓到＝預設值估算並標「用預設匯率」，只有不支援的幣別才 missing；換匯區間提醒（fx-usd-high／low）只看抓到的美元匯率、預設值不觸發。**「丙」口徑（William 2026-09-04 裁，取代 09-03 的「乙」）：不管哪種外幣都同一條規則**——①先用上次抓到的匯率（`settings.usdTwd`／`settings.fxTwd`）②沒有才用預設值 `FX_DEFAULT_TWD`（USD 31／GBP 41／JPY 0.2；匯率波動通常不大、誤差可接受）、**部位照常計入**、累計 `defaultFx: [{currency, count, rate}]` 讓總覽與投資頁標「有幾筆用預設匯率」（提醒 `fx-default`＝info）③只有**不支援的幣別**（表上沒有、也沒有預設）才無法換算＝不計入、累計 `missingFx: [{currency, count, liabilities}]`（提醒 `fx-missing`：有負債＝danger「負債被低估、淨值可能被高估」，否則 warn）。零餘額／零股數不算曝險、不登記。缺 `currency` 仍預設 TWD；這裡沒有任何 `|| 1`。**降低缺匯率的機率＝多管道**：`lib/services/market-data.js getQuotes()` 的匯率代號 Yahoo 抓不到時依序退到 `open.er-api.com` → currency-api（jsDelivr CDN），開 app 自動抓（>1 小時）＋投資頁「更新報價」；全部失敗＝沿用上次的值（`reason:'no-data'`），**永遠不讓 app 因缺匯率停擺**。種子與 `emptyDb()` 不預填匯率（預設值住在常數）。日線 `usdTwd`／`gbpTwd`／`jpyTwd` 記**實際採用**的匯率（上次抓到的或預設值）。考題＝`test/derive.test.js`（丙）＋`test/fx-sentinel.test.js`（哨兵匯率）；備援管道的考題在 market-data 那份考卷。

**記得同步這裡**：**多幣別損益**：換算優先序＝IBKR `pnlBase` → `fxRateToBase`（**必須是正數**）→ USD 直通 → 設定匯率估算（需標註）→ **預設匯率估算（丙-2，William 2026-09-04 裁；交易摘要另標 `defaultCurrencies`，IB 現金流不分設定與預設、只計 `estimatedNoFx` 並在文字講「設定或預設」）**→ 只有**不支援的幣別**才不計入（需標註）。匯率表＝`public/modules/fx-rates.js` 同一份（`tradePnlBase` 與 `ib-sync fxToBase` 都走它），與資產換算不再兩種話。不可把非 USD 金額默默當 USD 加總。⚠️ **「缺幣別」與「缺匯率」是兩種病，處置不同（2026-07-28 全域政策，勿再退回）**：換算優先序（`pnlBase` → 正的 `fxRateToBase` → USD 直通 → 設定或預設估算 → 只有不支援的幣別不計入）講的是**缺匯率**；**缺幣別一律不猜**——`|| 'USD'` 這種寫法在本 repo 曾長出四份（`ib.js` 現金交易與成交紀錄、`ib-sync.js` 新持股、前端 `portfolio-calculations.js`），後果是 Flex Query 少勾一個 Currency 欄，GBP 100 的股息就被當成 USD 100 加總（少算 27%）且畫面零註記。正確處置＝**有「正的」`fxRateToBase` 就照算（那條與幣別無關）；否則跳過＋分開計數回報**（`income.skippedNoCurrency`／`syncIb` 的 `skippedNoCurrency`），新持股缺幣別**不入庫**（猜錯幣別＝市值/淨資產/上限全錯）；既有持股保住原幣別、數量價格照常更新。**「幣別不支援」與「報表沒給幣別」的訊息必須分開講**，使用者才修得到對的地方。考題＝`test/ib-parser-money.test.js`＋`test/securities-sync.test.js`。**交易損益**（交易摘要＋XIRR）共用 `portfolio-calculations.js tradePnlBase()`，兩處口徑必須一致（否則 XIRR 漏估外幣賣出、年化偏低）。**現金流**（IB 股息/利息）在 `lib/ib.js parseStatement()` 解析時就套同一優先序（`lib/services/ib-sync.js` 依 settings 傳入估算器 `fxToBase`），估算/略過筆數存 `income.estimatedNoFx`/`skippedNoFx`＋幣別，前端與 PDF 都要註記。 ⚠️ **`fxRateToBase` 是 0／負數＝報表壞值，不是匯率（2026-09-02 補；夜班稽核 #407 當時記錄在案的缺口）**：`Number.isFinite(0)` 是 true，照乘會讓 GBP 100 的股息變 0 元、負數還會把收入變成支出，而 `count` 照加＝畫面標「已計入」、`skippedNoFx` 是 0（一個字都不提）——**比少一筆更難察覺**。判準＝**壞值視同「沒有報表匯率」**：依序試 USD 直通、設定或預設匯率估算，只有不支援的幣別才不計入並回報；成交列把**壞掉的 `fxRateToBase` 正規化成 `null`**（與空字串同路），`pnlBase` 則**依同一組具名分支決定**——USD 走直通、保留原損益；**非 USD 且沒有合法報表匯率時才是 `null`**（前端 `tradePnlBase()` 收到 `null` 後依其具名分支處理：USD 直通／設定或預設匯率估算／只有不支援的幣別才歸 `missing` 並標註）。⚠️ 承重的那句是「**不可把 0 存進 `pnlBase`**」：前端 `tradePnlBase()` 先看 `pnlBase != null`、才看 `fxRateToBase > 0`；後端若把 0 存進 `pnlBase`，前端的 `fxRateToBase > 0` 分支就不會被評估，所以判準必須在後端。**不要把本規則讀成「壞 fx 一律 null」**——那會反過來打掉 USD 退路（反面斷言釘在 `test/ib-sync-integrity.test.js`：壞 fx 的 USD 成交 `pnlBase` 必須仍是原損益）。估算分支的 `est > 0` 本來就是同一個判準。考題＝`test/ib-sync-integrity.test.js`。

## XIRR 資金加權年化

**改這裡**：**XIRR（資金加權年化，台幣）**

**記得同步這裡**：**XIRR（資金加權年化，台幣）**：現金流＝第一筆月快照市值（流出）＋各月快照投入增量（流出）＋IB 賣出已實現損益逐筆按成交日（`tradePnlBase`×usdTwd，流入，與交易摘要同口徑、含設定或預設匯率估算（旗標只在有賣出真的進了現金流時才點；美元匯率本身是預設值時，進了現金流的每一筆都算估算））＋今日市值（流入）。**賣出只用 Δcost 會漏掉已實現損益，必須用 ibTrades 修正**；用估算時 header 標「含匯率估算」。不含股息利息；台股手動賣出未納入；快照未滿 60 天不顯示；|年化|>500% 視為資料異常。實作在 `portfolio-calculations.js portfolioXirr()/xirrRate()`（僅此一份）。快照資料曾含 seed 示範殘留（2026-07-10 已清），**判斷 XIRR 異常先懷疑快照資料**。
