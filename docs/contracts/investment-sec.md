...
**記得同步這裡**：**資產換算（帳戶餘額／持股市值→台幣）**：匯率表＝`lib/derive.js fxRates()`（後端）與 `public/modules/portfolio-calculations.js fxTable()`（前端）**同口徑**（考題釘住）。**GBP／JPY 沒設（非正數）＝缺匯率＝ `null`，該筆不計入淨資產、槓桿、配置與曝險，並累計成 `missingFx: [{currency, count}]`**（`computeAssets`／`computeIb`／`buildSummary`／前端 `buildPortfolioModel` 都帶）——「乙」口徑（William 2026-09-03 裁）：**不再用猜的 40.8／0.215**，種子與 `emptyDb()` 也不再預填；匯率的唯一來源＝投資頁「更新報價」（`market-data.js` 寫 `settings.fxTwd`）或 `PATCH /api/settings`。畫面必須就地標註（總覽與投資頁的缺匯率註記＋提醒 `fx-missing`），不可默默變小。⚠️ USD 的 `|| 32` 預設是另一個口徑（設定頁可手填），不在乙的範圍。缺 `currency` 仍預設 TWD（既有判準）；表上沒有的幣別也算缺匯率（不再 `|| 1` 當台幣）。考題＝`test/derive.test.js`（乙）、`test/exposure-sync-integrity.test.js`。
