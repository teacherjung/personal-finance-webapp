...
**記得同步這裡**：**資產換算（帳戶餘額／持股市值→台幣）**：匯率表＝`lib/derive.js fxRates()`（後端）與 `public/modules/portfolio-calculations.js fxTable()`（前端）**同口徑**（考題釘住）。**GBP／JPY 沒設（非正數）＝缺匯率＝ `null`，該筆不計入淨資產、槓桿、配置與曝險，並累計成 `missingFx: [{currency, count, liabilities}]`**（`computeAssets`／`computeIb`／`buildSummary`／前端 `buildPortfolioModel` 都帶；**只登記真的有曝險的列**＝帳戶餘額非零、持股股數非零；`liabilities`＝其中幾筆是**負債**，判準＝負餘額**或** `LIABILITY_TYPES` 帳戶型別，與 `computeAssets` 既有的負債判準同一把尺）。**方向決定嚴重度**：缺匯率的負債被略過＝負債被**低估**、淨值**可能**被高估（資產也缺時方向推不出來——`missingFx` 只有筆數沒有金額；槓桿只在 IB 融資的外幣負現金缺匯率、且沒有 `lastEquity` 官方摘要時才跟著被低估）⇒ 提醒 `fx-missing` 升 **danger** 並講明方向；只有資產缺匯率＝淨值被低估 ⇒ warn。**同口徑的考題射程**：持股側比總市值＋`missingFx`、帳戶側比 `missingFx`（含 `liabilities`）；`cashV`／`loan`／槓桿數字沒有逐欄 parity 題。就地標註：投資頁 `missingFxNoteHtml` 有題，總覽 `missingFxFactHtml` 沒有（`dashboard.js` 綁著 DOM 環境）——「乙」口徑（William 2026-09-03 裁）：**不再用猜的 40.8／0.215**，種子與 `emptyDb()` 也不再預填；匯率的唯一來源＝投資頁「更新報價」（`market-data.js` 寫 `settings.fxTwd`）或 `PATCH /api/settings`。畫面必須就地標註（總覽與投資頁的缺匯率註記＋提醒 `fx-missing`），不可默默變小。⚠️ USD 的 `|| 32` 預設是另一個口徑（設定頁可手填），不在乙的範圍。缺 `currency` 仍預設 TWD（既有判準）；表上沒有的幣別也算缺匯率（不再 `|| 1` 當台幣）。考題＝`test/derive.test.js`（乙：帳戶／未知幣別／computeIb／槓桿與負債／summary 與提醒／前後端 parity）。
