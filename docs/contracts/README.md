...
> ③**「共用底層」那一份不看責任檔案清單——不分領域、每次開工都要讀**（2026-09-22 新增）：裡面每一條都跨兩個以上領域、或不屬於任何單一領域。⚠️ **理由只到這裡：這是新增一道「一律讀」的要求，不是補一個原本沒人負責的洞**——在此之前它們住在 AGENTS 的同步點表裡，上面第 6 行的「尚未拆出的領域照舊直接讀 AGENTS」本來就涵蓋得到（#633 r1 抓到原本的理由寫過頭、已收窄）。⚠️ **硬規則②不算在內**（複審後掃抓到）：②只要求「一個檔落在幾份契約就讀幾份」，而這 8 條當時**不在任何一份領域契約裡**、只在 AGENTS 那張表——照②把命中的契約讀完，仍然讀不到它們。⚠️ **入場條件寫死在那份契約的檔頭**：只落在一個領域的一律回那個領域，防它變成垃圾桶。
