...
> ③**「共用底層」那一份不看責任檔案清單——不分領域、每次開工都要讀**（2026-09-22 新增）：裡面每一條都跨兩個以上領域或不屬於任何單一領域，用「命中才讀」的規則會讓它們永遠沒人認領。⚠️ **入場條件寫死在那份契約的檔頭**：只落在一個領域的一律回那個領域，防它變成垃圾桶。
...
- 〔這一份**不分領域一律讀**（硬規則③）；下面的清單仍照實登記（契約內文提到的路徑一律要登記），它是**額外**的路徑路由、不取代硬規則③，所以這些檔案同時也出現在別的領域清單裡，那是正常的。〕
