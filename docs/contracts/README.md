...
> ⚠️ **#639 起，那一行索引不是手寫的——它由契約算出來**：第一格逐字等於該節**行首**那條
> `**改這裡**`（**整節必須剛好一條**），第二格只有指路，連結文字是「契約：<領域名>・<節標題>」。
> `test/contract-split.test.js` 逐行比對，對不上就紅、並把正確的那一行印出來讓人貼回去；
> 另一題擋住「表裡插一列沒帶契約連結的手寫列」。**所以索引與契約不會各說各話。**
> ⚠️ **「行首」與「剛好一條」是 Grok 複審後掃 #639 抓出來才補的**：原本只取「第一次出現」，
> 於是①同一節多加一條 `**改這裡**` ②在真的那條前面插一句不在行首的假的——**兩種都能讓索引與契約
> 說不同的話而整卷全綠**（實測）。
> 代價是**索引大多不再告訴你「要注意什麼」**（那些話回到契約的「記得同步這裡」；
> ⚠️ 不是全部——有些節的 `**改這裡**` 本身就帶著注意事項），裁示者 2026-09-23 選這一邊。
> **規則：開工前對照下表與下方各領域小節——你要改的檔案落在哪個領域，就必讀那份契約**；AGENTS 的同步點清單仍是總索引。
> **2026-09-23（#638）起同步點表 0 列未拆**，所以「尚未拆出的領域」現在是空的；哪天新增一個領域而還沒拆，
> 那一列就照舊直接讀 AGENTS 同步點清單（⚠️ 但**新列同樣要過上面那兩道**——既不能手寫內容，
> 也不能用「不帶契約連結」的方式躲過檢查）。
>
> **三條硬規則（Codex D4a-r1 抓的防護力缺口）**：
...
> ③**「共用底層」那一份不看責任檔案清單——不分領域、每次開工都要讀**（2026-09-22 新增）：裡面每一條都跨兩個以上領域、或不屬於任何單一領域。⚠️ **理由只到這裡：這是新增一道「一律讀」的要求，不是補一個原本沒人負責的洞**——在此之前它們住在 AGENTS 的同步點表裡，本頁頭「尚未拆出的領域照舊直接讀 AGENTS 同步點清單」那一句本來就涵蓋得到（#633 r1 抓到原本的理由寫過頭、已收窄）。⚠️ **硬規則②不算在內**（複審後掃抓到）：②只要求「一個檔落在幾份契約就讀幾份」，而這 8 條當時**不在任何一份領域契約裡**、只在 AGENTS 那張表——照②把命中的契約讀完，仍然讀不到它們。⚠️ **入場條件寫死在那份契約的檔頭**：只落在一個領域的一律回那個領域，防它變成垃圾桶。
> ②**同一檔案可以落在多個領域——每一個命中的契約都要讀**（`lib/derive.js`、`lib/schema.js` 這類共用底層必然多重命中；只讀一份＝漏規則）。

| 領域 | 範圍 | 契約檔 |
|---|---|---|
| 共用底層 | **跨兩個以上領域、或不屬於任何單一領域的同步點**：請求旗標／異常輸入防線／跨 await 的寫入安全／`settings` 與集合新增欄位／前後端同名複本 | [shared-foundation.md](shared-foundation.md) |
...
> 2026-09-03 以前上表第二欄把整個領域的責任檔案塞在**同一格**（一列幾百個路徑）；2026-09-03 起搬到這裡，**每個領域一個小節、一行一檔、依路徑排序**（比較方式＝JavaScript 預設 `sort()` 的字串順序；`test/contract-split.test.js` 強制每個小節與 MANIFEST 的 `files` 都排好序、不重複）。理由：兩支 PR 各自往同一領域加檔案時，改動落在**不同行**、git 通常能自動合併，就不會因為擠在同一行而衝突→被迫 rebase→head 變→複審結論閘失效、重拿一張「通過」。⚠️ **已知邊界**：兩支加的檔案依路徑排序後**剛好相鄰**時，README 與 MANIFEST 仍會衝突（git 看的是相鄰行的 hunk）——一行一檔降低的是撞行的**機率**，不是保證。
> **清單＝宣告的責任集合（下限）**，與拆分護欄考題的 MANIFEST 精確相等（2026-09-03 以前的表格版每列各抄一次，本節只寫這一次）。每行括號裡的說明照原表格**逐字**搬；原表格裡冠在一批檔案前面的粗體小標（籃C 補搬點名檔、#409 補宣告……）改成掛在該批每一檔後面的〔標籤〕——排序打散了批次，標籤讓「它原本屬於哪一批」仍看得出來。不屬於任何單一檔案的句子逐字收在各小節的「備註」——**唯一新寫的例外**＝投資小節備註開頭標了〔本版新寫〕的那句位置語（原文的「後面這一批」在依路徑排序後會指錯，所以補一句說明它原本指哪一批；考題不驗備註逐字，這條例外只靠這裡宣告）。
> ⚠️ **查法＝路徑優先，不是領域優先**：同一檔案常落在多個領域（硬規則②），而表格第三格直接連到契約、看不到檔案清單——**先在本頁搜尋你要改的完整路徑，命中幾個小節就讀幾份契約**，不要只看範圍句挑一份（`lib/derive.js` 出現在四個小節；`public/modules/cards.js` 這類三域檔沒寫進硬規則②，靠搜尋才找得全）。

### 共用底層

契約：[shared-foundation.md](shared-foundation.md)

備註（觸發方式）：

- 〔這一份**不分領域一律讀**（硬規則③）；下面的清單照實登記（⚠️ **不寫「一律」**：既有抽取式看不到「一格塞路徑＋識別字」那種寫法——#633 r1 抓到兩個、**複審後掃又抓到第三個**（`server.test.js seedTx`），三個都已補。⚠️ **這是同一個盲點的第三次**，所以這句只說「已補這三個」，**不保證沒有第四個**），它是**額外**的路徑路由、不取代硬規則③，所以這些檔案同時也出現在別的領域清單裡，那是正常的。〕
...
- 〔〔本版新寫〕〔位置語：「後面這一批」＝Codex #384 r27 補列、2026-09-03 以前的表格版裡緊接在「清單＝宣告的責任集合」那句後面的那一批；依路徑排序後這裡不分批、也不另存那一批的名單——要看當時的分批去讀表格版（本檔的 git 歷史）〕⚠️ 後面這一批是 Codex #384 r27 抓到的漏項。**它 r29 又找到 64 支，並撤回自己「補完就可合併」的判斷（「那次抽查範圍不足」）**——所以硬規則①已把「窮舉」降級成「宣告的責任集合」：這道護欄主要驗已宣告的集合彼此一致，**大體上驗不出人漏宣告了什麼**（#409 r6（2026-08-06）補了一道例外：**已宣告的正式程式 import 進來的模組**，兩邊一起漏列時會轉紅——那只關掉「從已宣告的檔案切出一個新模組卻沒登記」這一種漏法，其餘仍然靠人）。弱化這幾支考題前必讀本契約。〕
