...
> ③**「共用底層」那一份不看責任檔案清單——不分領域、每次開工都要讀**（2026-09-22 新增）：裡面每一條都跨兩個以上領域、或不屬於任何單一領域。⚠️ **理由只到這裡：這是新增一道「一律讀」的要求，不是補一個原本沒人負責的洞**——在此之前它們住在 AGENTS 的同步點表裡，上面第 6 行的「尚未拆出的領域照舊直接讀 AGENTS」與硬規則②本來就涵蓋得到（#633 r1 抓到原本的理由寫過頭、已收窄）。⚠️ **入場條件寫死在那份契約的檔頭**：只落在一個領域的一律回那個領域，防它變成垃圾桶。
...
> ⚠️ **查法＝路徑優先，不是領域優先**：同一檔案常落在多個領域（硬規則②），而表格第三格直接連到契約、看不到檔案清單——**先在本頁搜尋你要改的完整路徑，命中幾個小節就讀幾份契約**，不要只看範圍句挑一份（`lib/derive.js` 出現在三個小節；`public/modules/cards.js` 這類三域檔沒寫進硬規則②，靠搜尋才找得全）。
...
- 〔這一份**不分領域一律讀**（硬規則③）；下面的清單照實登記（⚠️ **不寫「一律」**：既有抽取式看不到「一格塞路徑＋識別字」那種寫法，#633 r1 抓到兩個漏網、已補），它是**額外**的路徑路由、不取代硬規則③，所以這些檔案同時也出現在別的領域清單裡，那是正常的。〕
