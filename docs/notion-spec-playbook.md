...
- **位置＋現況**：Notion「不簡單 › 榮祥森（理財中心）› **動態藍圖區（架構・規則・原理）**」＝給使用者看的白話視圖（AGENTS.md 仍是技術唯一真相）。目前＝**9 頁子系統**（1 消費說明→鑰匙→顯示／2 帳單匯入／3 分類與自動學習／4 資產・投資・IB／5 訂閱・提醒／6 資料安全／7 前端架構／8 協作機制／**9 每日洞察引擎**）＋一頁跨切面「**防撞護欄設計原理（G3–G5）**」。**動到某子系統的架構＝一併更新對應那頁**（一致性由動工方顧）。容器頁 id `3a39485922f58100aaedead0094987e0`；workspace＝teacherjung's Notion。
