...
> 📦 2026-08-04 自 `AGENTS.md` 舊「協作流程」節**逐字搬出**（那條指標 2026-09-17 起在「專案概觀」節末）（籃 A 文件瘦身）：這是純操作手冊（頁面位置、id、icon 清單、建頁踩坑），只有動 Notion 時才需要。規則位階不變——使用者 2026-07-20 定；**2026-07-22 起 Notion 更新也交給 Codex**，細節當時已從 Claude 的 memory 搬進共用文件（Codex 看不到 memory）。`AGENTS.md` 仍是技術唯一真相。

- **位置＋現況**：Notion「不簡單 › 榮祥森（理財中心）› **動態藍圖區（架構・規則・原理）**」＝給使用者看的白話視圖（AGENTS.md 仍是技術唯一真相）。頁面分成「子系統頁」與「跨切面原理頁」兩類，收在六大類群組頁底下。⚠️ **本檔刻意不維護頁數與完整清單**（會長、會漂）——動到架構前先 fetch 容器頁看現況。跨切面原理頁目前包含「防撞護欄設計原理（G3–G5）」與「錢的絕對邊界（AI 永遠碰不到你的錢）」（後者 2026-08-04 新建於「🔐 資料與安全」群組）。**動到某子系統的架構＝一併更新對應那頁**（一致性由動工方顧）。容器頁 id `3a39485922f58100aaedead0094987e0`；workspace＝teacherjung's Notion。
