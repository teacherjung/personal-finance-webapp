...
而**靜靜通過比沒有護欄更糟**（memory〈靜靜通過最危險〉）。這條路一年也走不到幾次，沒有噪音成本。
...
- [ ] 把 `setPdfChildScriptForTest` 的還原漏掉 ⇒ 同檔後面的題會紅（接縫自己會抓漏）
...
- **已量過的反證**：修好之後 ①正常路徑輸出逐字不變（3-2 第 8 步）②`task.destroy()` 在超標之後
  ...
  不再需要被 SIGKILL。
- 最大的殘餘風險：**C1 依賴「到達門檻比 20 秒絆索快」**。實測餘裕 5 倍以上、且慢只會更慢不會失效，
