...
| 每日洞察引擎 D0–D4（提醒牆、Δ chips、穩定 key、insightState 書籤） | `lib/derive.js`、`docs/daily-insights-engine-plan.md` | #192–#198 |
...
| 測試覆蓋率地圖（財務風險導向；Codex 製作、經 teacherjung 帳號提交） | `docs/test-coverage-map.md` | #202（`b69989d`，2026-07-22） |
...
| 月度回顧＋退款配對抵減（Codex：P0 退款匯入、P1 計算/API、P2 總覽卡；#235 改善預設空月與比較基準） | `public/modules/monthly-review-card.js`、`docs/archive/monthly-review-plan.md` | #230／#231／#232／#235 |
| 目標追蹤 D5-③（Claude 設計；Codex 審查＋實作：淨值目標、雙速度 ETA、達標報喜進提醒牆） | `lib/derive.js`（computeGoalTracking）、`public/modules/goal-tracking.js`、`docs/archive/goal-tracking-plan.md` | 計畫 #237；P1–P3＝#238／#240／#241 |
| 證券交易 S0 改名（銀行收支／信用卡費）＋設計藍圖 | `docs/archive/securities-trading-blueprint.md`（Codex 設計、Claude 審查；使用者裁決與實作修訂記於 §十三） | #236（後續隨 #242 修訂藍圖） |
...
## 協作沿革（歷史；2026-09-17 起現行版本＝根目錄 RULES.md 與 AGENTS.md「本專案協作附則」）
