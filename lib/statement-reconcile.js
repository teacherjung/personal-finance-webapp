...
export function reconcileCardStatement(parsed) {
  ...
  // 抄錯身分（退款↔繳款**跨桶**）兩種慣例都對不上＝擋；⚠️ 誠實劃界＝**桶內互抄分不出**
  // （慣例 B 的繳款與退款同住一個合計桶，對調不改變任何合計——徽章照實揭露）。容差 0（裁示①）。
  ...
  if (aiPath && t.newCharges != null && t.paidAndRefund != null && unjudgeableNeg === 0) {
    ...
    if (convA || convB) {
      ...
      problems.push({
        ...
        message: `明細與帳單摘要對不上（兩種常見印法都試過：退款折進「本期新增」時差 ${fmt(Math.abs(r2((sumPos - refundAbs) - t.newCharges)))}、`
          + `退款算進「已繳款退款」桶時差 ${fmt(Math.abs(r2(sumPos - t.newCharges)))}）`
