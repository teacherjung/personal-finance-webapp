...
export function reconcileCardStatement(parsed) {
  ...
  if (aiPath && t.newCharges != null && t.paidAndRefund != null && unjudgeableNeg === 0) {
    // 繳款列缺席（payAbs===0）＝常見合法印法（繳款只列在摘要），那一半驗不了就不驗——
    // 與 ai-parse-card.js 的 G2 同一組公式（那邊有完整理由與 r8 攻擊為何仍接得住的說明）。
    const convA = r2((sumPos - refundAbs) - t.newCharges) === 0 && (payAbs === 0 || r2(payAbs - t.paidAndRefund) === 0);
    const convB = r2(sumPos - t.newCharges) === 0 && (payAbs === 0 ? refundAbs <= t.paidAndRefund : r2((payAbs + refundAbs) - t.paidAndRefund) === 0);
    if (convA || convB) { checks.newVsRows = 'pass'; checks.paidVsRows = 'pass'; }
    else {
