 ...
 * level 只數**擋下型**驗證：C1 有跑＝medium（AI 路的慣例閘也是擋下型、但它跑的前提是 C1 四格
 * 齊全＝medium 已由 C1 撐起，不另立級別）；只剩影子或全 skip＝weak（誠實：影子不是保護）。
...
export function reconcileCardStatement(parsed) {
  ...
  return {
    // level 只數擋下型驗證：C1 有跑＝medium（AI 慣例閘跑的前提=C1 四格齊全，級別已被 C1 撐起）；
    // 模板路的影子檢查不是保護、不可讓它撐級別（誠實劃界②）
