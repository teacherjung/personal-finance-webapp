...
//         明細加總對摘要總額＝**分路**：模板路影子檢查（記 advisories 不擋）、AI 路慣例閘（擋）
//         ——完整分路說明見下方界線②。
...
//   ②中閘的「明細對摘要總額」（C2′/C3′）＝**分路（r3#2 更新）**：模板路照舊影子不擋
//     （William 2026-08-11 裁示；公式已依真郵寄版校準 P0.2）；**AI 路已依 William 2026-08-30
//     裁示③升級成慣例閘（擋）**——A/B 兩式與「桶內互抄分不出」的劃界見 reconcileCardStatement
//     的 aiPath 分支。模板路擋下的只有 C1（帳單自己印的那行等式，天生必平）。
 ...
 * advisories＝影子檢查的對不上：不影響 ok，供預覽顯示與後續校準。（模板路的升級之門仍開著、
 * 要再經裁決；AI 路已依 William 2026-08-30 裁示③升級成慣例閘＝aiPath 分支的 problems。）
 ...
 * ②（模板影子／AI 慣例閘）**明細正項−退款** vs 本期新增款項。公式依 2026-08-11 四份真郵寄版校準（P0.2）：
 *   版面語意實測＝「本期新增款項」是**已扣退款的淨額**、退款不在已繳款桶——標籤寫「(已繳款金額＋
 *   本期退款)」但數字不含退款（四份、退款筆數各異，本公式全部精準吻合）。
 * ③（模板影子／AI 慣例閘）**繳款列加總**（isPayment）vs 已繳款＋退款桶——同上校準。
 *   負數列缺旗標時用 desc 過 isCardPayment 重判（單一真相共用，r1#2）；連 desc 都沒有＝分不出
 *   繳款/退款＝②③兩道誠實 skip（不猜——猜成退款會讓真繳款把兩道一起誤鳴）。
 * ②③的擋/不擋分路（r1#3 更新）：**模板路維持影子不擋**（William 2026-08-11 裁示；樣本全來自
 *   同一張卡的乾淨帳期）；**AI 路已升級成慣例閘（擋）**——William 2026-08-30 裁示③（就是舊註解
 *   說「未來提請裁決」的那次裁決），A/B 兩種退款印法擇一全對，實作在下方 aiPath 分支。
 * level 只數 C1：有跑＝medium；沒跑＝weak（影子不是保護）。AI 慣例閘是擋下型但**不另撐級別**
 * ——它的執行條件是 newCharges＋paidAndRefund 在場（直呼本函式可造出 equation:skip＋慣例閘照跑
 * 的組合）；正式 AI 流程到不了這種形（G1 先要求四格全在），這裡不賭呼叫端、照實寫條件。
...
export function reconcileCardStatement(parsed) {
  ...
  const aiPath = Array.isArray(/** @type {any} */ (parsed)?.aiAdjustments);   // AI 卡片路的標記（模板路永遠沒有這欄）
  let sumPos = 0, refundAbs = 0, payAbs = 0, unjudgeableNeg = 0;
  for (const tx of parsed?.transactions || []) {
    // 具名調整列（裁示② 2026-08-30：要記帳）不參與列對總額——等式已用 adjSum 摺過，再計入＝重複計
    if (/** @type {any} */ (tx).isAdjustment) continue;
  ...
  if (t.due != null && t.prevDue != null && t.paidAndRefund != null && t.newCharges != null) {
    ...
    // AI 路等式容差 0（Grok 掃#1：G1 是 0、這裡若留 ±1＝兌票只重跑中閘那條路差 1 元照收，
    // 與裁示①和徽章「分毫不差」全部矛盾）；模板路照舊 CARD_EPS（行為零改變）。
    if (Math.abs(diff) > (aiPath ? 0 : CARD_EPS)) {
  ...
  // ②③：**AI 路＝慣例閘（擋）；模板路＝影子檢查（不擋、行為不變）**。
  // AI 路升級＝William 2026-08-30 裁示③（本段舊註解「升級為擋要經裁決」的那次裁決）：
  // 退款有的銀行折進「本期新增」（慣例 A＝遠銀式）、有的放「已繳款＋退款」桶（慣例 B）——
  // 單一公式當閘會誤擋其中一種真帳單，所以做成**擇一全對**：
  //   A：Σ(消費−退款) ＝ 本期新增 且 Σ繳款 ＝ 已繳款退款桶
  //   B：Σ消費 ＝ 本期新增 且 Σ繳款＋Σ退款 ＝ 已繳款退款桶
  // 擋得住＝退款被抄成繳款（A 慣例，消費和立刻不平）與金額對不齊的亂抄；⚠️ 誠實劃界＝
  // 繳款被抄成退款、金額恰等於「已繳款退款」桶＝算術不可區分（B 全等照樣成立），
  // 桶內互抄同理分不出——徽章照實揭露＋card-refund-check 就地提醒。容差 0（裁示①）。
  // 模板路照舊：mismatch 進 advisories、訊息把兩種可能都講、供校準；判不出的負數列存在時 skip。
  if (aiPath && t.newCharges != null && t.paidAndRefund != null && unjudgeableNeg === 0) {
    // 與 ai-parse-card.js 的 G2 同一組公式（完整理由、r1#1 的收緊與「繳款被抄成退款且金額恰等於桶
    // ＝算術不可區分」的誠實劃界都在那邊）。
    const convA = r2((sumPos - refundAbs) - t.newCharges) === 0 && (payAbs === 0 || r2(payAbs - t.paidAndRefund) === 0);
    const convB = r2(sumPos - t.newCharges) === 0 && r2((payAbs + refundAbs) - t.paidAndRefund) === 0;
    if (convA || convB) {
      checks.newVsRows = 'pass'; checks.paidVsRows = 'pass';
      // r1#1 的就地提醒（不擋）：明細沒有繳款列、卻有退款列、桶又有金額——若退款其實是繳款
      // （AI 字樣抄錯），金額恰等於桶時上面的閘分不出來；點名請使用者看那幾列。
      if (payAbs === 0 && refundAbs > 0 && t.paidAndRefund > 0) {
        advisories.push({
          code: 'card-refund-check',
          message: '明細裡沒有任何繳款列、但有退款列——若其中哪一筆其實是「繳卡款」（AI 把說明文字抄錯了），'
            + '驗算分不出來。請看一眼負數的列：真的是退款才勾選匯入。',
        });
      }
    }
    else {
      checks.newVsRows = 'fail'; checks.paidVsRows = 'fail';
      problems.push({
        code: 'card-rows-vs-totals',
        // 分量報不報＝比較的**兩側都非零**才報（r8#3→Grok 掃#3 通則化：任一側為 0 時
        // 「差額」＝另一側原值——零列、淨額恰 0、摘要印 0 三種形都會回聲；機密紀律同 G2）
        message: `明細與帳單摘要對不上（兩種常見印法都試過：退款折進「本期新增」時${(r2(sumPos - refundAbs) !== 0 && r2(t.newCharges) !== 0) ? `消費和差 ${fmt(Math.abs(r2((sumPos - refundAbs) - t.newCharges)))}` : '該式差額不報（一側為零＝報了就是另一側原值）'}${(payAbs !== 0 && r2(t.paidAndRefund) !== 0) ? `、繳款差 ${fmt(Math.abs(r2(payAbs - t.paidAndRefund)))}` : ''}；`
          + `退款算進「已繳款退款」桶時${(sumPos !== 0 && r2(t.newCharges) !== 0) ? `消費和差 ${fmt(Math.abs(r2(sumPos - t.newCharges)))}` : '該式差額不報'}${(r2(payAbs + refundAbs) !== 0 && r2(t.paidAndRefund) !== 0) ? `、桶差 ${fmt(Math.abs(r2((payAbs + refundAbs) - t.paidAndRefund)))}` : ''}）`
          + '——可能有漏讀、多讀，或繳款退款的字樣讀錯。AI 讀的帳單對不上就不收',
      });
    }
  } else if (!aiPath && t.newCharges != null && unjudgeableNeg === 0) {
  ...
  if (!aiPath && t.paidAndRefund != null && unjudgeableNeg === 0) {
  ...
  return {
    // level 只數 C1（見檔頭 JSDoc：AI 慣例閘是擋下型但不另撐級別；其執行條件比 C1 寬、
    // 直呼可造出 equation:skip＋慣例閘照跑）；模板路影子不是保護、不可撐級別（誠實劃界②）
