  ...
  if (aiPath && t.newCharges != null && t.paidAndRefund != null && unjudgeableNeg === 0) {
    ...
    if (convA || convB) {
      ...
      problems.push({
        ...
        // 兩式各報分量（r8#3）；該側沒有列＝不報（0 減整格摘要＝回聲原值，機密紀律同 G2）
        message: `明細與帳單摘要對不上（兩種常見印法都試過：退款折進「本期新增」時消費和差 ${fmt(Math.abs(r2((sumPos - refundAbs) - t.newCharges)))}${payAbs > 0 ? `、繳款差 ${fmt(Math.abs(r2(payAbs - t.paidAndRefund)))}` : ''}；`
          + `退款算進「已繳款退款」桶時消費和差 ${fmt(Math.abs(r2(sumPos - t.newCharges)))}${(payAbs > 0 || refundAbs > 0) ? `、桶差 ${fmt(Math.abs(r2((payAbs + refundAbs) - t.paidAndRefund)))}` : ''}）`
