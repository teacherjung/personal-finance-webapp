...
/**
 * 合計交叉驗證的**封閉狀態碼**（2026-08-19；William：「混幣讓合計交叉驗證關閉，但畫面說它擋得住」）。
 *
 * 這道檢查（裁示⑧b，接線在 `bank-import.js assertAiBankReconciled`）**不是每份帳單都跑得起來**：
 * 混幣帳單整道跳過、帳單沒印合計沒得對、配方路線根本不產這個欄位。跑不跑得起來**必須跟著裁決
 * 走到畫面**——否則說明區那句「帳單有印合計＝合計也擋」對混幣帳單（William 自己的帳單正是這型）
...
export const TOTALS_CHECK = Object.freeze({
  ...
  NOT_PRINTED: 'not-printed',         // 欄位在、三欄都 null＝帳單自己沒印合計（誠實缺席）
...
export function statementCurrencyLookup(parsed, masked) {
  ...
  const byMap = parsed?.accountCurrency ? parsed.accountCurrency[masked] : null;
