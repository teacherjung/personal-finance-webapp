...
/**
 * 服務層排隊的**長度上限**（Codex #371 r4 High）。
 *
 * 只有逾時、沒有長度上限的隊伍，等於把上游自己的 back-pressure 架空：SEC 原本有
 * `SEC_QUEUE_MAX_DEPTH`（#361 的全站 16 個），但請求在排到那道門之前就先卡在這裡了，
 * 於是那個上限**在 HOSTED 變成打不到的死碼**——而這裡的隊伍可以無限長。
 * ⇒ 呼叫端要能把自己的上限傳進來（`opts.maxWaiters`），這裡的預設只是最後一道保險。
 */
export const HEAVY_SLOT_MAX_WAITERS = 16;
...
/** @type {Map<string, number>} 每個呼叫端群組**執行中＋排隊中**的數量（見 withHeavySlot）。 */
const groupInUse = new Map();
...
/** 測試用：某個群組**執行中＋排隊中**的數量。 */
export function heavyAdmissionGroupInUseForTest(group) { return groupInUse.get(group) ?? 0; }
/** 測試用：目前排隊等名額的人數。 */
export function heavyAdmissionWaitingForTest() { return waiters.length; }
/** 測試用：重置（考題之間不互相污染）。 */
export function resetHeavyAdmissionForTest() { inFlight = 0; waiters.length = 0; groupInUse.clear(); }
 ...
 * 但「都會成功」這件事沒有被犧牲。
 *
 * ⚠️ **隊伍有長度上限**（Codex #371 r4 High）：只有逾時、沒有長度上限的話，
 * 呼叫端自己的 back-pressure（SEC 的 `SEC_QUEUE_MAX_DEPTH`）會被架空——請求全卡在這裡，
 * 排不到那道門，那個上限就變成打不到的死碼。呼叫端把自己的上限傳進來（`maxWaiters`），
 * 滿了 fail-fast，語意才回到「站台層級的 back-pressure」。
 *
 * @template T @param {() => Promise<T>} fn
 * @param {{waitMs?: number, maxWaiters?: number, group?: string, maxInGroup?: number}=} opts
 * @returns {Promise<T>}
 */
export async function withHeavySlot(fn, opts) {
  ...
  const maxInGroup = opts?.maxInGroup;

  // ⚠️ **群組上限數的是「執行中＋排隊中」，不是只數排隊中**（Codex #371 r5 High）。
  //    r5 版把呼叫端的上限當成 `maxWaiters`（只數 waiters），於是 maxQueueDepth=1 時，
  //    第一個 SEC refresh 是**執行中**（不在 waiters 裡），第二個照樣排得進去、等到逾時——
  //    #361 的 `secQueueDepth >= maxQueueDepth`（兩者一起算）沒有被還原。
  //    預設 16 的情況更清楚：1 個執行中 ＋ 16 個排隊中 ＝ 17，比 #361 寫下的上限多一個。
  let counted = false;
  if (group && Number.isFinite(maxInGroup)) {
    const cur = groupInUse.get(group) ?? 0;
    if (cur >= /** @type {number} */ (maxInGroup)) {
      throw Object.assign(new Error('伺服器正忙（同類工作已達上限），請稍後再試。'),
        { status: 503, code: 'heavy_queue_full' });
    }
    groupInUse.set(group, cur + 1);
    counted = true;
  }
  const uncount = () => {
    if (!counted) return;
    counted = false;
    const n = (groupInUse.get(/** @type {string} */ (group)) ?? 1) - 1;
    if (n > 0) groupInUse.set(/** @type {string} */ (group), n); else groupInUse.delete(/** @type {string} */ (group));
  };

  try {
    if (inFlight >= HEAVY_ADMISSION_MAX_INFLIGHT) {
      // 沒有群組上限的呼叫端也不可以排出無限長的隊伍——這是最後一道保險
      if (waiters.length >= (opts?.maxWaiters ?? HEAVY_SLOT_MAX_WAITERS)) {
        throw Object.assign(new Error('伺服器正忙（排隊的工作已達上限），請稍後再試。'),
          { status: 503, code: 'heavy_queue_full' });
      }
      ...
      await new Promise((resolve, reject) => {
        ...
        // ⚠️ **這裡不可以 unref**（正式環境那顆 Node 22 的 CI 抓到，本機 Node 26 全綠）：
        //    unref 之後這個計時器撐不住事件迴圈，迴圈一跑乾就出現
        //    「Promise resolution is still pending but the event loop has already resolved」
        //    ——考題被整批取消；而在正式環境代表**這個逾時可能永遠不會觸發**，
        //    等名額的請求就一直掛著。我原本寫的理由（「別讓等待卡住行程結束」）本身就是錯的：
        //    等待中的請求是**真的在進行的工作**，本來就該讓行程活著，最長也只有 HEAVY_SLOT_WAIT_MS。
