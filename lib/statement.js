...
// v1 只實作富邦解析器；明確是其他銀行就先擋下（避免用富邦格式硬解出錯誤資料）。
// 未填發卡行/名稱時預設嘗試富邦。未來新增銀行＝加 parseXxx() 並在此分流。
const OTHER_BANKS = ['台新', '國泰', '玉山', '中國信託', '中信', '花旗', '匯豐', '星展', '聯邦', '永豐', '兆豐', '第一銀', '華南', '元大', '凱基', '渣打', '新光', '遠東', '樂天', '王道'];
export async function parseStatementPdf(data, password, issuer = '') {
  const other = OTHER_BANKS.find(b => issuer.includes(b));
  if (other && !/富邦/.test(issuer)) {
    throw new Error(`帳單解析目前僅支援富邦（這張卡是「${other}」）。其他銀行之後會陸續支援。`);
  }
