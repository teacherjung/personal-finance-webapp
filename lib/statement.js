...
const CATEGORY_RULES = [
  ...
  ['工作', 'Notion', ['NOTION']],
  ['工作', 'Canva及其他工作軟體', ['CANVA', 'ADOBE', 'FIGMA', 'MICROSOFT', 'OFFICE 365', 'MICROSOFT 365', 'ELEVENLABS']],
  ...
  ['交通', '停車費', ['停車', '嘟嘟房', '停車場', '城市車旅', '和雲', 'TIMES', '拖吊', '阜爾運通', '普客二四', '順康資產', '台灣國際開', '林口四維路']],
  ...
  ['飲食', '餐廳', ['餐廳', '餐飲', '石二鍋', '鼎泰豐', '麥當勞', '肯德基', '摩斯', '漢堡', '火鍋', '燒肉', '燒臘', '食堂', '小吃', '拉圖爾', '12MINI', 'LADY M', 'LADY-M', '秘食', '墨爾', '炸牛', '爭鮮', '壽司', '涮涮', '鬍鬚張']],
  ...
  ['生活', '3C產品', ['3C', 'APPLE', 'STUDIO A', '德誼', '燦坤', '全國電子']],
...
// 兩種模板都支援：斜線民國(115/06/02) 與 7 碼民國(1150602，補印版)。說明有時拆三行（前一行＋此列無說明＋後一行），
// 以相鄰非交易列補回。金額＝兩個日期之後第一個純整數 cell（避開說明內含的地點碼與外幣金額）。
...
function parseTaishinPdf(lines) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (!desc) {   // 說明拆三行：前一行 descA ＋ 後一行 descB（都需為非交易列）
      ...
      desc = [pa, nb].filter(Boolean).join(' ');
    }
    if (!desc) continue;
...
// 主入口：data=Uint8Array（PDF 或 XLSX，自動偵測）。password 僅加密 PDF 需要。
// 富邦＝PDF（parseFubon）；台新＝PDF（parseTaishinPdf，郵寄加密版）＋XLSX（parseTaishinXlsx，官網下載版）。
// 未來新增銀行＝加 parseXxx() 並在此依 issuer/內容分流。
const OTHER_PDF_BANKS = ['國泰', '玉山', '中國信託', '中信', '花旗', '匯豐', '星展', '聯邦', '永豐', '兆豐', '第一銀', '華南', '元大', '凱基', '渣打', '新光', '遠東', '樂天', '王道'];
export async function parseStatement(data, password, issuer = '') {
  ...
  if (isXlsx) {
    ...
    return finalize(raw, '台新');
  }
  // 以下為 PDF：依 issuer 分流富邦/台新；其他銀行先擋下
  const lines = await extractLines(data, password);
  if (/台新/.test(issuer)) {
    const raw = parseTaishinPdf(lines);
    if (!raw.length) throw new Error('這份台新 PDF 找不到消費明細，或不是支援的格式；也可改上傳官網下載的 XLSX 明細。');
    return finalize(raw, '台新');
  }
  const other = OTHER_PDF_BANKS.find(b => issuer.includes(b));
  if (other && !/富邦/.test(issuer)) {
    throw new Error(`PDF 帳單目前支援富邦與台新（這張卡是「${other}」）。其他銀行之後會陸續支援。`);
  }
  const raw = parseFubon(lines);
  if (!raw.length) throw new Error('找不到消費明細——可能不是支援的帳單格式（PDF 支援富邦/台新、XLSX 支援台新），或該期沒有交易。');
  return finalize(raw, '富邦');
}
