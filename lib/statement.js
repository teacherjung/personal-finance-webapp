...
/** @param {string[][]} lines @returns {{ bank: string, bankEvidence: 'header'|'none', raw: RawTx[], noRows?: boolean, otherIssuer?: boolean, identityNoRows?: boolean }} */
export function parsePdfAuto(lines) {
  ...
  const best = byIdentity || (taishin.raw.length > fubon.raw.length ? taishin : fubon);   // 認不出身分才回到「筆數多者」
