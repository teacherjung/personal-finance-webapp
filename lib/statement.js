...
const STORE_CANON = [
  ...
  ['林口長庚醫院', /林口長庚/],              // 長庚醫療財團法人林口長庚紀念醫（截斷）
...
// 優步（Uber）＝平台不是店家：優步-皇冠大車隊（叫車）、優步-好麥永和豆漿店（外送）都要留「後面那個」才分得出來，
// 否則所有 Uber 消費會共用鑰匙「優步」（使用者定 2026-07-18「所有優步都改回原本的」）。長名在前，免被短名先吃掉。
const GATEWAY_PREFIX = /^(?:(?:連加|騰加數位|TAPPAY|momo)[*＊_]\s*|(?:聯信|藍新|點點付|優步福爾摩沙股份有公司|優步)[-－*＊_]\s*)+/i;
...
export function cleanStore(desc) {
  ...
  const s = body
    .replace(DELIVERY_PREFIX_ANY, '')                                                               // 外送平台前綴只砍掉（鑰匙＝餐廳本身）；「（FP）／（優食）」標記在顯示層由 applyDisplayLabels 加
...
/** 命中的外送平台標記（沒中回 ''）。 @param {string|undefined} desc @returns {string} */
const deliveryTagOf = (desc) => (DELIVERY_PREFIXES.find(([re]) => re.test(String(desc || '')))?.[1]) || '';
...
export function applyDisplayLabels(name, ctx = {}) {
  let s = String(name || '');
  if (!s) return s;
  const tag = deliveryTagOf(ctx.desc);
  if (tag) {
...
export const BRANCH_CHAINS = ['誠品生活', '健身工廠', '八方雲集', '少年家宵夜食堂', '石二鍋', '好市多', '威秀影城', '傳承永和豆漿大王'];
const CJK = '一-鿿';
