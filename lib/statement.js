...
export function cleanStore(desc) {
  ...
  // eTag 停車（使用者定 2026-07-18）：場站名藏在設備碼＋冒號後 → 「eTag 停車（場站名）」。
  // 名字本身已是停車語意，顯示層的停車費（）包裝由 PARK_WRAP_EXEMPT 豁免；場站的「?亭」缺字一併修補。
  const et = body.match(/^(?:eTag|ETAG)\s*停車\s*(?:\d{3,4}-[A-Z0-9]{1,4})?\s*[:：]?\s*(.*)$/i);
  if (et) {
    const venue = et[1].replace(/[?？]亭/g, '俥亭').trim();
    return withFx(venue ? `eTag 停車（${venue}）` : 'eTag 停車');
  }
