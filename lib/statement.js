...
export function applyDisplayLabels(name, ctx = {}) {
  ...
  if (parkSub && String(ctx.subcategory || '') === String(parkSub) && !s.startsWith('停車費（')
...
export function normalizeStoreDisplay(name, opts = {}) {
  ...
  if (/加油站$/.test(b0) && !/[（）]/.test(b0) && categorize(b0)[1] === '油錢') return applyBrandRename(s);
...
// 不硬造原則（自審B#1/#2/#8＋Codex 複審 2026-07-26）：①已是「…加油站」＝定點不動（世新加油站）
// ②分店要過 BRANCH_TAIL（純中文起頭 ≤10 字）才敢當主體——英數雜訊尾（TAIPEI）不轉③分店剝完是空
// （好市多（加油站）＝連鎖切分把「加油站」尾重包成假分店）＝還原「品牌加油站」，絕不坍縮成光桿「加油站」
// ④沒有分隔符也沒括號的（中油A123→中油、台塑石油新店站）＝靠 GAS_BRANDS 品牌前綴切，切不出分店就回
// 「品牌加油站」——**不可留原樣**：留著會讓畫面同時存在「泰山加油站」與「中油」兩種格式（Codex 複審）。
...
function gasStationDisplay(name) {
  ...
  if (/加油站$/.test(base)) return raw;                                  // 定點：已是「…加油站」
  let out = base;
  const m = base.match(/^([^（）]+)（([^（）]+)）$/);
  if (m) {
    const body = m[1].trim();
    // 分店淨化：摘前導「加油站」（好市多（加油站中壢店）→ 中壢店）→ 剝尾「站／店／加油站」
    const branch = m[2].replace(/^加油站/, '').replace(/(?:加油)?[站店]$/, '').trim();
    if (branch && BRANCH_TAIL.test(branch)) out = `${branch}加油站`;     // 品牌（分店）→ 分店加油站
    else out = /加油站$/.test(body) ? body : `${body}加油站`;            // 分店空/雜訊 → 品牌加油站（不坍縮）
  } else if (base.includes('加油站')) {
    const glued = base.match(/^[^（）]*加油站(.+)$/);                    // 黏寫未拆分店：柑園加油站林口二站
    const b2 = glued ? glued[1].replace(/(?:加油)?[站店]$/, '').trim() : '';
    if (b2 && BRANCH_TAIL.test(b2)) out = `${b2}加油站`;                 // 過不了 BRANCH_TAIL＝原樣、不硬造
  } else {
    for (const { prefix, brand } of GAS_BRANDS) {                        // 無分隔符無括號：品牌前綴切
      const hit = base.match(prefix);
      if (!hit) continue;
      const rest = base.slice(hit[0].length).trim();
      // 剩餘要「像分店」才敢當主體＝以站／店結尾（新店站→新店）；中油大樓、中油自助洗車 這種
      // 不像分店的一律原樣不動（硬切會生出「大樓加油站」）。剩餘為空或只剩「站」＝沒有分店。
      const stem = rest.replace(/(?:加油)?[站店]$/, '').trim();
      if (!rest || !stem) out = `${brand}加油站`;
      else if (/[站店]$/.test(rest) && BRANCH_TAIL.test(stem)) out = `${stem}加油站`;
      break;
    }
  }
  return out + tail;
...
/** @param {string} desc @returns {boolean} */
export const isCardPayment = (desc) => /(?:自動(?:轉帳)?扣繳|信用卡(?:自動)?(?:扣款|繳款)|信用卡款|感謝(?:您)?繳款|(?:本期)?已繳款|AUTOPAY|PAYMENT\s*THANK\s*YOU|THANK\s*YOU\s*PAYMENT)/i.test(String(desc || ''));
