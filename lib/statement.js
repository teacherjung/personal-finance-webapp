...
const GATEWAY_PREFIX = /^(連加|騰加數位|TAPPAY|momo)[*＊_]\s*/i;
...
export function cleanStore(desc) {
  ...
  for (const [canon, re] of STORE_CANON) if (re.test(body)) return normalizeStoreDisplay(withFx(canon));
  // FP 外送前綴（使用者定 2026-07）：先摘下、記住是外送，清理完在店名尾加「（FP）」。
  // 注意這會進 storeKey（＝cleanStore 輸出）——外送與內用本就是不同消費型態，分開記是合理的。
  const fpM = body.match(FP_PREFIX);
  const fpBody = fpM ? body.slice(fpM[0].length) : body;
  const s = fpBody
    ...
    .replace(/[\s（(＊*－\-:：]+$/, '')                                                              // 收尾多餘標點/空白
    .trim();
  const cleaned = normalizeStoreDisplay(withFx(s || fpBody));
  return fpM ? appendFpTag(cleaned) : cleaned;
}

// FP（foodpanda）外送前綴：FP-／FP＊／FP＋空白。要求「FP 後面接分隔符」才算，
// 避免誤傷「FPxxx」這種本來就叫 FP 開頭的店名。
const FP_PREFIX = /^FP[\s\-*＊]+/i;
/** 在店名尾端補「（FP）」外送標記；已有就不重複加（冪等）。 @param {string} name @returns {string} */
function appendFpTag(name) {
  const s = String(name || '');
  return (!s || /（FP）\s*$/.test(s)) ? s : `${s}（FP）`;
}
...
const BRAND_RENAME = [
  ...
  [/台灣普客二四/g, 'Times Parking'],   // 使用者定 2026-07：台灣普客二四 → Times Parking（普客二四＝Times 停車）
];
/** @param {string} s @returns {string} */
...
/**
 * 店名顯示層整理（cleanStore 收尾＋一次性遷移共用）：先統一分店格式（branchNormalize），再套品牌簡稱（applyBrandRename）。
 * 冪等；未來若要加更多顯示規則集中在這裡。 @param {string} name @returns {string}
 */
export function normalizeStoreDisplay(name) {
  return applyParkingLabel(applyBrandRename(branchNormalize(applyBrandCanon(name))));
}

// 停車標記（使用者定 2026-07）：店名含「停車」（含「停車場」）→ 統一顯示成「停車費（原店名）」，
// 收支列表一眼看得出是停車支出。冪等：已是「停車費（…）」不再包一層。
// 註：這是「店名字面」規則——像「嘟嘟房」「阜爾運通」這類名字沒有「停車」二字的停車場不會被套用
//（那些靠分類 交通/停車費 識別）；要改成「依分類」套用再說。
/** @param {string} name @returns {string} */
function applyParkingLabel(name) {
  const s = String(name || '');
  if (!s || !s.includes('停車') || s.startsWith('停車費（')) return s;
  return `停車費（${s}）`;
}
...
function applyBrandCanon(s) {
  ...
  for (const { prefix, brand } of BRAND_CANON) {
    ...
    if (!m) continue;
    // 剩餘去掉前導分隔符（台亞加油站-林口站 的「-」）與空白，才判斷是不是分店（Codex#5）
    const rest = str.slice(m[0].length).replace(/^\s*[-－﹣—–]?\s*/, '').trim();
    let out;
...
export const BRANCH_CHAINS = ['誠品生活'];
const CJK = '一-鿿';
