...
export function parsePdfAuto(lines) {
  ...
  //    判準＝說明落在**封閉幣別集**裡（`CURRENCIES` 來自 schema.js＝單一實作，另加幾個台灣帳單
  //    常印的 ISO 4217 碼——這是**現實世界的封閉集合**，不是「把繞法一條條列出來」）。
  //    ⚠️ 曾經多加一條「而且同一個值要重複出現」當第二層，**已拿掉**：封閉集本身就擋掉了
  //    「KFC」那一類，突變測試證明沒有任何一題分辨得出第二層在不在（＝證明不了的保證）。
  //    ⚠️ 誠實劃界：帳單若印**清單外**的幣別碼，這一層不會作用 ⇒ 退回筆數判準（＝既有行為，不會更糟）。
  const junk = (/** @type {RawTx[]} */ r) => (r.length
    ? r.filter((t) => CURRENCY_CODES.has(String(t.desc || '').trim())).length / r.length
    : 1);
