...
async function extractLines(data, password) {
  ...
  try {
    ...
    if (String(e?.name).includes('Password')) throw new Error('PDF 密碼錯誤（請到卡片追蹤確認「帳單 PDF 密碼」）');
...
function parseFubon(lines) {
  ...
  for (const cells of lines) {
    ...
    // 消費說明＝消費日之後、入帳日之前的文字
    const postIdx = cells.findIndex((c, i) => i > 0 && isRoc(c));
    const desc = (postIdx > 0 ? cells.slice(1, postIdx) : cells.slice(1, cells.length - 1)).join(' ').trim();
    // 外幣資訊（入帳日與金額之間若還有欄位，併進備註）
    const extra = postIdx > 0 ? cells.slice(postIdx + 1, cells.length - 1).join(' ').trim() : '';
    if (!desc) continue;
    txs.push({
