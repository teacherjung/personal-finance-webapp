...
//
// ⚠️ **機構名只能來自「解析器沒消耗掉、又不像交易列」的那些列**（判準在 lib/card-identity.js）。
...
//
// ⚠️ 「挑筆數多者」這個 tie-break **刻意不動**（既有行為）：它與機構名是兩件獨立的事——
//    機構名來自證據，列來自哪支解析器抓得多。兩支對彼此的列形狀大致互斥，實務上不衝突；
//    真要改成「用被認出來那家的解析器」會動到既有台新／富邦使用者的解析結果，不在本支範圍。
/** @param {string[][]} lines @returns {{ bank: string, bankEvidence: 'header'|'none', raw: RawTx[], noRows?: boolean, otherIssuer?: boolean }} */
export function parsePdfAuto(lines) {
  ...
  if (id.own.length === 0 && id.other.length > 0) return { bank: '', bankEvidence: 'none', raw: [], otherIssuer: true };
  ...
  const best = (byIdentity && byIdentity.raw.length) ? byIdentity
    : (taishin.raw.length > fubon.raw.length ? taishin : fubon);   // 認不出身分才回到「筆數多者」
...
export function parseStatementFromLines(lines) {
  ...
  const code = (parsed.noRows || parsed.otherIssuer) ? 'card_unrecognized' : null;
 ...
 *
 * ⚠️ 誠實劃界：這條路的身分證據是**欄位位置**（`parseTaishinXlsxRows` 讀第 0/2/4/7 欄），
 *    **不是版面指紋、也不是欄名**。別家 XLSX 只要欄序不同就會**靜靜抄錯欄**——實測
 *    `[['交易日期','入帳日','幣別','商店名稱','手續費','消費金額'],['2026/07/03',…,'15','1,250']]`
 *    會把「手續費 15」當成消費金額。本支**不修**（改成依欄名定位要有真檔的表頭字面），
 *    只用一題 documenting test 釘住現況並指向後續批次。所以它的 `bankEvidence` 是
 *    `xlsx-template` 而不是 `header`——型別上就標明「這個身分比較弱」。
 *
...
export function parseStatementFromXlsx(rows, allText) {
  ...
  const bank = raw.length ? '台新' : '';
  const bankEvidence = /** @type {'xlsx-template'|'none'} */ (raw.length ? 'xlsx-template' : 'none');
  const code = raw.length ? null : 'card_unrecognized';
  assertCardIdentityInvariants({ bank, bankEvidence, rows: raw, code });
