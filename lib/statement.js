...
// 富邦→PDF（parseFubon）；台新→XLSX（parseTaishinXlsx）。未來新增銀行＝加 parseXxx() 並在此分流。
const OTHER_PDF_BANKS = ['台新', '國泰', '玉山', '中國信託', '中信', '花旗', '匯豐', '星展', '聯邦', '永豐', '兆豐', '第一銀', '華南', '元大', '凱基', '渣打', '新光', '遠東', '樂天', '王道'];
export async function parseStatement(data, password, issuer = '') {
  ...
  // 以下為 PDF：v1 只實作富邦解析器，明確是其他銀行的 PDF 先擋下
  const other = OTHER_PDF_BANKS.find(b => issuer.includes(b));
  if (other && !/富邦/.test(issuer)) {
    throw new Error(`PDF 帳單目前僅支援富邦（這張卡是「${other}」）。${other === '台新' ? '台新請改上傳官網下載的 XLSX 明細。' : '其他銀行之後會陸續支援。'}`);
  }
  const lines = await extractLines(data, password);
  const raw = parseFubon(lines);
  if (!raw.length) throw new Error('找不到消費明細——可能不是支援的帳單格式（PDF 目前支援富邦、XLSX 支援台新），或該期沒有交易。');
