...
/** @param {string[][]} lines @returns {{ bank: string, raw: RawTx[], noRows?: boolean }} */
export function parsePdfAuto(lines) {
  ...
  // ⚠️ **兩支解析器都抓到 0 列＝這份版面我們讀不動**（2026-08-27）：原本這裡無條件挑「筆數多的那個」，
  //   於是別家銀行的帳單只要被 parseFubon／parseTaishinPdf 的寬鬆列判準（首格民國日期＋末格金額／
  //   前兩格都是民國日期）**碰巧抓到幾列**，就會被貼上「富邦」或「台新」的標籤一路往下跑——錯的
  //   機構名會進歸卡比對（issuerMatchesBank）與預覽標題，抓到的還是別家版面的垃圾列。
  //   （使用者的 5 份遠銀帳單實測在這裡是 0 列，沒有假陽性；但那是運氣、不是設計。）
  // ⚠️ **不可改用「行名關鍵字有沒有出現」當判準**（實測踩過）：那個計數掃的是**全文**，
  //   會把**消費明細裡的商店名**也算進去——遠銀帳單因為刷了「富邦人壽」「台新」之類的消費，
  //   台新命中 1 次、富邦命中 4 次，用它判會說「認得這個版面」。行名只適合在兩家之間**挑方向**
  //   （下面那兩行的既有用途），不適合拿來判「是不是我們認得的版面」。
  if (!taishin.raw.length && !fubon.raw.length) return { bank: '', raw: [], noRows: true };
  return taishin.raw.length > fubon.raw.length ? taishin : fubon;
}
...
/**
 * 抓到 0 列時，這是**哪一種** 0 列——判準＝**摘要四格讀不讀得到**（那幾個欄位名是內建範本的一部分）。
 * - 讀得到任一格＝我們認得這個版面、只是這一期沒有消費（合法；**不該**去燒 AI）＝`card_no_rows`
 * - 一格都讀不到＝這份版面我們根本不認得＝`card_unrecognized`（未來卡片 AI 救援的**唯一入口**，
 *   對齊銀行那條路的 `bank_unrecognized`）
 * ⚠️ 原本這兩種混在同一句話裡（「找不到消費明細——…或該期沒有交易」），使用者看不出是哪一種，
 *   程式也沒有機器判準可以據以決定該不該叫 AI。
 * ⚠️ 實測（使用者的遠銀帳單）：它**印了完整的等式行**（上期應繳總額／已繳款退款／本期新增款項／
 *   本期應繳總額…），但欄位名與 PDF 文字層的部首寫法都與內建鍵組不同 ⇒ 四格全讀不到 ⇒ 判認不得。
 *   也就是說「四格全無」在真實樣本上確實是「認不得版面」的可靠證據，不是理論推導。
 * 抽成獨立函式的理由：留在 parseStatement 裡就只有真 PDF 才測得到（合成不了）＝分流本身沒有考題撐。
 * @param {import('./statement-reconcile.js').CardTotals} totals
 */
export function noRowsError(totals) {
  const anyTotal = Object.values(totals || {}).some((v) => v != null);
  return anyTotal
    ? Object.assign(new Error('這份帳單認得出版面，但找不到任何消費明細——可能是這一期沒有交易。'), { status: 400, code: 'card_no_rows' })
    : Object.assign(new Error('這份 PDF 的版面內建範本不認得（目前只認得富邦與台新兩家的信用卡帳單）。'), { status: 400, code: 'card_unrecognized' });
}
...
export async function parseStatement(data, password) {
  ...
  if (isXlsx) {
    ...
    // XLSX 沒有「行名證據」可用（它本來就只支援台新官網那一種格式）＝抓不到列一律當版面認不得
    if (!raw.length) throw Object.assign(new Error('這份 XLSX 的版面內建範本不認得（目前只認得台新官網下載的「信用卡明細」格式）。'), { status: 400, code: 'card_unrecognized' });
    // statementDue＝totals.due（同鍵組同機關＝同值，省一次全文掃描）；statementTotals＝中閘四格（P0 對帳閘）
    const totals = extractStatementTotals(allText);
    return { ...finalize(raw, '台新'), lastFour: extractLastFour(allText), statementMonth: extractStatementMonth(allText), statementDue: totals.due, statementTotals: totals };
  }
  const lines = await extractPdfLines('statement', extractLines, data, password);
  const parsedAuto = parsePdfAuto(lines);
  const { bank, raw } = parsedAuto;
  // ⚠️ **兩件事要分開講**（2026-08-27）：原本「認不得版面」與「這期真的沒消費」共用同一句話
  //   （「…或該期沒有交易」），使用者看不出是哪一種，程式也沒有機器判準可以據以決定「該不該叫 AI 來救」。
  //   銀行對帳單那條路早有 `bank_unrecognized` 這個判準（AI 入口的唯一來源），信用卡這條路一直沒有。
  const allText = lines.map(l => l.join(' ')).join('\n');
  const totals = extractStatementTotals(allText);
  if (parsedAuto.noRows) throw noRowsError(totals);
  return { ...finalize(raw, bank), lastFour: extractLastFour(allText), statementMonth: extractStatementMonth(allText), statementDue: totals.due, statementTotals: totals };
}
