...
import { isRealDate, CURRENCIES } from './schema.js';   // 真日曆驗證（自主體檢：假日期不可流到寫入櫃檯才炸）

/** 帳單上可能印在「幣別欄」的 ISO 4217 碼（封閉集合；`CURRENCIES` 是 app 支援的那幾種＝單一實作，
 *  其餘是台灣信用卡帳單常見的外幣）。⚠️ 用途只有一個：判斷某支解析器是不是把幣別欄誤當成說明。 */
const CURRENCY_CODES = new Set([...CURRENCIES,
  'EUR', 'HKD', 'CNY', 'AUD', 'SGD', 'KRW', 'THB', 'MYR', 'CAD', 'CHF', 'NZD', 'VND', 'PHP', 'IDR', 'MOP', 'MXN', 'ZAR', 'SEK', 'DKK', 'NOK', 'TRY', 'INR', 'RUB', 'BRL', 'PLN', 'CZK', 'HUF', 'ILS', 'AED', 'SAR']);
...
export function parsePdfAuto(lines) {
  ...
  // ⚠️ **認不出身分時，先看哪一支讀出來的「說明」像人話**（Grok 2026-08-27 掃出）：
  //    原本只比筆數。富邦官網下載版（日期｜入帳日｜TWD｜金額，說明換行到下一列）兩支都會抓到列——
  //    `parseFubon` 讀出正確店名，`parseTaishinPdf` 把第三格的 **`TWD`** 當說明。實測使用者的
  //    5 份真富邦帳單目前是靠**筆數打平**才選到富邦的；只要跨頁把說明列拆走、或摘要列長得像兩個
  //    日期，筆數就會翻面 ⇒ 交出一整批 `desc:'TWD'`、還可能把應繳金額當成一筆消費。**錢會記錯。**
  //    判準：**幣別代碼永遠不是店名**（同檔 parseFubon 也把裸的 `TWD` 當雜訊丟掉）。
  //    ⚠️ 判準要**兩個條件同時成立**（Codex #518 r5#1 抓到我第一版的回歸）：第一版寫
  //    `/^[A-Z]{3}$/`＝任何三碼大寫，於是**合法店名「KFC」也被當成幣別** ⇒ 反而選錯解析器、
  //    漏記一筆、另一筆店名錯置。**「三碼大寫」不是幣別的定義。**
  //    判準要**直接描述那個現象**：解析器把「幣別欄」當說明時，**整欄都是同一個值**。
  //    所以看的是「**出現最多次的說明**是不是幣別碼，而且它覆蓋了這份解析的一半以上、至少兩列」。
  //    ⚠️ 只用「是不是幣別碼」不夠（Codex #518 r5#1／r6#1 連續兩次打臉）：
  //      第一版 `/^[A-Z]{3}$/`＝任何三碼大寫 ⇒ 合法店名 **`KFC`** 被當幣別；
  //      第二版換成封閉幣別集，但 **`HUF` 本身就是真品牌**（街頭服飾，台北有門市）⇒ 單筆 HUF 消費
  //      照樣被扣分 ⇒ 選錯解析器、漏記一筆、另一筆店名錯置。**幣別碼與店名的集合本來就相交。**
  //      「整欄同一個值」才是幣別欄與店名欄真正的差別——單筆消費不會重複，欄位值會。
  //    ⚠️ 我曾經拿掉「重複」那一層，理由寫「沒有考題分辨得出來」——**那個理由是錯的**：
  //      單筆 `HUF` 的樣本就分辨得出來。**「現有考題沒涵蓋」不等於「不可證明」**，
  //      不可以拿前者當刪除保護層的理由（考題已補：`r6#1`）。
  //    ⚠️ 誠實劃界：帳單若印**清單外**的幣別碼，這一層不作用 ⇒ 退回筆數判準（＝既有行為，不會更糟）。
  const junk = (/** @type {RawTx[]} */ r) => {
    if (!r.length) return 1;
    /** @type {Map<string, number>} */ const seen = new Map();
    for (const t of r) { const d = String(t.desc || '').trim(); seen.set(d, (seen.get(d) || 0) + 1); }
    let modal = '', n = 0;
    for (const [d, c] of seen) if (c > n) { modal = d; n = c; }
    return (CURRENCY_CODES.has(modal) && n >= 2 && n * 2 >= r.length) ? n / r.length : 0;
  };
  const best = byIdentity || (junk(taishin.raw) !== junk(fubon.raw)
    ? (junk(taishin.raw) < junk(fubon.raw) ? taishin : fubon)
    : (taishin.raw.length > fubon.raw.length ? taishin : fubon));   // 雜訊比一樣才回到「筆數多者」
