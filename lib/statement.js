...
// 帳單結帳日落在月初時，末筆消費日的月份會跟帳單期別對不上，兩期還可能撞成同一個月）。
// 只掃**表頭區**（前 1500 字）：明細區滿是消費日期，掃全文會抓到交易日而非期別。
// 優先序：①明寫的年月（115年6月／2026年6月）②結帳日／帳單日的月份。抓不到回 null，
// 上層退回「末筆消費日的月份」當保底，並在 UI 標示是推估值、可手動修正。
...
export function extractStatementMonth(text) {
  ...
  const toIso = (/** @type {string} */ y, /** @type {string} */ m) => {
  ...
  // ①「115年6月」「民國115年06月份」「2026年6月」——帳單期別最常見的寫法
  let m = head.match(/(?:民國\s*)?(1\d{2}|20\d{2})\s*年\s*(\d{1,2})\s*月/);
  if (m) { const r = toIso(m[1], m[2]); if (r) return r; }
  // ②「結帳日 115/06/21」「帳單日期：2026/06/21」——結帳日所在月份＝該期帳單
  m = head.match(/(結帳|帳單日|帳單日期|本期結帳)[^0-9]{0,12}(1\d{2}|20\d{2})\s*[/／.\-年]\s*(\d{1,2})/);
...
export async function parseStatement(data, password) {
  ...
  if (isXlsx) {
    ...
    return { ...finalize(raw, '台新'), lastFour: extractLastFour(allText), statementMonth: extractStatementMonth(allText) };
  }
  ...
  return { ...finalize(raw, bank), lastFour: extractLastFour(allText), statementMonth: extractStatementMonth(allText) };
}
