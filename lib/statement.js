...
export function normalizeStoreDisplay(name) {
  return applyBrandRename(branchNormalize(applyBrandCanon(name)));
}
...
function applyBrandCanon(s) {
  ...
  for (const { prefix, brand } of BRAND_CANON) {
    ...
    if (rest.startsWith('（') && rest.endsWith('）')) return brand + rest;   // 已格式化的分店 → 保留（冪等）
    if (rest && BRANCH_TAIL.test(rest)) return `${brand}（${rest}）`;         // 純中文分店 → 加括號
    return brand;                                                            // 無分店或剩餘是雜訊 → 只回品牌名
  }
  return str;
}
...
//            （「誠品生活新店」也可能被誤切成「誠品生」＋「活新店」）。新連鎖由使用者從「分店格式整理」預覽發現後增補。
// 冪等：已是「…（分店）」不動；結尾外幣註記（USD/9.99）先摘再接回、不當分店。
// ⚠️ 同步點（AGENTS.md）：cleanStore 收尾會呼叫此函式；「分店格式整理」一次性遷移（statement-import.js）也用它。
