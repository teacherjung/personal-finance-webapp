...
export function cleanStore(desc) {
  ...
  const s = body
    ...
    .replace(/[?？]亭停車/g, '俥亭停車')                                                             // 銀行缺字修補：「俥」超出 Big5，帳單印成「?亭停車」
...
//   例外白名單 PARK_WRAP_EXEMPT（使用者定 2026-07-18）：「儲值」不是在某停車場繳費，
//   包成「停車費（eTag自動儲值）」語意不對 → 維持原名。名字命中白名單＝不包（新的由使用者提出後加）。
// 順序：FP 先、停車後（FP 的「去分店」只作用在店名本體，不會誤拆停車包裝）。
export const FP_PREFIX = /^FP[\s\-*＊]+/i;
const PARK_WRAP_EXEMPT = [/^eTag自動儲值$/i];
/**
