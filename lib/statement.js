...
import { assertPageLimit, countTextItems, assertXlsxShell, assertXlsxSheetRanges } from './parse-limits.js';
...
async function extractLines(data, password) {
  ...
  try {
    ...
    if (String(e?.name).includes('Password')) {
      throw Object.assign(new Error(password
        ...
        : '這份 PDF 有加密，請先到卡片追蹤設定這張卡的「帳單 PDF 密碼」', { cause: e }), { status: 400 });
    }
  ...
  try {
    ...
    for (let i = 1; i <= doc.numPages; i++) {
      ...
      itemCount = countTextItems(itemCount, tc.items.length, '信用卡帳單 PDF');
...
/**
 * **XLSX 唯一准許的讀取入口**——兩段資源上限就掛在這裡（2026-07-28）。
 *
 * 位置就是重點（實測數字見 `lib/parse-limits.js` 檔頭）：
 *  ① `assertXlsxShell` 在 `XLSX.read` **之前**——read 自己就會把每一格建成物件、
 *     把 sharedStrings 全展開；一份 1MB 的壓縮炸彈在這一步就吃掉 1.7GB。
 *  ② `assertXlsxSheetRanges` 在 read **之後、`sheet_to_json` 之前**——後者的迴圈上界是
 *     `ws['!ref']`，而 `!ref` 直接抄自檔案裡的 `<dimension>`，SheetJS 完全不檢查上界。
 *     一份 1.9KB 的檔可以宣告 1.72e10 格，純 CPU 空轉、事件圈鎖死數十分鐘，
 *     **不會 OOM、行程也不會自己死**——沒有任何記憶體上限救得了它。
 *
 * ⚠️ **不要繞過這支自己呼叫 `XLSX.read`**（有考題盯著）。
 * @param {Uint8Array} data @returns {any}
 */
function readXlsxGuarded(data) {
  assertXlsxShell(data);
  const wb = XLSX.read(data, { type: 'array' });
  assertXlsxSheetRanges(wb, XLSX.utils);
  return wb;
}

// 台新 XLSX（官網下載「信用卡明細」）：西元日期、金額獨立欄，結構乾淨。
// 交易列＝ col0 消費日期(YYYY/MM/DD) | col1 入帳日 | col2 消費明細 | col3 幣別 | col4 金額 | … col7 外幣幣別/金額
/** @param {Uint8Array} data @param {any=} preRead 已經讀好的活頁簿（parseStatement 讀一次共用） @returns {RawTx[]} */
export function parseTaishinXlsx(data, preRead) {
  const wb = preRead || readXlsxGuarded(data);
...
/**
 * @param {Uint8Array} data @param {any=} preRead 已經讀好的活頁簿（parseStatement 讀一次共用）
 * ⚠️ `try/catch { return '' }` **只准包住「抽文字」那一段**，絕不可以包住 `readXlsxGuarded`——
 *    包進去的話，資源上限丟出來的錯會被這裡默默吞掉、回一個空字串，等於那道牆不存在
 *    （這是本次施工差點踩到的坑）。所以牆一律在 parseStatement 那一層先跑完。
 */
function xlsxAllText(data, preRead) {
  try {
    const wb = preRead || readXlsxGuarded(data);
    /** @type {string[]} */
    const out = [];
...
export async function parseStatement(data, password) {
  ...
  if (isXlsx) {
    // ⚠️ **讀一次就好**（2026-07-28）：舊版 parseTaishinXlsx 與 xlsxAllText 各自 `XLSX.read` 一次，
    //    同一份檔案被完整解析兩趟＝成本與峰值記憶體直接加倍。在 512MB 的機器上這不是小事。
    //    順帶讓兩段資源上限只跑一次、而且**跑在 xlsxAllText 的 try/catch 之外**（不會被吞掉）。
    const wb = readXlsxGuarded(data);
    const raw = parseTaishinXlsx(data, wb);
    if (!raw.length) throw new Error('這份 XLSX 找不到消費明細——目前 XLSX 支援台新官網下載的「信用卡明細」格式。');
    const allText = xlsxAllText(data, wb);
