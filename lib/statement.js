...
function gasStationDisplay(name) {
  ...
  /** 認得出分店用分店、認不出用品牌（品牌已含「加油站」就不再加） @param {string} stem @param {string} brand */
  const pick = (stem, brand) => (isGasBranch(stem) ? `${stem}加油站` : (/加油站$/.test(brand) ? brand : `${brand}加油站`));
  let out = base;
  const m = base.match(/^([^（）]+)（([^（）]+)）$/);
  if (m) out = pick(stemOf(m[2]), m[1].trim());                          // 品牌（分店）
  else {
    const glued = base.match(/^([^（）]*加油站)(.+)$/);                   // 黏寫未拆：柑園加油站林口二站
    if (glued) out = pick(stemOf(glued[2]), glued[1]);
    else for (const { prefix, brand } of GAS_BRANDS) {                   // 無分隔符無括號：品牌前綴切
      const hit = base.match(prefix);
      if (!hit) continue;
      out = pick(stemOf(base.slice(hit[0].length).trim()), brand);
      break;
    }
  }
  return out + tail;
