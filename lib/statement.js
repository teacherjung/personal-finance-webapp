...
// 店名清理（僅顯示用）：去掉金流/通路前綴與結尾的分店定位碼/城市雜訊，讓帳單店名好讀。
// **只影響顯示**（匯入後的 note、預覽的顯示）；分類與去重（stmtRef）仍用原始 desc，不受影響。
// 盡力而為：台新 PDF 常把店名截斷，救不回被截掉的字；全被清光則退回原字。
export function cleanStore(desc) {
  const raw = String(desc || '');
  const s = raw
    .replace(/^(FP-|連加\*|TAPPAY_)/i, '')                                                          // 前綴：外送/金流通路（保留 foodpanda- 這類本身可讀的字）
    .replace(/\s*[A-Z]\d{3,4}\s*(NEW\s*TAI?P?E?I?|New\s*Tai?p?e?i?|TAIPEI|Taipei|TW)?\s*$/i, '')   // 結尾：分店碼(＋城市)
    .replace(/\s*(NEW\s*TAI?P?E?I?|New\s*Tai?p?e?i?|TAIPEI|Taipei)\s*$/i, '')                       // 結尾：殘留城市
    .replace(/\d{8,}\s*$/, '')                                                                     // 結尾：長數字(統編/序號)
    .replace(/[\s（(＊*－-]+$/, '')                                                                  // 收尾：多餘標點/空白
    .trim();
  return s || raw;
}
