...
// ①FP 外送：帳單原文有 FP 前綴（FP-／FP＊／FP＋空白；要求有分隔符，免誤傷本來就叫 FPxxx 的店）→ 店名尾加「（FP）」
...
export function applyDisplayLabels(name, ctx = {}) {
  let s = String(name || '');
  if (!s) return s;
  if (String(ctx.subcategory || '') === '停車費' && !s.startsWith('停車費（')) s = `停車費（${s}）`;
  if (FP_PREFIX.test(String(ctx.desc || '')) && !/（FP）\s*$/.test(s)) s = `${s}（FP）`;
  return s;
}
