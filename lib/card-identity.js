 ...
 *
 * ⚠️ **不要在這裡用 `\s`**（Codex #518 r1#1 實測打穿過）：原本寫 `/台\s*新/`，而 `\s` **吃換行**——
 *    「電子服務平**台**」與「**新**戶刷卡禮」兩列被拼成「台新」，一份臺中商銀的帳單因此掛上台新標籤
 *    並自動歸卡。
 *    ⚠️ **誠實劃界**：修掉那個洞的是**結構**（`identifyIssuer` 逐列比對、`evidenceRows` 不再交出整串），
 *    不是這裡少了一個 `\s`。突變測試實證：把 `\s*` 放回來，**沒有任何一題會紅**——因為列內空白
 *    已經被 `squash` 去掉，`\s*` 只能匹配空字串。所以這一條是**冗餘的第二層、不是保證**；
 *    真正該守住的不變量是「比對永遠逐列進行」（那條有考題：`r1#1a` 與 `evidenceRows` 那題）。
...
export const OWN_ISSUERS = /** @type {const} */ ([
  { bank: '台新', re: /台新|臺新|Taishin|Richart/i },
  { bank: '富邦', re: /台北富邦|臺北富邦|TaipeiFubon/i },
]);
