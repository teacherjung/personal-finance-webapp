...
export function assertXlsxShell(data) {
  ...
  const fcnt = buf.readUInt16LE(eocd + 10);
  ...
  for (let n = 0; n < fcnt; n++) {
    ...
    // 只取中央目錄記的 uncompressed size：切資料一律用 local header 那份（與 SheetJS 一致），
    // 但兩份都要納入「宣告值過大」的判斷——攻擊者可能只在其中一份說謊。
    ...
    ({ usz } = applyZip64(cdExtra, usz, buf.readUInt32LE(off + 20)));
    ...
    // ⚠️ data descriptor（bit 3）：真正的大小寫在資料**後面**，local header 這裡是 0。
    //    我們沒有安全的方式在解壓前知道它多大 → fail-closed。正常的 xlsx 產生器不會這樣寫。
    if (flags & 0x08) malformed('這份 Excel 用了我們不支援的壓縮寫法（streaming）');
