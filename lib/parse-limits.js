 ...
 *      ・138KB 那份 → 擋下，峰值 254MB（原本 612MB 崩潰）
 *      ・207KB 那份 → 擋下，峰值 316MB（原本 704MB 崩潰）
 *
  ...
  try {
    for (;;) {
      ...
      if (n > MAX_PDF_TEXT_ITEMS) {
        await cancelStream(reader, '超過文字節點上限，主動中止 pdfjs 串流');
        throw Object.assign(
    ...
    // reader 沒讀完就離開時要放掉，否則 pdfjs 那邊的 stream 會留著
    await cancelStream(reader, '抽取結束，放掉 pdfjs 串流');
  }
