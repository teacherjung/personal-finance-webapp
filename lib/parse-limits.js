 ...
 *      修好之後，同一顆 207KB 炸彈檔由這道牆在 1.9 秒當場擋下、不再需要逾時去收。
 ...
 *    ⚠️ **這裡是這組數字的單一真相**（Codex #538 r1：`lib/pdf-isolate.js` 另抄了一份 406／456，
 *      兩份對不起來、也沒人發現。抄本會漂，那份已改成指過來）。
 *    ⚠️ **2026-08-29 重量（取消修好之後）**：macOS／Node 26／pdfjs 6.1.200／子行程
 *      `--max-old-space-size=256`，兩顆都回乾淨的 400，**子行程峰值 RSS 317MB／349MB**。
 *      與 2026-07-29 那組不同是正常的——當時是行程內直跑、heap 上限 400MB、機器與版本都不同；
 *      **舊的量測條件已無法重建，所以不宣稱兩組可以互相對照**。
 *
 ...
 *    帶了理由之後實測：`cancel ok` → `destroy` 2ms 完成、207KB 的炸彈檔 1.9 秒回乾淨的 400。
 *
 * ⚠️ **失敗要出聲、不可以純吞**：取消失敗會把「牆擋下了」靜靜變成「卡死」，
 *    而靜靜通過比沒有護欄更糟。這條路一年也走不到幾次，沒有噪音成本。
 *    （只印錯誤訊息，**不印 PDF 內容**——帳單內容含真實金額與帳號。）
...
async function cancelStream(reader, why) {
  try {
    ...
    console.error('[parse-limits] 取消 pdfjs 串流失敗（上游資源可能不會被釋放）：', String(e?.message || e));
  }
}
