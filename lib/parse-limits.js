...
/**
 * 一份 xlsx 裡最多幾個 ZIP 條目。
 *
 * ⚠️ **2026-07-29 從 64 調到 512**（Codex 定向複審實測：**微軟官方的 Excel 範例有 117 個部件**，
 *    64 會把它擋掉——那是誤殺正常檔案，不是防禦）。
 *
 * 實測（`--max-old-space-size=400`，量的是「數量 → 峰值 RSS」）：
 *      非工作表部件（styles/theme 之類）  1000 個 → 186MB／35ms   ← 便宜
 *      工作表                              1000 個 → 229MB／270ms  ← 真正貴的是這個軸
 *    危險區在 1000 以上（另有量到 1500 張表→547MB、10000 張→1113MB）。
 *    512 的餘裕：微軟那份 117 個有 4 倍；最壞情況（512 個**全是**工作表）約 128MB，
 *    加上 app 底噪仍在 512MB 以內。真實台新帳單只有 10 個。
 */
 ...
 *
 * ⚠️⚠️ **一定要從「中央目錄」枚舉，不可以循序掃 local header**（2026-07-29，自審抓到的繞過）。
 *    v1 是「從檔頭開始，一個 local header 接一個往下跳」，但 **SheetJS 走的是中央目錄**
 *    （`xlsx.mjs` 的 `parse_zip`：從尾巴找 EOCD → 讀 `start_cd` → 依每個目錄項目記的
 *    絕對位移 seek 到 local header）。**兩者枚舉的不是同一組條目**——
 *    在 local 條目之間插一段填充 byte，v1 讀到非 `0x04034b50` 就靜默結束、回報「檢查過且乾淨」，
 *    SheetJS 卻照中央目錄把真正的炸彈條目找出來解開。實測 <1MB 的檔把 RSS 推到 564MB。
 *    **牆要跟被保護的解析器讀同一份 metadata，否則牆看到的世界跟它看到的不是同一個。**
 *
 * ⚠️ **結構看不懂一律 fail-closed**：找不到 EOCD、目錄位置越界、項目讀不到、位移指到不是
 *    檔案開頭的地方、內容超出檔案範圍——全部拒收。v1 在這些情形 `break` 出迴圈**當成成功**，
 *    那本身就是第二種繞過方式。
 *
 * ZIP64 的 extra field 也要跟著採用（`applyZip64`）：SheetJS 會用它覆蓋大小，
 * 我們不採用的話又會變成「看到的大小不一樣」。
 *
 * data descriptor（bit 3，真正的大小寫在資料**後面**、local header 是 0）＝我們沒有安全的方式
 * 在解壓前知道它多大 → 拒收。正常的 xlsx 產生器不會這樣寫。
 * @param {Uint8Array} data
 */
export function assertXlsxShell(data) {
  const buf = Buffer.from(data.buffer, data.byteOffset, data.byteLength);
  const reject = (/** @type {string} */ msg, /** @type {string} */ code) => { throw xlsxTooBig(msg, code); };
  /** 結構看不懂一律拒收——**不可以靜默當成通過**（v1 就是這樣被繞過的）。 */
  const malformed = (/** @type {string} */ why) => reject(
    `這份 Excel 的結構我們看不懂（${why}），為了安全不予解析。請確認檔案沒有損壞，或改用 PDF 版帳單。`,
    'xlsx_malformed_zip');

  // ── 1) 找 End Of Central Directory ────────────────────────────────────────
  // ⚠️ **起點必須是 `length - 4`，不是 `length - 22`**（2026-07-29，規格符合性審查抓到）。
  //    SheetJS 是 `var i = blob.length - 4; while(簽章不符) --i;`——它從**最後 4 個 byte**
  //    開始往前找。v3 從 `length - 22` 開始（因為 EOCD 至少 22 bytes），
  //    於是「簽章出現在最後 21 個 byte 之內」時**兩邊會找到不同的 EOCD**——
  //    又是同一個病：牆跟解析器看的不是同一個東西。
  //
  // ⚠️ 而且**候選位置放不下完整 22 bytes 就直接拒絕**，不可以退回去找另一個
  //    「看起來比較合理」的 EOCD——那樣又會跟 SheetJS 分岔。
  let eocd = -1;
  for (let i = buf.length - 4; i >= 0; i--) {
    if (buf.readUInt32LE(i) === 0x06054b50) { eocd = i; break; }
  }
  if (eocd < 0) malformed('找不到 ZIP 的結尾目錄');
  if (eocd + 22 > buf.length) malformed('結尾目錄被截斷了');
  // 註解長度要對得起檔案長度——對不上代表結構有我們讀不懂的東西
  const commentLen = buf.readUInt16LE(eocd + 20);
  if (eocd + 22 + commentLen !== buf.length) malformed('結尾目錄的註解長度對不上檔案大小');
  // 分割／跨磁碟的 ZIP 兩邊都不支援——不要靠「後面偶然越界」失敗，明確拒絕
  if (buf.readUInt16LE(eocd + 4) !== 0 || buf.readUInt16LE(eocd + 6) !== 0) {
    malformed('這是分割成多個檔案的 ZIP，我們不支援');
  }
  // ⚠️⚠️ **EOCD 的條目數要讀 `+8`，不是 `+10`**（2026-07-29，Codex 定向複審抓到的第四次繞過）。
  //    EOCD 配置：+8＝「本磁碟的條目數」／+10＝「總條目數」。
  //    **SheetJS 讀的是 `+8`**（`xlsx.js` 與 `xlsx.mjs` 都是：找到 EOCD 後 `blob.l = i+4`
  //    再 `blob.l += 4` → 從 i+8 讀）。v2 讀 `+10`，於是一份「本磁碟=5、總數=0」的 ZIP
  //    讓牆量到 0 個條目、回報乾淨，SheetJS 卻照樣解開 5 個條目、18MiB。
  //
  //    ⚠️ 這與第三次繞過（local header vs 中央目錄）**是同一個病**：牆跟解析器看的不是同一個東西。
  //    我上一版修掉的是那個**實例**，不是那個**類別**。所以這裡除了改成 +8，
  //    還多加一道：**兩個欄位不一致就 fail-closed**——單一磁碟的正常 ZIP 兩者必定相等，
  //    不相等本身就是「有人在玩這個差異」的訊號。
  const fcnt = buf.readUInt16LE(eocd + 8);        // ← 與 SheetJS 同一個欄位
  const fcntTotal = buf.readUInt16LE(eocd + 10);
  if (fcnt !== fcntTotal) malformed('目錄的條目數自相矛盾');
  const startCd = buf.readUInt32LE(eocd + 16);
  if (startCd >= buf.length) malformed('目錄位置超出檔案範圍');
  if (fcnt > MAX_XLSX_ZIP_ENTRIES) {
    reject(
      `這份 Excel 的內部結構太複雜（${fcnt} 個部件，上限 ${MAX_XLSX_ZIP_ENTRIES}）。` +
      '正常的帳單檔只有十個左右——請確認下載到的是台新官網的「信用卡明細」。',
      'xlsx_too_many_entries');
  }

  // ── 2) 逐條走中央目錄，**條目數與順序都跟 SheetJS 一樣** ────────────────────────
  let off = startCd;
  let totalOut = 0;
  for (let n = 0; n < fcnt; n++) {
    if (off + 46 > buf.length || buf.readUInt32LE(off) !== 0x02014b50) malformed(`第 ${n + 1} 個目錄項目讀不到`);
    // ⚠️⚠️ **ZIP64 一律拒絕，不要自己解**（2026-07-29，規格符合性審查抓到的必修）。
    //    v3 加了 `applyZip64`：把 `0xFFFFFFFF` sentinel 換成 extra field 裡的真實值。
    //    **那讓牆變弱了**——因為 **SheetJS 0.18.5 根本不解 ZIP64**
    //    （`parse_extra_field` 只認 timestamp `0x5455`，不認 `0x0001`），
    //    它會照 `0xFFFFFFFF` 去配置緩衝區。於是：
    //        我的掃描器：sentinel → 解成小值 → 「安全」放行
    //        SheetJS    ：sentinel → 當成 4GB → 真的去要那麼大的空間
    //    **我為了「更精確」而多做的解碼，正好製造了一個繞過。**
    //
    //    正確的原則（值得記住）：**不要自己維護第二份近似解碼器**。
    //    要嘛跟解析器共用同一份 metadata，要嘛**明確拒絕它讀不懂的東西**。
    //    SheetJS 不支援 ZIP64 → 我們就拒絕 ZIP64，兩邊的理解才會一致。
    const cUszRaw = buf.readUInt32LE(off + 24);
    const cCszRaw = buf.readUInt32LE(off + 20);
    const cOffRaw = buf.readUInt32LE(off + 42);
    if (cUszRaw === 0xFFFFFFFF || cCszRaw === 0xFFFFFFFF || cOffRaw === 0xFFFFFFFF
        || buf.readUInt16LE(off + 34) === 0xFFFF) {
      malformed('這份 Excel 用了 ZIP64 格式，我們的解析器不支援');
    }
    const usz = cUszRaw;
    const nameLen = buf.readUInt16LE(off + 28);
    const efSz = buf.readUInt16LE(off + 30);
    const cmSz = buf.readUInt16LE(off + 32);
    const local = buf.readUInt32LE(off + 42);
    // ZIP64 的 extra field（0x0001）可以覆蓋大小——SheetJS 會採用它，我們也要
    const csz = cCszRaw;
    off += 46 + nameLen + efSz + cmSz;

    // ── 3) 跟著 offset 去讀 local header（SheetJS 讀的就是這一份大小）────────────
    if (local + 30 > buf.length || buf.readUInt32LE(local) !== 0x04034b50) {
      malformed(`第 ${n + 1} 個項目的位置指到不是檔案開頭的地方`);
    }
    const flags = buf.readUInt16LE(local + 6);
    const method = buf.readUInt16LE(local + 8);
    if (flags & 0x2041) malformed('這份 Excel 有加密（我們不支援）');
    let lUsz = buf.readUInt32LE(local + 22);
    let lCsz = buf.readUInt32LE(local + 18);
    const lNameLen = buf.readUInt16LE(local + 26);
    const lEfSz = buf.readUInt16LE(local + 28);
    // local header 的 sentinel 同樣拒絕（理由同上：SheetJS 會照原值配置）
    if (lUsz === 0xFFFFFFFF || lCsz === 0xFFFFFFFF) {
      malformed('這份 Excel 用了 ZIP64 格式，我們的解析器不支援');
    }
    // ⚠️ **data descriptor（bit 3）一定要支援，不可以拒收**（2026-07-29，Codex 定向複審實測）。
    //    v2 的註解寫「正常的 xlsx 產生器不會這樣寫」——**那是錯的**：
    //    **LibreOffice Calc 與 Google Sheets 匯出的檔案都會用**，而且 SheetJS 明確支援。
    //    拒收等於擋掉正常使用者的檔案。
    //    做法：bit 3 時 local header 的大小欄位是 0，**改用中央目錄那一份**
    //    （在這個情形下中央目錄才是權威的，SheetJS 也是拿中央目錄的值傳進 parse_local_file）。
    const streaming = (flags & 0x08) !== 0;
    if (streaming) { lUsz = usz; lCsz = csz; }

    // ⚠️ **宣告值與實際值各查一次，方向相反**（v1 被打出來的教訓，保留）：
    //    ・宣告「很大」→ 一定要擋：SheetJS 會照宣告值預先配置緩衝區
    //      （`_inflateRawSync(blob, _usz)`），1.4KB 的檔宣告 400MB → 峰值 RSS 694MB。
    //    ・宣告「很小」（含 0）→ **完全不能信**：兩個獨立的對抗 agent 用「兩邊都宣告 0」
    //      破過第一版。所以真實大小一律**自己解壓量**。
    const declared = Math.max(usz, lUsz);
    ...
    const dataStart = local + 30 + lNameLen + lEfSz;
    if (dataStart + lCsz > buf.length) malformed(`第 ${n + 1} 個項目的內容超出檔案範圍`);
    // ⚠️ 壓縮方法要跟 SheetJS 同一份 allowlist（它只准 0 與 8，其餘明確拒絕）。
    //    v3 對「非 0 一律當 deflate 試試看」——那又是一種「兩邊理解不同」。
    if (method !== 0 && method !== 8) malformed(`這份 Excel 用了我們不支援的壓縮方式（${method}）`);
    const raw = buf.subarray(dataStart, dataStart + lCsz);
    try {
      totalOut += method === 0
        ? raw.length
        : inflateRawSync(raw, { maxOutputLength: Math.max(1, MAX_XLSX_UNZIPPED_BYTES - totalOut) }).length;
    } catch {
      reject(
  ...
  return { entries: fcnt, unzippedBytes: totalOut };
}

// ⚠️ 這裡曾經有一支 `applyZip64()`，把 `0xFFFFFFFF` sentinel 換成 extra field 裡的真實值。
//    **2026-07-29 移除**：它讓牆變弱了。SheetJS 0.18.5 不解 ZIP64，會照 sentinel 去配置空間，
//    而我們解出小值就放行——「為了更精確而多做的解碼」正好製造了一個繞過。
//    現在改成**明確拒絕 ZIP64**（見上面的 sentinel 檢查）。
//    **不要把它加回來**——除非 SheetJS 哪天真的支援 ZIP64，而且我們改成共用它解出來的 metadata。
