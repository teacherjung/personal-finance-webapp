 ...
 */
export const PARSE_DIAG = /** @type {const} */ ({ CANCEL_FAILED: 'pdf_cancel_failed' });
/** @type {Set<string>} */
 ...
 * ⚠️ **界線**（同一件事以前在兩個地方說了兩種話，r3 抓到）：**我們手上的 PDF 內容一個字都不印**
 *    （chunk／items 都不進日誌，有考題釘著）；**pdfjs 丟出來的例外訊息會印**，那是診斷的本體，
 *    且已截到 200 字。兩者的分界就是「這串字是我們讀出來的內容，還是函式庫給的錯誤說明」。
