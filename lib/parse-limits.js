...
export function assertXlsxShell(data) {
  ...
  // ── 1) 找 End Of Central Directory：**從尾巴往前掃**，與 SheetJS 完全一致 ──────────
  //    （xlsx.mjs 的 parse_zip：`while(blob[i]!=0x50||…) --i`）
  let eocd = -1;
  ...
  for (let n = 0; n < fcnt; n++) {
    ...
    // 中央目錄的大小：平常只當「宣告值過大」的比對之一（攻擊者可能只在其中一份說謊），
    // **但遇到 data descriptor（bit 3）時它是唯一權威的來源**——那時 local header 是 0。
    let usz = buf.readUInt32LE(off + 24);
    ...
    const cdExtra = buf.subarray(off + 46 + nameLen, off + 46 + nameLen + efSz);
    let csz = buf.readUInt32LE(off + 20);
    ({ usz, csz } = applyZip64(cdExtra, usz, csz));
    ...
    const lExtra = buf.subarray(local + 30 + lNameLen, local + 30 + lNameLen + lEfSz);
    ({ usz: lUsz, csz: lCsz } = applyZip64(lExtra, lUsz, lCsz));
...
/**
 * ZIP64 的 extra field（header id 0x0001）可以覆蓋 uncompressed／compressed size。
 * **SheetJS 會採用它，所以我們也必須採用**——不然它看到的大小跟我們看到的又不是同一個。
 * @param {Buffer} extra @param {number} usz @param {number} csz
 */
function applyZip64(extra, usz, csz) {
  let i = 0;
  while (i + 4 <= extra.length) {
    const id = extra.readUInt16LE(i);
    const size = extra.readUInt16LE(i + 2);
    if (id === 0x0001) {
      // 只在原欄位是 0xFFFFFFFF（＝「看 ZIP64」）時才覆蓋，順序與規格一致
      let k = i + 4;
      if (usz === 0xFFFFFFFF && k + 8 <= extra.length) { usz = Number(extra.readBigUInt64LE(k)); k += 8; }
      if (csz === 0xFFFFFFFF && k + 8 <= extra.length) { csz = Number(extra.readBigUInt64LE(k)); }
      break;
    }
    i += 4 + size;
  }
