 ...
 *    ⚠️ 死法不只 OOM（2026-08-02 追出）：也可能是 pdfjs 卡死在解壓、promise 永不 settle
 *      ——那條路 `readPageTextCapped` 也接不到（reader.read() 不會回來），要靠行程隔離的逾時。
 *    「逐頁累加」當初就是為了避免「等整份讀完才檢查」，但它在**單頁之內**仍然是事後才數。
 *    牆的形狀對，位置差一層。
 *
 * ✅ 改用 `streamTextContent()` 邊收邊數、超標就 `cancel()` 當場中止：
