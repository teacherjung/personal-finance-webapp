...
import { resolveFxTable, fxFor } from '../../public/modules/fx-rates.js';   // 匯率表唯一實作（丙-2）
...
export async function syncIb(fetchImpl = fetchFlex) {
  ...
  // 現金流缺 IBKR 匯率時，用設定匯率（沒抓到＝預設值；丙-2）估算為 USD 基準——與交易摘要 tradePnlBase 同一份匯率表（fx-rates.js）。
  // 只有不支援的幣別才回 null（上游列入 skippedNoFx）。
  const fxTable = resolveFxTable(pre);
  const fxToBase = (/** @type {string} */ cur) => {
    const c = String(cur || 'USD').toUpperCase();
    if (c === 'USD') return 1;
    const f = fxFor(fxTable, c);
    return f.missing ? null : f.rate / fxTable.rates.USD;
  };
  let data;
  ...
  // ⚠️ 守衛後**重建**物件、把欄位明確收斂成 number（Codex #407 r3 High）：
  //    parser 自 2026-08-05 起對缺欄／空白回 null（嚴格取值），直接指派原物件會讓型別
  //    變成 number|null 而與 IbEquity 契約衝突；放寬 IbEquity 允許 null 是錯的方向——
  //    那會削弱「壞官方淨值整包清成 null」這個資料契約（持久化的值只有兩種：完整或不存在）。
  db.settings.ib.lastEquity = (eq && Number.isFinite(eq.stock) && Number.isFinite(eq.cash))
    ? { stock: Number(eq.stock), cash: Number(eq.cash), date: String(eq.date || '') }
    : null;
