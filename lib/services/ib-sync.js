...
export async function syncIb(fetchImpl = fetchFlex) {
  ...
  const cashDetailMissing = Boolean(data.hasCashReport && !data.hasCashDetail);
  ...
  if (cashDetailMissing) console.warn('[ib-sync] Cash Report 只有 BASE_SUMMARY 彙總列、沒有任何幣別明細 → 沿用現有現金餘額（不歸零，避免誤清）。若非預期，請確認 Flex Query 的 Cash Report 有勾「Currency Breakout」。');
  return { ok: true, updated, created, missing, skippedCurrencies, cashZeroed, cashReportMissing, cashDetailMissing, cash: data.cashByCurrency, equity: data.equity, account: data.account };
}
