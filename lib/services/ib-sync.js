...
export async function syncIb(fetchImpl = fetchFlex) {
  ...
  // ⚠️ 只在 Cash Report 區塊「確實存在」時歸零——整個區塊缺失（設定漏勾/查詢失敗）代表「沒資料」，
  // 不是「現金為 0」，硬歸零會把真實餘額誤清；此時保留舊值並回報，讓使用者看得見異常。
  let cashZeroed = 0;
  const cashReportMissing = !data.hasCashReport;
  if (data.hasCashReport) {
  ...
  return { ok: true, updated, created, missing, skippedCurrencies, cashZeroed, cashReportMissing, cash: data.cashByCurrency, equity: data.equity, account: data.account };
}
