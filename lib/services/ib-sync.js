...
export async function syncIb(fetchImpl = fetchFlex) {
  ...
  if (Number(data.statementCount) > 1) {
    throw Object.assign(new Error(
      `這份 Flex Query 內含 ${data.statementCount} 個帳戶的報表。目前僅支援單一帳戶——多帳戶會讓現金與淨值只剩最後一個帳戶的值、同代號持股互相覆蓋。請在 IBKR 的 Flex Query 設定裡只選一個帳戶（或替每個帳戶各建一個 Query）。`), { status: 400 });
  }
  ...
  let cashDetailMissing = Boolean(data.hasCashReport && !data.hasCashDetail);
  if (data.hasCashDetail) {
    ...
    if (base && curOk(base) && Number.isFinite(data.baseSummaryCash)) {
      ...
      acc.currency = base;
      for (const a of db.accounts || []) {
        if (a.ibCashCur && a.ibCashCur !== base && Number(a.balance) !== 0) { a.balance = 0; cashZeroed++; }
      }
  ...
  if (cashFromSummary) console.warn('[ib-sync] Cash Report 只有彙總列 → 以基準幣別總額入帳（各幣別明細不可得；其他幣別的 IB 現金帳戶已歸零避免重複計算）。');
  return { ok: true, updated, created, missing, skippedCurrencies, cashZeroed, cashReportMissing, cashDetailMissing, cashFromSummary, cash: data.cashByCurrency, equity: data.equity, account: data.account };
}
