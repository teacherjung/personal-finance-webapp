...
import { normalizeIbTrade, assignSeqSuffix } from './security-trades.js';
...
export async function syncIb(fetchImpl = fetchFlex) {
  ...
  let secTradesAdded = 0, secTradesUpdated = 0, secTradesSkipped = 0;
  {
    const normalized = (Array.isArray(data.rawTrades) ? data.rawTrades : []).map(normalizeIbTrade).filter(t => t != null);
    const usable = /** @type {NonNullable<ReturnType<typeof normalizeIbTrade>>[]} */ (normalized.filter(t => t && !t.flags.unknownType && !t.flags.missingAccount && CURRENCIES.includes(String(t.currency).toUpperCase())));   // 幣別牆：不支援幣別（EUR…）放行會在櫃檯枚舉驗證炸掉整次同步
    secTradesSkipped = normalized.length - usable.length;
    assignSeqSuffix(usable);
    db.securityTrades = db.securityTrades || [];
    /** @type {Map<string, any>} */
    const byRef = new Map(db.securityTrades.map(r => [String(r.sourceRef || ''), r]));
    const importedAt = new Date().toISOString();
    for (const t of usable) {
      const { flags: _flags, ...fields } = t;
      /** @type {any} */
      const row = {};
      ...
      if (existing) {   // 重疊期間重同步：以來源最新值就地更新（id/importBatch/importedAt 保留首次）
  ...
  if (secTradesSkipped) console.warn(`[ib-sync] 證券交易集合跳過 ${secTradesSkipped} 筆（取消列/未知買賣別/缺帳戶識別）——查帳頁不顯示這些列；ibTrades 鏡像照舊保留供交易摘要/XIRR。`);
  return { ok: true, updated, created, missing, skippedCurrencies, cashZeroed, cashCollapsed, cashReportMissing, cashDetailMissing, cashDetailIncomplete, cashFromSummary, cashBaseUnsupported, cashSummaryMissing, secTradesAdded, secTradesUpdated, secTradesSkipped, cash: data.cashByCurrency, equity: data.equity, account: data.account };
}
