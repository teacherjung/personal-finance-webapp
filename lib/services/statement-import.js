...
export async function previewAuto(b64, password) {
  ...
  if (parsed.lastFour) {
    ...
    // ⚠️ **認不出機構時，末四碼唯一命中也只當「候選」、不自動選**（Codex #518 r1#2）：
    //    末四碼不是全域唯一，而「認不出機構」代表那些列可能是別家版面被內建範本硬讀出來的垃圾。
    //    分支④說的是「禁止自動歸卡」——原本這一段排在機構判斷**之前**，等於整條分支④被繞過。
    if (hit.length === 1 && parsed.bank) resolved = hit[0];
