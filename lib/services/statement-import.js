...
export function normalizeBranches(dryRun = false) {
  ...
  for (const t of db.transactions || []) {
    ...
    // 舊 note 可能已包顯示標記（停車費（…）＝巢狀括號，整理規則比不到）→ 先拆殼、整理、再重新上標記（皆冪等）
    const afterNote = applyDisplayLabels(normalizeStoreDisplay(stripDisplayLabels(beforeNote)), { desc: orig, subcategory: t.subcategory });
    ...
    if (!dryRun && t.storeKey) {   // 內部 key 一併對齊（不列入 note 預覽）：有原文→用 cleanStore 重算（規則升級全套上）；沒有→就地正規化
      ...
      const newKey = (orig ? cleanStore(orig) : normalizeStoreDisplay(oldKey)) || oldKey;
      if (newKey !== oldKey) {
