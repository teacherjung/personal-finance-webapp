...
import { backupBeforeIrreversible } from './backup.js';
 ...
 * ⚠️ 實際套用前會做一顆 `pre-normalize` 備份，而且**備份沒做成就不寫**——直接 throw（`code:'backup_failed'`）
 * 讓呼叫端去問使用者（William 2026-08-06 裁決，見 `backupBeforeIrreversible`）。
 *（原註解寫「自動備份（save 內建 .bak）」是**錯的**：`saveDb` 不碰任何 .bak，啟動備份 `.bak` 由
 *  `store.js backupOnce` 每個行程只寫一次——真正保護這次操作的是下面那顆 `pre-normalize`。）
 * @param {boolean} [dryRun]
 * @param {{proceedWithoutBackup?: boolean}} [opts] 使用者已看過「這次沒存成還原檔」的確認、選擇繼續
 */
export async function normalizeBranches(dryRun = false, opts = {}) {
  const db = await getDb();
  ...
  const learnedNameChanges = [];
  {
    ...
    if (!dryRun) {
      ...
      if (changes.length || keyChanged || learnedRemapped || learnedNamesFixed) {
        // 動手之前先存一份（Codex r3#7）：整理會改掉學過的名字，有些還原不回來。
        // ⚠️ 回傳值不可以丟掉——備份沒做成就不要動資料（William 2026-08-06 裁決）。這裡 throw 而不是
        // 回旗標，是因為本函式的回傳形狀是「整理結果」、塞不進第二種語意；唯一的呼叫路徑
        // `normalizeIfRulesChanged` 會把它翻成既有的 needsConfirmation 形狀去問使用者，
        // 維護後門 `POST /api/statement/normalize-branches` 則照原味 409 回報（那條路沒有人可以問）。
        if (!backupBeforeIrreversible('pre-normalize') && !opts.proceedWithoutBackup) {
          throw Object.assign(new Error('這次沒存成還原檔（可能是磁碟空間不足），店名整理已停下、資料一個字都沒改'),
            { status: 409, code: 'backup_failed' });
        }
        await saveDb(db);
      }
    }
...
export function applyCategoryToStoreDb(db, storeKey, category, subcategory = '') {
  ...
  for (const o of origs) {
    ...
    if (o === key) continue;
    const oe = getOwn(lc, o);                        // 先確認是自有鍵再讀（原本 lc[o] 會先摸到原型）
 ...
 *（本函式上方原本有一份**重複且過期**的 JSDoc，寫著回傳不是 Promise——C4a async 化後就錯了，順手刪。）
 *
 * **停下來問使用者**有兩種原因，共用同一個 `needsConfirmation` 欄位：
 *   `true`＝整理會蓋掉學過的分類／自訂名（既有，Codex r4#2）；
 *   `'backup_failed'`＝這次的 `pre-normalize` 還原檔沒存成（William 2026-08-06 裁決）。
 * ⚠️ 刻意共用一個欄位而不是各開一個：前端只要看「這個欄位是不是 truthy」就知道要停下來問，
 * 將來多一種原因也**不會被靜靜忽略**（各開欄位的話，前端不認得的新原因會直接掉進「沒事發生」）。
 * 兩種都**不記指紋**＝維持待決，下次開 app 會再問一次。
 *
 * @param {boolean} [force] 略過「不可逆效果」的確認閘門（使用者已在 UI 預覽並確認時才傳 true）
 * @param {{proceedWithoutBackup?: boolean}} [opts] 使用者已看過「這次沒存成還原檔」的確認、選擇繼續。
 *   **與 `force` 是兩件事**：那個放行的是學習表衝突，這個放行的是沒有安全網；合成一個旗標的話，
 *   使用者按掉其中一個問題就等於連另一個也一起答應了。
 * @returns {Promise<{ran:boolean, rulesHash:string, needsConfirmation?:boolean|'backup_failed', changed?:number,
 ...
 */
export async function normalizeIfRulesChanged(force = false, opts = {}) {
  ...
  let r;
  try {
    r = await normalizeBranches(false, opts);
  } catch (e) {
    // 備份沒做成＝翻成「停下來問」，走與學習表衝突**完全相同**的那條路：不記指紋、維持待決。
    // 其他錯誤原樣往上（不可以把真的壞掉靜靜吞成「需要確認」）。
    if (/** @type {any} */ (e)?.code !== 'backup_failed') throw e;
    return { ran: false, needsConfirmation: 'backup_failed', rulesHash: hash,
      changed: preview.changed, keyChanged: preview.keyChanged };
  }
