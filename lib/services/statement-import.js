...
import { isRealMonth, isRealDate } from '../schema.js';
...
const conformTxs = (/** @type {Db} */ db, /** @type {any[]} */ txs) =>
  txs.map(t => {
    ...
    const store = t.storeCustom ? t.store : applyDisplayLabels(t.store, { desc: t.desc, subcategory: s });
...
export function importRows(cardId, rows, statementMonth = '', statementDue = null) {
  ...
  for (const r of list) {
    ...
    if (!r.date || !isRealDate(String(r.date)) || !(amount > 0) || amount > 1e8 || !r.stmtRef || !desc || desc.includes('|')) { skipped++; continue; }
    ...
    const note = custom || applyDisplayLabels(cleanStore(desc), { desc, subcategory });
...
export function normalizeBranches(dryRun = false) {
  ...
  for (const t of db.transactions || []) {
    ...
    const afterNote = customName ? normalizeStoreDisplay(customName)
      : applyDisplayLabels(orig ? auto0 : normalizeStoreDisplay(stripDisplayLabels(beforeNote)),
        { desc: orig, subcategory: t.subcategory });
  ...
  const learnedNameChanges = [];
  {
    ...
    for (const [k, v] of Object.entries(newLc)) {
      ...
      const nv = normalizeStoreDisplay(v.name);   // 不 stripDisplayLabels：顯示標記逐字保留，只套品牌改寫規則
      if (nv && nv !== v.name) {
...
export function applyCategoryToStoreDb(db, storeKey, category, subcategory = '') {
  ...
  for (const t of targets) {
    ...
    t.note = custom || applyDisplayLabels(cleanStore(o), { desc: o, subcategory: s });
  }
  const lc = (db.learnedCategories ||= {});
 ...
 */
export function renameStoreDisplay(orig, name, category, subcategory, reset = false, clearBrand = false, applyAll = false) {
  const db = getDb();
  ...
  let applied = null;
  if (applyAll && !reset && hasCat && newCat) applied = applyCategoryToStoreDb(db, storeKeyOf(o), newCat, newSub);
  saveDb(db);
