...
import { normalizeAiCard, assertAiCardGrounded, reconcileAiCard } from '../ai-parse-card.js';   // 批二：卡片 AI 的驗收與驗算（純模組）
import { issueAiTicket, redeemAiTicket } from '../ai-confirm-ticket.js';   // 批二：AI 預覽確認票（換卡重預覽不重跑模型）
 ...
 */
async function aiCardRoute(bytes, password, db, opts) {
  ...
  for (const model of [engine.models.primary, engine.models.escalation]) {
    ...
    try { reconcileAiCard(parsed); return { parsed, aiModel: model }; }
...
function aiCardToParsed(/** @type {ReturnType<typeof normalizeAiCard>} */ a) {
  return { bank: '', bankEvidence: /** @type {const} */ ('none'),
    transactions: finalize(a.transactions, '').transactions,
...
export async function previewAuto(b64, password, opts = {}) {
  ...
  let parsed = null;
  /** @type {{aiModel:string, aiIssuer:string, aiTicket:string}|null} */ let ai = null;
  try { parsed = await parseWithPool((b, pw) => parseStatement(b, pw), bytes, statementPasswordPool(db, password ? [String(password)] : [])); }
  catch (e) {
    // 批二：內建範本認不得＋呼叫端**明送 useAi:true** ⇒ 走 AI 救援；旗標缺席＝原錯誤原句丟回、零 AI 呼叫。
    // ⚠️ 只救 card_unrecognized：密碼錯（pdf_password）換模型救不了，照舊丟回讓前端跳密碼窗。
    if (opts.useAi === true && /** @type {any} */ (e)?.code === 'card_unrecognized') {
      ...
      const aiTicket = issueAiTicket({ parsed, aiModel: r.aiModel, aiIssuer: r.parsed.issuer, aiCalls: opts.aiBudget?.used?.() ?? 0 });
      ai = { aiModel: r.aiModel, aiIssuer: r.parsed.issuer, aiTicket };
  ...
  const base = { bank: parsed.bank, bankEvidence: parsed.bankEvidence || 'none', lastFour: parsed.lastFour || null,
    ...
    ...(ai ? { engine: /** @type {const} */ ('ai'), aiModel: ai.aiModel, aiIssuer: ai.aiIssuer, aiTicket: ai.aiTicket } : {}),
...
export async function previewForCard(cardId, b64, password, parse = parseStatement, opts = /** @type {{aiTicket?:string}} */ ({})) {
  ...
  if (opts.aiTicket) {
    ...
    return { bank: parsed.bank, bankEvidence: parsed.bankEvidence || 'none', lastFour: parsed.lastFour || null,
      ...
      engine: /** @type {const} */ ('ai'), aiModel: ticket.aiModel, aiIssuer: ticket.aiIssuer,
      aiTicket: issueAiTicket({ parsed, aiModel: ticket.aiModel, aiIssuer: ticket.aiIssuer, aiCalls: ticket.aiCalls || 0 }),
      resolvedCard: { id: card.id, name: card.name, lastFour: card.lastFour || null },
      transactions: previewRowsForCard(db, card.id, parsed.transactions) };
  }
  const bytes = decode(String(b64 || ''));
  let parsed;
