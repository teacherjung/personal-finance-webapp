...
// ⚠️ **空機構名必須回 false**（2026-08-27）：`String(x).includes('')` 恆真 ⇒ 解析器判不出是哪一家時
//    （`bank: ''`，見 lib/card-identity.js）**每一張信用卡**都會算成「這家的卡」，於是「該銀行只有
//    一張卡就自動歸」那條會把別家帳單自動歸到使用者唯一的那張卡上——錢記到錯的卡，而且沒有任何提示。
// ⚠️ 匯出只為了讓考題直接打這個判準（它是「錢會不會記到別張卡」的守門），不是給別的模組用。
export const issuerMatchesBank = (/** @type {any} */ issuer, /** @type {string} */ bank) => (bank ? String(issuer || '').includes(bank) : false);
...
export async function previewAuto(b64, password) {
  ...
  if (!resolved && !candidates.length) {
    // ⚠️ **認不出是哪一家就不准自動歸卡**：`bank: ''` 時 `bankCards` 是空的（issuerMatchesBank 守門），
    //    所以會落到候選清單讓使用者自己選。這一步刻意**不**因為「反正只有一張卡」就幫他決定——
    //    別家帳單被讀出來的列本來就可能是垃圾，自動歸卡等於把垃圾直接記到他的卡上。
    if (parsed.bank && bankCards.length === 1) resolved = bankCards[0];
