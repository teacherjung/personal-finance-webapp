...
export function importRows(cardId, rows, statementMonth = '') {
  const db = getDb();
  const card = (db.cards || []).find(c => c.id === cardId);
  if (!card) throw apiError(404, '找不到卡片');
  const stmtMonth = /^\d{4}-\d{2}$/.test(String(statementMonth || '')) ? String(statementMonth) : '';
...
export function listBatches() {
  ...
  for (const t of db.transactions || []) {
    ...
    const g = groups[t.importBatch] || (groups[t.importBatch] = {
      ...
      count: 0, amount: 0, minDate: t.date, maxDate: t.date, stmtMonth: ''
    });
...
export function normalizeBranches(dryRun = false) {
  ...
  if (!dryRun) {
    ...
    // 沒有任何變動就不寫檔：自動整理每次開 app 都可能跑，空跑也 saveDb 會一直洗掉 .bak 這個救命備份
    if (changes.length || keyChanged || learnedRemapped || learnedNamesFixed) saveDb(db);
  }
  return { changed: changes.length, keyChanged, learnedRemapped, learnedNamesFixed, changes: dryRun ? changes.slice(0, 200) : undefined };
}
...
// 有變動才寫檔、save 內建 .bak），結果回報給前端提示。指紋＝lib/statement.js 的內容雜湊——
// 規則散在資料表與 cleanStore 的 regex 兩處，雜湊整個檔案才不會漏（誤判只會多跑一次無害的整理）。
let rulesHashCache = '';
/** @returns {string} */
function rulesHash() {
  if (!rulesHashCache) {
    const src = readFileSync(new URL('../statement.js', import.meta.url), 'utf8');
    rulesHashCache = createHash('sha1').update(src).digest('hex').slice(0, 12);
  }
  return rulesHashCache;
}
/**
 ...
 */
export function normalizeIfRulesChanged() {
...
export function setBatchMonth(batchId, month) {
  ...
  if (!id) throw apiError(400, '缺少批次代號');
  if (mm && !/^\d{4}-\d{2}$/.test(mm)) throw apiError(400, '帳單年月格式須為 YYYY-MM（例：2026-01）');
