...
// 卡側末四碼安全字串化＝前後端共用單一實作（{toString:null} 炸彈、壞型別攤平；理由見該檔檔頭）
...
import { cardIssuerBank, cardCertainlyNot } from '../card-identity.js';
...
// 這張**卡片**的發卡行是不是「這份帳單的機構」。**判準本體在 lib/card-identity.js 的 cardIssuerBank**
// ——它先查機構代號（`card.issuerId`，2026-09-02 起），查不到才退回文字那條路
//（清單的名稱／別名 → 與「帳單是誰印的」共用的 `OWN_ISSUERS` 樣式）。
// ⚠️ **吃的是卡片物件、不是 `issuer` 字串**（2026-09-02 從 `issuerMatchesBank(issuer, bank)` 改名而來）：
//    改名是刻意的——代號與顯示字串分成兩欄之後，只傳 `c.issuer` 進來會**靜靜漏掉代號**、
//    回到升級前的判準，而這條路上是錢。改名讓沒跟上的呼叫端當場丟 `is not a function`。
// ⚠️ **同一個寫法被兩家以上宣稱就判不出身分**（今天＝「富邦」「富邦銀行」——香港富邦官方也自稱
//    「富邦銀行」）：那種卡落到「請使用者自己選卡」，要永久修好就在卡片編輯窗挑清單上的
//    「台北富邦銀行」或「富邦銀行（香港）」——挑完就有代號，從此不再走文字那條路。
//    猜錯的代價是錢記到別張卡，不猜的代價是多按一次。
...
//    「富邦銀行（香港）有限公司」對富邦帳單為真 ⇒ 台北富邦的帳單＋同末四碼的
//    香港富邦卡 ⇒ 直接自動歸到那張香港卡。`lib/bank-alias.js:17` 早就記著這個撞名危害。
// ⚠️ 匯出只為了讓考題直接打這個判準（它是「錢會不會記到別張卡」的守門），不是給別的模組用。
export const cardMatchesBank = (/** @type {any} */ card, /** @type {string} */ bank) => (bank ? cardIssuerBank(card) === bank : false);
...
export async function previewAuto(b64, password, opts = {}) {
  // 快照時刻＝**讀櫃（getDb）之前**（r8#1→r9#2 收乾淨）：疑似名單的「其後已自證」窗口以它為界，
  // 晚一步蓋＝db 舊快照＋新時間戳的縫隙裡完成的自證會被清洗。
  ...
  try { parsed = await parseWithPool((b, pw) => parseStatement(b, pw), bytes, statementPasswordPool(db, password ? [String(password)] : [])); }
    ...
    if (/** @type {any} */ (e)?.code === 'card_unrecognized') {
      const rec = await cardRecipeRoute(bytes, password, db);   // observedAt 已於 getDb 之前取（見函式開頭）
      if (rec.hit) {
        ...
        const aiTicket = issueAiTicket({ parsed, aiModel: '', aiIssuer: rec.hit.parsed.issuer, aiKind: 'card', snapshotAt: observedAt,
        ...
        const aiTicket = issueAiTicket({ parsed, aiModel: r.aiModel, aiIssuer: r.parsed.issuer, aiKind: 'card', snapshotAt: observedAt,
  ...
  //   **且其餘每一張卡都「確定是別家」（unsureCards 守門，見下）、且沒有末四碼衝突（那張卡
  //   登記著與帳單不同的一組＝退手選；守門見下）**→自動
  //（`cardMatchesBank` 空機構名回 false ⇒ 認不出機構時 bankCards 是空的、走不到②）；③否則回候選
  //   ——末四碼有命中但不能安全自動＝候選先沿用那些命中的卡；末四碼完全沒命中才進「確定同行∪不確定」。
  // ⚠️ ①②原本都是無條件的，兩條都會把交易寫到錯的卡——理由見下面各自的 ⚠️ 註解。
  const bankCards = cards.filter(c => cardMatchesBank(c, parsed.bank));
  ...
  //    擋自動歸並進候選。完整判準與代價＝`cardCertainlyNot`／`issuerCertainlyNot` 檔頭（William 2026-08-28 裁示
  //    「擋：不確定就問我」）。⚠️ **這道守門相對於「拿掉它」是單向的**（只把自動變成手選），
  //    但**整支代號改動不是**：代號讓更多卡被證明「確定是別家」⇒ 這裡剩下的更少 ⇒ 分支②更容易成立
  //    ⇒ 有些帳單從手選變成自動（J10 就是）。放寬的依據是使用者自己在清單上挑的那一項，不是猜的；
  //    完整說明＝`cardCertainlyNot` 檔頭「方向不是單向的」那一節。不要把這行讀成「陷阱不會發生」。
  const unsureCards = cards.filter(c => !bankCards.includes(c) && !cardCertainlyNot(c, parsed.bank));
  let resolved = null, candidates = [];
  if (parsed.lastFour) {
    const hit = cards.filter(c => cardLastFourText(c.lastFour) === String(parsed.lastFour));
  ...
  if (!resolved && !candidates.length) {
    ...
    // ⚠️ **末四碼衝突守門**（Grok 複審後掃 #520 2026-08-28 抓到；base 就有的缺口）：帳單自己印了
    //    末四碼、庫裡卻沒有任何卡對得上（走得到這裡＝上面的 `hit` 是空的），而唯一同行卡登記著
    //    **另一組**末四碼 ⇒ 帳單印的數字與卡片登記的數字明顯衝突，退手選（J5 同哲學：訊號打架
    //    不自動）。失敗劇本：新辦同行第二張卡、還沒建進 app 就上傳新卡帳單 ⇒ 新卡消費全記到舊卡。
    //    ⚠️ 這裡**刻意不比「不相等」**：走得到這裡且 `parsed.lastFour` 有值，這張卡的末四碼若相等
    //    早就進了上面的 `hit`（相等＋同行＝分支①自動）——多寫一個恆真的比較，就是上一段記過的
    //    「沒有任何考題分辨得出它在不在」的假保險。**「有登記」的判準＝缺欄／null／空字串以外
    //    都算**（Codex r1#1 實測：第一版用 truthiness，`lastFour` 存成 0／false 這類 falsy 壞值
    //    會被靜靜當成「沒登記」＝壞資料重新拿回自動歸卡；`cards.lastFour` 不在 FIELD_SCHEMA、
    //    櫃檯原樣放行非字串）。壞型別照 #520 既有裁定「壞型別的答案＝字串化的答案」：
    //    String(0)='0' 就是「另一組」＝退手選——方向單向（自動→請使用者選），可接受。
    //    ⚠️ 「有沒有登記」比對**精確空字串、不字串化**（Codex r2#1 實測：`String([])` 剛好是
    //    `''`，字串化判會把 `[]`／`['']` 這種陣列殼當成「沒登記」＝壞資料又溜回自動歸卡）；
    //    字串化只留在訊號比對（上面的 `hit`）——`['5678']` 攤平後與帳單相等仍走分支①。
    //    ⚠️ 同行卡**沒登記**末四碼＝證明不了衝突 ⇒ 照舊自動（判準取捨，理由攤開：①這道守門防的
    //    是「兩個數字互相打架」，沒登記＝只有單邊訊號、沒有東西可打架——與「帳單沒印末四碼」
    //    （J2b，從來都自動）同一個證據狀態；②反過來擋的代價＝一張沒登記末四碼的卡會讓那家銀行
    //    每期帳單都退手選＝把守門做成「整個不自動」；③新卡失敗劇本在「沒登記」這格本來就防不住
    //    ——帳單沒印末四碼時同樣防不住，擋了只擋到一半、卻天天付代價）。
    //    考題＝J9（衝突退手選）／J9b（沒登記照舊自動）／J2b（帳單沒印末四碼照舊自動）。
    const soleLastFour = bankCards[0]?.lastFour;   // 分支②只在單卡時成立，取第 0 張即可
    const lastFourRegistered = soleLastFour != null && soleLastFour !== '';
    if (bankCards.length === 1 && !unsureCards.length
      && !(parsed.lastFour && lastFourRegistered)) resolved = bankCards[0];
    else {
  ...
  if (resolved) {
    // 卡側 lastFour **只當顯示**、一律過 lastFourText（Codex r3#1 的同族：原樣塞進 JSON，
    // 炸彈只是搬到前端的模板字串上炸——同一份預覽換個地方死）。炸不出＝null。
    return { ...base, resolvedCard: { id: resolved.id, name: resolved.name, lastFour: cardLastFourText(resolved.lastFour) || null },
      transactions: previewRowsForCard(db, resolved.id, parsed.transactions) };
  }
  return { ...base, resolvedCard: null,
    candidates: candidates.map(c => ({ id: c.id, name: c.name, lastFour: cardLastFourText(c.lastFour) || null })) };
}
  ...
  if (opts.aiTicket) {
    ...
    try {
      ...
      return { bank: parsed.bank, bankEvidence: parsed.bankEvidence || 'none', lastFour: parsed.lastFour || null,
        ...
        resolvedCard: { id: card.id, name: card.name, lastFour: cardLastFourText(card.lastFour) || null },   // 只當顯示；同 previewAuto 的安全字串化
        transactions };
    } catch (e) {
