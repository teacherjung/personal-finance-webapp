...
import { parseStatement, normalizeStoreDisplay, isServiceFee, cleanStore, categorize, applyDisplayLabels } from '../statement.js';
...
export function normalizeBranches(dryRun = false) {
  ...
  for (const t of db.transactions || []) {
    ...
    const afterNote = applyDisplayLabels(normalizeStoreDisplay(beforeNote), { desc: orig, subcategory: t.subcategory });
  ...
  // 學習表 key 一併正規化（storeKey 級才需要，且已正規化→其實是 no-op）；正規化後若撞 key，保留先到的。
  ...
  if (!dryRun) {
    ...
    for (const [k, v] of Object.entries(lc)) {
      const nk = origSet.has(k) ? k : normalizeStoreDisplay(k);   // 是帳單原文（原文級 key）→ 原樣保留
...
export function renameStoreDisplay(orig, name, category, subcategory, reset = false) {
  ...
  if (!isServiceFee(o) && !isServiceFee(newName)) {   // 服務費不學（同 learning.js 檔頭說明）
    ...
    if (reset && Object.hasOwn(lc, autoName)) {   // autoName＝cleanStore(o)＝這個原文的 storeKey
      const sharedByOther = (db.transactions || []).some(t => {
        if (t.source !== 'stmt' || !t.stmtRef) return false;
        const parts = String(t.stmtRef).split('|');
        if (parts.length < 4) return false;
        const oo = parts.slice(3).join('|').trim();
        return oo !== o && cleanStore(oo) === autoName;
      });
      if (!sharedByOther) delete lc[autoName];
    }
  }
  saveDb(db);
