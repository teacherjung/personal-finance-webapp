...
import { parseStatement, normalizeStoreDisplay, isServiceFee, cleanStore, categorize } from '../statement.js';
...
export function renameStoreDisplay(orig, name, category, subcategory, reset = false) {
  ...
  // 自動判斷值：顯示名＝cleanStore；分類＝內建分類器再過別名/生效樹校正（與匯入同口徑）
  const autoName = cleanStore(o);
