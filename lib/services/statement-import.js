...
import { parseStatement, normalizeStoreDisplay, isServiceFee, cleanStore, storeKeyOf, storeKeyOfName, origFromStmtRef, categorize, applyDisplayLabels, stripDisplayLabels } from '../statement.js';
...
export function importRows(cardId, rows) {
  ...
  for (const r of list) {
    ...
    const storeKey = String(r.storeKey || storeKeyOf(String(r.desc || '')));   // 身分鑰匙（品牌層、不含分店與顯示標記）
...
export function normalizeBranches(dryRun = false) {
  ...
  for (const t of db.transactions || []) {
    ...
    /** @param {string} k */
    const hasName = (k) => Boolean(k && Object.hasOwn(lc0, k) && lc0[k]?.name);   // hasOwn：擋 constructor 這類原型鍵（Codex#7 同教訓）
    const hasCustomName = !orig || hasName(orig) || hasName(sk0) || hasName(oldKey);
    const base = hasCustomName ? normalizeStoreDisplay(stripDisplayLabels(beforeNote)) : auto0;
    ...
    // 舊 storeKey 級學過的顯示名，是「帶分店的名字」：搬到品牌層 key 會讓同品牌其他分店被連動改名
    // （＝12MINI 桃/新 連動 bug 的翻版）→ 記下來，稍後改掛到「原文級」這個正確的層。
    if (orig && oldKey && oldKey !== sk0 && hasName(oldKey)) nameByOrig.set(orig, String(lc0[oldKey].name));
    if (!dryRun && t.storeKey) {   // 內部 key 一併對齊（不列入 note 預覽）：有原文→用 storeKeyOf 重算（規則升級全套上）；沒有→就地正規化
      const newKey = (orig ? sk0 : storeKeyOfName(normalizeStoreDisplay(oldKey))) || oldKey;
      if (newKey !== oldKey) {
        if (!skMap.has(oldKey)) skMap.set(oldKey, newKey);
        t.storeKey = newKey;
      }
    }
  }
  ...
  if (!dryRun) {
    ...
    for (const [k, v] of Object.entries(lc)) {
      const nk = origSet.has(k) ? k : (skMap.get(k) ?? normalizeStoreDisplay(k));   // 是帳單原文（原文級 key）→ 原樣保留
      if (nk !== k) learnedRemapped++;
      const val = (nk !== k && v && typeof v === 'object' && 'name' in v)
        ? Object.fromEntries(Object.entries(v).filter(([f]) => f !== 'name'))   // 搬到品牌層時丟掉 name（改掛原文級，見下）
        : v;
      newLc[nk] = newLc[nk] || val;
    }
    ...
    for (const v of Object.values(newLc)) {
 ...
 */
export function renameStoreDisplay(orig, name, category, subcategory, reset = false) {
  const db = getDb();
  ...
  if (!isServiceFee(o) && !isServiceFee(newName)) {   // 服務費不學（同 learning.js 檔頭說明）
    ...
    if (reset && Object.hasOwn(lc, sk)) {
      const sharedByOther = (db.transactions || []).some(t => {
        if (t.source !== 'stmt' || !t.stmtRef) return false;
        const oo = origFromStmtRef(t.stmtRef);
        return Boolean(oo) && oo !== o && storeKeyOf(oo) === sk;
      });
      if (!sharedByOther) delete lc[sk];
    }
  }
  saveDb(db);
