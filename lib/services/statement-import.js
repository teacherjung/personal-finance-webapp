...
import { parseStatement, branchNormalize } from '../statement.js';
import { applyLearned, learnFromImport } from './learning.js';
import { effectiveTree, conform } from './categories.js';
/** @typedef {import('../types.js').Db} Db */

/** 把一批交易的 (category,subcategory) 校正到目前生效的分類樹（內建分類被使用者刪/改名時→其他/未分類）。 */
const conformTxs = (/** @type {Db} */ db, /** @type {any[]} */ txs) => {
  const tree = effectiveTree(db);
  return txs.map(t => { const [c, s] = conform(tree, t.category, t.subcategory); return { ...t, category: c, subcategory: s }; });
};
...
export function importRows(cardId, rows) {
  ...
  const tree = effectiveTree(db);   // 一次取樹，逐列校正分類到樹內合法值
  for (const r of list) {
    ...
    const [category, subcategory] = conform(tree, String(r.category || '生活'), String(r.subcategory || ''));
...
/**
 * 分店格式整理（一次性，使用者定 2026-07）：把既有交易的顯示說明（note）統一成「主體（分店）」，
 * 並對齊 storeKey 與學習表 key（future 匯入的 storeKey 會是正規化格式，舊 key 才對得上）。
...
export function normalizeBranches(dryRun = false) {
  ...
  for (const t of db.transactions || []) {
    ...
    const afterNote = branchNormalize(beforeNote);
    if (afterNote !== beforeNote) {
      changes.push({ id: t.id, before: beforeNote, after: afterNote });
      if (!dryRun) t.note = afterNote;
    }
    if (!dryRun && t.storeKey) t.storeKey = branchNormalize(String(t.storeKey));   // 內部 key 一併對齊（不列入 note 預覽）
  }
