...
import { applyLearned, learnFromImport, customStoreName } from './learning.js';
...
export function importRows(cardId, rows, statementMonth = '', statementDue = null) {
  ...
  for (const r of list) {
    ...
    // 顯示名一律從 db 權威重算（不信前端傳回的 r.store，同 storeKey，Codex#8）：
    // 使用者自訂名＝逐字（使用者定 2026-07-20，不貼標記）；沒自訂＝自動名（cleanStore＋上下文標記，冪等）。
    const custom = customStoreName(db, desc, storeKey);
    const note = custom || applyDisplayLabels(String(r.store || r.desc || ''), { desc: r.desc, subcategory });
