...
/** settings.rememberedStatementPasswords（JSON 字串）→ string[]；壞形狀＝[]（不炸匯入）。 @param {any} db */
...
/** 統一密碼池（去重、順序即嘗試序）。 @param {any} db @param {string[]} [extra] 本次輸入/指定卡密碼（最優先） */
export function statementPasswordPool(db, extra = []) {
  const cards = (db?.cards || []).filter((/** @type {any} */ c) => (c.type || 'credit') === 'credit');
  return [...new Set([...extra.filter(Boolean), '', ...cards.map((/** @type {any} */ c) => c.pdfPassword).filter(Boolean), ...rememberedPasswords(db)])];
}
...
export async function parseWithPool(parseFn, bytes, pool) {
  ...
  for (const pw of pool) {
    try { return await parseFn(bytes, pw); }
      ...
      lastErr = e;
      const pwErr = /** @type {any} */ (e)?.code === 'pdf_password' || /密碼|加密|Password/i.test(String(/** @type {any} */ (e)?.message || ''));
      if (!pwErr) break;
    }
  }
  throw lastErr;
...
export async function rememberStatementPassword(password) {
  ...
  if (pw.length > 100) throw apiError(400, '密碼長度異常，不像帳單密碼');
  const db = await getDb();
  const list = rememberedPasswords(db);
  if (!list.includes(pw)) {
    list.push(pw);
    while (list.length > 8) list.shift();   // 上限 8 組：防無限累積；最舊的先讓位
...
export async function previewForCard(cardId, b64) {
  ...
  const bytes = decode(String(b64 || ''));
  let parsed;
  // P0.5：指定卡密碼最優先，之後照統一池（''→各卡→記住的）——與 previewAuto 同一套嘗試序
  try { parsed = await parseWithPool((b, pw) => parseStatement(b, pw), bytes, statementPasswordPool(db, card.pdfPassword ? [card.pdfPassword] : [])); }
