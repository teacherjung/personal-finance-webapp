...
/** String() 但吞得住「連原始值轉換都炸」的值（Codex #541 r3#1：`{toString:null}` 這族——
 *  `cards.lastFour` 在 CRUD 白名單、FIELD_SCHEMA 沒有它的型別收斂，經櫃檯**原樣落庫**；
 *  對它 String() 會丟 TypeError ⇒ 一張壞卡炸掉整份帳單預覽、根本走不到守門）。
 *  炸不出字串＝回 null＝比不上任何訊號；「有沒有登記」的判準（!= null／!== ''）不做原始值
 *  轉換、天然不炸——這族值因此算「登記著壞值」＝衝突退手選（考題＝statement-pipeline 的 J9d）。
 *  @param {any} v */
function lastFourText(v) { try { return String(v); } catch { return null; } }
...
export async function previewAuto(b64, password, opts = {}) {
  ...
  if (parsed.lastFour) {
    const hit = cards.filter(c => lastFourText(c.lastFour) === String(parsed.lastFour));
  ...
  if (resolved) {
    ...
    return { ...base, resolvedCard: { id: resolved.id, name: resolved.name, lastFour: resolved.lastFour ? lastFourText(resolved.lastFour) : null },
      transactions: previewRowsForCard(db, resolved.id, parsed.transactions) };
  }
  return { ...base, resolvedCard: null,
    candidates: candidates.map(c => ({ id: c.id, name: c.name, lastFour: c.lastFour ? lastFourText(c.lastFour) : null })) };
}
  ...
  if (opts.aiTicket) {
    ...
    try {
      ...
      return { bank: parsed.bank, bankEvidence: parsed.bankEvidence || 'none', lastFour: parsed.lastFour || null,
        ...
        resolvedCard: { id: card.id, name: card.name, lastFour: card.lastFour ? lastFourText(card.lastFour) : null },   // 只當顯示；同 previewAuto 的安全字串化
        transactions };
    } catch (e) {
