 ...
 * ⚠️ **這條路刻意沒有「操作前自動備份」**（William 2026-08-08 裁決）：不產生 `pre-normalize.bak`、
 * 不擋、不問。救援手段＝每日滾動備份（30 天）與使用者自己按的「匯出備份」。
 *（順帶訂正一個更早的錯註解：`saveDb` 不碰任何 .bak；啟動備份 `.bak` 由 `store.js backupOnce`
 *  每個行程只寫一次，對「一天內整理好幾次」本來就沒有保護力。）
 ...
 *
 * **停下來問使用者**只有一種原因：`needsConfirmation:true`＝整理會蓋掉學過的分類／自訂名
 *（既有，Codex r4#2）。⚠️ 這個欄位刻意保持「truthy 就停下來問」的形狀：將來多一種原因時，
 * 前端不認得也**不會被靜靜忽略**（各開欄位的話，新原因會直接掉進「沒事發生」）。
 * 停下來時**不記指紋**＝維持待決，下次開 app 會再問一次。
 *（2026-08-06 曾有第二種原因 `'backup_failed'`＝操作前備份沒存成；William 2026-08-08 裁決
 *  整層拿掉操作前備份，那條路連同旗標一起移除——不是漏掉的，別順手補回來。）
 *
