...
function aiCardToParsed(/** @type {ReturnType<typeof normalizeAiCard>} */ a) {
  ...
  // 調整列**分開 finalize**（r1#2）：finalize 有「國外交易服務費緊接前一筆消費＝繼承其分類」的
  // 相鄰特例——那個前提對帳單層級的調整列不成立（尾接在消費列後面會把服務費歸進最後一家店的
  // 分類）。分開跑＝調整列只跟自己相鄰；仍走同一支 finalize（isPayment/店名鑰匙/分類同口徑）。
  const finTx = finalize(a.transactions, '').transactions;
  const finAdj = finalize(adjRows, '').transactions;
...
export async function previewAuto(b64, password, opts = {}) {
  ...
  try { parsed = await parseWithPool((b, pw) => parseStatement(b, pw), bytes, statementPasswordPool(db, password ? [String(password)] : [])); }
    ...
    if (opts.useAi === true && /** @type {any} */ (e)?.code === 'card_unrecognized') {
      ...
      const aiTicket = issueAiTicket({ parsed, aiModel: r.aiModel, aiIssuer: r.parsed.issuer, aiKind: 'card', aiCalls: opts.aiBudget?.used?.() ?? 0 });
  ...
  if (opts.aiTicket) {
    ...
    try {
      ...
      return { bank: parsed.bank, bankEvidence: parsed.bankEvidence || 'none', lastFour: parsed.lastFour || null,
        ...
        aiTicket: issueAiTicket({ parsed, aiModel: ticket.aiModel, aiIssuer: ticket.aiIssuer, aiKind: 'card', aiCalls: ticket.aiCalls || 0 }),
