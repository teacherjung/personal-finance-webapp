...
import { getDb, saveDb, backupNow, uid } from '../repo.js';
...
import { isRealMonth } from '../schema.js';
...
export function importRows(cardId, rows, statementMonth = '', statementDue = null) {
  const db = getDb();
  const card = (db.cards || []).find(c => c.id === cardId);
  if (!card) throw apiError(404, '找不到卡片');
  ...
  for (const r of list) {
    ...
    (db.transactions ||= []).push({
      ...
      stmtRef: r.stmtRef, source: 'stmt', importBatch: batchId, importedAt, autoCat, autoSub,
...
export function normalizeBranches(dryRun = false) {
  ...
  const learnedNameChanges = [];
  {
    ...
    for (const [k, v] of Object.entries(lc)) {
      ...
      const val = (isBrandKey && !isOrigKey && v && typeof v === 'object' && 'name' in v)
    ...
    if (!dryRun) {
      ...
      if (changes.length || keyChanged || learnedRemapped || learnedNamesFixed) {
        backupNow('pre-normalize');   // 動手之前先存一份（Codex r3#7）：整理會改掉學過的名字，有些還原不回來
        saveDb(db);
      }
    }
  }
  return { changed: changes.length, keyChanged, learnedRemapped, learnedNamesFixed,
    learnedConflicts: learnedConflicts.slice(0, 50),
    learnedNameChanges: learnedNameChanges.slice(0, 50),
 ...
 */
export function applyCategoryToStore(storeKey, category, subcategory = '') {
  const db = getDb();
  ...
  const lc0 = db.learnedCategories || {};   // 迴圈裡要查「這個原文有沒有自訂名」
  let changed = 0;
  ...
  for (const t of targets) {
    ...
    // 顯示名跟著分類走（使用者定 2026-07-19）：與單筆編輯同一套規則——顯示名有一部分取決於分類
    //（子類＝停車費 → 包成「停車費（店名）」）。這條路以前只改分類不重算 note，
    // 「同店一起改」之後那 N 筆的名字全部停在舊樣子（實測確認）。
    // 自訂名（原文級學習有 name）以自訂名為底重上標記，沒有就用自動名；兩者都冪等。
    if (!o) continue;
    const custom = Object.hasOwn(lc0, o) && typeof lc0[o]?.name === 'string' && lc0[o].name ? String(lc0[o].name) : '';
    const base = custom ? normalizeStoreDisplay(stripDisplayLabels(custom)) : cleanStore(o);
    t.note = applyDisplayLabels(base, { desc: o, subcategory: s });
  }
  const lc = (db.learnedCategories ||= {});
  const e = lc[key] || {};
  e.category = c; e.subcategory = s;
  delete e.name;                                     // 品牌層永不留顯示名（Codex#3）
  lc[key] = e;
  ...
  for (const o of origs) {
    const oe = lc[o];
    if (!oe || !Object.hasOwn(lc, o) || !oe.category) continue;
 ...
 * @returns {{ran: boolean, rulesHash: string, changed?: number, keyChanged?: number, learnedRemapped?: number, learnedNamesFixed?: number}}
 */
/**
 * 規則有更新才整理（開 app 時呼叫；同一版規則只跑一次）。
 * @param {boolean} [force] 略過「不可逆效果」的確認閘門（使用者已在 UI 預覽並確認時才傳 true）
 * @returns {{ran:boolean, rulesHash:string, needsConfirmation?:boolean, changed?:number,
 *   keyChanged?:number, learnedRemapped?:number, learnedNamesFixed?:number,
 *   learnedConflicts?:any[], learnedNameChanges?:any[]}}
 */
export function normalizeIfRulesChanged(force = false) {
  ...
  const preview = normalizeBranches(true);
  ...
  if (!force && (conflicts.length || nameChanges.length)) {
    return { ran: false, needsConfirmation: true, rulesHash: hash,
      ...
      learnedConflicts: conflicts, learnedNameChanges: nameChanges };
  }
 ...
 */
export function setBatchMonth(batchId, month) {
  const db = getDb();
  const id = String(batchId || '');
