...
import { parseStatement, normalizeStoreDisplay, isServiceFee, cleanStore } from '../statement.js';
...
/**
 * 改顯示名（店名對照表的編輯，使用者定 2026-07；2026-07-18 改以「帳單原文」為準）：
 * 只影響「這個原文」的帳單交易（同原文的所有月份整批改），並以原文為 key 記進學習表——
 * 銀行截斷的店名（FP-12MINI (桃… / (新…）會共用同一個 storeKey，若以 storeKey 為準會把
 * 不同分店連動改名（使用者實際踩到）；以原文為準才能各自取名。
 * 學習口徑：新名＝該原文的自動清理名 → 清除自訂（恢復自動）；否則記 name（未來匯入同原文沿用，
 * applyLearned 的原文級覆蓋優先於 storeKey 級）。服務費不學。
 * @param {string} orig 帳單原文（＝stmtRef 第 4 段） @param {string} name
 */
export function renameStoreDisplay(orig, name) {
  const db = getDb();
  ...
  if (!isServiceFee(o) && !isServiceFee(newName)) {   // 服務費不學（同 learning.js 檔頭說明）
    const lc = (db.learnedCategories ||= {});
    if (newName === cleanStore(o)) {                  // 改回自動名＝取消自訂
      if (lc[o]) { delete lc[o].name; if (!Object.keys(lc[o]).length) delete lc[o]; }
    } else {
      const e = lc[o] || {};
      e.name = newName;
      lc[o] = e;
