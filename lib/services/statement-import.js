  ...
  try { parsed = await parseWithPool((b, pw) => parseStatement(b, pw), bytes, statementPasswordPool(db, password ? [String(password)] : [])); }
    ...
    if (/** @type {any} */ (e)?.code === 'card_unrecognized') {
      ...
      if (rec.hit) {
        ...
        const aiTicket = issueAiTicket({ parsed, aiModel: '', aiIssuer: rec.hit.parsed.issuer, aiKind: 'card',
        ...
        const aiTicket = issueAiTicket({ parsed, aiModel: r.aiModel, aiIssuer: r.parsed.issuer, aiKind: 'card',
...
export async function importRows(cardId, rows, statementMonth = '', statementDue = null, opts = /** @type {{aiTicket?: string, aiEngineFactory?: (key: string) => any, aiBudget?: any}} */ ({})) {
  ...
  try {
    if (cardTicket) {
      ...
      if (cardTicket.recipeUse) recordRecipeApplied(db, { ...cardTicket.recipeUse, imported: r.imported });
      ...
      if (r.imported > 0 && Array.isArray(cardTicket.suspectRecipeIds) && cardTicket.suspectRecipeIds.length) {
        markRecipesSuspect(db, cardTicket.suspectRecipeIds, String(cardTicket.issuedAt || ''), 'card');
      }
    }
    await saveDb(db);
  } catch (e) {
...
export async function generateCardRecipeAfterImport(ticket, opts = {}) {
  try {
    ...
    const saved = await saveParseRecipe(/** @type {any} */ (candidate), { kind: 'card', ...(rebirthId ? { rebirthId, notAfter: String(ticket.issuedAt || '') } : {}) });
