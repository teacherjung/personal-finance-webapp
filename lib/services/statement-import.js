...
export function rethrowParseError(e) {
  ...
  // 2026-08-27 加入卡片版的兩個判準：`card_unrecognized`（版面認不得＝未來 AI 救援的唯一入口）與
  //   `card_no_rows`（認得版面但這期沒交易＝**不該**去燒 AI）。⚠️ 漏在這裡加＝上面那段註解記的洞重演：
  //   code 在重包時被丟掉，前端只會收到一個沒有判準的 400。
  if (err?.code === 'pdf_password' || err?.code === 'card_unrecognized' || err?.code === 'card_no_rows') {
