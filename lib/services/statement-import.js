...
import { issuerBank, issuerMaybeBank } from '../card-identity.js';
...
/** 對帳閘（P0，2026-08-11；判準在 statement-reconcile.js）：**擋下型只有 C1 摘要等式**（那行天生是平的，
 * 不平＝我們讀錯＝整份 400）；「明細加總 vs 摘要總額」（C2/C3）＝**影子檢查**，mismatch 只記 advisories
 * 不擋（r1#1：分期/年費只列摘要的合法版面天生對不上）。缺交叉欄（如官網 XLSX 只有本期帳單金額）＝C1
 * 開不了＝誠實降級弱閘、照舊放行。
...
export async function previewAuto(b64, password) {
  const db = await getDb();
  ...
  //   **且沒有任何一張卡「說不定也是這家」（maybeCards 守門，見下）**→自動
  //（`issuerMatchesBank` 空機構名回 false ⇒ 認不出機構時 bankCards 是空的、走不到②）；③否則回候選
  //   ——末四碼有命中但不能安全自動＝候選先沿用那些命中的卡；末四碼完全沒命中才進「確定同行∪可能同行」。
  // ⚠️ ①②原本都是無條件的，兩條都會把交易寫到錯的卡——理由見下面各自的 ⚠️ 註解。
  const bankCards = cards.filter(c => issuerMatchesBank(c.issuer, parsed.bank));
  // ⚠️ **歧義寫法的卡不可以靜靜出局**（工作流對抗驗證 2026-08-28；Codex #520 四輪都沒看到）：
  //    `bankCards` 是**族群過濾**，而下面分支②是**唯一性**判準（只剩一張就自動歸）。把一張
  //    「說不定就是這家」的卡移出族群 ⇒ 「兩張、有歧義、該問使用者」會靜靜變成「只剩一張、自動歸卡」。
  //    實測：兩張台北富邦卡（舊卡 issuer 是自由文字「富邦」、新卡是「台北富邦銀行」）＋末四碼讀不出來的
  //    台北富邦帳單 ⇒ base 問使用者、只做清單化會自動歸到新卡。**那等於把「不猜」換成在別處多猜一次。**
  //    所以只要有任何一張卡的發卡行寫法**有歧義、而歧義的其中一家正是這份帳單的機構**，就整個不自動歸。
  //    方向單向：它只會把「本來要自動」變成「請使用者選」，不會讓任何一張卡多拿到自動歸卡。
  const maybeCards = cards.filter(c => issuerMaybeBank(c.issuer, parsed.bank));
  ...
  if (parsed.lastFour) {
    ...
    // ⚠️ 末四碼這條路**不受歧義影響**：它比的是那張卡自己的末四碼，不是族群大小
    //    （歧義卡若同末四碼，`issuerMatchesBank` 對它回 false ⇒ 本來就只當候選）。
  }
  if (!resolved && !candidates.length) {
    ...
    // ⚠️ `maybeCards` 非空＝族群裡有「說不定是這家」的卡被上面的過濾擋在外面 ⇒ 不自動歸，
    //    並且把它們一起列進候選（使用者要看得到那張舊寫法的卡，才選得到它）。
    // ⚠️ 候選**照原卡片順序**取聯集，不可 bankCards 先、maybeCards 後（Codex #520 r5#1 實測）：
    //    前端的選卡 select 沒有空白提示項，第一項就是預設選中——串接順序會把歧義情境的預選
    //    從 base 的舊卡靜靜偏向正式寫法的新卡，等於在「請使用者選」的畫面上又多押了一次注。
    if (bankCards.length === 1 && !maybeCards.length) resolved = bankCards[0];
    else {
      const keep = new Set([...bankCards, ...maybeCards]);
...
export async function previewForCard(cardId, b64, password, parse = parseStatement) {
  const db = await getDb();
