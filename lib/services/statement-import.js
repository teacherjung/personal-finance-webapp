...
import { parseStatement, normalizeStoreDisplay, isServiceFee, cleanStore, categorize, applyDisplayLabels, stripDisplayLabels } from '../statement.js';
...
export function importRows(cardId, rows) {
  ...
  for (const r of list) {
    ...
    const storeKey = String(r.storeKey || cleanStore(String(r.desc || '')));   // 穩定 key（清理後原名、不含顯示標記）
...
export function normalizeBranches(dryRun = false) {
  ...
  for (const t of db.transactions || []) {
    ...
    // 手動記帳（無原文）只能就地整理。
    const sk0 = orig ? cleanStore(orig) : '';
    /** @param {string} k */
    const hasName = (k) => Boolean(k && Object.hasOwn(lc0, k) && lc0[k]?.name);   // hasOwn：擋 constructor 這類原型鍵（Codex#7 同教訓）
    const hasCustomName = !orig || hasName(orig) || hasName(sk0) || hasName(String(t.storeKey || ''));
    const base = hasCustomName ? normalizeStoreDisplay(stripDisplayLabels(beforeNote)) : sk0;
    ...
    if (!dryRun && t.storeKey) {   // 內部 key 一併對齊（不列入 note 預覽）：有原文→用 cleanStore 重算（規則升級全套上）；沒有→就地正規化
      const oldKey = String(t.storeKey);
      const newKey = (orig ? sk0 : normalizeStoreDisplay(oldKey)) || oldKey;
      if (newKey !== oldKey) {
  ...
  if (!dryRun) {
    ...
    for (const t of db.transactions || []) {
      ...
      const parts = String(t.stmtRef).split('|');
      if (parts.length >= 4) origSet.add(parts.slice(3).join('|').trim());
    }
...
/**
 * 編輯店名與分類（合併版「帳單店名／分類學習」卡的編輯，使用者定 2026-07-18）：
...
export function renameStoreDisplay(orig, name, category, subcategory, reset = false) {
  ...
  if (!isServiceFee(o) && !isServiceFee(newName)) {   // 服務費不學（同 learning.js 檔頭說明）
    ...
    // ⚠️ storeKey＝cleanStore(o)「不含」顯示標記；autoName 帶標記（#97 起兩者不同），不可拿 autoName 當 key 找。
    const sk = cleanStore(o);
    if (reset && Object.hasOwn(lc, sk)) {
      const sharedByOther = (db.transactions || []).some(t => {
        if (t.source !== 'stmt' || !t.stmtRef) return false;
        const parts = String(t.stmtRef).split('|');
        if (parts.length < 4) return false;
        const oo = parts.slice(3).join('|').trim();
        return oo !== o && cleanStore(oo) === sk;
      });
