...
export function rethrowParseError(e) {
  ...
  if (err?.code === 'pdf_password') /** @type {any} */ (wrapped).code = 'pdf_password';
  throw wrapped;
}
...
/** 密碼池 fail-safe 上限（P0.5 r1#2＝防 DoS）：無論值怎麼進來（記住端點／通用 PUT／匯入舊備份），
 * 讀取端一律硬限「最多 MAX_REMEMBERED 組、每組 ≤ MAX_PW_LEN 字」——池大小＝每次上傳的解析嘗試次數，
 * 不設上限＝一個超大 settings 值就能讓每次上傳跑上千次重型 PDF 解析（HOSTED 繞過請求級限速）。 */
export const MAX_REMEMBERED = 8;
export const MAX_PW_LEN = 100;
...
/** 統一密碼池（去重、順序即嘗試序）。extra（本次輸入/指定卡密碼）＝最優先、也套長度上限（r1#2）。 @param {any} db @param {string[]} [extra] */
export function statementPasswordPool(db, extra = []) {
  ...
  return [...new Set([...safeExtra, '', ...cards.map((/** @type {any} */ c) => c.pdfPassword).filter(Boolean), ...rememberedPasswords(db)])];
}
...
export async function previewForCard(cardId, b64, password) {
  const db = await getDb();
  ...
  try { parsed = await parseWithPool((b, pw) => parseStatement(b, pw), bytes, statementPasswordPool(db, extra)); }
