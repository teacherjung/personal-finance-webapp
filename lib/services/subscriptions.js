...
import { rolledNextCharge, chargeAnchorDay } from '../../public/modules/subscriptions-model.js';
 ...
 * @returns {Promise<{rolled: {id:string, name:string, from:string, to:string}[]}>}
 */
export async function rollDueSubscriptions(todayIso) {
  ...
  for (const sub of db.subscriptions || []) {
    ...
    // 錨點跟著存（Codex 複審 2026-07-26）：只留收月底後的結果，下個月再推就會從 28 起算、一路縮下去。
    // 這是服務層擁有的欄位（不進 CRUD 白名單）；使用者自己改日期時 chargeAnchorDay 會被判定失效、改用新號數。
    sub.chargeAnchorDay = chargeAnchorDay(sub);
    sub.nextCharge = next;
  }
  if (rolled.length) await saveDb(db);
  return { rolled };
}
