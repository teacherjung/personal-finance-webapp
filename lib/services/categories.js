...
/** 目前生效的內轉子分類清單 [{label, role?}]。 @param {any=} db */
export function effectiveTransferSubs(db = getDb()) {
 ...
 * @returns {{ok:true, subs:{label:string,role?:'out'|'in'|'settle'}[], changedTx:number}} */
export function saveTransferSubs(payload) {
  const db = getDb();
...
/**
 * 匯入用（收入版，stage 3 銀行自動分箱）：收入**無別名機制**（收入本是手動選，見 saveIncomeTree 註解）——
 * 只 conform 到目前生效的收入樹，被刪除的收入分類落「其他/其他收入」（不產生樹外孤兒）。改名的收入分類
 * 無法自動沿用（收入沒有 alias 表），但匯入是「預覽→使用者確認」、可當場改。
 * @param {any} db @param {string} cat @param {string} sub @returns {[string,string]}
 */
export function resolveImportIncome(db, cat, sub) {
  return conformIncome(effectiveIncomeTree(db), cat, sub);
}
