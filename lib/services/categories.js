...
import { sanitizeTransferSubs, DEFAULT_TRANSFER_SUBS } from '../schema.js';
...
export async function saveIncomeTree(payload) {
  ...
  for (const r of (Array.isArray(payload?.subRenames) ? payload.subRenames : []))
    if (isProtoKey(String(r?.to || '').trim())) throw reserved(String(r?.to || '').trim());
  // 動手之前先存一份（理由同 saveTree 的 pre-categories；UI「收入分類管理」同樣寫著「儲存前自動備份。」）：
  // 收入分類改名會連動改寫全庫的 income 交易、刪除會把那些交易改歸「其他／其他收入」，都還原不回來。
  // **tag 與支出樹刻意分開**：共用一顆的話，先改支出、再改收入時，第二次備份會蓋掉第一次的救命檔。
  // 擺在保留字檢查之後＝會 400 的請求不留無用備份（同 saveStoreRules 的順序）。
  // 考題：test/vault-and-backup-integrity.test.js 第二節。
...
export async function saveTransferSubs(payload) {
  ...
  for (const r of (Array.isArray(payload?.renames) ? payload.renames : []))
    if (isProtoKey(String(r?.to || '').trim())) throw reserved(String(r?.to || '').trim());
  const newSubs = sanitizeTransferSubs(payload?.subs);
...
export async function saveTree(payload) {
  ...
  for (const r of (Array.isArray(payload?.subRenames) ? payload.subRenames : []))
    if (isProtoKey(String(r?.to || '').trim())) throw reserved(String(r?.to || '').trim());
  // 動手之前先存一份（比照 saveStoreRules 的 `pre-rules`；docs/contracts/data-storage.md「不可逆整批操作前的
  // 真備份 backupNow」節明寫「新增其他不可逆的整批操作時，一併加一個 tag」）：**改名會改寫全庫交易的分類、
  // 刪除會把那些交易改歸「其他／未分類」**，兩者都還原不回來，而 UI（public/modules/settings.js「支出分類管理」）
  // 早就寫著「儲存前自動備份。」——2026-08-05 之前這句話在正式程式裡沒有對應的呼叫，是空頭支票（#410 r5 抓到）。
  // 擺在保留字檢查之後＝會 400 的請求不留無用備份（同 saveStoreRules 的順序）。
  // 考題：test/vault-and-backup-integrity.test.js 第二節。
  backupNow('pre-categories');
