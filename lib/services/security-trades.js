 ...
 * @property {string} [commissionCurrency] 手續費幣別（IB ibCommissionCurrency；與交易幣別相同時省略——不同幣別的費用不可混單位，Codex S2r1#3）
 * @property {{unknownType?:boolean, missingId?:boolean, missingAccount?:boolean, missingCurrency?:boolean, missingCore?:boolean}} flags
 */
...
export function normalizeIbTrade(raw) {
  ...
  // 幣別**不可猜**（Codex S2r1#1，高）：Currency 是 Flex 可個別勾選的欄位——缺席時預設 USD 會把 GBP 交易
  // 默默記成 USD（違反 AGENTS「絕不把非 USD 默默當 USD」）。缺＝flag，由同步端跳過＋指路補勾欄位。
  const cur = raw?.currency != null && String(raw.currency).trim() !== '' ? String(raw.currency).toUpperCase().trim() : '';
  ...
  return {
    ...
    // 手續費幣別（Codex S2r1#3）：IB 有獨立 ibCommissionCurrency（美元交易可收英鎊手續費）——與交易幣別
    // 不同才存；缺席＝與交易幣別相同（慣例）。不同幣別的費用在任何加總都不可混單位（S3 顯示要分開標）。
    ...(raw?.ibCommissionCurrency && String(raw.ibCommissionCurrency).toUpperCase().trim() !== cur
      ? { commissionCurrency: String(raw.ibCommissionCurrency).toUpperCase().trim() } : {}),
    sourceRef: ibSourceRef(raw),                   // 官方識別碼版（含帳戶指紋，Codex S2r1#4）；缺識別碼＝退路指紋
    flags: {
      ...(side === null ? { unknownType: true } : {}),
      ...(hasOfficialId(raw) ? {} : { missingId: true }),
      ...(acct ? {} : { missingAccount: true }),
      ...(cur ? {} : { missingCurrency: true }),   // Codex S2r1#1：缺幣別不可猜 USD
      // 核心金額（價/成交額/應收付）缺席＝不可入庫（與台新預覽 blocker 同一把尺；藍圖 §七）
      ...([absOrNull(raw?.tradePrice), absOrNull(raw?.tradeMoney) ?? absOrNull(raw?.proceeds), absOrNull(raw?.netCash)].some(v => v == null)
        ? { missingCore: true } : {}),
    },
  };
}
...
export function ibSourceRef(raw) {
  // 帳戶指紋入鍵（Codex S2r1#4、藍圖 §六優先序 2「帳戶身分＋官方識別碼」）：IB 文件**沒有**承諾
  // transactionID/tradeID 跨帳戶全域唯一——不含帳戶時，兩個帳戶的同號交易會互相覆蓋。
  // 舊格式（無指紋段）由 ib-sync 的一次性遷移升級（旗標＝段數）。
  const fp = accountFingerprint(String(raw?.accountId || ''));
  if (raw?.transactionID) return `ib|txn|${fp}|${raw.transactionID}`;
  if (raw?.tradeID) return `ib|trd|${fp}|${raw.tradeID}`;
  if (raw?.ibExecID) return `ib|exe|${fp}|${raw.ibExecID}`;
  const fpParts = [fp, ibDate(raw?.tradeDate) || '', String(raw?.symbol || '').toUpperCase(),
    String(raw?.buySell || '').toUpperCase(), Math.abs(Number(raw?.quantity) || 0), Number(raw?.tradePrice) || 0, Number(raw?.netCash) || 0].join('|');
  return `ib|fp|${fpParts}`;
}
