...
import { resolveFxTable } from '../../public/modules/fx-rates.js';   // 匯率表唯一實作（丙）
 ...
 * **支援的外幣匯率都留底**（Codex r3#10）：系統支援的外幣（＝`schema.js` 的 `CURRENCIES` 扣掉本幣 TWD），只存 usdTwd 的話，
...
export async function recordDailyValue(now = nowLocal()) {
  ...
  const a = computeAssets(db);
  const ib = computeIb(db);
  const fxTable = resolveFxTable(db.settings || {});   // 丙：記「實際採用」的匯率（上次抓到的或預設值），與 derive 同一份實作
  const row = {
    date: now.date,
    ...
    usdTwd: fxTable.rates.USD, gbpTwd: fxTable.rates.GBP, jpyTwd: fxTable.rates.JPY   // 實際採用的匯率（丙）；更早的列缺值寫 0 或 null，目前沒有程式讀，將來讀的人把 0／null 當「缺」
  };
