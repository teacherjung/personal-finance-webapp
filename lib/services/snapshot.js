...
export async function recordDailyValue(now = nowLocal()) {
  ...
  const fxOrNull = (/** @type {unknown} */ v) => { const n = Number(v); return Number.isFinite(n) && n > 0 ? n : null; };
  const row = {
    date: now.date,
    ...
    gbpTwd: fxOrNull(fx.GBP), jpyTwd: fxOrNull(fx.JPY)   // 缺匯率記 null（乙）；更早的列缺值寫 0。目前沒有程式讀這兩格，將來讀的人要把 0 與 null 都當「缺」
  };
