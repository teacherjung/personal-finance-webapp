...
function writeMonthlySnapshot(now) {
  const db = getDb();
 ...
 * @returns {import('../types.js').DailyValue|null} 剛寫入的今日這行；時鐘倒退時回 null（不寫）
 */
export function recordDailyValue(now = nowLocal()) {
  const db = getDb();
 ...
 * 會變成同一次開 app 寫兩遍全庫。
 * @returns {{recorded: boolean, snap: any, daily: import('../types.js').DailyValue|null, skipped?: string}}
...
export function takeSnapshotIfDue() {
  ...
  const daily = recordDailyValue(now);
  const existing = (getDb().snapshots || []).find(s => s.month === now.month);
  if (existing && existing.date === now.date) return { recorded: false, snap: existing, daily };
  return { recorded: true, snap: writeMonthlySnapshot(now), daily };
}
