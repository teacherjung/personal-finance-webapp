...
export async function getCape(opts = {}) {
  ...
  try {
    ...
    let manual = null;
    try { manual = getSettings()?.capeManual; } catch { /* store 壞掉：退回無手動值 */ }
...
export async function getRealYield(opts = {}) {
  ...
  try {
    ...
    let manual = null;
    try { manual = getSettings()?.signals?.realYieldManual; } catch { /* store 壞掉：不讓 fallback 拖垮程式 */ }
...
export async function refreshQuotesIfStale(opts = {}) {
  ...
  // ⚠️**await 之前只讀「發請求需要的」**（新鮮度判斷＋要抓哪些代號）；整包 db 等回應**之後**才 getDb() 再寫——
  //   Yahoo 要數秒，若拿 await 前的舊快照整包寫回，這段期間新增的交易/店名整理會被無聲覆蓋（Codex r13#1，高；
  ...
  const FX_SYMS = ['TWD=X', 'GBPTWD=X', 'JPYTWD=X'];   // Yahoo 匯率代號：美元/英鎊/日幣 兌台幣
  ...
  // await 完才取最新整包，只把匯率與對應持股價**合併**進去（不動其他欄位）。
  const db = getDb();
