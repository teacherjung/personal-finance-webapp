...
export async function refreshQuotesIfStale(opts = {}) {
  ...
  // 只有正數才寫回（與前端 portfolio-quotes 同口徑；Codex #556 r3）：壞值不可算成 NaN／null 蓋掉舊匯率
  const usd = positiveRate(quotes['TWD=X']?.price), gbp = positiveRate(quotes['GBPTWD=X']?.price), jpy = positiveRate(quotes['JPYTWD=X']?.price);
  if (usd) { settings.usdTwd = Math.round(usd * 1000) / 1000; gotAny = true; }
  const fxTwd = { ...(settings.fxTwd || {}) };
  if (gbp) { fxTwd.GBP = Math.round(gbp * 1000) / 1000; gotAny = true; }
  if (jpy) { fxTwd.JPY = Math.round(jpy * 10000) / 10000; gotAny = true; }
