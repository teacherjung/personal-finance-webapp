...
export function learnFromBankEdit(db, item, prev) {
  ...
  if (!String(item.note || '')) item.note = bankAutoNote(item, db);
}
...
/** 自己的現金帳戶主體名（末碼比對；名字去尾「（Richart）」這類括號）。帶「-」的他行代碼帳號不查
 *（824-…6146 是對方在他行的帳號，末碼撞到自己帳戶純屬巧合、不可誤標「轉入到：」）。
...
function ownAccountNameByAcct(db, acct) {
  ...
  return acc ? String(acc.name).replace(/（[^（）]*）\s*$/, '').trim() || null : null;
}
/**
 * 「摘要・備註」→ 好讀顯示名（純函式）。
