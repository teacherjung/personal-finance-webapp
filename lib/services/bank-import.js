...
/** 找這筆交易對應帳戶的幣別（先比帳單概要區的完整遮罩帳號、再比 db 登記帳號的前綴+末碼；找不到當 TWD）。
...
/** 用交易的帳戶找帳戶名（顯示用）：優先比**完整遮罩帳號的前綴+末碼**（免同末碼誤抓房貸帳戶名），
 * 退回末碼比對。找不到回「台新 末XXXX」。 @param {any} db @param {import('../bank-statement.js').BankTx} tx @param {ParsedBank} parsed */
...
export function previewBankTxForDb(db, parsed) {
  ...
  const rows = (parsed.transactions || []).map(tx => {
    ...
    const cls = resolveCls(db, classifyBankTx(tx, own));
    const bankRef = bankRefWithOcc(tx, occ);
    const duplicate = existing.has(bankRef) || seen.has(bankRef);
    seen.add(bankRef);
    return {
      date: tx.date, account: accountNameForTx(db, tx, parsed),
      summary: tx.summary, note: tx.note, amount: tx.amount, direction: tx.direction, currency, foreign,
