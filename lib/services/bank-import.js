...
function maskedParts(masked) {
  ...
  //   登記帳號與它「對不對得上」不在這裡判——那是 prefixFits／acctShapesCompatible 的事（完整對完整＝整串全等）。
...
/** 遮罩形能不能**逐位覆蓋**一個完整號：寬度相同、可見數字逐位相同、星號段剛好蓋住藏掉的位數
 * （`900100****3301` 蓋得住 `90010011223301`、蓋不住 16 碼的 `9001001122333301`——四顆星只可能代表四個字元，
 * 寬度不同就不可能是同一顆戶；Codex #504 r1#1）。分隔符不算寬度。 @param {string} masked @param {string} full */
function maskCoversFull(masked, full) {
  const m = String(masked || '').replace(/＊/g, '*').replace(/[^\d*]/g, '');
  const f = digitsOf(full);
  if (!m.includes('*') || !f || m.length !== f.length) return false;
  for (let i = 0; i < m.length; i++) if (m[i] !== '*' && m[i] !== f[i]) return false;
  return true;
}
/** 登記帳號與帳單帳號「對得上」（末碼由呼叫端先比；帳單沒前綴可比＝不擋）。
 * 帳單是**遮罩**：登記的數字以帳單可見前綴開頭（既有判準，一個位元組不動）。
 * 帳單印**完整號**（AI 路線、帳單本來就沒遮）：登記的也是完整號＝**整串全等**；登記的是**遮罩形**（自動建戶存的
 *   `900100****3301`）＝遮罩要**逐位覆蓋**完整號（`maskCoversFull`）——不然同一顆戶只因為帳單換了印法就裂戶。
 *   ⚠️ 只放給**帳號本身帶星號**的登記戶；使用者親手填的完整帳號一律整串比。
 * @param {string} accountNo 登記帳號 @param {string} stmtAcct 帳單帳號（遮罩形、完整號、或 `x****末碼` 佔位） */
function prefixFits(accountNo, stmtAcct) {
  const { prefix } = maskedParts(stmtAcct);
  if (!prefix) return true;
  if (hasStar(stmtAcct)) return digitsOf(accountNo).startsWith(prefix);
  const reg = normalizeMaskShape(accountNo);   // 使用者親手填的遮罩可能用 X（`900100XXXX3301`）＝同一把改寫再比
  return hasStar(reg) ? maskCoversFull(reg, stmtAcct) : digitsOf(reg) === digitsOf(stmtAcct);
}
...
function matchAccount(existing, pa, bank) {
  ...
  const strictHit = cands.find(a => prefixFits(a.accountNo, pa.masked));
  if (strictHit) return { hit: strictHit, ambiguous: false };
...
function txCurrency(db, parsed, tx) {
  ...
  const acc = (db.accounts || []).find(a => {
    ...
    return prefixFits(a.accountNo, stmtAcct);
  });
...
function ownSuffixSet(db, parsed) {
  const set = new Set();
  // 完整號（AI 路線）的末碼是程式取的末四碼；備註若照銀行的遮罩印法只印末三碼（`****162`），末三碼也要算自己的
  const addTails = (/** @type {string} */ s, /** @type {string} */ masked) => { if (!s) return; set.add(s); if (!hasStar(masked) && s.length === 4) set.add(s.slice(-3)); };
  for (const pa of parsed.accounts || []) addTails(pa.suffix, pa.masked);
  for (const masked of Object.keys(parsed.accountCurrency || {})) addTails(accountSuffix(masked), masked);
...
function accountNameForTx(db, tx, parsed) {   // 測試出口在檔尾（accountNameForTxForTest）——掛名繞開定存戶的行為要能直測
  ...
  // ⚠️ 帳單印**完整號**時，「退回只末碼」只准退到**與完整號不衝突**的登記戶（登記的是它的尾段＝只填了末幾碼，
  //   或登記的以它結尾＝前面多了銀行代碼）；登記了另一個完整號、只是末碼相同的戶**不可**被退路撿到
  //   （Codex #504 r1#1：A 戶 90010011223301 在庫、B 戶 90020011223301 的交易被掛成 A）。遮罩帳單的退路照舊。
  const fullNo = (!hasStar(stmtAcct) && /^\d{5,}$/.test(digitsOf(stmtAcct))) ? digitsOf(stmtAcct) : '';
  const fallbackOk = (/** @type {any} */ a) => !fullNo || fullNo.endsWith(digits(a)) || digits(a).endsWith(fullNo);
  const acc = (fullStar && bySuffix.length > 1) ? null
    : (bySuffix.find(a => prefixFits(a.accountNo, stmtAcct)) || bySuffix.find(fallbackOk) || null);   // 先前綴+末碼、退回只末碼
...
/** 兩個帳號印法「可能是同一顆戶」（疑似重複的前綴否決用）。
 * 末碼：兩邊都看得到＝一邊要是另一邊的尾段（`****162` 對 `…3162` 算、`****2301` 對 `…3301` 不算）。
 * 前綴：一邊看不到＝不否決（跨版式印法本來就不同）；兩邊都是遮罩＝可見前綴全等（既有判準）；兩邊都是完整號＝整串全等；
 * 一遮一全＝完整號以遮罩的可見前綴開頭、以可見末碼結尾——**刻意不驗寬度**（預審 #504：`900300****162` 的星號數
 * 與藏掉的位數不同＝同一顆戶的完整號印法會被判成另一顆＝同一份帳單匯兩次、一聲不吭；疑似重複是**提醒**、畫面看得見，
 * 寬一分的代價是多問一句、窄一分的代價是錢算兩次——與餘額身分那把（`maskCoversFull` 驗寬度）刻意不同）。
 * ⚠️ 但兩個**不同完整號**仍要否決（Codex #504 r1#2：預設跳過那幾筆，互報＝真交易少匯一筆）。
 * @param {string} a @param {string} b */
function acctShapesCompatible(a, b) {
  const ta = idxSuffix(a), tb = idxSuffix(b);
  if (ta && tb && !ta.endsWith(tb) && !tb.endsWith(ta)) return false;
  ...
  const [m, f] = sa ? [a, b] : [b, a];
  const fd = digitsOf(f);
  return fd.startsWith(acctPrefix(m)) && fd.endsWith(idxSuffix(m));
}
...
function similarTxIndex(db, wanted) {
  ...
  for (const t of db.transactions || []) {
    ...
    const key = `${instKey(inst)}|${suffix.slice(-3)}|${t.date}|${dir}|${Number(t.amount) || 0}`;   // 鍵用末三碼（印法不同時可見末碼長度不同：`****162` vs 完整號的 3162）、精度交給 acctShapesCompatible
...
function similarKey(bank, suffix, tx) {
  return `${instKey(bank)}|${String(suffix).slice(-3)}|${tx.date}|${tx.direction}|${Number(tx.amount) || 0}`;
}
