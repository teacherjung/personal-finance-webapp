 ...
 * ②帳單印完整號：登記完整號＝比對形**全等**（`AB12345678` 不等於 `12345678`）；登記遮罩形＝可見頭尾都對上且至少藏一碼（`headTailMatch`）＝hit、其餘 no。
 * ③帳單是遮罩（有可見前綴）、登記完整號＝帳單遮罩對登記號 `headTailMatch`＝hit、其餘 no——「藏幾碼＝幾顆星」不是證據，所以沒有「寬度不同＝ambiguous」這一格。
...
function accountFit(accountNo, stmtAcct) {
  ...
  if (!S) return (R ? headTailMatch(Rc, Sc) : Rc === Sc) ? 'hit' : 'no';
...
function matchAccount(existing, pa, bank) {
  ...
  //   但可見頭尾都對得上＝同一顆戶（星號數本來就不等於藏掉的位數）——一起交給裁決器，不可因末碼長度不同就新建第二顆
  //   （r2#2：兩顆餘額都進資產＝多算一份）。
...
function accountNameForTx(db, tx, parsed) {   // 測試出口在檔尾（accountNameForTxForTest）——掛名繞開定存戶的行為要能直測
  ...
  // 退路（使用者登記的 accountNo 未必以遮罩露出的前綴開頭：可能只填末幾碼、或前面多了銀行代碼）：**恰一顆**同末碼候選、
  //   它自己沒有遮罩、而且它的數字是帳單可見數字的尾段（只填了末幾碼）或以帳單完整號結尾（銀行代碼前置）才准；
  //   登記了另一個號、只是末碼相同的戶不可被撿（r1#1／r2#1）。
  const visible = digitsOf(stmtAcct);
  const fullNo = !hasStar(stmtAcct) && /^\d{5,}$/.test(visible);   // 帳單印完整號＝登記的以它結尾（前面多了銀行代碼）也算一致
  const partialOk = (/** @type {any} */ a) => !hasStar(cleanAcct(a.accountNo)) && (visible.endsWith(digits(a)) || (fullNo && digits(a).endsWith(visible)));
...
function refSuffixReadings(masked) {
  ...
  if (!/^\d{3,}$/.test(m)) return [];
  const out = [{ suffix: m, shape: `*${m}` }];
  if (m.length >= 5) out.push({ suffix: m.slice(-4), shape: m });
  return out;
}
/** 兩個帳號印法「可能是同一顆戶」（疑似重複的前綴否決用）：一邊看不到前綴＝不否決（跨版式印法本來就不同）；
 * 兩邊都是遮罩＝可見前綴全等（既有判準）；兩邊都是完整號＝整串全等；一遮一全＝可見頭尾都對上且至少藏一碼（`headTailMatch`，
 * 與身分那把同一把尺）。
...
function acctShapesCompatible(a, b) {
  ...
  return sa ? headTailMatch(a, b) : headTailMatch(b, a);
}
