...
function matchAccount(existing, pa, bank) {
  ...
  if (prefix) {
    if (cands.length === 1 && cands[0].accountNoSuffixOnly === true) return { hit: cands[0], ambiguous: false };
    if (cands.some(a => a.accountNoSuffixOnly === true)) return { hit: null, ambiguous: true };   // 標記戶＋別的同末碼戶並存＝分不出，停手
  }
