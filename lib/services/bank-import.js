...
/** 遮罩形與完整號「頭尾都對得上」：完整號以遮罩的可見前綴開頭、以可見末碼結尾（字母也算可見）。
 * ⚠️ **刻意不驗「藏幾碼＝幾顆星」**：星號數不等於藏掉的位數是常態（本 repo 既有考題就用 `900100****3301` 配 15 碼的
 *   `900100123453301`），「四顆星＝四個字元」不是證據；但**至少要藏一碼**。可見段全對＝這種印法能給的全部證明（與兩份遮罩帳單互比同一個強度）。
 * @param {string} masked @param {string} full */
function headTailMatch(masked, full) {
  const seg = maskSegments(masked);
  const f = cleanAcct(full);
  if (!seg || !seg.head || !f || hasStar(f)) return false;
  // 星號至少代表**有東西被藏起來**：完整號的長度必須比可見頭＋尾更長（`900100****3301` 對 10 碼的 `9001003301`＝不是；
  //   Codex #504 r4#1）；不要求藏幾碼＝幾顆星。
  return f.length > seg.head.length + seg.tail.length && f.startsWith(seg.head) && f.endsWith(seg.tail);
}
 ...
 * ②帳單印完整號：登記完整號＝比對形**全等**（`AB12345678` 不等於 `12345678`）；登記**遮罩形**＝可見頭尾都對上且至少藏一碼
 *   （`headTailMatch`）＝**ambiguous**、其餘 no——登記的遮罩是自動建戶抄來的、證明不了它就是這個完整號（`900100****3301` 可能是
 *   `90010011113301` 也可能是 `90010099993301`；Codex #504 r5#1）；停手請使用者到資產頁把帳號補成完整號，之後完整對完整全等就命中。
 * ③帳單是遮罩（有可見前綴）、登記**完整號**＝帳單遮罩對登記號 `headTailMatch`＝hit、其餘 no——登記的完整號是使用者給的身分，
 *   帳單遮罩與之一致就是它（既有行為）；「藏幾碼＝幾顆星」不是證據，所以沒有「寬度不同＝ambiguous」這一格。
 * ④兩邊都是遮罩：登記的看不到前綴（全星號登記戶）＝no（走 Stage 1 的標記戶寬鬆徑或新建，既有行為）；比對形**全等**＝hit；
 *   頭尾相容（一邊的可見前綴是另一邊的開頭、可見末碼是另一邊的結尾）＝ambiguous；其餘 no。
...
function accountFit(accountNo, stmtAcct) {
  ...
  if (!S) {
    ...
    return headTailMatch(Rc, Sc) ? 'ambiguous' : 'no';
  }
  if (!R) return headTailMatch(Sc, Rc) ? 'hit' : 'no';
  if (!R.head) return 'no';
  if (Rc === Sc) return 'hit';
  const headsOk = R.head.startsWith(S.head) || S.head.startsWith(R.head);
  const tailsOk = R.tail.endsWith(S.tail) || S.tail.endsWith(R.tail);
  return (headsOk && tailsOk) ? 'ambiguous' : 'no';
}
...
function matchAccount(existing, pa, bank) {
  ...
  const strict = resolveCandidates([...cands, ...shortTailCands], pa.masked);
 ...
 * 兩邊都是遮罩＝可見前綴全等（既有判準）；兩邊都是完整號＝整串全等；一遮一全＝只在**既有列是完整號、新列是遮罩**時比可見頭尾
 * （`headTailMatch`，與身分那把同一把尺、同一個方向）；既有列是遮罩、新列是完整號＝證明不了＝不算。
...
function acctShapesCompatible(a, b) {
  ...
  // 一遮一全＝與身分那把同一個方向性：**既有列是完整號**（身分已知）、新列的遮罩與之一致＝算；**既有列是遮罩**、新列是完整號
  //   ＝證明不了（`900100****3301` 可能是另一顆 `…9999…`）＝不算——算了會被預設跳過、吃掉另一顆戶的真交易（Codex #504 r5#1）。
  return sa ? false : headTailMatch(b, a);
}
