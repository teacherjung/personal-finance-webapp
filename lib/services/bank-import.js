 ...
 * @returns {Promise<{hit: {parsed: ParsedBankFull, reconcile: any, recipeId: string, usedVersion: 'current'|'previous', usedRecipe: object}|null, gateFailedIds: string[]}>}
 */
  ...
  for (const row of rows) {
    ...
    for (const usedVersion of /** @type {const} */ (['current', 'previous'])) {
      ...
      try {
        ...
        return { hit: { parsed, reconcile, recipeId: String(row.id), usedVersion, usedRecipe: JSON.parse(JSON.stringify(recipe)) }, gateFailedIds };
 ...
 * @param {any} db @param {{id:string, usedVersion:'current'|'previous', usedRecipe:object}} use
 */
    ...
    if (/** @type {any} */ (e)?.code !== 'pdf_password') {
      ...
      if (rec.hit) {
        ...
        const aiTicket = issueAiTicket({ parsed: rec.hit.parsed, aiModel: '',
          recipeUse: { id: rec.hit.recipeId, usedVersion: rec.hit.usedVersion, usedRecipe: rec.hit.usedRecipe },
...
export async function applyBankStatement(b64, password, parse = parseBankStatement, opts = {}) {
  ...
  /** @type {{id:string, usedVersion:'current'|'previous', usedRecipe:object}|null} */ let recipeUse = null;   // P2-2
  ...
  try {
    ...
    if (directGateFailedIds.length) markRecipesSuspect(db, directGateFailedIds, new Date().toISOString());
    await saveDb(db);
