...
export async function aiBankRoute(b64, password, db, seams = {}) {
  ...
  if (!arb.ok) {
    ...
    if (matches.length === 1) {
      ...
      // 的情境也照亮）。attested 沒有第二份合法答案＝無從比較＝不帶這個欄位（帶空陣列會被畫面唸成
      // 「差異：（空）」的怪話）。
      return { parsed: w.parsed, reconcile: w.reconcile, aiModel: w.model, lines, dualRead: valid.length === 2 ? /** @type {const} */ ('arbitrated') : /** @type {const} */ ('attested'), ...(tv.length ? { dualReadTextVariance: tv } : {}), ...(valid.length === 2 && first.diffs.length ? { dualReadDiffs: first.diffs } : {}) };
    }
  }
