...
export function learnFromBankEdit(db, item, prev) {
  ...
  if (noteChanged) {
    if (item.note) e.name = item.note;
    else { delete e.name; item.note = bankAutoNote(item); }   // 清空＝回復預設自動名（使用者定 2026-07-21：刪掉修改的說明→回自動生成）
  }
  setOwn(lb, bankKey, e);
}
...
/** 開 app 自動對齊帳戶名（比照 snapshot/auto、normalize-auto）：修正「帳戶改名後、既有交易顯示名沒跟上」的舊資料。
 * 僅在有變動時寫檔。 @returns {{changed:number}} */
