...
function aiEligible(e, opts) {
  ...
  return opts?.useAi === true && code !== 'pdf_password' && code !== 'bank_mixed_currency';
}
...
const AI_SERVICE_CODES = ['ai_auth', 'ai_unavailable', 'ai_truncated', 'ai_budget_exceeded'];   // 換模型救不了、照實丟的錯（預審 C1#1：預算錯不入名單＝仲裁那發被上限擋下時被吞成「三讀不一致」——白話說明與正確下一步（明天恢復／調上限）整句丟失，還誤指使用者去手動記帳）
