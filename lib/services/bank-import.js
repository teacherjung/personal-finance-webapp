 ...
 * 判定順序（先到先算）：
 * ①比對形**全等**＝hit（任何形狀）。
 * ②帳單是「只比末碼」的形（`****3301`／`x****3302` 佔位）＝語言有交集就 hit——除末碼外什麼都看不到、末碼對得上就算
 *   （既有語意；`9001****301` 的登記戶對 `****3301` 也算）；**多顆由裁決器停手**。多段的 `****22**3301` 不算這種，往下逐段比。
...
function accountFit(accountNo, stmtAcct) {
  ...
  if (Sc === Rc) return 'hit';
  if (suffixOnlyShape(Sc)) return acctPatternsIntersect(Sc, Rc) ? 'hit' : 'no';   // 只比末碼＝末碼那段要對得上（語言交集）
