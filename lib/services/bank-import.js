...
function importDebitCardLedgerToDb(db, parsed, /** @type {ReturnType<typeof debitCardPlan>} */ cardPlan, /** @type {string[]} */ refByIndex) {
  ...
  /** @type {Map<number, number>} */ const txOfA = new Map(pairs.map((p) => [p.card, p.tx]));
  /** @type {Map<number, string>} */ const batchOfA = new Map();
  for (const [aIdx, txIdx] of txOfA) {
    const ref = refByIndex[txIdx] || '';
    const owner = ref ? (db.transactions || []).find((/** @type {any} */ t) => t.source === 'bank' && canonRef(t.bankRef) === ref) : null;
    if (owner && owner.importBatch) batchOfA.set(aIdx, String(owner.importBatch));
  }
  const out = [];
  let imported = 0, skipped = 0, unresolved = 0;
  for (const c of plan.cards) {
    // fail-closed（理論角落）：配對的 D 列在庫裡找不到（例如被「疑似重複」跳過）＝這筆的生命週期沒有家，
    //   整筆（含服務費）不記、計入「對不上」——正常路徑到不了這裡（能匯的 D 列若有已分類的攣生列，
    //   debitCardPlan 的 cashflowCategorized 擋門早就把整組擋下）。
    ...
    //   剛寫進去的列（importBatch＝這次的卡片批次）與 kept 裡**非重複**的列逐一同序對應（importRowsToDb 依序寫入）。
    const written = (db.transactions || []).filter((/** @type {any} */ t) => t.importBatch === r.batchId && t.source === 'stmt');
    let j = 0;
    for (let k = 0; k < rows.length; k++) {
      if (/** @type {any} */ (rows[k]).duplicate) continue;
      if (written[j]) written[j].bankBatch = batchOfA.get(keptMeta[k].aIdx);
      j++;
    }
