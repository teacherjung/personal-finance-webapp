...
import { resolveImportCategory, resolveImportIncome, conformTransferSub } from './categories.js';
 ...
 * （剛學好的 type/分類；有自訂名才覆蓋 note、否則各自 note 保留）。內轉子分類（含交割/內轉出入）逐字套；分類過生效樹
 * 校正。比照信用卡 applyCategoryToStore：保留字/找不到目標＝明確 400/404。 @param {string} bankKey */
export function applyLearnedBankToExisting(bankKey) {
  ...
  const cls = resolveCls(db, { type: learned.type, category: learned.category || '', subcategory: learned.subcategory || '' });
