...
import { reconcileBankStatement, gateFailureMessage, statementCurrencyLookup } from '../statement-reconcile.js';
...
import { linesToText, normalizeAiBank } from '../ai-parse.js';
 ...
 *
 * ⚠️ 兩個設計取捨：
 * ①**比末碼、不比完整遮罩**：不同版面的遮罩印法可能不同（`900200****3302` vs `****3302`）。
 * ②**只提醒、不擋**（William 裁示）：同一天、同一個帳戶、同樣金額的兩筆真實交易是可能的
 *   （同一家店刷兩次、分兩次轉同額）。擋掉會吞掉真交易，比重複更難發現。
 * @param {any} db @returns {Map<string, number>}
 */
/** 遮罩帳號的**可見前綴**（`900200****3302` → `900200`；只印末碼的 `****3302` → `''`）。
 * `x****3302` 是 bankRefBase 在沒有遮罩時自己補的佔位，當成「不知道」。 */
function acctPrefix(masked) {
  const m = String(masked || '').match(/^([0-9A-Za-z]+)\*/);
  return (!m || m[1] === 'x') ? '' : m[1];
}

/** 疑似重複的既有交易索引：`機構|末碼|日期|方向|金額` → 這一格出現過的**可見前綴集合**。
 *
 * ⚠️ **機構要進 key**（r1#2）：只比末碼的話，一銀 `900200****3302` 與台新的同日同額同方向
 * 會被說成「同一個帳戶」——那是兩家不同銀行的兩個帳戶，警語會直接說謊。
 * ⚠️ **前綴另存不進 key**：跨版式的重點就是兩份 PDF 的遮罩印法可能不同（一份印 `****3302`、
 * 一份印 `900200****3302`）。所以前綴**只在兩邊都印得出來時**才拿來否決；有一邊不知道就放行。
 * ⚠️ `wanted`＝本次帳單真正會查的 key（先算好再掃）：不限制的話會替**全部**既有銀行交易建索引，
 * 10 萬筆約多吃 28MiB（審查者實測）——而其中絕大多數這次根本查不到。
 * @param {any} db @param {Set<string>} wanted
 */
function similarTxIndex(db, wanted) {
  ...
  for (const t of db.transactions || []) {
    ...
    const masked = isB2 ? parts[2] : parts[1];
    const suffix = accountSuffix(String(masked || ''));
    const dir = txDirection(db, t);
    if (!suffix || !dir || !t.date) continue;
    const key = `${inst}|${suffix}|${t.date}|${dir}|${Number(t.amount) || 0}`;
...
function similarKey(bank, suffix, tx) {
  return `${bank}|${suffix}|${tx.date}|${tx.direction}|${Number(tx.amount) || 0}`;
}
...
export function previewBankTxForDb(db, parsed) {
  ...
  const rows = (parsed.transactions || []).map(tx => {
    ...
    seen.add(bankRef);
    // 疑似重複（只在「不是明確重複」且「會匯入」的列才問）：帳號末碼＋日期＋方向＋金額都對得上既有交易
    const simSuffix = tx.acctSuffix || accountSuffix(tx.acctMasked || '');
    const simSeen = simSuffix ? simIdx.get(similarKey(bank, simSuffix, tx)) : undefined;
    const simPrefix = acctPrefix(tx.acctMasked || '');
    // 前綴只在**兩邊都印得出來**時才否決（跨版式的遮罩印法本來就可能不同）
    const similar = !duplicate && !foreign && !!simSeen
      && (simPrefix === '' || simSeen.has('') || simSeen.has(simPrefix));
...
//   ②**同意旗標**（★1 子項 LOCAL 拍板「每次送前先問」）：呼叫端要**明送 useAi:true** 才進得來——
//     前端確認窗（P1b-2）按了「同意」才帶旗標；旗標缺席＝模板解析器的原錯誤原句丟回、**零 AI 呼叫**。
...
//     刻意相反，見收支契約「匯入對帳閘」節）。
// 模型階梯（★3 拍板）：Haiku 先解；答案卷驗收不過（ai_bad_answer/ai_refusal）或對帳閘紅＝升 Sonnet
...
function assertAiBankReconciled(parsed, db) {
 ...
 * @typedef {{models:{primary:string,escalation:string}, parseOnce:(text:string, model:string)=>Promise<any>}} AiEngine
...
export async function aiBankRoute(b64, password, db, seams = {}) {
  ...
  for (const model of [engine.models.primary, engine.models.escalation]) {
    /** @type {any} */ let parsed;
    try { parsed = normalizeAiBank(await engine.parseOnce(text, model)); }   // 引擎交原始答案、服務層自己驗收（縱深防禦）
    catch (e) {
      const code = /** @type {any} */ (e)?.code;
      if (code === 'ai_bad_answer' || code === 'ai_refusal') { lastErr = e; continue; }   // 答案壞＝換大模型再試一次
      throw e;   // 鑰匙/服務錯誤：升級救不了，照實丟（訊息已白話、不含內文）
    }
    try { return { parsed, reconcile: assertAiBankReconciled(parsed, db), aiModel: model }; }
    catch (e) { lastErr = e; }   // 閘紅／弱閘拒收＝★3 拍板的「閘紅了才升 Sonnet」；第二次仍紅就照實擋
  }
...
// 驗「不平衡＝400、apply 未寫入任何資料」。第四參數 opts：路由只傳 `useAi`（同意旗標，P1b-1）；
...
export async function previewBankStatement(b64, password, parse = parseBankStatement, opts = {}) {
  ...
  try {
    ...
    const aiTicket = issueAiTicket({ parsed: r.parsed, aiModel: r.aiModel });
 ...
 * 不跑模型、不碰密碼池**（AI 非確定性：重跑會讓使用者確認的 A 變成寫入的 B）。兩條路都在
 * 「讀→（閘）→套→寫之間無外部 IO await」的既有不變量內：AI 的外部 IO 全發生在 preview。
 * @param {string} b64 @param {string=} password @param {typeof parseBankStatement} [parse] 測試接縫（見上）
 * @param {{useAi?:boolean, aiTicket?:string, aiEngineFactory?:(key:string)=>AiEngine, aiExtract?:typeof extractBankLines}} [opts] */
export async function applyBankStatement(b64, password, parse = parseBankStatement, opts = {}) {
  ...
  if (ticket) {
    ...
    try {
      ...
      if (!aiEligible(e, opts)) throw e;
      // 認不得＋有同意旗標但**沒有票**：不在這裡跑模型（那正是 r4#1 的病）——請前端先走預覽。
  ...
  try {
    ...
    if (ai) assertAiBankReconciled(parsed, db);   // AI 路線寫入前用 fresh db 重過閘（含 ★6 強閘要求）
    ...
    await saveDb(db);
    return { ok: true, ...bal, transactions: tx, ...(ai || {}) };
  } catch (e) {
    // ⚠️ 寫入失敗＝把票放回去（2026-08-12 William 實測踩到）：票是同步取走的（擋並發），
    //    但這條路失敗時（例如帳單讀不到現值參考日）票就沒了 ⇒ 使用者只能重新上傳、**再花一次 AI 費用**。
