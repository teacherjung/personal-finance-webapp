 ...
 * ⚠️ **刻意不驗寬度**：星號數不等於藏掉的位數是常態（本 repo 既有考題就用 `900100****3301` 配 15 碼的 `900100123453301`），
 *   「四顆星＝四個字元」不是證據；可見段全對＝這種印法能給的全部證明（與兩份遮罩帳單互比同一個強度）。
 ...
 * 帳單看不到前綴（全星號、`x****末碼` 佔位）＝'hit'（末碼對得上就算；**多顆同末碼由裁決器停手**）。
 * 一遮一全（不分哪邊是登記）：可見頭尾都對上＝hit（`headTailMatch`）、其餘 no——星號數不是證據，所以沒有「寬度不同＝ambiguous」這一格。
 * 兩邊都是遮罩：比對形**全等**才 hit；頭尾相容（一邊的可見前綴是另一邊的開頭、可見末碼是另一邊的結尾）＝ambiguous；
 *   登記的看不到前綴（全星號登記戶）＝no（走 Stage 1 的標記戶寬鬆徑或新建，既有行為）；其餘 no。
 * 兩邊都是完整號：比對形**全等**（`AB12345678` 不等於 `12345678`）。
 ...
 * 兩邊都是遮罩＝可見前綴全等（既有判準）；兩邊都是完整號＝整串全等；一遮一全＝可見頭尾都對上（`headTailMatch`，與身分那把
 * 同一個強度、同樣不驗寬度）。
