...
export function learnFromBankEdit(db, item, prev) {
  ...
  if (!String(item.note || '')) item.note = bankAutoNote(item);
}

/** 銀行交易的「預設自動顯示名」＝摘要・原始備註。優先用匯入存好的 `autoNote`；舊資料（無 autoNote）退而從 `bankRef`
 * 反解（尾兩段＝summary｜note，去掉批內出現序 #N）。清空自訂說明時回復到它。 @param {any} t */
function bankAutoNote(t) {
  if (t.autoNote) return String(t.autoNote);
  const parts = String(t.bankRef || '').split('|');
  if (parts[0] !== 'bank' || parts.length < 8) return '';
  const note = parts.slice(7).join('|').replace(/#\d+$/, '');   // note 可能含 '|'：第 7 段起全取回；末尾 #N＝出現序，非原文
  return [parts[6], note].filter(Boolean).join('・');
}
...
/** 「同類一起改」（Q2乙，使用者定 2026-07-21）：把某 bankKey **學過的規則**套用到**所有既有同 bankKey 的銀行交易**
 * （收支頁編輯時勾「同時套用到其他 N 筆同類」→ 先 PUT 觸發 learnFromBankEdit、再呼叫本函式）。以 learnedBank 為準
 * （剛學好的 type/分類；有自訂名才覆蓋 note、否則各自 note 保留）。**逐筆過方向護欄**（Codex r13#2）：收入只套進帳、
 * 支出只套出帳、內轉可兩向但子分類依本筆方向重播（out→內轉出/in→內轉入/交割中性）；方向不符者略過並計入 skipped。
 * 分類過生效樹校正。比照信用卡 applyCategoryToStore：保留字/找不到目標＝明確 400/404。
 * @param {string} bankKey @returns {{changed:number, skipped:number}} */
export function applyLearnedBankToExisting(bankKey) {
  const db = getDb();
 ...
 * ②被 early-return bug 洗成空白的說明（空 note → 自動名）。零操作、僅在有變動時寫檔。 @returns {{changed:number}} */
export function reconcileAccountNamesAuto() {
  const db = getDb();
  let changed = reconcileBankTxAccountNames(db);
  for (const t of db.transactions || []) {   // 順手補回空白說明（存款息之類被洗空的舊資料）
    if (t.source !== 'bank' || String(t.note || '')) continue;
    const auto = bankAutoNote(t);
    if (auto) { t.note = auto; changed++; }
  }
...
export function importBankTxToDb(db, parsed) {
  ...
  for (const tx of parsed.transactions || []) {
    ...
    const cls = resolveCls(db, rawCls);
    const autoNote = [tx.summary, tx.note].filter(Boolean).join('・');           // 預設自動顯示名（摘要・原始備註）
