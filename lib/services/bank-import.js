...
function matchAccount(existing, pa, bank) {
  ...
  if (dupInStatement && cands.some(a => a.accountNoSuffixOnly === true)) return { hit: null, ambiguous: true };
  ...
  if (prefix) {
    ...
    if (cands.length === 1 && stillPartial(cands[0])) return { hit: cands[0], ambiguous: false };
    if (cands.some(stillPartial)) return { hit: null, ambiguous: true };   // 真標記戶＋別的同末碼戶並存＝分不出，停手
  }
...
export function previewBalancesForDb(db, parsed, opts = {}) {
  ...
  markDupSuffixInStatement(parsed.accounts || []);    // 帳單內同末碼多帳號＝寬鬆徑停手（G1，preview 同判準）
...
export function applyBalancesToDb(db, parsed, opts = {}) {
  ...
  markDupSuffixInStatement(parsed.accounts || []);    // 帳單內同末碼多帳號＝寬鬆徑停手（G1）
  const cdRows = annotateCdRows(parsed, bank);
  const seen = new Set();
 ...
 * @param {any[]} accounts parsed.accounts */
function markDupSuffixInStatement(accounts) {
  /** @type {Map<string, Set<string>>} */ const bySuf = new Map();
  for (const pa of accounts || []) {
    if (!pa?.suffix) continue;
    const g = bySuf.get(pa.suffix) || new Set();
    g.add(String(pa.masked || '')); bySuf.set(pa.suffix, g);
  }
  for (const pa of accounts || []) {
