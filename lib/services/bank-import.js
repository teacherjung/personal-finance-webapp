...
export async function aiBankRoute(b64, password, db, seams = {}) {
  ...
  if (valid.length === 2) {
    ...
    if (cmp.agree) { const w = valid[1]; return { parsed: w.parsed, reconcile: w.reconcile, aiModel: w.model, lines, dualRead: /** @type {const} */ ('agree') }; }
  }
  ...
  if (!arb.ok) {
    ...
    // 與**恰一份**全欄一致＝採用那份。⚠️ 誠實註記（R3b 模式）：`=== 1` 與 `>= 1` 在可達狀態下行為
    // 等價——兩份有效答案彼此不一致⇒Fable 不可能同時與兩份全一致；只有一份有效時 matches∈{0,1}。
    // 寫 `=== 1` 是把「恰一份」的裁示語意直接寫進判準（防未來改動悄悄放寬），不是現在就有第二條路。
    if (matches.length === 1) { const w = matches[0]; return { parsed: w.parsed, reconcile: w.reconcile, aiModel: w.model, lines, dualRead: /** @type {const} */ ('arbitrated') }; }
  }
...
export async function previewBankStatement(b64, password, parse = parseBankStatement, opts = {}) {
  ...
  try {
    ...
    return { ...previewBalancesForDb(db, r.parsed), transactions: previewBankTxForDb(db, r.parsed), reconcile: r.reconcile, engine: 'ai', aiModel: r.aiModel, aiTicket, ...(/** @type {any} */ (r).dualRead ? { dualRead: /** @type {any} */ (r).dualRead } : {}) };
  }
