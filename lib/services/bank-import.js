...
import { parseBankStatement, extractBankLines, accountSuffixAny as accountSuffix, DEBIT_CARD_SUMMARIES, linkDebitCardRows } from '../bank-statement.js';
...
function maskedParts(masked) {
  ...
  if (/^\d{5,}$/.test(full)) return { prefix: full.slice(0, -4), suffix: full.slice(-4) };
...
function prefixFits(accountNo, stmtAcct) {
  ...
  return hasStar(accountNo) ? maskCoversFull(accountNo, stmtAcct) : digitsOf(accountNo) === digitsOf(stmtAcct);
}
 ...
 * 兩邊都是遮罩＝可見前綴全等（既有判準）；兩邊都是完整號＝整串全等；一遮一全＝遮罩逐位覆蓋（`maskCoversFull`）。
 * ⚠️ 疑似重複預設**跳過**那幾筆，所以這裡寬一分＝真交易少匯一筆（Codex #504 r1#2：兩個不同完整號被當成都沒前綴而互報）。
