...
/**
 * 配方生成（P2-3）：**AI 票路線兌票寫入成功之後**才跑（第二呼叫、一律 RECIPE_MODEL＝Opus，裁示⑥）。
 ...
 * 重生（裁示②④）：票上 suspectRecipeIds 有值＝這版面先前有配方失靈——寫回**第一個**候選列
 * （舊 current 降 previous、rebirths+1、streak 歸零）；多於一個候選＝其餘照樣被 markRecipesSuspect
 * 標疑似、等下次重生（殘餘：一次只重生一列，記入契約）。
 * @param {string} b64 @param {string|undefined} password @param {any} ticket 兌出的 AI 票
 ...
 */
export async function generateRecipeAfterImport(b64, password, ticket, opts) {
  try {
    const db = await getDb();   // 唯讀用途：鑰匙＋密碼池（生成寫入走 repo 櫃檯自己的交易）
    const key = String(db?.settings?.aiApiKey || '');
    ...
    const extract = opts.aiExtract || extractBankLines;
    const lines = await parseWithPool((b, pw) => extract(b, pw), decode(b64), statementPasswordPool(db, password ? [String(password)] : []));
    ...
    const saved = await saveParseRecipe(candidate, rebirthId ? { rebirthId } : {});
...
export async function applyBankStatement(b64, password, parse = parseBankStatement, opts = {}) {
  ...
  try {
    ...
    await saveDb(db);
    // P2-3：AI 票路線寫入成功＝順手生成配方（外部 IO 在恢復邊界與寫入交易**之外**；失敗不連坐）
    const recipeGen = (ai && ticket) ? await generateRecipeAfterImport(b64, password, ticket, opts) : null;
