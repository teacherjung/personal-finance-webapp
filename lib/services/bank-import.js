...
function debitCardPlan(db, parsed) {
  ...
  /** @type {Map<number, string>} */ const homeOfTx = new Map();
  {
    ...
    for (const [i, t] of txs.entries()) {
      ...
      homeOfTx.set(i, owner && owner.importBatch ? String(owner.importBatch) : 'NEW');
    }
  }
  for (const [k, g] of groups) {
    ...
    if (g.cards.length > 1 && new Set(g.txs.map((/** @type {number} */ t) => homeOfTx.get(t) || 'NEW')).size > 1) { skipped.unmatched += g.cards.length; continue; }   // 同鍵群跨批＝整群不記、不留空（r5#1）
...
function importDebitCardLedgerToDb(db, parsed, /** @type {ReturnType<typeof debitCardPlan>} */ cardPlan, /** @type {string[]} */ refByIndex) {
  ...
  // 同鍵多筆（ambiguous）的「跨批次群」在**計畫層**就整群排除（debitCardPlan 的 homeOfTx 判準，Codex #509 r5#1）
  //   ——能走到這裡的歧義群，成員的 D 列必然同一個家，逐筆綁與群組綁同一個答案，這裡就不再抄一份群組邏輯。
