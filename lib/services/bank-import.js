...
import { parseBankStatement, extractBankLines, accountSuffix, DEBIT_CARD_SUMMARIES, linkDebitCardRows } from '../bank-statement.js';
...
function debitCardPlan(db, parsed) {
  ...
  // 帳戶那邊**早就在**的刷卡列（source:'bank'、刷卡摘要、帶分類、同末碼）：鍵＝日期|方向|金額
  ...
  for (const t of db.transactions || []) {
    ...
    categorized.add(`${t.date}|${t.dir || ''}|${Number(t.amount) || 0}`);
  }
  ...
  // 同鍵多筆（ambiguous）整群一起判：群裡任一筆不可記＝整群都不記、整群都不留空（最保守）
  ...
  for (const p of link.pairs) {
    ...
    if (!importableRow(cardRows[p.card])) { g.ok = false; skipped.unreadable++; }
    groups.set(k, g);
  }
...
export function previewBankTxForDb(db, parsed) {
  ...
  const cardPlan = debitCardPlan(db, parsed);   // 簽帳金融卡明細：哪幾列 D 區刷卡要留空（對得上、且那筆會記進卡片帳本）
...
function planDebitCardLedger(db, parsed) {
  ...
  const plan = debitCardPlan(db, parsed);
...
export function importBankTxToDb(db, parsed, opts = {}) {
  ...
  const cardPlan = debitCardPlan(db, parsed);   // 同預覽那一套（純函式、同一份 db 快照＝同答案）
