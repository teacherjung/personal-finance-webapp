...
export async function aiBankRoute(b64, password, db, seams = {}) {
  ...
  if (!arb.ok) {
    ...
    if (matches.length === 1) { const w = matches[0]; const tv = aiAnswersAgree(w.parsed, /** @type {any} */ (arb).parsed).textVariance; return { parsed: w.parsed, reconcile: w.reconcile, aiModel: w.model, lines, dualRead: valid.length === 2 ? /** @type {const} */ ('arbitrated') : /** @type {const} */ ('attested'), ...(tv.length ? { dualReadTextVariance: tv } : {}) }; }
  }
