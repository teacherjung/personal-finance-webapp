...
export function previewBalancesForDb(db, parsed) {
  ...
  // ⚠️ 2026-08-13 起 `blocked` 只代表「**這次不會更新餘額**」——apply **不再整份擋下**，交易照樣匯入。
  //    欄位名沿用是為了不動呼叫端；語意已變，讀到這裡的人不要照舊名字推論行為。
...
export async function applyBankStatement(b64, password, parse = parseBankStatement, opts = {}) {
  ...
  try {
    ...
    //    ⚠️ 原本這裡舉的例子是「帳單讀不到現值參考日」——那個情境 2026-08-13 起**不再失敗**
    //    （改成餘額不更新、交易照匯）。現在的失敗路徑是：getDb 掛掉／對帳閘紅／saveDb 櫃檯清理擋下。
