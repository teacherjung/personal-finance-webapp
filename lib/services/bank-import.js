...
import { getDb, saveDb, uid, updateSettings } from '../repo.js';
...
export async function applyBankStatement(b64, password, parse = parseBankStatement, opts = {}) {
  ...
  if (recipeGen) {
    try {
      ...
      const db2 = await getDb();   // 讀→改→寫之間無外部 IO（櫃檯規矩③）
      const today = new Date().toISOString().slice(0, 10);
      await updateSettings({ recipeBirthStats: recordBirth(db2?.settings?.recipeBirthStats, code, bank, today) });
