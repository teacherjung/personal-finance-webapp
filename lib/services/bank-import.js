 ...
 */
function similarTxIndex(db) {
  /** @type {Map<string, number>} */
  const idx = new Map();
  for (const t of db.transactions || []) {
    if (t.source !== 'bank') continue;
    const parts = String(t.bankRef || '').split('|');
    const masked = parts[0] === 'bank2' ? parts[2] : parts[1];   // 雙格式（P1a）：bank2 的帳號在第 3 段
    const suffix = accountSuffix(String(masked || ''));
    const dir = txDirection(db, t);
    if (!suffix || !dir || !t.date) continue;
    const key = `${suffix}|${t.date}|${dir}|${Number(t.amount) || 0}`;
...
export function previewBankTxForDb(db, parsed) {
  ...
  const simIdx = similarTxIndex(db);
  ...
  const rows = (parsed.transactions || []).map(tx => {
    ...
    const similar = !duplicate && !foreign && !!simSuffix
      && (simIdx.get(`${simSuffix}|${tx.date}|${tx.direction}|${Number(tx.amount) || 0}`) || 0) > 0;
