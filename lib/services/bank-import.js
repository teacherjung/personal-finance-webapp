...
/** 一筆**既有**銀行交易的實際金流方向。優先用匯入存好的 `dir`（不可竄改的事實）；舊資料（本次修正前匯入、無 dir）
 * 退而從 type/子分類推：income→in、expense→out、內轉出/內轉入角色→out/in、交割(settle)/未知→null。null＝方向不明，
 * 呼叫端須保守處理（不硬套收入/支出）。 @param {any} db @param {any} t @returns {'in'|'out'|null} */
...
export function importBankTxToDb(db, parsed) {
  ...
  for (const tx of parsed.transactions || []) {
    ...
    (db.transactions ||= []).push({
      ...
      ledger: 'cashflow', source: 'bank', dir: tx.direction, bankRef, bankKey, importBatch: batchId, importedAt,
    });
    imported++;
  }
