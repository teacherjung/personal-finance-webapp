...
/** 預覽（解析 PDF → 對帳閘 → 純邏輯）：回帳戶餘額變動＋交易分箱＋對帳裁決三塊。
...
export async function previewBankStatement(b64, password, parse = parseBankStatement) {
  const parsed = await parse(decode(b64), password);
  const db = await getDb();   // 先讀 db（閘的幣別判定要用它補位，P0.1 r1#2）再過閘
