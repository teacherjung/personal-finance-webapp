...
import { parseBankStatement, extractBankLines, accountSuffix as accountSuffixStar, accountSuffixAny as accountSuffix, normalizeMaskShape, DEBIT_CARD_SUMMARIES, linkDebitCardRows } from '../bank-statement.js';
// ⚠️ 本檔的 accountSuffix ＝**寬版** accountSuffixAny（有星號＝同窄版；至少五碼純數字＝末四碼）：完整帳號的列（AI 路線
//   匯進來的）一旦存在，每一把末碼索引都必須認得它，否則重複匯入或漏擋。兩版只在「至少五碼純數字」上不同——
//   去重鍵帳號段的純數字有兩種解讀（純末碼祖父鍵／完整號），讀既有列的地方一律走 refSuffixReadings 兩種都收
//   （它用窄版 accountSuffixStar 判「有沒有星號」）；不可拿寬版直接截成四碼（Codex #504 r4#2）。
...
/** 帳號的**比對形**：NFKC＋遮罩改寫（X／圓點→半形星，與 AI 驗收同一把 `normalizeMaskShape`）＋全形星→半形＋分隔符
 * （空白／連字號）剝掉；**字母保留**（`AB****5678` 的 AB 是可見身分，剝掉＝變成 `****5678`、什麼號都蓋得住——Codex #504 r2#1／r3#1）。
 * @param {string} s */
const cleanAcct = (s) => normalizeMaskShape(String(s || '')).replace(/＊/g, '*').replace(/[\s-]/g, '');
/** 遮罩形拆可見段：`900100****3301` → {head:'900100', tail:'3301', body}；`x****3302` 佔位（bankRefBase 對無遮罩列補的）
 * 與全星號＝head ''（看不到前綴）；沒星號＝null。 @param {string} acct */
function maskSegments(acct) {
  const body = cleanAcct(acct);
  const first = body.indexOf('*'), last = body.lastIndexOf('*');
  if (first < 0) return null;
  const head = body.slice(0, first);
  return { head: head === 'x' ? '' : head, tail: body.slice(last + 1), body };
}
/** 遮罩形的可見片段（依星號段切開）：`900100**22**3301` → ['900100','22','3301']；`****3301` → ['','3301']；`x****3302` 佔位＝['','3302']。 @param {string} masked */
function maskParts(masked) {
  const body = cleanAcct(masked);
  const parts = body.split(/\*+/);
  if (parts[0] === 'x') parts[0] = '';
  return parts;
}
/** 遮罩形能不能蓋住一個完整號：**每一段可見文字依序吻合**（第一段是開頭、最後一段是結尾、中間各段依序出現——字母也算可見；
 * `900100**22**3301` 對 `9001009911993301` 的 `22` 與 `11` 衝突＝蓋不住，Codex #504 r6#1），**每段星號至少遮一碼**
 * （`900100****3301` 對 10 碼的 `9001003301`＝不是，r4#1）；不要求藏幾碼＝幾顆星（repo 既有考題就用四顆星配 15 碼）。
 * 餘額身分與疑似重複**共用這一支**。 @param {string} masked @param {string} full */
function maskCoversFull(masked, full) {
  const f = cleanAcct(full);
  if (!hasStar(masked) || !f || hasStar(f)) return false;
  const parts = maskParts(masked);
  if (!f.startsWith(parts[0])) return false;
  let pos = parts[0].length;
  for (let i = 1; i < parts.length; i++) {
    const seg = parts[i];
    const minStart = pos + 1;                                 // 這段星號至少遮一碼
    if (i === parts.length - 1) return f.endsWith(seg) && f.length - seg.length >= minStart;
    const idx = f.indexOf(seg, minStart);
    if (idx < 0) return false;
    pos = idx + seg.length;
  }
  return true;
}
/** 兩個遮罩形「相容」（可能是同一顆戶）：可見段數相同時中間各段要全等、開頭一邊是另一邊的開頭、結尾一邊是另一邊的結尾；
 * 段數不同時只比開頭與結尾。**相容≠命中**——命中只有比對形全等。 @param {string} a @param {string} b */
function masksCompatible(a, b) {
  const pa = maskParts(a), pb = maskParts(b);
  const ha = pa[0], hb = pb[0], ta = pa[pa.length - 1], tb = pb[pb.length - 1];
  if (!(ha.startsWith(hb) || hb.startsWith(ha))) return false;
  if (!(ta.endsWith(tb) || tb.endsWith(ta))) return false;
  if (pa.length === pb.length) for (let i = 1; i < pa.length - 1; i++) if (pa[i] !== pb[i]) return false;
  return true;
}
/** 登記帳號對帳單帳號的身分判定（末碼由呼叫端先比）：`'hit'`＝可見段全對／`'no'`＝不是／`'ambiguous'`＝相容但證明不了。
 * ⚠️ **相容≠命中**（Codex #504 r3#1）：`9001****3301` 只代表「某個 9001 開頭、3301 結尾的戶」，不能證明就是登記的
 *   `900133****3301`——可見段不完全相同就只能 ambiguous；挑一顆＝蓋錯餘額、新建＝同一顆戶拆兩顆（資產多算一份）。
 *   兩邊比的都是 `cleanAcct` 的比對形（字母保留、分隔符剝掉、遮罩統一成半形星）。
 * 判定順序（先到先算）：
 * ①帳單看不到前綴（全星號、`x****末碼` 佔位）＝'hit'——不論登記的長什麼樣（含登記也是全星號）；末碼對得上就算、**多顆同末碼由裁決器停手**。
 * ②帳單印完整號：登記完整號＝比對形**全等**（`AB12345678` 不等於 `12345678`）；登記**任何遮罩形**（含看不到前綴的 `****3301`，
 *   r6#2）＝遮罩蓋得住完整號（`maskCoversFull`）＝**ambiguous**、其餘 no——登記的遮罩是自動建戶抄來的、證明不了它就是這個完整號
 *   （`900100****3301` 可能是 `…1111…` 也可能是 `…9999…`；r5#1）；停手請使用者到資產頁把帳號補成完整號，之後完整對完整全等就命中。
 *   Stage 1 的標記戶（`accountNoSuffixOnly`）不進裁決器、走 matchAccount 自己的寬鬆徑。
 * ③帳單是遮罩（有可見前綴）、登記**完整號**＝帳單遮罩蓋得住登記號（`maskCoversFull`）＝hit、其餘 no——登記的完整號是使用者給的身分，
 *   帳單遮罩與之一致就是它（既有行為）；「藏幾碼＝幾顆星」不是證據，所以沒有「寬度不同＝ambiguous」這一格。
 * ④兩邊都是遮罩：登記的看不到前綴（全星號登記戶）＝no（走 Stage 1 的標記戶寬鬆徑或新建，既有行為）；比對形**全等**＝hit；
 *   相容（`masksCompatible`：開頭／結尾一邊是另一邊的、段數相同時中段全等）＝ambiguous；其餘 no。
 * @param {string} accountNo 登記帳號 @param {string} stmtAcct 帳單帳號 @returns {'hit'|'no'|'ambiguous'} */
function accountFit(accountNo, stmtAcct) {
  const S = maskSegments(stmtAcct), Sc = cleanAcct(stmtAcct);
  const R = maskSegments(accountNo), Rc = cleanAcct(accountNo);
  if (S && !S.head) return 'hit';
  if (!S) {
    if (!R) return Rc === Sc ? 'hit' : 'no';
    return maskCoversFull(Rc, Sc) ? 'ambiguous' : 'no';
  }
  if (!R) return maskCoversFull(Sc, Rc) ? 'hit' : 'no';
  if (!R.head) return 'no';
  if (Rc === Sc) return 'hit';
  return masksCompatible(Rc, Sc) ? 'ambiguous' : 'no';
}
...
function matchAccount(existing, pa, bank) {
  ...
  const cands = existing.filter(base);
  // 帳單印**完整號**、登記的是遮罩形而可見末碼比帳單末碼**短**（`900300****162` 對完整號的 3162）：末碼過不了 base，
  //   但可見頭尾都對得上＝可能是同一顆戶（星號數本來就不等於藏掉的位數）——一起交給裁決器判 ambiguous 停手，不可因
  //   末碼長度不同就新建第二顆（r2#2：兩顆餘額都進資產＝多算一份）。
  const shortTailCands = hasStar(pa.masked) ? [] : existing.filter(a => {
    if (cands.includes(a) || !eligible(a)) return false;
    const seg = maskSegments(a.accountNo);
    return !!(seg && seg.tail && seg.tail !== suffix && suffix.endsWith(seg.tail));
  });
  ...
  const strict = resolveCandidates([...cands, ...shortTailCands].filter(a => !isFlagged(a)), pa.masked);
...
export function previewBalancesForDb(db, parsed, opts = {}) {
  ...
  for (const pa of parsed.accounts) {
    ...
    rows.push({
      ...
      // ambiguous（Stage 1）＝末碼撞到多顆帳戶、分不出是哪一顆：**不更新也不新建**（挑一顆＝把餘額
...
export function applyBalancesToDb(db, parsed, opts = {}) {
  ...
  for (const pa of parsed.accounts) {
    ...
    const m = cd ? { hit: existing.find(a => a.cdKey && canonCdKey(a.cdKey) === canonCdKey(cd.cdKey)) || null, ambiguous: false } : matchAccount(existing, pa, bank);   // cdKey 比對形（Stage 4 祖父：舊戶的機構段是原寫法）
    const acc = m.hit;
    if (m.ambiguous) { skipped++; continue; }   // 末碼撞多顆＝不動（與預覽的 'ambiguous' 同判準；挑一顆或新建都比不動更危險）
    if (acc) {
...
function debitCardPlan(db, parsed) {
  ...
  for (const t of db.transactions || []) {
    ...
    if (!refSuffixReadings(masked).some(r => suffixes.has(r.suffix))) continue;   // 純數字段兩種解讀都查（祖父末碼／完整號）
  ...
  const importableRow = (/** @type {import('../bank-statement.js').DebitCardRow} */ r) =>
    isRealDate(r.date) && Number.isFinite(r.amount) && r.amount !== 0 && Math.abs(r.amount) <= 1e8 && !!r.desc && !r.desc.includes('|');   // 與 importRowsToDb 的拒收條件同一套
...
function txCurrency(db, parsed, tx) {
  ...
  const cands = (db.accounts || []).filter(a => {
    ...
    const d = String(a.accountNo || '').replace(/\D/g, '');
    return !!suffix && d.length >= suffix.length && d.endsWith(suffix);
  });
  const acc = resolveCandidates(cands, stmtAcct).hit;   // 與餘額同一個裁決器：證明不了／多顆＝當沒有（退回 TWD）
...
function accountNameForTx(db, tx, parsed) {   // 測試出口在檔尾（accountNameForTxForTest）——掛名繞開定存戶的行為要能直測
  ...
  const r = resolveCandidates(bySuffix, stmtAcct);
  ...
  const acc = r.hit || ((!r.ambiguous && bySuffix.length === 1 && partialOk(bySuffix[0])) ? bySuffix[0] : null);
...
function acctPrefix(masked) {
  ...
  if (star <= 0) return '';                                   // 沒有遮罩星號、或整串以星號開頭＝看不到前綴（完整號不走這裡：一遮一全先判方向）
  const head = raw.slice(0, star).replace(/[^0-9A-Za-z]/g, '');
  return (!head || head === 'x') ? '' : head;
}
/** 去重鍵帳號段的**末碼解讀**（疑似重複索引與金融卡計畫的既有列側）：遮罩＝星號後的數字，一種解讀；
 * **純數字段有兩種解讀**，兩種都要收——①純末碼祖父鍵（`bank|3302|…`、`bank|12345|…`：機構維度之前只記末碼，
 * `accountSuffix` 的 `\d+` 本來就允許五碼以上的末碼）＝整段就是末碼、前綴不知道 ②AI 路線的完整帳號（至少五碼）＝末四碼。
 * 只取②＝五碼以上的祖父鍵再也對不上（Codex #504 r4#2：`bank|12345|…` 對 `900100****12345` 不提醒＝跨版型重匯多算現金流）。
 * 回 [{suffix, shape}]：shape 是給形狀否決用的印法——祖父解讀用 `*末碼`（看不到前綴），完整號解讀用原段。
 * @param {string} masked @returns {Array<{suffix:string, shape:string}>} */
function refSuffixReadings(masked) {
  const m = String(masked || '');
  const star = accountSuffixStar(m);
  if (star) return [{ suffix: star, shape: m }];
  const out = [];
  if (/^\d{3,}$/.test(m)) out.push({ suffix: m, shape: `*${m}` });                   // 祖父末碼解讀：只限原始純數字段
  const clean = cleanAcct(m);                                                          // 完整號解讀：AI 照抄的分隔符（`900-100-1122-3301`）先剝（r5#2）
  if (!hasStar(clean) && /^\d{5,}$/.test(clean)) out.push({ suffix: clean.slice(-4), shape: clean });
  return out;
}
/** 兩個帳號印法「可能是同一顆戶」（疑似重複的前綴否決用）：一邊看不到前綴＝不否決（跨版式印法本來就不同）；
 * 兩邊都是遮罩＝可見前綴全等（既有判準）且中段不衝突；兩邊都是完整號＝整串全等；一遮一全＝只在**既有列是完整號、新列是遮罩**
 * 時比遮罩蓋不蓋得住（`maskCoversFull`，與身分那把同一支、同一個方向）；既有列是遮罩、新列是完整號＝證明不了＝不算。
...
function acctShapesCompatible(a, b) {
  ...
  const pa = acctPrefix(a), pb = acctPrefix(b);
  if (pa === '' || pb === '') return true;                   // 兩遮、一邊看不到前綴＝不否決（跨版式印法本來就不同）
  return pa === pb && masksCompatible(a, b);                 // 兩遮＝可見前綴全等（既有判準）＋中段不衝突（r6#1）
}
...
function similarTxIndex(db, wanted) {
  ...
  for (const t of db.transactions || []) {
    ...
    for (const { suffix, shape } of refSuffixReadings(masked)) {
 ...
 * @param {any} db @param {ParsedBankFull} parsed
 * @returns {{ cards: {cardId:string, name:string, lastFour:string, exists:boolean, rows:any[]}[], count:number, duplicate:number, skipped:{unmatched:number, unreadable:number, cashflowCategorized:number}, error:string }} */
function planDebitCardLedger(db, parsed, /** @type {ReturnType<typeof debitCardPlan>|null} */ cardPlan = null) {
  ...
  const cardRows = plan.importable.map((i) => all[i]);   // 只有對得上、抄得完整、帳戶那邊不是早就帶分類記過的筆
  const error = String(parsed.cardRowsError || '');
  if (!cardRows.length) return { cards: [], count: 0, duplicate: 0, skipped: plan.skipped, error };
  const bank = stmtBank(parsed);
  /** @type {Map<string, import('../bank-statement.js').DebitCardRow[]>} */ const byCard = new Map();
  for (const r of cardRows) { const g = byCard.get(r.lastFour) || []; g.push(r); byCard.set(r.lastFour, g); }
  const cards = [];
  let count = 0, duplicate = 0;
  for (const [lastFour, rows] of byCard) {
    ...
    const finalized = finalizeCardRows(rows.map((r) => ({ date: r.date, desc: r.desc, amount: r.amount })), bank).transactions;
    ...
    cards.push({ cardId, name, lastFour, exists: Boolean(hit), rows: preview });
  }
...
function importDebitCardLedgerToDb(db, parsed, /** @type {ReturnType<typeof debitCardPlan>} */ cardPlan, /** @type {string} */ bankBatchId) {
  ...
  for (const c of plan.cards) {
    ...
    // 新建的卡要用真 id 重算列（stmtRef 的第一段＝卡片 id，是去重與「帳單原文」的身分）
    const rows = created ? previewRowsForCard(db, String(card.id), c.rows) : c.rows;
    const r = importRowsToDb(db, card, rows, stmtMonth, null);
    // 生命週期跟著那份銀行帳單（Codex #503 r1#3）：卡片帳本這幾筆蓋上 bankBatch＝銀行匯入批次代號——
    //   信用卡那邊的匯入紀錄不列它、不准改卡／單獨刪（改卡會讓 stmtRef 換卡片 id＝重匯時再記一次；單獨刪＝
    //   D 區那幾列已留空、消費就少算）；從「銀行匯入紀錄」刪那份帳單時一起拿掉。
    for (const t of db.transactions || []) if (t.importBatch === r.batchId && t.source === 'stmt') t.bankBatch = bankBatchId;
  ...
  return { cards: out, imported, skipped, notRecorded: plan.skipped, error: plan.error };
}
...
export function assertAiBankReconciled(parsed, db) {
  ...
  } else if (parsed.transactions.some((/** @type {any} */ x) => (parsed.accountCurrency?.[x.acctMasked] || 'TWD') !== 'TWD')) {
...
async function aiTryModel(/** @type {AiEngine} */ engine, /** @type {string} */ model, /** @type {string} */ text, /** @type {any} */ db) {
  try {
    ...
    return { ok: /** @type {const} */ (true), parsed, reconcile: assertAiBankReconciled(parsed, db), model };
  } catch (e) { return { ok: /** @type {const} */ (false), err: e, model }; }
}
const AI_SERVICE_CODES = ['ai_auth', 'ai_unavailable', 'ai_truncated'];   // 服務類錯：換模型救不了、照實丟
...
export async function aiBankRoute(b64, password, db, seams = {}) {
  ...
  if (valid.length === 2) {
    ...
    if (cmp.agree) { stage(STAGES.VERIFY); const w = valid[1]; return { parsed: w.parsed, reconcile: w.reconcile, aiModel: w.model, lines, dualRead: /** @type {const} */ ('agree'), ...(cmp.textVariance.length ? { dualReadTextVariance: cmp.textVariance } : {}) }; }
  }
  ...
  if (!arb.ok) {
    ...
    if (matches.length === 1) {
      ...
      // 存在「兩份的差異」這個鍵）。
      return { parsed: w.parsed, reconcile: w.reconcile, aiModel: w.model, lines, dualRead: valid.length === 2 ? /** @type {const} */ ('arbitrated') : /** @type {const} */ ('attested'), ...(tv.length ? { dualReadTextVariance: tv } : {}), ...(() => {
 ...
 * @param {{useAi?:boolean, aiEngineFactory?:(key:string)=>AiEngine, aiExtract?:typeof extractBankLines, onStage?:(f:any)=>void}} [opts] */
 ...
 * @param {string} b64 @param {string=} password @param {typeof parseBankStatement} [parse] 測試接縫（見上）
 * @param {{useAi?:boolean, aiTicket?:string, skipSimilar?:boolean, aiEngineFactory?:(key:string)=>AiEngine, aiExtract?:typeof extractBankLines, aiRecipeGen?:typeof generateRecipeAfterImport, birthWrite?:typeof updateRecipeBirthStats}} [opts] */
export async function applyBankStatement(b64, password, parse = parseBankStatement, opts = {}) {
  ...
  try {
    ...
    const cardLedger = importDebitCardLedgerToDb(db, parsed, cardPlan, /** @type {any} */ (tx).batchId);   // 簽帳金融卡明細→卡片帳本（模板路線才有 cardRows；AI／配方路線＝空）
