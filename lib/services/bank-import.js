...
import { parseBankStatement, extractBankLines, accountSuffix, DEBIT_CARD_SUMMARIES } from '../bank-statement.js';
...
function matchAccount(existing, pa, bank) {
  ...
  const strictHit = cands.find(a => !prefix || String(a.accountNo || '').replace(/\D/g, '').startsWith(prefix));
 ...
 * A 區記到卡片帳本（帶分類）；D 區的「刷卡消費／刷卡退貨」列**分類留空**——同繳卡費模型：現金流照算錢進錢出，
 * 消費分析只算卡片帳本那一份，錢不算兩次。**只在這份帳單真的帶 A 區**（cardRows 非空）時才留空；綜合對帳單
 * 沒有 A 區，它的刷卡列照舊分類（否則消費分析會少算）。學過的規則也讓位：A 區那一份才是分類的家。
 * @param {ParsedBankFull} parsed @param {import('../bank-statement.js').BankTx} tx */
function debitCardLine(parsed, tx) {
  return Array.isArray(parsed.cardRows) && parsed.cardRows.length > 0 && DEBIT_CARD_SUMMARIES.includes(tx.summary);
}
...
function accountNameForTx(db, tx, parsed) {   // 測試出口在檔尾（accountNameForTxForTest）——掛名繞開定存戶的行為要能直測
  ...
  const acc = (fullStar && bySuffix.length > 1) ? null
    : (bySuffix.find(a => !prefix || digits(a).startsWith(prefix)) || bySuffix[0]);   // 先前綴+末碼、退回只末碼
 ...
 * cardRows＝簽帳金融卡明細限定（A 區刷卡消費明細；Stage 5b 記到卡片帳本）——optional。
 * @typedef {ParsedBank & {transactions:import('../bank-statement.js').BankTx[], totals?: {txCount:number|null, totalOut:number|null, totalIn:number|null}, cardRows?: import('../bank-statement.js').DebitCardRow[]}} ParsedBankFull */
...
export function previewBankTxForDb(db, parsed) {
  ...
  const rows = (parsed.transactions || []).map(tx => {
    ...
    const isCardLine = debitCardLine(parsed, tx);
 ...
 * @param {any} db @param {ParsedBankFull} parsed
 * @returns {{ cards: {cardId:string, name:string, lastFour:string, exists:boolean, rows:any[]}[], count:number, duplicate:number }} */
function planDebitCardLedger(db, parsed) {
  const cardRows = Array.isArray(parsed.cardRows) ? parsed.cardRows : [];
  if (!cardRows.length) return { cards: [], count: 0, duplicate: 0 };
  const bank = stmtBank(parsed);
 ...
 * @param {any} db @param {ParsedBankFull} parsed
 * @returns {{ cards: {cardId:string, name:string, lastFour:string, created:boolean, imported:number, skipped:number}[], imported:number, skipped:number }} */
function importDebitCardLedgerToDb(db, parsed) {
  const plan = planDebitCardLedger(db, parsed);
  const bank = stmtBank(parsed);
  ...
  return { cards: out, imported, skipped };
}
...
export function importBankTxToDb(db, parsed, opts = {}) {
  ...
  for (const tx of parsed.transactions || []) {
    ...
    const { bankKey, name } = learned;
    const cls = debitCardLine(parsed, tx) ? { type: tx.direction === 'out' ? 'expense' : 'income', category: '', subcategory: '' } : resolveCls(db, learned.cls);
...
export async function previewBankStatement(b64, password, parse = parseBankStatement, opts = {}) {
  ...
  const plan = planDebitCardLedger(db, parsed);   // 簽帳金融卡明細：A 區刷卡消費要記到哪張卡、幾筆（沒有 A 區＝空）
  return { ...previewBalancesForDb(db, parsed, { deterministic: true }), transactions: previewBankTxForDb(db, parsed), reconcile,
    cardLedger: { cards: plan.cards.map((c) => ({ cardId: c.cardId, name: c.name, lastFour: c.lastFour, exists: c.exists })), count: plan.count, duplicate: plan.duplicate } };   // 模板路線＝唯一的確定性解析
}
...
export async function applyBankStatement(b64, password, parse = parseBankStatement, opts = {}) {
  ...
  try {
    ...
    const cardLedger = importDebitCardLedgerToDb(db, parsed);   // 簽帳金融卡明細→卡片帳本（模板路線才有 cardRows；AI／配方路線＝空）
