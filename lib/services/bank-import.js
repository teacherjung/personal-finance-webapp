...
import { parseBankStatement, extractBankLines, accountSuffixAny as accountSuffix, normalizeMaskShape, DEBIT_CARD_SUMMARIES, linkDebitCardRows } from '../bank-statement.js';
// ⚠️ 本檔的 accountSuffix ＝**寬版** accountSuffixAny（有星號＝同窄版；至少五碼純數字＝末四碼）：完整帳號的列（AI 路線
//   匯進來的）一旦存在，每一把末碼索引都必須認得它，否則重複匯入或漏擋。兩版只在「至少五碼純數字」上不同——放寬前
//   這種值在本檔的呼叫點只可能是假想的五碼以上純末碼祖父鍵（3～4 碼的祖父鍵兩版都取不出、走各呼叫點的純數字退路）。
...
/** 帳號字串的比對形：NFKC、全形星→半形、分隔符（空白／連字號）剝掉；**字母保留**（`AB****5678` 的 AB 是可見身分，
 * 剝掉＝變成 `****5678`、什麼完整號都蓋得住——Codex #504 r2#1）。 @param {string} s */
const cleanAcct = (s) => String(s || '').normalize('NFKC').replace(/＊/g, '*').replace(/[\s-]/g, '');
/** 遮罩形拆可見段：`900100****3301` → {head:'900100', tail:'3301', body:'900100****3301'}；沒星號＝null。 @param {string} masked */
function maskSegments(masked) {
  const body = cleanAcct(masked);
  const first = body.indexOf('*'), last = body.lastIndexOf('*');
  if (first < 0) return null;
  return { head: body.slice(0, first), tail: body.slice(last + 1), body };
}
/** 遮罩形能不能**逐位覆蓋**一個完整號：寬度相同、可見字元逐位相同（字母也算可見）、星號段剛好蓋住藏掉的位數
 * （`900100****3301` 蓋得住 `90010011223301`、蓋不住 16 碼的 `9001001122333301`——四顆星只可能代表四個字元；Codex #504 r1#1）。
 * @param {string} masked @param {string} full */
function maskCoversFull(masked, full) {
  const seg = maskSegments(masked);
  const f = cleanAcct(full);
  if (!seg || !f || hasStar(f) || seg.body.length !== f.length) return false;
  for (let i = 0; i < f.length; i++) if (seg.body[i] !== '*' && seg.body[i] !== f[i]) return false;
  return true;
}
/** 登記帳號對帳單帳號的身分判定（末碼由呼叫端先比）：`'hit'`＝證明是同一顆／`'no'`＝不是／`'ambiguous'`＝證明不了
 * （呼叫端停手：不更新餘額、不新建、不掛既有戶——挑一顆＝蓋錯餘額，新建＝同一顆戶拆成兩顆、資產多算一份；Codex #504 r2#1/#2）。
 * 帳單沒前綴可比（全星號、`x****末碼` 佔位）＝`'hit'`（不擋，既有語意）。
 * 帳單是**遮罩**：登記完整號＝數字以可見前綴開頭（既有判準）；登記也是遮罩＝**逐段比可見前綴**（不可把登記的前後段
 *   數字串起來再 startsWith——`9001****3301` 串成 90013301、對 `900133****3301` 的前綴 900133 拼出不存在的相鄰關係）：
 *   全等＝hit；登記的更長且以帳單前綴開頭＝hit（登記得更具體、帳單與之一致＝既有行為）；帳單的更長且以登記前綴開頭＝
 *   ambiguous（登記的證明不了）；登記看不到前綴＝no（既有行為）。
 * 帳單印**完整號**：登記完整號＝整串全等；登記遮罩形（親手填的 X 遮罩先過同一把改寫）＝頭尾都對得上才算候選，
 *   再驗寬度與可見位（`maskCoversFull`）＝hit；頭尾對得上但寬度不同（`900300****162` 對 14 碼）＝ambiguous——
 *   星號數與藏掉位數未必相同，可能是同一顆戶；新建會多算一份資產、更新會蓋錯，只能停手請使用者認。
 * @param {string} accountNo 登記帳號 @param {string} stmtAcct 帳單帳號 @returns {'hit'|'no'|'ambiguous'} */
function accountFit(accountNo, stmtAcct) {
  const { prefix } = maskedParts(stmtAcct);
  if (!prefix) return 'hit';
  const reg = normalizeMaskShape(accountNo);
  const regSeg = maskSegments(reg);
  if (hasStar(stmtAcct)) {
    if (!regSeg) return digitsOf(reg).startsWith(prefix) ? 'hit' : 'no';
    const stmtHead = /** @type {{head:string}} */ (maskSegments(stmtAcct)).head;
    if (!regSeg.head) return 'no';
    if (regSeg.head === stmtHead || regSeg.head.startsWith(stmtHead)) return 'hit';
    return stmtHead.startsWith(regSeg.head) ? 'ambiguous' : 'no';
  }
  const f = cleanAcct(stmtAcct);
  if (!regSeg) return digitsOf(reg) === digitsOf(f) ? 'hit' : 'no';
  if (!f.startsWith(regSeg.head) || !f.endsWith(regSeg.tail)) return 'no';
  return maskCoversFull(reg, f) ? 'hit' : 'ambiguous';
}
...
function matchAccount(existing, pa, bank) {
  ...
  //   但頭尾都對得上＝可能是同一顆戶（星號數未必等於藏掉的位數）——accountFit 說「證明不了」就停手，不可新建第二顆
  //   （r2#2：兩顆餘額都進資產＝多算一份）。
  const shortTailAmbiguous = hasStar(pa.masked) ? [] : existing.filter(a => {
    if (cands.includes(a) || !eligible(a)) return false;
    const seg = maskSegments(normalizeMaskShape(a.accountNo));
    if (!seg || !seg.tail || seg.tail === suffix || !suffix.endsWith(seg.tail)) return false;
    return accountFit(a.accountNo, pa.masked) === 'ambiguous';
  });
  ...
  // 嚴格徑（既有判準，一個位元組不動）：可見前綴也要對（免尾碼碰撞）。
  ...
  const fits = cands.map(a => ({ a, fit: accountFit(a.accountNo, pa.masked) }));
  const strictHit = fits.find(x => x.fit === 'hit');
  if (strictHit) return { hit: strictHit.a, ambiguous: false };
  if (fits.some(x => x.fit === 'ambiguous') || shortTailAmbiguous.length) return { hit: null, ambiguous: true };   // 證明不了＝停手（不更新、不新建）
...
function debitCardPlan(db, parsed) {
  ...
  for (const t of db.transactions || []) {
    ...
    const suf = accountSuffix(masked) || (/^\d{3,}$/.test(masked) ? masked : '');
    if (!suffixes.has(suf)) continue;
...
function txCurrency(db, parsed, tx) {
  ...
  const acc = (db.accounts || []).find(a => {
    ...
    return accountFit(a.accountNo, stmtAcct) === 'hit';
  });
...
function accountNameForTx(db, tx, parsed) {   // 測試出口在檔尾（accountNameForTxForTest）——掛名繞開定存戶的行為要能直測
  ...
  // ⚠️ 全星號遮罩（金融卡）＝沒有前綴可分辨（Grok #494 掃 G3）：多顆同末碼時退回 bySuffix[0]
  //   ＝交易在**匯入當下**就掛到別人名下，之後的 reconcileBankTxAccountNames（歧義不改名）救不回。
  //   分不出＝不掛既有戶、用帳單概要的 autoName（「台新 1234」）——與餘額 ambiguous 同一條紀律。
  const fullStar = /^[*＊]+\d+$/.test(String(tx.acctMasked || ''));
  // ⚠️ 帳單印**完整號**時，「退回只末碼」只准退到**與完整號不衝突、而且自己沒有遮罩**的登記戶（登記的是它的尾段＝
  //   只填了末幾碼，或登記的以它結尾＝前面多了銀行代碼）；登記了另一個完整號、只是末碼相同的戶**不可**被退路撿到
  //   （Codex #504 r1#1：A 戶 90010011223301 在庫、B 戶 90020011223301 的交易被掛成 A）；登記的是遮罩形＝只走
  //   accountFit（r2#1：`9001****3301` 的數字 90013301 是 77777790013301 的尾段、卻不是同一顆）。任一候選「證明不了」
  //   （ambiguous）＝不掛、用 autoName（與餘額停手同一條紀律）。遮罩帳單的退路照舊。
  const fullNo = (!hasStar(stmtAcct) && /^\d{5,}$/.test(digitsOf(stmtAcct))) ? digitsOf(stmtAcct) : '';
  const fits = bySuffix.map(a => ({ a, fit: accountFit(a.accountNo, stmtAcct) }));
  const fallbackOk = (/** @type {any} */ a) => !fullNo || (!hasStar(normalizeMaskShape(a.accountNo)) && (fullNo.endsWith(digits(a)) || digits(a).endsWith(fullNo)));
  const acc = (fullStar && bySuffix.length > 1) ? null
    : (fits.find(x => x.fit === 'hit')?.a
      || ((fullNo && fits.some(x => x.fit === 'ambiguous')) ? null : (bySuffix.find(fallbackOk) || null)));   // 先前綴+末碼、退回只末碼
...
function acctPrefix(masked) {
  ...
  if (star < 0) {                                              // 沒有星號：完整號（至少五碼）＝末四碼以前全部是前綴；其餘（純末碼祖父鍵）＝看不到
    const d = raw.replace(/[\s-]/g, '');
    return /^\d{5,}$/.test(d) ? d.slice(0, -4) : '';
  }
  if (star === 0) return '';                                   // 整串以星號開頭＝看不到前綴
  const head = raw.slice(0, star).replace(/[^0-9A-Za-z]/g, '');
  return (!head || head === 'x') ? '' : head;
}
/** 索引用末碼（疑似重複索引的鍵）：帳號的可見末碼（遮罩＝星號後的數字、完整號＝末四碼、純末碼祖父鍵＝自己）。 @param {string} masked */
function idxSuffix(masked) {
  return accountSuffix(masked) || (/^\d{3,}$/.test(String(masked || '')) ? String(masked) : '');
}
/** 兩個帳號印法「可能是同一顆戶」（疑似重複的前綴否決用）：一邊看不到前綴＝不否決（跨版式印法本來就不同）；
 * 兩邊都是遮罩＝可見前綴全等（既有判準）；兩邊都是完整號＝整串全等；一遮一全＝遮罩逐位覆蓋（`maskCoversFull`，驗寬度）。
 * ⚠️ 疑似重複在畫面上**預設跳過**那幾筆（Codex #504 r2#4）——這把尺寬一分＝真交易少匯一筆，所以與餘額身分那把**同樣嚴**，
 *   不做「頭尾對得上就算」的寬鬆版；代價寫在契約已知取捨 (c′)：同一顆戶在兩種版面印成不同末碼長度（`****162` 對完整號）
 *   時互相認不出。
...
function acctShapesCompatible(a, b) {
  ...
  if (sa && sb) return pa === pb;
  if (!sa && !sb) return digitsOf(a) === digitsOf(b);
  return sa ? maskCoversFull(a, b) : maskCoversFull(b, a);
}
...
function similarTxIndex(db, wanted) {
  ...
  for (const t of db.transactions || []) {
    ...
    set.add(masked);                                         // 存整個印法（不只前綴）——否決要比形狀（acctShapesCompatible）
    idx.set(key, set);
  }
  return idx;
}
