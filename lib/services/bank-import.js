...
function matchAccount(existing, pa, bank) {
  ...
  if (/** @type {any} */ (pa).suffixOnly === true) {
    ...
    const sameBank = cands.filter(a => a.bank === bank);
    if (unproven.length) return { hit: null, ambiguous: true };
    if (sameBank.length > 1) return { hit: null, ambiguous: true };
    return { hit: sameBank[0] || null, ambiguous: false };
  }
...
export function previewBalancesForDb(db, parsed, opts = {}) {
  ...
  for (const pa of parsed.accounts) {
    ...
    seen.add(key);
    const m = cd ? { hit: existing.find(a => a.cdKey === cd.cdKey) || null, ambiguous: false } : matchAccount(existing, pa, bank);
    const acc = m.hit;
  ...
  if (!blocked && ref) {
    const liveKeys = new Set([...cdRows.values()].map(c => c.cdKey));
...
export function applyBalancesToDb(db, parsed, opts = {}) {
  ...
  return { bank: parsed.bank || null, referenceDate: ref, balancesSkipped: false, noAccounts: !(parsed.accounts || []).length, updated, created, skipped, unsupported, createdNames,
 ...
 * ①**存下來的原文欄**（Stage 2 起：`bankSummary`/`bankNote`＝解析器抄下來的原文，一字未改）；
 * ②舊資料沒有那兩欄 ⇒ 從去重鍵 `bankRef` 反解。②是**反解、不是原文**，兩種寫法會還原錯（#497 r1#3 校正——
 *   原本這裡把①寫成「備註含 `|`」，是反的）：
 *   ・**摘要**自己含 `|` ⇒ 切點錯位：摘要只取到第一個 `|` 之前，剩下的整段被歸給備註。
 *   ・**備註**以 `#數字` 結尾 ⇒ 被當成批內出現序剝掉。（備註**中間**含 `|` 反而沒事：摘要之後的段落會整段接回來。）
 *   這兩種就是 Stage 2 存兩欄的理由。
...
function bankRawText(t) {
  ...
  const bank = isB2 ? String(parts[1] || '') : '台新';
  // 判定用 typeof 'string'＝「這兩欄**存在、而且是字串**」，不是 truthy：空字串是**合法原文**
  // （備註欄空白是常態）。⚠️ 誠實劃界兩條，都有考題釘著（test/bank-raw-text.test.js）：
  //   ・typeof 與 truthy 只有在**兩欄都是空字串**時結果才不同（truthy 會整筆掉回②）；一欄有值時同結果。
  //   ・`null` **不算原文**（#497 r1#1）：`FIELD_SCHEMA` 的 'str' 全站接受 null＝清空，所以外部備份／
  //     手改的資料可能帶 null 進來。這裡當成「沒有這兩欄」而退回②反解——那是**退化、不是說謊**
  //     （②本來就是舊資料走的路，且絕不編造原文）；不為此改動 'str' 的全站語意。
  if (typeof t.bankSummary === 'string' || typeof t.bankNote === 'string') {
    return { summary: String(t.bankSummary || ''), note: String(t.bankNote || ''), bank, ok: true };
  }
...
export async function reconcileAccountNamesAuto() {
  ...
  for (const t of db.transactions || []) {
    ...
    // ⚠️ 這一段**刻意仍讀 bankRef 反解**、不改讀 Stage 2 的原文欄：這裡要重現的是「當年那段程式產出過的字串」，
    //    而當年產出它的就是這個反解——改用原文欄會在**摘要含 `|`**（或備註以 `#數字` 結尾）的列算出另一個字串，
...
export function importBankTxToDb(db, parsed, opts = {}) {
  ...
  for (const tx of parsed.transactions || []) {
    ...
    (db.transactions ||= []).push({
      ...
      // 帳單原文分兩欄存（Stage 2，使用者定 2026-08-22）：摘要與備註各自留底、一字未改。
      // note 是**顯示用**的組合結果（過濾器好讀版或使用者自訂名），改了就回不去原文；
      // 在此之前唯一的原文留底是去重鍵 bankRef 的尾兩段，而那要靠切 `|` 反解——**摘要含 `|`**（切點錯位）
      // 或**備註以 `#數字` 結尾**（被當出現序剝掉）就還原錯。**去重鍵與學習鑰匙仍用 tx.summary/tx.note 原文計算、格式一個位元組沒動**
      // （動了＝重匯同帳單認不出重複＝現金流翻倍）；這兩欄純粹是留底，不參與任何判定。
...
export async function previewBankStatement(b64, password, parse = parseBankStatement, opts = {}) {
  ...
  return { ...previewBalancesForDb(db, parsed, { deterministic: true }), transactions: previewBankTxForDb(db, parsed), reconcile };   // 模板路線＝唯一的確定性解析
}
