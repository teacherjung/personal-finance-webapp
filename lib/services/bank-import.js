 ...
 * ②舊資料沒有那兩欄 ⇒ 從去重鍵 `bankRef` 反解。②是**反解、不是原文**：備註自己含 `|`、
 *   或以 `#數字` 結尾時，切回來的就不等於原文（切點與出現序都會誤判）——那正是 Stage 2 存兩欄的理由。
...
function bankRawText(t) {
  ...
  // 判定用 typeof 'string'＝「這兩欄**存在**」，不是 truthy：空字串是**合法原文**（備註欄空白是常態）。
  // ⚠️ 誠實劃界：兩種寫法只有在**兩欄都是空字串**時結果才不同（truthy 會整筆掉回②、拿去重鍵裡的字
  // 冒充原文）；一欄有值時兩者同結果。釘住這個差別的是 test/bank-raw-text.test.js 的探針考題。
...
export async function reconcileAccountNamesAuto() {
  ...
  for (const t of db.transactions || []) {
    ...
    //    而當年產出它的就是這個反解——改用原文欄會在備註含 `|` 的舊列算出另一個字串，反而認不出自動名（誤判成使用者自訂、不再升級）。
    const parts = String(t.bankRef || '').split('|');
...
export function importBankTxToDb(db, parsed, opts = {}) {
  ...
  for (const tx of parsed.transactions || []) {
    ...
    (db.transactions ||= []).push({
      ...
      // 在此之前唯一的原文留底是去重鍵 bankRef 的尾兩段，而那要靠切 `|` 反解——備註含 `|`
      // 或以 `#數字` 結尾就切錯。**去重鍵與學習鑰匙仍用 tx.summary/tx.note 原文計算、格式一個位元組沒動**
