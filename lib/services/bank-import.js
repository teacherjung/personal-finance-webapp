...
export async function recipeBankRoute(b64, password, db, seams = {}) {
  ...
  // ⚠️⚠️ **兩趟掃描**（Codex #523 r10）：「哪一張規則卡獲勝」也是一種**「是哪一個」**。
  //   #523 讓 `recipeMatches` 收「舊尺或新尺」之後，候選集合變大 ⇒ 這裡的「照列序取第一張成功的」
  //   會選到 **main 選不到的卡**。實測：帳單印連字 `Oﬃce`，一張卡的暗號寫正規字 `Office`、另一張
  //   寫原文 `Oﬃce`，兩張都能完整解析且都過強閘 ⇒ 依列序選到前卡 ⇒ 真台幣戶被標成 `USD` ⇒
  //   那戶的交易被當外幣**不匯入** ⇒ **現金流漏帳**，而閘仍是 `strong`。
  //   ⇒ **第一趟只用舊尺比對版面（逐字＝main：main 選誰我們就選誰）；一張都沒中才跑第二趟用聯集。**
  //   這樣「絕不比 main 差」是**建構上成立**的：第二趟只在 main 本來就會退 AI 時才發生。
  //   ⚠️ 「沒中」指的是**整趟跑完都沒有 hit**（含拒解與閘紅退下一張），不是只看第一張。
  for (const pass of /** @type {const} */ (['old', 'either'])) {
  for (const row of rows) {
    ...
    for (const usedVersion of /** @type {const} */ (['current', 'previous'])) {
      ...
      try {
        if (!recipeMatches(lines, recipe, pass === 'old' ? { ruler: 'old' } : {})) continue;
