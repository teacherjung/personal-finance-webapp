...
import { recipeMatches, parseWithRecipe } from '../parse-recipe.js';   // 配方快取（P2-2）：同版式 app 內解、零元零外送
 ...
 * @returns {Promise<{hit: {parsed: ParsedBankFull, reconcile: any, recipeId: string, usedVersion: 'current'|'previous'}|null, gateFailedIds: string[]}>}
 */
  ...
  for (const row of rows) {
    ...
    for (const usedVersion of /** @type {const} */ (['current', 'previous'])) {
      ...
      try {
        ...
        return { hit: { parsed, reconcile, recipeId: String(row.id), usedVersion }, gateFailedIds };
      } catch { continue; }   // 閘紅＝退上一版（迴圈自然做到）
    }
    if (matchedThisRow && row?.id) gateFailedIds.push(String(row.id));   // 中版面但沒有任何一版過閘
  }
 ...
 * @param {any} db @param {{id:string, usedVersion:'current'|'previous'}} use
 */
export function recordRecipeApplied(db, use) {
  const row = (Array.isArray(db?.parseRecipes) ? db.parseRecipes : []).find((/** @type {any} */ r) => String(r?.id) === use.id);
  if (!row) return;   // 配方在 preview 與 apply 之間被移除＝計數沒對象，匯入本身照常
...
/** apply（AI 票路線）寫入成功前標記「預覽時中版面但閘紅」的配方＝疑似過期（裁示②；同交易原子寫）。
 * @param {any} db @param {string[]} ids */
export function markRecipesSuspect(db, ids) {
  if (!ids?.length) return;
  const now = new Date().toISOString();
  for (const row of (Array.isArray(db?.parseRecipes) ? db.parseRecipes : [])) {
    if (ids.includes(String(row?.id))) { row.suspect = true; row.graduateStreak = 0; row.graduated = false; row.updatedAt = now; }
  }
}
...
export async function previewBankStatement(b64, password, parse = parseBankStatement, opts = {}) {
  ...
  try {
    ...
    if (/** @type {any} */ (e)?.code !== 'pdf_password') {
      ...
      if (rec.hit) {
        return { ...previewBalancesForDb(db, rec.hit.parsed), transactions: previewBankTxForDb(db, rec.hit.parsed),
          reconcile: rec.hit.reconcile, engine: 'recipe', recipeId: rec.hit.recipeId };
      }
    }
    ...
    const aiTicket = issueAiTicket({ parsed: r.parsed, aiModel: r.aiModel, suspectRecipeIds: recipeGateFailedIds });
 ...
 * @param {string} b64 @param {string=} password @param {typeof parseBankStatement} [parse] 測試接縫（見上）
 * @param {{useAi?:boolean, aiTicket?:string, skipSimilar?:boolean, aiEngineFactory?:(key:string)=>AiEngine, aiExtract?:typeof extractBankLines}} [opts] */
export async function applyBankStatement(b64, password, parse = parseBankStatement, opts = {}) {
  ...
  /** @type {{id:string, usedVersion:'current'|'previous'}|null} */ let recipeUse = null;   // P2-2
  ...
  if (ticket) {
    ...
    try {
      ...
      if (/** @type {any} */ (e)?.code !== 'pdf_password') {
        ...
        if (rec.hit) { parsed = rec.hit.parsed; recipeUse = { id: rec.hit.recipeId, usedVersion: rec.hit.usedVersion }; }
      }
      if (!recipeUse) {
        if (!aiEligible(e, opts)) throw e;
  ...
  try {
    ...
    if (recipeUse) recordRecipeApplied(db, recipeUse);
    if (ticket?.suspectRecipeIds?.length) markRecipesSuspect(db, ticket.suspectRecipeIds);
    await saveDb(db);
    return { ok: true, ...bal, transactions: tx, ...(ai || {}), ...(recipeUse ? { engine: 'recipe', recipeId: recipeUse.id } : {}) };
  } catch (e) {
