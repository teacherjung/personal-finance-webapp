...
function maturedCdAccounts(accounts, parsed, bank, liveKeys, ref) {
  ...
  return (accounts || []).filter(a => {
    ...
    const endIso = end ? end.replace(/\//g, '-') : '';
    if (!isRealDate(endIso)) return false;                     // 讀不出迄日＝不歸零（fail-safe）
    return ref > endIso;                                       // 現值參考日已過迄日
  });
}
  ...
  for (const pa of parsed.accounts) {
    ...
    if (acc) {
      ...
      (db.accounts ||= []).push({
        ...
        // cdKey（分開列管）：定存帳戶的身分鍵——之後每期帳單憑它精確更新這一筆；到期後帳單不再印
        // ＝這顆帳戶**不自動歸零也不刪**（錢的紀錄寧可留著給使用者看見、名稱帶著期別一眼可辨）。
