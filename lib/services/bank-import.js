...
/** 帳號印法的**語言記號**：可見字元＝字面、每一段星號＝「至少一個任意字元」（合併連續星號）；`x****3302`（bankRefBase 對無遮罩列
 * 補的佔位：`x`＋星號開頭）的 `x` 不是字面。完整號＝全字面。 @param {string} acct @returns {string[]} */
function acctTokens(acct) {
  let body = cleanAcct(acct);
  if (/^x\*/.test(body)) body = body.slice(1);
  /** @type {string[]} */ const out = [];
  for (const ch of body) { if (ch === '*') { if (out[out.length - 1] !== '*') out.push('*'); } else out.push(ch); }
  return out;
}
 ...
 * 完整號、`9001**11**3301` 對 `9001**9911**3301` 其實同時描述 `90015991173301`）。 @param {string} a @param {string} b */
function acctPatternsIntersect(a, b) {
...
/** 印法是不是「只比末碼」的那種：單一段星號開頭、後面只有末碼（全星號 `****3301`、佔位 `x****3302`）——帳單印成這樣＝除末碼外
 * 什麼都看不到（多段的 `****22**3301` 不算，它露了中段、要逐段比）。 @param {string} acct */
const suffixOnlyShape = (acct) => /^x?\*+[0-9A-Za-z]+$/.test(cleanAcct(acct));
/** 登記帳號對帳單帳號的身分判定（末碼由呼叫端先比）：`'hit'`＝證明是同一顆／`'no'`＝不是／`'ambiguous'`＝相容但證明不了。
 ...
 * 判定順序（先到先算）：
 * ①比對形**全等**＝hit（任何形狀）。
 * ②帳單是「只比末碼」的形（`****3301`／`x****3302` 佔位）＝hit——除末碼外什麼都看不到、末碼對得上就算（既有語意）；
 *   **多顆同末碼由裁決器停手**。多段的 `****22**3301` 不算這種，往下逐段比。
...
function accountFit(accountNo, stmtAcct) {
  ...
  if (Sc === Rc) return 'hit';
  if (suffixOnlyShape(Sc)) return 'hit';
...
function matchAccount(existing, pa, bank) {
  ...
  const shortTailCands = hasStar(pa.masked) ? [] : existing.filter(a => {
    ...
    const tail = (cleanAcct(a.accountNo).match(/\*([0-9A-Za-z]+)$/) || [])[1] || '';
    return !!(tail && tail !== suffix && suffix.endsWith(tail));
  });
 ...
 * 已知取捨 (d′)：台新若印完整號、走 AI 路線，那些列是 `bank|完整號|…`＝當祖父末碼讀（整串當末碼）＝對遮罩列的疑似重複／金融卡擋門
 * 認不出（寧可認不出，也不可把祖父鍵冒充成另一顆戶）。
