...
function matchAccount(existing, pa, bank) {
  ...
  // ⚠️ 帳單自己只有末碼（簽帳金融卡明細 pa.suffixOnly＝prefix 必空）＝整條都是寬鬆比對——
  //    **唯一命中才配**：多顆同末碼時挑第一顆＝把餘額蓋到別人頭上（Codex #492 r1 點過 find 取
  //    第一不安全）。歧義照實回報，呼叫端顯示「這次不動」、不 create（create＝第三顆戶）。
  if (/** @type {any} */ (pa).suffixOnly === true && cands.length > 1) return { hit: null, ambiguous: true };
  ...
  // **suffixOnly 戶**（accountNo 只有末四碼＝嚴格徑的 startsWith 必掛）——同銀行＋同幣別＋末碼
  // 唯一命中＝同一顆帳戶，呼叫端會把帳號**補登**成完整遮罩並清掉標記（帳號資訊只增不減）。
  if (prefix) {
    const loose = cands.filter(a => a.accountNoSuffixOnly === true);
    if (loose.length === 1) return { hit: loose[0], ambiguous: false };
    if (loose.length > 1) return { hit: null, ambiguous: true };   // 兩顆 suffixOnly 同末碼＝分不出哪顆，停手
  }
...
export function applyBalancesToDb(db, parsed, opts = {}) {
  ...
  for (const pa of parsed.accounts) {
    ...
    if (acc) {
      ...
      // 金融卡帳單（沒有前綴可給）配到完整號碼戶時，這裡不會動 accountNo（maskedParts 對
      // `**********8791` 取不出 prefix ⇒ 條件不成立）。補登在新舊判定**之前**——帳號不分新舊，
      // 較舊的帳單也載著正確的完整帳號。
...
export function reconcileBankTxAccountNames(db) {
  ...
  for (const t of db.transactions || []) {
    ...
    const acc = matchAccount(accounts, { suffix, masked, currency: 'TWD' }, isB2 ? parts[1] : '台新').hit;   // 銀行匯入只進 TWD（外幣明細不匯入），故一律以 TWD 比對
