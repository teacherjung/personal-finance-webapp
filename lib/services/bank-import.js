...
import { STAGES, makeStageSink } from '../progress-stages.js';   // 上傳進度：後端只推代碼、句子住前端（零插值＝機密機械化）
...
/** 找既有定存戶：先精確比 cdKey；找不到再用**期間盲**鍵配對——但**只在其中一邊的期間是空的**時候
 * （Codex #488 r1#2：同一筆定存這期沒印期間、下期印了，精確比對會裂成第二戶＝總額憑空多一份）。
 * 兩邊期間都有值而不同＝真的是不同期別的定存，不配（那正是分開列管要分的東西）。
 * @param {any[]} existing @param {string} cdKey @returns {any|null} */
function findCdAccount(existing, cdKey) {
  const exact = existing.find(a => a.cdKey === cdKey);
  if (exact) return exact;
  const blind = (/** @type {string} */ k) => { const p = String(k || '').split('|'); if (p.length < 6) return null; const q = [...p]; q[3] = ''; return q.join('|'); };
  const meBlind = blind(cdKey), mePeriod = String(cdKey || '').split('|')[3] || '';
  if (!meBlind) return null;
  // ⚠️ 盲配的三道門（Codex #488 r2#1：只用 find 抓第一個＝**會把已歸零的舊定存復活**，總額 51→102）：
  //   ①**不復活已歸零的戶**（balance 0／已加註到期＝那筆錢已經結束，不可因為新一列缺期間就被認成它）
  //   ②**候選必須唯一**（同銀行/末碼/幣別/本金有兩顆以上＝歧義，機器分不出來就不猜）
  //   兩道任一不成立＝不盲配（回 null＝照舊新建一戶）。⚠️ 誠實劃界：新建也可能讓帳面多一筆，
  //   但那是**看得見的多一戶**（預覽會列、名稱帶期別），而復活已歸零的戶是**看不見的加錢**。
  // ⚠️ 這裡**刻意沒有**「本批已配過」的第三道門：加過、但突變測試證明它到不了——配到之後 cdKey 就被
  //   單向升級成帶期間的鍵，同批第二列不是被「兩邊都有期間」擋掉、就是因為序號 #n 不同而算另一個鍵。
  //   拿掉假保證比留著好看的死程式誠實（家規：護欄不能自己證明自己有在跑）。
  const cands = existing.filter(a => {
    if (!a.cdKey) return false;
    if (!(Number(a.balance) > 0)) return false;
    if (/[（(]已到期[）)]/.test(String(a.name || ''))) return false;
    const theirPeriod = String(a.cdKey).split('|')[3] || '';
    if (mePeriod && theirPeriod) return false;   // 兩邊都有期間＝不同期別，不配
    return blind(a.cdKey) === meBlind;
  });
...
function maturedCdAccounts(accounts, parsed, bank, liveKeys, ref, deterministic) {
  ...
  // AI／配方的 accounts 契約沒有 kind ⇒ annotateCdRows 產出空的 liveKeys ⇒「這期不再印它」恆成立
  // ⇒ 明明還印著的定存會被歸零（審查者可達情境：AI 解同一張帳單、預覽同時出現 create＋mature-zero）。
  // 判準＝本批**有任何一列帶 kind 欄**（模板恆有、AI/配方恆無）；沒有＝不判死活（契約「AI/配方照舊」）。
...
export function previewBalancesForDb(db, parsed, opts = {}) {
  ...
  for (const pa of parsed.accounts) {
    ...
    seen.add(key);
    const acc = cd ? findCdAccount(existing, cd.cdKey) : matchAccount(existing, pa, bank);
...
export function applyBalancesToDb(db, parsed, opts = {}) {
  ...
  for (const pa of parsed.accounts) {
    ...
    if (acc) {
      ...
      acc.balanceAsOf = ref;
      // 期間盲配到的定存戶：把身分鍵升級成資訊較全的那一把（這期帳單印了期間、上期沒印）——
      // 不升級的話每一期都要靠盲配，而盲配只在「有一邊是空的」時成立，總有一天配不到又裂戶。
      // ⚠️ **單向**升級（Codex #488 r2#2）：只在「既有的期間是空的、新的有值」時補上。無條件覆蓋會在
      //   AI 這期漏抄期間時把既有的期間**抹掉**＝到期資訊永久遺失，之後別顆同額定存又可能盲配到它。
      if (cd && acc.cdKey && acc.cdKey !== cd.cdKey
        && !(String(acc.cdKey).split('|')[3] || '') && (String(cd.cdKey).split('|')[3] || '')) acc.cdKey = cd.cdKey;
      updated++;
...
function accountNameForTx(db, tx, parsed) {   // 測試出口在檔尾（accountNameForTxForTest）——掛名繞開定存戶的行為要能直測
  ...
  // ＝交易持久化掛在定存名下。kind 缺席（AI/配方產線）＝視同活存（舊行為）。
