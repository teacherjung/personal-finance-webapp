...
export async function generateRecipeAfterImport(ticket, opts) {
  try {
    ...
    //   ⚠️ 失敗代碼回報**最後一趟實際卡住的那一關**（Codex #523 r12#2）：出生統計是「該放寬哪一關」
    //   的證據來源，而相容字版面在 `'old'` 那一趟必然停在 `recipe_birth_match`（暗號本來就對不上）
    //   ——記第一趟等於把「預期中的 old miss」當成根因，把 new 真正卡住的 parse／statement／reproduce
    //   系統性蓋掉。⇒ 每一趟覆寫，最後留下的就是最後一趟的。
    ...
    for (const ruler of RECIPE_RULERS) {
      ...
      birthFail = fail;   // 覆寫＝留下最後一趟實際卡住的那一關（理由見上）
      actual = null;
    }
