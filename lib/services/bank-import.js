...
/** AI 路線的閘政策（★6）：先過既有對帳閘（不一致＝400），再要求級別必須是強閘。 @param {ParsedBankFull} parsed @param {any} db */
function assertAiBankReconciled(parsed, db) {
  const gate = assertBankReconciled(parsed, db);
  if (gate.level !== 'strong') {
    throw apiError(400, 'AI 讀不到這份帳單的逐筆餘額，翻譯結果無從驗算——照規矩不收（寧可暫時不能匯入，不把沒驗算過的數字記進帳本）。這份請改用手動記帳。', 'ai_weak_refused');
  }
  return gate;
}
...
export async function previewBankStatement(b64, password, parse = parseBankStatement, opts = {}) {
  ...
  try {
    ...
    return { ...previewBalancesForDb(db, r.parsed), transactions: previewBankTxForDb(db, r.parsed), reconcile: r.reconcile, engine: 'ai', aiModel: r.aiModel };
  }
 ...
 * P0.5 池的取用＝解析**前**另一次唯讀 getDb（只組密碼池）；寫入用的整包照舊在解析完成後才讀——
 * 「讀→（閘）→套→寫之間無外部 IO await」的既有不變量原樣保留（P1b-1：AI 的 transport 呼叫也是
 * 外部 IO＝全部發生在寫入用 getDb **之前**；AI 路線在階梯內先用舊 db 過閘、寫入前用 fresh db **重過閘**）。
 * ⚠️ apply 自己重跑池（不是收前端帶回的「中選密碼」）：池內容全是機密、絕不回前端。
 * @param {string} b64 @param {string=} password @param {typeof parseBankStatement} [parse] 測試接縫（見上）
 * @param {{useAi?:boolean, aiEngineFactory?:(key:string)=>AiEngine, aiExtract?:typeof extractBankLines}} [opts] */
export async function applyBankStatement(b64, password, parse = parseBankStatement, opts = {}) {
  ...
  try {
    ...
    if (!aiEligible(e, opts)) throw e;
    const r = await aiBankRoute(b64, password, db0, { engineFactory: opts.aiEngineFactory, extract: opts.aiExtract });   // 全部外部 IO 在此完成
    parsed = r.parsed;
    ai = { engine: 'ai', aiModel: r.aiModel };
  }
