 ...
 * @param {string} [bank] 帳單的開戶機構（stmtBank(parsed)；reconcile 舊格式 bankRef 無機構段時缺席＝不驗機構）
 */
...
function bankAutoNote(t, db) {
  ...
  if ((parts[0] === 'bank' && parts.length >= 8) || (isB2 && parts.length >= 9)) {
    ...
    if (db) return bankDisplayNote(summary, note, { direction: bankDirFromRef(t.bankRef), accountNameOf: (a) => ownAccountNameByAcct(db, a), bank: isB2 ? parts[1] : '台新' });
...
export function previewBankTxForDb(db, parsed) {
  ...
  const rows = (parsed.transactions || []).map(tx => {
    ...
    const displayNote = name || bankDisplayNote(tx.summary, tx.note, { direction: tx.direction, accountNameOf: (a) => ownAccountNameByAcct(db, a), bank });
    return {
...
export function importBankTxToDb(db, parsed) {
  ...
  for (const tx of parsed.transactions || []) {
    ...
    const autoNote = bankDisplayNote(tx.summary, tx.note, { direction: tx.direction, accountNameOf: (a) => ownAccountNameByAcct(db, a), bank });
...
// 驗「不平衡＝400、apply 未寫入任何資料」。正式呼叫端（路由）一律只傳兩參數＝走真解析器。
...
export async function previewBankStatement(b64, password, parse = parseBankStatement) {
  const db = await getDb();   // 密碼池＋閘的幣別補位（P0.1 r1#2）都要它；預覽全程唯讀
  const parsed = await parseWithPool((b, pw) => parse(b, pw), decode(b64), statementPasswordPool(db, password ? [String(password)] : []));
  const reconcile = assertBankReconciled(parsed, db);   // 對不上＝這裡就 400（前端既有錯誤路徑會原句顯示）
 ...
 * 「讀→（閘）→套→寫之間無外部 IO await」的既有不變量原樣保留。
 * ⚠️ apply 自己重跑池（不是收前端帶回的「中選密碼」）：池內容全是機密、絕不回前端。
 * @param {string} b64 @param {string=} password @param {typeof parseBankStatement} [parse] 測試接縫（見上） */
export async function applyBankStatement(b64, password, parse = parseBankStatement) {
  const pool = statementPasswordPool(await getDb(), password ? [String(password)] : []);
  const parsed = await parseWithPool((b, pw) => parse(b, pw), decode(b64), pool);
  const db = await getDb();   // PDF 解析（外部 IO）完成後才讀整包；讀→（閘）→套→寫之間無外部 IO await
  assertBankReconciled(parsed, db);   // 寫入路徑 fail-closed——閘一定在任何寫入之前
