...
import { RECIPE_MODEL, pickRecipeCandidate, AI_ARBITER_MODEL, dualReadWanted, aiAnswersAgree, aiDiffSummary } from '../ai-parse.js';
...
function maturedCdAccounts(accounts, parsed, bank, liveKeys, ref) {
  // r1#1【高】**適用前提＝這批 accounts 真的帶得出定存結構**（模板路線才有 kind/period）：
...
export function previewBalancesForDb(db, parsed) {
  ...
  for (const pa of parsed.accounts) {
    ...
    seen.add(key);
    const acc = cd ? (existing.find(a => a.cdKey === cd.cdKey) || null) : matchAccount(existing, pa, bank);
  ...
  if (!blocked && ref) {
    ...
    for (const a of maturedCdAccounts(existing, parsed, bank, liveKeys, ref)) {
...
export function applyBalancesToDb(db, parsed) {
  ...
  for (const a of maturedCdAccounts(db.accounts || [], parsed, bank, liveKeys, ref)) {
...
export async function aiBankRoute(b64, password, db, seams = {}) {
  ...
  const extract = seams.extract || extractBankLines;
  stage(STAGES.AI_START);   // ⚠️ 只在 HOSTED 停止線／鑰匙／引擎三道都過之後才推（那三道零 AI 呼叫，提早推＝#455 那型假進度）
  ...
  if (valid.length === 2) {
    ...
    if (cmp.agree) { const w = valid[1]; return { parsed: w.parsed, reconcile: w.reconcile, aiModel: w.model, lines, dualRead: /** @type {const} */ ('agree'), ...(cmp.textVariance.length ? { dualReadTextVariance: cmp.textVariance } : {}) }; }
  }
  // 不一致（或只有一讀有效＝另一讀掛掉也算「沒有兩份互證」）＝送 Fable 獨立第三讀仲裁（裁示⑦a）。
  stage(STAGES.AI_ARBITRATE);
  ...
  if (!arb.ok) {
    ...
    if (matches.length === 1) {
      ...
      const w = matches[0];
      const tv = valid.length === 2 ? aiAnswersAgree(valid[0].parsed, valid[1].parsed).textVariance : aiAnswersAgree(w.parsed, /** @type {any} */ (arb).parsed).textVariance;
...
export async function previewBankStatement(b64, password, parse = parseBankStatement, opts = {}) {
  ...
  /** @type {ParsedBankFull} */ let parsed;
  try {
    stage(STAGES.OPEN_PDF);        // 開檔＋逐一試已存密碼（**不報第幾組／共幾組**——池大小是機密）
    stage(STAGES.TEMPLATE_TRY);
    ...
    if (/** @type {any} */ (e)?.code !== 'pdf_password') {
      stage(STAGES.RECIPE_TRY);   // 零費用、內容不外送（規則卡張數是使用者側寫＝不報）
      const rec = await recipeBankRoute(b64, password, db, { extract: opts.aiExtract });
      stage(rec.hit ? STAGES.RECIPE_HIT : STAGES.RECIPE_MISS);
    ...
    return { ...previewBalancesForDb(db, r.parsed), transactions: previewBankTxForDb(db, r.parsed), reconcile: r.reconcile, engine: 'ai', aiModel: r.aiModel, aiTicket, ...(/** @type {any} */ (r).dualRead ? { dualRead: /** @type {any} */ (r).dualRead } : {}), ...(/** @type {any} */ (r).dualReadTextVariance ? { dualReadTextVariance: /** @type {any} */ (r).dualReadTextVariance } : {}) };
  }
  stage(STAGES.VERIFY);
