...
export async function listParseRecipes() {
  ...
  return (Array.isArray(rows) ? rows : []).map((/** @type {any} */ r) => ({
    id: String(r?.id ?? ''), bank: String(r?.bank || ''), kind: r?.kind === 'card' ? 'card' : 'bank',   // 批四：面板分得出信用卡卡
...
export async function recipeBankRoute(b64, password, db, seams = {}) {
  ...
  // 批四：櫃子共用、路由分流——kind==='card' 的列是信用卡規則卡，不進銀行路（不濾的話每次銀行
  // 匯入都會拿卡片卡試一輪＝被 validateRecipeStrict 拒掉再吞成 miss，浪費且會誤進疑似過期名單）
  const rows = rowsAll.filter((/** @type {any} */ r) => /** @type {any} */ (r)?.kind !== 'card');
  ...
  //   ⚠️ 保證的精確範圍（Codex #523 r14）：old 有 hit 的輸入＝與 main 逐字相同（結構保證）。
  //   old 沒 hit 時 main 會**退給 AI**——`'new'` 命中＝以卡代 AI＝P2 既有取捨的延伸暴露
  //   （William 2026-08-29 裁示「甲」接受；碰撞卡＋強閘盲區那條鏈在 main 上對舊尺命中同樣存在）。
  //   ⚠️ 「沒中」指**整趟**（含拒解與閘紅退下一版／下一張），不是只看第一張。
  //   ⚠️ 每一趟內部**從頭到尾同一把尺**（比對／驗證／解析全部傳同一個 `ruler`）——混用是那五輪的病根。
  // ⚠️ `currentMatched` 要記**整個 route 對這一列的觀測**、不是只記勝出那一趟的（Codex #523 r12#1）：
  //   放在 ruler 迴圈裡的話，「第一趟 current 中版面但失靈、第二趟用 previous 跨尺救回」會在第二趟
  //   被重設成 false ⇒ apply 走「只是別版面服役」分支 ⇒ **壞的 current 永遠升不掉、last-known-good
  //   永遠升不上去、也永遠無法重新畢業**（實測：兩趟每期重演、streak 卡住不動）。
  /** @type {Map<string, boolean>} 列 id → 「任何一趟裡 current 曾經中版面」 */
  const currentMatchedByRow = new Map();
  ...
  for (const row of rows) {
    ...
    for (const usedVersion of /** @type {const} */ (['current', 'previous'])) {
      ...
      try {
        ...
        return { hit: { parsed, reconcile, recipeId: row.id, usedVersion, currentMatched: currentMatchedByRow.get(row.id) === true, usedRecipe: JSON.parse(JSON.stringify(recipe)) },
          gateFailedIds: gateFailedIds.filter((id) => id !== row.id) };
      } catch { continue; }   // 閘紅＝退上一版（迴圈自然做到）
    }
    if (currentMatchedByRow.get(row.id) === true && !gateFailedIds.includes(row.id)) gateFailedIds.push(row.id);   // **現行版**中版面但整列沒過＝疑似過期候選（只看 previous 失靈不算——它本來就是備胎）；兩趟可能重複標同一列
  }
  }
 ...
 */
export function recordRecipeApplied(db, use, ownKind = 'bank') {
  ...
  if (!row) return;
  // 鎖同櫃（#532 r3#3；同 markRecipesSuspect/saveParseRecipe）：preview→apply 間同 id 換代成
  // 另一櫃的列＝不可跨櫃改對方的畢業/版本狀態；計數靜默跳過、匯入照常。
  if (((/** @type {any} */ (row)).kind === 'card' ? 'card' : 'bank') !== ownKind) return;   // 配方在 preview 與 apply 之間被移除＝計數沒對象，匯入本身照常（守衛同時證明票鎖住了 parsed）
...
export function markRecipesSuspect(db, ids, snapshotAt, ownKind = 'bank') {
  ...
  for (const row of (Array.isArray(db?.parseRecipes) ? db.parseRecipes : [])) {
    ...
    // 鎖同櫃（#532 r2#4）：preview 與匯入之間若備份還原/換代讓同 id 變成**另一櫃**的列，
    // 跨櫃清洗會把對方的畢業狀態歸零——各線只動自己那一櫃（缺席 kind＝bank）。
    if ((row.kind === 'card' ? 'card' : 'bank') !== ownKind) continue;
 ...
 */
/** 出生三關的**深度**（`generateRecipeAfterImport` 兩趟取最深的失敗碼——理由見該處註解）。
 * 順序＝真實的通過順序：零內容驗證 → 認得版面 → 解得動 → 對照出生帳單 → 逐欄重現。 */
const BIRTH_GATE_DEPTH = /** @type {Record<string, number>} */ ({
  recipe_birth_strict: 1, recipe_birth_match: 2, recipe_birth_parse: 3,
  recipe_birth_statement: 4, recipe_birth_reproduce: 5,
});
...
export async function generateRecipeAfterImport(ticket, opts) {
  try {
    ...
    //   ⚠️ 失敗代碼回報兩趟裡**走得最深的那一關**（Codex #523 r12#2→r13#1 兩輪換來的）：
    //   出生統計是「該放寬哪一關」的證據來源，兩個方向都會出錯——
    //   ・記**第一趟**：相容字版面在 `'old'` 必然停在 `recipe_birth_match`（暗號本來就對不上），
    //     把 new 真正卡住的 parse／statement／reproduce 蓋掉。
    //   ・記**最後一趟**：反方向也可達——暗號是「A＋U+030A」印在相鄰兩格時，old 一路走到 parse
    //     才因表頭錯誤失敗，new 卻在第一關就因 NFKC 合成成單一字元 `Å` 而 strict 太短
    //     ⇒ 統計會指向「放寬 minLen」，但 old 早就通過那道安全門，真正要處理的是 parse。
    //   ⇒ 取**最深**：淺層失敗不可以蓋掉更可行動的根因。
    ...
    for (const ruler of RECIPE_RULERS) {
      ...
      if (birthFail === null || BIRTH_GATE_DEPTH[fail] > BIRTH_GATE_DEPTH[birthFail]) birthFail = fail;   // 取最深（理由見上）
      actual = null;
    }
...
export async function previewBankStatement(b64, password, parse = parseBankStatement, opts = {}) {
  ...
  try {
    ...
    if (/** @type {any} */ (e)?.code !== 'pdf_password' && /** @type {any} */ (e)?.code !== 'bank_mixed_currency') {   // 混台外幣＝版面歧義，配方路的答案一樣是拒收
      ...
      const hasRecipes = Array.isArray(db?.parseRecipes) && db.parseRecipes.some((/** @type {any} */ r) => /** @type {any} */ (r)?.kind !== 'card');   // 批四：卡片卡不算銀行的（進度碼別謊報 RECIPE_TRY）
...
export async function applyBankStatement(b64, password, parse = parseBankStatement, opts = {}) {
  ...
  if (ticket && ticket.aiKind === 'card') {
    // 票綁用途要**兩端**都鎖（#532 Grok 掃#6）：卡片端對異種票「先還再拒」已有；銀行端先前
    // 靠「卡片 parsed 沒有餘額鏈⇒AI 級閘 weak 拒收」的巧合擋住——那不是 kind 鎖，形狀哪天
    // 變了就穿，而且錯誤訊息會誤導（說「讀不到逐筆餘額」而不是「票開錯用途」）。
    if (typeof opts.aiTicket === 'string') restoreAiTicket(opts.aiTicket, ticket);
    throw apiError(400, '這張預覽憑證是信用卡帳單開的，不能用來匯入銀行帳單——請重新預覽一次。', 'ai_ticket_invalid');
  }
