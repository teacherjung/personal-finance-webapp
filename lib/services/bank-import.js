...
import { parseBankStatement, extractBankLines, accountSuffixAny as accountSuffix, DEBIT_CARD_SUMMARIES, linkDebitCardRows } from '../bank-statement.js';   // 末碼一律寬版（AI 路線可交沒遮罩的完整帳號；有星號的行為與 accountSuffix 相同）
...
function maskedParts(masked) {
  ...
  //   嚴格徑照樣「前綴＋末碼都要對」（等於整串都要對），不會比遮罩帳號寬。
...
/** 登記帳號的可見前綴與帳單的可見前綴「對得上」（末碼由呼叫端先比）。
 * ①登記的是完整號或更長的可見前綴＝它以帳單前綴開頭（既有判準）。
 * ②登記的是**遮罩形**（自動建戶存的 `900100****3301`）、帳單這次印得更多（完整號 `90010011223301`、
 *   或露更多碼的遮罩）＝帳單前綴以登記的可見前綴開頭也算對得上——不然同一顆戶只因為帳單換了印法就裂戶。
 *   ⚠️ 只放給**帳號本身帶星號**的登記戶；使用者親手填的完整帳號不走②（完整對完整＝整串都要對）。
 *   已知取捨：遮罩藏掉的那幾碼本來就比不到，②與「兩份遮罩帳單互比」的寬度相同、沒有更寬。
 * @param {string} accountNo 登記帳號 @param {string} prefix 帳單可見前綴（空＝帳單沒前綴可比＝不擋） */
function prefixFits(accountNo, prefix) {
  if (!prefix) return true;
  const raw = String(accountNo || '');
  if (raw.replace(/\D/g, '').startsWith(prefix)) return true;
  const reg = /[*＊]/.test(raw) ? maskedParts(raw).prefix : '';
  return !!reg && prefix.startsWith(reg);
}
...
function matchAccount(existing, pa, bank) {
  ...
  const strictHit = cands.find(a => prefixFits(a.accountNo, prefix));
...
function txCurrency(db, parsed, tx) {
  ...
  const acc = (db.accounts || []).find(a => {
    ...
    return prefixFits(a.accountNo, prefix);
  });
...
function accountNameForTx(db, tx, parsed) {   // 測試出口在檔尾（accountNameForTxForTest）——掛名繞開定存戶的行為要能直測
  ...
  const acc = (fullStar && bySuffix.length > 1) ? null
    : (bySuffix.find(a => prefixFits(a.accountNo, prefix)) || bySuffix[0]);   // 先前綴+末碼、退回只末碼
