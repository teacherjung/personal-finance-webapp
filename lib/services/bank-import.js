...
export async function generateRecipeAfterImport(ticket, opts) {
  try {
    ...
    let rebirthId = ticket.suspectRecipeIds?.[0];
    if (rebirthId) {
      // W5：重生也吃世代檢查（A4 同理）——候選列其後已自證＝不降它的版、改走新建
      const target = (Array.isArray(db.parseRecipes) ? db.parseRecipes : []).find((/** @type {any} */ r) => r?.id === rebirthId);
      if (target && typeof target.lastUsedAt === 'string' && typeof ticket.issuedAt === 'string' && target.lastUsedAt > ticket.issuedAt) rebirthId = undefined;
    }
...
export async function applyBankStatement(b64, password, parse = parseBankStatement, opts = {}) {
  ...
  // P2-3（W4 結構保證）：走到這裡＝saveDb 已成功、票已消耗——生成的 await 在 try/catch **之外**，
  // 它再怎麼失敗都不可能觸發票放回或把成功的匯入報成失敗（不靠「函式自己不會 reject」的軟前提）。
  const recipeGen = (ai && ticket) ? await generateRecipeAfterImport(ticket, opts) : null;
