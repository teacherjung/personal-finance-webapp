...
async function throughSecQueue(fn, opts, guard) {
  ...
  const run = secQueue.then(async () => {
    try {
      // 總時限：輪到自己時才發現預算（含排隊等待）已耗盡＝不再發出這個請求。
      // 逾時可歸因於 SEC 慢（branded）＝記入 lastError 是對的、使用者看得到原因。
      if (guard && opts.now() > guard.deadlineAt) {
...
async function fetchSecResource(url, stage, opts, accept, readResponse, deadlineAt) {
  ...
  for (let attempt = 0; attempt < MAX_ATTEMPTS; attempt += 1) {
    ...
    try {
      ...
      }, opts, deadlineAt ? { deadlineAt, stage } : undefined);
      ...
      // ①branded＋可重試（fetch 連線失敗）→ 重試；②branded 不可重試（HTTP/讀取層已定性）→ 原樣丟；
      // ③abort（我們自己的計時器）→ 重試後定性為逾時；④**其他一切＝內部例外，原樣外拋**——
      // 舊版在這裡替它們補上 stage 包成 sec_network_error，內部 bug 因此穿上 SEC 外衣。
...
async function fetchOfficialFundamentals(symbol, opts) {
  ...
  const [submissions, companyFacts] = await Promise.all([
    fetchSecJson(`${SEC_SUBMISSIONS_ROOT}/CIK${ticker.cik}.json`, 'submissions', opts, deadlineAt),
    fetchSecJson(`${SEC_FACTS_ROOT}/CIK${ticker.cik}.json`, 'company-facts', opts, deadlineAt)
  ]);
