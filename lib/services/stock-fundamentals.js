...
async function throughSecQueue(fn, opts, guard) {
  ...
  const run = secQueue.then(async () => {
    try {
      // 輪到自己時預算已耗盡＝不再發出這個請求（r2：pacing sleep 後再驗一次——
      // sleep 本身可能把時間推過線，驗在 sleep 前等於沒驗）
  ...
  if (!guard) return run;
  // r2（Codex #361 r1 blocking 1）：排隊「等待者」也要有硬期限——只在輪頭檢查的話，
  // 深度 16×每支 10 秒＝尾端要等 150 秒才知道逾時，「最多 60 秒」是假話。
  // race＋可清除計時器：到點就對呼叫端回 sec_timeout；底下的排隊任務照常輪到、
  // 輪頭檢查會跳過 fetch 並在 finally 釋放名額（深度守恆不受 race 影響）。
