...
import { getDb, saveDb, snapshotTo, dataDir, backupNow, backupSupported } from '../repo.js';
...
/**
 * 不可逆整批操作前的**安全網閘門**（唯一實作，呼叫點不要各自寫一份）。
 *
 * 🧑‍⚖️ William 2026-08-06 裁決：**備份沒做成就不要動資料**——先擋下來、把話講明白，
 * 讓使用者自己決定要不要在沒有還原檔的情況下繼續。在那之前這裡是「失敗只寫一行 console.warn、
 * 照樣往下做」，而畫面上仍然只顯示儲存成功——使用者是在按下**不可逆**的整批改名之前
 * 讀到「會自動備份」那句話的，安全網斷了卻沒人告訴他，是最糟的組合。
 * （呼應本領域既有的同款判準：`docs/contracts/income-expense.md`「預覽失敗時不可繼續儲存
 *  ——安全帶斷了就該停車」。）
 *
 * ⚠️ **HOSTED 直接放行**，不是遺漏：雲端版**設計上就沒有**操作前備份（`lib/repo.js` 的
 * `backupNow` 一律回 false，理由見該處），把它當成「備份失敗」會讓雲端使用者每一次存規則
 * 都被擋下來、而且永遠過不去。雲端那一半靠**文案**誠實交代（設定頁與店名規則面板都寫著
 * 「只有本機版才有那一份」），不靠這道閘門。
 * ⚠️ 所以**回 false 一定是「本機版真的失敗了」**——呼叫端可以直接照這個語意對使用者說話。
 * ⚠️ 本函式**會真的做一次備份**（不是只探測）：成功就順手把安全網架好，失敗才回 false。
 *
 * @param {string} tag 備份用途標記（同 `backupNow`；一個不可逆操作一個 tag，勿共用）
 * @returns {boolean} true＝可以動手（安全網備妥，或這個模式本來就沒有這一份）
 */
export function backupBeforeIrreversible(tag) {
  if (!backupSupported()) return true;
  return backupNow(tag);
}
