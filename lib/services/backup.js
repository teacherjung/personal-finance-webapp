 ...
 * @returns {{ran:boolean, created:boolean, date:string, file:string|null, pruned:string[], prunedFailed:string[], failStreak:number, error:string, dir:string}}
 */
export function dailyBackupIfDue(today) {
  ...
  try {
    ...
    writeStatus({ lastBackupDate: date, backupFailStreak: 0, backupLastError: '', backupLastErrorAt: '' });
    ...
    writeStatus({ backupFailStreak: failStreak, backupLastError: message, backupLastErrorAt: new Date().toISOString() });
