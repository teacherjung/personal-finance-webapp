...
//   - 操作前 `<tag>.bak`（store.js backupNow）＝**正式操作路徑零呼叫**（William 2026-08-08 拿掉那一層，見下方裁決註解）。
...
/**
 * 🧑‍⚖️ **這裡刻意沒有「不可逆操作前的自動備份」**（William 2026-08-08 裁決，取代 08-06 的
 * 「擋下＋讓使用者確認」）。曾經有一支 `backupBeforeIrreversible(tag)` 住在這裡，連同
 * `pre-rules`／`pre-normalize` 兩顆 `.bak` 一起拿掉了。
 *
 * **為什麼拿掉**：那層網會自己失敗，而失敗時畫面仍說「儲存成功」——2026-08-06 先改成
 * 「擋下＋問使用者」，但那讓一個單純的操作長出兩個確認框、兩個旗標、三處文案。
 * William 讀完完整拆解後選擇整層移除：畫面只誠實寫「儲存後沒有「復原」可以按」，不承諾任何自動還原檔。
 *
 * **使用者剩下的救援手段**（都還在，沒有一起拿掉）：
 *   ①**每日滾動備份 30 天**（本檔上半部）＝唯一能回到「上週三」的那一種；
 *   ②使用者自己按的「匯出備份」；③啟動 `.bak`（每個行程一顆，只保護「這次開機之前」）。
 * ⚠️ 代價是誠實的：救援粒度從「按下去的前一秒」退成「今天第一次開 app」。這是拍板接受的。
 *
 * ⚠️ **看到「不可逆操作沒有備份」想順手補的人請先讀這段**：那是拍板拿掉的、不是漏掉的。
 * 考題 `test/vault-and-backup-integrity.test.js` 有一題釘著這件事（改動前先去看那題）。
 */
