...
/**
 * 目前的登入狀態＋**這位使用者的 Supabase client**（C4b）。未登入回 null。
 * 為什麼要一起回 client：HOSTED 的資料層必須用「帶著這位使用者 JWT 的 client」去查 Postgres，
 * RLS 才有東西可以比對（裁決⑥：一般資料讀寫絕不可用 service_role）。gate 為了驗身分本來就建了
 * 一個，順手交棒比讓資料層自己再建一個更省、也更不容易接錯憑證。
 * **getUser＝到 Supabase 驗簽**，不是只讀 cookie 面值。
 * @param {any} req @param {any} res @returns {Promise<{user: {id:string, email:string}, supabase: any}|null>}
 */
export async function currentSession(req, res) {
  const supabase = supabaseFor(req, res);
  const { data } = await supabase.auth.getUser();
  return data?.user ? { user: projectUser(data.user), supabase } : null;
}

/** 目前使用者（**getUser＝到 Supabase 驗簽**，不是只讀 cookie 面值）。未登入回 null。 @param {any} req @param {any} res */
export async function currentUser(req, res) {
  return (await currentSession(req, res))?.user || null;
}
