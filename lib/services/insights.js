...
// 設計原則（docs/daily-insights-engine-plan.md）：解讀優先於價格、平靜日不造噪音、報喜也報憂、教育不建議、失敗靜默降級。
//
// ⚠️ **書籤更新時機**：`GET /api/insights` 被讀取＝視為「看過了」，當下更新書籤（insightState）。只有總覽會呼叫它。
// ⚠️ **read-await-write**：先 await 外部資料（CAPE/實質利率，market-data 快取、失敗靜默 null），**外部 IO 都完成之後
//    才 getDb→算→saveDb**（中間不夾外部 IO await；C4a 後 getDb/saveDb 本身是 await，但 LOCAL 下只隔 microtask、
//    不讓出事件圈給別的請求，見 repo.js 檔頭）——避免整包 db 跨外部 IO 被別的寫入覆蓋（同 D1 refreshQuotesIfStale / syncIb r3#1）。
...
import { resolveFxTable } from '../../public/modules/fx-rates.js';   // 書籤記「實際採用」的匯率（丙）
...
export async function getInsights(opts = {}) {
  ...
  // 2) 外部 IO 都完成之後才取整包 db（避免跨外部 IO 覆蓋）；之後到 saveDb 為止不夾外部 IO await。
  ...
  // 4) 更新書籤（讀取＝看過了）：roll lastSeenAt→prevSeenAt，存目前快照。同一次寫檔（讀寫之間無外部 IO，db 未過期）。
  ...
  db.insightState = {
    ...
    tiers,
    usdTwd: resolveFxTable(settings).rates.USD,   // 上次看時實際採用的美元匯率（沒抓到＝預設值，不是 null；丙）
  };
  await saveDb(db);
