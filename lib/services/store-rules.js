...
import { origFromStmtRef, cleanStore, normalizeStoreDisplay } from '../statement.js';
 ...
 * @param {any} rules 已 sanitize 的規則 @returns {string[]} 問題訊息（空＝冪等）
 */
export function checkRulesIdempotent(rules) {
  return withRules(rules, () => {
    ...
    for (const kind of ['canon', 'brand', 'rename']) {
      for (const e of rules[kind] || []) {
        ...
        // brand/rename 用 normalizeStoreDisplay（cleanStore 收尾呼叫的同一條）。
        const again = kind === 'canon' ? cleanStore(to) : normalizeStoreDisplay(to);
        if (again !== to) errs.push(`規則產生的「${to}」還會被其他規則再改成「${again}」——` +
...
export function saveStoreRules(rules) {
  ...
  const idemErrs = checkRulesIdempotent(clean);
  ...
  saveDb(db);
  const r = normalizeIfRulesChanged(true);   // force：UI 儲存前已預覽＋確認過不可逆效果（#133），不再問第二次；內含 getDb()＝規則入櫃檯
