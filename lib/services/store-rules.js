 ...
 *
 * ⚠️ **備份沒做成就不存**（William 2026-08-06 裁決，見 `backupBeforeIrreversible`）：回
 * `{ok:false, needsConfirmation:'backup_failed'}` 且**一個位元組都不寫**，等使用者看過確認、
 * 帶 `proceedWithoutBackup` 再送一次才動手。刻意**不 throw**、走 200＋旗標，是沿用本檔案家族
 * 既有的形狀（`normalizeIfRulesChanged` 的 `{ran:false, needsConfirmation:true}`），前端只要
 * 多認一個 reason，不必去解析錯誤字串。
 * ⚠️ 前端**必須**認得這個回傳（沒認＝畫面顯示「規則已儲存」但其實什麼都沒存＝比原本的病更糟）；
 * 考題 `test/vault-and-backup-integrity.test.js` 第二節有一題盯著前端兩個呼叫端都接了。
 *
 * @param {any} rules
 * @param {{proceedWithoutBackup?: boolean}} [opts] `proceedWithoutBackup`＝使用者已看過
 *   「這次沒存成還原檔」的確認、明確選擇繼續。**與既有的 `force` 是兩件事**（那個是「學習表會被
 *   蓋掉」的確認），刻意不合併成一個旗標：合併的話按掉其中一個問題等於連另一個也一起放行。
 * @returns {Promise<{ok:true, rules:any, changed:number, keyChanged:number, learnedRemapped:number, learnedNamesFixed:number}
 *   | {ok:false, needsConfirmation:'backup_failed'}>}
 */
export async function saveStoreRules(rules, opts = {}) {
  ...
  // ⚠️ 這一步的**回傳值不可以丟掉**：丟掉的話備份失敗時使用者仍然只看到「儲存成功」，
  // 而他正是因為那句承諾才敢按下不可逆的整批改名（本函式 docstring 的裁決）。
  if (!backupBeforeIrreversible('pre-rules') && !opts.proceedWithoutBackup) {
    return { ok: false, needsConfirmation: 'backup_failed' };
  }
  const db = await getDb();
  db.settings = { ...db.settings, storeRules: clean };
  await saveDb(db);
  // ⚠️ 這裡的 `proceedWithoutBackup: true` 是**無條件**的，理由要寫清楚免得被當成漏洞：
  // 走到這一行只有兩種可能——①`pre-rules` 成功＝這次操作**之前**的完整狀態已經存下來了，
  // 安全網就是那一顆，裡層 `pre-normalize` 再失敗也不會讓網消失；②使用者剛剛明確選了
  // 「沒有還原檔也要繼續」。兩種情況都不該為同一個動作再問第二次。
  const r = await normalizeIfRulesChanged(true, { proceedWithoutBackup: true });   // force：UI 儲存前已預覽＋確認過不可逆效果（#133），不再問第二次；內含 getDb()＝規則入櫃檯
