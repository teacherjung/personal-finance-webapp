 ...
 *
 * ⚠️ **這條路刻意沒有「操作前自動備份」**（William 2026-08-08 裁決，取代 08-06 的「擋下＋確認」）：
 * 不產生 `pre-rules.bak`、不擋、不問。畫面只誠實寫「儲存後沒有「復原」可以按」，不承諾任何自動還原檔。
 * 使用者的救援手段＝**每日滾動備份（30 天）** 與**他自己按的「匯出備份」**。
 * ⚠️ 未來看到「不可逆操作沒有備份」想順手補的人請先讀這一段：**那是拍板拿掉的，不是漏掉的**
 * （考題 `test/vault-and-backup-integrity.test.js` 有一題釘著這條路不得再出現操作前備份）。
 *
 * @param {any} rules
