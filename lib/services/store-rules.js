 ...
 * ③**學習表衝突**（`learnedConflicts`）——兩把鑰匙併成一把時，兩邊手動教過的分類只留得下一個，
 *   **把規則刪掉也救不回來**。這是整個自助化唯一不可逆的效果，預覽一定要講出來，
 *   否則使用者看到「4 筆顯示名會變」就按下去，教過的分類默默消失（自審 r3 抓到）。
 * @param {any} rules
...
export function previewStoreRules(rules) {
  ...
  return { changed: r.changed, keyChanged: r.keyChanged, changes: r.changes || [],
    learnedConflicts: r.learnedConflicts || [] };
}
