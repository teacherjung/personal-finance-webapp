 ...
 * @returns {{items: HealthItem[], dismissed: number, allIds: string[]}}
 */
...
export function dismissHealthItem(id, clearAll = false) {
  ...
  if (clearAll) {
    ...
    saveDb(db);
    return { ok: true, dismissed: 0, pruned: Object.keys(db.settings?.healthDismissed || {}).length };
  }
  const k = String(id || '');
  if (!k) throw Object.assign(new Error('缺少項目編號'), { status: 400 });
  const { allIds } = runHealthCheck();
