 ...
 * @returns {{items: HealthItem[], dismissed: number}}
 */
export function runHealthCheck() {
  ...
  for (const [key, g] of byKey) {
    ...
    if (cats.size >= 2) {
      ...
      items.push({ id: `D2|${key}`, type: 'key-mixed', chip: '鑰匙吃錯店', severity: 3,
  ...
  // ---- D7 分期分裂（Apple Xinyi A第03/12期 → 一筆分期 N 把鑰匙）----
  {
    ...
    for (const [base, e] of inst) {
      items.push({ id: `D7|${base}`, type: 'installment', chip: '分期分裂', severity: 2,
  ...
  for (const [orig, o] of byOrig) {
    ...
    items.push({ id: `D4|${orig}`, type: 'cat-drift', chip: '分類漂移', severity: 1,
  ...
  for (const [orig, o] of byOrig) {
    if (isServiceFee(orig)) continue;
    const note = String(o.txs[0]?.note || '');
    const bare = stripDisplayLabels(note);
    const hit = NOISE_PATTERNS.find(([re]) => re.test(bare));
    if (!hit) continue;
    items.push({ id: `D3|${orig}`, type: 'note-noise', chip: '名字有雜訊', severity: 1,
  ...
  return { items: visible, dismissed: items.length - visible.length };
}
 ...
 */
export function dismissHealthItem(id, clearAll = false) {
  const db = getDb();
  ...
  saveDb(db);
  return { ok: true, dismissed: Object.keys(db.settings.healthDismissed || {}).length };
}
