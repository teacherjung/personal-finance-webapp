...
/** @param {any} metric */
function hasMixedAnnualSources(metric) {
  const sources = new Map();
  for (const fact of metric?.annual || []) {
    collectOfficialSources(fact, String(metric?.key || ''), sources);
  }
  return [...sources.values()].some(metricSources => metricSources.size > 1);
}
...
/**
 * 低順位 tag 只能往「已採用序列之後」補，不得回頭填舊洞或改寫同期。
 * @param {any[]} target
 * @param {any[]} candidates
 * @param {string} boundary
 */
function appendNewerPeriods(target, candidates, boundary) {
  let latest = boundary;
  const addedFacts = [];
  for (const fact of candidates) {
    const end = String(fact.periodEnd || '');
    if (!end || end <= latest) continue;
    target.push(fact);
    addedFacts.push(fact);
    latest = end;
  }
  return { latest, added: addedFacts.length > 0, addedFacts };
}
...
function selectMetricFromTags(
  ...
  /** @type {{unit:string,annual:any[],quarters:any[],ytdExcludedKeys:Set<string>,multipleUnits:boolean,sourceTags:Set<string>,allowAnnualFallback:boolean,allowQuarterFallback:boolean}|null} */
  let chosen = null;
  let annualBoundary = '';
  let quarterBoundary = '';
  for (const tag of tags) {
    ...
    if (!chosen) {
      ...
      chosen = {
        ...
        multipleUnits: choices.length > 1,
        sourceTags: new Set(
          [...primary.annual, ...primary.quarters]
            .map(officialTagKey)
            .filter(source => source !== null)
        ),
        allowAnnualFallback: primary.annual.length > 0,
        allowQuarterFallback: primary.quarters.length > 0
      };
    ...
    const annualResult = chosen.allowAnnualFallback
      ? appendNewerPeriods(chosen.annual, fallback.annual, annualBoundary)
      : { latest: annualBoundary, added: false, addedFacts: [] };
    const quarterResult = chosen.allowQuarterFallback
      ? appendNewerPeriods(chosen.quarters, fallback.quarters, quarterBoundary)
      : { latest: quarterBoundary, added: false, addedFacts: [] };
    annualBoundary = annualResult.latest;
    quarterBoundary = quarterResult.latest;
    if (annualResult.added || quarterResult.added) {
      if (choices.length > 1) chosen.multipleUnits = true;
      for (const fact of [...annualResult.addedFacts, ...quarterResult.addedFacts]) {
        const source = officialTagKey(fact);
        if (source) chosen.sourceTags.add(source);
      }
  ...
  if (chosen.ytdExcludedKeys.size > 0) {
    addWarning(
      warnings,
      'YTD_EXCLUDED',
      `${definition.label}略過 ${chosen.ytdExcludedKeys.size} 筆六個月／九個月累計值，沒有冒充單季。`,
      key
    );
  }
...
function binaryDerived(metrics, key, label, formula, leftKey, rightKey, calculate, outputUnit) {
  ...
  const annual = [];
  if (left && right && !hasMixedAnnualSources(left) && !hasMixedAnnualSources(right)) {
...
function dilutedSharesChange(metrics) {
  ...
  for (let index = 1; index < rows.length; index += 1) {
    ...
    if (prior.unit !== current.unit
      || officialSourceKey(prior) !== officialSourceKey(current)
      || prior.value === 0) continue;
