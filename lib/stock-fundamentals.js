...
/**
 * 低順位 tag 不改寫同期；一般只補更新期。兩個來源若至少兩個重疊期間完全同值，
 * 才把它們視為同口徑並補中間缺口，避免 Alphabet 型合法年度永久缺一格。
 * @param {any[]} target
 * @param {any[]} candidates
 * @param {string} boundary
 */
function mergeFallbackPeriods(target, candidates, boundary) {
  let latest = boundary;
  const addedFacts = [];
  ...
  for (const fact of candidates) {
    ...
    else conflictingOverlap = true;
  }
  const canBackfill = !conflictingOverlap && matchingOverlaps >= 2;
  for (const fact of candidates) {
...
function selectMetricFromTags(
  ...
  /** @type {{unit:string,annual:any[],quarters:any[],ytdExcludedKeys:Set<string>,multipleUnits:boolean,sourceTags:Set<string>}|null} */
  let chosen = null;
  let annualBoundary = '';
  let quarterBoundary = '';
  for (const tag of tags) {
    ...
    const annualResult = mergeFallbackPeriods(chosen.annual, fallback.annual, annualBoundary);
    const quarterResult = mergeFallbackPeriods(chosen.quarters, fallback.quarters, quarterBoundary);
