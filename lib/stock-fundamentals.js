...
const RAW_SEC_METRICS = {
  ...
  capitalExpenditure: {
    ...
    // William 2026-09-02 裁示換成 PaymentsForCapitalImprovements（真科目；實查 CY2024
    // 138 家申報、其中 120 家前兩科目都沒報，多為房地產類）。它排最後＝純新增退路：
    // 語意偏「改良既有資產」、比 ProductiveAssets（廣義購置）窄，且不動既有公司的採用順序。
    tags: [
  ...
  shareRepurchases: {
    ...
    // William 2026-09-02 裁示換成 PaymentsForRepurchaseOfEquity（普通股＋特別股合計，
    // 最接近原意；實查 CY2024 149 家申報、其中 134 家沒報第 1 順位）。
    // 「只買回特別股」那顆（...PreferredStockAndPreferenceStock）裁示不加：
    // 經濟性質偏還債，顯示成「股票回購」會被誤讀成回饋普通股股東。
