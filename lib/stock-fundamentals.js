...
function selectMetricFromTags(
  ...
  /** @type {{unit:string,annual:any[],quarters:any[],ytdExcludedKeys:Set<string>,multipleUnits:boolean}|null} */
  let chosen = null;
  let annualBoundary = '';
  let quarterBoundary = '';
  for (const tag of tags) {
    ...
    if (!chosen) {
      ...
      const primaryBoundary = latestPeriodEnd([...chosen.annual, ...chosen.quarters]);
      annualBoundary = latestPeriodEnd(chosen.annual) || primaryBoundary;
      quarterBoundary = latestPeriodEnd(chosen.quarters) || primaryBoundary;
    ...
    const annualResult = appendNewerPeriods(chosen.annual, fallback.annual, annualBoundary);
    const quarterResult = appendNewerPeriods(chosen.quarters, fallback.quarters, quarterBoundary);
