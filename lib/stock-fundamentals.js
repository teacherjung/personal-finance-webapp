...
function mergeFallbackPeriods(target, candidates, boundary, outputLimit) {
  ...
  if (materialOverlapConflict) {
    return { latest, added: false, addedFacts, overlapConflict: hasRelevantMissingPeriod };
  }
  ...
  return { latest, added: addedFacts.length > 0, addedFacts, overlapConflict: false };
}
...
function selectMetricFromTags(
  ...
  for (const tag of tags) {
    ...
    const annualResult = mergeFallbackPeriods(chosen.annual, fallback.annual, annualBoundary, 5);
    const quarterResult = mergeFallbackPeriods(chosen.quarters, fallback.quarters, quarterBoundary, 1);
    annualBoundary = annualResult.latest;
    quarterBoundary = quarterResult.latest;
    if (annualResult.overlapConflict || quarterResult.overlapConflict) {
      addWarning(
        warnings,
