...
/**
 * @param {Map<string,{priority:number,fact:any}>} periods
 * @param {any} fact
 * @param {number} priority
 */
function keepPreferredPeriod(periods, fact, priority) {
  const key = periodKey(fact);
  const current = periods.get(key);
  if (!current || priority < current.priority) periods.set(key, { priority, fact });
}
 ...
 * @param {any[]} warnings
 */
function selectMetricFromTags(taxonomyFacts, cik, key, definition, tags, warnings) {
  /** @type {Map<string, {unit:string,annual:Map<string,{priority:number,fact:any}>,quarters:Map<string,{priority:number,fact:any}>,ytdExcluded:number}>} */
  ...
  for (const [priority, tag] of tags.entries()) {
    ...
    for (const [unit, rows] of Object.entries(/** @type {Record<string, any>} */ (units))) {
      ...
      let group = groupedByUnit.get(unit);
      if (!group) {
        group = { unit, annual: new Map(), quarters: new Map(), ytdExcluded: 0 };
        groupedByUnit.set(unit, group);
      }
      group.ytdExcluded += selected.ytdExcluded;
      for (const fact of selected.annual) keepPreferredPeriod(group.annual, fact, priority);
      for (const fact of selected.quarters) keepPreferredPeriod(group.quarters, fact, priority);
    }
  }

  const choices = [...groupedByUnit.values()].map(group => {
    const annual = [...group.annual.values()].map(item => item.fact).sort((left, right) => (
      String(left.periodEnd).localeCompare(String(right.periodEnd))
      || String(left.periodStart || '').localeCompare(String(right.periodStart || ''))
    )).slice(-5);
    const quarters = [...group.quarters.values()].map(item => item.fact).sort((left, right) => (
      String(left.periodEnd).localeCompare(String(right.periodEnd))
      || String(left.periodStart || '').localeCompare(String(right.periodStart || ''))
    ));
    return {
      ...
      periodCount: annual.length + quarters.length,
      ytdExcluded: group.ytdExcluded
    };
  });
  ...
  const latest = selected.latestQuarter || selected.annual.at(-1);
  return {
    key,
...
function selectMetric(taxonomyFacts, cik, key, definition, warnings, currentDebtLabelHints = {}) {
  ...
  const selected = selectMetricFromTags(taxonomyFacts, cik, key, definition, definition.tags, warnings);
...
function selectCurrentDebt(taxonomyFacts, cik, key, definition, labelHints, warnings) {
  const sources = definition.currentDebtSources;
  const total = selectMetricFromTags(taxonomyFacts, cik, key, definition, sources.total, warnings);
