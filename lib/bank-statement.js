 ...
 * ⚠️ **只對「數字＋分隔符＋遮罩字元」組成的字串做遮罩改寫**——含其他字母的（`ABX12345678`）不改遮罩：那個 X 分不出是
 * 遮罩還是帳號本文，改寫後會被當成遮罩帳號收進來；不改＝沒星號的英數帳號取不出末碼＝白話拒收（與放寬前相同；
 * 本來就帶半形星號的英數帳號 `AB****5678` 照舊收）。**NFKC 對每個字串都做**（全形數字／全形分隔符／全形字母折成半形
 * ——同一個字的另一種印法，不算改寫；不折的話全形數字帳單整份進不來）。沒有遮罩的完整帳號數字內容不動。
