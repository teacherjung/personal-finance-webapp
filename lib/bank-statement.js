 ...
 * 但身分仍要有個末碼可比。兩者都拿不到＝''（整串被遮掉、或不是帳號長相）。模板路線不用這支（它們的帳號一律帶星號）。
 * @param {string} s */
 ...
 * 沒有遮罩的完整帳號原樣不動。寫進帳本與去重鍵的就是改寫後的形＝同一份帳單不論 AI 這次用哪種符號抄，鍵都一樣。
 * @param {string} s */
export function normalizeMaskShape(s) {
  return String(s || '').normalize('NFKC').replace(/[*xX•●·○◯]+/g, (run) => '*'.repeat(run.length));
}
