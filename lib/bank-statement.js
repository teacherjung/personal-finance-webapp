...
export function parseTaishinDebitCardRows(lines) {
  ...
  for (const line of lines) {
    ...
    // 主列＝「扣款日 消費日」合在一格；抽字偶爾會拆成兩格（各一個日期）＝也收
    const dateish = /^\d{4}\//.test(first);
    let md = first.match(/^(\d{4})\/(\d{2})\/(\d{2})\s+(\d{4})\/(\d{2})\/(\d{2})$/);
    let amtIdx = 1;
    if (!md && /^\d{4}\/\d{2}\/\d{2}$/.test(first) && /^\d{4}\/\d{2}\/\d{2}$/.test(second)) {
    ...
    const amount = cells.length > amtIdx ? parseAmount(String(amtCell.s)) : null;
  ...
  // 店名（主列上方）與消費地區（主列下方）歸位：**每片只歸 y 最近的那一筆**、且距離 ≤ 上限；店名可能換行＝上方可多片
  for (const f of frags) {
    let best = null, bestDy = Infinity;
    for (const r of rows) { const dy = Math.abs(f.y - r.y); if (dy < bestDy) { best = r; bestDy = dy; } }
    if (!best || bestDy > ORPHAN_MAX_DY_DEBIT) continue;
    if (f.y > best.y) best.desc = (best.desc ? `${best.desc} ` : '') + f.t;   // 上方＝店名；frags 依列序＝由上而下接起來
    else best.region = (best.region ? `${best.region} ` : '') + f.t;
  }
