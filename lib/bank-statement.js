...
/** 這一列是不是 D 區的表頭（六欄到齊）。 @param {BLine} line */
function isDetailHeader(line) {
  const cells = line.cells.map((c) => String(c.s).trim());
  return DETAIL_HEADS.every((h) => cells.includes(h));
}
 ...
 *
 * @param {BLine[]} lines
 * @returns {{ transactions: BankTx[], skippedZero: number }}
 */
export function parseTaishinDebitDetail(lines) {
  ...
  for (const line of lines) {
    if (isDetailHeader(line)) {
      sawHeader = true;
      // 跨頁會重印表頭＝每次都重新定位（版面微調不必改程式）
      const at = (/** @type {string} */ name) => {
        const c = line.cells.find((x) => String(x.s).trim() === name);
        return c ? c.x : 0;
      };
    ...
    // ⚠️ **離場錨點**：A/B/C 若因為多張卡或跨頁被重印在明細之後，「表頭之前不收」就守不住了
    //    （那時只剩欄位幾何在擋＝運氣，不是界線）。遇到那幾區的標題就關掉收集。
    if (/已消費未扣款|消費支出類別|本月消費金額共計|本月退款金額共計|卡號末四碼/.test(joined)) { inDetail = false; continue; }
    ...
    const outAmt = out ? (parseAmount(String(out.s)) ?? 0) : 0;
    const inAmt = inn ? (parseAmount(String(inn.s)) ?? 0) : 0;
    if (outAmt && inAmt) {
  ...
  if (sawHeader && !transactions.length && !skippedZero) {
    // ⚠️ **認得版面卻一列都收不到＝不可以回報成功**（靜靜匯入 0 筆比擋下來更糟：使用者以為
    //    匯進去了）。丟既有的 bank_unrecognized＝退回 AI 救援那條路，不新增第二種錯誤碼。
    throw Object.assign(new Error('讀得到簽帳金融卡明細的表頭，卻一列交易都讀不出來（帳單版面可能與預期不同，請回報）'),
      { status: 400, code: 'bank_unrecognized' });
  }
  return { transactions, skippedZero };
}
 ...
 * @param {BLine[]} lines
 * @returns {{ bank:'台新', referenceDate:string|null, accounts:never[],
 *   accountCurrency:Record<string,string>, transactions:BankTx[], noAccountReason:'masked-suffix-only' }}
 */
export function parseTaishinDebit(lines) {
  ...
  // ⚠️ **刻意不產 accounts＝不自動建帳戶、不更新餘額**（2026-08-20 設計裁決，兩個實測理由）：
  //   ①**會裂戶**：這個版面的帳號只印得出末四碼（`**********8791`），自動建戶會把 accountNo 寫成它
  //     ⇒ 之後同一個真實帳戶的**綜合對帳單**（`900100****8791`）過 `matchAccount` 時，
  //     `d.startsWith(prefix)` 拿 '8791' 比 '900100' 必然 false ⇒ **再建第二顆帳戶＝同一筆錢被算兩次**
  //     （實測：先綜合後金融卡沒事、先金融卡後綜合裂戶——不對稱，所以不能賭使用者的匯入順序）。
  //   ②**末筆對概要會變成自證**：這份帳單沒有獨立的「帳戶概要區餘額」，唯一的餘額來源就是明細
  //     最後一列；把它同時放進 accounts 再拿去對明細末筆＝同一個數字對自己、恆綠，
  //     還會讓「這個帳戶吃到一道擋下型檢查」的計數被灌水、畫面因此說一句假話。
  //   代價照實記：這條路**不會更新帳戶餘額**（餘額仍由綜合對帳單那條路更新），只匯入明細。
  //   ⚠️ `accountCurrency` 仍要有這個帳號——下游靠它判幣別，也靠它把這個帳號算成「自己人」
  //   （`ownSuffixSet`）才不會把帳戶互轉誤判成收支。
  return { bank: '台新', referenceDate: pickReferenceDate(lines), accounts: [],
    accountCurrency: { [masked]: 'TWD' }, transactions,
    // ⚠️ **不建帳戶要說得出口**（同「跳過要說得出口」那一課）：預覽窗原本的空表句是
    //    「帳單裡沒有可更新的帳戶」——對這個版面那是**誤導**（不是帳單沒有，是我們刻意不建）。
    //    封閉代碼交給前端翻白話（零插值、帳單欄值一個都不外送）。
    noAccountReason: /** @type {const} */ ('masked-suffix-only') };
}
