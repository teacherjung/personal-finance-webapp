...
/**
 * 帳戶幣別表的**唯一登記器**（三條解析路共用：`parseBankSummary`／`parseWithRecipe`／`normalizeAiBank`）。
 * 病根＝**外幣綜合帳戶**：同一個遮罩帳號同時掛多種幣別（2026-08-26 用 William 的真帳單證實＝JPY+USD 同號，
 * 三份跨三個月穩定重現；P0.1 的檔頭 2026-08-11 就記過這個形狀，當時只把閘的射程移開、沒讓表容得下它）。
 * `masked → currency` 是一對一，裝不下兩種幣別，所以三處原本都寫 last-wins（後者為準）。
 * **last-wins 不能留**，兩個獨立理由：
 *   ①**它是順序相依的**：同一份帳單，兩個模型把幣別表兩列的順序寫反（兩份都對），壓平後一份是 JPY、
 *     一份是 USD ⇒ 雙讀比對判成硬差異「帳戶幣別表」⇒ 送仲裁、仲裁的順序同樣隨機 ⇒ 三份互不相同 ⇒
 *     `ai_disagree` 整份擋下。也就是「必然拒收」被換成「隨機拒收＋白燒一發仲裁」。
 *   ②**它會猜錯幣別**：勝出者若是 TWD，外幣列會被當台幣記進帳本；勝出者若是外幣，同號的真台幣列會整組
 *     不入帳（漏帳）。兩個方向都是錢錯。
 * 所以改成**歧義哨兵**：同一個 masked 收到第二種幣別就填 `UNKNOWN_CURRENCY`，且**黏著**（之後再收到什麼都還是它）。
 * 這讓結果與登記順序無關，下游一律走既有的「分不出＝不驗算、不入帳」路（`twdCheckable`／`txCurrency`）——
 * fail-closed 勝過猜一個幣別，與本檔上面那段「不確定就不可猜」同一條規矩。
 * ⚠️ **三處必須同時改、同一支 PR**：配方出生第三關 `recipeReproduces` 逐鍵比對兩邊的 `accountCurrency`，
 *    只改一處＝那個鍵永遠對不上 ⇒ 配方**永遠孵不出來**、同版面每期都付全額 AI 錢——而這件事**沒有任何
 *    既有考題會抓到**（是本支新寫的 `test/multi-currency-account.test.js` 在守；那卷紅了就是這個洞）。
 * ⚠️ 誠實劃界：這只解決「表達不了就整份拒收」。**明細列上沒有幣別欄**，所以多幣別帳號的每一筆交易仍然
 *    判不出自己是哪一幣（下游一律當「分不出」＝不入帳）。逐筆幣別＝下一批的工作。
 * @param {Record<string,string>} map @param {string} masked @param {string} currency
 */
export function noteAccountCurrency(map, masked, currency) {
  // own-property 讀（原型鍵防線，同 statementCurrencyLookup／txCurrency 的既有規矩）：map 是字面 {}，
  // masked 若是 constructor/toString 這類原型成員名，直讀會拿到 function ⇒ prev !== currency ⇒ **第一次
  // 登記就被降成哨兵**；__proto__ 更是整個登記不落地。正式路徑進不來（遮罩要有數字），縱深防禦。
  const prev = Object.hasOwn(map, masked) ? map[masked] : undefined;
  map[masked] = (prev == null || prev === currency) ? currency : UNKNOWN_CURRENCY;
}
...
export function parseBankSummary(lines) {
  ...
  for (const line of lines) {
    ...
    noteAccountCurrency(accountCurrency, masked, currency);   // 記**所有**帳戶的幣別（含餘額空白被略過的）→ 明細判幣別不 fail-open 成 TWD；同號多幣別＝哨兵（見登記器）
