...
export function parseTaishinDebitDetail(lines) {
  ...
  for (const line of lines) {
    ...
    // 錨點涵蓋 A/B/C 三區各自的標題（Grok #492 掃後補 A）：A 區靠表頭的「外幣折換日」（獨特字樣，
    //   D 區與備註不會出現）＋「卡號末四碼」雙保險——真檔的 A 區順序是表頭→卡號末四碼→明細，
    //   表頭被抽字拆掉時第二道仍在。
    if (/已消費未扣款|消費支出類別|本月消費金額共計|本月退款金額共計|卡號末四碼|外幣折換日/.test(joined)) { inDetail = false; continue; }
...
export function parseTaishinDebit(lines) {
  ...
  //   ①**根本沒定位到表頭**——辨識用的是整份文字、定位用的是單一列，PDF 抽字把六欄拆成兩列時
  //     前者說認得、後者找不到 ⇒ 迴圈一列都沒進去。
  //   ②定位到了，但一列交易都讀不出來。
  //   兩者都丟既有的 bank_unrecognized＝退回 AI 救援那條路，不新增第二種錯誤碼。
  if (!sawHeader) {
    throw Object.assign(new Error('認得這是簽帳金融卡明細，卻定位不到「日期／摘要／支出／存入／餘額／備註」那一列表頭（帳單版面可能與預期不同，請回報）'),
      { status: 400, code: 'bank_unrecognized' });
  }
