...
export function makeCurrencyTable() {
  ...
  return {
    ...
    note(masked, currency) {
      ...
      // ⚠️ **等價印法也要互相看見**（Grok #517 掃描第 1 條）：鍵是原字串（下游 statementCurrencyLookup
      //   靠它 exact 查表，不能換），所以 `900300****0363`(JPY) 與 `900300-****-0363`(USD) 原本會各自
      //   成鍵、各自單一幣別＝**不會塌成哨兵**。用同一把 acctPatternsIntersect 掃一遍既有鍵：語言相交
      //   且幣別不同＝兩邊一起變哨兵（黏著）。這樣「身分尺」才是真的統一，不是只接在 hasMixedTwd 上。
      for (const other of Object.keys(map)) {
        if (other === masked || map[other] === map[masked]) continue;
        if (!acctPatternsIntersect(other, masked)) continue;
        map[other] = UNKNOWN_CURRENCY; map[masked] = UNKNOWN_CURRENCY;
      }
    },
