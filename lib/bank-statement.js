...
const MASK_RUN = /[xX•●·○◯×]{2,}/g;                // 非星號遮罩字元**連續兩個以上**才當遮罩（單一個 X 可能是帳號本文／檢查碼——
                                                   //   Codex #504 r2#5；星號本身不需改寫，NFKC 已把全形星折成半形）
 ...
 * 遮罩帳號收進來；不改＝沒星號的英數字串取不出末碼＝白話拒收（與放寬前相同；本來就帶半形星號的 `AB****5678` 照舊收）。**NFKC 對每個字串都做**（全形數字／全形分隔符／全形字母折成半形
