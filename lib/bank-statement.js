 ...
 *   accountCurrency:Record<string,string>, transactions:BankTx[], cardRows:DebitCardRow[] }}
 */
export function parseTaishinDebit(lines) {
  ...
  //   讀不到 A 區表頭＝空陣列（這份帳單沒有那一區、或版面不同）——不擋 D 區；讀到了卻抄不對＝那一支自己丟 bank_unrecognized。
  const cardRows = parseTaishinDebitCardRows(lines).rows;
  return { bank: '台新', referenceDate: pickReferenceDate(lines), accounts,
    accountCurrency: { [masked]: 'TWD' }, transactions, cardRows };
}
