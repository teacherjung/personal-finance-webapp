...
export function parseTaishinDebitDetail(lines) {
  ...
  for (const line of lines) {
    ...
    // Stage 3（William 2026-08-20 排定）：抽字有時把六欄表頭**拆成相鄰兩列**（#492 r1#1 實測）。
    //   以前＝定位不到 ⇒ 整份 bank_unrecognized ⇒ 落到 AI（花錢）。現在＝相鄰兩列都是純碎片、
    //   y 夠近、欄名合起來六個到齊 ⇒ 當成一列表頭。**三個條件缺一不可**，其餘形狀照舊喊認不得：
    //   拆成三列、兩列隔太遠、合起來仍缺欄、碎片裡混了別的字——寧可退回 AI，也不猜一個界線
    //   （表頭是「錢不被算兩次」的界線，猜錯的代價是把 A/B/C 區的列收進來）。
    const frag = headFragment(line);
    if (frag) {
      if (pendingFrag && Math.abs(pendingFrag.y - line.y) <= HEADER_SPLIT_MAX_DY) {
        const merged = [...pendingFrag.cells, ...line.cells];
        const names = new Set(merged.map((c) => String(c.s).trim()));
        if (DETAIL_HEADS.every((h) => names.has(h))) { locate(merged); continue; }
      }
...
export function parseTaishinDebit(lines) {
  ...
  //   ①**根本沒定位到表頭**——辨識用的是整份文字、定位用的是單一列（Stage 3 起：或相鄰兩列的碎片
  //     合起來）；拆成三列、隔太遠、湊不齊六欄時前者說認得、後者找不到 ⇒ 迴圈一列都沒進去。
  //   ②定位到了，但一列交易都讀不出來。
  //   兩者都丟既有的 bank_unrecognized＝退回 AI 救援那條路，不新增第二種錯誤碼。
  if (!sawHeader) {
    throw Object.assign(new Error('認得這是簽帳金融卡明細，卻定位不到「日期／摘要／支出／存入／餘額／備註」那一列表頭（相鄰兩列合起來看也湊不齊；帳單版面可能與預期不同，請回報）'),
      { status: 400, code: 'bank_unrecognized' });
  }
