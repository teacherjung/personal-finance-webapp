...
/**
 * A 區「刷卡消費明細」＝同一筆錢的**另一種印法**（Stage 5a，William 2026-08-20 排定「一份帳單產出兩種明細」）。
 * D 區把刷卡記成「刷卡消費／刷卡退貨」兩種摘要＝**錢的流向**（進現金流）；A 區每筆多了消費日、店名、
 * 消費地區＝**買了什麼**（給消費分析用的卡片帳本）。本函式只**讀出** A 區、不決定它怎麼入帳——入帳那半
 * 是下一支的事（同一筆錢兩區都出現，怎麼不算兩次要另案裁決）；這裡的契約只有「抄得對、抄不對就說」。
 *
 * 版面（真檔量到）：每一筆佔三列——店名列（y 高約 5）、主列（「扣款日 消費日」合在一格＋國外交易服務費＋
 * 台幣金額，金額右對齊、退款印負號）、消費地區列（y 低約 4）。店名與地區都落在「消費明細 / 消費地區」
 * 那一欄的 x 帶裡（表頭 x 到「外幣折換日」表頭 x 之間）＝靠 **x 帶＋y 距離**歸到最近的主列，
 * 與 D 區跨行備註同一套想法（距離上限共用同一個量到的數字）。
...
export function parseTaishinDebitCardRows(lines) {
  ...
  for (const line of lines) {
    ...
    if (!inA) continue;
    // 離場：B 區（總額／類別表）、C 區（已消費未扣款）、D 區表頭——都不是 A 區的列
    if (/本月消費金額共計|本月退款金額共計|消費支出類別|已消費未扣款/.test(joined) || isDetailHeader(line)) { inA = false; continue; }
    const m4 = joined.match(/卡號末四碼[:：]?(\d{4})/);
    if (m4) { lastFour = m4[1]; continue; }
    const first = String(cells[0]?.s || '').trim();
    const md = first.match(/^(\d{4})\/(\d{2})\/(\d{2})\s+(\d{4})\/(\d{2})\/(\d{2})$/);
    if (!md) {
      // 不是主列＝店名或消費地區的片段（只收落在「消費明細」欄 x 帶裡的；別欄的字不收，免把金額當店名）
      const right = (/** @type {any} */ c) => c.x + (c.w || 0);
      if (cells.length && cells.every((c) => c.x >= xDesc - 12 && right(c) <= xFx)) frags.push({ y: line.y, t: cells.map((c) => String(c.s).trim()).join(' ') });
      continue;
    }
    const postDate = `${md[1]}-${md[2]}-${md[3]}`, date = `${md[4]}-${md[5]}-${md[6]}`;
    if (!isRealDate(postDate) || !isRealDate(date)) {
      throw Object.assign(new Error('簽帳金融卡的刷卡消費明細有一列的日期不是真日曆日（帳單版面可能與預期不同，請回報）'), { status: 400, code: 'bank_unrecognized' });
    }
    const amtCell = cells[cells.length - 1];
    const amount = cells.length >= 2 ? parseAmount(String(amtCell.s)) : null;
    if (amount == null) {
      throw Object.assign(new Error('簽帳金融卡的刷卡消費明細有一列讀不出台幣金額（帳單版面可能與預期不同，請回報）'), { status: 400, code: 'bank_unrecognized' });
    }
    // 主列中間的格：台幣列只有一格＝「國外交易服務費」（真檔每列印 0）；多於一格＝外幣列（沒真檔可量）
    //   ＝原文收進 extra、不拆欄不猜語意。
    const mid = cells.slice(1, -1).map((c) => String(c.s).trim()).filter(Boolean);
  ...
  // 店名（主列上方）與消費地區（主列下方）歸位：各取 y 最近、距離 ≤ 上限的片段；店名可能換行＝上方可多片
  for (const r of rows) {
    const above = frags.filter((f) => f.y > r.y && f.y - r.y <= ORPHAN_MAX_DY_DEBIT).sort((a, b) => b.y - a.y);
    const below = frags.filter((f) => f.y < r.y && r.y - f.y <= ORPHAN_MAX_DY_DEBIT).sort((a, b) => b.y - a.y);
    r.desc = above.map((f) => f.t).join(' ').replace(/\s*\/\s*$/, '').trim();   // 欄名是「消費明細 / 消費地區」：店名尾端印著分隔用的「/」
    r.region = below.map((f) => f.t).join(' ').trim();
  }
  return { rows: rows.map((r) => ({ postDate: r.postDate, date: r.date, amount: r.amount, fee: r.fee, lastFour: r.lastFour, desc: r.desc, region: r.region, extra: r.extra })), sawHeader };   // y 只在歸位時用，不外流
}
...
/**
 * 把 A 區的筆對到 D 區的列（純函式；Stage 5a 只產對照表，不改任何一邊）。
 * 對法＝**扣款日＋金額（方向由正負決定）一對一**，同日同額多筆照兩區各自的列印順序配；
 * 店名刻意**不**進對法（A 區是店名原文、D 區備註是截短版＋授權碼，字面不穩）。
 * @param {DebitCardRow[]} cardRows @param {BankTx[]} transactions
 * @returns {{ pairs: {card:number, tx:number}[], unmatchedCards: number[], unmatchedTxs: number[] }} 三者都是索引
 */
export function linkDebitCardRows(cardRows, transactions) {
  ...
  transactions.forEach((t, i) => {
    ...
    const k = `${t.date}|${t.direction}|${t.amount}`;
    const g = pool.get(k) || []; g.push(i); pool.set(k, g);
  });
  const pairs = [], unmatchedCards = [];
  cardRows.forEach((r, i) => {
    const k = `${r.postDate}|${r.amount < 0 ? 'in' : 'out'}|${Math.abs(r.amount)}`;
    const g = pool.get(k);
    const tx = g && g.length ? g.shift() : undefined;
    if (tx === undefined) unmatchedCards.push(i); else pairs.push({ card: i, tx });
  });
