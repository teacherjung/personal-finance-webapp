...
export async function extractBankLines(data, password) {
  ...
  // 或幾十萬個文字節點（壓縮炸彈），解析時把記憶體吃光。超標**明確拒絕**，不靜默截斷。
  assertPageLimit(doc.numPages, '銀行對帳單 PDF');
  let itemCount = 0;
  for (let i = 1; i <= doc.numPages; i++) {
    const page = await doc.getPage(i);
    const tc = await page.getTextContent();
    itemCount = countTextItems(itemCount, tc.items.length, '銀行對帳單 PDF');
 ...
 * @param {BLine[]} lines
 * @returns {{ referenceDate: string|null, accounts: {suffix:string, masked:string, balance:number, currency:string, label:string, note:string}[], accountCurrency: Record<string,string> }}
 */
  ...
  for (const line of lines) {
    ...
    if (isMaskedAccount(c[0]?.s) && isBankDate(c[1]?.s)) {
      ...
      if (!txnCell) {   // 退路（右緣分不出欄）：舊「左緣+中線」法
        ...
        txnCell = amtCells.find(x => x.x >= lower && x.x < xBal) || null;
 ...
 * @returns {Promise<{ bank:'台新', referenceDate:string|null, accounts:{suffix:string,masked:string,balance:number,currency:string,label:string,note:string}[], accountCurrency:Record<string,string>, transactions:BankTx[] }>}
 */
