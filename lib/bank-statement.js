 ...
 * 真實帳單：數十列 × 十幾字元 ≈ 數百字元。10,000 是數十倍餘裕，對應最壞情形約 1 秒以內
 *（本機實測基準：Σlen=7,000 時 finalize 約 0.4 秒）。 */
...
export function makeCurrencyTable() {
  ...
  const recompute = () => {
    ...
    for (const k of keys) {
      ...
      for (const o of keys) {
        if (o === k || !acctPatternsIntersect(o, k)) continue;
  ...
  return {
    ...
    note(masked, currency) {
      ...
      // ⚠️ 這裡**只更新自己那一格**（等價印法的互看留給 finalize——見 recompute 的說明）
    },
    /**
    ...
    finalize() {
      ...
      let chars = 0;
      for (const k of byRaw.keys()) chars += k.length;
      if (byRaw.size > MAX_CURRENCY_ROWS || chars > MAX_CURRENCY_CHARS) {
