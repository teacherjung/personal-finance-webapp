...
/** 帳號的**身分形**（比「是不是同一個帳號」用）：正規形再把連續星號壓成一個——**星號數不是資訊**
 * （同一個帳號在不同區塊/不同讀法可能印成 `900300****0363` 與 `900300*****0363`）。
 * ⚠️ 刻意**不動 `canonMasked`**：那支是雙讀逐欄比對在用的，壓星號會改到那邊的語意（另一支 PR 的事）。 */
export function accountIdentity(/** @type {any} */ s) { return canonMasked(s).replace(/\*+/g, '*'); }
 ...
 * ⚠️ 混台外幣的判定用**身分形**分組（r2#2／r3#1）：`900300****0363`、`900300-****-0363`、`900300*****0363`
 *    都是同一個帳號——分隔符與**星號數**都不是資訊。用原字串（或只剝分隔符）分組會被等價印法繞過
 *    （兩次實測都做到 imported=2、閘仍 strong、db 真的多兩筆台幣交易）。
...
export function makeCurrencyTable() {
  ...
  /** @type {Map<string, Set<string>>} */ const byCanon = new Map();  // 混台外幣判定用（不同印法算同一個帳號）
  const add = (/** @type {Map<string, Set<string>>} */ m, /** @type {string} */ k, /** @type {string} */ v) => {
    if (!m.has(k)) m.set(k, new Set());
    /** @type {Set<string>} */ (m.get(k)).add(v);
  };
  return {
    map,
    /** 登記一列。 @param {string} masked @param {string} currency */
    note(masked, currency) {
      add(byRaw, masked, currency);
      add(byCanon, accountIdentity(masked), currency);   // 身分形＝連星號數都不算資訊（r3#1：星號多一個就繞過去了）
      // own-property 讀（原型鍵防線，同 statementCurrencyLookup／txCurrency 的既有規矩）：map 是字面 {}，
      // masked 若是 constructor/toString 這類原型成員名，直讀會拿到 function ⇒ 第一次登記就被降成哨兵。
    ...
    /** 有沒有同一個帳號同時掛台幣與外幣（＝我們真的判不出每一筆是哪一幣）。 */
    hasMixedTwd() {
      for (const set of byCanon.values()) if (set.size > 1 && set.has('TWD')) return true;
      return false;
    },
  };
}
...
export function parseBankSummary(lines) {
  ...
  // 同號混台幣＋外幣＝三條路一律拒收（見 makeCurrencyTable 的說明）。專屬 code：讓 previewBankStatement
