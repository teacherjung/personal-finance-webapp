...
export function acctPatternsIntersect(a, b) {
...
export function makeCurrencyTable() {
  ...
  return {
    ...
    note(masked, currency) {
      ...
      const prev = map[masked];   // null-proto＝直讀就是 own-property，沒有原型成員可誤命中
     ...
     *   Codex #517 連三輪各示範一種繞法（分隔符 r2#2／星號數 r3#1／完整號 r5#1），全都做到 imported=2、
     *   閘仍 strong、db 真的多兩筆台幣交易——手寫的形狀規則永遠有下一個邊界，換成那把數學判準才收斂
     *   （#504 十四輪學到的同一課）。保守方向：**語言有交集就算同一個帳號**（寧可誤擋、不可誤放）。
     */
    hasMixedTwd() {
      for (const t of twdAccts) for (const f of fxAccts) if (acctPatternsIntersect(t, f)) return true;
      return false;
    },
  };
}
