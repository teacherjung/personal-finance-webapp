...
export function makeCurrencyTable() {
  ...
  return {
     ...
     */
    finalize() {
      if (byRaw.size > MAX_CURRENCY_ROWS) {
        throw Object.assign(new Error(`這份帳單的帳戶列數異常多（超過 ${MAX_CURRENCY_ROWS} 個），不像真的對帳單——為了不讓它拖垮服務，這份不收。`), { status: 400, code: 'bank_too_many_accounts' });
      }
      recompute();
    },
