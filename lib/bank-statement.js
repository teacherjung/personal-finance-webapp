 ...
 * 有遮罩＝同 accountSuffix；**沒有遮罩的完整帳號**（純數字、可夾 `-`／空白，至少五碼）＝末四碼——帳單沒遮、程式就不動，
 * 但身分仍要有個末碼可比。兩者都拿不到＝''（整串被遮掉、或不是帳號長相）。模板路線的**解析器**不用這支（它們的帳號一律帶星號）；
...
export function accountSuffixAny(s) {
  const star = accountSuffix(s);
  if (star) return star;
  const raw = String(s || '').normalize('NFKC');
  if (/[*]/.test(raw)) return '';                      // 有星號卻取不出末碼（整串遮掉）＝不是「沒遮」
  const digits = raw.replace(/[\s-]/g, '');
  return /^\d{5,}$/.test(digits) ? digits.slice(-4) : '';
}

/** 遮罩字元（改寫成半形星號的對象）：星號、X／x、圓點類、乘號 ×、井號 #。帳單上見過或常見的遮罩印法；
 * 新印法要加進來＝連考題一起加（漏掉的後果是 fail-closed 拒收，不是錯認）。 */
export const MASK_CHARS = '*xX•●·○◯×#';
const MASK_RUN = /[*xX•●·○◯×#]+/g;                 // 與 MASK_CHARS 同一組字元（考題逐字驗兩邊一致）
 ...
 * ⚠️ **只改寫「數字＋分隔符＋遮罩字元」組成的字串**——含其他字母的（`ABX12345678`）一個字不動：那個 X 分不出是
 * 遮罩還是帳號本文，改寫後會被當成遮罩帳號收進來；不動＝下游取不出末碼＝白話拒收（AI 路線不收英數帳號，與放寬前相同）。
 * 沒有遮罩的完整帳號**數字內容不動**；NFKC 會把全形數字折成半形（同一個數字的另一種印法，不算改寫——不折的話
 * 全形數字帳單整份進不來）。寫進帳本與去重鍵的就是改寫後的形＝同一份帳單不論 AI 這次用哪種符號抄，鍵都一樣。
 * @param {string} s */
