...
function parseStatement(j) {
  ...
  for (const s of stmts) {
    ...
    if (ct.length) {
      income = income || { dividends: 0, paymentInLieu: 0, withholdingTax: 0, interestPaid: 0, interestReceived: 0, other: 0, count: 0, skippedNoFx: 0 };
      for (const c of ct) {
        // 多幣別換算優先序（AGENTS.md）：fxRateToBase → USD 直通 → 缺匯率不計入並標註筆數，
        ...
        else if ((c.currency || 'USD') === 'USD') amt = Number(c.amount || 0);
