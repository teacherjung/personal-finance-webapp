  ...
  for (const s of stmts) {
    ...
    if (ct.length) {
      ...
      for (const c of ct) {
        ...
        // 有**正的** fxRateToBase 就照算（那條與幣別無關、實測正確）；沒有匯率又不知道幣別＝**跳過並回報**。
        // 匯率必為正數：0／負數＝報表壞值不是匯率——照乘的話金額歸零（或變號）卻 count++
        // ＝畫面「已計入」、skippedNoFx 是 0，一個字都不提。同缺匯率往下走階梯
        //（USD 直通→估算→跳過並回報）；估算那格的 `est > 0` 本來就是同一道牆（#407 複審記錄在案的缺口）。
    ...
    for (const t of tradeRows) {
      ...
      // 同上：缺幣別不冒充 USD。**缺幣別不擋換算**——有「正的」fxRateToBase 仍照算（那條與幣別無關）；
      // 缺幣別「又」沒有有效匯率時 pnlBase 才落到 null，前端 tradePnlBase 因此把它歸進 `missing`
      // 並在畫面標註（而不是默默把 GBP 損益當美元加進 XIRR）。反例釘在 ib-parser-money：缺幣別＋1.27 ⇒ 38.1。
      const currency = String(t.currency || '');
      const fxRaw = t.fxRateToBase;
      const fxNum = fxRaw == null || fxRaw === '' ? null : Number(fxRaw);
      // 匯率必為正數（同上方現金流的牆）：0／負數＝報表壞值，照乘會把已實現損益歸零／變號。
      // 而前端 tradePnlBase 的階梯是「pnlBase 優先」（portfolio-calculations.js 自己那道 `> 0`
      // 正值牆排在 pnlBase 之後）——後端先把 0 塞進 pnlBase，前端的牆就永遠輪不到。
      // 壞值把 `fxRateToBase` 正規化成 null＝與空字串同路；`pnlBase` 接著照下一行的階梯走——
      // **USD 仍然直通、保留原損益**，非 USD 才是 null（前端據此歸 missing 並標註）。
      // ⚠️ 不要「順手」把這裡改成壞 fx 一律 null：那會打掉 USD 退路（反面斷言在 ib-sync-integrity）。
