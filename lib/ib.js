...
import { assertXmlSize, assertRowLimit } from './parse-limits.js';
  ...
  for (const s of stmts) {
    ...
    if (ct.length) {
      ...
      for (const c of ct) {
        ...
        // 有 fxRateToBase 就照算（那條與幣別無關、實測正確）；沒有匯率又不知道幣別＝**跳過並回報**。
    ...
    assertRowLimit(tradeRows.length, '成交紀錄');
