...
export function parseStatement(j, fxToBase = () => null) {   // export 供考題直測旗標語意（hasCashReport/hasCashDetail）
  ...
  for (const s of stmts) {
    ...
    for (const c of asArray(s?.CashReport?.CashReportCurrency)) {
      ...
      if (cur === 'BASE_SUMMARY') {   // 彙總列＝基準幣別總額，留底供「只有彙總列」的合法報表使用（r6#1）
        const v = Number(c.endingCash ?? c.endingSettledCash ?? c.startingCash);
        if (Number.isFinite(v)) baseSummaryCash = v;
        continue;
      }
      if (!cur) continue;
      hasCashDetail = true;
  ...
  return { account, positions, cashByCurrency, hasCashReport, hasCashDetail, baseCurrency, baseSummaryCash, statementCount: stmts.length, accountCount, equity, income, trades, period };
}
