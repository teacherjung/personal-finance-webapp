...
export function parseStatement(j, fxToBase = () => null) {   // export 供考題直測旗標語意（hasCashReport/hasCashDetail）
  ...
  let hasCashDetail = false;   // 區塊裡有沒有「真實幣別」明細列（Codex r5#2：只有 BASE_SUMMARY 彙總列
                               // ＝有區塊沒明細，不可據此把現金歸零——彙總列非零卻零明細正是「報表不完整」的長相）
  ...
  return { account, positions, cashByCurrency, hasCashReport, hasCashDetail, equity, income, trades, period };
}
