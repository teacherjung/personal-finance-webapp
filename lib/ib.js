  ...
  for (const s of stmts) {
    ...
    if (ct.length) {
      ...
      for (const c of ct) {
        ...
        // 依序試 USD 直通、設定估算，否則跳過並回報；設定估算分支的 `est > 0` 本來就是同一個判準（#407 複審記錄在案的缺口）。
    ...
    for (const t of tradeRows) {
      ...
      //（USD 直通／設定估算／否則歸 `missing` 並在畫面標註），而不是默默把 GBP 損益當美元加進 XIRR。反例＝`test/ib-parser-money.test.js` 的「成交紀錄缺幣別且沒有有效匯率」那題（缺幣別＋1.27 仍得 38.1）。
      ...
      // **USD 仍然直通、保留原損益**，非 USD 才是 null（前端 tradePnlBase 收到 null 後依其具名分支處理：USD 直通／設定估算／否則歸 missing 並標註）。
