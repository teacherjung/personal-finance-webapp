...
/** @param {any} j Flex 回應（fast-xml-parser 解出來的物件） @param {(cur: string) => (number|null)} [fxToBase] */
  ...
  for (const s of stmts) {
    ...
    if (ct.length) {
      ...
      for (const c of ct) {
        ...
        let amt;
        if (fx != null && Number.isFinite(fx) && fx > 0) amt = Number(c.amount || 0) * fx;
    ...
    for (const t of tradeRows) {
      ...
      const fxRateToBase = fxNum != null && Number.isFinite(fxNum) && fxNum > 0 ? fxNum : null;
      ...
      trades.push({
        ...
        pnlBase: fxRateToBase != null ? pnl * fxRateToBase : (currency === 'USD' ? pnl : null)
      });
    }
  }
...
/** @param {string|undefined} token 設定可能沒填 @param {string|undefined} queryId @param {(cur: string) => (number|null)} [fxToBase] */
