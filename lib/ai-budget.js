...
// 兩個範圍：
//   - **單張（perBill）**：一次 preview→apply 鏈的所有發數。⚠️ 誠實劃界（預審 C1#3）：帳單沒有
//     伺服器端身分——**重新預覽＝重新起算**（預覽失敗或票過期重來都是新額度），「同一份帳單
//     反覆重試」的總量由**單日上限**兜底，不是單張。preview 期間在記憶體計，
...
/** 上限尺：非法／缺值＝回退預設；正數＝取整**夾到至少 1**（預審 C1#2：0.5 這種值 posnum 擋不住、
 * 落回預設會把使用者想調低的上限靜靜調高——夾到 1 才是尊重「調低」的方向）。
 * 0 或負數仍回預設（把 AI 整條路關死是拔鑰匙的事、不是上限的事）。 @param {any} v @param {number} fallback */
function capOf(v, fallback) {
  const n = Number(v);
  return Number.isFinite(n) && n > 0 ? Math.max(1, Math.floor(n)) : fallback;
}
/** 計數尺（loadBill／billUsed 用）：非負整數、壞值當 0——與上限尺分開（上限夾 1、計數收 0）。 @param {any} v */
function countOf(v) {
  const n = Number(v);
...
/** 每日用量滾動：同一天＝沿用計數、換日往前＝歸零。⚠️ **絕不倒寫較新的日**（Codex #515 r3#1：
 * 跨本地午夜的既有請求——仲裁最長等 300 秒——用凍住的舊日期蓋掉 {新日, n:19}＝當日 19 發被遺忘、
 * 保險絲失準）：db 的日期比我們新＝以 db 為準、計數接著數。形狀永遠回 {date, n}（壞資料當 0 重來）。
...
export function rollDaily(usage, today) {
  ...
  if (typeof usage?.date === 'string' && usage.date > today) return { date: usage.date, n: clean };
  return { date: today, n: usage?.date === today ? clean : 0 };
}

/** @param {'bill'|'day'} kind @param {number} cap */
function budgetError(kind, cap) {
  if (kind === 'bill') {
    return apiError(400, `這份帳單已經讓 AI 讀了 ${cap} 發（你設的單張上限）。讀到這個數通常是版面有問題，不是再試一次就會成——這份請先改用手動記帳；確定要再試的話，到設定頁「AI 解析鑰匙」卡把單張上限調高。`, 'ai_budget_exceeded');
  }
  return apiError(400, `今天已經讓 AI 讀滿 ${cap} 發（你設的單日上限）。這道保險絲是防程式出錯狂重試、把費用燒掉的；明天會自動恢復，急用的話到設定頁「AI 解析鑰匙」卡把單日上限調高。`, 'ai_budget_exceeded');
}

/**
 * 建一份預算（**每個 HTTP 請求一份**，路由層組、閉包進 engine 工廠）。
 * - take()：要一發名額。超限＝throw（400、code `ai_budget_exceeded`、訊息已白話含下一步）。
 *   ⚠️ **序列化鏈＝主力防線**（預審 C1#5 用真櫃檯實證，推翻我先前「主力是 updater 讀 live 計數」
 *   的說法）：LOCAL 的 mutate **整段同步**、await 縫在 take 自己身上——不序列化時，兩個並發 take
 *   的 updater 會在彼此的單張增量（await 之後才 +1）落地前**同步連跑**，邊界上（已用 5、上限 6）
 *   兩發都看到「還剩 1」一起過＝上限 6 變 7。考題用**真櫃檯**釘（假櫃檯在 updater 前多一個
 *   await 會把這個洞遮住——stub 語意必須跟真實作對齊，那次就是這樣漏的）。
 *   updater 內讀 live bill.used＝第二層（櫃檯有交錯語意時再兜一次）。
 ...
 *   多人前置（P3 其餘）要重裁，不是沿用。
 * - today＝**函式**（每一發呼叫時取當下本地日）：請求建立時凍一份日期字串＝跨午夜的請求
 *   整路用舊日寫每日計數（r3#1）；rollDaily 另有「不倒寫較新日」兜底（時鐘怪異時以 db 為準）。
...
export function makeAiBudget({ updateUsage, today, billUsed = 0 }) {
  const bill = { used: countOf(billUsed) };
  ...
  async function doTake() {
    ...
    const day = today();   // 每一發都問一次「今天是哪天」（r3#1：凍在請求建立時＝跨午夜倒寫）
    await updateUsage((settings) => {
      caps = budgetCaps(settings);
      const u = rollDaily(settings?.aiUsage, day);
  ...
  return {
    ...
    loadBill(n) { bill.used = countOf(n); },
