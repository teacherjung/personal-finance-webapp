 ...
 *   並發安全的**主力**＝單張裁決在 updater **裡面**讀 live 的 bill.used（updater 是同步函式＝
 *   裁決原子；實測拔掉下面的序列化鏈、邊界考題照樣綠）。序列化鏈是**第二層保險**：它保證
 *   updateUsage 看到的呼叫嚴格一次一個——櫃檯實作若有批次/交錯語意（CAS 重試的排程細節），
 *   不靠它就得靠排程運氣。⚠️ 誠實劃界：這層在現有考題下是等價的（突變電池記錄在案），
 *   守它的是「不賭 mutate 排程」這個決定，不是某道會紅的考題。
