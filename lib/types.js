 ...
 * @property {string} [bankSummary]              銀行交易的**帳單原文摘要**（Stage 2，2026-08-22）：解析器抄下來、一字未改；顯示名可被改寫，原文不動。服務層寫、非 CRUD
 * @property {string} [bankNote]                 銀行交易的**帳單原文備註**（同上）：與摘要分開存——揉成一句就分不回來。⚠️ **純留底、不參與任何判定**（分類／內轉／去重／學習讀的是匯入當下解析器給的原文，不是這一欄）
