 ...
 * @property {string} [issuer]                   發卡行名稱＝**顯示＋確認**（2026-09-02 起）：有可解析的 `issuerId` 時它負責
 *                                               **確認**那個代號（確認不了＝身分「說不清楚」，見下）；**沒有**可解析代號時，
 *                                               它才是身分本身（文字判準）。⚠️ 不可以寫成「只當顯示」——那句話會讓下一個人
 *                                               以為改它不影響歸卡，而它兩種角色都會影響
 ...
 * @property {string} [currency]
 * @property {number|null} [usdTwd]             美元兌台幣；未設定或 null＝用 fx-rates.js 的預設值並標註（丙）
 ...
 * @property {number|null} [gbpTwd]              當日採用的英鎊匯率（實際採用值：上次抓到的或預設值；更早的列缺值可能是 0 或 null。目前沒有程式讀這格，將來讀的人要把 0 與 null 都當「缺」）
 * @property {number|null} [jpyTwd]              當日採用的日圓匯率（同上）——三種都留底才分得出
 *                                               「淨值變動」是資產本身動了還是匯率動了
 */
