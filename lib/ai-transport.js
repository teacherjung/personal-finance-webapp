...
import { AI_BANK_MODELS, AI_BANK_SCHEMA, buildBankSystem, RECIPE_SCHEMA, buildRecipeSystem } from './ai-parse.js';
  ...
  return async ({ model, system, user, schema }) => {
    ...
    try {
      res = await fetch('https://api.anthropic.com/v1/messages', {
        ...
        // 上界；逾時＝AbortError→ai_unavailable（catch 網路類），frame 結束＝原文參照釋放。
