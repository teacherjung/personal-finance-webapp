...
import { AI_CARD_MODELS, buildCardSystem, AI_CARD_SCHEMA, CARD_RECIPE_SCHEMA, buildCardRecipeSystem } from './ai-parse-card.js';   // 批二：信用卡答案卷（同一條傳輸）
 ...
 * budget（成本護欄 C1）＝**每一發都先過 take() 才准 fetch**：這裡是唯一發射點，雙讀／仲裁／
 * 單讀升級／配方生成全收斂於此——擋在這裡＝未來新增的呼叫路徑也繞不開。超限＝take() 自己
 * throw（400、ai_budget_exceeded、訊息白話）＝這一發根本沒出門、不花錢。
 * @param {string} apiKey
 * @param {{ take: () => Promise<void> }} budget
 * @returns {(req: {model:string, system:string, user:string, schema:object}) => Promise<any>}
 */
export function anthropicTransport(apiKey, budget) {
 ...
 * budget **必帶**（成本護欄 C1）：漏接＝組裝當場炸（接線錯誤即刻可見），不是靜靜不設防——
 * 預設無護欄的話，未來新增的組裝點忘了帶就是無上限（silent-pass 那一族）。
 * @param {string} apiKey
 * @param {{ take: () => Promise<void> }} budget
 */
/**
 * 信用卡版引擎工廠（批二）：同一條傳輸、換答案卷與提示詞。budget 規矩同銀行版（漏接當場炸）。
 * @param {string} apiKey
 * @param {{ take: () => Promise<void> }} budget
 */
export function makeAnthropicCardEngine(apiKey, budget) {
  if (typeof budget?.take !== 'function') throw Object.assign(new Error('AI 引擎缺成本護欄（呼叫端沒帶 budget）——這是程式接線錯誤，不是你的操作問題'), { status: 500, code: 'ai_budget_missing' });
  const transport = anthropicTransport(apiKey, budget);
  return {
    models: AI_CARD_MODELS,
    /** @param {string} text @param {string} model */
    parseOnce: (text, model) => transport({ model, system: buildCardSystem(), user: String(text || ''), schema: AI_CARD_SCHEMA }),
    /** 規則卡生成（批四）：同一條傳輸、換答案卷與提示詞；模型由呼叫端帶（一律 Opus）。 @param {string} text @param {string} model */
    generateRecipe: (text, model) => transport({ model, system: buildCardRecipeSystem(), user: String(text || ''), schema: CARD_RECIPE_SCHEMA }),
  };
}

export function makeAnthropicBankEngine(apiKey, budget) {
  if (typeof budget?.take !== 'function') throw Object.assign(new Error('AI 引擎缺成本護欄（呼叫端沒帶 budget）——這是程式接線錯誤，不是你的操作問題'), { status: 500, code: 'ai_budget_missing' });
  const transport = anthropicTransport(apiKey, budget);
  return {
