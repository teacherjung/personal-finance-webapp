...
// 前綴錨定＋數字邊界（r10#1）：抽字會把日期與店名併成同一格（「2026/07/04 甲店」——
// card-identity.js 記載的同一抽取器行為）；只認整格純日期＝併格交易列被當雜訊（互抵漏抄）。
// 整格純日期照樣命中（字串結尾滿足邊界）。
const DATE_LIKE = /^\d{2,4}[-/.]\d{1,2}[-/.]\d{1,2}(?!\d)/;
const DATE_LIKE_CJK = /^\d{2,4}年\d{1,2}月\d{1,2}日?(?!\d)/;   // r6#1：民國/中文印法（115年07月04日）也是申報形
// r9#1：無分隔印法也是申報形——西元 8 位（20260704）與民國 7 位（1150704），月/日位要像月/日
//（不然 8 位大金額全被當日期＝雜訊列大量誤擋）。DATE_PARSERS 不支援這些格式＝一律拒解退 AI。
...
const dateLike = (/** @type {string} */ s) => {
  ...
  // r11#1：但剝空白會把「2026/07/04 7-ELEVEN」黏成「…047-ELEVEN」＝數字邊界 (?!\d) 不成立
  // ＝數字開頭店名／第二個日期的併格又被當雜訊（互抵漏抄）。空白是 token 邊界——
  // **原樣（保留空白）與剝空白兩種形都測**，任一命中＝申報形。
  const raw = String(s || '').trim();
  const forms = [raw, raw.normalize('NFKC'), raw.replace(/\s+/g, ''), raw.normalize('NFKC').replace(/\s+/g, '')];
  return forms.some((f) => DATE_LIKE.test(f) || DATE_LIKE_CJK.test(f) || DATE_LIKE_BARE.test(f));
};
