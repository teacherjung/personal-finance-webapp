...
export function parseCardWithRecipe(lines, recipe, opts) {
  ...
  /** @type {{label: string, amount: number, date: null}[]} */ const adjustments = [];
  for (const label of recipe.adjustmentLabels) {
    ...
    if (v !== 0) adjustments.push({ label, amount: v, date: null });   // 0＝帳單印了空項，等同缺席
  }
