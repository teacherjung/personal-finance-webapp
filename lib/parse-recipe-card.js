...
export function parseCardWithRecipe(lines, recipe, opts) {
  ...
  for (let i = headerHit[0] + 1; i < lines.length; i++) {
    ...
    // 首格**不是日期**＝雜訊列（小計/換頁），跳過安全的前提＝驗算閘接住漏抄。
    // 首格**是日期**＝申報形的列——形狀或金額歧義**不准**當雜訊跳過（r1#1：一正一負兩筆同時被
    // 跳掉會在加總上互相抵銷、閘看不到）＝整份拒解（寧擋勿收，退回 AI/認不得）。
    if (!d0) continue;
    if (recipe.detail.rowShape === 'date-date-desc-amount') {
      const d1 = toDate(cells[1] || '');
      if (!d1 || cells.length < 4) throw parseFail('明細列的形狀讀不出');
