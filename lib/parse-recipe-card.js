...
const dateLike = (/** @type {string} */ s) => {
  ...
  // 一般解＝**逐 token 起點、逐段黏合**：從每個 token 開始往後黏、每黏一段就測前綴日期形；
  // 黏合段不含更後面的 token＝店名的數字吃不掉無分隔形的邊界。整格純日期／剝空白／原樣
  // 都是這個掃描的特例。劃界：日期黏在文字**後面**（「甲店2026/07/04」）不在 token 起點＝
  // 認不成申報形（抽取器的併格方向＝日期欄在前；反向黏合沒有可判的訊號）。
  const raw = String(s || '').trim();
  for (const base of [raw, raw.normalize('NFKC')]) {
