...
export function validateCardRecipeStrict(raw, opts) {
  ...
  if (!r.totalsLabels || typeof r.totalsLabels !== 'object' || Array.isArray(r.totalsLabels)) errs.push('totalsLabels：必須是物件');
    ...
    // r4#1：四格標籤**兩兩相異**（該趟的尺後）——出生月四格恰好等額時，同一個標籤可矇混四格
    // 並通過重現；下一期四格分家，引擎會把四格全讀成同一格的值、兩道驗算還可能剛好平。
    const seen = new Set();
    for (const f of ['prevDue', 'paidAndRefund', 'newCharges', 'due']) {
      const key = typeof r.totalsLabels[f] === 'string' ? norm(r.totalsLabels[f]) : `#${f}`;
      if (seen.has(key)) errs.push('totalsLabels：四格標籤必須兩兩相異（共用標籤＝下一期讀錯格）');
      seen.add(key);
    }
  }
...
export function parseCardWithRecipe(lines, recipe, opts) {
  ...
  for (let i = headerHit[0] + 1; i < lines.length; i++) {
    ...
    // 停止判定**先看日期**（r4#3）：交易列的某格恰等於停止錨時，先 break 會把該列與其後整段吞掉
    // ＝互抵漏抄又開一扇門。日期開頭又撞錨＝分不出是交易還是收尾＝整份拒解（寧擋勿猜）。
    if (stopHit && (d0 || dateLike(cells[0] || ''))) throw parseFail('明細列撞上停止錨');
    if (stopHit) break;
    // 首格分三種（r1#1→r2#1/#3 的完整判準）：
    // ・**過規則卡日期格式**＝申報形列——一律要嘛成為交易、要嘛整份拒解，不准當雜訊跳過
    //   （一正一負兩筆同時被跳掉會在加總上互相抵銷、驗算閘看不到）。
    // ・**長得像日期、但不過格式**（不補零/別種印法）＝版面日期印法漂了——整份拒解退回 AI
    //   （r2#1：當雜訊跳過＝同一個互抵漏洞從別的門進來）。
    // ・**不像日期**＝雜訊列（小計/換頁），跳過安全（驗算閘接住）。
    if (!d0) {
      if (dateLike(cells[0] || '')) throw parseFail('明細列的日期印法與規則卡不符');
      continue;
    }
