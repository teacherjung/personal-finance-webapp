...
//    軍備競賽由此收口）。代價照實講：帳單裡有未宣告的小計列＝那個版面學不成規則卡、
//    每期照舊走 AI（誤擋方向）。驗算閘仍在（防抄錯數字），但不再獨自承擔「跳過安全」。
//
...
const moneyLike = (/** @type {string} */ s) => {
  ...
  // r17#1：正負號印在幣別記號**前面**（+NT$100／-NT$100）——剝記號時把號留下來
  f = f.replace(/^([+-]?)(?:NT\$|NTD|TWD|US\$|USD|HK\$|\$|€|¥|£)\s*/i, '$1').replace(/\s*(?:元|圓|NTD|TWD|USD)$/i, '');
