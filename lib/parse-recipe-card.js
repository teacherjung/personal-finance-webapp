...
export function parseCardWithRecipe(lines, recipe, opts) {
  ...
  for (let i = headerHit[0] + 1; i < lines.length; i++) {
    const cells = lines[i];
    if (cells.some((c) => stops.has(norm(c)) || totalLabelNorms.has(norm(c)))) break;
    ...
    if (!d0) {
      if (DATE_LIKE.test(String(cells[0] || '').trim())) throw parseFail('明細列的日期印法與規則卡不符');
      continue;
    }
...
export function validateCardRecipeAgainstStatement(lines, recipe, answer, opts) {
  ...
  const toDate = DATE_PARSERS[recipe.dateFormat];
  const isTxLine = (/** @type {string[]} */ cells) => !!toDate(cells[0] || '');
  ...
  for (const [path, v] of slots) {
    ...
    for (const i of findLabelLines(lines, v, norm)) {
