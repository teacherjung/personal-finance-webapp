 ...
 * ④負數列是繳款還是退款，靠 AI 抄回的 desc 字樣判定（desc 不接地）——退款被誤抄成繳款字樣
 *   ＝匯入端把它當繳款跳過、退款無聲漏帳（r8#1）。影子檢查②③（statement-reconcile.js）會對
 *   繳款加總 vs 已繳款桶提醒但不擋；**升級成閘＝該檔註解明定要經 William 裁決**，先照實揭露
 * ⑤容差 1 元的縫隙（Grok 掃#1）：恰好差 1 元的漏抄／多抄一筆（帳單頁碼「1」也接得到地）
 *   兩道閘都放行——CARD_TOLERANCE 要不要收到 0＝金額口徑，留 William 裁；徽章已揭露。
