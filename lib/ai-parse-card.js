 ...
 * ⑤桶裡的退款被**漏抄**（r12#1）：繳款沒列在明細（常見合法印法＝A 式那半不驗桶）時，
 *   「已繳款退款」桶整格沒有可對的列——桶裡的退款漏抄、等式照樣平、匯入就少記那筆退款。
 *   機械上關不掉（強驗桶＝把「繳款只列摘要」的常見版面全數誤擋）；徽章照實揭露、釘子考題釘著
...
export function reconcileAiCard(parsed) {
  ...
  if (Math.abs(g1) > CARD_TOLERANCE) {
    // 差額只在**至少兩個等式項非零**時才報（Grok 掃#2）：只有一項非零時「差額」＝那一格的原值
    //（其他格全 0、應繳 731 ⇒「差了 731 元」＝回聲摘要），與 r9#2 同一條機密紀律。
    const nonZero = [t.prevDue, t.paidAndRefund, t.newCharges, adjSum, t.due].filter((v) => v !== 0).length;
    const gap = nonZero >= 2 ? `差了 ${Math.abs(Math.round(g1))} 元，` : '差額不報＝報了就是唯一非零那格的原值；';
    throw apiError(400,
      `帳單的應繳等式對不上（上期應繳 −已繳退款 ＋本期新增 ＋具名調整 與 本期應繳 ${gap}`
  ...
  if (!convA && !convB) {
    ...
    // 分量報不報＝**比較的兩側都非零才報**（r9#2→Grok 掃#3 通則化）：任一側為 0 時「差額」
    // ＝另一側的原值（零列、淨額恰為 0、摘要印 0 三種形都會把原值回聲出去）。
    const part = (/** @type {string} */ name, /** @type {number} */ lhs, /** @type {number} */ rhs) =>
      (lhs !== 0 && rhs !== 0 ? [`${name}差 ${Math.abs(Math.round(lhs - rhs))} 元`] : []);
    const aParts = [...part('消費和', sumPos - refundAbs, nc), ...part('繳款', payAbs, pr)];
    const bParts = [...part('消費和', sumPos, nc), ...part('桶', payAbs + refundAbs, pr)];
    const detail = (aParts.length || bParts.length)
      ? `兩種常見印法都試過：退款折進「本期新增」時${aParts.join('、') || '該式無列可比'}；退款算進「已繳款退款」桶時${bParts.join('、') || '該式無列可比'}`
      : '明細一筆都沒讀到、帳單摘要卻有金額（差額不報＝報了就是回聲摘要原值）';
    throw apiError(400,
      `逐筆加總與帳單的摘要對不上（${n} 筆，${detail}）——可能有漏抄、多抄、金額抄錯，或繳款退款的字樣抄錯。`
      + '為了不把沒驗算過的數字記進帳本，這一份不收；請改用手動記帳。', 'ai_reconcile_failed');
  }
}

// ── 規則卡生成（批四；模式同銀行 P2-3：填格子、照抄字面、嚴禁交易內容）──────────
import { CARD_RECIPE_LIMITS, CARD_RECIPE_ROW_SHAPES } from './parse-recipe-card.js';
import { RECIPE_DATE_FORMATS, RECIPE_FORMAT_VERSION } from './parse-recipe.js';

export const CARD_RECIPE_SCHEMA = {
  type: 'object', additionalProperties: false,
  required: ['bank', 'docAnchors', 'dateFormat', 'totalsLabels', 'adjustmentLabels', 'lastFourLabel', 'monthLabel', 'detail'],
  properties: {
    bank: { type: 'string', description: '發卡機構名（帳單抬頭印的，照抄）' },
    docAnchors: { type: 'array', maxItems: CARD_RECIPE_LIMITS.docAnchors, items: { type: 'string' },
      description: '2–4 個「這份版面獨有」的字面標題文字（照版面逐字抄、不含數字），用來認出同版面' },
    dateFormat: { type: 'string', enum: [...RECIPE_DATE_FORMATS], description: '明細日期的印法' },
    totalsLabels: {
      type: 'object', additionalProperties: false, required: ['prevDue', 'paidAndRefund', 'newCharges', 'due'],
      properties: {
        prevDue: { type: 'string', description: '「上期應繳總額」那一格的字面標籤（照這份帳單印的抄）' },
        paidAndRefund: { type: 'string', description: '「已繳款／退款」合計格的字面標籤' },
        newCharges: { type: 'string', description: '「本期新增款項」格的字面標籤' },
        due: { type: 'string', description: '「本期應繳總額」格的字面標籤' },
      },
    },
    adjustmentLabels: { type: 'array', maxItems: CARD_RECIPE_LIMITS.adjustmentLabels, items: { type: 'string' },
      description: '不在明細區的具名金額列標籤（循環信用利息、違約金、年費、分期攤還…）——這家帳單會印的都列進來；沒有＝空陣列' },
    lastFourLabel: { type: ['string', 'null'], description: '卡號末四碼旁印的字面標籤；帳單沒印末四碼＝null' },
    monthLabel: { type: ['string', 'null'], description: '帳單期別／結帳日旁印的字面標籤（同列要有日期）；沒有＝null' },
    detail: {
      type: 'object', additionalProperties: false, required: ['headerAnchor', 'rowShape', 'stopAnchors'],
      properties: {
        headerAnchor: { type: 'string', description: '消費明細表頭列的字面文字（如 交易日）' },
        rowShape: { type: 'string', enum: [...CARD_RECIPE_ROW_SHAPES],
          description: '明細列的長相：date-date-desc-amount＝消費日＋入帳日＋說明＋金額；date-desc-amount＝消費日＋說明＋金額' },
        stopAnchors: { type: 'array', maxItems: CARD_RECIPE_LIMITS.stopAnchors, items: { type: 'string' },
          description: '明細區結束列的字面文字（如 本期合計）；沒有＝空陣列（讀到摘要標籤自動停）' },
      },
    },
  },
};

/** 卡片規則卡生成提示（system）：規矩同銀行 buildRecipeSystem（單一精神、各自的格子）。 */
export function buildCardRecipeSystem() {
  return [
    '你是信用卡帳單「版面規則卡」的填表員。根據使用者提供的帳單文字，把版面結構填進固定格子。',
    '規則：',
    '1. 每一格只准照抄版面上印的**字面文字**（標題、表頭、標籤）；一個字都不可以改寫或翻譯。',
    '2. 枚舉欄位（dateFormat/rowShape）只准從格子說明的清單選。',
    '3. **嚴禁任何交易內容**：金額、卡號、日期值、店名都不可以出現在任何格子裡。',
    '4. docAnchors 挑「這份版面獨有、每期都會印」的標題文字 2 到 4 個。',
    '5. adjustmentLabels 列出這家帳單「不在明細區的具名金額列」的標籤（利息／違約金／年費／分期…）——它們決定應繳等式怎麼平。',
    '6. 版面沒有的欄位照格子說明填 null 或空陣列；不可以硬湊。',
  ].join('\n');
}

/**
 * 候選規則卡白名單化（零信任，同銀行 pickRecipeCandidate）：只搬 schema 內的鍵、蓋上
 * formatVersion——AI 多給的鍵一律丟棄；**刻意不替 AI 補 null/[]**（漏欄＝出生第一關擋，
 * 不是這裡好心修補）。深層合法性交給出生把關。
 * @param {any} raw
 */
export function pickCardRecipeCandidate(raw) {
  if (!raw || typeof raw !== 'object') return { formatVersion: RECIPE_FORMAT_VERSION };
  /** @type {any} */ const out = { formatVersion: RECIPE_FORMAT_VERSION };
  for (const k of ['bank', 'docAnchors', 'dateFormat', 'totalsLabels', 'adjustmentLabels', 'lastFourLabel', 'monthLabel', 'detail']) {
