...
export function reconcileAiCard(parsed) {
  ...
  // 兩種各自全對才收其一；抄錯身分（退款↔繳款跨桶）兩種慣例都會對不上＝擋。
  ...
  // payAbs===0 時繳款桶驗不了、只驗得了退款那一半；有列出來（payAbs>0）就要對得上。
  // r8 攻擊不因此漏接：退款被抄成繳款會把退款移出消費和（A 的第一式）或移出桶（B 的第二式），
  // 兩式在**金額**上就對不上了，缺席放行救不了它。
  const eq = (/** @type {number} */ x, /** @type {number} */ y) => Math.abs(x - y) <= CARD_TOLERANCE;
  const convA = eq(sumPos - refundAbs, nc) && (payAbs === 0 || eq(payAbs, pr));
  const convB = eq(sumPos, nc) && (payAbs === 0 ? refundAbs <= pr + CARD_TOLERANCE : eq(payAbs + refundAbs, pr));
  if (!convA && !convB) {
    const gA = Math.abs(Math.round((sumPos - refundAbs) - nc)), gB = Math.abs(Math.round(sumPos - nc));
    throw apiError(400,
      `逐筆加總與帳單的摘要對不上（${n} 筆，兩種常見印法都試過：退款折進「本期新增」差 ${gA} 元、`
      + `退款算進「已繳款退款」桶差 ${gB} 元）——可能有漏抄、多抄、金額抄錯，或繳款退款的字樣抄錯。`
