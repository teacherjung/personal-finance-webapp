...
export function reconcileAiCard(parsed) {
  ...
  if (!convA && !convB) {
    ...
    // 消費側同規則（r9#2）：該側**零列**時 0 減整格摘要＝把原值當差額回聲出去——一律「有列才報」。
    const consHas = sumPos > 0 || refundAbs > 0;
    const aParts = [...(consHas ? [`消費和差 ${Math.abs(Math.round((sumPos - refundAbs) - nc))} 元`] : []),
      ...(payAbs > 0 ? [`繳款差 ${Math.abs(Math.round(payAbs - pr))} 元`] : [])];
    const bParts = [...(consHas ? [`消費和差 ${Math.abs(Math.round(sumPos - nc))} 元`] : []),
      ...((payAbs > 0 || refundAbs > 0) ? [`桶差 ${Math.abs(Math.round((payAbs + refundAbs) - pr))} 元`] : [])];
