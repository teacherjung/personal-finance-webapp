...
export function assertAiCardGrounded(parsed, text) {
  ...
  toks.forEach((t, i) => { if (WELL_FORMED.test(t.tok)) addEntry(t.tok, [i]); });
  ...
  // 末四碼恰等於某格摘要時，虛構明細可以借卡號那格當「金額位置」。偏好較長的 token（完整卡號
  // 那型，占掉它不影響任何金額）；找不到含末四碼的 token＝接不了地，照樣 miss。
  if (parsed.lastFour != null) {
    const lf = parsed.lastFour;
    const cands = toks
      .map((t, i) => ({ i, s: t.tok.replace(/,/g, '') }))
      .filter((c) => c.s.includes(lf));
