 ...
 * 尾綴負號／括號負數這類印法會被讀成正數、配不上帶負號的宣稱（fail-closed 方向，寧擋勿收）。
...
export function assertAiCardGrounded(parsed, text) {
  ...
  toks.forEach((t, i) => addEntry(t.tok, [i]));
  for (let i = 0; i + 1 < toks.length; i++) {
    if (/^[^\S\n]+$/.test(stripped.slice(toks[i].end, toks[i + 1].start))) addEntry(toks[i].tok + toks[i + 1].tok, [i, i + 1]);
  }
