 ...
 *
 * ⚠️ 誠實劃界（同銀行線）：這是防禦縱深不是閘、不宣稱窮盡——日期與店名刻意不接地
 * （民國→西元轉換後字面必然不同）；同一個金額在帳單多處印過時仍可能互借；
 * 尾綴負號／括號負數這類印法會被讀成正數、配不上帶負號的宣稱；金額被拆進**三個以上** cell
 * （1,234,│567,│890）只做相鄰兩兩拼接＝接不回來；貪婪配位理論上可能比完美配對多擋
...
export function assertAiCardGrounded(parsed, text) {
  ...
  for (let i = 0; i + 1 < toks.length; i++) {
    ...
    if ((toks[i].tok.endsWith(',') || toks[i + 1].tok.startsWith(',')) && WELL_FORMED.test(combined)) {
  ...
  // 候選與偏好（r4#2 收緊）：①**只認不含逗號的 token**——卡號從不印千分位，「91,234」是金額、
  //   剝掉逗號才含 1234 純屬巧合，讓它入選＝占錯位、把真卡號 token 留給虛構明細借。
  //   ②排序＝「大到不可能是金額」（完整卡號那型，占它零成本）＞「末四碼」本人（token 恰為四碼）
  //   ＞其餘（越長越像卡號片段）。找不到＝接不了地，照樣 miss。
  if (parsed.lastFour != null) {
    const lf = parsed.lastFour;
    const cands = toks
      .map((t, i) => ({ i, raw: t.tok }))
      .filter((c) => !c.raw.includes(',') && c.raw.includes(lf));
    const score = (/** @type {{raw: string}} */ c) => {
      const v = Math.abs(Number(c.raw));
      if (Number.isFinite(v) && v > CARD_LIMITS.amountAbs) return 0;
      if (c.raw === lf) return 1;
      return 2;
    };
    cands.sort((a, b) => score(a) - score(b) || b.raw.length - a.raw.length);
    const pick = cands.find((c) => !used.has(c.i));
    if (!pick) throw miss('lastFour');
    used.add(pick.i);
  }
