...
// ⚠️ 這裡**不可以**寫成 `… + sep`：那是一個含 import.meta 的 BinaryExpression，
//    進入點守衛的選擇器（`BinaryExpression:has(MetaProperty)`）會擋下來。分隔符在使用處才接。
 ...
 *        ——環境變數任何人都能自己設（Codex #538 r4）。它真正買到的是：**破壞面被關在我們自己的假替身裡**。
...
export function setPdfChildScriptForTest(path) {
  ...
  //      ②**就算被呼叫，也只能換成 `test-doubles/` 底下的檔案**（下面那道路徑檢查）——
  //        沒有辦法指到任意腳本，破壞面被關在我們自己的假替身裡。
  ...
  if (path !== null) {
    ...
    // 前綴比對要帶分隔符，否則 `…/test-doubles-evil/x.js` 也會被當成合格
    if (!abs.startsWith(DOUBLES_DIR + sep)) {
      throw new Error(`子行程腳本只能換成 test-doubles/ 底下的檔案（收到：${abs}）`
        + '——這道限制讓「就算有人繞過前面兩層」的破壞面關在我們自己的假替身裡。');
    }
  }
