...
//    修好之後實測（macOS／Node 26／子行程 `--max-old-space-size=256`／pdfjs 6.1.200）：
//    兩顆攻擊檔都由文字節點牆擋下、回乾淨的 400，子行程峰值 RSS 317MB／349MB，父行程不長。
//    ⚠️ **記憶體數字的單一真相在 `lib/parse-limits.js` 的 `readPageTextCapped`**——這裡不再抄一份
//   （抄本會漂：本檔原本寫 612→406／704→456，那邊寫 612→254／704→316，兩份對不起來也沒人發現）。
//
// ## 為什麼是「隔離」而不是「再蓋一道牆」
//
...
// 子行程怎麼死都不影響父行程。實測：兩個攻擊檔都讓子行程死掉，**父行程 RSS 完全沒變**（停在 47MB）。
//
 ...
 *    三種死法各自釘死、完全不靠時間。
 * ⚠️ **誠實劃界**（Codex #538 r1）：實作是 `childScriptOverride ?? CHILD`，所以「正式路徑永遠用
 *    `CHILD`」**不是程式強制得了的保證**。目前撐得住的說法是：production tree 沒有任何呼叫者、
 *    這個值也不由外部輸入決定；考題檔用 `afterEach` 集中還原（漏還原會有題轉紅，突變驗過）。
 * @param {string|null} path
 */
export function setPdfChildScriptForTest(path) { childScriptOverride = path; }
