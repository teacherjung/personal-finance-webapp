...
/** 考題用：縮小子行程回傳量上限（否則要真的灌 64MB 才踩得到牆）。傳 null 還原。
 *  與 `setPdfTimeoutForTest` 同款接縫：只改門檻、不改判定與收尾——考題踩的是正式那條路。 @type {number|null} */
let resultLimitOverride = null;
/** @param {number|null} bytes */
export function setPdfResultLimitForTest(bytes) { resultLimitOverride = bytes; }
...
function runInChild(kind, data, password) {
  return new Promise((resolve, reject) => {
    ...
    child.stdout.on('data', (b) => {
      out += b;
      // ⚠️ 這道牆拆了不會有任何一題轉紅——2026-09-02 第二輪稽核實測（壓縮炸彈解開後灌回主行程＝全站 OOM 重啟）；
      //    現由 test/pdf-isolate.test.js 的「回傳量炸彈」題釘住：超標必須當場 SIGKILL 並回 400 pdf_result_too_large。
      if (out.length > (resultLimitOverride ?? MAX_RESULT_BYTES)) { tooBig = true; try { child.kill('SIGKILL'); } catch { /* 已經死了 */ } }
    });
