...
function runInChild(kind, data, password) {
  return new Promise((resolve, reject) => {
    ...
    //    改走 stdin 首行 JSON 標頭（任意字元都安全），base64 內容接在換行之後。
    // ⚠️ **底下沒有給 `env` 選項是刻意的，但它不是「乾淨」——是整份 `process.env` 直接繼承**。
    //    所以「不要把密碼放進 env」不只是「不要寫 `env: {…}`」，**也包括不要先寫進 `process.env` 再 spawn**
    //    （後者原本完全沒有東西在看：那一題 2026-09-25 之前是搜本檔原始碼的 `/env\s*:\s*\{[^}]*password/`）。
    // ⚠️ **考題怎麼釘的（2026-09-25 換過形狀）**：`test/pdf-isolate.test.js` 讓子行程自己回報
    //    它**實際拿到**的 argv 與 env 有沒有含那個密碼（替身＝`test-doubles/pdf-child-echo-io.js`），
    //    不再搜本檔的字面——舊那一版**換個變數名就繞過**（實測 39 題全綠，而 `ps` 讀得到明碼）。
    //    射程：釘的是這一條 `runInChild`（argv 只在下面組一次、`kind` 只是參數），
