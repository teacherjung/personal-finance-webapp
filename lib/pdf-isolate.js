 ...
 *    三種死法各自釘死、完全不靠時間。
 * ⚠️ **誠實劃界**（Codex #538 r1/r2）：實作是 `childScriptOverride ?? CHILD`，所以「正式路徑永遠用
 *    `CHILD`」**不是這一行程式強制得了的**。撐住它的是別的東西，兩樣都有考題：
 *      ①`test/pdf-isolate.test.js` 的架構題——正式程式碼（lib／public／scripts／server.js，
 *        含還沒 git add 的新檔）**不可以有任何呼叫者**；有人加一個就轉紅。
 *      ②同檔的 `afterEach` 集中還原——考題之間不會互相污染（拿掉 hook 會有題轉紅）。
 *    這個值也不由任何外部輸入決定（不讀 env、不讀請求）。
