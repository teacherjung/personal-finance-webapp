...
//    上限**（off-heap 管不到）；而且**死法不只 OOM**——更常見的是 pdfjs 卡死在解壓、promise 永不
//    settle，子行程事件迴圈一空就 `code 0` 靜默退出（不是 exit 13、也沒有 OOM 訊息）。
...
/**
 * 抽取 PDF 的每一行——**HOSTED 走子行程、LOCAL 直接跑**。
 * @param {string} kind @param {(d: Uint8Array, p?: string) => Promise<any[]>} inProcess
 * @param {Uint8Array} data @param {string=} password @returns {Promise<any[]>}
 */
