...
import { loadKv, saveKv, currentVersions } from './store-pg.js';
 ...
 * 而且那條路仍然走 CAS（先讀目前版本再比對），只是不要求「是我剛才讀的那一版」。
 * @param {Db} db @param {{overwrite?: boolean}=} opts
 */
export async function saveDb(db, opts) {
  if (!isHosted()) { save(db); return db; }
  const stamped = /** @type {any} */ (db)[VERSIONS];
  const expected = opts?.overwrite ? await currentVersions() : stamped;
  if (!expected) {
    throw Object.assign(
