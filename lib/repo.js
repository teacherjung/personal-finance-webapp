...
/**
 * 配方寫入櫃檯（P2-3）：**唯一**的 parseRecipes 服務內寫入口（apply 的計數走同交易 db 直改、不經這裡）。
 ...
 * @param {{rebirthId?: string, bank?: string}} [o]
...
export async function saveParseRecipe(recipe, o = {}) {
  return mutate((db) => {
    ...
    const target = o.rebirthId ? rows.find((/** @type {any} */ r) => r?.id === o.rebirthId) : null;
    if (target) {
