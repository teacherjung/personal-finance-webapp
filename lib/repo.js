...
// （本段之後還有一支 `backupSupported()`：它**不碰檔案**、只回答「這個模式有沒有備份這回事」，
//  所以上面那三條「憑空建出本機 SQLite」的理由對它不適用；同步簽名的理由則相同。）
 ...
 * 這個模式**有沒有「操作前備份」這回事**。
 *
 * 為什麼要有這一支：`backupNow` 的 `false` 有**兩種完全不同的意思**——
 * ①本機版真的備份失敗（硬碟滿、權限不足）＝安全網不見了，是**異常**；
 * ②HOSTED 一律回 false ＝這個模式**設計上就沒有**這一份（上面那行的「不做也不假裝做」），是**常態**。
 * 呼叫端只看布林值分不出來，把②當成①的話，雲端版每一次不可逆操作都會被當成備份失敗擋下來。
 * ⚠️ 判準集中在這裡、不要在服務層各自 `isHosted()` 一次——散開來就會與 `backupNow` 的實作走散
 * （哪天它在別的情況也回 false，只有這裡改得到）。閘門的唯一實作在 `lib/services/backup.js`。
 * ⚠️ **維持同步簽名**（同上面三支的理由）：純模式判斷、不碰檔案，沒有 async 的理由。
 * ⚠️ **誠實劃界：本函式目前一個呼叫點都沒有**（唯一的使用端是 08-06 那層閘門，已於 08-08 隨裁決移除；
 *    每日備份走的是 `snapshotTo`，不經過這裡）。留著是因為那個「兩種 false 要分開」的判斷本身沒有錯，
 *    但依「沒用到的程式直接刪」（William 2026-08-03）它的去留待裁決——**不要因為它存在就去接一個新用途**。
 * @returns {boolean} true＝這個模式有操作前備份（＝false 真的代表失敗）
 */
export function backupSupported() { return !isHosted(); }
