...
//
// **async 契約（C4a，2026-07-27）**：全部函式改為 async——HOSTED 模式（C4b）的 Postgres client
// 必然非同步，櫃檯簽名先行統一，呼叫端一律 `await`。LOCAL 模式底層仍是同步 SQLite、
// 函式體內沒有任何真正的 await 點：呼叫後立即完成、只隔一個 microtask——
// **Node 在清空 microtask queue 之前不會處理下一個 HTTP 請求**，所以 LOCAL 下
// 「await getDb() → 改 → await saveDb()」與改造前的同步版本一樣，不會被並發請求插隊，
// 讀改寫的原子性不變（test/repo-async.test.js 有考題證明）。
// ⚠️ 呼叫端規矩：read-modify-write（getDb…saveDb）之間**不可再夾真正的外部 IO await**
//（fetch／fs／timer）——那會打開交錯窗口；既有的少數先例（ib-sync 等「先 fetch 完才開櫃檯」
// 不在此列）盤點記錄在 C4b 的 CAS 待辦，C4a 不新增。
...
export async function getDb() { return loadSynced(); }

/** 寫回整包資料（與 getDb 成對使用）。 @param {Db} db */
export async function saveDb(db) { save(db); return db; }

/** 讀某個集合（不存在回空陣列）。 @param {string} col @returns {Promise<any[]>} */
export async function getCollection(col) { return /** @type {any} */ (loadSynced())[col] || []; }
 ...
 * 物件動手，一旦允許 async 就等於在讀寫之間開 await 窗口——HOSTED（C4b）的 CAS 也依賴這個假設。
...
export async function getSettings() { return loadSynced().settings; }
