...
export async function updateAiUsage(updater) {
  return mutate((db, skip) => {
    const next = updater(db.settings);
    const cur = db.settings?.aiUsage;
    // 值沒變＝skip 不落盤（預審 C1#6：被上限擋下的 take 也會走到這裡——寫回相同值仍是一次全庫
    // 寫入，HOSTED 下還會推 kv 版本讓並發寫對撞；沒變就別動）。
    if (cur && next && cur.date === next.date && cur.n === next.n) { skip(); return cur; }
    db.settings = { ...db.settings, aiUsage: next };
    return next;
  });
}
 ...
 * @param {{rebirthId?: string, notAfter?: string, bank?: string, kind?: string}} [o]
 *   kind＝櫃子列的種類標籤（批四：'card'＝信用卡規則卡；缺席＝銀行既有卡）——記在列上不進配方本體，
...
export async function saveParseRecipe(recipe, o = {}) {
  return mutate((db) => {
    ...
    // 重生鎖同 kind（#532 r1#4）：rebirthId 撞到**別種**的列（生成在途被還原/換代讓同 id 變異 kind）
    // ＝不可跨線覆寫版本與畢業狀態——改走新建。判準同樣在 mutate 內用 fresh row 做。
    const kindOf = (/** @type {any} */ v) => (String(v || '') === 'card' ? 'card' : 'bank');   // 缺席 ≡ 'bank'（r4#6：顯式 bank 的合法備份不可被誤判跨櫃）
    if (target && kindOf((/** @type {any} */ (target)).kind) !== kindOf(o.kind)) target = null;
    if (target) {
    ...
    rows.push({ id, bank: String(/** @type {any} */ (recipe)?.bank || o.bank || ''), current: recipe,
      ...(o.kind ? { kind: String(o.kind) } : {}),   // 批四：種類標籤（銀行既有卡缺席＝'bank' 語意）
