...
//   子 → 父：stdout ＝ 一行 JSON。`{ok:true, result, diag}` 或 `{ok:false, message, status, diag}`
//   （`diag` ＝ 封閉集合的診斷代碼陣列，見 `lib/parse-limits.js` 的 `PARSE_DIAG`；零內容）
 ...
 * **唯一的寫出口**——診斷代碼只在這裡掛上去。
 *
 * ⚠️ 為什麼收成一個（Grok 掃描 #2，2026-09-02）：原本成功與失敗兩條路**各自**寫
 *    `diag: await takeDiag()`，而考題只驗「有一個叫 diag 的陣列」⇒ 把成功路那一份改成 `diag: []`
 *    照樣全綠（`Array.isArray([])` 是真）。**與其為兩條路各補一題，不如讓它只有一條路**：
 *    分支不存在，就沒有「其中一條靜靜壞掉」這回事。
 * @param {Record<string, any>} payload
 */
async function writeOut(payload) {
  process.stdout.write(JSON.stringify({ ...payload, diag: await takeDiag() }));
}
...
main().then(async (payload) => { done(); await writeOut(payload); }).catch(async (e) => {
  done();
  ...
  // ⚠️ 診斷代碼由 `writeOut` 統一掛上（封閉集合、零內容，見 lib/parse-limits.js 的 PARSE_DIAG）：
  //    子行程的 stderr 只在「沒有 stdout 且不是資源／逾時」那條路才會被父行程截斷記錄——
  //    正常回了 JSON（含使用者層 400）的路完全不看它，取消失敗這種「原錯照樣回得去」的狀況
  //    因此在 HOSTED 完全靜默。那個欄位是它唯一的出聲管道。
  await writeOut({
    ok: false,
