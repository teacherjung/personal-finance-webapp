 ...
 * ⚠️⚠️ **`path` 為 `null` ＝「這一筆沒有穩定身分」**（2026-07-29，Codex 定向複審 v4 抓到的 blocking）。
 *    以前無論如何都給一個字串路徑，缺 id 時算出 `accounts..accountNo`。
 *    那個字串**看起來像座標，其實不是**——它會把「目前資料裡那筆沒有 id 的帳戶」
 *    和「匯入檔裡另一筆完全不同、也沒有 id 的帳戶」對到同一格。
 *    兩側各只有一筆時連撞號都數不出來，於是**新帳戶會靜靜地拿到舊帳戶的完整帳號**（還回 200）。
 *    後果就是這張清單本來要防的事情反過來發生：錯的帳號拿去配銀行帳單、判自家末碼、分內轉。
 *
 *    所以身分不明時**寧可不給座標**：剝除照做（回傳值照用，備份檔一樣不帶帳號），
 *    但呼叫端不可以拿它當「還原的座標」。判準與 `lib/store-pg.js` 對 `cards..pdfPassword`
 *    的處置一致：**寧可少救一個欄位，也不要把資料寫錯。**
 *
 * @template T @param {T} db @param {(value: string, path: string|null) => string} fn @returns {T}
 */
export function mapBackupOnlyPii(db, fn) {
  ...
  if (Array.isArray(d.accounts)) {
    for (const a of d.accounts) {
      ...
      a.accountNo = fn(String(a.accountNo ?? ''), id ? `accounts.${id}.accountNo` : null);
    }
  }
  return db;
}
