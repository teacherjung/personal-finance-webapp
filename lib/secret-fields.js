...
//    **HOSTED 刻意相反**（C5 裁決⑤）：走 `stripSecretsForBackup` 剝掉三個機密＋`accountNo`，
...
export function projectSettings(s) {
  ...
  const { taishinSecPdfPassword, rememberedStatementPasswords, ...rest } = s;
  let pwCount = 0;
  ...
  const out = { ...rest, taishinSecPdfPasswordSet: isSet(taishinSecPdfPassword), rememberedStatementPasswordsCount: pwCount };
 ...
 *    帳號**（`lib/services/bank-import.js` 的 `bankRefBase`），而**銀行對帳單匯入自動建立的帳戶，
