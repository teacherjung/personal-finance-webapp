 ...
 *
 * ⚠️ **射程：只管「辨識」**（暗號、銀行身分、區段／收尾錨點、表頭與欄位座標）。
 * **拒收型的守門**（欄名重複、區段錨點互為子字串、出生對照的等值與位置）**刻意只走新尺＝比 main 嚴**
 * ——那是 fail-closed 的方向（配方孵不出來就走 AI），是刻意的，不要「順手」也改成 `||`。
...
export function parseWithRecipe(lines, recipe) {
  ...
  for (const line of lines) {
    ...
    const hit = recipe.summary.sections.find(s => hitEither(rawJoinedLine, s.anchor));   // 舊尺或新尺（裁示「甲」）
    if (hit) { section = { currency: hit.currency }; pendingCurrency = null; continue; }
    // 收尾錨點也是舊尺或新尺（裁示「甲」）——只用新尺會讓 main 收得了尾的版面收不了尾。
    if (endAnchorOld && (squash(cells[0] || '').startsWith(endAnchorOld) || recipeNorm(cells[0]).startsWith(endAnchorNew))) { section = null; continue; }
    if (!section) {
