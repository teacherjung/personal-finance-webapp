...
export function parseWithRecipe(lines, recipe) {
  ...
  // 銀行身分綁定（r8#2＋r9#1 收緊）：配方的 bank 會變成機構戳與 bank2 去重鍵的機構段——
  // 別家銀行的帳單若共用暗號與幾何，冒名解析＝跨行去重與帳戶歸屬全錯。機械驗證＝
  // 銀行名必須出現在**非交易列**（版面文字）：出現在交易摘要（「合成郵局卡費」）不算數
  // ——那是乙行帳單上關於甲行的一筆交易，不是發單機構（r9#1 實測形）。
  const bankQ = squash(recipe.bank);
  const isTx = makeIsTxRow(recipe);
  if (!lines.some(l => !isTx(l.cells) && squash(l.cells.map(c => c.s).join('')).includes(bankQ))) {
    throw parseFail('銀行身分對不上');
  }
  const toDate = DATE_PARSERS[recipe.dateFormat];
