...
export function parseWithRecipe(lines, recipe) {
  ...
  // **舊尺**的同一份判準：只給參考日用（理由見概要迴圈裡參考日那段——新尺提早認出表頭會把
  // main 讀得到的參考日整個跳掉）。⚠️ 不要拿它做別的判斷：明細那邊要的是新尺（那是本支的目的）。
  const hOutP = squash(recipe.detail.headerOut), hInP = squash(recipe.detail.headerIn), hBalP = squash(recipe.detail.headerBalance);
  const isHeaderLineOld = (/** @type {string} */ t) => t.includes(hOutP) && t.includes(hInP) && t.includes(hBalP);
  ...
  for (const line of lines) {
    ...
    const plainJoined = squash(cells.join(''));   // ⚠️ 舊尺——參考日與它的表頭 guard 專用，刻意不是上面那條
    ...
    if (!isHeaderLineOld(plainJoined) && !refLocked && referenceDate == null && refAnchor) {
