 ...
 *    `parseWithRecipe` 裡 label 那一段（原文留底契約／精準去重鍵／帳號身分）。
...
/** 從整列文字裡撈出所有日期。回 `{iso, at}`——`at`＝在**輸入字串**裡的起始位移，
 * 供呼叫端判斷「這個候選在不在錨點之後」（理由見 parseWithRecipe 裡參考日那段）。 */
const DATE_SCANNERS = {
  'west-slash': (/** @type {string} */ t) => [...t.matchAll(/(\d{4})\/(\d{2})\/(\d{2})/g)].map(m => ({ iso: `${m[1]}-${m[2]}-${m[3]}`, at: m.index ?? 0 })),
  'roc-slash': (/** @type {string} */ t) => [...t.matchAll(/(?<!\d)(\d{2,3})\/(\d{2})\/(\d{2})/g)].map(m => ({ iso: `${String(Number(m[1]) + 1911).padStart(4, '0')}-${m[2]}-${m[3]}`, at: m.index ?? 0 })),
};
  ...
  for (const line of lines) {
    ...
    if (!refLocked && referenceDate == null && refAnchor) {
      ...
      if (joined.includes(refAnchor)) {
        ...
        // ⚠️ **候選完全在舊尺上產生、只有「在不在錨點之後」用新尺判**（Codex #523 r1→r2 兩輪
        //   換來的形狀；我前兩版各錯一次，教訓留在這裡）：
        //   ①正規化會**多出**候選——全形日期變得掃得到 ⇒ anchored-date 取第一個就換人
        //     （r1 實測：base 讀 2026-06-30、只換尺讀 2099-05-06＝現值日被寫成未來日，
        //      往後每期真帳單都比它舊 ⇒ 餘額更新一路被當 stale 跳過）。
        //   ②正規化也會**吃掉**候選——`２115/05/06` 經 NFKC 黏成四位數，`(?<!\d)` 把民國日期
        //     整顆消掉（r2 實測三種策略：2026→2027、2027→null、null→2028，各自寫錯或靜靜停更新）。
        //     ⚠️ 我 r1 補的濾網只擋得住①：它只能**刪**候選，救不回被吃掉的。
        //   ⇒ 唯一站得住的形狀＝**掃描一律跑在未正規化的整列上**（候選集合與順序與 base 逐字相同），
        //     「在錨點之後」則把該候選**之前那段前綴**過新尺再找錨點——相容字錨點也定得了位。
        // 代價（誠實）：整份用全形印參考日的版面照舊讀不到 ⇒ referenceDate=null ⇒ 餘額不更新
        //（同規則 1a「寧可 null」；與 base 逐字相同，不是本支新增的限制）。
        const rawJoined = squash(cells.join(''));
        const ds = scanDates(rawJoined).filter(d => recipeNorm(rawJoined.slice(0, d.at)).includes(refAnchor)).map(d => d.iso);
        if (ds.length > 0) {
          refLocked = true;
    ...
    // ⚠️⚠️ **引擎輸出的文字欄一律只 squash、不過 `recipeNorm`——這是一條界線，不是還沒做**
