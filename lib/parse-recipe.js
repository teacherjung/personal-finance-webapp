...
export function parseWithRecipe(lines, recipe) {
  ...
  for (const line of lines) {
    ...
    if (section.currency !== 'BY-CODE' && !cells.some(isMaskedAccount)) {
      ...
      if (cells.some(c => CURRENCIES.includes(c.trim()) && c.trim() !== secCur)) {
    ...
    if (section.currency !== 'BY-CODE') {
      ...
      if (cells.some(c => CURRENCIES.includes(c.trim()) && c.trim() !== secCur2)) {
        throw parseFail('固定幣別區出現矛盾的幣別標題');
      }
    } else {
      // BY-CODE 帳戶列自帶的明確幣別格必須與 sticky 一致（r16#2＝r15#1 同族漏口：
      // sticky=JPY、帳戶列印著 USD ⇒ 原版把帳戶標成 JPY、USD 掉進 note＝美元餘額被建成日圓）
      const rowCodes = [...new Set(cells.map(c => c.trim()).filter(c => CURRENCIES.includes(c)))];
  ...
  for (const line of lines) {
    ...
    if (isHeaderLine(jc)) {   // 與概要迴圈同一份判準（單一實作）
      ...
      // 金額區**之前**（帳號/日期/摘要標籤）＝摘要身分區、值本來就歸摘要——不在此限（劃界入契約）。
      {
        ...
        const zoneStart = Math.min(xOut, xInCol);
        for (const cell of c) {
    ...
    if (dateIso && restFrom) {
      ...
      let outCount = 0, inCount = 0;   // 同窗多候選＝歧義（r16#1：先到先贏會讓 $100 被記成 $7）
      ...
      if (outCount > 1 || inCount > 1 || (outCell && inCell)) throw parseFail('欄位判定歧義');
      ...
      if (!txnCell) {
        ...
        // 退路也收全部候選、超過一格＝歧義（r17#1：.find() 先到先贏——次月版面多出的
        // 可選欄讓 7 蓋掉 100、與 r16 同族；「一窗一格」的規則兩條路同一把尺）
        const fbCands = amtCells.filter(x => {
          if (!strictMoney(x)) return false;   // 退路同一把尺（r11#1）
        ...
        if (fbCands.length > 1) throw parseFail('欄位判定歧義');
        txnCell = fbCands[0] || null;
