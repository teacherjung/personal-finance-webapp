...
export function parseWithRecipe(lines, recipe) {
  ...
  // 同號混台幣＋外幣＝三條路一律拒收（見 bank-statement.js makeCurrencyTable 的說明）。
  // 配方路線走 parseFail＝這張配方對這份帳單「讀不動」，呼叫端 fail-closed 當 miss（不會靜靜產出歧義結果）。
  if (curTable.hasMixedTwd()) throw parseFail(MIXED_CURRENCY_MSG);
