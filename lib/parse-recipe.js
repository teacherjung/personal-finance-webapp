...
export function parseWithRecipe(lines, recipe) {
  ...
  for (const line of lines) {
    ...
    if (!refLocked && referenceDate == null && refAnchor) {
      ...
      let cut = -1;
      for (let i = 1; i <= rawJoined.length; i++) {
        if (recipeNorm(rawJoined.slice(0, i)).includes(refAnchor)) { cut = i; break; }
      }
      if (cut >= 0) {
