...
/**
 * 配方比對的**同一把尺**：先 NFKC 正規化、再去空白。文字層每一個「配方槽位 ↔ 帳單原文」
 * 的比對點，**兩邊都過它**。
 *
 ...
 *
 * ⚠️ **射程只到文字層（誠實劃界）**：金額（`parseAmount`）、日期格（`DATE_PARSERS`）、
 * 遮罩帳號（`isMaskedAccount`）三支**刻意不過這把尺**——它們與內建範本共用實作，改了就是改
 * 台新的 bankRef（既有去重鍵一個位元組都不能變）。代價：值層印相容字（例如全形數字的金額）
 * 的帳單，交易列認不得 ⇒ 拒解退 AI（fail-closed，不是錯資料）。
 * ⚠️ **參考日的日期掃描是混合的**：錨點要靠正規化後的字串才定得了位，但**日期的值仍走舊尺**
 * ——候選會拿未正規化的整列文字濾一次（落點見 `parseWithRecipe` 參考日那段）。
 * 我原本在這裡寫「全形印法的參考日現在讀得到、方向只有變好」——**那句是錯的**：只換尺會讓
 * 全形日期多出候選、把已存卡的現值日改讀成另一個日期（Codex #523 r1 抓到，非 fail-closed）。
 * ⚠️ **殘餘（誠實）**：NFKC 不保證「子字串關係」在正規化後仍成立——暗號結尾的基字，若文件裡
 * 緊跟著組合附標，兩邊正規化後可能合成成另一個字。CJK 帳單沒有這種形，不為它加機關。
 * @param {unknown} s
 */
 ...
 * @param {number=} minLen 過 `recipeNorm` 後的最短長度（版面錨點與金額欄標題＝2：1 字錨點會在交易列上誤觸發）
 * ——量的是**真正拿去比對的那個字串**的長度，不是原字面。
 */
function checkSlot(path, v, maxLen, errs, minLen = 1) {
  ...
  if (q.length < minLen) errs.push(`${path}：至少 ${minLen} 個字（太短＝什麼版面都認）`);
...
export function validateRecipeStrict(raw) {
  ...
  if (typeof r.bank === 'string') {
    ...
    // 走同一把尺＝**行為等價的整理**（誠實記錄，別去替它找考題）：這一行原本就先 squash 再 NFKC，
    // 兩種順序只在「NFKC 會生出空白」那一族字元上分得開，而剝完機構詞的殘餘兩種順序都非空
    // ——構造不出會轉紅的案例。改它的理由是「這支檔只留一把尺」，不是修 bug。
  ...
  if (!r.summary || typeof r.summary !== 'object' || Array.isArray(r.summary)) errs.push('summary：必須是物件');
    ...
    if (Array.isArray(r.summary.sections)) {
      const qs = r.summary.sections.map(s => recipeNorm(s?.anchor));   // 用引擎命中時的同一把尺判歧義（尺不同＝「驗過不重疊」卻在文件上重疊）
  ...
  if (!r.detail || typeof r.detail !== 'object' || Array.isArray(r.detail)) errs.push('detail：必須是物件');
    ...
    const hs = [r.detail.headerOut, r.detail.headerIn, r.detail.headerBalance, r.detail.headerNote,
      ...
      .filter(h => typeof h === 'string').map(recipeNorm);
...
/** 從整列文字裡撈出所有日期。回 `{iso, raw}`——`raw`＝**帳單上真的印出來的那串字**，
 * 供呼叫端把「只因為正規化才冒出來的候選」濾掉（理由見 parseWithRecipe 裡參考日那段）。 */
const DATE_SCANNERS = {
  'west-slash': (/** @type {string} */ t) => [...t.matchAll(/(\d{4})\/(\d{2})\/(\d{2})/g)].map(m => ({ iso: `${m[1]}-${m[2]}-${m[3]}`, raw: m[0] })),
  'roc-slash': (/** @type {string} */ t) => [...t.matchAll(/(?<!\d)(\d{2,3})\/(\d{2})\/(\d{2})/g)].map(m => ({ iso: `${String(Number(m[1]) + 1911).padStart(4, '0')}-${m[2]}-${m[3]}`, raw: m[0] })),
};
 ...
 * 留著，於是「暗號抄得越忠實，越認不得自己出生的那份帳單」。 @param {BLine[]} lines @param {ParseRecipe} recipe */
export function recipeMatches(lines, recipe) {
  const text = lines.map(l => recipeNorm(l.cells.map(c => c.s).join(''))).join('\n');
  return recipe.docAnchors.every(a => text.includes(recipeNorm(a)));
}
...
export function parseWithRecipe(lines, recipe) {
  ...
  const refAnchor = recipe.refDate.strategy === 'none' ? null : recipeNorm(recipe.refDate.anchor);
  const endAnchor = recipeNorm(recipe.summary.endAnchor);
  ...
  const hOutQ = recipeNorm(recipe.detail.headerOut), hInQ = recipeNorm(recipe.detail.headerIn), hBalQ = recipeNorm(recipe.detail.headerBalance);
  ...
  {
    const bankQ = recipeNorm(recipe.bank);
    let bankSeen = false;
    for (const l of lines) {
      const j = recipeNorm(l.cells.map(c => c.s).join(''));
  ...
  for (const line of lines) {
    ...
    const joined = recipeNorm(cells.join(''));
    ...
    if (!refLocked && referenceDate == null && refAnchor) {
      ...
      if (at >= 0) {
        ...
        // ⚠️ **錨點用新尺定位、日期「值」仍用舊尺**（Codex #523 r1 High）：只拿正規化後的字串掃，
        // 全形印的日期會**多**出候選 ⇒ anchored-date 取第一個就可能改讀成另一個日期。
        // 實測反例：錨點後是「備查期限２０９９／０５／０６ 本期末2026/06/30」——base 讀 2026-06-30、
        // 只換尺後讀 2099-05-06；那是合法日期、會寫進 balanceAsOf，往後每期真帳單都比它舊 ⇒
        // 餘額更新被當 stale 一路跳過。**不是 fail-closed，是靜靜讀錯**，而且受害的是**已存的卡**。
        // ⇒ 候選必須是帳單**真的用值層印法印出來**的那些：拿未正規化的整列文字濾一次。
        // 代價（誠實）：整份用全形印參考日的版面照舊讀不到 ⇒ referenceDate=null ⇒ 餘額不更新
        //（同規則 1a「寧可 null」；與 base 行為逐字相同，不是本支新增的限制）。
        // ⚠️ **殘餘（我自己找的，沒有機械保證）**：濾網比對的是**整列**未正規化文字，不是「錨點
        //   之後」那一段——錨點若印相容字，raw 那邊定不了位（那正是本支要修的病）。所以同一串
        //   日期若**錨點前**用值層印法印過、**錨點後**才印全形，那個全形候選會被放行。要中這條
        //   得同時滿足三件事（同一串日期、一前一後、印法一正一全形），比它取代掉的那個洞窄得多；
        //   收得更緊的作法（raw 也定位錨點）會讓「相容字錨點＋正規字日期」整族讀不到參考日
        //   ——那正是 William 手上帳單的形狀。**選窄的殘餘，不選假護欄。**
        const rawJoined = squash(cells.join(''));
        const ds = scanDates(joined.slice(at + refAnchor.length)).filter(d => rawJoined.includes(d.raw)).map(d => d.iso);
        if (ds.length > 0) {
          refLocked = true;
    ...
    const hit = recipe.summary.sections.find(s => joined.includes(recipeNorm(s.anchor)));
    if (hit) { section = { currency: hit.currency }; pendingCurrency = null; continue; }
    if (endAnchor && recipeNorm(cells[0]).startsWith(endAnchor)) { section = null; continue; }
    if (!section) continue;
    ...
    // ⚠️ **輸出的文字也走同一把尺（William 2026-08-28 裁示「乙」；Codex #523 r1 High②）**。
    //   原本這裡刻意只 squash，理由是「輸出值原樣進 bankRef、磨掉相容字＝重匯認不出重複」。
    //   那個理由不夠：留著相容字的代價比去重更貴——`classifyBankTx` 的關鍵字是正規字寫的，
    //   `信⽤卡款`(U+2F64) 過不了 `/卡費|信用卡款/` ⇒ **繳卡費從「不分類」掉進「其他/未分類」，
    //   與已分類的卡明細重複計算**（實測；`similarKey` 不看摘要，接得住同日同額重傳、接不住
    //   次月不同日期金額的分類漂移）。`bankKeyOf` 只壓空白，同樣會把兩種字形當兩把學習鑰匙。
    //   正規化之後，配方路線與 AI 路線（AI 多半輸出正規字）記的字一致 ⇒ 分類、學習鑰匙、
    //   bankRef 一次全對齊。⚠️ 代價（誠實）：帳單位元組印相容字時，帳本裡記的是正規字
    //   （畫面上一模一樣；預覽與匯入同源＝所見即所得仍成立）。
    //   ⚠️ 祖父面（誠實、無機械保證）：若已經存在「暗號也抄成相容字」而孵成的舊卡，它先前
    //   匯入的列帶的是相容字 ⇒ 與新輸出的 bankRef 不同 ⇒ 重傳時精準去重認不出，由疑似重複接住。
    //   ⚠️ 正規字帳單完全不受影響——NFKC 對已正規的字串是恆等。
    const label = recipeNorm(cells.slice(0, accIdx).join(' '));
    ...
    const note = recipeNorm(after.filter(c => c !== balCellS && !isMoneyCell(c)).join(' '));
  ...
  for (const line of lines) {
    ...
    if (isHeaderLine(jc)) {   // 與概要迴圈同一份判準（單一實作）
      ...
      // 金額區**之前**（帳號/日期/摘要標籤）＝摘要身分區、值本來就歸摘要——不在此限（劃界入契約）。
      {
        const declared = new Set([hOutQ, hInQ, hBalQ,
          ...(headerNote ? [recipeNorm(headerNote)] : []), ...headerIgnore.map(recipeNorm)]);
        zoneStart = Math.min(xOut, xInCol);
        for (const cell of c) {
          if (cell.x >= zoneStart && !declared.has(recipeNorm(cell.s))) throw parseFail('表頭有未宣告的欄位');
        }
      }
      ...
      for (const roleName of [hOut, hIn, hBal, ...(headerNote ? [headerNote] : []), ...headerIgnore]) {
        const rq = recipeNorm(roleName);   // 與 findX 同一把尺：尺比 findX 窄，第二格就漏數＝它的值靜靜落錯窗
        if (c.filter(x => x.s === roleName || recipeNorm(x.s) === rq).length > 1) throw parseFail('表頭有重複的欄位');
      }
      inDetail = true;
      continue;
    }
    ...
    if (dateIso && restFrom) {
      ...
      const summary = recipeNorm(rest.filter(x => x.x < xBal && !inIgn(x.x) && parseAmount(x.s) == null && !/\$/.test(x.s)).map(x => x.s).join(''));
  ...
  const transactions = rows.map(r => ({
    ...
    // ⚠️ 交易備註**只做正規化那一半、不 squash**（其餘三個文字欄本來就 squash 過）：這一欄今天
    //   保留格間空白（那是它的內容），而本次要改的是字體、不是空白——多改一項就多一個沒人要求
    //   的行為變化。同一把尺的「值層／文字層」劃界見 `recipeNorm` 檔頭。
    amount: r.amount, balance: r.balance, note: r._notes.sort((a, b) => b.y - a.y).map(n => n.t).join('').normalize('NFKC'),
  }));
...
export function validateRecipeAgainstStatement(lines, recipe, parsed) {
  ...
  // 兩邊同尺（與 recipeMatches／引擎一致）：尺比引擎窄＝「相容字寫法的交易內文」躲過等值與位置兩道約束
  for (const t of parsed.transactions || []) for (const v of [t.summary, t.note]) if (v) contentTexts.add(recipeNorm(v));
  for (const a of parsed.accounts || []) for (const v of [a.label, a.note]) if (v) contentTexts.add(recipeNorm(v));
  // bank 槽只做**等值**檢（r8#3：單槽直通路——「敏感收款人」填進 bank 三關全過）；
  // 不做位置檢：銀行短名本來就會出現在交易列（「台新卡費」），位置約束必誤殺。
  if (contentTexts.has(recipeNorm(recipe.bank))) errs.push('bank：與帳單內文相等（疑似交易內容，拒收）');
  for (const [path, slot] of anchorSlots) {
    const q = recipeNorm(slot);
    if (contentTexts.has(q)) { errs.push(`${path}：與帳單內文相等（疑似交易內容被誤選為錨點，拒收）`); continue; }
    for (const line of lines) {
      if (!recipeNorm(line.cells.map(c => c.s).join('')).includes(q)) continue;
...
export function recipeReproduces(expected, actual) {
  ...
  // 這把尺也是 `recipeNorm`（William 2026-08-27 指示「不管 AI 抄寫什麼樣的字體都要可以判斷」）——
  // r4#1 當初擇的就是「比對前兩邊同法正規化」，字體只是空白之外的第二種印法差，同一條理由延用：
  // AI 抄回來的摘要若把帳單原文的相容字寫成正規字（模型普遍會正規化），嚴格等值＝這些銀行的卡
  // **永遠孵不出來**、每期付全額 AI 錢，而兩串字在畫面上一模一樣。黃金樣本本身仍一個字不動。
  // ⚠️ **放寬的代價由「引擎輸出也正規化」收掉**（William 2026-08-28 裁示「乙」，落點見上面 label 那段）：
  //   否則這裡放行的字體差會一路流到 `classifyBankTx`（繳卡費漏認＝重複計算）與 `bankKeyOf`
  //   （兩把學習鑰匙）——Codex #523 r1 High② 實測。兩邊一起改，放寬才是安全的。
  //   ⚠️ 殘餘（誠實）：AI 若**忠實照抄**相容字，AI 路線那次記的仍是相容字 ⇒ 兩條路線再度分叉。
  //   那是既有面（`lib/ai-parse.js` 的 `str()` 只 trim），不是本支打開的；真正的收口＝照
  //   `canonRef` 的祖父條款把 bankRef 的文字段也納入**比對形**，動到錢的核心、另一支 PR。
  //   金額／日期／方向／帳號欄不在軟等值清單、照樣嚴格＝「配方所得＝使用者確認的所得」對數字仍成立。
  const softEq = (/** @type {any} */ a, /** @type {any} */ b) => a === b || (typeof a === 'string' && typeof b === 'string' && recipeNorm(a) === recipeNorm(b));
  ...
  for (const [masked, eg] of expGroups) {
    ...
    if (eg.length === 1) {
      ...
      // ⚠️ 與上面 `softEq` **同一把尺**（Codex #523 r1 Medium）：這條是單帳戶組那條的平行路徑，
      // 只換一邊＝「同遮罩多幣別」的版面（外幣綜合帳戶，本檔別處承認的真實形狀）照舊孵不出卡。
      const key = (/** @type {any} */ a) => JSON.stringify(ACC_FIELDS.map(f => (SOFT.has(f) && typeof a[f] === 'string' ? recipeNorm(a[f]) : a[f])));
