...
export function validateRecipeStrict(raw) {
  ...
  if (!r.summary || typeof r.summary !== 'object' || Array.isArray(r.summary)) errs.push('summary：必須是物件');
    ...
    if (!Array.isArray(r.summary.sections) || r.summary.sections.length < 1 || r.summary.sections.length > RECIPE_LIMITS.sections) {
      ...
      r.summary.sections.forEach((s, i) => {
        ...
        if (typeof s.currency !== 'string' || (s.currency !== 'BY-CODE' && !/^[A-Z]{3}$/.test(s.currency))) {
  ...
  if (!r.detail || typeof r.detail !== 'object' || Array.isArray(r.detail)) errs.push('detail：必須是物件');
    ...
    // 抓到同一格＝互斥角色共用 x：忽略欄撞支出欄名＝整欄支出被吃掉、備註撞存入欄名＝存款
    // 變備註（r6#2 實測：翻向後餘額鏈照樣自洽、強閘放行）。名字兩兩不同 ⇒ 同一格不可能同時
    // 精確等於 A 又 squash 等於 B（那需要 squash(A)==squash(B)）⇒ x 互斥由此結構性成立，
    // 引擎不再另設一道到不了的 x 檢查（靜靜通過最危險——到不了的護欄不寫）。
...
export function parseWithRecipe(lines, recipe) {
  ...
  /** @type {Set<string>} 帳戶「身分」（遮罩＋幣別）——date-first 的「恰一帳戶」要數這個：
   * accountCurrency 的鍵是遮罩＝同遮罩多幣別會 last-wins 塌成一鍵（r2#4，外幣綜合帳戶真實形狀），
   * 數鍵數會把歧義版面放行、外幣利息被套上台幣權威幣別。 */
  const acctIdentities = new Set();
  /** @type {string|null} */
  ...
  for (const line of lines) {
    ...
    acctIdentities.add(`${masked} ${currency}`);
  ...
  if (soleMasked && (soleMasked.length !== 1 || acctIdentities.size !== 1)) {
    throw parseFail('date-first 版面需要恰好一個帳戶');   // 身分數（遮罩＋幣別）也要恰一——r2#4
  }
  ...
  const orphans = [];
  let xOut = 0, xInCol = 0, xBal = 0, xNote = 0, mid = 0, inDetail = false;
  /** @type {{x:number, kind:'ign'|'out'|'in'}[]} 欄起點（依 x 升序）——右緣分欄的判準表 */
  let colStarts = [];
  /** @type {number[]} 忽略欄 x（退路下界要用「金額兩欄左側」的那些） */
  let ignXs = [];
  /** @type {{s:number, e:number}[]} 忽略欄的完整區間表（含**餘額右側**的——r1#1：餘額分支先吞
   * 所有 x≥xBal 的數字，餘額右邊的序號欄會被當成餘額；區間終點＝下一個已知欄起點） */
  let ignIntervals = [];
  const inIgn = (/** @type {number} */ x) => ignIntervals.some(iv => x >= iv.s && x < iv.e);
  ...
  for (const line of lines) {
    ...
    if (isHeaderLine(jc)) {   // 與概要迴圈同一份判準（單一實作）
      ...
      xOut = findX(hOut); xBal = findX(hBal); xInCol = findX(hIn);
      ...
      ignIntervals = [...ignXs, ...noteIgn].map(s => ({ s, e: bounds.find(b => b > s) ?? Infinity }));
      mid = (xOut + xInCol) / 2;
      inDetail = true;
      continue;
    }
    ...
    if (dateIso && restFrom) {
      ...
      for (const cell of amtCells) {
        ...
        if (cell.x >= xBal) { if (!inIgn(cell.x) && !balCell) balCell = cell; continue; }
        // 主路也要先過**左緣**忽略區間（r5#1：寬序號/備註格左緣在忽略區、右緣跨進金額窗
        // ＝0067890 變支出、777 變收入且強閘放行——與退路同一把尺，起點在哪一欄就是哪一欄的格）
        if (inIgn(cell.x)) continue;
      ...
      if (!txnCell) {
        // 退路下界＝金額兩欄**左側**的忽略欄起點（右側忽略欄不縮小下界——預審①：原版 xIgn 混用
        // 會把左側真金額整列靜默丟棄）；忽略欄本身兩條路都以 colOf 擋。
        // ⚠️ 欄序不假設「支出在左」（r1#1：存入欄在支出欄左側的版面，原版下界 xOut-60 會把
        // 存入欄整列漏掉、中線判向也翻面——首筆入帳消失後強閘照樣 strong＝無聲少一筆）。
        const colMin = Math.min(xOut, xInCol);
        const leftIgns = ignXs.filter(x => x < colMin);
        const lower = leftIgns.length ? Math.min(...leftIgns) : (colMin - 60);
        txnCell = amtCells.find(x => {
          if (x.x < lower || x.x >= xBal) return false;
          // 左緣也要過忽略區間（r2#1：寬序號格右緣跨過 xBal ⇒ colOf 回 null、反而被退路收下
          // ＝幽靈存款——右緣分類失效時，左緣落在忽略區間就足以判它是序號）
          if (inIgn(x.x)) return false;
          return colOf(x.w ? x.x + x.w : x.x) !== 'ign';
        }) || null;
        // 中線判向要跟著欄序：左半＝靠左的那一欄，不必然是支出
        if (txnCell) direction = (txnCell.x < mid) === (xOut < xInCol) ? 'out' : 'in';
      }
      ...
      if (rowIdent === 'date-first' && !bs) continue;
...
export function validateRecipeAgainstStatement(lines, recipe, parsed) {
  ...
  const errs = [];
  const toDate = DATE_PARSERS[recipe.dateFormat];
  const isTxRow = (/** @type {Cell[]} */ c) => recipe.detail.rowIdent === 'acct-date'
