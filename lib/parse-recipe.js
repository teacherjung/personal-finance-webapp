...
export function parseWithRecipe(lines, recipe) {
   ...
   * ⚠️ **只量不擋（William 2026-08-26 裁示「甲」）——本數字今天不改變任何行為，也刻意沒有生產端讀者。**
   * 它存在的唯一理由＝把「配方看不到的東西有多少」變成可量測的數字，等 D 校準拿得到真帳單時
   * 直接讀，而不是再猜一次。**不可**把它接成拒解／退 AI／標疑似過期——那三條路都被實測打掉了
   * （見本檔下方「配方的幣別歸屬：兩個已知的洞」那段）。
  ...
  for (const line of lines) {
    ...
    if (!section) {
      // 不屬於任何宣告區段的列＝配方看不到它。只計數、不改行為（裁示「甲」）。
  ...
  //     A/B 已證實是**既有缺口**（main 8bb51fb 與 #517 輸出逐字相同），非哨兵批引入。
  //   ②**錨點吞併**（2026-08-26 新測）：`sections.find(s => joined.includes(s.anchor))` 是**子字串**比對，
  ...
  //   ⚠️ **為什麼不在這裡補第三道牆**（三個方向各自被實測打掉，勿再重試）：
  ...
  //       早就在比帳戶數＝出生月的涵蓋完整已被強制；配方真描述了外幣區時第②關先丟 bank_mixed_currency，
  //       新關永遠碰不到。實測 5 種形狀只擋到 1 種。本檔「固定幣別區出現矛盾的幣別標題」那段的
  //       r14#2 早就寫過同一課：**出生三關看不到後續月份、只有引擎自己能擋**。
  //   ⚠️ 今天的射程：真帳單量測 0 次（William 三份台新：掛多幣別的帳號 1 個、同時含台幣的 0 個），
  //     且台新走內建範本、根本不經過配方（配方只在範本回 bank_unrecognized 時才誕生）。
