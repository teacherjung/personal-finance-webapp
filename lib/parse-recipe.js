...
export function parseWithRecipe(lines, recipe) {
  ...
  for (const line of lines) {
    ...
    if (dateIso && restFrom) {
      ...
      if (rowIdent === 'date-first' && !bs) {
        // 有金額格「跨」餘額欄界（左緣在界內、右緣越界）＝可能是左緣漂移的大額餘額
        // （r10#2：+100,000 的餘額左緣漂過 xBal ⇒ 原版當無餘額丟列＝首筆入帳消失且強閘照綠）
        // ——分不出是寬金額還是漂移餘額＝歧義拒解；真的整列無餘額（資訊列）才是靜默跳過（r4#1）。
        if (amtCells.some(x => x.x < xBal && (x.w ? x.x + x.w : x.x) >= xBal)) throw parseFail('欄位判定歧義');
        continue;
      }
