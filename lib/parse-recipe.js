...
export function parseWithRecipe(lines, recipe) {
  ...
  for (const line of lines) {
    ...
    if (dateIso && restFrom) {
      ...
      // 同列同時有支出與存入候選＝歧義（r15#2：模板的 outCell || inCell 慣性會無條件取支出
      // ——多出來的數字欄「7」蓋掉真存入「100」＝反向交易且首筆盲區強閘照綠；台新版面
      // 「兩欄只填一個」是版面事實、泛化引擎不能拿它當假設）
      if (outCell && inCell) throw parseFail('欄位判定歧義');
