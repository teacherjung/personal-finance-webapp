...
/**
 * 相容字的那把尺：先 NFKC 正規化、再去空白。
 *
 * **為什麼需要它**：PDF 文字層會印**相容字**——康熙部首 `⾦`(U+2FA6) 與正常的 `金`(U+91D1)
 * 看起來一模一樣、位元組不同；全形 `１２３` 與 `123`、連字 `Oﬃce` 與 `Office` 同理。
 * AI 產配方時輸出的多半是正規字，於是暗號永遠對不上帳單原文。這種失敗**不會有任何錯誤訊息**：
 * 配方認不得版面＝當作沒有規則卡＝每個月照舊送一次 AI，「學一次、之後免費」對那些銀行等於不存在。
 *
 * **為什麼是「NFKC 先、去空白後」**：有一族字元自己不是空白、NFKC 之後會**生出**空白
 * （`¯`(U+00AF)、`‾`(U+203E)、`゛`(U+309B) 等）。反過來做那個空白會留著＝尺只做了一半。
 * @param {unknown} s
 */
export const recipeNorm = (s) => squash(String(s || '').normalize('NFKC'));

/**
 * ⚠️⚠️ **一次解析只用一把尺——這是本檔最重要的一條結構規則（William 2026-08-29 裁示）。**
 *
 * 走過的彎路（Codex #523 r6→r11 **五輪**，每一輪都是同一個病根的不同臉）：原本想「舊尺與新尺
 * 取聯集」，但兩把尺**不可比較**（NFKC 會把跨格相鄰的字合成掉，所以有些東西舊尺認得、新尺反而
 * 不認得），於是「同一次判斷裡混用兩把尺」在每一層各爆一次：表頭邊界 → 歧義守門 → 區段仲裁
 * → 選哪張規則卡 → 完整解析。**五次的後果都一樣**：真台幣帳戶被標成外幣 ⇒ 那戶的交易被當外幣
 * **不匯入** ⇒ 現金流漏帳，而強閘照樣是 `strong`。
 *
 * ⇒ **正解＝整份跑兩次、每次只用一把尺**（呼叫端負責：先 `'old'`、整份失敗才 `'new'`）：
 *  ・`'old'` ＝ `squash`，**逐字就是本檔在 #523 之前的行為**（也就是 main）——這是預設值，
 *    所以任何沒有明講尺的呼叫端都拿到 main 的語意。
 *  ・`'new'` ＝ `recipeNorm`，只在 `'old'` **整份**失敗時才跑。
 *    ⚠️ **這一趟的保證要說得精確**（Codex #523 r14 抓到我原句「結構上不可能比 main 差」寫過頭）：
 *    「`'old'` 有答案的所有輸入」與 main 逐字相同——這一半是結構保證。但 old 沒答案時 main
 *    **不是沒有答案，它退給 AI**（通常讀對）；`'new'` 命中＝**以卡代 AI**，承擔的是 P2 規則卡
 *    功能既有的那個取捨（卡片可能文字上認得、語意上標錯，圍欄＝出生三關＋防撞名＋強閘＋
 *    疑似過期＋預覽確認——同一條鏈在 main 上對舊尺命中同樣存在，已移植實測）。
 *    William 2026-08-29 裁示「甲」＝接受這個延伸暴露，照實入契約。
 * ⚠️ **不要再把兩把尺混進同一次判斷**——那正是上面那五輪的病根。
 */
export const RECIPE_RULERS = Object.freeze(/** @type {const} */ (['old', 'new']));   // freeze＝Codex r15 殘餘清單（exported mutable array）
/** @param {{ruler?: 'old'|'new'}} [opts] */
const rulerOf = (opts) => (opts && opts.ruler === 'new' ? recipeNorm : squash);
 ...
 * @param {number=} minLen **該趟的尺**處理後的最短長度（版面錨點與金額欄標題＝2：1 字錨點會在
 * 交易列上誤觸發）。old 趟＝squash 後（＝main）；new 趟量的是它真正拿去比對的字串
 * （`㈱` 在 old 趟是 1 字＝拒收、在 new 趟是 `(株)` 3 字＝該趟自洽——有考題釘）。
 */
// export（批四）：卡片規則卡的驗證器重用同一對原語——複製＝漂移副本（本檔 :25 的鐵則）。
export function checkSlot(path, v, maxLen, errs, minLen = 1, norm = squash) {
  ...
  if (norm(v).length < minLen) errs.push(`${path}：至少 ${minLen} 個字（太短＝什麼版面都認）`);   // 量「這一趟真的拿去比對的那個字串」
...
export function checkKeys(path, obj, allowed, errs) {
 ...
 */
export function validateRecipeStrict(raw, opts) {
  const norm = rulerOf(opts);   // 一次驗證只用一把尺（見 RECIPE_RULERS）
  /** @type {string[]} */
  const errs = [];
  ...
  checkSlot('bank', r.bank, RECIPE_LIMITS.bank, errs, 1, norm);
  ...
  if (!Array.isArray(r.docAnchors) || r.docAnchors.length < 1 || r.docAnchors.length > RECIPE_LIMITS.docAnchors) {
    ...
    r.docAnchors.forEach((a, i) => checkSlot(`docAnchors[${i}]`, a, RECIPE_LIMITS.anchor, errs, 2, norm));
  }
  ...
  if (!r.refDate || typeof r.refDate !== 'object' || Array.isArray(r.refDate)) errs.push('refDate：必須是物件');
    ...
    } else checkSlot('refDate.anchor', r.refDate.anchor, RECIPE_LIMITS.anchor, errs, 2, norm);
  }
  ...
  if (!r.summary || typeof r.summary !== 'object' || Array.isArray(r.summary)) errs.push('summary：必須是物件');
    ...
    if (!Array.isArray(r.summary.sections) || r.summary.sections.length < 1 || r.summary.sections.length > RECIPE_LIMITS.sections) {
      ...
      r.summary.sections.forEach((s, i) => {
        ...
        checkSlot(`summary.sections[${i}].anchor`, s.anchor, RECIPE_LIMITS.anchor, errs, 2, norm);
    ...
    checkSlot('summary.endAnchor', r.summary.endAnchor, RECIPE_LIMITS.anchor, errs, 2, norm);
    ...
    if (Array.isArray(r.summary.sections)) {
      const qs = r.summary.sections.map(s => norm(String(s?.anchor ?? '')));
  ...
  if (!r.detail || typeof r.detail !== 'object' || Array.isArray(r.detail)) errs.push('detail：必須是物件');
    ...
    checkSlot('detail.headerOut', r.detail.headerOut, RECIPE_LIMITS.header, errs, 2, norm);
    checkSlot('detail.headerIn', r.detail.headerIn, RECIPE_LIMITS.header, errs, 2, norm);
    checkSlot('detail.headerBalance', r.detail.headerBalance, RECIPE_LIMITS.header, errs, 2, norm);
    if (r.detail.headerNote !== null) checkSlot('detail.headerNote', r.detail.headerNote, RECIPE_LIMITS.header, errs, 1, norm);
    ...
    } else r.detail.headerIgnore.forEach((h, i) => checkSlot(`detail.headerIgnore[${i}]`, h, RECIPE_LIMITS.header, errs, 1, norm));
    // **全部**文字角色（金額三欄＋備註＋忽略欄）兩兩不可同名（**該趟的尺**後判——Grok #523 掃到
    // 這行還寫死 squash：old 趟等價、new 趟就是同趟混尺的病）——撞名＝findX
    ...
    const hs = [r.detail.headerOut, r.detail.headerIn, r.detail.headerBalance, r.detail.headerNote,
      ...
      .filter(h => typeof h === 'string').map(norm);
...
// export（批四）：卡片引擎同一組日期格式（枚舉 RECIPE_DATE_FORMATS 共用＝格式表不分家）。
export const DATE_PARSERS = {
...
/** 從**該趟的尺**處理後的整列文字裡撈出所有日期（轉 ISO）——呼叫端傳進來的是 norm 過的字串。 */
const DATE_SCANNERS = {
...
/** 這份帳單像不像這張配方講的版面：暗號（**該趟的尺**後）必須**全部**出現在全文。
...
export function recipeMatches(lines, recipe, opts) {
  const norm = rulerOf(opts);   // 一次比對只用一把尺（見 RECIPE_RULERS）
  const text = lines.map(l => norm(l.cells.map(c => c.s).join(''))).join('\n');
  return recipe.docAnchors.every(a => text.includes(norm(a)));
}
 ...
 */
export function parseWithRecipe(lines, recipe, opts) {
  const norm = rulerOf(opts);   // ⚠️ **這一趟從頭到尾只用這一把尺**（見 RECIPE_RULERS；混用是 r6–r11 五輪的病根）
  const errs = validateRecipeStrict(recipe, opts);
  if (errs.length) throw parseFail('配方本身不合格');
  if (!recipeMatches(lines, recipe, opts)) throw parseFail('版面暗號對不上');
  ...
  // 參考日的錨點也用**這一趟的那把尺**——不是因為它需要相容字，而是因為**同一趟不可以混尺**
  // （混用是 r6–r11 五輪的病根）。⚠️ 這一格曾在 #523 中途被 William 裁示「整段退回 main」，
  //   理由是「正規化會讓日期候選多出來／被吃掉 ⇒ 已存卡的現值日被改讀」。改成「整份跑兩次」之後
  //   那個風險**結構上不存在**：`'old'` 那一趟逐字就是 main，而 `'new'` 只在 old 整份失敗時才跑
  //   ——那時 main 根本沒有答案，沒有「main 讀到 X、我們讀到 Y」的空間。
  const refAnchor = recipe.refDate.strategy === 'none' ? null : norm(recipe.refDate.anchor || '');
  const endAnchor = norm(recipe.summary.endAnchor);
  ...
  const hOutQ = norm(recipe.detail.headerOut), hInQ = norm(recipe.detail.headerIn), hBalQ = norm(recipe.detail.headerBalance);
  ...
  {
    const bankQ = norm(recipe.bank);
    let bankSeen = false;
    for (const l of lines) {
      const j = norm(l.cells.map(c => c.s).join(''));
  ...
  for (const line of lines) {
    ...
    const hit = recipe.summary.sections.find(s => joined.includes(norm(s.anchor)));
    if (hit) { section = { currency: hit.currency }; pendingCurrency = null; continue; }
    if (endAnchor && norm(cells[0] || '').startsWith(endAnchor)) { section = null; continue; }
    if (!section) {
    ...
    // ⚠️ label／帳戶 note／交易 summary／交易 note 四個**輸出**文字欄一律維持原樣（只 squash、
    //   交易 note 連 squash 都不做）——它們同時是「帳單原文留底」（`lib/types.js` 契約寫著一字未改）、
    //   **精準去重鍵**（`bankRefBase` 把 summary/note 拼進 bankRef）與**帳號身分的來源**
    //   （`noteAccountSuffixes`；NFKC 會把 `①` 折成 `1` ⇒ `****①234` 變 `****1234` ⇒ 一般支出被判成
    //   內轉 ⇒ **真支出從現金流消失**。`lib/bank-statement.js` 的 `foldWidth` 早就為了同一個理由
    //   拒絕對帳號整串做 NFKC）。**正規化只准活在比對上，不准流進存下來的字。**
  ...
  //   ②**錨點吞併**（2026-08-26 新測；引用的是**當時**的程式——#523 起那行改傳該趟的尺 `norm`，
  //     old 趟行為不變、本段結論照舊成立）：`sections.find(...)` 是**子字串**比對，
  ...
  for (const line of lines) {
    const c = line.cells;
    const jc = norm(c.map(x => x.s).join(''));
    if (isHeaderLine(jc)) {   // 與概要迴圈同一份判準（單一實作）
      // findX：先精確等於，再退**該趟的尺**等於（有些版面標題帶空白／相容字）；抓不到＝結構性失敗退 AI
      const findX = (/** @type {string} */ name) => {
        const hit = c.find(x => x.s === name) || c.find(x => norm(x.s) === norm(name));
        return hit ? hit.x : 0;
      };
      ...
      // 金額區**之前**（帳號/日期/摘要標籤）＝摘要身分區、值本來就歸摘要——不在此限（劃界入契約）。
      {
        const declared = new Set([hOutQ, hInQ, hBalQ,
          ...(headerNote ? [norm(headerNote)] : []), ...headerIgnore.map(norm)]);
        zoneStart = Math.min(xOut, xInCol);
        for (const cell of c) {
          if (cell.x >= zoneStart && !declared.has(norm(cell.s))) throw parseFail('表頭有未宣告的欄位');
        }
      }
      ...
      for (const roleName of [hOut, hIn, hBal, ...(headerNote ? [headerNote] : []), ...headerIgnore]) {
        const rq = norm(roleName);
        if (c.filter(x => x.s === roleName || norm(x.s) === rq).length > 1) throw parseFail('表頭有重複的欄位');
      }
      inDetail = true;
      continue;
    }
 ...
 * 錨點真的來自版面、不是來自交易：
 *  ①**等值約束**：錨點（**該趟的尺**後）不得與任何交易的摘要/備註、帳戶的標籤/備註**相等**
 ...
 */
export function validateRecipeAgainstStatement(lines, recipe, parsed, opts) {
  const norm = rulerOf(opts);   // 一次驗證只用一把尺（見 RECIPE_RULERS）
  /** @type {string[]} */
  const errs = [];
  ...
  for (const t of parsed.transactions || []) for (const v of [t.summary, t.note]) if (v) contentTexts.add(norm(String(v)));
  for (const a of parsed.accounts || []) for (const v of [a.label, a.note]) if (v) contentTexts.add(norm(String(v)));
  // bank 槽只做**等值**檢（r8#3：單槽直通路——「敏感收款人」填進 bank 三關全過）；
  // 不做位置檢：銀行短名本來就會出現在交易列（「台新卡費」），位置約束必誤殺。
  if (contentTexts.has(norm(String(recipe.bank || '')))) errs.push('bank：與帳單內文相等（疑似交易內容，拒收）');
  for (const [path, slot] of anchorSlots) {
    const q = norm(String(slot || ''));
    if (contentTexts.has(q)) { errs.push(`${path}：與帳單內文相等（疑似交易內容被誤選為錨點，拒收）`); continue; }
    for (const line of lines) {
      if (!norm(line.cells.map(c => c.s).join('')).includes(q)) continue;
...
export function recipeReproduces(expected, actual) {
  ...
  // ⚠️⚠️ **這把尺刻意停在「只吃空白」、不吃字形（Grok #523 掃到、判定＝進待辦，不是遺漏）**：
  //   黃金樣本是 **AI 的答案卷**（票裡那份）、actual 是引擎輸出（照帳單原文）。AI 若把內容欄
  //   正規化（`信用卡款`）而帳單印相容字（`信⽤卡款`），這裡就對不上 ⇒ 卡孵不出來 ⇒ 那種帳單
  //   每期照付 AI（fail-closed、不產生錯資料）。**單獨放寬會讓去重鍵分叉**（引擎那半的文字原樣進
  //   bankRef）——William 2026-08-29 裁示：放寬必須與「分類／去重比對形」那一層**一起做**（已排
  //   下一支：canonRef 式比對形），且該先量 AI 實際怎麼抄。⇒ 本支只修「認版面」那一半，
  //   內容欄的字形寬容刻意不做。
