...
export function parseWithRecipe(lines, recipe) {
  ...
  // 幣別「長相」（恰三碼大寫）＝衝突偵測的判準（r22#1：EUR 是真幣別、只是不在支援清單——
  // 用白名單做偵測會讓它隱形、歐元帳戶列繼承 TWD＝歐元交易進台幣現金流且強閘照綠）；
  // 「採用」（sticky 標題設幣別）才用 CURRENCIES 白名單（r14 建議：QQQ 不可被當幣別採用）。
  const codeShaped = (/** @type {string} */ t) => /^[A-Z]{3}$/.test(t);
  ...
  for (const line of lines) {
    ...
    if (section.currency !== 'BY-CODE' && !cells.some(isMaskedAccount)) {
      ...
      if (cells.some(c => codeShaped(c.trim()) && c.trim() !== secCur)) {
    ...
    if (section.currency !== 'BY-CODE') {
      ...
      if (cells.some(c => codeShaped(c.trim()) && c.trim() !== secCur2)) {
        throw parseFail('固定幣別區出現矛盾的幣別標題');
      }
    } else {
      // BY-CODE 帳戶列自帶的明確幣別格必須與 sticky 一致（r16#2＝r15#1 同族漏口：
      // sticky=JPY、帳戶列印著 USD ⇒ 原版把帳戶標成 JPY、USD 掉進 note＝美元餘額被建成日圓）
      const rowCodes = [...new Set(cells.map(c => c.trim()).filter(c => codeShaped(c)))];
