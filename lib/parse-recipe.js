...
export function recipeReproduces(expected, actual) {
  ...
  // ⚠️ **這把尺刻意還沒跟上文字比對的 recipeNorm（誠實劃界，待裁）**：AI 抄回來的摘要若把帳單原文的
  //   相容字（康熙部首等）寫成正規字，這裡就對不上 ⇒ 配方孵不出來，而那正是 recipeNorm 要解的病。
  //   沒有一起改的理由：這一格比的是「AI 答案 vs 引擎輸出」，放寬它＝放行「兩邊文字其實不同」的配方，
  //   而引擎那半的文字會原樣進 bankRef（見上面 label 那段）⇒ 同帳單兩條路線的去重鍵不同。
  //   要不要放寬是**去重鍵的取捨**、不是比對尺的取捨，所以留給裁示，不順手改。
