...
export function parseWithRecipe(lines, recipe) {
  ...
  // 銀行身分綁定（r8#2）：配方的 bank 不是裝飾——它會變成機構戳與 bank2 去重鍵的機構段。
  // 別家銀行的帳單若恰好共用暗號與幾何，冒名解析＝跨行去重與帳戶歸屬全錯。機械驗證＝
  // 文件全文必須印有這家銀行的名字（帳單都會印發單機構；沒印＝配方孵不出來，fail-closed）。
  const fullText = lines.map(l => squash(l.cells.map(c => c.s).join(''))).join('\n');
  if (!fullText.includes(squash(recipe.bank))) throw parseFail('銀行身分對不上');
  ...
  for (const line of lines) {
    ...
    if (dateIso && restFrom) {
      ...
      for (const cell of amtCells) {
        ...
        if (inIgn(cell.x)) {
          if (colOf(cell.w ? cell.x + cell.w : cell.x) !== 'ign') throw parseFail('欄位判定歧義');
          continue;
        }
