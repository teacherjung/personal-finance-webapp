 ...
 * 這把尺只准活在**比對**上——暗號、銀行身分、參考日錨點、區段／收尾錨點、表頭的識別與座標、
 * 配方自己的兩道防撞名、出生對照的等值與位置約束。**三處刻意不過它，而且都是界線不是遺漏**：
 ...
 *    一起做（落點見 `softEq` 那一段）。
 * ⚠️ **參考日**：錨點用這把尺定位，但**候選一律在未正規化的整列上產生**——正規化既會多出候選、
 * 也會吃掉候選（兩個方向的實測反例都寫在 `parseWithRecipe` 參考日那段）。
 * ⚠️ 我在這裡寫過「全形印法的參考日現在讀得到、方向只有變好」——**那句是錯的**，且我為它補的
 * 濾網只擋得住其中一個方向。兩次都是 Codex #523 抓到的；留著這兩行是為了讓下一個人別再走一次。
...
/** 參考日錨點「慢路徑」的掃描上限（字元）。慢路徑只在**帳單把錨點印成相容字**時才走，
 * 而它逐位重算前綴＝平方級。Codex #523 r4#2 同機量測：1,000 字約 1.5ms、10,000 字約 134ms、
 * 40,000 字約 2s、80,000 字約 7.8s（`lib/parse-limits.js` 只數文字節點、`lib/pdf-isolate.js`
 * 只限回傳位元組，兩道都擋不住「單一超長 text item」，而這段跑在**父行程**）。
 * ⇒ 設一個看得見的上限：超過就當找不到錨點 ⇒ `referenceDate=null` ⇒ 餘額不更新（既有 fail-safe）。
 * ⚠️ 這個數字**沒有真帳單量測撐著**（誠實：手上沒有可量的樣本），選它的理由只有「正常帳單列
 * 遠短於它，而它的最壞成本仍在毫秒級」。拿得到真帳單量測時請回頭校準。 */
const ANCHOR_SCAN_MAX = 2000;
...
/** 從整列文字裡撈出所有日期（轉 ISO）。⚠️ 呼叫端**必須先切到錨點之後再餵進來**：切片會重設
 * `roc-slash` 那條 `(?<!\d)` 左界，而那個左界是判準的一部分（見 parseWithRecipe 參考日那段）。 */
const DATE_SCANNERS = {
...
export function parseWithRecipe(lines, recipe) {
  ...
  for (const line of lines) {
    ...
    if (!refLocked && referenceDate == null && refAnchor) {
      ...
      // 拿舊蓋新——正是規則 1a 要防的事故）。
      // ⚠️ **切點要落在未正規化的那一列上**（Codex #523 r3）：`cells.join('')` 不加分隔符，
      //   錨點若以 ASCII 數字結尾、日期又緊鄰下一格（`結算1` ＋ `115/06/30` ⇒ `結算1115/06/30`），
      //   整列掃描時 `roc-slash` 的 `(?<!\d)` 會把候選整顆吃掉；base 先切片、切片本身重設了左界。
      //   實測 main 讀得到 2026-06-30／2026-05-31、整列掃描版兩個都 null＝**真回歸**。
      //   ⇒ 用「第一個**前綴正規化後含錨點**的位移」當切點：正規化只用來**定位**（相容字錨點才定得了），
      //   切完之後掃描器吃的是原文、左界與 base 逐字相同。
      // 切點是**兩段式**（Codex #523 r4 兩條換來的；⚠️ 這一格刻意**不是**「兩邊都過同一把尺」，
      // 別照抄別處的說法）：
      //  ①**快路徑＝main 原本那條**（原文裡找正規化後的錨點）：帳單用值層印法印錨點時走這條——
      //    O(n)、位置與 base 逐字相同。正常帳單與**所有既有規則卡**都只走這裡。
      //  ②**慢路徑**只在①落空（＝帳單把錨點印成相容字）時才跑：用整把尺在**未去空白的原文**上
      //    逐位找，再換算成去空白後的位移。⚠️ 一定要在**未去空白**的原文上做——先 squash 再 NFKC
      //    會把「基字＋空白＋組合附標」合成掉（`A`＋空白＋U+030A ⇒ `Å`）而找不到錨點：實測
      //    main 讀得到 2026-05-31、squash-first 版讀 null（r4#1）。那正是這把尺的順序存在的理由。
      //    慢路徑有上限 `ANCHOR_SCAN_MAX`（平方級成本，理由見該常數）。
      const raw = cells.join('');
      const plain = squash(raw);
      const fast = plain.indexOf(refAnchor);
      let cut = fast >= 0 ? fast + refAnchor.length : -1;
      if (cut < 0) {
        const lim = Math.min(raw.length, ANCHOR_SCAN_MAX);
        for (let i = 1; i <= lim; i++) {
          if (recipeNorm(raw.slice(0, i)).includes(refAnchor)) { cut = squash(raw.slice(0, i)).length; break; }
        }
      }
      if (cut >= 0) {
        ...
        // ⚠️ **正規化只用來定位錨點，候選一律從原文的切片上產生**（Codex #523 r1→r2→r3 三輪
        //   換來的形狀；我前三版各錯一次，教訓留在這裡別再走）：
        //   ①拿正規化後的字串掃 ⇒ 會**多出**候選（全形日期變得掃得到，anchored-date 取第一個就換人：
        //     base 讀 2026-06-30、換尺讀 2099-05-06＝現值日被寫成未來日，往後每期真帳單都被當 stale）。
        //   ②同樣的字串也會**吃掉**候選（`２115/05/06` 經 NFKC 黏成四位數，`(?<!\d)` 把民國日期整顆
        //     消掉：三種策略 2026→2027、2027→null、null→2028）。我為①補的濾網擋不住②——濾網只能
        //     刪候選、救不回被吃掉的。
        //   ③改成「整列掃原文＋用前綴判在不在錨點後」仍然錯：**切片本身是判準的一部分**。錨點以
        //     ASCII 數字結尾、日期緊鄰下一格（`結算1`＋`115/06/30` ⇒ `結算1115/06/30`）時，整列掃描
        //     的 `(?<!\d)` 會把候選吃掉，而 base 因為先切片、左界被重設 ⇒ main 讀得到、它讀不到。
        //   ④切點若在**去空白後**的字串上逐位算，會把「基字＋空白＋組合附標」合成掉而找不到錨點
        //     （r4#1，同樣是 main 讀得到、它讀不到）⇒ 切點的兩段式寫法見上面那段。
        //   ⇒ 正解＝**先算切點、再把切片餵給掃描器**：切片與 base 逐字相同 ⇒
        //     候選集合、順序、左界全部相同，同時相容字錨點也定得了位。
        // 代價（誠實）：整份用全形印參考日的版面照舊讀不到 ⇒ referenceDate=null ⇒ 餘額不更新
        //（同規則 1a「寧可 null」；與 base 逐字相同，不是本支新增的限制）。
        const ds = scanDates(plain.slice(cut));
        if (ds.length > 0) {
          refLocked = true;
