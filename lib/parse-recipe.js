...
export function parseWithRecipe(lines, recipe) {
  ...
  for (const line of lines) {
    ...
    // ⚠️ label／note／summary 三個文字欄是**輸出值**、不是比對點——所以刻意只 squash、不過 recipeNorm：
    //   它們會原樣進 bankRef 去重鍵（`lib/services/bank-import.js` 的 bankRefBase 把 summary/note 拼進鍵裡），
    //   把相容字磨成正規字＝同一份帳單重匯認不出重複。要動它們是「改去重鍵」，不是「改比對尺」。
...
export function recipeReproduces(expected, actual) {
  ...
  // ⚠️ **代價（誠實劃界，不是無聲的）**：這樣孵出來的卡，之後走配方路線匯入時記的是**帳單原文的
  //   字體**，而當初走 AI 路線那次記的是 AI 抄的字體 ⇒ 兩者的 bankRef 去重鍵不同 ⇒ 同一份帳單
  //   重傳時**精準去重認不出來**。接住它的是第二層「疑似重複」提醒（`similarKey`＝機構｜末碼｜日期｜
  //   方向｜金額，不看摘要）＝使用者會被問一句，不是靜靜多記一筆。
  //   ⚠️ 這個分歧**不是本次新造的**：softEq 對空白本來就寬鬆（拆格版面的卡今天就孵得出來，
  //   AI 那份帶格間空白、引擎那份 squash 過），字體只是同一族的第二種形。
  //   ⚠️ 真正的收口在**去重那一層**：照 `canonRef` 的祖父條款作法（存好的鍵一個位元組不改、
  //   比對時兩邊都正規化）把文字段也納進比對形——那動到錢的核心，另一支 PR 自己過審。
  //   金額／日期／方向／帳號欄不在此列、照樣嚴格＝「配方所得＝使用者確認的所得」對數字仍成立。
