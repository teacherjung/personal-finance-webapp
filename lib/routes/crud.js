...
// 帳戶改名 → 連動更新既有交易的 account（顯示名是 denormalized 字串）：使用者定 2026-07-21「改一次、處處同步」——
// 銀行帳戶頁改名後，收支記帳列表與匯入跳窗都跟著變。同一次寫檔完成（beforeSave），全有或全無。
const propagateAccountRename = (/** @type {any} */ db, /** @type {any} */ item, /** @type {any} */ prev) => {
  const from = String(prev?.name || ''), to = String(item?.name || '');
  if (!from || from === to) return;
  for (const t of db.transactions || []) if (t.account === from) t.account = to;
};
...
for (const col of COLLECTIONS) {
  ...
  crudRoutes.put(`/api/${col}/:id`, (req, res) => {
    ...
    const beforeSave = col === 'transactions' ? learnFromTxEdit : col === 'accounts' ? propagateAccountRename : undefined;
