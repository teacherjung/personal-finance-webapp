...
import { bankLinkedStatementRow } from '../services/bank-import.js';
...
for (const col of COLLECTIONS) {
  ...
  crudRoutes.delete(`/api/${col}/:id`, asyncRoute(async (req, res) => {
    if (col === 'transactions') {
      // 簽帳金融卡明細（Stage 5b）：那份帳單在兩本帳裡的列互為條件（卡片帳本那筆記了、帳戶那筆才留空）——
      // 單筆刪任一邊都會讓另一邊說謊（刪卡片列＝消費少算；逐筆刪光帳戶列＝銀行批次從匯入紀錄消失、卡片列變孤兒）。
      // 整份帳單是一個單位：要拿掉請從收支頁「匯入紀錄」刪那份帳單（連帶兩本帳一起）。Codex #503 r2#1。
      const list = await getCollection('transactions');
      const t = (list || []).find((/** @type {any} */ x) => x.id === req.params.id);
      if (t && bankLinkedStatementRow(list, t)) {
        return res.status(400).json({ error: '這筆是金融卡帳單自動記的（帳戶明細與刷卡消費明細互相對應）——要移除請到收支頁「匯入紀錄」刪那份帳單，兩本帳會一起拿掉；不能單筆刪。' });
      }
    }
...
// securityTrades／stockFundamentals 都有專屬 GET（回應契約與權限語意不同），不可再開裸集合端點。
for (const col of READONLY_COLLECTIONS) {
  if (col === 'securityTrades' || col === 'stockFundamentals' || col === 'parseRecipes') continue;   // parseRecipes：內部快取、P2-3 才有前端徽章需求，不開裸端點
