...
import { projectCard } from '../secret-fields.js';
...
for (const col of COLLECTIONS) {
  ...
  crudRoutes.get(`/api/${col}`, (req, res) => res.json(col === 'cards' ? getCollection(col).map(projectCard) : getCollection(col)));
  crudRoutes.post(`/api/${col}`, (req, res) => {
    ...
    res.json(col === 'cards' ? projectCard(created) : created);   // 寫入端也剝機密（Codex r10#2）
  });
  crudRoutes.put(`/api/${col}/:id`, (req, res) => {
    ...
    res.json(col === 'cards' ? projectCard(item) : item);   // 寫入端也剝機密（Codex r10#2）——PUT 只改別的欄位也別把存的 pdfPassword 吐回
  });
