    ...
    const beforeSave = col === 'transactions'
      ? (/** @type {any} */ db, /** @type {any} */ item, /** @type {any} */ prev) => {
          ...
          if (applyAll) {   // 銀行走 learnedBank 規則、帳單走本筆分類；鑰匙來自已存的 bankKey/storeKey（服務層擁有、不隨 patch 動）
            ...
            // 國外交易服務費**不支援整批改**（分類跟隨所屬消費，r2-Codex#3）：對它傳播會 throw、在原子路徑
            // 連本筆編輯一起 rollback（G3 對抗審查 confirmed）。改為**略過傳播**（服務費 applyAll 本就無意義）
            // ——本筆編輯照常存，不因無效的整批請求丟失。前端也不對服務費列顯示「同店一起改」勾選框。
            else if (item.source === 'stmt' && item.storeKey && !isServiceFee(item.storeKey)) applied = applyCategoryToStoreDb(db, String(item.storeKey), item.category, item.subcategory || '');
          }
        }
      : col === 'accounts' ? onAccountSave : undefined;
    let item;
