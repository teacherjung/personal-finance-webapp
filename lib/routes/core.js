...
import { sanitizeSettings, sanitizeItem, COLLECTIONS, READONLY_COLLECTIONS } from '../schema.js';
...
coreRoutes.post('/api/import', (req, res) => {
  ...
  // 集合逐筆清理數值欄位（Codex 第三輪）：壞備份的 holdings.price:'oops' 通過「是陣列」檢查後，
  // 若直接 ...b 展開，會讓 buildSummary 的 Number() 變 NaN、污染 netWorth。逐筆走 sanitizeItem
  // 把數值欄位的壞值剝掉（其餘欄位含 id 原樣保留）。
  // filter(Boolean)：把非物件的元素（[null] 之類，sanitizeItem 回 null）濾掉，避免 buildSummary 讀 null.currency 崩
  /** @type {Record<string, any>} */
  const cleanCollections = {};
  for (const col of [...COLLECTIONS, ...READONLY_COLLECTIONS]) {
    if (Array.isArray(b[col])) cleanCollections[col] = b[col].map((/** @type {any} */ it) => sanitizeItem(col, it)).filter(Boolean);
  }
