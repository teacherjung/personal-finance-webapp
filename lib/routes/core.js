...
coreRoutes.get('/api/refund-pairs', (req, res) => {
  const { pairs, unmatchedRefunds } = pairRefunds(getDb());
  res.json({
...
coreRoutes.post('/api/backup/daily', (req, res) => res.json(dailyBackupIfDue(nowLocal().date)));
