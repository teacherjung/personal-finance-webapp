 ...
 *    **突變測試逐項實測過，兩道不對稱**：
 *    ・只拿掉這裡 → **來回⑨紅**（目前 `id: 7` 進了 Map，匯入的 `id: "7"` 本身是合法字串、
 *      取用那道會放行，於是照樣繼承）。這一道是必要的。
 *    ・只拿掉那裡 → **考題全綠**。因為這一道擋住之後 Map 裡本來就不會有身分不明的路徑，
 *      取用那道目前殺不掉——它是**刻意留的第二層**，不是有考題撐著的必要條件。
 *    留著的理由：這個 repo 的「牆與被保護的東西讀的不是同一份資料」已經咬過五次，
 *    第二層很便宜。**但別因為它殺不掉就以為它是死碼，也別因為它在就放心拿掉第一道。**
 *
