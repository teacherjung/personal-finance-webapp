...
import { sanitizeSettings, validateImportItem, sanitizeLearned, sanitizeLearnedBank, COLLECTIONS, READONLY_COLLECTIONS } from '../schema.js';
import { buildSummary, buildMonthlyReview, monthKey } from '../derive.js';
...
coreRoutes.get('/api/monthly-review', (req, res) => {
  const requestedMonth = typeof req.query.month === 'string' ? req.query.month : '';
  res.json(buildMonthlyReview(getDb(), requestedMonth));
});
...
coreRoutes.get('/api/transfer-subcategories', (req, res) => res.json(effectiveTransferSubs()));
coreRoutes.post('/api/transfer-subcategories', (req, res) => {
