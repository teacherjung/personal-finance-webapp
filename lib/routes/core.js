...
coreRoutes.post('/api/import', asyncRoute(async (req, res) => {
  ...
  if (isHosted()) {
    ...
    /** 同一條路徑被兩筆以上帳戶佔用（同一個 id 出現兩次）。 @type {Set<string>} */
    const ambiguousPii = new Set();
    // ⚠️⚠️ 回填只准在**三個條件同時成立**時發生，缺一不可。這是踩過三次才收斂出來的：
    //
    //  ①**這一筆有穩定身分**（`path !== null`）。`accounts[].id` 不在必填欄位裡（`lib/schema.js`），
    //    缺 id 時 `mapBackupOnlyPii` 給的是 `null` 而不是字串座標——因為
    //    「目前資料裡那筆沒有 id 的帳戶」與「匯入檔裡另一筆完全不同、也沒有 id 的帳戶」
    //    會對到同一格，而**兩側各只有一筆時連撞號都數不出來**（Codex 定向複審 v4 抓到，
    //    實測：匯入一筆「完全不同的新帳戶」，它靜靜地拿到了舊帳戶的完整帳號、還回 200）。
    //  ②**目前資料那側沒有同 id 重複**——否則回填來源本身就不確定是誰的。
    //  ③**匯入後那側沒有同 id 重複**——否則兩筆會**同時**從同一條路徑取到同一個號碼
    //    （Codex 定向複審 v3 抓到）。
    //
    //  退出只關掉「從現值回填」這條退路；**檔案裡自己帶的非空值照收**——
    //  那是該筆自己的值，不會錯配。
    //  判準與 `lib/store-pg.js` 對 `cards..pdfPassword` 的處置一致：
    //  **寧可少救一個欄位，也不要把資料寫錯。**
    //
    // 📌 ①出現兩次（走訪來源時擋一次、最後回填時再擋一次）是**刻意的**，實測結果如下：
    //    只拿掉其中任一道 → 考題全綠（另一道補得上）；**兩道都拿掉 → 來回⑦轉紅**。
    //    也就是說單獨看每一行都「殺不掉」，很像可以順手簡化的死碼——**不要**。
    //    前面那道防的是「keptPii 被沒有身分的值汙染」，後面那道防的是「汙染了也不准被取用」。
    //    這個檔案所在的這一族（牆與被保護的東西讀不同資料）已經咬過五次，兩道便宜的守衛值得留著。
    ...
    mapBackupOnlyPii(merged, (v, path) => v || (path === null ? '' : keptPii.get(path)) || '');
  }
