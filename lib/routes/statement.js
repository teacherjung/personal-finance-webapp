...
// 後帶上）②真引擎在**這裡**組裝注入（本檔全靜態路徑＝外連登記閘可審計；服務層與 crud.js 拿不到 fetch）
...
statementRoutes.post('/api/bank-statement/apply', wrap(async (req, res) => res.json(await applyBankStatement(req.body.data, req.body.password, undefined, { useAi: req.body.useAi === true, aiEngineFactory: makeAnthropicBankEngine }))));
