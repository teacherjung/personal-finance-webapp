...
// 帳單店名／分類學習（合併卡）編輯：以「帳單原文」為準改顯示名＋分類（同原文整批改＋記學習，
// 未來匯入沿用；不同分店可各自取名）。reset=true＝整列還原自動判斷並清除學習。
statementRoutes.post('/api/statement/rename-store', wrap((req, res) =>
  res.json(renameStoreDisplay(req.body?.orig, req.body?.name, req.body?.category, req.body?.subcategory, !!req.body?.reset))));
