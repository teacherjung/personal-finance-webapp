...
statementRoutes.post('/api/statement/normalize-branches', wrap(async (req, res) => {
  ...
  // ⚠️ 這條維護後門**不接** proceedWithoutBackup：備份沒做成時 normalizeBranches 會 throw 409，
  // wrapRoute 原味回報。理由是這條路沒有使用者可以問（是 Claude/Codex 排查時 curl 的），
  // 而「沒有安全網就繼續」這個決定不該由排查的人默默替使用者做。真的要繞，先手動做一份備份。
...
// ⚠️ 兩個旗標**分開送**（勿合併）：`force`＝使用者已確認「學習表會被蓋掉」；
// `proceedWithoutBackup`＝使用者已確認「這次沒存成還原檔也要繼續」。合成一個的話，
// 按掉其中一個問題就等於連另一個也一起放行。
statementRoutes.post('/api/statement/normalize-auto', wrap(async (req, res) => res.json(
  await normalizeIfRulesChanged(!!req.body?.force, { proceedWithoutBackup: !!req.body?.proceedWithoutBackup }))));
...
statementRoutes.post('/api/statement/rules', wrap(async (req, res) => res.json(
  await saveStoreRules(req.body?.rules, { proceedWithoutBackup: !!req.body?.proceedWithoutBackup }))));
