...
import { previewBankStatement, applyBankStatement, applyOptsFromBody, listBankBatches, deleteBankBatch, listLearnedBank, deleteLearnedBank, applyLearnedBankToExisting, listParseRecipes, deleteParseRecipe } from '../services/bank-import.js';
import { makeAnthropicBankEngine, makeAnthropicCardEngine } from '../ai-transport.js';   // P1b-1 真引擎唯一組裝點（本檔全靜態路徑＝外連登記閘可審計）；批二加卡片引擎（同一條傳輸）
import { makeAiBudget } from '../ai-budget.js';   // 成本護欄 C1：每個請求一份預算，engine 工廠閉包它＝每一發先過 take()
import { updateAiUsage } from '../repo.js';   // 單日發數的原子櫃檯（settings.aiUsage，server-owned）
import { nowLocal } from '../services/snapshot.js';   // 本地日曆日（UTC 會讓台北早上的「今天」早一天）
import { wrapRoute as wrap, streamNdjson } from './route-helpers.js';   // 錯誤口徑歸戶（系統優化 U2）；語意同原本地 wrap；串流＝上傳進度（2026-08-18）

export const statementRoutes = Router();

// 自動預覽（免選卡）／指定卡片預覽／匯入。password 可選（P0.5）：留白＝自動試統一密碼池
// 批二 AI 路（規矩同銀行端點）：`useAi` 嚴格布林、真引擎在這裡組裝注入、成本護欄每請求一份。
const cardAiWiring = () => {
  const budget = makeAiBudget({ updateUsage: updateAiUsage, today: () => nowLocal().date });
  return { aiEngineFactory: (/** @type {string} */ key) => makeAnthropicCardEngine(key, budget), aiBudget: budget };
};
statementRoutes.post('/api/statement/preview', wrap(async (req, res) => res.json(await previewAuto(req.body.data, req.body.password, { useAi: req.body.useAi === true, ...cardAiWiring() }))));
...
statementRoutes.post('/api/cards/:id/statement/preview', wrap(async (req, res) => res.json(await previewForCard(req.params.id, req.body.data, req.body.password, undefined, { aiTicket: typeof req.body.aiTicket === 'string' ? req.body.aiTicket : undefined }))));
statementRoutes.post('/api/cards/:id/statement/import', wrap(async (req, res) =>
  // 批四：aiTicket 選配——AI 票＝匯入成功後學規則卡（Opus 一發、走同一份成本護欄）；
  // 規則卡票＝畢業/疑似過期計數。票缺席＝照舊匯入、只是這期不學。
  res.json(await importRows(req.params.id, req.body.transactions, req.body.statementMonth, req.body.statementDue,
    { aiTicket: typeof req.body.aiTicket === 'string' ? req.body.aiTicket : undefined, ...cardAiWiring() }))));

// 銀行對帳單（三層重構 stage 2）：預覽概要區帳戶餘額變動／套用（更新+自動建帳戶）。
// 密碼在 body、**預設只供本次使用**（用完即丟）；只有使用者勾「記住」才依 LOCAL/HOSTED 機密規則儲存（P0.5，見匯入密碼池節）；永不寫 log。
// P1b-1 AI 路線：①**AI 要求旗標**＝只解讀 body 的 `useAi`（布林 true 才算；前端在要求 AI 解讀時帶上
// ——aiAskBeforeSend 打開＝先經確認窗按「同意」，2026-08-13 起預設不問直接帶）＋ apply 另收 `aiTicket`（字串才算＝preview 回的確認票，r4#1：apply 憑票寫入同一份答案、
// 不重跑模型）②真引擎在**這裡**組裝注入（本檔全靜態路徑＝外連登記閘可審計；服務層與 crud.js 拿不到 fetch）
// ③測試接縫（aiExtract 等）不在這裡組裝＝HTTP 端塞什麼都注入不了。
// 進度串流（2026-08-18）：**同一條路徑**、由 body 的 `stream === true` 切換回應形態——刻意不新開端點，
// 因為新路徑要同時登記 body 上限白名單（http-body）、外連登記三張表、限速表與 HOSTED 入場管制，
// 漏登任一處就是機械閘紅或 413；同路徑＝那四處全部沿用既有登記。
// 不帶 stream 的呼叫端（既有考題、外部）行為**一格未動**：照舊一發 JSON。
// 成本護欄（C1，William 2026-08-26 拍板：單張 6 發／單日 20 發／超限擋下白話說明）：**每個請求一份**
// 預算物件，engine 工廠閉包它＝transport 每一發先過 take()；同一份也交給服務層（aiBudget）——
// preview 發票時把已用發數寫進票、apply 兌票後續數，「單張」才橫跨兩個請求數得齊。
const aiWiring = () => {
  const budget = makeAiBudget({ updateUsage: updateAiUsage, today: () => nowLocal().date });   // 函式＝每發取當下日（r3#1）
  return { aiEngineFactory: (/** @type {string} */ key) => makeAnthropicBankEngine(key, budget), aiBudget: budget };
};
statementRoutes.post('/api/bank-statement/preview', wrap(async (req, res) => {
  const opts = { useAi: req.body.useAi === true, ...aiWiring() };
  if (req.body.stream === true) return streamNdjson(res, (onStage) => previewBankStatement(req.body.data, req.body.password, undefined, { ...opts, onStage }));
  return res.json(await previewBankStatement(req.body.data, req.body.password, undefined, opts));
}));
statementRoutes.post('/api/bank-statement/apply', wrap(async (req, res) => res.json(await applyBankStatement(req.body.data, req.body.password, undefined, { ...applyOptsFromBody(req.body), ...aiWiring() }))));
...
statementRoutes.get('/api/parse-recipes', wrap(async (req, res) => res.json(await listParseRecipes())));   // 規則卡管理：只有身分與統計投影（配方內容不外送）
statementRoutes.post('/api/parse-recipes/delete', wrap(async (req, res) => res.json(await deleteParseRecipe(req.body?.id))));
...
statementRoutes.post('/api/statement/normalize-branches', wrap(async (req, res) => {
  if (req.body?.dryRun === true) return res.json(await normalizeBranches(true));
...
statementRoutes.post('/api/statement/health/dismiss', wrap(async (req, res) =>
  res.json(await dismissHealthItem(req.body?.id, req.body?.clearAll === true))));
...
statementRoutes.post('/api/statement/normalize-auto', wrap(async (req, res) => res.json(
  await normalizeIfRulesChanged(req.body?.force === true))));
...
statementRoutes.post('/api/statement/rename-store', wrap(async (req, res) =>
  res.json(await renameStoreDisplay(req.body?.orig, req.body?.name, req.body?.category, req.body?.subcategory, req.body?.reset === true, req.body?.clearBrand === true, req.body?.applyAll === true))));
