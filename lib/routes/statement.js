...
statementRoutes.post('/api/statement/normalize-auto', wrap(async (req, res) => res.json(await normalizeIfRulesChanged(!!req.body?.force))));
...
statementRoutes.post('/api/statement/rules', wrap(async (req, res) => res.json(await saveStoreRules(req.body?.rules))));
