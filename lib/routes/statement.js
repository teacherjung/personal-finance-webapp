...
statementRoutes.get('/api/bank-statement/batches', wrap((req, res) => res.json(listBankBatches())));
statementRoutes.post('/api/bank-statement/batch/delete', wrap((req, res) => res.json(deleteBankBatch(req.body?.batchId))));
// 銀行收支「真·學習」已學規則：檢視／刪除（設定頁「銀行收支學習」用；教錯的救援）
statementRoutes.get('/api/bank-learned', wrap((req, res) => res.json(listLearnedBank())));
statementRoutes.post('/api/bank-learned/delete', wrap((req, res) => res.json(deleteLearnedBank(req.body?.key))));
// 同類一起改（Q2乙）：把某 bankKey 學過的規則套用到所有既有同鑰匙的銀行交易（編輯時勾「同時套用」）
statementRoutes.post('/api/bank-tx/apply-learned', wrap((req, res) => res.json(applyLearnedBankToExisting(req.body?.bankKey))));
...
statementRoutes.post('/api/statement/normalize-branches', wrap((req, res) => {
  if (req.body?.dryRun) return res.json(normalizeBranches(true));
  if (req.body?.force !== true) {
    return res.status(400).json({ error: '維護端點需明確帶 force:true 才會套用（不經確認閘門、學習衝突會直接合併且不可逆）；預覽請帶 dryRun:true。' });
  }
  res.json(normalizeBranches(false));
}));
...
statementRoutes.post('/api/statement/normalize-auto', wrap((req, res) => res.json(normalizeIfRulesChanged(!!req.body?.force))));
// 店名規則自助管理（第三帖）：讀取／全庫影響預覽／存檔並立即套用；孤兒學習條目清單（唯讀）
statementRoutes.get('/api/statement/rules', wrap((req, res) => res.json(getStoreRules())));
statementRoutes.post('/api/statement/rules/preview', wrap((req, res) => res.json(previewStoreRules(req.body?.rules))));
statementRoutes.post('/api/statement/rules', wrap((req, res) => res.json(saveStoreRules(req.body?.rules))));
statementRoutes.get('/api/statement/learned/orphans', wrap((req, res) => res.json(listOrphanLearned())));
...
statementRoutes.post('/api/statement/rename-store', wrap((req, res) =>
  res.json(renameStoreDisplay(req.body?.orig, req.body?.name, req.body?.category, req.body?.subcategory, !!req.body?.reset, !!req.body?.clearBrand, !!req.body?.applyAll))));
