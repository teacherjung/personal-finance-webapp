...
marketRoutes.get('/api/quotes', async (req, res) => {
  ...
  const out = Object.create(null);   // null-proto（Codex r7#4）：symbols 是呼叫端字串，__proto__ 在普通物件上會吞鍵
...
marketRoutes.get('/api/cape', async (req, res) => {
  ...
  try {
    ...
    // getSettings()→load() 在 store 損毀時會 throw；這裡在 try 外，若不再包一層會變成未處理例外→整個程式掛掉
    let manual = null;
    try { manual = getSettings()?.capeManual; } catch { /* store 壞掉：退回無手動值，不讓 fallback 再拖垮程式 */ }
...
marketRoutes.get('/api/realyield', async (req, res) => {
  ...
  try {
    ...
    let manual = null;
    try { manual = getSettings()?.signals?.realYieldManual; } catch { /* store 壞掉：不讓 fallback 再拖垮程式 */ }
