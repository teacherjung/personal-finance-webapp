...
/** @typedef {{parsed:any, aiModel:string, tenant:string, suspectRecipeIds?:string[], recipeUse?:{id:string, usedVersion:'current'|'previous', currentMatched?:boolean, usedRecipe:object}, issuedAt?:string, exp:number, timer?:any}} Ticket */
...
export function issueAiTicket(payload, now = Date.now()) {
  ...
  tickets.set(id, { parsed: payload.parsed, aiModel: payload.aiModel, tenant: tenantKey(), suspectRecipeIds: payload.suspectRecipeIds || [],
