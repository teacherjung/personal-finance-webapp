...
/** @typedef {{parsed:any, aiModel:string, tenant:string, lines?:any[], suspectRecipeIds?:string[], aiCalls?:number, aiIssuer?:string, aiKind?:string, snapshotAt?:string, recipeUse?:{id:string, usedVersion:'current'|'previous', currentMatched?:boolean, usedRecipe:object}, issuedAt?:string, exp:number, timer?:any}} Ticket */
...
export function issueAiTicket(payload, now = Date.now()) {
  ...
  // aiKind（批二 Grok 掃#4）：票綁用途——銀行/配方票的 parsed 沒有卡片摘要、拿到卡片預覽會被
  // 弱閘放行成「銀行列變卡片消費」；兌票端認 kind、發票端寫 kind（沒寫＝舊線的票＝卡片端拒收）。
  tickets.set(id, { parsed: payload.parsed, aiModel: payload.aiModel, tenant: tenantKey(), lines: payload.lines, suspectRecipeIds: payload.suspectRecipeIds || [],
    ...
    ...(typeof payload.aiKind === 'string' && payload.aiKind ? { aiKind: payload.aiKind } : {}),
    // snapshotAt（批四 r7#4）：疑似名單/重生世代檢查要用**原始預覽時刻**——換卡重發票會刷新
    // issuedAt，用它當快照＝把「其後已自證」的窗口蓋掉（自證的卡被錯標疑似/降版）。
    ...(typeof payload.snapshotAt === 'string' && payload.snapshotAt ? { snapshotAt: payload.snapshotAt } : {}),
