...
/** @typedef {{parsed:any, aiModel:string, suspectRecipeIds?:string[], exp:number, timer?:any}} Ticket */
...
export function issueAiTicket(payload, now = Date.now()) {
  ...
  tickets.set(id, { parsed: payload.parsed, aiModel: payload.aiModel, suspectRecipeIds: payload.suspectRecipeIds || [], exp: now + AI_TICKET_TTL_MS, timer });
  return id;
}
...
export function restoreAiTicket(id, ticket, now = Date.now()) {
  ...
  timer?.unref?.();
  tickets.set(key, { parsed: ticket.parsed, aiModel: ticket.aiModel, exp: ticket.exp, timer });
  return true;
}
