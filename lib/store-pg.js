...
//   本機   kv(key, data)                    一次交易寫全部 KV_KEYS
//   雲端   kv(user_id, key, data, version)  一次 RPC 寫全部 KV_KEYS（plpgsql 函式＝單一交易）
...
/**
 * 讀出這位使用者的整包資料。
 * **新租戶（一列都沒有）＝回 `emptyDb()`**，不是一包空 key——`settings` 若是 `{}`，
...
export async function loadKv() {
  ...
  //    而 `saveKv` 每次寫入都重寫全部 KV_KEYS——那個空字串會在使用者下一次隨手記帳時把原密文
...
// ⚠️ 這裡曾經有一支 `currentVersions()`（只讀版本、不讀資料），給 `/api/import` 的整包覆蓋用。
//    **2026-07-28 移除**：它的存在本身就是那個 bug——「寫入前一刻重抓目前版本」讓 CAS 只保護
//    重抓到寫入那一瞬間，而不是呼叫端讀資料到寫入的整段窗口，等於自己蓋章給自己看
//    （Codex 收官審查 #2，已重現「新存的 IB token 被舊值無聲蓋掉、還回 200」）。
//    覆蓋路徑現在一律由呼叫端交出 `getDb()` 當時的版本戳（`saveDb(db, { overwrite, from })`）。
//    **不要把它加回來**——它沒有任何正當呼叫者，加回來就是把那個洞一起加回來。
...
export async function saveKv(db, expected) {
  ...
  //    **每個清除入口的按鈕都由「這個欄位現在有值嗎」的投影把關**（布林 `…Set`，或密碼池那種 `…Count` 組數），
  //    而解不開時記憶體裡是空字串／零組，投影就是 false／0，那個 checkbox 或清除鈕根本不會渲染。
  //    ⚠️ 這裡刻意**不寫死入口有幾個、也不寫死行號**（鐵則 10：寫死的數字自己會漂——P0.5 加第四個機密時就漏改過）；
  //    新增機密欄位時，照這個判準給它一個「有值才出現」的清除入口即可。
