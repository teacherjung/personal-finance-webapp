...
export function sanitizeStoreRules(raw, bad) {
  ...
  return {
    ...
    rename: take('rename', src.rename, (e) => {
      if (!isObj(e)) return null;
      const match = str(e.match), to = str(e.to);
      return (okStr(match, 'rename') && okStr(to, 'rename')) ? { match, to } : null;
    }),
...
export function compileStoreRules(raw) {
  ...
  return {
    ...
    // ⚠️ 比對「完整品牌名 or 使用者填的（截斷）寫法」，**完整版排前面**（正規表示式的 | 取先中的）：
    ...
    brand: r.brand.map(e => ({
      prefix: new RegExp(`^(?:${escapeRe(e.to)}|${escapeRe(e.match)})`, 'i'), brand: e.to })),
    // rename＝BRAND_RENAME 同形狀：全域字串取代（保留分店與其餘部分）
    rename: r.rename.map(e => /** @type {[RegExp, string]} */ ([new RegExp(escapeRe(e.match), 'g'), e.to])),
