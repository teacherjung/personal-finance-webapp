// @ts-check
// 店名規則「資料化」（第三帖，使用者定 2026-07-19）——純模組、幾乎零相依
//（**不 import repo/statement**，避免循環；唯一的 import 是 C4b 的 `lib/tenant.js`，
// 它自己只用 node 內建的 async_hooks，不會把相依鏈接回來）。
import { tenantRulesSlot } from './tenant.js';
//
...
export function sanitizeStoreRules(raw, bad) {
  ...
  return {
    ...
    canon: take('canon', src.canon, (e) => {
      if (!isObj(e)) return null;
      const match = str(e.match), to = str(e.to);
      // mode **缺席**才用預設 contains；**有值但非法**＝壞規則，整條剝除（Codex r4#6）——
      // 原本默默降成最寬的 contains，會讓拼字錯誤（'containz'）意外命中一大票店家。
      if (e.mode !== undefined && !MATCH_MODES.includes(str(e.mode))) return null;
      const mode = e.mode === undefined ? 'contains' : str(e.mode);
 ...
 * 那對「櫃檯單欄寫入」是對的——不能讓一條壞規則卡死整個資料庫。
 * （⚠️ Codex r10#3 更正：**匯入整包備份**已改成 fail-closed——sanitizeStoreRules 照樣寬鬆剝除＋回報壞值，
 * 但 /api/import 的 handler 會**依回報的壞值整份退回 400、什麼都不動**，避免默默清空使用者手做的規則/分類樹。）
...
// **誰負責餵進來**：`lib/repo.js` 的每一次讀取（「規則入櫃檯」，見 AGENTS.md 同步點）——
// 任何要用到店名規則的程式碼，一定得先向櫃檯拿資料，所以在櫃檯同步就不可能忘記、也不必額外讀檔。
//
// ⚠️ **住在哪裡分兩種（C4b，2026-07-27）**：
//   LOCAL（單人本機）＝下面的**模組級單例** `moduleSlot`，與 C4b 之前完全相同。
//   HOSTED（多人）  ＝**每個請求自己的槽**（`lib/tenant.js` 的 AsyncLocalStorage）。
// 為什麼非改不可：模組級單例是跨請求共用的全域狀態。多人情境下 A 的請求剛把自己的規則設進來、
// 一個 await 之後 B 的讀取就把它洗掉，A 後半段的店名清理用的是 B 的規則；更糟的是
// `userRulesFingerprint()` 會被寫進 `settings.storeRulesHash`（statement-import.js），
// 於是「B 的規則指紋」被**持久化**到 A 的資料裡——之後 A 的店名整理要嘛永遠不再跑、
// 要嘛每次開 app 都重跑，而且清快取修不好。
// 覆蓋層（預覽）同理：它也是請求範圍的東西，一起搬進槽裡。

const DEFAULT_RAW = emptyStoreRules();
const DEFAULT_RAW_JSON = JSON.stringify(DEFAULT_RAW);
const DEFAULT_COMPILED = compileStoreRules(DEFAULT_RAW);

/** @typedef {{raw: any, rawJson: string|null, compiled: any, override: any}} RulesSlot */
/** LOCAL 的槽（模組級單例）。 @type {RulesSlot} */
const moduleSlot = { raw: DEFAULT_RAW, rawJson: DEFAULT_RAW_JSON, compiled: DEFAULT_COMPILED, override: null };

/** 這一刻該用哪個槽：有租戶 context（HOSTED 的請求中）就用它自己的，否則用模組級單例。 @returns {RulesSlot} */
function slot() {
  const t = tenantRulesSlot();
  if (!t) return moduleSlot;
  // 這個請求還沒被餵過規則 → 以**內建預設**起步（不是 moduleSlot 的現值，避免沾到別人的東西）
  if (t.rawJson === null) { t.raw = DEFAULT_RAW; t.rawJson = DEFAULT_RAW_JSON; t.compiled = DEFAULT_COMPILED; }
  return t;
}
...
export function setUserRules(raw) {
  ...
  if (json === s.rawJson) return false;
  s.raw = clean;
  s.rawJson = json;
  s.compiled = compileStoreRules(clean);
  return true;
}
...
export function setRulesOverride(raw) {
  slot().override = raw === null || raw === undefined ? null : compileStoreRules(raw);
}

/** 目前生效的使用者規則（已編譯，statement.js 用）：預覽覆蓋優先。 */
export function getUserRules() { const s = slot(); return s.override || s.compiled; }

/** 目前生效的使用者規則（純資料，UI 顯示與規則指紋用）。 */
export function getUserRulesRaw() { return slot().raw; }

/** 規則指紋的一部分：使用者規則的正規化字串（改了規則 → 開 app 自動重跑店名整理）。 @returns {string} */
export function userRulesFingerprint() { return slot().rawJson || DEFAULT_RAW_JSON; }
