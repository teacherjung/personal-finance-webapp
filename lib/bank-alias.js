 ...
 * ⚠️ 也**不含**「儲蓄銀行／商業儲蓄銀行」：台灣只有「上海商業儲蓄銀行」用它，而剝掉後剩下的城市名
 *    會跟香港「上海商業銀行」、中國「上海銀行」撞在一起（預審實測三家被壓成同一個短名＝餘額互蓋）。
 *    這一族走 PROTECTED（整串原樣、不剝）。
 * 這張表與 parse-recipe.js 的 GENERIC_FI 回答的是**不同問題**（那邊問「是不是只剩通用詞」、要寬；
 * 這邊問「剝掉會不會變別家」、要窄），所以是兩張表、不是漂掉的複本。 */
const COMPANY_SUFFIXES = ['股份有限公司', '有限公司'];   // 公司型態字：先剝，剝完再過受保護名（「中國銀行股份有限公司」＝中國銀行）
const STRIP_SUFFIXES = ['國際商業銀行', '商業銀行', '信用合作社', '信合社', '商銀', '銀行'];

/** **受保護的全名**：剝掉後綴會變成別家（或變成城市名）的機構——整串原樣回，不剝、不查表。
 * 判準＝「剝完的核心是另一家銀行的核心、或是地名」。名單刻意短：只收預審實測會撞的。 */
const PROTECTED = new Set(['上海商業儲蓄銀行', '上海商業銀行', '上海銀行', '中國銀行', '中國國際商業銀行']);
...
/** 縮寫 → 受保護全名（先於剝銀行後綴查，否則「上海商銀」會被剝成「上海」）。 */
...
const ALIASES = /** @type {Record<string, string>} */ ({
  ...
  bankoftaiwan: '台灣銀行', 台銀: '台灣銀行',   // 裸「台灣」刻意**不**列：地名不是機構（同 PLACE_NAMES 的方向）
});

/** 英文的通用後綴（同 STRIP_SUFFIXES 的中文版；大小寫不拘、只認結尾）：公司型態字（Co., Ltd.／Limited／Inc.）
 * 與銀行字樣各一條，逐輪剝——「Taishin Bank Co., Ltd.」要剝兩輪才到「Taishin」（Codex #499 r2#1）。 */
...
/** 剝完只剩**地名**＝不剝（Codex #499 r2#2）：「臺中商業銀行」「高雄銀行」「彰化商業銀行」剝掉銀行字樣就是城市，
 * 城市不是機構——停在剝之前的那個形。代價＝同一家的不同寫法（彰化商業銀行／彰化銀行）會並存成兩個短名，
 * 那是漏合併的方向，可以再加別名；亂合併才是不可逆的方向。名單＝台灣縣市＋本表已遇到的地名。 */
const PLACE_NAMES = new Set(['台北', '新北', '桃園', '台中', '台南', '高雄', '基隆', '新竹', '苗栗', '彰化', '南投', '雲林', '嘉義', '屏東', '宜蘭', '花蓮', '台東', '澎湖', '金門', '連江', '馬祖', '上海', '中國', '台灣', '香港', '日本', '美國']);
 ...
 * 順序：①基本形 ②一個迴圈：每輪先查受保護全名／縮寫（命中就停），再剝一個後綴（公司型態字先、銀行後綴次之、英文後綴）直到剝不動；剝完只剩地名就停在剝之前 ③查別名表 ④純拉丁字母名摺疊成大寫。
 * 剝到空字串（輸入只有「銀行」之類）＝回**剝之前**的形——「銀行」不是任何一家，但也不可回空讓呼叫端以為缺席。
 * @param {string} raw @returns {string}
 */
/** 剝後綴的核心（正式路徑的中段）：回 { base, core, stop }——stop＝中途命中受保護名／縮寫的結果（有值就是答案）。
 * @param {string} raw */
function stripCore(raw) {
  const base = baseForm(raw);
  if (!base) return { base, core: '', stop: '' };
  // 單一迴圈：每一輪**先**看是不是受保護全名／縮寫（整串比對、不剝銀行後綴），再剝一個後綴。
  //   公司型態字（股份有限公司）排在銀行後綴前面剝——官方登記名帶著它（「中國銀行股份有限公司」），
  //   不先剝就過不了受保護名、會一路剝成城市名（Codex #499 r1#1：中國銀行與中國國際商業銀行被壓成「中國」＝餘額互蓋）。
  let core = base;
  for (;;) {
    if (PROTECTED.has(core)) return { base, core, stop: core };
    const abbr = lookup(ABBREV, core);
    if (abbr) return { base, core, stop: abbr };
    const before = core;
    let next = core;
    for (const suf of [...COMPANY_SUFFIXES, ...STRIP_SUFFIXES]) if (next.endsWith(suf)) { next = next.slice(0, -suf.length).trim(); break; }
    if (next === before) for (const re of STRIP_SUFFIXES_EN) { const t = next.replace(re, '').trim(); if (t !== next) { next = t; break; } }
    if (next === before || !next) break;
    if (PLACE_NAMES.has(next)) break;   // 剝完只剩地名＝停在剝之前（「高雄銀行」不是「高雄」）
    core = next;
  }
  return { base, core, stop: '' };
}

export function canonicalBank(raw) {
  const { base, core, stop } = stripCore(raw);
  if (!base) return '';
  if (stop) return stop;
  if (!core) return base;
  const hit = lookup(ALIASES, lookupKey(core));
  if (hit) return hit;
  // 不在表上的純拉丁字母名（外商）：大小寫摺疊成大寫當比對形——同一家的「HSBC」「hsbc」不可被當兩家
  return /^[A-Za-z0-9 .&'-]+$/.test(core) ? core.toUpperCase() : core;
}
...
export function aliasEntriesForTest() {
  return {
    ...
    /** 正式路徑剝完後綴後拿去查表的鍵；中途命中受保護名／縮寫＝不會查表，回 ''。 @param {string} raw */
    lookupKeyOf: (raw) => { const { core, stop } = stripCore(raw); return stop || !core ? '' : lookupKey(core); },
  };
}
