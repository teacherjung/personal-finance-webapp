 ...
 * 順序：①基本形 ②一個迴圈：每輪先查受保護全名／縮寫（命中就停），再剝一個後綴（公司型態字先、銀行後綴次之、英文後綴）直到剝不動 ③查別名表 ④純拉丁字母名摺疊成大寫。
...
export function canonicalBank(raw) {
  ...
  for (;;) {
    ...
    if (abbr) return abbr;
    const before = core;
    for (const suf of [...COMPANY_SUFFIXES, ...STRIP_SUFFIXES]) if (core.endsWith(suf)) { core = core.slice(0, -suf.length).trim(); break; }
    if (core === before) core = core.replace(STRIP_SUFFIXES_EN, '').trim();
    if (core === before || !core) break;
  }
  if (!core) return base;
