...
export function canonicalBank(raw) {
  ...
  let core = base;
  for (;;) {
    if (PROTECTED.has(core)) return core;
...
/** 測試接縫：別名表的每一條（鍵＝比對形、值＝短名）＋比對形函式——讓「別名表每一條都有考題」變成機械保證，
 * 不是口頭的。正式呼叫端不用。 */
export function aliasEntriesForTest() { return { entries: Object.entries(ALIASES), keyOf: lookupKey }; }
