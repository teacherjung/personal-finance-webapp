...
//   其他機構兩種寫法並存＝漏合併，看得出來、可事後補別名；**疑似重複的提醒**另有一把寬鬆的尺（bank-import.js instKey），
//   那只是提醒、不動錢。新機構要進這把尺＝使用者貼抬頭後加，並先過考題的「兩兩不同」表。
...
export function canonRef(ref) {
  ...
  // 真交易吞成重複（Codex #499 r1#2）。內建範本與 AI 路線的台新列帳號段一律帶星號（含沒遮罩時自己補的 `x****末碼`）。
