...
const STRIP_SUFFIXES = ['股份有限公司', '有限公司', '國際商業銀行', '商業銀行', '信用合作社', '信合社', '商銀', '銀行'];
...
/** 縮寫 → 受保護全名（先於剝後綴查，否則「上海商銀」會被剝成「上海」）。 */
 ...
 * 順序：①基本形 ②受保護全名／縮寫（整串比對，不剝）③剝通用後綴（重複剝到剝不動）④查別名表 ⑤純拉丁字母名摺疊成大寫。
...
export function canonicalBank(raw) {
  ...
  if (!base) return '';
  if (PROTECTED.has(base)) return base;
  const abbr = ABBREV[base];
  if (abbr) return abbr;
  let core = base;
  for (;;) {
    const before = core;
  ...
  if (!core) return base;
  const hit = ALIASES[lookupKey(core)];
  if (hit) return hit;
