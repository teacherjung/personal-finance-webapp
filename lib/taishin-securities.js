// @ts-check
// 台新「證券」電子對帳單解析器（證券交易 S1，設計藍圖 docs/archive/securities-trading-blueprint.md §三）。
...
export async function extractSecuritiesLines(data, password) {
  ...
  const lines = [];
  let pageBase = 0;
  // 解析器資源上限（2026-07-28）：見 lib/parse-limits.js 檔頭。超標明確拒絕，不靜默截斷。
  // ⚠️ **整段包在 try/finally 裡**：上限一 throw 就會跳過 `task.destroy()`，
  //    pdfjs 的 worker 與已配置的頁面資源就留著不放——「防資源耗盡」的那條路自己在漏資源。
  try {
    assertPageLimit(doc.numPages, '證券對帳單 PDF');
    let itemCount = 0;
    for (let i = 1; i <= doc.numPages; i++) {
      ...
      const tc = await readPageTextCapped(page, itemCount, '證券對帳單 PDF');
      itemCount = tc.count;
...
/** 這份 PDF 像不像台新「證券」對帳單（與銀行綜合對帳單、信用卡帳單區分）。
 * 真版面校準（2026-07-24，使用者 2026/03 真單）：全文**沒有**「成交明細」字樣——成交表的段落
 * 標題是「上市/上櫃/興櫃」。可靠標記＝「有價證券」＋「電子對帳單」（第一頁問候句）或
 * 「應收付金額」（成交表表頭）；保留舊的「(成交|交易)明細」相容其他版式。 @param {SLine[]} lines */
export function isSecuritiesStatement(lines) {
  const text = lines.map(l => squash(l.cells.map(c => c.s).join(''))).join('\n');
  if (!/證券/.test(text) || /帳戶概要區/.test(text)) return false;
  return /(成交|交易)明細/.test(text)
    || (/有價證券/.test(text) && /電子對[帳賬]單/.test(text))
    || /應收付金額/.test(text);
}
...
export function extractSecStatementMonth(lines) {
  for (const line of lines.slice(0, 40)) {   // 年月在表頭區；限前段免掃到法規文字裡的年月
    ...
    // 月份過真實日曆（Codex S2r1#7）：'2026年13月' 只驗字形會產出 '2026-13'，連鎖讓 MM/DD 年份推斷全錯。
    const mk = (/** @type {number} */ y, /** @type {any} */ mo) => { const v = `${y}-${String(Number(mo)).padStart(2, '0')}`; return isRealMonth(v) ? v : null; };
    let m = t.match(/(\d{4})年(\d{1,2})月/);
    if (m) return mk(Number(m[1]), m[2]);
    m = t.match(/(?<!\d)(\d{2,3})年(\d{1,2})月/);
    if (m) return mk(Number(m[1]) + 1911, m[2]);
    // 後界改「非數字」（真版面校準：問候句是「…您2026/03在台新綜合證券…」，/03 後面接中文字，
    // 原本要求 月/[/.]/行尾 會漏接）；(?!\d) 仍擋 '2026/034' 之類誤咬
...
export function resolveSecDate(s, stmtMonth) {
  // 真版面校準：成交日印成「03/03@」（@＝電子單註記），尾端註記符號先剝掉再解析
  const str = String(s || '').trim().replace(/[\s@＠*※]+$/, '');
...
// 真版面校準（2026-07-24）：上層另有 融資自備款/融資金額/現沖數量/融券手續費/證所稅、下層另有
// 融券擔保品/融券保證金/資券利息/債息或扣繳息/健保費 等欄——**必須映射成佔位欄**（xIgnore*）：
// band 分欄的邊界是「下一個已映射欄的表頭左緣」，不映射＝相鄰欄的 band 會吞下這些欄的值
//（融資單的融資金額會黏進手續費）。佔位欄只用來劃界，parseTaishinSecurities 不讀它們。
...
const H_A = {
  ...
  '融資自備款': 'xIgnoreA1', '融資金額': 'xIgnoreA2', '現沖數量': 'xIgnoreA3',
  '融券手續費': 'xIgnoreA4', '證所稅': 'xIgnoreA5',
};
/** @type {Record<string,string>} */
const H_B = {
  ...
  '幣別': 'currency',
  '應收付金額': 'netSettlement',   // 真版面：應收付金額印在**下層**（每筆第二行）——上層地圖照樣保留相容其他版式
  '融券擔保品': 'xIgnoreB1', '融券保證金': 'xIgnoreB2', '資券利息': 'xIgnoreB3',
  '債息或扣繳息': 'xIgnoreB4', '健保費': 'xIgnoreB5',
};

/**
 * 表頭 token 抽取（含**相鄰儲存格合併**）：真版面把「數 量」印成「數」「量」兩個儲存格——
 * 單格比不到字典時，試著把相鄰 1~2 格併起來再比（x 取第一格左緣）。回傳 [{x, field}]。
 * @param {Cell[]} cells @param {Record<string,string>} dict @returns {{x:number, field:string}[]}
 */
function headerTokens(cells, dict) {
  /** @type {{x:number, field:string}[]} */
  const cols = [];
  for (let i = 0; i < cells.length; i++) {
    for (const take of [1, 2, 3]) {
      if (i + take > cells.length) break;
      const k = squash(cells.slice(i, i + take).map(c => c.s).join(''));
      if (Object.hasOwn(dict, k)) { cols.push({ x: cells[i].x, field: dict[k] }); i += take - 1; break; }
    }
  }
  return cols;
}
const isHeaderLine = (/** @type {Cell[]} */ cells, /** @type {Record<string,string>} */ dict) =>
  headerTokens(cells, dict).filter(c => !c.field.startsWith('xIgnore')).length >= 2;   // 佔位欄不算數（防融資欄湊數誤判）
...
function colsOf(cells, dict) {
  return headerTokens(cells, dict).sort((a, b) => a.x - b.x);
}
...
// 真版面校準：交割彙總列印的是「應收付｜03/05｜454,423」（沒有「日」字）——(?!金額) 排除表頭的
// 「應收付金額」與每筆的金額欄（金額欄只有數字、不含「應收付」三字，安全）；舊版式「應收付日期…」
// 也被涵蓋（日≠金）。「合計/總計」不在此列——那是表尾總結，見 parse 迴圈的攔截。
const SUMMARY_RE = /應收付(?!金額)/;
...
export function parseTaishinSecurities(lines) {
  ...
  for (const line of lines) {
    ...
    if (!accountRaw) {
      // 真版面校準 r4（2026/06 實單）：帳號可含英文字母（8B52-…；分公司也可能印成文字「新莊分公司」）
      // ——字元集放寬到英數；防誤咬：抓到的字串至少要有 4 個數字（遮罩 label 取末四碼也靠它）
      const m = joined.match(/帳[戶號][:：]?([A-Za-z\d*-]{6,})/);
      if (m && m[1].replace(/\D/g, '').length >= 4) accountRaw = m[1];
    }

    // 雙層表頭：上層（A 欄名）與下層（B 欄名）各自比對；兩層都到齊才進明細模式
    if (isHeaderLine(line.cells, H_A)) { colsA = colsOf(line.cells, H_A); continue; }
    if (colsA.length && isHeaderLine(line.cells, H_B)) { colsB = colsOf(line.cells, H_B); headerFound = true; continue; }
    ...
    // 表尾「合計/總計」總結列攔截（真版面 r4：四月實單只有一筆成交、**沒有**逐交割的應收付列——
    // 合計列的最後一個金額是手續費合計，拿去當交割彙總核對必然不符＝誤擋整份）。跳過、不吃成員：
    // 成員留給後續含「應收付」的總結列核對；都沒有就走收尾 fallback 群組（sumMatches null、不阻擋）。
    if (/合計|總計/.test(joined) && !/應收付(?!金額)/.test(joined)) { pendingA = null; continue; }

    // 交割彙總列：應收付（＋日期）→ 回填交割日＋交叉核對群組加總
    if (SUMMARY_RE.test(joined)) {
      ...
      // 真版面校準：表尾另有「合計」總結列（手續費合計/全月應收付）——沒有成員的彙總列不是交割群組，
      // 跳過不記（不然每份單尾端都多出 tradeCount:0 的幽靈群組）
      if (!members.length) { pendingA = null; continue; }
      let sumMatches = null;
      if (gTotal != null && members.length) {
        const sides = members.map(t => taishinSide(t.rawType));   // 嚴格枚舉真相表（自審：與 sideHint 判準統一）
        const allKnown = sides.every(s => s !== null);
        const allSame = allKnown && sides.every(s => s === sides[0]);
        if (allSame) {
          // 同向群組：各筆**絕對值**加總 ≈ |合計|。⚠️ 取 abs（真版面 r4：六月式版面買進應收付印負數
          // ——不取 abs，全買群組會拿負和去比正合計、必不符）。
          const sum = members.reduce((acc, t) => acc + Math.abs(t.netSettlement || 0), 0);
          sumMatches = Math.abs(sum - Math.abs(gTotal)) < 1;
        } else if (allKnown) {
          // 混合買賣（真版面 r4 校準，2026/06 實單）：月末「應收付」＝**淨額**（賣－買；該單印帶號 -971,909，
          // 8 筆帳面驗算分毫不差）。比對兩邊都取絕對值——淨額大小相等即核對成立，容忍「印帶號 vs 印絕對值」兩種版式。
          const signed = members.reduce((acc, t, i) => acc + (sides[i] === 'sell' ? 1 : -1) * Math.abs(t.netSettlement || 0), 0);
          sumMatches = Math.abs(Math.abs(signed) - Math.abs(gTotal)) < 1;
        }
        // 含未知類別 → 保持 null（方向不可猜，核對不了）
      }
    ...
    if (pendingA && Math.abs(pendingAY - line.y) <= DY_MAX) {
      ...
      if (b.rawType || b.name || b.grossAmount) {
        ...
        // 真版面：應收付金額印在第二行（B 層）——A 層沒讀到才用 B 層的（兩層都印時以 A 為準，相容舊版式）
        if (pendingA.netSettlement == null && b.netSettlement != null) pendingA.netSettlement = num(b.netSettlement);
 ...
 * 兩邊都印年的跨年單「115年1月~116年3月」）。
 * 只掃前段（年月都印在表頭區）；單月的「115年1月」不會命中。
 *
 * ⚠️ **兩條正則是擇一（∨），各自守不同的版式，缺一角就會有一種跨度單靜靜過關**
 * （去重鍵 sourceRef 含對帳單年月，跨度後段的交易會全部掛錯年月＝去重鍵毒化）。
 * 兩條各有考題在 `test/taishin-securities.test.js`；主入口拿它 throw 400 的那條接線在
 * `test/pdf-extract-integrity.test.js`。
...
export function isMultiMonthHeader(lines) {
  for (const line of lines.slice(0, 40)) {
    ...
    // 「年N月＋連接號」＝跨度，**不看右邊印什麼**。這一條唯一守得住的是「115年1月~116年3月」這種
    // 兩邊都印年的跨年單：上面那條要求連接號右邊緊接「N月」，而那裡是「116年」，接不住。
    if (/年\d{1,2}月[~～〜至\-—–－]/.test(t)) return true;
  }
  return false;
}
...
export async function parseTaishinSecuritiesPdf(data, password) {
  const lines = await extractPdfLines('securities', extractSecuritiesLines, data, password);
