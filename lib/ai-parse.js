...
export const AI_BANK_SCHEMA = {
  ...
  properties: {
    ...
    referenceDate: { type: ['string', 'null'], description: '帳戶餘額的現值參考日（餘額算到哪一天），西元 YYYY-MM-DD。'
      + '帳單沒印這個欄位、但有印「帳單期間 A ~ B」時，填 B（結束日）；兩者都沒有＝null' },
    accountCurrencies: {
      type: 'array',
...
export function buildBankSystem() {
  return [
    ...
    '1a. referenceDate（現值參考日＝餘額算到哪一天）：帳單若直接印「現值參考日」就照抄；'
      + '**若沒印，但印了「帳單期間 A ~ B」「本期起訖 A ~ B」這類區間，就填 B（結束日）**——'
      + '期末餘額本來就是截至區間結束那天。⚠️ 只認**結束日**：不可填開始日、不可填今天、'
      + '不可自己推算或補一個沒印在帳單上的日期。兩者都沒有＝null。',
