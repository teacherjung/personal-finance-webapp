 ...
 * **textVariance（建議面：不觸發、徽章註明「已採 Opus 版」）**＝機構名／交易備註／帳號印法
...
export function aiAnswersAgree(a, b) {
  ...
  const suffixCur = (/** @type {Record<string,string>} */ m) => Object.entries(m).map(([k, v]) => `${String(k).slice(-4)}\u0000${v}`).sort();
  const sa = suffixCur(ca), sb = suffixCur(cb);
  ...
  // 帳戶錢組成＝末碼＋幣別＋餘額（錢面嚴格）；masked 印法差異＝建議面
  const accMoney = (/** @type {any} */ x) => [x?.suffix, x?.currency, x?.balance].join('\u0000');
  ...
  if (ta.length !== tb.length) diffs.push('交易筆數');
    ...
    if (!soft(e?.acctMasked, f?.acctMasked) && e?.acctSuffix === f?.acctSuffix) { if (!textVariance.includes('帳號印法')) textVariance.push('帳號印法'); }
