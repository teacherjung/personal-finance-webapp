...
/** 兩個遮罩帳號的身分判別（P2-4c「完整號碼認身分」）：
 * 'same'＝正規化後相等（分隔符差異歸零、連「印法」提示都不列）；
 * 'conflict'＝**兩邊都印得出可見前綴、而且對不上**（900200 vs 900311）＝不同戶——錢的歸屬、要觸發
 *   （P1a「可見前綴只在兩邊都印得出來時才否決」同款判準；前綴一長一短但尾端相容（0200 vs 900200）
 *   ＝同戶的不同印法、不否決）；末碼不同也算 conflict（呼叫端多半已由末碼 hard 先擋）；
 * 'variance'＝其餘（少印前綴、星號長短不同）＝印法差異、只列建議面。 */
export function maskedCmp(/** @type {any} */ x, /** @type {any} */ y) {
  const cx = canonMasked(x), cy = canonMasked(y);
  if (cx === cy) return 'same';
  if (accountSuffix(cx) !== accountSuffix(cy)) return 'conflict';
  const pre = (/** @type {string} */ c) => { const m = /^([0-9]+)[*]/.exec(c); return m ? m[1] : ''; };
  const px = pre(cx), py = pre(cy);
  if (px && py && !px.endsWith(py) && !py.endsWith(px)) return 'conflict';
  return 'variance';
}
  ...
  if (sa.length !== sb.length || sa.some((k, i) => k !== sb[i])) diffs.push('帳戶幣別表');
    ...
    for (const [s0, ksA] of ma) {
      ...
      if (ksA.length === 1 && ksB.length === 1) {
        ...
        if (mc === 'conflict') diffs.push('帳戶帳號');
        else if (mc === 'variance' && !textVariance.includes('帳號印法')) textVariance.push('帳號印法');
      }
    }
  }
  const accMoney = (/** @type {any} */ x) => [collide.has(String(x?.suffix ?? '')) ? canonMasked(x?.masked) : '', x?.suffix, x?.currency, x?.balance].join('\u0000');
