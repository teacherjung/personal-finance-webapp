...
import { accountSuffixAny, normalizeMaskShape, noteAccountCurrency } from './bank-statement.js';
...
export function normalizeAiBank(raw) {
  ...
  // 每個 masked **列過哪些幣別**（成員判準用）：外幣綜合帳戶＝同號多幣別（真帳單證實 JPY+USD 同號），
  // 一致性檢查若沿用「等值」就必然打架＝整份拒收（3 份真帳單 100% 重現、與模型用力程度無關）。
  // 對外交出的 accountCurrency 走 noteAccountCurrency＝多幣別填哨兵（見那支的說明）。
  /** @type {Map<string, Set<string>>} */
  const listedCurrencies = new Map();
  for (let i = 0; i < raw.accountCurrencies.length; i++) {
    ...
    if (!listedCurrencies.has(masked)) listedCurrencies.set(masked, new Set());
    /** @type {Set<string>} */ (listedCurrencies.get(masked)).add(currency);
    noteAccountCurrency(accountCurrency, masked, currency);   // 同帳號多幣別＝哨兵（三條路同一份判準）
  }
  ...
  for (const set of listedCurrencies.values()) {   // 只看幣別集合＝訊息不帶帳號（r1#1）
    if (set.size > 1 && set.has('TWD')) {
      // ⚠️ 訊息**不帶末碼／任何欄值**（Codex #517 r1#1）：本模組的錯誤訊息一律只講欄名與序號，
      //   末碼是帳號的一部分＝欄值。使用者靠「同一個帳號同時列了台幣與外幣」這句就找得到那一列。
      throw apiError(400, '這份帳單有同一個帳號同時列了台幣與外幣，目前分不出每一筆是哪一種幣別——為了不把沒驗算過的數字記進帳本，這份不收。請改用手動記帳。', 'ai_bad_answer');
    }
  }
  ...
  const accounts = raw.accounts.map((/** @type {any} */ a, /** @type {number} */ i) => {
    ...
    const listed = listedCurrencies.get(masked);
  ...
  const transactions = raw.transactions.map((/** @type {any} */ t, /** @type {number} */ i) => {
    ...
    if (!listedCurrencies.has(acctMasked)) throw apiError(400, `AI 答案卷的 transactions[${i}] 帳號不在 accountCurrencies 裡（每個交易帳號都要有幣別身分）`, 'ai_bad_answer');
