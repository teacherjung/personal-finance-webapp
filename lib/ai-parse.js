...
export function normalizeAiBank(raw) {
  ...
  const accounts = raw.accounts.map((/** @type {any} */ a, /** @type {number} */ i) => {
    ...
    if (kindGiven) {
      ...
      if (has && typeof a.kind !== 'string') throw apiError(400, `AI 答案卷的 accounts[${i}].kind 型別不對（要字串 demand/time）`, 'ai_bad_answer');
      const k = has ? String(a.kind).trim() : 'demand';   // 這一列沒填＝視同活存（整份已因別列而結構化）
