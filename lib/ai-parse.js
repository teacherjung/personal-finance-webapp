...
/** 遮罩帳號正規化（P2-4c，William 2026-08-17 核准「程式端帳號正規化」）：NFKC 後只留數字與 `*`
 * ——剝 `-`、空格、全形分隔符。⚠️ 「照抄」鐵則不動 AI（要它邊讀邊改寫＝多一步可出錯、也破壞接地
 * 的逐字對照）；格式統一全在程式端＝確定性、零費用。只用於**比對與身分判別**——寫進帳本的 masked
 * 照舊存 AI 抄的原樣（動儲存形＝動去重鍵＝另一支的事、本支不碰）。 */
export function canonMasked(/** @type {any} */ s) { return String(s ?? '').normalize('NFKC').replace(/[^0-9*]/g, ''); }
 ...
 * P1a 同款取捨，1~2 碼前綴的鑑別力本來就低 ②字母與非 ASCII 遮罩符（•、x）被正規化剝掉——台灣銀行
 * 帳號常態純數字＋星號；帶字母的號碼形若成真實形再收（殘餘記契約）。 */
...
export function aiAnswersAgree(a, b) {
  ...
  const suf = (/** @type {any} */ k) => accountSuffix(canonMasked(k));   // P2-4c：末碼也從正規形取
  ...
  const periodsBy = (/** @type {any[]} */ accs) => {
    ...
    for (const x of accs || []) {
      ...
      const k = [x?.suffix, x?.currency, x?.balance].join('\u0000');
 ...
 * 認不出兩個完整日期＝回空字串（fail-safe：無期間＝到期歸零那段直接不判死活，寧可留著）。
...
export function canonPeriod(v) {
  ...
  if (a > b) return '';   // 起日晚於迄日＝抄反了（Codex #488 r2#3：收下它，下期讀成正確順序時兩邊期間都有值而不同＝拒絕盲配＝裂成兩戶）
  return `${a}~${b}`;
}
    ...
    accounts: {
      ...
      items: {
        ...
        properties: {
          ...
          // 重複、**靜靜少算一筆**（William 的兩筆 51 美元實例）。補這兩欄＝分開列管與到期歸零在 AI 路線
          // 也生效，且對帳閘的「定存概要列不對末筆」skip 才認得出來。
          kind: { type: 'string', enum: ['demand', 'time'], description: '存款類別：定期存款/定存/整存整付＝time，其餘（活存、支存、綜存）＝demand' },
...
export function normalizeAiBank(raw) {
  ...
  // ⚠️ 「**AI 有講才算數**」（2026-08-18）：只要有任何一列填了 kind，整份就當**結構化答案**、逐列帶
  //    kind/period；一列都沒填＝退回舊形狀（accounts 完全沒有 kind 欄）。這條界線是刻意的——到期歸零
  //    那段用「本批 accounts 有沒有 kind 欄」判斷自己適不適用（#483 r1#1）：如果這裡替沒講的答案補一個
  //    預設 'demand'，就會讓「AI 其實沒看懂定存」的答案冒充結構化，把還印在帳單上的定存誤歸零。
  const kindGiven = raw.accounts.some((/** @type {any} */ a) => typeof a?.kind === 'string' && a.kind.trim());
  const accounts = raw.accounts.map((/** @type {any} */ a, /** @type {number} */ i) => {
    ...
    /** @type {any} */ let cd = {};
    if (kindGiven) {
      const k = String(a?.kind || 'demand').trim();
