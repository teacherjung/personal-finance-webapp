...
export function normalizeAiBank(raw) {
  ...
  for (const [masked, set] of listedCurrencies) {
    if (set.size > 1 && set.has('TWD')) {
      throw apiError(400, `這份帳單的同一個帳號同時列了台幣與外幣（帳號 …${accountSuffixAny(masked) || ''}），目前分不出每一筆是哪一種幣別——為了不把沒驗算過的數字記進帳本，這份不收。請改用手動記帳。`, 'ai_bad_answer');
    }
  }
