  ...
  return {
    ...
    detail: o.detail && typeof o.detail === 'object'
      ? { rowIdent: o.detail.rowIdent, headerOut: o.detail.headerOut, headerIn: o.detail.headerIn,
          ...
          // headerNote：**null 是合法值**（版面沒有備註欄；validateRecipeStrict 明文放行 null、引擎完整支援）
          // ——一律保留鍵、undefined 正規化成 null。⚠️ 撤回稍早的假宣稱「null＝strict 紅」：那句是誤讀
          // 自己探針輸出寫出來的（探針測的是**缺鍵**、不是 null）——預審實測打臉、照家規當場改口。
          headerNote: o.detail.headerNote ?? null,
          // Grok G4：headerIgnore **一律保留**（空陣列＝合法的「沒有可忽略欄」）——丟掉鍵＝strict 紅＝
          // 沒有單號欄的正常版面永遠存不成卡（首版就這樣壞、實測抓到）
          headerIgnore: Array.isArray(o.detail.headerIgnore) ? o.detail.headerIgnore : [] }
      : undefined,
  };
}
