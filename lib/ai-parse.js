...
export const AI_BANK_SCHEMA = {
  ...
  required: ['bank', 'referenceDate', 'accounts', 'transactions'],
  properties: {
...
export function buildBankSystem() {
  return [
    ...
    '6. 概要區「餘額欄空白」的帳戶不要列進 accounts。摘要/備註欄原文照抄（含機器味文字）。',
  ].join('\n');
}
...
export function normalizeAiBank(raw) {
  ...
  if (raw.referenceDate != null) {
    ...
    if (!isRealDate(d)) throw apiError(400, `AI 答案卷的現值參考日不是真日期（${d}）`, 'ai_bad_answer');
    referenceDate = d;
  }
  ...
  const transactions = raw.transactions.map((/** @type {any} */ t, /** @type {number} */ i) => {
    ...
    if (!isRealDate(date)) throw apiError(400, `AI 答案卷的 transactions[${i}].date 不是真日期（${date}）`, 'ai_bad_answer');
