...
/** 雙讀比對（裁示⑦）：兩份**已各自通過驗收**的答案，逐欄比對「會影響帳本與去重鍵」的欄位——
 * 金額／方向／日期／帳號末碼／幣別／收支說明（＝summary＋note，#458 名詞統一的同一口徑：兩欄都進
 * bankRef 去重鍵）／帳戶餘額組成／機構名／現值參考日。**diffs 只帶欄位路徑、絕不帶欄值**（值可能是
 * 帳單內容＝機密紀律，同 recipeReproduces）。文字欄（summary/note）空白不敏感（r4#1 同理：兩個模型
 * 對格間空白的取法本來就會不同，嚴格等值＝正常帳單天天仲裁）；金額／日期／帳號照樣嚴格。
 * 帳戶 label／note **刻意不比**（不進帳本金額也不進去重鍵；兩個模型的措辭差異不該觸發仲裁）——
 * 一致時採用哪份的 label/note 由「採用 Opus 版」決定（服務層固定採 escalation 那份）。
 * transactions **嚴格比順序**（匯入順序影響畫面；順序不同＝不算一致＝寧可仲裁）。
 * @param {any} a @param {any} b
 * @returns {{agree: boolean, diffs: string[]}} */
export function aiAnswersAgree(a, b) {
  ...
  if (!soft(a?.bank, b?.bank)) diffs.push('機構名');   // r1#1：機構名也是文字欄——兩模型只差排版空白（「第一 銀行」）＝白花仲裁甚至誤判三讀不同
  if (a?.referenceDate !== b?.referenceDate) diffs.push('現值參考日');
  const ca = a?.accountCurrency || {}, cb = b?.accountCurrency || {};
  const ka = Object.keys(ca).sort(), kb = Object.keys(cb).sort();
  if (ka.length !== kb.length || ka.some((k, i) => k !== kb[i]) || ka.some((k) => ca[k] !== cb[k])) diffs.push('帳戶幣別表');
  const accKey = (/** @type {any} */ x) => [x?.masked, x?.suffix, x?.currency, x?.balance].join('\u0000');
  const A = (Array.isArray(a?.accounts) ? a.accounts : []).map(accKey).sort();
  const B = (Array.isArray(b?.accounts) ? b.accounts : []).map(accKey).sort();
  ...
  if (ta.length !== tb.length) diffs.push('交易筆數');
    ...
    for (const [name, label, softly] of /** @type {[string,string,boolean][]} */ ([
      ...
      ['acctMasked', '帳號', false], ['acctSuffix', '帳號末碼', false], ['summary', '摘要', true], ['note', '備註', true],
    ])) {
