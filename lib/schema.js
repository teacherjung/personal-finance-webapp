...
/** @type {Record<string, Record<string, 'number'|'boolean'|'array'|'str'|'date'|'month'|string[]>>} */
export const FIELD_SCHEMA = {
  ...
  history: { amount: 'number', month: 'month' },
 ...
 * @param {'number'|'boolean'|'array'|'str'|'date'|'month'|string[]|undefined} spec @param {any} v
...
function validateField(spec, v) {
  ...
  if (spec === 'date') {                                                // YYYY-MM-DD、空字串（未設）、null；其餘拒絕（壞日期會默默不被計入現金流）
    const ok = v === null || v === '' || (typeof v === 'string' && /^\d{4}-\d{2}-\d{2}$/.test(v));
    return ok ? { ok: true, value: v } : { ok: false, reject: true };
  }
  if (spec === 'month') {                                               // YYYY-MM、空、null
    const ok = v === null || v === '' || (typeof v === 'string' && /^\d{4}-\d{2}$/.test(v));
    return ok ? { ok: true, value: v } : { ok: false, reject: true };
  }
