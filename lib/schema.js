...
export const FIELD_SCHEMA = {
  ...
  transactions: { amount: 'number', date: 'date', autoCat: 'str', autoSub: 'str', stmtMonth: 'month', stmtDue: 'number',
    stmtRef: 'str', storeKey: 'str', source: 'str', importBatch: 'str', importedAt: 'str', ledger: ['card', 'cashflow'], bankRef: 'str', dir: ['in', 'out'], autoNote: 'str' },   // bankRef＝銀行對帳單交易去重鍵；dir＝銀行交易金流方向（Codex r13#2）；autoNote＝預設自動顯示名（清空自訂說明時回復用，使用者定 2026-07-21）——皆服務層寫、非 CRUD 白名單   // 服務層擁有的欄位（非 CRUD）仍須驗型別（r2-Codex#8、r11）：數字型會讓 origFromStmtRef 的 .split／批次聚合走樣
  subscriptions: {
