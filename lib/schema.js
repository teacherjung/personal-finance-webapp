...
export const WRITABLE_FIELDS = {
  ...
  cards: ['name', 'type', 'issuer', 'issuerId', 'network', 'lastFour', 'level', 'memberId',
    'statementDay', 'dueDay', 'annualFee', 'expiry', 'benefits', 'note', 'pdfPassword'],   // issuerId＝發卡機構代號（2026-09-02）：卡片表單寫、身分判準讀（`cardIssuerBank`）；沒有它（或與 issuer 互相矛盾）就退回 issuer 字串那條路。⚠️ 兩欄是同一個身分的兩半，跨欄規則見下方 pickWritable
...
export function pickWritable(col, body) {
  ...
  //    ⚠️ **只做這一個方向**：反過來「只送 `issuerId` 不送 `issuer`」不補顯示名——那等於櫃檯替
  //    使用者編一段畫面文字。那個方向的安全由判準側收口（`codedIssuer`：兩欄矛盾就不採信代號），
  //    今天也沒有任何呼叫端只送代號。
