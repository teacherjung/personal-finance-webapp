...
export const FIELD_SCHEMA = {
  ...
  transactions: { amount: 'number', date: 'date', autoCat: 'str', autoSub: 'str', stmtMonth: 'month', stmtDue: 'number',
    stmtRef: 'str', storeKey: 'str', source: 'str', importBatch: 'str', importedAt: 'str', ledger: ['card', 'cashflow'], bankRef: 'str', dir: ['in', 'out'] },   // bankRef＝銀行對帳單交易去重鍵；dir＝銀行交易金流方向（服務層寫、非 CRUD 白名單，供「同類一起改」方向護欄，Codex r13#2）   // 服務層擁有的欄位（非 CRUD）仍須驗型別（r2-Codex#8、r11）：數字型會讓 origFromStmtRef 的 .split／批次聚合走樣
  subscriptions: {
