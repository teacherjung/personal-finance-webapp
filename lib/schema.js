...
export const FIELD_SCHEMA = {
  ...
  //     消費端（`issuerBank`／`issuerMatchesBank`／`bank-import.js` 的 `sameBank(String(c.issuer),…)`／
  //     卡片頁的 `esc(c.issuer)`／表單的 `issuerFormValues`），都先 `String()` 再用；有**行為考題**釘住的
  //     是 `issuerBank` 與 `issuerFormValues` 兩支（`test/card-issuers.test.js` 的容忍界線題），
