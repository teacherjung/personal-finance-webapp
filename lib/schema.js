...
export const FIELD_SCHEMA = {
  ...
  transactions: { amount: 'number', date: 'date', autoCat: 'str', autoSub: 'str', stmtMonth: 'month', stmtDue: 'number',
    stmtRef: 'str', storeKey: 'str', source: 'str', importBatch: 'str', importedAt: 'str', refundOf: 'str', ledger: ['card', 'cashflow'], bankRef: 'str', bankKey: 'str', dir: ['in', 'out'], autoNote: 'str', bankSummary: 'str', bankNote: 'str', bankBatch: 'str' },   // refundOf＝帳單匯入服務層寫的退款配對標記（P0 先存 null；不進 CRUD 白名單）；bankRef＝銀行對帳單交易去重鍵；bankKey＝銀行學習鑰匙（Codex r14#4：漏驗型別→物件/陣列 bankKey 能穿過匯入牆、被 String() 成 "[object Object]" 而多筆撞成同鑰匙、「同類一起改」誤改別筆）；dir＝金流方向（Codex r13#2）；autoNote＝預設自動顯示名（清空自訂說明時回復用，使用者定 2026-07-21）；bankSummary/bankNote＝帳單原文留底（Stage 2：摘要與備註各自存，非字串會讓顯示層 bankRawText 拿錯東西當原文）——皆服務層寫、非 CRUD 白名單   // 服務層擁有的欄位（非 CRUD）仍須驗型別（r2-Codex#8、r11）：數字型會讓 origFromStmtRef 的 .split／批次聚合走樣
  subscriptions: {
  ...
  cards: { statementDay: 'number', dueDay: 'number', annualFee: 'number', expiry: 'str', issuer: 'str', type: ['credit', 'membership', 'debit'] },   // debit＝簽帳金融卡（Stage 5b）：刷卡消費明細記到它的卡片帳本、沒有結帳／繳款；銀行匯入可自動建   // expiry 只驗「須為字串」（格式寬鬆相容舊資料；數字會讓 cards 頁 .slice 崩，Codex#10-1）
  // issuer 同款只驗「須為字串」（Codex #520 r3#2）：**刻意不加枚舉**——發卡行是「可選清單＋其他（自行輸入）」，
  // 封閉了清單以外的機構（會員卡的發卡商店、清單漏掉的銀行）就填不進去。但型別要驗：沒驗之前 CRUD 與備份匯入
  // 都放行 `issuer: {…}`，卡片表單再把它變成字串 `"[object Object]"` ⇒ 那是「三條會動到既有值的路徑」之外的第四條。
