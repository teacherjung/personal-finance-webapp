...
export const READONLY_COLLECTIONS = ['portfolioSnapshots', 'ibTrades', 'dailyValues', 'securityTrades', 'stockFundamentals'];   // securityTrades＝證券交易共同集合（S2）；stockFundamentals＝SEC 官方資料每租戶快取
...
export const LONG_TEXT_FIELDS = {
  transactions: ['note', 'stmtRef', 'autoNote', 'bankRef'],
  cards: ['benefits', 'note'],
  insurance: ['coverage'],
  watchlist: ['note'],
...
export const SETTINGS_FIELD_TYPES = {
  ...
  // 記住的帳單密碼池（P0.5，使用者 2026-08-11 拍板）：JSON.stringify(string[]) 存成**單一字串**——
  // 陣列逐元素加密走不通（AAD 用索引＝重排就解不開、塌成同路徑＝進 C6 撞號不救名單），
  // settings 固定路徑的字串＝機密四條路（加密/解密/匯出剝除/匯入不採用）全從 mapSecrets 自動接軌。
  // 空字串＝沒有記住任何密碼。寫入走服務層端點（記住/清除），不建議前端直寫。
  rememberedStatementPasswords: 'string'
};
