...
// 型別代碼：number｜boolean｜array｜str（必須是字串）｜date（YYYY-MM-DD 或空）｜month（YYYY-MM 或空）｜string[]（枚舉）。
...
/** @type {Record<string, Record<string, 'number'|'boolean'|'array'|'str'|'date'|'month'|'monthreq'|string[]>>} */
 ...
 * @param {'number'|'boolean'|'array'|'str'|'date'|'month'|'monthreq'|string[]|undefined} spec @param {any} v
...
function validateField(spec, v) {
  ...
  if (spec === 'date') {                                                // YYYY-MM-DD 或空（未設）；null 矯正為 ''（Codex#10-2：讀取端 .slice/.startsWith 遇 null 會崩）
    ...
    return (typeof v === 'string' && /^\d{4}-\d{2}-\d{2}$/.test(v)) ? { ok: true, value: v } : { ok: false, reject: true };
  }
  if (spec === 'month') {                                               // YYYY-MM 或空；null 矯正為 ''
    if (v === null || v === '') return { ok: true, value: '' };
    return (typeof v === 'string' && /^\d{4}-\d{2}$/.test(v)) ? { ok: true, value: v } : { ok: false, reject: true };
  }
...
export function sanitizeSettingsDeep(input) {
  ...
  // 自訂支出分類樹（settings.expenseTree）：物件 { 大類:string → 子類:string[] }。壞 key/value 剝除。
  if ('expenseTree' in out) {
    if (!isObj(out.expenseTree)) { bad.push('settings.expenseTree'); delete out.expenseTree; }
    else {
      /** @type {Record<string, any>} */
      const clean = {};
      for (const [k, v] of Object.entries(out.expenseTree)) {
        if (typeof k === 'string' && k.trim() && Array.isArray(v) && v.every(s => typeof s === 'string')) clean[k] = v;
        else bad.push('expenseTree.' + k);
      }
      out.expenseTree = clean;
    }
  }
  // 分類器別名：categoryAliases＝{string→string}；subAliases＝{string→{string→string}}。壞值剝除。
  if ('categoryAliases' in out) {
    if (!isObj(out.categoryAliases)) { bad.push('settings.categoryAliases'); delete out.categoryAliases; }
    else {
      /** @type {Record<string, any>} */
      const clean = {};
      for (const [k, v] of Object.entries(out.categoryAliases)) { if (typeof v === 'string') clean[k] = v; else bad.push('categoryAliases.' + k); }
      out.categoryAliases = clean;
    }
  }
  if ('subAliases' in out) {
    if (!isObj(out.subAliases)) { bad.push('settings.subAliases'); delete out.subAliases; }
    else {
      /** @type {Record<string, any>} */
      const clean = {};
      for (const [p, m] of Object.entries(out.subAliases)) {
        if (!isObj(m)) { bad.push('subAliases.' + p); continue; }
        /** @type {Record<string, any>} */
        const mm = {};
        for (const [k, v] of Object.entries(m)) { if (typeof v === 'string') mm[k] = v; else bad.push('subAliases.' + p + '.' + k); }
        clean[p] = mm;
      }
      out.subAliases = clean;
    }
  }
