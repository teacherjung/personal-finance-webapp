...
// 系統實際支援的幣別（public/modules/fx-rates.js 只認這四種：沒抓到匯率用預設值照算；表外幣別＝無法換算、不計入並標註——「丙」口徑）。
 ...
 * **清乾淨之後可能一項都不剩**——要不要用預設值補，由呼叫端決定（讀端補、寫端擋，見 sanitizeTransferSubs）。
 * @param {any} input @returns {{label:string, role?:'out'|'in'|'settle'}[]}
 */
export function cleanTransferSubs(input) {
...
/**
 * 讀端用：清乾淨之後一項都不剩就回預設（避免內轉沒有任何子分類可選）。
 * ⚠️ 寫端不要用這支——「使用者送來的清單清完是空的」與「他真的想要預設那三項」在這裡看起來一模一樣，
 * 用在寫端就會把「我全刪了」默默存成預設值還回報成功。寫端走 cleanTransferSubs 自己判空。
 * @param {any} input @returns {{label:string, role?:'out'|'in'|'settle'}[]}
 */
export function sanitizeTransferSubs(input) {
  const out = cleanTransferSubs(input);
...
// 錯型別若進了資料庫，fx-rates.js 的 positiveRate 會把它判成「沒抓到」而默默退到預設值（更早以前是 Number(s.usdTwd||32) 變 NaN 污染核心計算；Codex 高severity 實測）——櫃檯擋在前面才不會靜靜算錯。
...
export const SETTINGS_FIELD_TYPES = {
  // usdTwd：正數或 null（null＝明確清除＝改用自動抓到的／預設匯率；丙）
  currency: 'string', usdTwd: 'posnumopt', emergencyFundMonths: 'number', allocationDriftPct: 'number',
