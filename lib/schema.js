...
export const FIELD_SCHEMA = {
  ...
  accounts: { balance: 'number', currency: CURRENCIES, ibCashCur: CURRENCIES, balanceAsOf: 'date', bank: 'str', cdKey: 'str', type: ['cash', 'investment', 'property', 'insurance-cv', 'other', 'mortgage', 'loan', 'liability', 'creditcard'] },   // balanceAsOf＝餘額現值參考日（服務層寫、非 CRUD；銀行對帳單「較新才覆蓋」的依據，stage 2）；bank＝開戶機構戳（P1a 機構維度：matchAccount 憑它擋跨行誤配——服務層寫、非 CRUD，但備份匯入會保留、需擋非字串（truthy 錯型會永久硬擋正確比對、falsy 錯型繞過護欄，r1#3）；cdKey＝定存身分鍵（2026-08-18 分開列管：機構|末碼|幣別|起迄日|#序——存單號帳單不印、同值定存靠列印序分辨；服務層寫、非 CRUD；**有 cdKey＝這顆是一筆定存、不進 matchAccount 泛用比對**）
