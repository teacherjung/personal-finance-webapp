...
export function sanitizeSettingsDeep(input) {
  ...
  bad.push(...ruleBad);
  // 記住的帳單密碼池（P0.5 r1#2）：櫃檯寫入端也套上限（≤8 組×100 字），一個超大值進不了 db
...
export const SETTINGS_FIELD_TYPES = {
  ...
  //（池大小＝每次上傳的解析次數，r1#2）。改由 `pickRememberedStatementPasswords`（寫入端上限 8 組×100 字）
 ...
 * 字串**，而池大小＝每次上傳的解析嘗試次數，一個超大值就能讓上傳跑上千次重型 PDF 解析。改走這道**寫入端上限**
 * ＝解析成 ≤8 組、每組 ≤100 字的 JSON 字串再存（讀取端 `rememberedPasswords` 另有同款 fail-safe 上限）。
...
export function pickRememberedStatementPasswords(src, bad) {
  ...
  const clean = arr.filter((x) => typeof x === 'string' && x && x.length <= 100).slice(0, 8);
