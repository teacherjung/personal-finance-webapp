...
export const FIELD_SCHEMA = {
  ...
  parseRecipes: {
    ...
    bank: 'str', current: 'object', previous: 'object',
...
export function sanitizeSettingsDeep(input) {
  ...
  // 規則卡出生統計（2026-08-19）：**server-owned**（唯一寫入路徑＝`repo.updateRecipeBirthStats`，由
  // applyBankStatement 在配方生成之後呼叫）——不列進 SETTINGS_FIELD_TYPES ⇒ 前端 PUT /settings 自動丟掉（同 quotesLastAt
  ...
  if ('recipeBirthStats' in out) {
    ...
    else { if (out.recipeBirthStats != null) bad.push('settings.recipeBirthStats'); delete out.recipeBirthStats; }
  }
  if ('signals' in out) {
 ...
 *   ⚠️ allowIbSyncFields 同時也保留**其他 server-owned 欄位**（目前＝規則卡出生統計 recipeBirthStats）：
...
export function sanitizeSettings(input, opts = {}) {
  ...
  for (const [k, v] of Object.entries(input)) {
    ...
    if (k === 'recipeBirthStats' && allowIbSyncFields) continue;
