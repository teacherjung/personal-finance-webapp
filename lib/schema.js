...
export const FIELD_SCHEMA = {
  ...
  transactions: { amount: 'number', date: 'date', autoCat: 'str', autoSub: 'str', stmtMonth: 'month' },   // autoCat/autoSub/stmtMonth 由匯入服務層寫（非 CRUD），仍須驗型別（r2-Codex#8）
  subscriptions: {
...
export function sanitizeSettings(input, opts = {}) {
  ...
  for (const [k, v] of Object.entries(input)) {
    if (k === 'signals' || k === 'fxTwd' || k === 'ib' || k === 'expenseTree' || k === 'categoryAliases' || k === 'subAliases') continue;   // 巢狀/自訂分類，下面處理
...
export function sanitizeCategorySettings(src, bad) {
  ...
  if ('categoryAliases' in src) {
    if (!isObj(src.categoryAliases)) { bad?.push('settings.categoryAliases'); }
      ...
      /** @type {Record<string, any>} */
      const a = {};
      for (const [k, v] of Object.entries(src.categoryAliases)) { if (typeof v === 'string') a[k] = v; else bad?.push('categoryAliases.' + k); }
