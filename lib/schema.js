...
export const FIELD_SCHEMA = {
  ...
  insurance: { premium: 'number', cashValue: 'number' },
  cards: { statementDay: 'number', dueDay: 'number', annualFee: 'number' },
  history: { amount: 'number' },
  holdings: { quantity: 'number', price: 'number', avgCost: 'number', cost: 'number' },
  watchlist: { targetPrice: 'number', lastPrice: 'number' },
...
/**
 * 依欄位型別（FIELD_SCHEMA 的值）驗證/轉換一個值。未指定型別＝自由字串、原樣放行。
 * @param {'number'|'boolean'|'array'|string[]|undefined} spec @param {any} v @returns {{ok: boolean, value?: any}}
 */
function validateField(spec, v) {
  if (spec === undefined) return { ok: true, value: v };                 // 自由字串：放行
  if (Array.isArray(spec)) return (typeof v === 'string' && spec.includes(v)) ? { ok: true, value: v } : { ok: false };   // 枚舉
  if (spec === 'number') return coerceNum(v);
  if (spec === 'boolean') {                                              // 布林：擋 'false' 字串被當 truthy
    if (typeof v === 'boolean') return { ok: true, value: v };
    if (v === 'true') return { ok: true, value: true };
    if (v === 'false') return { ok: true, value: false };
    return { ok: false };
  }
  if (spec === 'array') return Array.isArray(v) ? { ok: true, value: v } : { ok: false };
...
/**
 * 匯入用：驗有型別的欄位（壞值剝掉→由 derive 的 ||0／預設接手），其餘欄位（含 id、字串）原樣保留。
 * 與 pickWritable 的差別：匯入是「還原完整物件」，要保留 id/stmtRef 等，不做白名單剝除。
 * 非物件的元素（[null] 之類）回傳 null，由呼叫端過濾掉（否則 buildSummary 讀 null.currency 會崩）。
 * @param {string} col @param {any} item @returns {any|null}
 */
