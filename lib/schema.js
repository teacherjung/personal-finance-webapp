...
export const READONLY_COLLECTIONS = ['portfolioSnapshots', 'ibTrades', 'dailyValues', 'securityTrades'];   // securityTrades＝證券交易共同集合（S2）：IB 同步與台新匯入服務寫、前端只 GET
...
export const FIELD_SCHEMA = {
  ...
  accounts: { balance: 'number', currency: CURRENCIES, ibCashCur: CURRENCIES, balanceAsOf: 'date', type: ['cash', 'investment', 'property', 'insurance-cv', 'other', 'mortgage', 'loan', 'liability', 'creditcard'] },   // balanceAsOf＝餘額現值參考日（服務層寫、非 CRUD；銀行對帳單「較新才覆蓋」的依據，stage 2）
  ...
  transactions: { amount: 'number', date: 'date', autoCat: 'str', autoSub: 'str', stmtMonth: 'month', stmtDue: 'number',
    stmtRef: 'str', storeKey: 'str', source: 'str', importBatch: 'str', importedAt: 'str', refundOf: 'str', ledger: ['card', 'cashflow'], bankRef: 'str', bankKey: 'str', dir: ['in', 'out'], autoNote: 'str' },   // refundOf＝帳單匯入服務層寫的退款配對標記（P0 先存 null；不進 CRUD 白名單）；bankRef＝銀行對帳單交易去重鍵；bankKey＝銀行學習鑰匙（Codex r14#4：漏驗型別→物件/陣列 bankKey 能穿過匯入牆、被 String() 成 "[object Object]" 而多筆撞成同鑰匙、「同類一起改」誤改別筆）；dir＝金流方向（Codex r13#2）；autoNote＝預設自動顯示名（清空自訂說明時回復用，使用者定 2026-07-21）——皆服務層寫、非 CRUD 白名單   // 服務層擁有的欄位（非 CRUD）仍須驗型別（r2-Codex#8、r11）：數字型會讓 origFromStmtRef 的 .split／批次聚合走樣
  subscriptions: {
 ...
 * @param {string[]} errors @param {{values?:string[], date?:boolean, monthOrDate?:boolean, url?:boolean, protoValue?:boolean}} [opts]
 */
...
function sanitizeScorecard(input, path, errors) {
  ...
  if ('reasons' in src) {
    ...
    if (reasons) {
      /** @type {Record<string, any>} */
      const clean = {};
      for (const key of RESEARCH_SCORE_KEYS) researchString(reasons, clean, key, `${path}.reasons`, errors);
      out.reasons = clean;
    }
  }
  return out;
...
function sanitizeValuationScenarios(input, path, errors) {
  ...
  for (const key of ['method', 'assumptions']) researchString(src, out, key, path, errors);
...
function sanitizeCheckpoint(item, path, errors) {
  ...
  researchString(src, out, 'note', path, errors);
  return out;
}
...
function sanitizeCatalyst(item, path, errors) {
  ...
  researchString(src, out, 'text', path, errors);
...
export function sanitizeDbForWrite(input, opts = {}) {
  ...
  for (const col of ALL_COLLECTIONS) {
    ...
    out[col].forEach((/** @type {any} */ item, /** @type {number} */ i) => {
      ...
      if (dropItem) return;
      if (col === 'research') {
        const deep = sanitizeResearchItem(o);
        o = deep.item;
...
export function sanitizeSettingsDeep(input) {
  ...
  for (const [k, kind] of Object.entries(SETTINGS_FIELD_TYPES)) {
    ...
    const ok = kind === 'number' ? isNum(v) : kind === 'posnum' ? (isNum(v) && v > 0)
      : kind === 'posnumopt' ? (v === null || (isNum(v) && v > 0))
...
/** @type {Record<string, 'number'|'posnum'|'posnumopt'|'string'|'manual'>} */
...
export function sanitizeSettings(input, opts = {}) {
  ...
  for (const [k, v] of Object.entries(input)) {
    if (k === 'signals' || k === 'fxTwd' || k === 'ib' || k === 'expenseTree' || k === 'incomeTree' || k === 'categoryAliases' || k === 'subAliases' || k === 'incomeCategoryAliases' || k === 'incomeSubAliases' || k === 'storeRules') continue;   // 巢狀/自訂分類/店名規則，下面處理
