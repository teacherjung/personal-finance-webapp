...
export const FIELD_SCHEMA = {
  ...
  transactions: { amount: 'number', date: 'date', autoCat: 'str', autoSub: 'str', stmtMonth: 'month', stmtDue: 'number',
    stmtRef: 'str', storeKey: 'str', source: 'str', importBatch: 'str', importedAt: 'str', ledger: ['card', 'cashflow'], bankRef: 'str' },   // bankRef＝銀行對帳單交易去重鍵（服務層寫，stage 3）   // 服務層擁有的欄位（非 CRUD）仍須驗型別（r2-Codex#8、r11）：數字型會讓 origFromStmtRef 的 .split／批次聚合走樣
  subscriptions: {
...
export function sanitizeSettingsDeep(input) {
  ...
  for (const f of ['expenseTree', 'incomeTree', 'categoryAliases', 'subAliases']) {
...
export function sanitizeSettings(input, opts = {}) {
  ...
  for (const [k, v] of Object.entries(input)) {
    if (k === 'signals' || k === 'fxTwd' || k === 'ib' || k === 'expenseTree' || k === 'incomeTree' || k === 'categoryAliases' || k === 'subAliases' || k === 'storeRules') continue;   // 巢狀/自訂分類/店名規則，下面處理
