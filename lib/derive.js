...
function subCostForMonth(sub, mk) {
  if (sub.cycle === 'lifetime') return 0;
  ...
  if (endMk === mk) return base * Math.min(Number(endsOn.slice(8, 10)) || 30, 30) / 30;
  return base;
}
...
export function computeLeverage(db, ib) {
  ...
  const hasLoan = negCash < 0 && ibValue > 0;
...
export function buildSummary(db) {
  ...
  return {
    ...
    ib: { totalValue: ib.totalValue, totalPnl: ib.totalPnl, count: ib.positions.length,
      leverage: lev.leverage, loan: lev.loan, mcDist: lev.mcDist, hasLoan: lev.hasLoan },
