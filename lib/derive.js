 ...
 * `liabilities`＝其中幾筆是負債（負餘額或負債型帳戶）——缺匯率的負債被略過會把負債**低估**、淨值**可能**高估
 *（資產也缺時方向推不出來；槓桿只在 IB 融資的外幣負現金缺匯率、且沒有官方淨值摘要而自算時才跟著被低估），
...
function computeReminders(db) {
  ...
  // 0) 缺匯率（乙）：外幣部位沒匯率＝不計入淨值、槓桿與配置——要讓人看見方向：資產缺＝淨值被低估（warn）、負債缺＝負債被低估、淨值可能被高估（danger），不是默默變小
  ...
  if (missingFx.length) {
    ...
    // 方向不同、嚴重度不同：缺匯率的**負債**被略過＝負債被低估、淨值**可能**被高估（資產也缺時方向推不出來——missingFx 只有筆數沒有金額）→ danger；
    // 只有資產缺匯率＝淨值被低估（保守方向）→ warn。槓桿只在「IB 融資的外幣負現金缺匯率、且沒有 lastEquity 官方摘要」時才跟著被低估，所以放在 detail 用條件句講。
    out.push(liab > 0
      ? { key: 'fx-missing', level: 'danger', module: '投資',
        title: `有 ${n} 筆外幣部位沒有匯率（${curs}），其中 ${liab} 筆是負債：負債被低估、淨值可能被高估`,
        detail: '生存優先：看不見的負債最危險（缺的若是 IB 融資的外幣負現金、且沒有 IB 官方淨值摘要而要自算槓桿時，槓桿也會被低估）。到投資頁按「更新報價」會自動抓匯率；在那之前不要把目前的淨值當真（自算槓桿時，槓桿也一樣）。' }
      : { key: 'fx-missing', level: 'warn', module: '投資',
        title: `有 ${n} 筆外幣資產沒有匯率（${curs}），未計入淨值（淨值被低估）`,
        detail: '到投資頁按「更新報價」會自動抓匯率；在那之前這些帳戶／持股不算進淨資產與配置（沒有 IB 官方淨值摘要而自算槓桿時，也不算進槓桿）。' });
  }
...
export function computeLeverage(db, ib) {
  ...
  if (eqIb && (Number(eqIb.stock) > 0 || Number(eqIb.cash) < 0)) {
    ...
    negCash = (db.accounts || []).filter(a => a.ibCashCur).reduce((sum, a) => {
      const f = fxFor(rates, a.currency);
      if (f.missing) return sum;   // 缺匯率＝不計入（乙）。這筆負現金在 computeAssets 會算成「缺匯率的負債」，提醒 fx-missing 升 danger；槓桿被低估只在這條自算路徑成立（有 lastEquity 時不經這裡），所以提醒用條件句講
