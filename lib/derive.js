...
function computeReminders(db) {
  ...
  if (negCash < 0 && ibValLev > 0) {
    ...
    // 上限任何時期一體適用（生存優先原則）：訊號期加碼只用新資金、不舉新債。
    // 斷頭距離：市場再跌 x% 觸及 IB 強平線，x = 1 − 借款 ÷ ((1−維持率) × 持倉市值)。
    const levCap = Number(s.levCapPct || 1.3);
    const maint = Number(s.ibMaintenancePct ?? 25) / 100;
    const mcDist = ibValLev > 0 ? Math.max(0, 1 - (-negCash) / ((1 - maint) * ibValLev)) * 100 : 100;
    if (lev >= 1.1) {
      ...
      out.push({ level: mcDist < 40 ? 'danger' : lev > levCap ? 'warn' : 'info', module: '投資',
        ...
        detail: (lev > levCap
          ...
          : '使用融資中：波動會被放大。')
          + `斷頭距離約 ${mcDist.toFixed(0)}%（市場再跌這麼多會觸及 IB 強平線，維持率 ${Math.round(maint * 100)}%）。` + intTxt });
    }
  }
