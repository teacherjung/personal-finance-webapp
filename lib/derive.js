 ...
 * 從視窗內第一個真的有收入／支出的月份起畫，之後的空月保留 0，避免把未開始記帳的月份偽裝成零收支。
 * @param {Db} db
...
export function computeCashflowHistory(db, currentMonth = monthKey(), limit = 12) {
  ...
  for (const t of db.transactions || []) {
    ...
    if (monthSet.has(mk) && (!firstRecorded || mk < firstRecorded)) firstRecorded = mk;
  }
  if (!firstRecorded) return [];
  return months.filter(mk => mk >= firstRecorded).map(mk => computeCashflow(db, mk));
}
