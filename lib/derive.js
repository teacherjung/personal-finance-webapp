...
/** 缺匯率累計器：`[{currency, count}]`，順序＝首次出現。 */
function missingFxTracker() {
  /** @type {Map<string, number>} */
  const m = new Map();
  return {
    add(/** @type {string} */ cur) { m.set(cur, (m.get(cur) || 0) + 1); },
    list() { return [...m].map(([currency, count]) => ({ currency, count })); }
  };
}
...
export function computeAssets(db) {
  ...
  for (const a of db.accounts || []) {
    ...
    if (f.missing) { missing.add(f.cur); continue; }   // 缺匯率＝這筆不計入（乙）；missingFx 讓畫面標註
  ...
  for (const h of db.holdings || []) {
    ...
    if (f.missing) { missing.add(f.cur); continue; }   // 缺匯率＝不計入（乙）
...
function computeReminders(db) {
  ...
  // 0) 缺匯率（乙）：外幣資產沒匯率＝不計入淨值、槓桿與配置——要讓人看見「少算了」，不是默默變小
  const missingFx = computeAssets(db).missingFx;
  if (missingFx.length) {
    out.push({ key: 'fx-missing', level: 'warn', module: '投資',
      title: `有 ${missingFx.reduce((n, m) => n + m.count, 0)} 筆外幣資產沒有匯率（${missingFx.map(m => m.currency).join('、')}），未計入淨值`,
...
export function computeIb(db) {
  ...
  const positions = (db.holdings || []).map(h => {
    const f = fxFor(rates, h.currency);   // 缺 currency 預設台幣（與帳戶端一致），避免台股被當美元灌 32 倍
    if (f.missing) missing.add(f.cur);
...
export function computeLeverage(db, ib) {
  ...
  if (eqIb && (Number(eqIb.stock) > 0 || Number(eqIb.cash) < 0)) {
    ...
    negCash = (db.accounts || []).filter(a => a.ibCashCur).reduce((sum, a) => {
      const f = fxFor(rates, a.currency);
      if (f.missing) return sum;   // 缺匯率＝不計入（乙）；提醒引擎的 fx-missing 會把「槓桿少算」講出來
