...
function computeReminders(db) {
  ...
  for (const sub of db.subscriptions || []) {
    ...
    if (chargeDays >= 0 && chargeDays <= 7 && sub.status !== 'ending' && !sub.endsOn) {
      ...
      out.push({ key: `sub-charge-overdue-${sub.id}`, level: 'warn', module: '訂閱',
  ...
  for (const p of db.insurance || []) {
    ...
    if (d >= 0 && d <= 30) {
      ...
      out.push({ key: `ins-pay-overdue-${p.id}`, level: 'danger', module: '保險',
  ...
  if (ib.totalValue > 0 && netWorth > 0) {
    ...
    for (const p of ib.positions) if (p.layer === 'stock') stockBySym[p.symbol] = (stockBySym[p.symbol] || 0) + p.marketValue;
