...
import { isCardPayment, origFromStmtRef } from './statement.js';
...
const compOf = (h) => COMPOSITION[normalizePortfolioSymbol(h.symbol)]
...
/**
 * 兩帳聯集的「消費視角」純計算積木。不寫 db，每次用當下全庫重配退款：
 * 先匯退款、後補舊月消費時就能自癒。退款只配「同卡＋同 storeKey＋同額＋較早」，一對一；
 * 部分退款或無法證明的退款寧可列為未對應，絕不猜測。
 * @param {Db|any} db
...
export function consumptionByMonth(db) {
  ...
  for (const purchase of purchases) {
    ...
    const storeKey = String(purchase.storeKey || '').trim();
  ...
  for (const refund of refunds) {
    ...
    const storeKey = String(refund.storeKey || '').trim();
    ...
    if (purchase) {
      paired.add(purchase);
      add(purchase, -Math.abs(Number(refund.amount)));
    } else {
      unmatchedRefunds.push({
        id: String(refund.id || ''), date: String(refund.date), amount: Math.abs(Number(refund.amount)),
        account: String(refund.account || ''), storeKey, store: String(refund.note || storeKey || '未命名退款'),
      });
    }
  }
...
export function buildMonthlyReview(db, requestedMonth = '', now = new Date()) {
  ...
  const selectedMonth = visibleKeys.includes(requestedMonth) ? requestedMonth : (visibleKeys.at(-1) || null);
...
// 過去六個月「信用卡帳本」平均消費（緊急預備金過渡期保險用；只算有卡消費的月份，比照 avgMonthlyExpense）。
