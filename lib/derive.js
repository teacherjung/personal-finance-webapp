...
export function computeReminders(db) {
  const out = [];
  const s = db.settings || {};
  ...
  // 2) 緊急預備金不足
  const { byClass } = computeAssets(db);
  ...
  // 6) 投資組合：單一個股佔投組超過上限（逐檔比對；個股需個別研究基本面）
  const ib = computeIb(db);
  if (ib.totalValue > 0) {
    const stockCap = Number(s.ibConcentrationPct || 5);
    for (const p of ib.positions) {
      if (p.layer === 'stock' && p.weight > stockCap) {
        out.push({ level: 'warn', module: '投資',
          title: `${p.symbol} ${p.weight.toFixed(1)}%（超過個股上限 ${stockCap}%）`,
          detail: '單一個股曝險超出上限，依紀律應考慮減碼或暫停加碼。' });
      }
    }
  }

  // 7) 融資槓桿：只看 IBKR（IB 持倉 ÷ IB 淨值，融資＝IB 現金帳戶負餘額）
  // 排除台新現金存款與台新證券的台股，避免灌大或稀釋槓桿。
  const ratesLev = fxRates(db);
  const ibValLev = ib.positions.filter(p => p.source === 'ib').reduce((sum, p) => sum + p.marketValue, 0);
  const negCash = (db.accounts || []).filter(a => a.ibCashCur).reduce((sum, a) => {
  ...
  if (negCash < 0 && ibValLev > 0) {
    ...
    if (lev >= 1.1) {
      ...
      out.push({ level: lev >= 1.6 ? 'warn' : 'info', module: '投資',
        title: `融資槓桿 ${lev.toFixed(2)}x（借款 ${fmt(-negCash)}）`,
        detail: (lev >= 1.6
          ? `槓桿偏高：市場下跌時淨值跌幅會放大約 ${lev.toFixed(1)} 倍，留意維持保證金。`
          : '使用融資中：波動會被放大。') + intTxt });
    }
  }

  // 8) 匯率：美元/台幣進入分批換匯區間
  const usdTwd = fxRates(db).USD;
...
export function computeAllocation(db) {
