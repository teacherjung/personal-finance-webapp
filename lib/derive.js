...
import { resolveFxTable, fxFor, fxUsageTracker } from '../public/modules/fx-rates.js';   // 匯率表唯一實作（丙）
...
// 匯率表＝public/modules/fx-rates.js 的唯一實作（丙：上次抓到的 → 預設值；不支援的幣別才不計入）。
/** @param {Db} db */
function fxRates(db) { return resolveFxTable(db.settings || {}); }
...
export function computeAssets(db) {
  ...
  for (const a of db.accounts || []) {
    ...
    if (Number(a.balance || 0) !== 0) usage.note(f, LIABILITY_TYPES.has(a.type || '') || Number(a.balance || 0) < 0);   // 有曝險才登記
    if (f.missing) continue;   // 不支援的幣別＝無法換算、不計入（丙③）；預設匯率的照常計入（丙②）
  ...
  for (const h of db.holdings || []) {
    ...
    if (f.missing) continue;   // 不支援的幣別才不計入（丙③）
  ...
  return { assets, liabilities, netWorth: assets - liabilities, byClass, defaultFx: usage.defaultFx(), missingFx: usage.missingFx() };
}
...
function computeReminders(db) {
  ...
  // 0) 匯率（丙）：用了預設匯率＝照常計入、只提示（info；誤差通常不大）；不支援的幣別＝無法換算、不計入（warn，
  //    其中有負債時 danger：負債被低估、淨值可能被高估）。兩者都要讓人看見，不是默默算。
  const { defaultFx, missingFx } = computeAssets(db);
  if (defaultFx.length) {
    const n = defaultFx.reduce((sum, m) => sum + m.count, 0);
    const list = defaultFx.map(m => `${m.currency} ${m.rate}`).join('、');
    out.push({ key: 'fx-default', level: 'info', module: '投資',
      title: `有 ${n} 筆外幣部位用的是預設匯率（${list}）`,
      detail: '還沒抓到即時匯率：開 app 會自動抓（Yahoo，抓不到再走備援 API），投資頁「更新報價」可再試（十分鐘內會沿用剛抓到的值）。匯率波動通常不大，先用預設值算的誤差可接受。' });
  }
  if (missingFx.length) {
    ...
    out.push(liab > 0
      ? { key: 'fx-missing', level: 'danger', module: '投資',
        title: `有 ${n} 筆部位的幣別（${curs}）系統不支援、無法換算，其中 ${liab} 筆是負債：負債被低估、淨值可能被高估`,
        detail: '看不見的負債最危險。請把該帳戶／持股改成系統支援的幣別（TWD／USD／GBP／JPY）。' }
      : { key: 'fx-missing', level: 'warn', module: '投資',
        title: `有 ${n} 筆部位的幣別（${curs}）系統不支援、無法換算，未計入淨值`,
        detail: '請把該帳戶／持股改成系統支援的幣別（TWD／USD／GBP／JPY）。' });
  }
  ...
  if (loan > 0 && !Number.isFinite(lev)) {
    ...
    if (lev >= 1.1) {
      ...
      const intTxt = inc && inc.interestPaid ? `近一年融資利息 ${fmt(Math.abs(inc.interestPaid) * ratesLev.rates.USD)}。` : '';
  ...
  const idleUsd = (db.accounts || []).filter(a => a.ibCashCur && Number(a.balance) > 0)
    .reduce((sum, a) => { const f = fxFor(ratesLev, a.currency || 'USD'); return f.missing ? sum : sum + Number(a.balance) * f.rate; }, 0) / ratesLev.rates.USD;
  ...
  // 8) 匯率：美元/台幣進入分批換匯區間
  const usdTwd = ratesLev.rates.USD;   // 沿用規則 7 已算好的匯率表
  // 預設值不是市場匯率（Grok #556）：沒抓到美元匯率時，預設 32 會剛好撞上 fxHigh 預設 32 而假報「已達高點」——只有抓到的才判換匯區間
  const usdIsLive = ratesLev.sources.USD === 'live';
  const fxHigh = Number(s.fxHigh || 32), fxLow = Number(s.fxLow || 28);
  if (!usdIsLive) { /* 用預設值＝不判換匯區間（fx-default 提醒會另外講） */ } else if (usdTwd >= fxHigh) {
...
export function computeIb(db) {
  ...
  const positions = (db.holdings || []).map(h => {
    const f = fxFor(rates, h.currency);   // 缺 currency 預設台幣（與帳戶端一致），避免台股被當美元灌 32 倍
    if (Number(h.quantity || 0) !== 0) usage.note(f);   // 零股數不算曝險
    const fx = f.missing ? 0 : f.rate;    // 不支援的幣別＝市值／成本記 0 並帶 fxMissing（自然不進總數；丙③）；預設匯率照算（丙②）
    const marketValue = Number(h.price || 0) * Number(h.quantity || 0) * fx;
    const costBasis = holdingCost(h) * fx;
    return { ...h, marketValue, costBasis, unrealizedPnl: marketValue - costBasis, weight: 0, pnlPct: 0, fxMissing: f.missing, fxSource: f.source };
  });
  ...
  return {
    ...
    defaultFx: usage.defaultFx(), missingFx: usage.missingFx()
  };
}
...
export function computeLeverage(db, ib) {
  ...
  if (eqIb && (Number(eqIb.stock) > 0 || Number(eqIb.cash) < 0)) {
    ibValue = Number(eqIb.stock) * rates.rates.USD;
    negCash = Math.min(Number(eqIb.cash) || 0, 0) * rates.rates.USD;
  } else {
    ...
    negCash = (db.accounts || []).filter(a => a.ibCashCur).reduce((sum, a) => {
      const f = fxFor(rates, a.currency);
      if (f.missing) return sum;   // 不支援的幣別才不計入（丙③）；預設匯率照算（丙②）
...
export function buildSummary(db) {
  ...
  return {
    ...
    defaultFx: assets.defaultFx,   // 用了預設匯率的筆數（丙②）：總覽就地標註用
    missingFx: assets.missingFx,   // 不支援的幣別、未計入的筆數（丙③）
