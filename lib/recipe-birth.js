// @ts-check
/**
 * 版面規則卡的**出生紀錄**（2026-08-19）：規則卡是「同版面第二次起可以不花 AI」那條省錢路的指望，
 * 而它的出生會在七個關卡的任一關失敗。這份統計就是那些關卡的**證據來源**——沒有它，「該放寬哪一關」
 * 只能用猜的。
 *
 * ⚠️ 界線：本模組**只記錄與顯示，不參與判準**。統計數字不得回頭影響出生流程或解析結果。
 *
...
/** 真實日曆檢查（`2026-99-99`／`2026-02-31` 都要擋掉——外形對不代表是日期）。
 * ⚠️ 刻意**不** import schema.js 的 isRealDate：schema 反過來 import 本檔（消毒器），會成循環。 */
...
export function recordBirth(stats, code, bank, today) {
  ...
  out[key] = {
    ...
    lastAt: isRealYmd(today) ? String(today) : prev.lastAt,
...
export function sanitizeBirthStats(stats) {
  ...
  for (const k of BIRTH_CODES) {
    ...
    out[k] = {
      ...
      lastAt: isRealYmd(v.lastAt) ? String(v.lastAt) : '',
