...
// 資料檔路徑可用環境變數覆寫（B0：測試用隔離的暫存資料檔跑、絕不碰真實 store.json）。
// 平常不設 STORE_FILE ＝行為與以往完全相同。
const FILE = process.env.STORE_FILE || join(DATA_DIR, 'store.json');
...
function ensure() {
  ...
  if (!existsSync(FILE)) {
    if (existsSync(SEED)) copyFileSync(SEED, FILE);   // 首次啟動（含測試的暫存檔）從 seed 複製
...
export function load() {
  ...
  try {
    ...
    // fail closed：檔案損毀時絕不回空資料庫——否則下一次 save() 會把壞檔備份成 .bak、再用空資料覆蓋正本（資料全滅）。
    throw new Error(`data/store.json 讀取失敗（檔案可能損毀）：${e.message}。請先不要進行任何操作，到 data/ 資料夾把 store.json.bak 改名為 store.json（還原上一次的備份）後重啟伺服器。`, { cause: e });
  }
}
...
export function save(db) {
  ...
  // 寫入前先備份上一版，避免資料損毀
  if (existsSync(FILE)) {
    try { copyFileSync(FILE, FILE + '.bak'); }
    catch (e) { console.warn('[store] 備份 .bak 失敗（仍會寫入 store.json）:', /** @type {any} */ (e).message); }
  }
  // 原子寫入：先寫暫存檔、再 rename 覆蓋（同檔案系統上 rename 是原子操作）。
  // 避免直接 writeFileSync 覆蓋時被強制關機/斷電/硬碟滿，把 store.json 留成半截壞檔。
  const tmp = FILE + '.tmp';
