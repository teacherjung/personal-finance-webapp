...
/** kv 的鍵＝settings、learnedCategories、learnedBank（銀行收支學習）、與所有集合名。 */
const KV_KEYS = ['settings', 'learnedCategories', 'learnedBank', 'transferSubs', ...ALL_COLLECTIONS];
/** 這些 kv 鍵是「物件（map）」，缺列時預設 {}；其餘（集合）預設 []。 */
const KV_MAP_KEYS = new Set(['settings', 'learnedCategories', 'learnedBank']);
