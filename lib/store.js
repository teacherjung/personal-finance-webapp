...
/** 把整包資料寫進 kv（單一交易＝全有或全無）。 @param {import('node:sqlite').DatabaseSync} d @param {Record<string, any>} obj */
function writeAll(d, obj) {
...
/** 首次啟動／舊版搬家：store.json 比資料庫新（含「還沒搬過」）就整包匯入（strip 模式清理＋警告）。 */
...
function migrateIfNeeded(d) {
  ...
  const meta = /** @type {any} */ (d.prepare("SELECT data FROM kv WHERE key='__migratedFromJsonAt'").get());
  const migratedAt = meta ? Number(JSON.parse(meta.data)) : 0;
  if (LEGACY && existsSync(LEGACY)) {
    const mtime = statSync(LEGACY).mtimeMs;
    // mtime 較新＝上次搬家後舊版程式又寫過 store.json（例如尚未重啟就繼續記帳）→ 以 json 為準重搬，不遺失資料
    if (!has || mtime > migratedAt) {
      ...
      const clean = sanitizeDbForWrite({ ...emptyDb(), ...parsed }, { mode: 'strip' });   // 舊資料用剝除＋警告，不卡死
      writeAll(d, clean);
      d.prepare('INSERT INTO kv(key,data) VALUES(?,?) ON CONFLICT(key) DO UPDATE SET data=excluded.data')
        .run('__migratedFromJsonAt', JSON.stringify(Date.now()));
      console.log(`[store] 已把 store.json 搬進 SQLite（store.db）。原 store.json 保留不動、當作備份。`);
      return;
    }
  }
  if (!has) {
    const seedDb = existsSync(SEED) ? { ...emptyDb(), ...JSON.parse(readFileSync(SEED, 'utf8')) } : emptyDb();
...
function backupOnce(d) {
  ...
  } catch (e) { console.warn('[store] 啟動備份 .bak 失敗（不影響使用）:', /** @type {any} */ (e).message); }
}
