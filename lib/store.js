 ...
 * ⚠️ **沒有任何正式操作路徑呼叫它**（William 2026-08-08 裁決拿掉「不可逆操作前自動備份」那一層，
 * 完整理由見 `lib/services/backup.js` 檔中的裁決註解）：`lib/repo.js` 仍照櫃檯慣例轉供一支同名函式，
 * 但那支自己也沒有呼叫端。保留低階實作是因為它本身沒有錯、將來若有需要可再用；
 * 但**要用之前先去讀那段裁決**，不要以為是誰忘了接。
...
/**
 * 把目前資料庫做成**一致的完整快照**到任意路徑：`VACUUM INTO` 到 `.tmp` → 成功才原子改名。
 * 三種備份（啟動 `.bak`／操作前 `<tag>.bak`／每日滾動）共用這一份實作＝寫法只有一種、不會走散。
