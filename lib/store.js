...
import { sanitizeDbForWrite, ALL_COLLECTIONS, validateImportItem } from './schema.js';
 ...
 * **C4b 起 export**：HOSTED 的 Postgres adapter（`lib/store-pg.js`）必須用**同一組鍵**，
 * 各抄一份就是兩份會走散的真相（新增鍵時只改一邊＝那個鍵在雲端版永遠寫不進去，且同樣不報錯）。 */
export const KV_KEYS = ['settings', 'learnedCategories', 'learnedBank', 'transferSubs', 'insightState', ...ALL_COLLECTIONS];
/** 這些 kv 鍵是「物件（map）」，缺列時預設 {}；其餘（集合）預設 []。 */
export const KV_MAP_KEYS = new Set(['settings', 'learnedCategories', 'learnedBank', 'insightState']);
...
function open() {
  ...
  try {
    ...
    migrateSecTradesContractIfNeeded(db);   // 證券合約收緊（Codex S3r2#4）配套：違約舊列一次性清掃
  } catch (e) {
    try { db.close(); } catch { /* 關閉失敗不影響重置 */ }
    db = null;
...
/**
 * securityTrades 合約收緊一次性清掃（Codex S3r2#4 配套，2026-07-24）：r2 把三個核心金額列必填、
 * buy→out／sell→in 進櫃檯 throw——但 strip 清理只在 JSON 搬家時跑，SQLite 日常載入不會。若庫裡
 * 躺著收緊前寫入的違約列（#242→#243 之間的 IB 同步理論上可能產出缺核心金額列），每次 saveDb 都會
 * 在櫃檯炸掉＝整個 app 寫不了。照 ledger 搬家前例：啟動時一次性**大聲警告＋整筆濾除**。
 * 判準＝**逐列跑一次真櫃檯（throw 模式）**——與未來每次 saveDb 的門完全同一套、零漂移空間
 *（對抗驗證抓到的教訓：validateImportItem 對「必填數字欄存了非數字」只默默剝欄不記 errors，
 * 拿它當判準會讓這種列逃過清掃、之後在任意一次無關寫入被櫃檯整列濾除——沒備份、沒紀錄）。
 * IB 列下次同步自動補回（同步窗內）；台新列重傳同一份對帳單即可。
 * 動手前先備份 pre-sec-contract.bak（不可逆整批操作慣例）。
 * @param {import('node:sqlite').DatabaseSync} d
 */
function migrateSecTradesContractIfNeeded(d) {
  if (metaNum(d, '__secTradesContractAt')) return;
  const row = /** @type {any} */ (d.prepare('SELECT data FROM kv WHERE key=?').get('securityTrades'));
  const rows = row ? JSON.parse(row.data) : [];
  /** 這一列將來過 throw 模式櫃檯會不會活著留下？（throw＝炸出、必填數字欄格式壞＝整列濾除，都算死） */
  const survivesCounter = (/** @type {any} */ t) => {
    try { return sanitizeDbForWrite({ settings: {}, securityTrades: [t] }, { mode: 'throw' }).securityTrades.length === 1; }
    catch { return false; }
  };
  const keep = [];
  /** @type {string[]} */
  ...
  for (const t of Array.isArray(rows) ? rows : []) {
    if (survivesCounter(t)) keep.push(t);
    else {
      const why = /** @type {any} */ (validateImportItem('securityTrades', t)).errors.join('、');   // 病因盡量說人話（僅供訊息，判準在上面）
      droppedWhy.push(`${t?.symbol || '?'} ${t?.tradeDate || ''}（${why || '欄位格式不合約'}）`);
    }
  }
  if (droppedWhy.length) {
    const bak = `${FILE}.pre-sec-contract.bak`, tmp = bak + '.tmp';
    try {
      if (existsSync(tmp)) rmSync(tmp);
      d.exec(`VACUUM INTO '${tmp.replace(/'/g, "''")}'`);
      renameSync(tmp, bak);
    } catch (e) {
      console.warn('[store] pre-sec-contract 備份失敗（啟動 .bak 仍含清掃前狀態）:', /** @type {any} */ (e).message);
      try { rmSync(tmp); } catch { /* tmp 可能不存在 */ }
    }
  }
  const up = d.prepare('INSERT INTO kv(key,data) VALUES(?,?) ON CONFLICT(key) DO UPDATE SET data=excluded.data');
  d.exec('BEGIN IMMEDIATE');
  try {
    if (droppedWhy.length) up.run('securityTrades', JSON.stringify(keep));
    up.run('__secTradesContractAt', JSON.stringify(Date.now()));
    d.exec('COMMIT');
  } catch (e) {
    d.exec('ROLLBACK');
    throw e;
  }
  if (droppedWhy.length) console.warn(`[store] 證券交易合約收緊：濾除 ${droppedWhy.length} 筆違約舊列——${droppedWhy.join('；')}。IB 列下次同步自動補回、台新列重傳對帳單即可；清掃前狀態已備份 store.db.pre-sec-contract.bak`);
}
...
/**
 * 「動手之前先存一份」：產生 `store.db.<tag>.bak`。
 * **與啟動備份 `.bak` 是不同的檔**——啟動備份每個行程只寫一次，對「一天內做了好幾次整批操作」
 * 沒有保護力，所以那一顆不能當成「操作前的安全帶」拿去對使用者宣稱。
 * 每個 tag 一顆、重複執行就覆蓋（檔案數量有上限，不會愈積愈多）。
 * best-effort（同啟動備份的設計決策）：**本函式自己**不擋任何事，失敗只警告並回 false。
 * ⚠️ **把它接上任何操作路徑之前，先讀 `lib/services/backup.js` 的設計註解**：不可逆操作前的
 * 自動備份是本專案刻意不做的產品決定（`test/vault-and-backup-integrity.test.js` 的〈裁決〉那一題
 * 釘著幾條指定路徑不得呼叫），不是誰忘了接。`backupNow` 這個低階函式本身沒有錯，留著是為了真的需要時不必重寫。
...
export function backupNow(tag) {
  ...
  const bak = `${FILE}.${safe || 'manual'}.bak`;
  try {
    snapshotTo(bak);
    return true;
  } catch (e) {
...
/**
 * 把資料庫**呼叫當下的內容**做成一致的完整快照到任意路徑：`VACUUM INTO` 到 `.tmp` → 成功才原子改名。
 * ⚠️ **這不是本檔唯一一份 VACUUM→rename**（`backupOnce` 與搬家函式各有自己的一份，劃界見
 * test/vault-and-backup-integrity.test.js 的〈備份的原子替換與殘骸清理〉那一節）：
 * 新的備份路徑請呼叫 `snapshotTo`，但別寫「大家都共用它」——改壞其中一支，守著另一支的題不會紅。
 * ⚠️ 與 `backupNow` 的差別：**失敗會丟例外**（呼叫端要據實回報，例如每日備份要記失敗次數並在畫面警告）；
 * 失敗時一定清掉半成品 `.tmp`，不留下會被誤認成備份的殘骸。
 * @param {string} destPath 目的檔完整路徑（呼叫端負責確保資料夾存在與檔名安全）
...
export function snapshotTo(destPath) {
  ...
  try {
    ...
    if (existsSync(tmp)) rmSync(tmp);
    d.exec(`VACUUM INTO '${tmp.replace(/'/g, "''")}'`);
    renameSync(tmp, dest);   // 原子替換：VACUUM 中途失敗（硬碟滿）時，上一顆備份仍完好
    return dest;
  } catch (e) {
    try { rmSync(tmp); } catch { /* tmp 可能不存在 */ }
    throw e;
...
/** 資料檔所在資料夾（每日備份服務要在它底下開 `backups/`；只有 store.js 知道 FILE 在哪）。 @returns {string} */
export function dataDir() { return dirname(FILE); }

export function emptyDb() {
  return {
    settings: {
      currency: 'TWD',
      // usdTwd 刻意不預填（丙）：沒抓到＝fx-rates.js 的預設值 32 照算並標「用了預設值」；種子塞 32 會把它冒充成「抓到的」
      ...
      netWorthTarget: null,          // 淨資產目標（台幣元；前端以萬元輸入，null＝尚未設定）
      taishinSecPdfPassword: '',     // 台新證券對帳單 PDF 密碼（比照 pdfPassword：機密投影、絕不送瀏覽器；''＝未設定）
      rememberedStatementPasswords: '',   // 記住的帳單密碼池（P0.5）：JSON.stringify(string[]) 存單一字串（AAD 穩定）；機密投影剝除；''＝沒記住
      aiAskBeforeSend: false,        // 送 AI 前先問？（William 2026-08-13：預設不問；設定頁可打開）
      aiDualRead: true,              // 新版式雙讀（裁示⑦ 2026-08-16：預設開；只有明確 false 才關＝dualReadWanted）
      aiApiKey: '',                  // AI 解析鑰匙（P1b-1，★3 拍板＝Anthropic）：比照 flexToken（LOCAL 明文／HOSTED 加密／投影剝除只回 aiApiKeySet）；''＝未設定
      ...
      fxTwd: {},                     // 其他幣別兌台幣：沒抓到＝用 fx-rates.js 的預設值照算並標註（丙）；開 app／「更新報價」會自動填 GBP/JPY
    ...
    stockFundamentals: [], // SEC 官方基本面每租戶快取（服務專寫；與使用者研究內容分開）
    parseRecipes: [],    // 帳單配方快取（P2-2）：AI 讀懂一次後的版面規則卡；bank-import 服務專寫
    ...
    ibTrades: [],
    securityTrades: [],            // 證券交易共同集合（S2）：IB 同步雙寫＋台新證券對帳單匯入；服務層寫、前端唯讀        // IBKR 成交紀錄（同步時整批更新；交易摘要與 XIRR 已實現損益修正使用中）
