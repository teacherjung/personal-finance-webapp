 ...
 * ⚠️ 但**呼叫端不可以把那個 false 丟掉**——2026-08-06 之前兩個呼叫點都丟掉了，結果備份失敗時
 * 使用者畫面上仍然只看到「儲存成功」。閘門的唯一實作在 `lib/services/backup.js`
 * 的 `backupBeforeIrreversible(tag)`（擋下＋讓使用者確認繼續），新的不可逆操作一律走它、不要直接叫這支。
