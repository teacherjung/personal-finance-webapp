...
```bash
node scripts/review-loop.js review     <branch> [--pr N]                    # ①／⑤
node scripts/review-loop.js review-fix <branch> --proposal <檔案> [--pr N]  # ③
node scripts/review-loop.js status     <branch>
```

它只做三件事：**用一致的提示詞叫 Codex**、**把產出存檔到 `.codex-reviews/`**、
**檢查該有的東西在不在**。不會自動改程式、不會自動合併、不會替任何人做判斷。

模型固定 `gpt-5.6-sol` ＋ `xhigh`＝**照 William 2026-07-31 裁決**（審查一律 xhigh，AI 不可自行降級；
機械執行動作才用一般設定）。這支工具做的都是審查，所以固定 xhigh 是對的、不是漏了分級。

兩道守門（都實測過）：
- 提案檔開頭沒標作者 → 擋下（不變量檢查不了）
- 提案是 **Codex** 寫的卻要叫 Codex 審 → 擋下（那就是自己審自己；反方向的 ③ 由 Claude 直接做）
- 跑完自動檢查審查 worktree 乾不乾淨——**審查者留下改動就是違反角色**，要當成一個發現
