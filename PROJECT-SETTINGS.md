...
- 家族網的動詞：create、place、submit、send、stage、preview、prepare、draft、amend、modify、edit、update、cancel、delete、execute、close、open、buy、sell、purchase、exercise、liquidate、replace、redeem、pay；名詞：order、trade、position、instruction、stock、share、security、securities、etf、option、future、bond、asset、fund、crypto、coin、locate、invoice、bill；唯讀前綴（豁免到哪一道、代價與射程＝`tools/forbidden-tools.js` 的「兩張樣式表」那一段，本檔不重述）：get、list、search、fetch、read、query、view、show、describe、has、check、retrieve、export、download
- 額外樣式：2 條；另一張 patternsReadSafe：4 條（兩張的分工＝見「兩張樣式表」；樣式本身不抄進來，正本在 settings.json）
