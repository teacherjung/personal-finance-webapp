...
- 家族網的動詞：create、place、submit、send、stage、preview、prepare、draft、amend、modify、edit、update、cancel、delete、execute、close、open、buy、sell、purchase、exercise、liquidate、replace、redeem、pay；名詞：order、trade、position、instruction、stock、share、security、securities、etf、option、future、bond、asset、fund、crypto、coin、locate、invoice、bill；唯讀前綴（**只豁免 `patterns` 那一張額外樣式表**，不豁免動詞名詞那兩道、也不豁免 `patternsReadSafe`；2026-09-24 兩次收緊）：get、list、search、fetch、read、query、view、show、describe、has、check、retrieve、export、download
- 額外樣式（**只在沒有唯讀前綴時生效**）：2 條；連唯讀前綴也擋的額外樣式：4 條（樣式本身不抄進來，正本在 settings.json）
- 額外樣式：\(\^\|\_\)\(transfer\|withdraw\(al\)\?\|deposit\|remit\(tance\)\?\|payout\|disburse\(ment\)\?\|payment\|wire\)s\?\(\_\|\$\)、\(\^\|\_\)\(convert\|exchange\|swap\)\_\\w\*\?\(currenc\|fund\|money\|cash\|asset\|crypto\|coin\|stock\|share\|securit\)
