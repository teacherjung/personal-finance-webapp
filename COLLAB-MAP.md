...
>   表頭與分隔列照抄、不放註腳、不放 HTML 註解。**但這是給人看的寫作約定，大半不是考題守得住的**——
>   **哪幾種會紅、哪幾種不會（都是實測過的例子，不宣稱列完），逐條在 `test/collab-map.test.js` 檔頭；
>   本檔不複述**（複述就是第二份會漂的複本，那正是本檔存在的理由）。
>   ⚠️ 唯一在這裡留一句的：**全檔不准有 HTML 註解**——那由另一題守著，**不在上面那份檔頭列舉裡**，
>   只照指路走會漏掉（Grok #587 掃後點名）。
...
- **有些地方有機器盯著，有些沒有。** 哪些有＝以 [test/collab-map.test.js](test/collab-map.test.js)
  ...
  閘表跟**合併步驟裡登記的那一組**對不對得上。⚠️ **只保證「某處找得到」**——不保證它還在原本那一章底下，
  也不保證階層或語意關係。
- **那道檢查是行級的，看不懂 Markdown 結構**（William 2026-09-02 裁定的射程）：
  它防的是**自然的腐爛**（檔案搬家、節改名、加了新閘沒登記），**不是**有人刻意把地圖寫成怪格式。
  **實測過的例子（哪幾發綠、哪幾發紅；不宣稱列完）**在 [test/collab-map.test.js](test/collab-map.test.js) 檔頭。
- **驗不到什麼，同一份檔頭逐條列著**（本檔不複述）。其中最該記住的一條：
  **「這張表有沒有偷抄規則判準」機器讀不出來——那條紀律只有人在守。**
...
| 怎麼問 William、他沒回怎麼辦（留痕怎麼貼；⚠️ 2026-09-10 起沒有時限＝等他） | [AGENTS.md](AGENTS.md)「問法與逾時預設」 |
...
這張表＝**合併步驟裡以標準指令登記的那幾支**（從 `REVIEW-AND-MERGE.md` 的「合併也由 Codex 代執行」
...
⚠️ 另有一道兜底，防「自報自己是閘、卻沒登記進合併步驟」的東西留在 `scripts/` 裡。
**那一道 2026-09-09 搬去 `test/collab-invariant-docs.test.js` 了**（它守的是「自報的閘有沒有登記」，
跟路由無關，不該跟著這張表走）。
**兩者認得到什麼、認不到什麼（副檔名、指令形狀、只認哪個名字、哪種閘會完全隱形），
逐條寫在 `test/collab-invariant-docs.test.js` 的兜底那一題旁邊——這裡刻意不複述**：
...
Grok 複審後掃有沒有做、掃描與複審的先後時序、審查提示詞有沒有涵蓋固定維度、問 William 的待裁留言有沒有貼——
漏做不會有東西擋你，全靠自律。
⚠️ 2026-09-10 之前這一行還列了「逾時暫定留言有沒有貼、時限有沒有數到」：**新問題**沒有時限可數了，⏳ 也**規定**不再產生新的（⚠️ 那是**規則在保證，不是機器擋得住**——那支工具沒有在驗日期；Grok #595 複審後掃抓到這裡把它寫成事實）。**舊的 ⏳ 仍要核對**（時間差照它自己內文寫的算；`REVIEW-AND-MERGE.md` 也仍要求審查者核對逾時判定）。正本＝AGENTS「問法與逾時預設」。
