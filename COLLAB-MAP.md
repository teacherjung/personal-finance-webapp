...
>   下面「我想查⋯⋯」與「合併步驟專用的閘」兩區採**固定窄版面**：一格一個連結、
>   表頭與分隔列照抄、不放註腳、不放 HTML 註解。**要改版面，就要同步改**
>   [test/collab-map.test.js](test/collab-map.test.js) 的文法——這是刻意的摩擦，不是 bug。
...
- **那道檢查是行級的，看不懂 Markdown 結構**（William 2026-09-02 裁定的射程）：
  例如把閘表包進程式碼區塊、拿掉表頭、在儲存格裡插一個沒跳脫的 `|`——它都看不出來。
  它防的是**自然的腐爛**（檔案搬家、節改名、加了新閘沒登記），不是有人刻意把地圖寫成怪格式。
  **實測過的完整清單（哪幾發綠、哪幾發紅）**在 [test/collab-map.test.js](test/collab-map.test.js) 檔頭。
