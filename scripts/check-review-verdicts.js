...
// `docs/github-branch-protection-setup.md` 的「第二步：分身分」）。
// 它防的是**混淆與遺漏**，不是惡意。
//
// 退出碼：
 ...
 *   一律不在名單上），sha 長相的字只准一個——出界的壞留言**不可重述**，
 *   改走同身分豁免（`#543`：資格判定已把這一類導過去；在那之前只能關掉重開 PR 或問 William）。
 *   名單放寬要連著這裡與考題一起改。
 *   ⚠️ **反引號配對是對「整條重述行」的要求，不是壞行的性質**（`Codex #543 r1` 糾正）：
 *   RESTATE 自報 sha 的左右反引號各自 optional ⇒ 重述者永遠湊得出偶數 ⇒ 壞行的反引號奇偶
 *   **不構成**「不可重述」，也因此不進豁免資格判定。
 ...
 *   最後一級救濟不能因為守則而搆不到）：「原第一行雜湊：<該行去頭尾空白後的 SHA-256>」＝精確指認、
 ...
 * ・⚠️ **資格分類的停戰劃界**（William 2026-08-17 裁定；#477 豁免資格家族三輪 High 後）：
 *   資格分類對 Unicode 變體（VS15／VS16、全形字母、Markdown 粗體、容器內 🤖）的覆蓋是
 *   **列舉性**的——已知存在「變體讓身分可讀的行被歸到第三類、被跨身分豁免」的繞法。
 *   停戰理由＝這些繞法**全數需要蓄意偽造「William 特准」**，而本閘威脅模型明寫防打錯字
 *   不防惡意（造假者可直接偽造整則合規「通過」標頭，容易得多）＝豁免不增加造假者的
 *   邊際攻擊力。若未來威脅模型升級（分身分帳號上線），本段連同資格分類一起重審。
 */
 ...
 * 含名單外字元的**引文**＝那一行宣告（重述或豁免的逐字引文型）**不生效且收件截止**（fail-closed）。
 * ⚠️ 這不等於「那則壞留言沒救」（`Codex #543 r8 Low②` 糾正）：`#543` 起這一類由**同身分豁免＋
 * 雜湊指認**接手——雜湊型宣告不含引文、不受本守則約束，資格判定的 quotableAsRestate 會認出它，
 * 閘的處方也會直接指過去。在 `#543` 之前才是「只能關掉重開 PR 或問 William」。
...
export function verdictProblems(comments, head, reviewerRole = null) {
  ...
  for (const m of malformed) {
    ...
    // ⚠️ **身分比對不做任何額外正規化**（`Codex #543 r7 Medium①` 糾正了發射者的判斷）：
    //    `Grok #543 掃第 6 條`指出一個真實死角——隱形字元若長在壞行的**來源欄**，收件端必拒
    ...
    //    ——雜湊指認救得了引文腿，救不了身分腿。要解它必須讓**所有身分操作**採同一套正規化
    //    （結論聯集、來源相似度警告、豁免資格一起改），那是另一支的題目；本支只修死角、不動身分語意。
    ...
    if (taker) {
      ...
      problems.push(`有一則留言用了 🤖 記號、但標頭格式不合規${m.excerpt}\n`
        ...
        + '「接不了」有兩種：①輪次或 sha 欄損壞（metadata 讀不出）②**metadata 讀得出、但這一行引不動**'
        + '（引文含白名單外的字元／整行含隱形字元／sha 長相的字數不對）——第②種照上面那條重述指引做'
        + '一定會被收件端拒收，請直接走豁免，且引文出界時要用「原第一行雜湊」那個變體。'
        + (rid && metaReadable && !quotableAsRestate
          ? '\n    ⚠️ **這一則正是第②種**（身分與 metadata 都讀得出、但引文白名單／隱形字元／sha 歧義'
            + '三者有一不過）——不要再試重述，請走**同身分**豁免＋雜湊指認。'
          : ''));
    }
  }
