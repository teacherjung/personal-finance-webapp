...
export function looksLikeVerdict(body) {
  ...
  // ⚠️ **兩條判準，缺一不可**（Codex #385 r6 用 #383 的真實留言打出來的）：
  //
  //   (a) 這一行**以結論用詞開頭**，後面句子就結束、或接的是標點（但**不含冒號**）
  //   (b) 這一行**同時**出現「結論／審查結果／複審結果／複審完成／審查完成」**與**一個結論用詞
  //
  // 為什麼要 (b)：真實的留言長這樣——
  //   `## Claude 複審 r1｜結論：**需修改後再審**（不可直接合併）`
  // 用詞不在行首（前面有「Claude 複審 r1｜結論：」），而我 r5 的前綴是**逐字集合**，
  // 對不上就整則看不見 ⇒ 另一個 session 的「通過」直接放行 ⇒ **#383 原樣重現**。
  //
  // 為什麼 (a) 要排除冒號：`- 通過：1395/1395 題。`「不可合併：兩個 state 要分開存」
  // 這種「用詞當標籤、後面接說明」的寫法是報告的日常句型，不是結論。
  //
  // 為什麼 (b) 的關鍵詞不能只寫「結果」：「測試結果：通過 1392 題」「突變測試結果：通過率…」
  // 會全部中招——那是本專案每天在寫的句子（r5 的教訓）。
  const MARKER = /(?:結論|審查結果|複審結果|複審完成|審查完成)/u;
  const startsWithVerdict = new RegExp(`^(?:${words})(?:\\s*$|\\s*[^\\p{L}\\p{N}\\s：:])`, 'u');
  const hasVerdictWord = new RegExp(`(?:${words})`, 'u');

  /** 去掉行內 code：**任意長度的反引號**都算（`` \`\`x\`\` `` 也是合法寫法）。 @param {string} l */
  const noCode = (/** @type {string} */ l) => l.replace(/(`+)[^`]*\1/gu, '');

  const lines = text.split('\n').map((l) => noCode(l));
  // 機器人記號的判斷要在**剝掉行內 code 之後**：「來歷標頭要從 `🤖` 開始。」是在講格式。
  ...
  return lines
    .map((l) => l
      ...
      .replace(/[*_]/gu, '')
      .trim())
    .some((l) => startsWithVerdict.test(l) || (MARKER.test(l) && hasVerdictWord.test(l)));
}
