 ...
 *   （＝JS `length`／UTF-16 編碼單位，不是 Unicode 字元數），或兩者正規化後既不相等、
 *   也不互相包含 ⇒ **不提醒**。正規化＝NFKC → 小寫 → 拿掉所有非文字非數字的字元。
  ...
  for (let i = 0; i < ids.length; i++) {
    for (let j = i + 1; j < ids.length; j++) {
      ...
      warnings.push(`「${ids[i].who}」與「${ids[j].who}」的來源${why}`
        ...
        + '**輪次各自要大於「自己」現有的最高輪次**（想兩則共用同一輪就取大於全體最高的那個；'
        + '照原輪次補發撤銷不掉——同輪相反結論＝照樣阻擋、同輪同結論＝不取代舊 sha）。\n'
        + '    ⚠️ **這兩個身分在這支 PR 裡會一直存在**（留言歷史刪不掉）：之後每推一次 commit，'
        + '**兩個都要再補到新的 head**；「只用固定的那一個字串」是**下一支 PR** 才開始'
