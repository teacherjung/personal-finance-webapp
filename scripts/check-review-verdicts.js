...
export function looksLikeVerdict(body) {
  ...
  // ⚠️ **判準倒過來：偵測「有人在下結論這個動作」，不是比對「結論寫成什麼字」**
  //    （Codex #385 r7 用 25 份真實報告打出來的——**只抓到 9 份，漏 16 份**）。
  //
  //    漏掉的全是它自己前一天寫過的正常措辭：
  //      「結論（三選一）：需修正後再審」「結論：有阻擋問題，修正後重審。」
  //      「目前不建議合併」「目前不可以合併。」「可以合併。」
  //    因為我在比對**三個逐字用詞**，而人講話有無限種說法——**那是列舉的第七次**。
  //
  //    倒過來之後判準只有三種結構訊號，每一種都小而穩定：
  //      (1) 有 🤖（想用這個格式卻寫錯）
  //      (2) 某一行**以結論用詞開頭**，後面句子結束或接標點（**不含冒號**——
  //          「通過：1395/1395 題」是標籤句型，不是結論）
  //      (3) 某一行是**標題**且提到「結論／可以合併／審查結果…」，
  //          或某一行是「<標記>（…）：」這種**引出結論的句型**
  //    ——(3) 不要求後面接的字是三選一之一：**「他正在下結論」本身就要有合規標頭**。
  const MARKER = '(?:結論|審查結果|複審結果|審查結論|複審結論|複審完成|審查完成)';
  const startsWithVerdict = new RegExp(`^(?:${words})(?:\\s*$|\\s*[^\\p{L}\\p{N}\\s：:])`, 'u');
  /** 標題行提到結論／可否合併 ⇒ 這則留言在下結論。 */
  const headingAnnounces = new RegExp(`^#{1,6}\\s.*(?:${MARKER}|可以合併|可否合併|能否合併)`, 'u');
  /** 「結論（三選一）：」「審查結果：」這種**引出**結論的句型（括號註記可有可無）。 */
  const marlerIntroduces = new RegExp(`(?:^|[|｜\\s])${MARKER}(?:（[^）]*）|\\([^)]*\\))?\\s*[：:]`, 'u');

  /** 去掉行內 code：**任意長度的反引號**都算。 @param {string} l */
  const noCode = (/** @type {string} */ l) => l.replace(/(`+)[^`]*\1/gu, '');

  const lines = text.split('\n').map((l) => noCode(l));
  // 機器人記號要在剝掉行內 code 之後才判：「來歷標頭要從 `🤖` 開始。」是在講格式。
  if (lines.some((l) => /🤖/u.test(l))) return true;
  // ⚠️ 先剝掉粗體／斜體再判**引出句型**（真實語料抓到的）：
  //    #383 那則「## Claude 第二輪獨立複審」的結論寫成 `**結論：通過，可以合併。**`，
  //    前面是 `**` 而不是行首或空白 ⇒ 邊界對不上就整則看不見。
  //    **那正是造成 #383 的兩份相反複審之一。**
  const plain = lines.map((l) => l.replace(/[*_]/gu, ''));
  if (plain.some((l) => headingAnnounces.test(l.trim()))) return true;
  if (plain.some((l) => marlerIntroduces.test(l))) return true;
  ...
  return lines
    .map((l) => l
      ...
      .replace(/[*_]/gu, '')
      .trim())
    .some((l) => startsWithVerdict.test(l));
}
