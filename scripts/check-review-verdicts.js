...
/**
 * **寬鬆正規化**：NFKC（全形括號／逗號折成半形）＋小寫＋拿掉所有非文字非數字的字元。
 ...
 * ・抓得到的只有「長得像」＝正規化後相等、或一個整個包在另一個裡面。**抓不到三族**
 *   （後兩族是 Codex #456 r1 補的——我原本只寫了第一族，那是說了撐不住的話）：
 *   ①完全不同的名字（`CLI` vs `第二輪複審`）；
 *   ②**共同前綴、不同後綴**：`codex CLI (…, xhigh)` 與 `codex CLI (…, medium)` 互不包含 ⇒ 不提醒。
 *     那正是文件禁止「把 effort 寫進來源」要防的漂法——**規矩比提醒可靠**；
 *   ③正規化後短於 `LOOSE_MIN` 的名字（`桌面` vs `桌面版`）：短字串互相包含沒有指示性。
...
export function sourceLookalike(a, b) {
  ...
  if (x === y) return '去掉標點、空白與全形半形差異之後**完全相同**';
  const [short, long] = x.length <= y.length ? [x, y] : [y, x];
  // 包含關係才是 #453 那個真實形狀（`cligpt56solxhigh` 整個包在 `codexcligpt56solxhigh` 裡）——
  // 只比「正規化後相等」的話，漏掉工具名前綴的那次就抓不到。
  if (long.includes(short)) return '去掉標點與空白之後，其中一個**整個包在另一個裡面**（多半是工具名前綴打了一次、漏了一次）';
  return null;
}
  ...
  for (let i = 0; i < ids.length; i++) {
    for (let j = i + 1; j < ids.length; j++) {
      ...
      warnings.push(`「${ids[i].who}」與「${ids[j].who}」的來源${why}`
        ...
        + '    ↳ 若真的是同一次審查：**兩個身分各自**對目前 head 補發一則合規結論，'
        + '**輪次要大於這兩個身分現有的最高輪次**（照原輪次補發撤銷不掉——同輪相反結論＝照樣阻擋、'
        + '同輪同結論＝不取代舊 sha），之後固定用同一個逐字來源字串'
