...
export const TIERS = /** @type {const} */ ({
  ...
  F: { name: '工具安全設定', action: '不是重啟，兩個檔各有各的動作（AGENTS「錢的絕對邊界」節機械層）：.codex/hooks.json＝Codex 側副本——matcher 或指令一改，Codex 的信任雜湊就失效、hook 標成 Modified 並停止執行，William 要在 Codex 介面 /hooks 對該檔重新按「信任」，家目錄那份手動同步；.claude/settings.json＝Claude Code 權限層正本（permissions.deny＋PreToolUse 的兩組：python v6 那組、套件那組）——**只有 v6 那組**的 matcher 或指令一改，才要同步 .codex/hooks.json 那份副本（成對驗會逼；那份只准一組、放的是 v6）；**套件那組只在 Claude 側、不可以同步進 .codex/hooks.json**，照下面②的做法處理；不論改哪一組都要重開 Claude Code session 讓新設定載入、並確認家目錄 ~/.claude/settings.json 的 deny 仍在。v6 那組與它的副本驗＝test/codex-money-hook 的身分互鎖與成對驗；套件那組驗＝test/money-kit-hook。｜協作套件那幾份（Claude 側與 Codex 全域層各跟原本的 v6 並存；哪一天接上、驗過＝看 PROJECT.md 與機器表的日期，這段只寫長期有效的步驟）：分三類——①執行時會讀的（settings.json 的 forbidden 那一塊、tools/forbidden-tools.js、tools/settings-data.js、tools/package.json）：Claude 側鉤子每次呼叫都重讀那棵樹的這幾份，改檔（含沒提交的改動、切分支）下一次工具呼叫就生效；Codex 全域層讀的是固定複本，要在合併後的主幹上跑 node tools/guard-copy.js 重抽、關掉所有 Codex 視窗、在原位置換上新印出的那一組、William 重按信任、叫清單上刻意放的那個無害測試工具（＝mcp__guard_canary__ping，本體 tools/canary-server.js，照第 8 題 a 永久留在 forbidden.deny：它真的存在、就算真的執行了也不會碰到禁區、不會造成任何改變，**而且不是禁區連接器（forbidden.servers）上的工具——那上面的每一支，連唯讀的查詢、提醒、觀察清單，都不可以拿來試**；驗收只准用它，不可以拿任何別的工具代替），要看到這一組的拒絕理由（含「在拒絕清單上」）——任何會碰到禁區的真工具（包括禁區連接器上的每一支），不論在不在清單上、不論攔截器擋不擋得住，一律絕不可以叫來試；驗收只准用那個無害測試工具，最後才刪舊複本。②只在安裝時用的（templates/hook-claude.json、templates/hook-codex.json、templates/hook-codex-global.json、tools/guard-copy.js）：已裝好的鉤子不會重讀它們——範本的指令有改，就要把新指令換進已安裝的那一份（Claude 側＝專案 .claude/settings.json 的**套件那組**（不是 v6 那組）、改完開新對話；Codex 專案層＝本專案沒有裝套件的這一份（Codex 照搬家第 6 題 a 走全域層；.codex/hooks.json 是 v6 的副本，照前面那句處理），這個範本改了不用換；Codex 全域層＝照①重抽並換組、重按信任）；只改 tools/guard-copy.js 而範本沒改，已安裝的不受影響，下次抽複本才用到。不做＝繼續跑舊指令或舊清單，沒有機器提醒。③測試鈕（tools/canary-server.js、.mcp.json）：本體或登記一改，Claude 側要開新對話、Codex 側要關掉所有視窗再開，然後再叫一次測試鈕——要看到**那一組**的拒絕理由（含「在拒絕清單上」）才算數；**看到 pong 不算通過**：在已經接上的那一側看到 pong＝那一組沒載入或沒在跑，要停下來查（哪一側接上了看 PROJECT.md）；只有還沒接上攔截器的那一側，回 pong 才是預期（對照組，證明它真的在、真的無害）（它是驗收攔截器的唯一合法方式，壞了等於沒有驗收工具；裝法＝templates/canary-install.md）。步驟與目的地規則＝tools/guard-copy.js 檔頭與 templates/hook-codex-global.json 的說明。' },
});
...
export const RULES = [
  ...
  // 測試鈕（搬家第 3 步）：驗收攔截器唯一合法的那一支，本體與它的登記都跟攔截器同一級
  ['F', /^\.mcp\.json$/],
  ['F', /^tools\/canary-server\.js$/],
  ['A', /^db\//],
  ['B', /^package(-lock)?\.json$/],
  ['C', /^lib\//],
  ['C', /^server\.js$/],
  ...
  ['E', /^scripts\/(check-ci-really-ran|check-cross-pr-merge|check-pr-collab-fields|check-pr-merge-gate|check-review-verdicts|check-worktree-integrity|audit-grok-scan|grok-scan|grok-relay|grok-auth-refresh|grok-sandbox-canary|c6-adversarial|acceptance-tier|pending-rulings)\.js$/],
