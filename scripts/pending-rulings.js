...
// ## 這支在解什麼
//
// RULES I2 規定：問了 William 就**等他**（本專案的留痕形狀住 `AGENTS.md`「本專案協作附則」的「審查回饋處置」那顆）。
...
//
// ⚠️ **這不是閘**：它不擋任何事、不自報閘名、不寫任何留言、也不替我判任何一題該怎麼辦（判準住 RULES I1〜I3 與附則那顆，
// 這裡照著判就會變成第二份規則書）。**「不是閘」靠的是「沒有人把它接進 pre-push／CI／合併程序（附則、settings.json 的 gates、tools/merge.js）」，不是靠退出碼**——
...
// ・**編輯痕跡用的是 `updated_at ≠ created_at`**，而切換日前的合併手冊（REVIEW-AND-MERGE.md，已刪）要審查者核對的是 GraphQL 的
//   `includesCreatedEdit`／`lastEditedAt`——兩個訊號在本 repo 的樣本上一致，但不是同一個欄位，輸出裡有寫明
//   （RULES I3 只說「貼出後不編輯」、沒指定欄位；附則「審查回饋處置」那顆同樣沒指定）。
// ・⚠️ **這裡以前有一句「時限的數字在 AGENTS 那顆是正本，這裡的常數由考題綁回去」——常數與那道綁回考題都刪了**
//   （2026-09-10 William 裁「拿掉時限」）。正本那一顆也不再有現行天數，只剩沿革括號裡的舊數字。
//   Grok #595 複審後掃抓到這句還用現在式教舊制，照實改寫。
// ・**留痕留言的內文只驗到「非有不可的那一欄」**：❓ 驗第一行的完整形狀（記號／名稱／真日期／標題），
//   ⚖️ 另驗「原話那一段」的段落形狀，⏳ 另驗「William 未裁、隨時可翻案」那一句。
//   規則正本還要求 ❓ 寫出選項／建議預設／類別（⚠️ **2026-09-10 起沒有「時限」那一欄**），
//   舊格式的 ⏳ 還要寫授權依據／逾時判定／照哪個預設做了什麼——
//   **這幾欄這支沒有驗**（缺了照樣算數）。沒驗的原因是它們沒有固定字串形狀，猜著驗會開始長成第二份規則書；
//   缺欄不會讓問題被誤判成已結（那只看第一行＋上面那兩欄），但也不要拿這支當「留痕寫齊了」的證明。
import { execFileSync } from 'node:child_process';
import { isMainModule } from '../lib/is-main.js';
import { gitEnv } from '../lib/git-env.js';
// 🤖 的有效內容判準這支不自己另立＝直接用舊複審聯集閘那一支的函式（第 7 步退役前仍是本專案在跑的閘；
// 附則「審查回饋處置」那顆講的就是「閘怎麼看」——它跟套件閘的差別寫在下面 shapeOf() 用它的那幾行）。
...
// 由**問這一題的那一方**撤回。**這是三種結局裡唯一由發問者單方面發動的**，所以形狀卡得比另外兩種緊（見 WITHDRAW_REASON）。
// ⚠️ 2026-09-10 之前這一段寫死 Claude（Codex #594 r8 Low）：正式行為在同一支 PR 裡已經改成收合法角色集合，
//   說明卻還停在舊的一方——**說明落後於行為**，不是功能還壞著（實測 Codex 署名的撤回會被收下）。
 ...
 * ⚠️ 為什麼要枚舉：另一種結局的發動者是 William（他回了），撤回的發動者是**發問的那一方**自己——
 *   （⚠️ 2026-09-10 之前還有第三種發動者「時鐘（時限到了）」，時限拿掉之後**沒有時鐘可以發動結局**了；
 *   Grok #595 複審後掃抓到這句還用現在式列它）
 *   「我自己決定不問了」正是這種收法最危險的用法。枚舉逼我在三種**客觀可查**的理由裡挑一個，
 *   挑不出來就不是撤回，該去問他（規矩正本＝RULES I1〜I3；三種理由與固定字樣住 AGENTS「本專案協作附則」的「審查回饋處置」那顆）。
 * ⚠️ 誠實劃界：這驗得出「有沒有挑一種」，驗不出「挑的那一種是不是真的」。真正的網在**輸出**上——
 *   撤回的題目一律印在清單裡、附理由與連結，讓 William 一眼看得到我撤了什麼（安全網做在輸出上，不做在判斷裡）。
 */
// ⚠️ 類別後面**必須接一個全形括號的說明**（裡面要有非空白的字；說明本身可以帶括號，取到行尾最後一個 `）`）：只比對前綴的話，`撤回理由：題目依附的東西沒了但其實還在`
//   也算數，而報告會照樣印出「理由：題目依附的東西沒了」——印的跟留言裡寫的不一樣，安全網就失效了。
// ⚠️ **捕獲的是整行理由（類別＋括號裡的說明），不是只有類別**（#582 r1 Medium①）：括號裡寫的正是
...
// ⚠️ 但**說明裡面可以有括號**（#582 r8 Medium）：具體依據常常要引 PR 標題或功能名，而那些本來就帶括號。
//   所以取到**行尾最後一個 `）`**，只要求整行以它收尾；上一版禁掉內層 `）`，把正常的依據一起擋掉了。
 ...
 * ⚠️ **撤回者不寫死**（2026-09-10 Grok #594 複審後掃）：發問與貼 ⚖️ 在同一支 PR 裡已經改成兩邊都做得到，
 *   撤回卻還逐字只認「Claude 撤回」——於是 Codex 實作的支只剩兩條路：**假稱是 Claude 撤的**（假紀錄），
 *   或**撤了不算數**（那題永遠掛在「還沒回」的清單上）。兩種都比「讓撤回變寬」更像紀錄在說謊。
 * ⚠️ **照實劃界：這裡收的是 `ROLES`，那份名單是「PR 協作欄位的合法角色」，比「誰可能撤回」寬**
 *   （`William` 也在裡面，而他不會來撤自己的題）。仍然用它，是因為**另立一份「誰可以撤回」的名單＝
 *   第二份會漂的複本**——這個專案被同一種漂移咬過很多次。收得寬的代價寫在這裡，不假裝它剛好。
 * ⚠️ 這道形狀驗不了「署名的是不是本人」：三方共用同一個 GitHub 帳號。規則書那邊寫的是寫作要求。
 */
const WITHDRAW_PHRASE = new RegExp(`^(?:${ROLES.join('|')}) 撤回、William 未回；他隨時可以要我重問[ \t]*$`, 'mu');
...
//   ⚠️ 照實劃界：這樣抓得到的是「第一行帶 🚫、但內文不合規」那一種。第一行**連記號都打錯**的撤回
//   （例如 🚫 少了、或整行寫成別的樣子）不會進「形狀不合」，會被當成普通留言略過——那是刻意的取捨
//   （收「撤回」這個日常詞的代價是把複審留言全撈進來），不是「不可能發生」（#582 r10 Low③）。
...
export function visible(body) {
  ...
  // 只認頂層＋圍欄追蹤。**判準正本＝`AGENTS.md`「本專案協作附則」的「審查回饋處置」那顆裡「引用 ❓ 網址必須寫在留言最外層才算數」那一句**（規矩＝RULES I3）
...
export function shapeOf(c) {
  ...
  // 🤖 的判準**不自己另立一套**：附則「審查回饋處置」那顆講的是「結論聯集閘會怎麼看」，
  // 所以直接用閘的同一支函式——這支綁的仍是舊複審聯集閘那一支（它剝 fence、`>` 引用與行內反引號之後才掃）。
  // 自己寫 `body.includes('🤖')` 比它嚴：切換日前的 AGENTS「留痕」那顆明教「引 Codex 的發現一律放 `>` 引用或反引號」，
  // 照做的裁示會被判成形狀不合，已經裁過的問題就冒回「還沒回」（#579 r5 High②）。
  // ⚠️ 套件的結論聯集閘（templates/verdict-header.md）讀整份原文、什麼都不剝——跟舊函式對「引用裡的 🤖」判法不同；
  //    這支換綁要等第 7 步舊閘退役，這裡照實記、不先改行為。
...
export function classify(comments, nowMs) {
  ...
  for (const ask of asks) {
    ...
    const item = {
      ...
      closedBy: hits.map((x) => ({
        ...
        reason: x.kind === 'withdraw' ? (visible(x.c.body).match(WITHDRAW_REASON)?.[1] ?? null) : null,   // 整行（類別＋說明）
      })),
    };
    // 優先序：**他的話最大**。有 ⚖️ 就是已結，即使我後來（或先前）撤回過——他要回答一題我撤掉的，
    // 只要貼一則 ⚖️ 就把它拿回來，不必先撤銷我的撤回（撤回者的動作不可以擋住 William 的話）。
  ...
  //    情境：他貼了 ⚖️、原文裡也有那一題的網址，但**沒寫在最外層**（引言／圍欄／縮排裡），
  //    而那一題剛好已經被我撤回或逾時暫定 ⇒
...
function one(item, i) {
  ...
  if (item.edited) lines.push('   這則被編輯過＝不算起算：要另貼一則新的、重新起算（規則＝RULES I3；形狀在 AGENTS 附則「審查回饋處置」那顆）');
...
export function render(r, meta) {
  ...
  if (provisional.length) {
    // ⚠️ **這一段只做一件事：把它們印出來**（Codex #595 r1 Low 把我上一版的因果講反了，這裡照實改）。
    //   ・**刪掉這一段不會讓那些題目冒回「還沒回」**——它實測過：分類與配對完全不動，`pending` 仍是 0、
    //     `provisional` 仍是 1。刪掉失去的是**看得見**：那些已經被暫定處置掉的題目連網址都不會印。
    //   ・真的會讓它們冒回「還沒回」的是**破壞分類**（例如把 ⏳ 那條形狀從解析器拿掉），不是這一段。
    //   ・**「只會是舊的」是規則保證、不是這支工具驗出來的**：規則 2026-09-10 起禁止再貼新的
    //     （William 裁「拿掉時限」），但**解析器沒有在驗日期**——今天貼一則合規的 ⏳ 照樣會被收下、
    //     照樣分到這一堆（它實測過）。⚠️ **所以標題不提新舊**——2026-09-10 中間有一版寫「舊制…不再有新的」，那等於替機器打包票，已經改掉（Grok #595 複審後掃、Codex r6 Low）。
    // ⚠️ **標題不可以宣稱「這一堆都是舊的」**（Grok #595 複審後掃）：解析器**沒有在驗日期**，
    //   今天貼一則合規的 ⏳ 照樣會被收進這一堆。註解承認了、畫面卻寫成事實，讀報告的人會以為機器認得出新舊。
    //   ⚠️ 所以**標題不提新舊**，改成這一段自己印一行「沒有在驗日期」（Codex #595 r6 Low：上一版這裡還寫
    //   「標題寫『舊制』」，那是中間版本的措辭、已經不是現況）。
    out.push('', `照預設先做過、他還沒裁（隨時可翻案）：${provisional.length} 則`,
      '   ⚠️ 規則 2026-09-10 起禁止再貼新的這種留言，但**這支工具沒有在驗日期**——這一堆裡若混進新的，我看不出來。',
      ...provisional.map((x, i) => one(x, i + 1)));
  }
