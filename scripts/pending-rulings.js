...
export function visible(body) {
  ...
  // 的容器解析。所以去量了真語料再決定：
  //
  //   量的時候（2026-09-07）：全庫第一行合規的 ❓／⚖️／⏳ 留言裡，**含引用網址的每一則都寫在頂層零縮排**，
  //   一層縮排與兩層以上都是 0 則。（筆數會長，所以這裡記的是**結論與量法**，不是當下的數字。）
  //
  ...
  const topLevel = lines.map((line) => {
    ...
    // ⚠️ **圍欄的開門要先看**：CommonMark 允許圍欄縮排 0〜3 格，而下面「縮排就當看不見」會把
    // 縮排一格的開門行整行丟掉、於是那道圍欄從來沒開過，裡面縮排零格的網址就被當成可見（#579 r22 High①）。
    // 關門的判斷在上面（`fence` 那一段），一樣不受縮排影響。
    const open = /^ {0,3}(`{3,}|~{3,})(.*)$/.exec(line);
    if (open && !(open[1][0] === '`' && open[2].includes('`'))) {
      fence = { ch: open[1][0], len: open[1].length };
      return '';
    }
    // HTML 註解的開門同理：縮排一格的 `<!--` 也要算開門（下面的成對剝除靠的是文字，不是行）。
    if (QUOTE.test(line) || indent > 0) return indent > 0 && line.includes('<!--') ? line.trimStart() : '';
    return line;
  }).join('\n');
