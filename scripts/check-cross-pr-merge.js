...
//
// 因為這個專案自己有證據：PR 說明的必填欄位是寫在規則書裡、大家都同意的規則，
// 而 2026-08-02 實測**連續三支（#374／#375／#376）漏填**。
// 「記得在合併前做試合併」是完全同一種規則——要在正確的時機、由記得的人、手動執行，
// 而**被忘記的那一次不會有任何徵兆**。
//
// 做成閘之後，規則書只需要多一個**名字**（切換日 2026-09-17 之前靠自報的 `MERGE_GATE` 強迫文件摘要點名它；
// 切換後＝settings.json 的 gates 登記，tests/settings.test.js 對帳磁碟上的閘），不需要多一條要記住的規則。**摘要會落後，名字不會。**
//
...
// 而執行合併的人自己就是負載）。退出碼 1 可以重跑一次、附帶限制——規矩＝RULES.md H4
// 「紅了只准重跑一次」（套件那一道閘登記在 settings.json gates 的「跨變更試合併」）；
// 限制句的正本只在本檔的 `RERUN_LIMITS` 這一串，退出碼 1 的訊息逐字帶它（2026-09-17 搬家第 5 步起，
// 舊文件裡那一步已刪、沒有第二份）。
...
/**
 * 「紅了重跑一次」的限制句——**正本只在腳本這一串**（2026-09-17 搬家第 5 步起：規矩本身＝RULES.md H4
 * 「紅了只准重跑一次」，那裡沒有這一串；舊文件裡的第二份已刪）。退出碼 1 的訊息逐字帶它，
 * 考題釘住：常數被掏空、訊息漏帶、訊息裡這一串前面接了否定詞，都會紅。這一串本身不承載「幾個」——
    ...
    message: '跨 PR 試合併：**合起來會壞**\n'
      ...
      + (bad.some((r) => r.kind === 'red')
        ? '\n\n⚠️ **合起來測試紅**：這種 GitHub **不會**顯示——兩支各自的 CI 都是綠的、'
          ...
          + `可以重跑**一次**（${RERUN_LIMITS}；規矩＝RULES.md H4，閘的登記＝settings.json gates 的「跨變更試合併」）。`
        : ''),
  };
}
