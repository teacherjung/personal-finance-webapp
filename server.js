...
if (isHosted()) {
  ...
  // **哪幾道＝上面的 `RATE_LIMITS` 表**——這裡刻意不重抄道數與編號（原本這幾行寫「②③④⑤」
  //「四道」，而表上的 post-gate 早就不只四道；寫死的自己會漂）。各道的理由多半寫在表上那一筆
  // 旁邊，這裡補的是表上沒展開的部分：
  //   ・上傳解析類：解 PDF／XLSX／XML，全站最貴的 CPU 操作——**多人化才成立的攻擊面**
  //      （以前只有你自己會反覆丟大檔，現在任何一位受邀使用者都可以）。
  //   ・**會對外連線**的那幾道：猛打它們等於拿我們的伺服器去打上游服務
  //      （哪些端點會對外＝上面的 `OUTBOUND_ENDPOINTS`，同樣不在這裡重抄名單）。
