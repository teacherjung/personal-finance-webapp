...
- **發射前置檢查（Grok 預審試行期間、每一輪都查；發射紀律，非合併閘）**：協作欄位「實作者」＝Claude 的支，
  發**每一輪**（r1、r2、r3…）前先確認 PR 描述已附 Grok 預審成績單**＋鎖工具配方聲明**（或漏跑紀錄＝五格記 0＋附原因，照條款仍占分母）；成績單在場但**無配方聲明的掃描＝當未跑**；**未附＝發射者先補齊**（補跑預審、
  ...
  （條款變動時同步本行；試滿裁決後隨條款一併處置）。〔為什麼掛這裡、且每一輪都查：規則只寫在 AGENTS＝開工時
  讀一次，比規則早出生的 session 看不到（2026-08-15 #464 實測漏跑）；只查 r1 又會漏接「規則生效時
  已進 r2/r3」的開放案（同日 #466 實證）——發射每一輪都是必經點、發射者必讀本節。〕
