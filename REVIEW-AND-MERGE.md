...
- **r1 發射前置檢查（Grok 預審試行期間）**：實作者＝Claude 的支，發 r1 前先確認 PR 描述已附
  Grok 預審成績單（或漏跑紀錄）。條款唯一真相＝AGENTS「Grok 的邊界」節，此處只指路不重述；
  試滿裁決後本行隨條款一併處置。〔為什麼掛這裡：規則只寫在 AGENTS＝開工時讀一次，比規則早出生的
  session 看不到（2026-08-15 #464 實測漏跑）；發射 r1 是每支 PR 必經點、發射者必讀本節。〕
