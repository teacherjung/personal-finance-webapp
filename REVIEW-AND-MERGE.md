...
- **發射前置檢查（Grok 預審常設、每一輪都查；發射紀律，非合併閘）**：協作欄位「實作者」＝Claude 的支，
  發**每一輪**（r1、r2、r3…）前先確認 PR 描述已附預審**輕量證據五件**（受掃版本與配方聲明**含驗屍退出碼 0**——逐字指令 `node scripts/audit-grok-scan.js --workspace <掃描 cwd>`，**無配方聲明或驗屍非 0＝當未跑**／材料 SHA-256／
  ...
  **尾輪**提示詞多問防漏網自評一句（逐條會／不會／不確定；定義＝AGENTS 該節），請 Codex 執行合併前確認**兩級標記與絆線檢查**已回填；補跑一律用**鎖工具配方**（安全理由與要求＝AGENTS 該節；逐字旗標）：
