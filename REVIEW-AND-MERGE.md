...
1. **三個合規結論字串**：`通過`／`需修改後再審`／`不可合併`。**只認這三種**——
   寫「要求修改」「通過（無阻擋）」都算標頭寫壞，會變成清不掉的阻擋
   （2026-08-06 五支 PR 全卡在這裡，唯一救濟＝重述行，見 `AGENTS.md` 的 ③）。
   ⚠️ 帶括號補充的結論（`通過（1 Medium 已關閉）`）**機械閘完全看不見**，一樣算寫壞。
2. **這一輪要用的來源字串**：從下面的標準表挑一個，**一個字都不要改**。

#### 為什麼來源字串是**機械身分**（2026-08-14 PR #453 實際踩到）

閘認「誰是同一位審查者」的判準是 **`角色` ＋ `來源`**，而且來源**只摺疊空白**——
全形／半形括號、有沒有 `codex` 前綴、有沒有逗號，通通算成不同的人。

那天同一個 codex CLI，來源在不同輪次被打成 `CLI（gpt-5.6-sol xhigh）` 與
`codex CLI (gpt-5.6-sol, xhigh)`，於是：

- 閘把同一位審查者拆成**兩個身分**；
- r8 的「需修改後再審」掛在其中一個身分底下，而他**從來沒撤銷過它**（新留言掛到另一個身分）
  ⇒ 合併被擋。規則是「阻擋只能由**同一位**審查者用更新的輪次撤銷」，別人的「通過」不算數；
- 補了那一半之後，另一半的結論還停在**舊 sha** ⇒ 再擋一次
  （規則：**有結論的審查者都要審到目前的 head**）。

⇒ **每個審查工具固定一個逐字不變的來源字串。** 它是身分，不是描述文字。
...
- **會變的東西不准寫進來源**：模型名、reasoning effort、輪次、日期、PR 編號。
  ...
  （本專案的審查模型與 effort 由本檔開頭那條規則固定成一組，寫進來源不增加任何資訊、只增加打錯的機會。）
...
1. 跑 `node scripts/check-review-verdicts.js <N>`，看輸出末尾那行「現況：」——
   那是**閘眼中的審查者名單**。認出哪幾個其實是同一位（兩個來源長得像時閘也會主動提醒）。
2. 選定**從此固定**的那一個來源字串（原則：選最早那輪用過的，或最短、最不容易打錯的）。
3. **兩個身分都要各自收尾**——用**它自己的**那個來源字串，對**目前的 head** 補發一則合規結論。
   ⚠️ **輪次要往上跳**（Codex #456 r1 阻擋：這條不寫，照文件做會失敗）。
   閘的規則是**每個身分各自**算的：補發標頭的 `r<n>` 必須**嚴格大於「那個身分自己」現有的最高輪次**。
   兩則想寫同一個輪次比較好記的話，就取**大於全體最高輪次**的那一個——例如 r8 與 r9 漂成兩位，
   兩則都寫 `r10`（⚠️ 那是**安全的簡化寫法**，不是閘的必要條件：分別寫 r9 與 r10 也會過。
   r2 精確化——把簡化寫法講成閘的規則，就是這份文件一直在修的那種失真）。
   照原輪次補發會這樣死：
   - 用同一個 r8 補發「通過」＝**同一輪出現相反結論** ⇒ fail-closed，照樣阻擋；
   - 用同一個 r9 補發，新的那則**不會取代**舊的 ⇒ 那個身分仍然停在舊 sha。

   兩件事都要做完：掛著阻擋的身分只有它自己撤銷得掉（別人說「通過」不會解除）；
   沒阻擋、但停在舊 sha 的身分也要補到目前 head，否則它的「通過」已經不適用。

   內文第一句註明**這是同一次審查的補發**（例：「這則是 r10，接續 r8 那一次審查，
   當時來源打成 `CLI（gpt-5.6-sol xhigh）`」）。閘不讀這句，它是給人看的稽核說明。
4. ⚠️ **這一支 PR 裡，兩個身分會一直存在**（Codex #456 r3 阻擋：這條不寫，照文件做會在下一次
   推 commit 時被重新鎖住）。留言歷史刪不掉，閘每次都重新看到它們，而且要求
   **每一個沒有阻擋的身分都停在目前 head**。所以：
   - **本 PR 內**：之後每推一次 commit，**兩個身分都要再補一次到新的 head**。
     只補選定的那一個，閘照樣紅（「另一個的『通過』是對舊 sha 說的」）。
   - **「只用選定的那一個字串」是從下一支 PR 才開始**——新 PR ＝ 留言歷史重新開始，
     那時候閘只看得到標準來源那一個身分。

   一句話的心智模型：**本 PR 內所有已經出現過的身分都要跟著 head；新 PR 起只用標準來源。**
5. ⛔ **不可以編輯舊的結論留言**。GitHub 的編輯直接蓋掉內容，`gh pr view` 也只看得到編輯後的版本
   ⇒ **改審查紀錄會洗掉稽核軌跡**，事後沒有人查得出當初到底寫了什麼。
   壞標頭的合規救濟同樣是「補一則新留言」（重述行），不是改舊的——**這道閘的一切修正都靠補發**。

⚠️ **誠實劃界**：提醒的邊界**就是演算法本身**，講性質、不列清單。

**正規化**（只用在這道提醒，**不是**身分比對——身分比對只摺疊空白）分三步：
①`NFKC`（全形英數與括號折成半形）②轉小寫 ③拿掉所有非文字非數字的字元（標點、空白全刪）。

**判準**：兩個名字任一個正規化後**短於 3 個字**，或兩者正規化後**既不相等、也不互相包含**
⇒ **不提醒**。（⚠️「3 個字」是 JS 的 `length`＝**UTF-16 編碼單位**，不是 Unicode 字元數：
`𠮷a` 是兩個字元卻算 3 ⇒ 會比。這種名字現實不會出現，寫出來是因為**不精確的宣稱就是失真**。）

⚠️ 這段話被打回兩次：r1 我寫「抓不到的有**三族**」，Codex r2 當場又找出三族不在名單上
（換序、中間打錯一個字、共同後綴不同前綴）；r3 又指出「正規化」沒定義、門檻的字數說法不精確。
**列舉補不完、宣稱要撐得住**——兩個都是本專案認過很多次的病型，我在自己的劃界裡各犯一次。

下面只是**例示**（不是清單）：

- `CLI` vs `第二輪複審`（完全不同的名字）
- `codex CLI (gpt-5.6-sol, xhigh)` vs `codex CLI (gpt-5.6-sol, medium)`（共同前綴、不同後綴）
- `本機 codex CLI` vs `桌面 codex CLI`（共同後綴、不同前綴）
- `codex CLI xhigh` vs `xhigh codex CLI`（同樣的字換了順序）
- `codex CLI xhigh` vs `codex CLl xhigh`（中間一個字打錯）
- `桌面` vs `桌面版`（正規化後不到 3 個字）

⚠️ 第二例正是上面「effort 不准寫進來源」要防的漂法 ⇒ **規矩比提醒可靠**。

它也**永遠不會**自動把兩個身分併成一位（不同 session 本來就該是不同審查者，自動正規化＝削弱這道閘）。
