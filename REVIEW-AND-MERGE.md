...
3. **兩個身分都要各自收尾**——用**它自己的**那個來源字串，對**目前的 head** 補發一則合規結論：
   - 掛著阻擋的那個身分，只有它自己撤銷得掉（別人說「通過」不會解除）；
   - 沒阻擋、但停在舊 sha 的那個身分，也要補到目前 head，否則它的「通過」已經不適用。

   內文第一句註明**這是同一次審查的補發**（例：「這則是 r8 的補發，同一次審查，
...
⚠️ **誠實劃界**：閘的相似提醒只抓「長得像」（正規化後相等、或一個包住另一個），
同一位審查者兩輪取了**完全不同**的名字它看不出來；它也**永遠不會**自動把兩個身分併成一位
（不同 session 本來就該是不同審查者，自動正規化＝削弱這道閘）。**所以真正的防線是上面那張表。**
