...
> ⚠️ **走到這裡通常已經太晚**（發射者做，不算在下列步驟裡）：Grok 複審後掃的時機是**Codex 通過之後、`gh pr ready` 轉正式之前**——PR 走到合併步驟時多半早已轉正式，那時才補掃＝每撈到一條要修的就多燒一次完整 CI（正是 2026-08 額度月中爆掉的那條路）。做法見本檔「怎麼執行」節那一條。
...
- **開 PR 一律 `--draft`；審查輪全程維持草稿；結論＝「通過」→ 先做 Grok 複審後掃（見「怎麼執行」節那一條）
  ...
  ⚠️ 掃**排在轉正式之前**：撈到要修的東西時人還在草稿期，修正推送不燒全卷 CI（排在 ready 之後＝
  每一條屬實發現多燒一次全卷，正是下面那條燒爆額度的路）。
