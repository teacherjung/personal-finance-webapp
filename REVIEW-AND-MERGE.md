...
> **常態分工＝三方協作框架 v4（AGENTS.md「三方協作框架」節，2026-07-24 裁決）**：Claude 實作、**Codex 唯讀審查**（獨立複審、對抗測試、同步點檢查、風險分析）、William 決定與驗收（⚠️ 這裡的「決定」＝**產品決定、需求優先序、畫面驗收、合併裁決**，與 AGENTS.md 角色分工表逐字一致；**不含**「審查回饋修不修」——那一項自 2026-08-14 起由 Claude 判斷，見 AGENTS.md「審查回饋處置」。⚠️ 摘要句原本只寫「決定與驗收」，比表格鬆，讀起來像審查回饋也要他拍板）。**高風險 PR（金額公式/資料庫/搬家/匯入/機密/共用底層）＝Codex 複審後才合併**——William 指定審某支 PR 時，在該 PR 的拋棄式審查樹看內容（樹已釘住受審 commit，`git diff origin/main...HEAD`）。
