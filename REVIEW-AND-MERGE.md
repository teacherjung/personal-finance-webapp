...
> ⚠️ **請人代合併之前**（發射者做，不算在下列步驟裡）：Grok 複審後掃——見本檔「怎麼執行」節那一條。
...
- **複審通過後、發射合併前（Grok 複審後掃常設；發射紀律，非合併閘）**：協作欄位「實作者」＝
  Claude 的支，**Codex 給出「通過」之後、請 Codex 執行合併之前**，把該版本的 diff（**`base..head` 兩顆都寫死 SHA**）抽給 Grok 掃一遍（**每支只掃這一次**），
  ...
  **退出碼非 0＝該掃作廢**。條款本文＝AGENTS「Grok 的邊界」節，本行是最短可執行版（條款變動時同步本行）。
