...
>   ⚠️ **但下列情況一律停下來問 William，不可自己拍板**（同一次裁示逐字給的）：
>   ①**Claude 自己不確定**該不該照 Codex 的建議改 ②動到**判準**或**金額口徑** ③動到**架構**。
>   ⚠️ **這三個詞哪些算、哪些不算，唯一真相＝AGENTS.md「審查回饋處置」的界線表**（例如：把程式修回既有契約**不算**動判準；純顯示格式**算**金額口徑；純搬家**不算**動架構）。
>   **本檔刻意不複製那張表**——同一份規則在兩個地方各寫一份，遲早各走各的。
...
> **常態分工＝三方協作框架 v4（AGENTS.md「三方協作框架」節，2026-07-24 裁決）**：Claude 實作、**Codex 唯讀審查**（獨立複審、對抗測試、同步點檢查、風險分析）、William 決定與驗收（⚠️ 這裡的「決定」**以 AGENTS.md 角色分工表 William 那一列為準**，本檔刻意不複述——那一列會長，複述的那份只會漂。**一般的審查回饋修不修不在他身上**，自 2026-08-14 起由 Claude 判斷；**判準／金額口徑／架構那三個例外仍要問他**，界線見 AGENTS.md「審查回饋處置」。⚠️ 原本只寫「決定與驗收」比表格鬆，讀起來像每一條審查回饋都要他拍板）。**高風險 PR（金額公式/資料庫/搬家/匯入/機密/共用底層）＝Codex 複審後才合併**——William 指定審某支 PR 時，在該 PR 的拋棄式審查樹看內容（樹已釘住受審 commit，`git diff origin/main...HEAD`）。
