...
3. **兩個身分都要各自收尾**——用**它自己的**那個來源字串，對**目前的 head** 補發一則合規結論。
   ⚠️ **輪次要往上跳**（Codex #456 r1 阻擋：這條不寫，照文件做會失敗）：補發標頭的 `r<n>`
   必須**大於這些漂掉身分現有的最高輪次**——例如 r8 與 r9 漂成兩位，**兩則補發都用 `r10`**。
   閘的規則是「**更新的**輪次才算撤銷」，照原輪次補發會這樣死：
...
⚠️ **誠實劃界**：閘的相似提醒只抓「長得像」＝正規化後**相等**、或一個**整個包在另一個裡面**。
**抓不到的有三族**（Codex #456 r1 補上後兩族——原本只寫了第一族，那是說了撐不住的話）：

- 完全不同的名字（`CLI` vs `第二輪複審`）；
- **共同前綴、不同後綴**——`codex CLI (gpt-5.6-sol, xhigh)` 與 `codex CLI (gpt-5.6-sol, medium)`
  互不包含 ⇒ 不提醒。⚠️ 那正是上面「effort 不准寫進來源」要防的漂法，**規矩比提醒可靠**；
- **很短的名字**（正規化後不到 3 個字，例：`桌面` vs `桌面版`）——短字串互相包含沒有指示性，
  刻意不比，免得整批提醒變成雜訊。
