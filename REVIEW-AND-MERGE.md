...
**任一個名字正規化後短於 3 個字，或兩者正規化後既不相等、也不互相包含，就不提醒。**

⚠️ 這句話 r2 改過寫法：上一版寫「抓不到的有**三族**」，而 Codex 當場又找出三族不在名單上
（換序、中間打錯一個字、共同後綴不同前綴）——**那是這支相對 `main` 唯一一處變糟**：
`main` 原本沒有任何窮盡宣稱，我卻新增了一句撐不住的。**列舉補不完，這是本專案認過很多次的病型。**
