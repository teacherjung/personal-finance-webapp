...
| 帳單多銀行/多格式（`parseStatement` 依位元組偵測 PDF/XLSX；PDF 再依**文件內容**判富邦/台新） | **PDF 銀行由文件內容判斷，不看使用者選的卡片**（`parsePdfAuto`：用「台新/富邦」行名關鍵字定方向，命中的解析器有結果就採用、否則挑筆數多者；卡片只決定記到哪＋提供 PDF 密碼）。**富邦＝PDF**（`parseFubon`：郵寄加密版說明同列、官網無密碼版說明換行下一列）；**台新＝PDF**（`parseTaishinPdf`：郵寄加密版，說明有時拆三行、支援斜線 115/06/02 與 7 碼 1150602 兩種民國日期、金額＝兩日期後第一個純整數）＋**XLSX**（`parseTaishinXlsx`＋SheetJS，官網下載，西元日期、金額獨立欄）。台新 PDF 已用同月 XLSX 交叉驗證（3月94/94、7月66/66，日期+金額零誤差）。各解析器回原始明細後共走 `finalize()`（分類＋國外交易服務費繼承＋`cleanStore()` 產生顯示用店名 `store`）。`parseStatement` 另回 `lastFour`（`extractLastFour` 從內容抓卡號末四碼，盡力而為、抓不到回 null）。**`store`＝清理過的顯示名，匯入後存進 `note` 顯示用；分類與 `stmtRef`（去重）一律用原始 `desc`，勿改用 `store`（否則分類失準、跨格式去重對不上）。`cleanStore` 流程：①先比對 `STORE_CANON` 已知品牌標準名（eTag停車/foodpanda/馬可先生/六必居/OMGYES/悠遊卡自動加值…，命中直接顯示標準名，使用者可續加）②否則走一般規則：去金流前綴（`連加*`/`騰加數位*`/`OPENAI*`/`TAPPAY_`/`FP-`）、截斷括號後段（`石二鍋(林口家樂`→`石二鍋`）、`、`後段、公司型態字（股份有限公司/有限公司…）、結尾 `/TW`、分店定位碼＋城市、設備碼、中文後殘留英文（`摩斯漢堡Mos B`→`摩斯漢堡`）**②之前先抽 FP 外送前綴**（`FP_PREFIX`＝`FP`＋分隔符；要求有分隔符，免誤傷本來就叫 FPxxx 的店）→ 清理完由 `appendFpTag` 在店名尾補「（FP）」（使用者定 2026-07；**會進 storeKey**——外送與內用是不同消費型態，分開記合理）。③收尾呼叫 `normalizeStoreDisplay`＝`applyBrandCanon`（品牌正規化）→`branchNormalize`（分店格式）→`applyBrandRename`（品牌簡稱）→`applyParkingLabel`（**店名含「停車」→「停車費（原店名）」**，冪等；名字沒「停車」二字的停車場如嘟嘟房/阜爾運通不套用——那些靠分類識別）。**`STORE_CANON` 命中也走 `normalizeStoreDisplay`**（否則 eTag停車 這類標準名吃不到停車標記）。**`branchNormalize`**：把「主體＋分店」統一成全形括號 `主體（分店）`（`統一超商-百福`→`統一超商（百福）`、`IKEA-新莊`→`IKEA（新莊）`）——**有分隔符（-／－…）者，分店限「純中文起頭」才切**（故 `ABC-MART`/`LADY-M` 不會被誤切）；**無分隔符的連鎖需列進 `BRANCH_CHAINS` 白名單**（目前＝`誠品生活`，因電腦無法自行判斷主體邊界）；冪等（已是 `…（分店）` 不再包）、結尾外幣註記 `（USD/9.99）` 先摘再接回。**`applyBrandCanon`**：品牌正規化表 `BRAND_CANON`（{prefix,brand}）——把「主體＋分店」拆開重組成標準品牌名，有分店才加括號（目前＝`台亞`/`台亞加油站`任何寫法→`台亞加油站`，`台亞林口第二交流道南站`→`台亞加油站（林口第二交流道南站）`；比 BRAND_RENAME 強、用於「連鎖名要換且要切分店」）。**`applyBrandRename`**：品牌顯示簡稱字串取代表 `BRAND_RENAME`（目前＝`全家便利商店`→`全家商店`、`台灣普客二四`→`Times Parking`，保留分店；純字串替換、用於「只換名不切分店」；使用者提出後加一條）。既有資料整理＝設定頁「店名格式整理」（**可重複執行**——每次新增整理規則後再跑一次即可套到舊資料）→ `POST /api/statement/normalize-branches`（用 `normalizeStoreDisplay`；`dryRun` 預覽 note before→after；套用同時對齊 storeKey 與學習表 key，save 自動 .bak）。台新 PDF/XLSX 都常截斷店名（`LOUISA COFFE`/`台灣國際開`），救不回被截字（盡力而為）。無公開店名對照表可匯入。**新增銀行＝加 `parseXxx()` 並補進 `parsePdfAuto`。`parseStatementPdf` 為 `parseStatement` 別名（相容） |
