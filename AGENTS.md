...
- **第八行「進度摘要」**（William 2026-09-21 裁；**同日稍早那條「語音回報檔」由他當場取消、由這一條取代**——他要改用 Codex 的語音功能唸，不要 Claude 自己做音檔）：六段與第七行「程序的附件」＝RULES J1，這裡不複述；**第八行是本專案自己加的**。他裁的是**只在本專案、不進套件**，落在這裡是實作者照 RULES K1 選的正本位置。回報時那一格寫「已更新（時間）」。
- **那份摘要檔**：**放在本專案主目錄的上一層**（跟倉庫同層、**不在任何倉庫裡**），檔名 `進度摘要.md`。⚠️ **刻意不寫絕對路徑**——公開倉庫不寫機器路徑（2026-09-21 為這件事退過一輪）；要用的人在對話裡問。放倉庫外是實作者選的：進倉庫會讓每次更新都污染變更紀錄，還得動 `.gitignore` 與它的登記考題。
- **他裁的內容規格**（前四件是他當場裁的；第五、六件是他轉述 Codex 的建議並要求照做——**都不是實作者自己想的**）：①每次**回報時同時更新**，把回報項目與日期時間記清楚 ②**只留最近七天** ③⚠️ **七天只刪「已經結束」的**——**還沒完成的工作、卡住的問題、等他裁示的事，一直留到處理完**，不受七天限制 ④最上面先有一段**目前全貌**：哪個專案、這件事在解決什麼問題、跟整體目標的關係 ⑤**完成程度要講明確**（寫好了／測試通過／審查通過／已經合併／他已驗收是**不同階段**），而且要寫**下一步是誰要做什麼**——不可以只寫「等待中」卻不說在等誰 ⑥每個重要結論寫**最後確認時間**與**證據位置**（證據集中放在摘要自己的「程序附件」區）。
- **更新時機**（他裁）：**完成一段、送審、遇到阻礙、收到審查結果**——這四種時候都更新，不是只有回報時。
- **寫法是實作者的操作化、不是他裁的**：寫成給人唸的散文（不用表格、不寫網址與代號、時間寫成「下午 3 點 21 分」），因為那份檔是要被唸出來的；「七天自動刪」**實際上是 Claude 每次寫的時候順手清**，**沒有任何背景程式在跑**——這件事寫在檔頭，不要讓人以為有機器在管。
- **Codex 的責任**（William 2026-09-21 轉述 Codex 自提、他要求寫進來）：每次回報前**重讀那份摘要**、重要結論**去核對證據**；紀錄過期或跟證據對不上就**直接告訴他**，不要照唸。
- ⚠️ **沒有任何機器在看這一行**（J1 標「靠自覺」）：那份檔不在版本控制裡，不會被任何考題、閘或審查看到，**正確性只有寫它的人顧**。
- ⚠️ 六段的**版面細節**（標籤粗體、段間空一行、還沒達成怎麼寫那些）目前仍寫在 `CLAUDE.md`，那是搬家前就在的落點；**本支不動它**，要不要照 RULES K1 一起搬過來是另一支的事。⚠️ 空段寫「沒有」**不在**那一份，它是 RULES J1 本身的規定。
