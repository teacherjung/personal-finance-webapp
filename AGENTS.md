...
- **那份摘要檔放哪、叫什麼＝實作者選的**（他沒有裁位置）：**本專案主目錄的上一層**（跟倉庫同層、**不在任何倉庫裡**），檔名 `progress-summary.md`。⚠️ **檔名用英文**——2026-09-22 裁示者指出檔名該用英文（**實作者轉述，沒有原話也沒有留痕**），照辦。⚠️ 射程要講準：**寫下來**的那條（RULES E6＋`test/doc-naming.test.js`）掃的是 `git ls-files`，**只管倉庫裡被追蹤的路徑**，**掃不到**這份倉庫外的檔——所以這裡跟的是**裁示者的慣例**，不是那條規則的適用範圍。至於「它在倉庫外所以可以中文」，那是**實作者自開的例外、沒有問過他**。理由（放倉庫外）：進倉庫會讓每次更新都污染變更紀錄，還得動 `.gitignore` 與它的登記考題。⚠️ **刻意不寫絕對路徑**——公開倉庫不寫機器路徑（2026-09-21 為這件事退過一輪）；要用的人在對話裡問。
...
- ⚠️ **結案時要把「決定」搬去永久紀錄**（2026-09-22 裁示者一問問出來的漏洞）：一件事收口時，如果它產生了「**以後別重議**」的決定，**要另外寫進 `PROJECT.md` 的「重要決定（已拍板，勿重議）」**（或有 ❓ 的話留成 ⚖️、或寫進提交訊息）。⚠️ 理由帶條件講：**收口之後那一條會落進摘要的「已結束」區，而那一區七天後會被清掉**（未完成、卡住、等裁示的**不受**七天限制、會一直留著）——**所以只寫在那一區的決定，七天後就沒有了**，下一個人會把它當新問題再問一次。⚠️ **兩者不互相取代**：摘要記的是會過期的進行式，`PROJECT.md` 那一節記的是**不該過期的決定**。（`PROJECT.md` 自己的整體定位見它開頭那幾句，**本條不改寫它**。）
