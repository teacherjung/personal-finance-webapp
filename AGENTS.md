...
⚠️ **架構那一列的兩欄要互斥**：「不算」那欄**只涵蓋**「在同一個模組邊界之內、而且不動資料流向與欄位歸屬」的純搬移。
   只要**跨模組**（抽成新模組、或從 A 模組搬到 B 模組）或**動到資料流向／欄位歸屬**，就落在「算」那一欄
   ——**即使沒有新增任何模組**。⚠️ 別把判準縮成「有沒有新增模組」：欄位改歸屬、資料改流向都不新增模組，
   但那正是架構。（`Codex #457 r3` 阻擋①就是抓到這句總結曾經縮得比上一欄窄。）
