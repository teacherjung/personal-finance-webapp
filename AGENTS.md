...
- **第八行「語音回報檔」**（William 2026-09-21 裁；他裁的是**只在本專案、不進套件**，落在這裡是實作者照 RULES K1 選的正本位置）：六段與第七行「程序的附件」＝RULES J1，這裡不複述；**第八行是本專案自己加的**。他當場裁的三件：①**只唸六段**、附件不唸 ②**兩分半以內**，超過就砍細節 ③**沒什麼好報的短回覆不做檔**、那一格寫「沒有」。另外兩件是**實作者的操作化、不是他裁的**：另外用講話的口氣重寫（第一句先講結果、句子短、不唸代號與英文、不唸標籤與符號），**不是把六段照唸**；長度以轉檔後的實際秒數為準。⚠️ **沒有任何機器在看這一行**（J1 標「靠自覺」）。
- **做法是可替換的、不是規矩**：現在用 macOS 內建的 `say`（聲音 `Meijia (Premium)`、`--data-format=aac` 輸出 `.m4a`）把口語稿轉檔再送過去。換工具、換聲音、換格式**不必改規矩也不必問**——只要產出仍符合上面那三件。
