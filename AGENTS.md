...
**一句話總綱**：Claude 負責建造，Codex 負責挑戰，William 負責決定與驗收；共享檔案一次只交給一人。（⚠️ 總綱這句的「決定」**以上方角色分工表 William 那一列為準**——這裡刻意不複述、也不寫「哪幾項」，因為那一列會長：2026-08-14 就多了「審查回饋的例外」。**一般的審查回饋修不修不在他身上**，由 Claude 判斷。摘要句比表格鬆是本檔反覆出現的漂移型態，這裡就地釘住。）高風險工作小步前進，每一步有考題、有證據、有退路。文件說明規則，契約連接模組，PR 隔離變更。
