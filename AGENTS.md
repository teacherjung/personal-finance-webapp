...
- **複審後掃（常設；William 2026-08-18 拍板**改序**——本條原名「預審參謀」＝送 Codex 前先掃，2026-08-16 轉常設、2026-08-18 改為複審後掃）**：Claude 實作的每支 PR，**Codex 給出「通過」之後、請 Codex 執行合併之前**，把該版本的 diff（**`base..head` 兩顆都寫死 SHA**）抽給 Grok 掃一遍——**每支只掃這一次**（第一次「通過」之後）：修完再審、再通過**不重掃**。本條要的是「複審漏了什麼」的證據，不是替最終 head 背書。
  ...
  - **呼叫紀律**：repo 外目錄餵材料檔（邊界前綴＋指示＋diff）、**不在任何 repo 目錄啟動它的 CLI**、**必鎖工具**（不鎖會越界＝#467 實測：自行搜家目錄、`cd` 進主目錄跑 git；逐字旗標與版本釘抄 REVIEW-AND-MERGE「怎麼執行」節的**鎖工具配方那一段**——**CLI 版本不同＝當未跑**、人工重驗工具清單後更新；**掃後必驗屍**＝跑 `node scripts/audit-grok-scan.js --workspace <掃描 cwd>` 機械稽核該次 session 日誌，**退出碼非 0＝該掃作廢**——2026-08-17 實測鎖工具旗標同版本會非決定性靜默失效，旗標只當第一層，**制度靠驗屍不靠運氣**（William 拍板））。材料以**已 commit、將上 GitHub 的內容**為限（與送 Codex 同一暴露等級），機密與真實資料不得入材料。
  - **PR 描述固定用小標 `### Grok 複審後掃`**（釘住字串才 grep 得出哪幾支跑了——#483 被記成零分的根因就是紀錄格式沒被釘住），內容二選一：①下列各項（刻意不寫件數——寫死的數字自己會漂）；②逐字「**未執行：<原因>**」。
    ...
    - **掃描時序**（一行）：對應**哪一則「通過」留言**（貼留言網址）＋掃描時間。⚠️ **自報、無機械保證**——留著它不是因為它擋得住造假，而是因為沒有它連「聲稱的先後」都不在紀錄上（#486 r1 High：head SHA 對得上≠先後對得上）。
