...
- **預審參謀（試行條款，William 2026-08-15 拍板；試滿裁決後本條改寫或刪除）**：Claude 實作完成、**送 Codex 正式審之前**，把 diff 抽出來給 Grok 掃一遍——Grok 不碰樹、不碰 repo（材料由發稿者抽給它，與讀者測試員同模式）。發現由 Claude **逐條驗證**，真問題且屬本支 diff 者送審前修掉；修不修照「審查回饋處置」既有界線。**Grok 意見不算正式複審、不進任何合併閘、不產生結論字串**（合併機器零改動）。試行 **10 支 PR**，每支在 PR 描述記五格成績單：**發現數／驗真數／獨家數／有效獨家數／誤報數**——**有效獨家**＝只有 Grok 提出＋驗證為真＋屬本支 diff＋送審前真的修掉（單看獨家數會被不重要的風格建議灌水；這一格量的是「沒有它，這支 PR 就帶著問題送審」的反事實價值）。期滿彙整三率給 William 裁決轉常設／收回／調整：**驗真率**＝驗真÷發現、**有效獨家率**＝有效獨家÷PR 數（**主指標**）、**誤報率**＝誤報÷發現（噪音成本）。
