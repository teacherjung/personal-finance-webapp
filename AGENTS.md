...
> - **Grok 補註（2026-08-14 實測）**：Grok CLI **會**載入使用者層 `~/.claude/settings.json` 的 permissions（上述兩支下單工具的 deny 都在載入之列）；PreToolUse hook 那層是否被它執行**未驗證**；其 CLI 未配置任何 MCP 連接器＝手上沒有錢類工具。約束 Grok＝本節條文（實測它自動讀本檔）＋三方協作框架「Grok 的邊界」節（教學線特約、程式線預設關門）。
...
| Grok | 教學線「外部記者」三職：腳本查核／每集開工時的選題雷達／就地解釋文案的零基礎讀者測試；程式線＝**預審參謀（試行）**＋**唯讀顧問（個案）**（完整邊界＝「Grok 的邊界」節） | 一切實作、正式複審與合併執行；不進主目錄與常設實作樹；X 輿情不得用於投資決策線 |
...
- **預審參謀（試行條款，William 2026-08-15 拍板；試滿裁決後本條改寫或刪除）**：Claude 實作完成、**送 Codex 正式審之前**，把 diff 抽出來給 Grok 掃一遍——Grok 不碰樹、不碰 repo：在 repo 外的工作目錄（如暫存區）餵材料檔、**不在任何 repo 目錄啟動它的 CLI**；材料以**已 commit、將上 GitHub 的內容**為限（與送 Codex 同一暴露等級），機密與真實資料不得入材料。發現由 Claude **逐條驗證**；修不修由 Claude 照「審查回饋處置」既有界線判斷（例外照舊問 William），**判定要修且屬本支 diff 者＝送審前修掉**、不留到正式輪。**Grok 意見不算正式複審、不進任何合併閘、不產生結論字串**（合併機器零改動）。試行＝Claude 實作的**連續 10 支 PR 全數入樣**（不挑支、**跳過不出分母**：跑不動＝該支五格記 0、附失敗原因與輸出，仍計入 PR 數——可自報的「技術跳過」就是挑樣本的後門），每支在 PR 描述附**同一份可稽核證據**：送掃前凍結的**自審已知清單**（可為空、要明說）＋**受掃 commit SHA 與材料檔識別**＋**Grok 回覆原文**＋**逐條判定**＋五格成績單（成績單是自報數字，前四樣才是 William 抽查時的對帳依據——缺自審清單，「獨家」就無法重算）。五格的操作定義：**發現數**＝Grok 列出的實質條目；**驗真數**＝Claude 判定屬實（不論輕重）；**誤報數**＝發現−驗真（不成立、幻覺、誤讀）；**獨家數**＝驗真且發稿者自審清單裡沒有（對照基準＝送掃當下已知，**不比事後 Codex**——修掉的問題 Codex 本來就看不到）；**有效獨家數**＝獨家＋屬本支 diff＋判定要修並送審前修掉（單看獨家會被風格建議灌水；這格量的是「沒有它，這支 PR 就帶著問題送審」的反事實價值）。期滿把 10 支**加總**算三率給 William 裁決轉常設／收回／調整：**驗真率**＝驗真÷發現、**有效獨家率**＝有效獨家÷PR 數（**主指標**；按定義是「每支平均件數」、可大於 1）、**誤報率**＝誤報÷發現（噪音成本）；任一率分母為 0＝該率記「**不適用**」並呈報原始數字，**不得默認成 0% 或 100%**。
