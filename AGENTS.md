...
**重疊 PR 讓道（2026-08-29 #526 擋住 #519 的教訓；William 2026-08-29 排定入冊＝候選 8-18）**：要動的檔案與**在途 PR 已宣告的預約**重疊時，**不要開著自己的 PR 等**——跨 PR 試合併閘只做試合併、只讀文字衝突、**不讀欄位裡的意圖**（在說明寫「不會搶」沒有機械效力），開著的 PR 就是閘眼中的衝突源，先預約的支反而被後開的擋住（#526 未送審就擋住了已審完六輪、走到合併最後一步的 #519，靠限時協調暫關才解）。正確做法三選一：①**晚開**——工作照做、commit 照推，等對方合併、自己 rebase 完再開 PR ②**開了立刻暫關**——closed 的 PR 閘不看（`scripts/check-cross-pr-merge.js` 現行只撈 `--state open`），隨時可 reopen、號碼與討論都保留 ③真要同時開著＝把自己的衝突 hunk 改到不撞（通常不值得）。暫關讓道時在 PR 留言寫明**「讓道、非放棄、誰負責重開」**，避免被當成棄單。
