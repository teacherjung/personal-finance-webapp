...
- **複審後掃（常設；William 2026-08-18 拍板**改序**——本條原名「預審參謀」＝送 Codex 前先掃，2026-08-16 轉常設、2026-08-18 改為複審後掃）**：Claude 實作的每支 PR，**Codex 給出「通過」之後、`gh pr ready` 轉正式之前**，把該版本的 diff（**`base..head` 兩顆都寫死 SHA**）抽給 Grok 掃一遍——**每支只掃這一次**（第一次「通過」之後）：修完再審、再通過**不重掃**。本條要的是「複審漏了什麼」的證據，不是替最終 head 背書。
  ...
  - **凡是本節判成「當未跑／不掃」的，一律同一個收口**（刻意不列舉理由、不寫數量——理由會長，寫死就會漏）：在固定小標下逐字寫「**未執行：<原因>**」，**不擋合併、不入任何計數**；要不要重掃是選項、不是義務。⚠️ **2026-08-22 起「作廢」這個字不再出現在新掃描裡**：沙箱制下驗屍不作廢（在盒子裡跑指令是准的），要嘛金絲雀不過＝**沒掃**（記「未執行」）、要嘛掃了＝**算數**；驗屍查到破口＝**事故**，另外處理。〔舊制留下 10 支 PR 裡 9 支記「未執行」、其中至少 2 支實際撈到真問題——紀錄又在說謊，這正是改沙箱的理由之一；那些舊紀錄保留原樣當歷史。〕⚠️ 〔舊制遺留的處置，只適用 2026-08-22 前已作廢的掃描；新制沒有「作廢」〕作廢掃描的內容仍可人工驗證後採用，但**依定義列為 Claude 自審、不計 Grok 出勤**，且**不得寫在固定小標底下**（那一區是日後人工翻表的統計區，混進作廢掃描的發現＝製造新的假金標準）——另立標題放。⚠️ **本條沒有任何機械保證，全靠自律**——免跑理由一律自報、無人複查、不擋合併；要求記那一行，只為了讓事後 grep 得出哪幾支跑了、哪幾支沒跑。（舊版「停跑不是逃生門」那句有東西撐著＝漏跑入計數、不可洗銷、連 3 支重裁；本版把撐著它的全刪了，所以那句話一併刪掉，不留誇大。）
