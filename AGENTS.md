...
- **他轉述 Codex 的建議、並要求照做的四件**（不是實作者自己想的，但也**不是他自己的判斷**——這一層要分開）：④最上面先有一段**目前全貌**：哪個專案、這件事在解決什麼問題、跟整體目標的關係 ⑤**完成程度要講明確**（寫好了／測試通過／審查通過／已經合併／他已驗收是**不同階段**），而且要寫**下一步是誰要做什麼**——不可以只寫「等待中」卻不說在等誰 ⑥每個重要結論寫**最後確認時間**與**證據位置**，證據**可以放在摘要自己的「程序附件」區**讓他需要時查核 ⑦**更新時機＝完成一段、送審、遇到阻礙、收到審查結果**，這四種時候都更新，不是只有回報時。
...
- ⚠️ **誠實劃界**（J1 標「靠自覺」）：那份檔在倉庫外，**本倉庫現有的考題、閘與變更 diff 都不涵蓋它的內容**；維護靠 Claude、人工核對靠 Codex（上一顆），而**那兩件有沒有真的做，沒有任何自動驗證**。這不等於「沒有人看得到」——人工審查看得到，只是沒有機器在替它把關。
