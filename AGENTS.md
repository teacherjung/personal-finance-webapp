...
- **複審後掃（常設；William 2026-08-18 拍板**改序**——本條原名「預審參謀」＝送 Codex 前先掃，2026-08-16 轉常設、2026-08-18 改為複審後掃）**：Claude 實作的每支 PR，**Codex 給出「通過」之後、`gh pr ready` 轉正式之前**，把該版本的 diff（**`base..head` 兩顆都寫死 SHA**）抽給 Grok 掃一遍——**每支只掃這一次**（第一次「通過」之後）：修完再審、再通過**不重掃**。本條要的是「複審漏了什麼」的證據，不是替最終 head 背書。
  ...
  - **呼叫紀律（2026-08-22 起＝沙箱制；William 裁示 B＋中間路）**：一律用 `node scripts/grok-scan.js --base <sha> --head <sha> --prompt <指示檔>` 跑，**不准手動啟動它的 CLI**——那支腳本把整條流程串成一支，讓「忘了某一步」變成做不到：①**建盒子**（`git archive` 出 head 的已 commit 原始碼＋APFS clone 的 `node_modules`；沒有 `data/store.db`、沒有 `.env`——材料仍以**已 commit、將上 GitHub 的內容**為限，與送 Codex 同一暴露等級）②**金絲雀**（`scripts/grok-sandbox-canary.js`：在盒子外放假機密、叫 `cat`／`ls`／`curl` 去讀，**任一隻活著＝沙箱是假的、不掃**，fail-closed）③**轉送器**（`scripts/grok-relay.js`：localhost → xAI 那一個寫死的位址，目的地不從請求取）④在 **`scripts/grok-sandbox.sb`** 沙箱裡跑 grok：`(deny default)`＋Apple 的 `system.sb` 再逐項放行——系統與工具鏈唯讀、盒子讀寫、**真 `~/.grok` 完全不進沙箱**（盒內的 grok 家是白名單複製的副本；登入狀態放沙箱專用的 `~/.grok-sandbox-auth`）；網路**只准連轉送器那一個 port**（會起本機伺服器再連自己的整合型考題在盒子裡跑不了，這是不讓它連到理財 app 4321 的代價）；`HOME`／`TMPDIR`／`GROK_HOME` 全指進盒子 ⑤**驗屍**（`scripts/audit-grok-scan.js` 數足跡＋查破口線索）⑥**盒子在所有出口清掉**，只留去機密的結果包（`launch.json`＋sessions）在 `~/.grok-scan-results/`。**Grok 在盒子裡跑指令是准的**（William 2026-08-20 拍板「允許跑指令、作廢制廢止」——當時條文沒跟上，#494 照裁示做、#491 照過期條文做，兩邊都沒錯，錯的是這裡沒更新）：在盒子裡它可以 `cat`、`grep`、跑 `node --test`——盒子裡只有：已 commit 的原始碼副本、`node_modules` 副本、白名單複製的 grok 家（執行檔、`config.toml`、`agent_id`、**`auth.json`＝它自己的登入憑證，這本來就是它需要的**）。沒有真實資料、沒有歷史 session、沒有其他憑證。⚠️ **為什麼從鎖工具改成沙箱**：2026-08-22 全量稽核 `~/.grok/sessions`，53 個 session 有 **52 個**留有工具足跡——鎖工具旗標的失效率不是「偶爾」，是常態；它進過主目錄讀原始碼跑 git、`ls` 過 Desktop／Documents（檔名已送 xAI）、`curl` 過外網（能 GET 就能 POST）；**碰 `store.db`／flexToken／`.env`／金鑰：0 次**——不是圍欄擋的，是它自己沒去。「請它不要做」擋不住，改成「作業系統不讓它做」。⚠️ **誠實劃界**：`sandbox-exec` 是 Apple 標為過時但 macOS 15.7 仍可用的工具，它消失這道圍欄就沒了；沙箱網路規則只認 localhost，放不了特定網域，所以靠轉送器；轉送器擋不住「把資料 POST 給 xAI」（那跟送 prompt 同一條路），真正的保護是它讀不到可以送的東西；金絲雀只證明列出的形狀擋得住、不證明沒有別的洞；**盒子裡放什麼仍是我們的責任**。失效條件三個不變：**CLI 版本不同＝當未跑**（轉送器的目的地是從 1.0.3 執行檔 `strings` 出來的，升版要重驗）、**金絲雀非 0＝不掃**（取代舊的「驗屍非 0＝該掃作廢」：驗屍現在記足跡、查破口，**破口＝沙箱破了＝事故**，不只是作廢）、**缺掃描時序一行＝當未跑**。
