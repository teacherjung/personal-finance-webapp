...
- **合併的決策與執行是兩件事，分開講**（2026-07-30 對齊；此前「使用者是最終合併者」「使用者合併」「PR 由 William 合併」三句散在三份文件、字面上都已與實務不符——本條是唯一規則來源）：
  - **決策（要不要合、何時合）＝永遠 William**（角色表「合併裁決」）。任何人不得自行決定合併——「審查通過」只是門檻之一，不是合併指令。
    - **常設授權（William 2026-08-13 拍板；2026-09-05 確認對稱適用，落點＝入冊本條的 PR 裡他的留言）**：Claude 實作的支，Codex 留下合規「通過」、Grok 複審後掃做完＝**視同 William 於送審時已決定合併**，直接請 Codex 依程序合併、**不必再問**；Codex 實作的支對稱——Claude 複審通過後由 Claude 依程序合併，同樣不必再問。這**不是**把決策權移轉：上一條一字不動，他**隨時可收回**本授權、也可對個案說「先不要合」；本條只把「每支都問」改成「預設同意、例外才問」。仍要問的例外＝「審查回饋處置」的界線表（判準／金額口徑／架構）與「錢的絕對邊界」那一族——那些從來不在任何常設授權內。〔為什麼要寫進來：這條 2026-08-13 起只存在 Claude 的記憶，Codex 讀本檔字面會以為每支都要等 William；2026-09-05 協作系統體檢查出後入冊。〕
