...
- **值不值得改**：**審查回饋不無條件全改**。只有三種情況才回頭改——①**考題其實沒在守它宣稱的東西**（假綠）②**註解／題名說了撐不住的保證** ③**真的產線 bug**。其餘一律寫進待辦，**不再為它多開一輪審查**。
  ...
  ⚠️ **第三輪自問表（William 2026-09-10 裁，原話逐字「2. 甲」，落點＝https://github.com/teacherjung/personal-finance-webapp/pull/596#issuecomment-5621280390 ；他先前原話逐字「我贊成乙。只是我們要怎麼立規矩幫助判斷：審核是不是軍備競賽？何時該換個方法？」「我們是不是該列出一些問題，當超過3輪的時候就問自己一遍，及時糾偏。」）**：**同一道檢查（或同一句話）連續第三輪被審查者抓到，動手改之前先答五題**——①我這一輪是在「多補一種寫法」，還是「換一個問題來回答」？補寫法＝跑步機，停。②我的檢查是不是在**重做別人已經做好的東西**（畫面怎麼呈現、某語言怎麼讀、某服務怎麼判斷）？是＝永遠追不上，換一個自己量得到的東西。③我想證明的是「**所有**情況都不會…」嗎？有限取樣證明不了「所有」——直接量那個東西本身，或把量到的範圍寫死、其餘照實說不保證。④審查者這一輪要我知道的，是**一件新的事**，還是只是「再想一種」？只是再想一種＝同一族，停。⑤我**真正在乎的是什麼**？我的檢查量的是不是那個？答完只有**四種動作**，選一個：**直接量事實**／**改規則書的寫法讓檢查變簡單**／**整段逐字釘住**／**把守得住的寫死、其餘照實說**。四個都不適用＝問 William 要不要繼續。⚠️ **留痕＝寫進下一顆修正的說明**（「第三輪自問：答了第②題，所以換做法」），**不另貼留言**。⚠️ **與上面那條是兩件事**：這一條管第三輪、私下答、寫進 commit 訊息；上面那條管第八輪起、公開貼在 PR 上。⚠️ **它不是閘**：沒有任何機器在數輪數、也沒有東西在看有沒有答——跟上面那條一樣全靠自律。〔回頭驗過三支：#454 一行小字審 23 輪，第①題在第十輪會叫停；#578 七輪，第③題第二輪會叫停；#596 四輪，第②⑤題第二輪會叫停。⚠️ 但**它救不了另一種**：#596 r6〜r8 是「我的修正又引入新的敘述問題」，那不是同一道檢查被反覆打穿，五題裡只有①勉強沾邊——照實說，這張表管的是**同一道檢查一直被繞**，不管**每次修正都生出新問題**。〕
