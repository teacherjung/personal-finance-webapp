...
10. **註解寫「為什麼」，不寫「現在是」**（William 2026-08-08 拍板，#417 燒掉七輪換來的）——
    ...
    ⚠️ 不寫死會漂的數字與序數（「兩題」「下一題」「共 N 處」）——點名 `file:line` 或題名關鍵字。
      **指同一支考題檔裡的另一題時，在註解裡寫「題名關鍵字」＋全形引號夾住題名的獨特片段**：
      那個記號有機械閘（`test/comment-test-refs.test.js`）驗它在同檔**恰好命中一題**，
      指到 0 題或 2 題都會紅。⚠️ 這條不是潔癖：#463 九輪審查有**三輪**抓到同一族
      （會漂的序數且已指錯、關鍵字命中 0 題、舊題名指到別支檔案），全是人工核對漏掉的。
      閘的射程與擋不住什麼寫在該檔檔頭（不帶記號的引用、跨檔引用、可達性都不在射程內）。
    ⚠️ **註解與字串裡都不可以有控制字元**：一個看不見的位元組（例如把哨兵寫成 `\0`）就能讓 git
      把整支檔案判成二進位，**GitHub 上看不到 diff、審查者等於審不到**——#463 的護欄自己踩過，
      `test/reminder-thresholds.test.js` 也帶著一顆進了 main。同一支閘盯著。
...
| Grok | 教學線「外部記者」三職：腳本查核／每集開工時的選題雷達／就地解釋文案的零基礎讀者測試；程式線＝**預審參謀（試行）**＋**唯讀顧問（個案）**（完整邊界＝「Grok 的邊界」節） | 一切實作、正式複審與合併執行；**沙箱條款落地前不進任何 repo 樹**（主目錄／實作樹／審查樹——一律材料制，見該節）；X 輿情不得用於投資決策線 |
...
**Grok 的邊界（William 2026-08-15 拍板；管道實測 2026-08-14＝Grok CLI 可用，先立規矩再開門）**——Grok **不是第四方程式協作者**：教學線特約＋程式線試行參謀，角色表那列的完整邊界就是本節：
...
- **預審參謀（試行條款，William 2026-08-15 拍板；試滿裁決後本條改寫或刪除）**：Claude 實作完成、**送 Codex 正式審之前**，把 diff 抽出來給 Grok 掃一遍——Grok 不碰樹、不碰 repo：在 repo 外的工作目錄（如暫存區）餵材料檔、**不在任何 repo 目錄啟動它的 CLI**、且**必鎖工具**（關閉檔案讀寫、終端、目錄列舉與網搜，只留純文字問答——2026-08-15 #467 實測：不鎖時 Grok 會自行搜家目錄、`cd` 進主目錄跑 git＝材料制被繞過；逐字旗標抄 REVIEW-AND-MERGE「怎麼執行」節的前置檢查行；**旗標名單以 grok 1.0.3 實測工具清單為準——CLI 版本不同＝當跑不動**、五格記 0 附版本，由人重驗工具清單後才恢復）；材料以**已 commit、將上 GitHub 的內容**為限（與送 Codex 同一暴露等級），機密與真實資料不得入材料。發現由 Claude **逐條驗證**；修不修由 Claude 照「審查回饋處置」既有界線判斷（例外照舊問 William），**判定要修且屬本支 diff 者＝送審前修掉**、不留到正式輪。**Grok 意見不算正式複審、不進任何合併閘、不產生結論字串**（合併機器零改動）。試行＝Claude 實作的**連續 10 支 PR 全數入樣**（不挑支、**跳過不出分母**：跑不動＝該支五格記 0、附失敗原因與輸出，仍計入 PR 數——可自報的「技術跳過」就是挑樣本的後門），每支在 PR 描述附**同一份可稽核證據**：送掃前凍結的**自審已知清單**（可為空、要明說；**凍結方式＝清單與完整指示寫進材料檔本身**）＋**受掃版本＝精確 base／head SHA**（不可用 `origin/main` 這類會移動的名稱）＋**材料檔內容定址識別**（材料全文入附錄、或記 SHA-256）＋**呼叫配方聲明**（鎖工具旗標＋grok 版本——沒帶聲明的新掃描＝當未跑）＋**掃描時序**（送掃發生在哪一輪之前；事後補跑照實標——反事實失真怎麼打折＝期滿彙整由 William 裁）＋**Grok 回覆原文**＋**逐條判定**＋五格成績單（成績單是自報數字，**對帳依據＝逐項點名的這一組：凍結自審清單、精確 base／head SHA、材料定址、配方聲明、Grok 原文、逐條判定**——不用「前 N 樣」這種會隨插入順序漂的序數；缺自審清單，「獨家」就無法重算）。五格的操作定義：**發現數**＝Grok 列出的實質條目；**驗真數**＝Claude 判定屬實（不論輕重）；**誤報數**＝發現−驗真（不成立、幻覺、誤讀）；**獨家數**＝驗真且發稿者自審清單裡沒有（對照基準＝送掃當下已知，**不比事後 Codex**——修掉的問題 Codex 本來就看不到）；**有效獨家數**＝獨家＋屬本支 diff＋判定要修並送審前修掉（單看獨家會被風格建議灌水；這格量的是「沒有它，這支 PR 就帶著問題送審」的反事實價值）。期滿把 10 支**加總**算三率給 William 裁決轉常設／收回／調整：**驗真率**＝驗真÷發現、**有效獨家率**＝有效獨家÷PR 數（**主指標**；按定義是「每支平均件數」、可大於 1）、**誤報率**＝誤報÷發現（噪音成本）；任一率分母為 0＝該率記「**不適用**」並呈報原始數字，**不得默認成 0% 或 100%**。
