...
- **複審後掃（常設；William 2026-08-18 拍板**改序**——本條原名「預審參謀」＝送 Codex 前先掃，2026-08-16 轉常設、2026-08-18 改為複審後掃）**：Claude 實作的每支 PR，**Codex 給出「通過」之後、`gh pr ready` 轉正式之前**，把該版本的 diff（**`base..head` 兩顆都寫死 SHA**）抽給 Grok 掃一遍——**每支只掃這一次**（第一次「通過」之後）：修完再審、再通過**不重掃**。本條要的是「複審漏了什麼」的證據，不是替最終 head 背書。
  ...
  - **PR 描述固定用小標 `### Grok 複審後掃`**（釘住字串才 grep 得出哪幾支跑了——#483 被記成零分的根因就是紀錄格式沒被釘住），內容二選一：①下列各項（刻意不寫件數——寫死的數字自己會漂）；②逐字「**未執行：<原因>**」。
    - **受掃版本與配方聲明**：精確 `base..head`（兩顆都寫死 SHA、不可用 `origin/main` 這類會移動的名稱）＋鎖工具旗標＋grok 版本＋驗屍退出碼與 session id＋**掃描時用的目錄路徑**——寫了路徑，**同一台機器上的另一方（Codex／William）**可以自己重跑驗屍複查，這是全條最能被複查的一格（驗屍讀的是家目錄下的 session 日誌，GitHub 上的讀者重跑不了）。記的是**當時掃的那顆 head，之後修過不回填**——它記的是「掃了哪一版」，不是「目前是哪一版」。⚠️ 誠實劃界：修過之後，**真正被合併的那顆 head 未必被 Grok 看過**；接住它的是複審重跑，不是本條。
    - **Grok 回覆原文**。
    - **逐條判定**（三選一）：**屬實·已修**（附觸發了第幾輪複審）／**屬實·進待辦**（附待辦去處）／**誤報**。〔為什麼要三個值：只有「屬實／誤報」時，照「值不值得改」該進待辦的那一條只能被漏記或被標成誤報——而懲罰亂標誤報的絆線已一併廢止，被污染的正好是新制唯一剩下的數字。〕
    - **掃描時序**（一行）：對應**哪一則「通過」留言**（貼留言網址）＋掃描時間。⚠️ **必須是掃的當下那一則**——撈到東西修完之後又出現新的通過，這一行**不改指**（改指最新那則：欄位仍齊、SHA 仍對得上當時的 head、三個失效字串仍在，但時序是假的；而「缺這一行＝當未跑」擋得住缺，擋不住填錯指向——這一格沒有機械保證）。⚠️ **自報、無機械保證**——留著它不是因為它擋得住造假，而是因為沒有它連「聲稱的先後」都不在紀錄上（#486 r1 High：head SHA 對得上≠先後對得上）。**缺這一行＝當未跑**（與「版本不同＝當未跑」「驗屍非 0＝該掃作廢」同級：整條推論就靠先後，先後沒記＝這一遍不成立）。
  ...
  - **凡是本節判成「當未跑／不掃」的，一律同一個收口**（刻意不列舉理由、不寫數量——理由會長，寫死就會漏）：在固定小標下逐字寫「**未執行：<原因>**」，**不擋合併、不入任何計數**；要不要重掃是選項、不是義務。⚠️ **2026-08-22 起「作廢」這個字不再出現在新掃描裡**：沙箱制下驗屍不作廢（在盒子裡跑指令是准的），要嘛金絲雀不過＝**沒掃**（記「未執行」）、要嘛掃了＝**算數**；驗屍查到破口＝**事故**，另外處理。〔舊制留下 10 支 PR 裡 9 支記「未執行」、其中至少 2 支實際撈到真問題——紀錄又在說謊，這正是改沙箱的理由之一；那些舊紀錄保留原樣當歷史。〕⚠️ 作廢掃描的內容仍可人工驗證後採用，但**依定義列為 Claude 自審、不計 Grok 出勤**，且**不得寫在固定小標底下**（那一區是日後人工翻表的統計區，混進作廢掃描的發現＝製造新的假金標準）——另立標題放。⚠️ **本條沒有任何機械保證，全靠自律**——免跑理由一律自報、無人複查、不擋合併；要求記那一行，只為了讓事後 grep 得出哪幾支跑了、哪幾支沒跑。（舊版「停跑不是逃生門」那句有東西撐著＝漏跑入計數、不可洗銷、連 3 支重裁；本版把撐著它的全刪了，所以那句話一併刪掉，不留誇大。）
