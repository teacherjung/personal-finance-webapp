...
| PR 說明 | **必填欄位**（清單的單一真相＝`scripts/check-pr-collab-fields.js` 的 `REQUIRED_FIELDS`，考題雙向對帳，這裡刻意不寫數字）：實作者／獨立審查者／預計修改的共享檔案（＝預約）／這支若完全失敗最糟失去什麼；再加「實際改了什麼＋驗收法」 | 每支 PR。⚠️ **模板在 `.github/pull_request_template.md`、由 `scripts/check-pr-collab-fields.js` 機械把關**——2026-08-02 實測連續三支（#374/#375/#376）漏填，證明只靠記憶維持不住 |
