...
- **複審後掃（常設；William 2026-08-18 拍板**改序**——本條原名「預審參謀」＝送 Codex 前先掃，2026-08-16 轉常設、2026-08-18 改為複審後掃）**：Claude 實作的每支 PR，**Codex 給出「通過」之後、請 Codex 執行合併之前**，把該版本的 diff（**`base..head` 兩顆都寫死 SHA**）抽給 Grok 掃一遍——**每支只掃這一次**（第一次「通過」之後）：修完再審、再通過**不重掃**。本條要的是「複審漏了什麼」的證據，不是替最終 head 背書。
  - **為什麼是這個順序（這是本條存在的主要理由，不要改回去）**：掃在前面時，Grok 挑出的問題會在送審前就修掉——Codex 永遠看不到出問題的版本，於是「這條 Codex 自己會不會抓到」**結構上量不到**，只能問他本人（自評；而且他已讀過預審原文＝非盲測）。掃在後面，Codex 的「通過」已經留在紀錄上：**之後撈到的每一條屬實發現，定義上就是正式複審漏掉的**——證據自動成立，不必自評、不必記帳、不必算率。整段歷史目前僅有的一次硬證據（#463：Codex r7 明文「無須另補題」→ Grok 點名→ r8 對照實證後撤回自己的判斷）正是這個順序意外發生的。代價照實記：失去「先掃掉低級錯誤、省 Codex 一點時間」那個好處——但那個好處本來就是量不出來的那一半。
  ...
  - **PR 描述固定用小標 `### Grok 複審後掃`**（釘住字串才 grep 得出哪幾支跑了——#483 被記成零分的根因就是紀錄格式沒被釘住），內容二選一：①下列各項（刻意不寫件數——寫死的數字自己會漂）；②逐字「**未執行：<原因>**」。
    ...
    〔2026-08-18 刪掉的兩項與理由：材料檔 SHA-256——條款自己就寫明「無可重建前像＝擋不了換稿」，留著只是儀式；掃描時序——受掃 head SHA 與 Codex「通過」留言標頭裡的審查 sha 對得上，時序即自證，再記一次是複本。〕
