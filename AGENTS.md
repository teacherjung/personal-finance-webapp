...
| 帳單多銀行/多格式（`parseStatement` 依位元組偵測 PDF/XLSX 分流） | **富邦＝PDF**（`parseFubon`：郵寄加密版說明同列、官網無密碼版說明換行下一列、民國日期）；**台新＝XLSX**（`parseTaishinXlsx`＋SheetJS，官網下載「信用卡明細」，西元日期、金額獨立欄）。台新 PDF 排版翻頁不穩→擋下建議改 XLSX。各解析器回原始明細後共走 `finalize()`（分類＋國外交易服務費繼承）。新增銀行＝加 `parseXxx()` 在 `parseStatement` 分流。`parseStatementPdf` 為 `parseStatement` 別名（相容） |
