...
| **AI 解析路線 P1b**（P1b-1 2026-08-12，★3 拍板＝Anthropic） | 雙檔分工（ai-parse 純模組／ai-transport 唯一外連檔＝入外連登記閘；服務層不可 import ai-transport、真引擎由 statement.js 路由組裝注入）；四道規矩＝HOSTED 停止線寫死→useAi 同意旗標→settings.aiApiKey 機密欄→★6 AI 只收強閘；模型階梯 Haiku→閘紅升 Sonnet 一次；引擎交原始答案、服務層自驗收；**確認票**＝preview 發票、apply 憑票寫入同一份答案不重跑模型（AI 非確定性，r4#1）；**前端（P1b-2）**＝每次送出前跳同意窗（不記住）、`useAi` 嚴格布林、票只活在單次預覽閉包、AI 入口只認 `bank_unrecognized`（對帳閘紅無 code 走不到）、設定頁鑰匙欄不回顯——完整契約 → [契約：收支記帳與匯入](docs/contracts/income-expense.md#ai-解析路線-p1b) |
...
| **架構** | **模組邊界／資料流向／欄位歸屬，任一有改就算**——新增、移動、**刪除、縮減**都在內（抽成新模組、把函式從模組 A 搬到 B、刪掉一個沒人用的模組、收掉一個 exported 邊界，全部算） | **那三個維度都沒改**才不算。純搬移只是最常見的例子（同檔搬函式、同模組內拆區塊） |

⚠️ **架構那一列刻意寫成「互補」，不是列舉**：判準只有一句——**那三個維度有沒有任何一個變了**。
   「算」與「不算」因此**既互斥又窮盡**，不會有案例兩邊都不落。⚠️ **不要再改回「有沒有新增什麼」那種問法**：
   刪掉一個模組、收掉一個 exported 邊界、把欄位改歸屬，全都沒有「新增」任何東西，卻是道地的架構改動。
   （這一族 `Codex #457` 連兩輪抓到：r3 是把判準縮成「有沒有新模組」，r4 是「刪除／縮減」落在兩欄之外
   ——**列舉補不完，所以改成補集**。這是本專案認過的病型：列舉繞法補不完就要關門。）
