...
- **JSON 請求大小分流（2026-07-22）**：單一真相在 `lib/http-body.js`，一般 API＝1 MB、信用卡／銀行帳單吃檔案的大型 POST（清單＝http-body.js 的 STATEMENT_FILE_POST_ROUTES）（另有一個只吃列的端點，僅 HOSTED 收到 1MB）＝15 MB、完整備份還原 `/api/import`＝50 MB。**安裝順序是安全不變量**：大型端點的 route-specific parser 必須先掛，最後才掛一般 parser；倒過來會讓大件入口先被 1 MB 擋掉。新增會接收大型內容的端點時，要加入集中清單並補 `test/request-limits.test.js`；尤其 `/api/import` 是資料救援入口，絕不可繼承一般 1 MB 上限。
...
- **程式線＝常設「複審後掃」（材料制、不進樹）＋進樹一律關門**〔⚠️ 2026-08-19 更正：原標題把程式線寫成「預設關門」，那是 2026-08-16 轉常設**之前**的狀態，與本節開頭「程式線常設『複審後掃』」直接打架。關門的是**進樹**，不是**掃**〕：未經 William 指名，Grok 不在本 repo 讀寫檔案、跑指令、開分支。指名時**同樣走材料制**：發稿者把指定範圍的檔案內容打包給它、在 repo 外目錄啟動，Grok **不進任何樹、不跑 repo 程式**（Codex r1：git worktree 只是另一個 checkout，對「會跑指令的未信任 CLI」不構成隔離保證）。要讓 Grok 進樹＝先在本節補上**已驗證的沙箱條款**、William 拍板，才開門。
- **複審後掃（常設；William 2026-08-18 拍板**改序**——本條原名「預審參謀」＝送 Codex 前先掃，2026-08-16 轉常設、2026-08-18 改為複審後掃）**：Claude 實作的每支 PR，**Codex 給出「通過」之後、`gh pr ready` 轉正式之前**，把該版本的 diff（**`base..head` 兩顆都寫死 SHA**）抽給 Grok 掃一遍——**每支只掃這一次**（第一次「通過」之後）：修完再審、再通過**不重掃**。本條要的是「複審漏了什麼」的證據，不是替最終 head 背書。
  ...
  - **呼叫紀律**：repo 外目錄餵材料檔（邊界前綴＋指示＋diff）、**不在任何 repo 目錄啟動它的 CLI**、**必鎖工具**（不鎖會越界＝#467 實測：自行搜家目錄、`cd` 進主目錄跑 git；逐字旗標與版本釘抄 REVIEW-AND-MERGE「怎麼執行」節的**鎖工具配方那一段**——**CLI 版本不同＝當未跑**、人工重驗工具清單後更新；**掃後必驗屍**＝跑 `node scripts/audit-grok-scan.js --workspace <掃描 cwd>` 機械稽核該次 session 日誌，**退出碼非 0＝該掃作廢**——2026-08-17 實測鎖工具旗標同版本會非決定性靜默失效，旗標只當第一層，**圍欄靠驗屍、不靠旗標的運氣**（William 拍板）。⚠️ **名實劃界（2026-08-18 自審）**：舊制下驗屍非 0 會記漏跑、入絆線計數＝有壓力逼出一遍乾淨的掃；本版把計數全刪了，**驗屍只證明「這一掃乾不乾淨」，不再強迫補一次乾淨的掃**（作廢後記一行即可出貨）。重掃是選項、不是義務——這句話的支撐比舊版弱，照實寫在這裡，不留誇大）。材料以**已 commit、將上 GitHub 的內容**為限（與送 Codex 同一暴露等級），機密與真實資料不得入材料。
  ...
  - **凡是本節判成「當未跑／該掃作廢」的，一律同一個收口**（刻意不列舉理由、不寫數量——理由會長，寫死就會漏）：在固定小標下逐字寫「**未執行：<原因>**」，**不擋合併、不入任何計數**；要不要鎖工具重掃是選項、不是義務。⚠️ 作廢掃描的內容仍可人工驗證後採用，但**依定義列為 Claude 自審、不計 Grok 出勤**，且**不得寫在固定小標底下**（那一區是日後人工翻表的統計區，混進作廢掃描的發現＝製造新的假金標準）——另立標題放。⚠️ **本條沒有任何機械保證，全靠自律**——免跑理由一律自報、無人複查、不擋合併；要求記那一行，只為了讓事後 grep 得出哪幾支跑了、哪幾支沒跑。（舊版「停跑不是逃生門」那句有東西撐著＝漏跑入計數、不可洗銷、連 3 支重裁；本版把撐著它的全刪了，所以那句話一併刪掉，不留誇大。）
