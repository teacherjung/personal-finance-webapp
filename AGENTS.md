...
**重疊 PR 讓道（2026-08-29 #526 擋住 #519 的教訓；William 2026-08-29 排定入冊＝候選 8-18）**：要動的檔案與**在途 PR 已宣告的預約**重疊時，**不要開著自己的 PR 等**——跨 PR 試合併閘把受審支和每一支 open PR **真的合起來跑三關**（文字衝突與「零檔案重疊、合起來測試紅」**都擋**——後者正是它的立閘事故型 #384/#385），**但不讀欄位裡的意圖**：在說明寫「不會搶」沒有機械效力，開著的 PR 就是閘眼中的衝突源，先預約的支反而被後開的擋住（#526 未送審就擋住了已審完六輪、走到合併最後一步的 #519，靠限時協調暫關才解）。**讓道＝上面「停下協調」的協調結果之一，不是繞過協調**；做法二選一：①**晚開**——先在**持有人的 open PR** 留言宣告「讓道中：本人在做 Y、會動到重疊檔 X，待本支合併後開 PR」，然後工作照做、commit 照推，等對方合併、自己 rebase 完再開 PR。⚠️ 那則留言是讓道期間的**弱宣告**（人查得到、閘與 `gh pr list` 讀不到——沒開的 PR 不在「誰在做什麼」的唯一即時來源上），所以對方合併後**盡快開 PR 恢復正式宣告**。②**開了暫關**——closed 的 PR 閘不看（`scripts/check-cross-pr-merge.js` 現行只撈 `--state open`），隨時可 reopen、號碼與討論都保留；讓道留言**自己的 PR 與持有人的 PR 各留一則**、寫明「讓道、非放棄、誰負責重開」（只寫在 closed PR＝之後沒有任何流程會再讀到它）。**重開或晚開後開 PR 之前，重查一次在途 PR 宣告**——可能還有別的讓道者在等同一批檔案。〔刻意沒有「把衝突 hunk 改到不撞」這個選項：hunk 不撞仍可能「合起來測試紅」被擋，那正是這道閘存在的理由。〕
