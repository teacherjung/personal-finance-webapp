...
- **複審後掃（常設；William 2026-08-18 拍板**改序**——本條原名「預審參謀」＝送 Codex 前先掃，2026-08-16 轉常設、2026-08-18 改為複審後掃）**：Claude 實作的每支 PR，**Codex 給出「通過」之後、請 Codex 執行合併之前**，把該版本的 diff（**`base..head` 兩顆都寫死 SHA**）抽給 Grok 掃一遍——**每支只掃這一次**（第一次「通過」之後）：修完再審、再通過**不重掃**。本條要的是「複審漏了什麼」的證據，不是替最終 head 背書。
  ...
  - **凡是本節判成「當未跑／該掃作廢」的，一律同一個收口**（刻意不列舉理由、不寫數量——理由會長，寫死就會漏：目前至少含圍欄失效、驗屍非 0、CLI 版本不符、CLI 不可用、缺時序那一行）：在固定小標下逐字寫「**未執行：<原因>**」，**不擋合併、不入任何計數**；要不要鎖工具重掃是選項、不是義務。⚠️ 作廢掃描的內容仍可人工驗證後採用，但**依定義列為 Claude 自審、不計 Grok 出勤**，且**不得寫在固定小標底下**（那一區是日後人工翻表的統計區，混進作廢掃描的發現＝製造新的假金標準）——另立標題放。⚠️ **本條沒有任何機械保證，全靠自律**——免跑理由一律自報、無人複查、不擋合併；要求記那一行，只為了讓事後 grep 得出哪幾支跑了、哪幾支沒跑。（舊版「停跑不是逃生門」那句有東西撐著＝漏跑入計數、不可洗銷、連 3 支重裁；本版把撐著它的全刪了，所以那句話一併刪掉，不留誇大。）
