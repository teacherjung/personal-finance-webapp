...
| **顯示標記 `applyDisplayLabels(name, {desc, subcategory})`**（使用者定 2026-07-18） | **只加在顯示名（交易 `note`）、絕不進 `storeKey`**——storeKey 是學習用身分鑰匙，混入標記會讓既有學習對不上。標記需要 `cleanStore` 看不到的上下文，所以由「知道上下文的三處」呼叫：`conformTxs`（匯入預覽）、`importRows`（寫入 note）、`normalizeBranches`（既有資料整理，原文從 `stmtRef` 第 4 段取回）。①**FP 外送**：帳單原文符合 `FP_PREFIX`（`FP`＋分隔符；要求有分隔符，免誤傷本來就叫 FPxxx 的店）→ 店名尾加「（FP）」。②**停車**：**子類＝`停車費`** → 包成「停車費（原店名）」——**用分類、非店名字面**，才涵蓋得到嘟嘟房/阜爾運通/Times Parking 這種名字沒有「停車」二字的停車場（使用者定：一律套用）。兩者皆冪等。`renameStoreDisplay` 的 `autoName` 也要套同一組標記，否則「打開編輯窗直接儲存」會把帶標記的現值誤記成自訂名 |**`branchNormalize`**：把「主體＋分店」統一成全形括號 `主體（分店）`（`統一超商-百福`→`統一超商（百福）`、`IKEA-新莊`→`IKEA（新莊）`）——**有分隔符（-／－…）者，分店限「純中文起頭」才切**（故 `ABC-MART`/`LADY-M` 不會被誤切）；**無分隔符的連鎖需列進 `BRANCH_CHAINS` 白名單**（目前＝`誠品生活`，因電腦無法自行判斷主體邊界）；冪等（已是 `…（分店）` 不再包）、結尾外幣註記 `（USD/9.99）` 先摘再接回。**`applyBrandCanon`**：品牌正規化表 `BRAND_CANON`（{prefix,brand}）——把「主體＋分店」拆開重組成標準品牌名，有分店才加括號（目前＝`台亞`/`台亞加油站`任何寫法→`台亞加油站`，`台亞林口第二交流道南站`→`台亞加油站（林口第二交流道南站）`；比 BRAND_RENAME 強、用於「連鎖名要換且要切分店」）。**`applyBrandRename`**：品牌顯示簡稱字串取代表 `BRAND_RENAME`（目前＝`全家便利商店`→`全家商店`、`台灣普客二四`→`Times Parking`，保留分店；純字串替換、用於「只換名不切分店」；使用者提出後加一條）。既有資料整理＝設定頁「店名格式整理」（**可重複執行**——每次新增整理規則後再跑一次即可套到舊資料）→ `POST /api/statement/normalize-branches`（用 `normalizeStoreDisplay`；`dryRun` 預覽 note before→after；套用同時對齊 storeKey 與學習表 key，save 自動 .bak）。台新 PDF/XLSX 都常截斷店名（`LOUISA COFFE`/`台灣國際開`），救不回被截字（盡力而為）。無公開店名對照表可匯入。**新增銀行＝加 `parseXxx()` 並補進 `parsePdfAuto`。`parseStatementPdf` 為 `parseStatement` 別名（相容） |
