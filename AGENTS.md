...
10. **註解寫「為什麼」，不寫「現在是」**（William 2026-08-08 拍板，#417 燒掉七輪換來的）——
    ...
    ⚠️ 不寫死會漂的數字與序數（「兩題」「下一題」「共 N 處」）——點名 `file:line` 或題名關鍵字。
      **指同一支考題檔裡的另一題時，寫成「題名關鍵字」＋全形引號夾住題名的獨特片段**：
      那個記號有機械閘（`test/comment-test-refs.test.js`）驗它在同檔**恰好命中一題**，
      指到 0 題或 2 題都會紅。⚠️ 這條不是潔癖：#463 五輪審查有**三輪**抓到同一族
      （會漂的序數且已指錯、關鍵字命中 0 題、舊題名指到別支檔案），全是人工核對漏掉的。
      閘的射程與擋不住什麼寫在該檔檔頭（不帶記號的引用、跨檔引用都不在射程內）。
...
| **AI 解析路線 P1b**（P1b-1 2026-08-12，★3 拍板＝Anthropic） | 雙檔分工（ai-parse 純模組／ai-transport 唯一外連檔＝入外連登記閘；服務層不可 import ai-transport、真引擎由 statement.js 路由組裝注入）；四道規矩＝HOSTED 停止線寫死→useAi AI 要求旗標（確認窗僅 aiAskBeforeSend 開啟時出現）→settings.aiApiKey 機密欄→★6 AI 只收強閘；模型階梯 Haiku→閘紅升 Sonnet 一次；引擎交原始答案、服務層自驗收；**確認票**＝preview 發票、apply 憑票寫入同一份答案不重跑模型（AI 非確定性，r4#1）；**前端（P1b-2）**＝認不出版面＝預設**直接送**（`aiAskBeforeSend` 開關可改回每次問；2026-08-13 拍板，取代「每次跳同意窗」）（不記住）、`useAi` 嚴格布林、票只活在單次預覽閉包、AI 入口只認 `bank_unrecognized`（對帳閘紅無 code 走不到）、設定頁鑰匙欄不回顯——完整契約 → [契約：收支記帳與匯入](docs/contracts/income-expense.md#ai-解析路線-p1b) |
