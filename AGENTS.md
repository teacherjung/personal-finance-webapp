...
這是三方協作（使用者 + Claude Code + Codex）的**單一真相來源**。動手前的**全域必讀**＝本檔「🛑 錢的絕對邊界」「鐵則（違反會壞事）」與「協作流程」的角色邊界三處——**不分任務一律適用、不經路由**；其餘先查 `docs/contracts/README.md` 路由表、讀命中的契約（全文太大，「整本先讀完」實際上做不到——CLAUDE.md 同款提醒）；改動若牽涉本文件的規則，請一併更新本文件。**協作規矩分住三份檔案（本檔／`REVIEW-AND-MERGE.md`／`CLAUDE.md`），哪一條在哪一節＝[COLLAB-MAP.md](COLLAB-MAP.md) 路由表**（只指路、不是正本、無規則效力，且刻意不完整——衝突時以本檔為準）。
