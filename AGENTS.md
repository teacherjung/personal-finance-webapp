...
| 支出分類（兩層：分類/子類，**使用者可自訂** 2026-07） | **生效樹＝`settings.expenseTree`（缺→用 `public/modules/categories.js` 的 `EXPENSE_TREE` 預設）**，統一由 `lib/services/categories.js` 的 `effectiveTree(db)` 提供。`EXPENSE_TREE` 現為**預設/種子**（非唯一真相）；`statement.js CATEGORY_RULES` 與 `core.js CATEGORY_MIGRATION` 的目標分類仍對得上「預設樹」的名字即可。**前端下拉不再讀常數**：`transactions.js` 每次 render 打 `GET /api/categories` 取生效樹（收入類仍走固定 `INCOME_CATEGORIES`、無子類）。設定頁「分類管理」編輯器 → `POST /api/categories`（body `{tree, parentRenames, subRenames}`）＝`saveTree`：**改名連動**既有交易與 `learnedCategories`、**刪除**的分類把交易改歸「其他/未分類」（只動 `type==='expense'` 且原分類在樹內者——收入/繳款不誤動）；改名 vs 刪+增用 `renames` 明確標記（diff 分不出）。**`其他/未分類` 是退路，`sanitizeTree` 強制保留、編輯器鎖定不可刪改**。匯入時 `conform(tree,cat,sub)` 把分類器輸出校正到生效樹（內建分類被使用者刪/改名→落「其他/未分類」，靠學習自我修正）。`schema.js sanitizeSettingsDeep` 驗 `expenseTree` 形狀。交易存 `category`+`subcategory`；未知支出預設 `DEFAULT_EXPENSE`（其他/未分類——與「生活/其他生活雜支」區隔：後者是已知生活雜項，前者是還沒判斷出來的） |
