...
> 3. **偏離要合進 main＝William 驗收過**（他點頭＝驗收）；合併後**把新做法回寫本節**——規則跟著定案走，本節永遠描述「現在的預設」。
> 4. **頁面級視覺考題（`*-forest-ui`／`portfolio-tables` 這類版面與格式釘）與本節同權**：UI PR 偏離慣例時，考題在**同一支 PR** 連動調整＝照章辦事、不算弱化違規。**不可連動放寬的鐵則面**：`esc()`／XSS、數值正確性（畫面上的數字必須等於正確計算的值）、欄位錯位（資料列格數）、PII 投影——這些不是視覺、是安全與正確，照舊受審查制度與「弱化考題先讀契約」約束。
