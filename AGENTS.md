...
兩個固定關卡：**合併前一句話檢查**——PR 說明要回答「這支若完全失敗，最糟會失去什麼？」；**合併後五分鐘檢查**——**依 `REVIEW-AND-MERGE.md` 合併步驟「回報合併結果與驗收分級」那一步的分級**（正本在那裡，本檔不抄清單）：要重啟的那級才由 William 重啟 App、以實際操作完成最核心的一條流程（＝每支 PR 附的「怎麼驗收」三句）；只動前端的那級重新整理看一眼；只動考題／文件／腳本的那級不需驗收（William 2026-09-06 體檢第 5 題裁示分級——此前本句寫成每支都要重啟，與合併步驟那一步、以及「標準全流程只適用高風險與新功能」互相矛盾）。
