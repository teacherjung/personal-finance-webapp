...
> - Claude Code 權限層已封鎖（`.claude/settings.json`，進版控）：`permissions.deny` 精確點名兩支工具全名，加上 `PreToolUse` deny hook（**家族攔截網＋白名單制**：判斷只有一份＝套件的 `tools/forbidden-tools.js`，清單只有一份＝根目錄 `settings.json` 的 `forbidden` 那一塊（八欄：連接器、白名單、逐字拒絕清單、動詞、名詞、唯讀前綴、額外樣式、名稱），鉤子那一組逐字等於 `templates/hook-claude.json`、每次呼叫重讀那棵樹的清單（改檔、切分支下一次呼叫就生效）；matcher 只是 `^mcp__` 粗篩；找不到 node 或檔＝退 2、所有 mcp__ 工具全擋（看得見的失敗，不是靜靜放行）；這裡刻意不重抄清單以免長出會漂的副本——**家族網那一層**只認工具名、不認連接器 UUID，連接器重連換了 UUID 照樣擋（⚠️ 白名單那一層**認 UUID**，換了就失效——見下面的誠實劃界）；唯讀動詞前綴 `get_`/`list_`/`search_` 等放行，所以不誤傷 `get_order_instructions` 等查詢工具）。python v6 那組（2026-09-02 起、詞表寫死在指令本體）2026-09-16〜09-18 曾與套件那組並存，2026-09-18 搬家第 8 步拆掉 python 那組（William 裁第 15 題 a：兩層考題都綠、並存一個完整變更週期無誤擋才拆）。⚠️ 拆掉之後**除了 `permissions.deny` 那兩支全名，沒有第二層**：其餘清單（連接器、白名單、家族網詞表）套件那組每次呼叫都活讀那棵樹的 `settings.json`——在實作樹上把 forbidden 改弱、或切到弱清單的分支，下一次呼叫就生效，沒有第二層兜底（python 那組以前是這個兜底；Codex 全域層因為讀倉庫外的固定複本＋指紋，沒有這個問題）。⚠️ **`.claude/settings.json` 本身在對話進行中被改，會不會當場生效＝本機沒量過**（鉤子那一行沒量過、`permissions.deny` 也沒量過）。鉤子那一行：平台官方鉤子文件（2026-09-18 讀）寫「設定檔裡鉤子的改動通常會被檔案監看自動接走」。`permissions.deny`：文件那一句沒有講到它。**兩者都不要當成「對話期間釘得住」**——這是閱讀方針（往保守的方向假設），不是量測結果，也不是在說它們「一定不釘」〔2026-09-18 更正：原寫「會話期釘住的只剩 `permissions.deny` 那兩支全名」，是沒量過的保證〕。要不要讓 Claude 側也釘指紋＝William 2026-09-18 裁 b（要釘；套件設計的改動，先在套件改、審過再同步進來；做法還在比較、還沒動工——在那之前本段講的現況不變）。William 機器的 user 層 `~/.claude/settings.json` 另有 v5 同款封鎖（不在 repo；拆舊層時未重驗）。
> - 考題＝`test/money-boundary.test.js`（本節條文的承重句、deny 清單、鉤子逐名配對：該擋的擋、可用名不誤傷、壞輸入 fail-closed、起不來單獨報）、`test/money-kit-hook.test.js`（那一組逐字等於範本且剛好一組、整張字表只餵它、不靠啟動目錄、找不到 node 退 2）、`test/codex-global-hook.test.js`（Codex 側：從這棵樹抽固定複本、真的跑全域層那條指令）、`test/money-canary.test.js`（測試鈕）；探針字表共用 `test/helpers/money-family-probes.js`（位元組由 `test/money-family-probes-integrity.test.js` 釘）。
> - **Codex 側（2026-09-17 起走家目錄全域層；專案層副本 `.codex/hooks.json` 2026-09-18 搬家第 8 步刪除，William 裁第 14 題 a、刪前在 Codex /hooks 看過它沒在跑）**：`~/.codex/hooks.json` 的那一組讀 `tools/guard-copy.js` 從**已合併進主幹**的版本抽出、放在倉庫外的固定複本（接線範本＝`templates/hook-codex-global.json`：複本路徑與指紋寫死在指令裡，複本不在、被改過、有連結或佔位沒換＝退 2 全擋；攔截器說拒絕＝改印到錯誤輸出、退 2）。改到 forbidden 那一塊、判斷程式、`tools/settings-data.js`、`tools/package.json` 或那份範本＝合併後重抽複本、關掉所有 Codex 視窗、換組、William 重按信任、按測試鈕（順序＝`tools/guard-copy.js` 檔頭⓪〜⑥）。⚠️ **它是條件式的**：Codex 的 hook 要 William 在 Codex 介面（`/hooks`）按過「信任」才會執行，信任狀態按指令那一行的雜湊存在 `~/.codex/config.toml`——**不在 repo**，改到那一行就要重按；**未信任時 `codex exec` 一聲不吭地跳過**（2026-09-01 本機實測）。所以「repo 裡有範本、家目錄有那一組」證明不了「Codex 那邊擋得住」——只有在真的 Codex 對話按測試鈕看到「在拒絕清單上」才算數（2026-09-17 量過一次）。
