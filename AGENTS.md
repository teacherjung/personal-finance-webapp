...
- **身分與管道**：William 機器裝有 Grok CLI（xAI）。實測它進本 repo 會**自動讀本檔與 CLAUDE.md**——寫在這節的字它開機就看得到；專案對它維持「未信任」預設。
...
- **程式線＝預設關門＋唯讀顧問（個案）**：未經 William 指名，Grok 不在本 repo 讀寫檔案、跑指令、開分支。指名時比照 Codex 模式①：只在該案的拋棄式審查樹（發射者備樹收樹）、絕對唯讀（不改檔、不 commit、不 push、不 checkout）。
- **預審參謀（試行條款，William 2026-08-15 拍板；試滿裁決後本條改寫或刪除）**：Claude 實作完成、**送 Codex 正式審之前**，把 diff 抽出來給 Grok 掃一遍——Grok 不碰樹、不碰 repo：在 repo 外的工作目錄（如暫存區）餵材料檔、**不在任何 repo 目錄啟動它的 CLI**；材料以**已 commit、將上 GitHub 的內容**為限（與送 Codex 同一暴露等級），機密與真實資料不得入材料。發現由 Claude **逐條驗證**；修不修由 Claude 照「審查回饋處置」既有界線判斷（例外照舊問 William），**判定要修且屬本支 diff 者＝送審前修掉**、不留到正式輪。**Grok 意見不算正式複審、不進任何合併閘、不產生結論字串**（合併機器零改動）。試行＝Claude 實作的**連續 10 支 PR 每支都跑**（不挑支；技術原因跳過要在 PR 描述記明、該支不計入 10 支），每支在 PR 描述記五格成績單並**附 Grok 回覆原文＋逐條判定**（成績單是自報數字，原文才是 William 抽查時的對帳依據）。五格的操作定義：**發現數**＝Grok 列出的實質條目；**驗真數**＝Claude 判定屬實（不論輕重）；**誤報數**＝發現−驗真（不成立、幻覺、誤讀）；**獨家數**＝驗真且發稿者自審清單裡沒有（對照基準＝送掃當下已知，**不比事後 Codex**——修掉的問題 Codex 本來就看不到）；**有效獨家數**＝獨家＋屬本支 diff＋判定要修並送審前修掉（單看獨家會被風格建議灌水；這格量的是「沒有它，這支 PR 就帶著問題送審」的反事實價值）。期滿把 10 支**加總**算三率給 William 裁決轉常設／收回／調整：**驗真率**＝驗真÷發現、**有效獨家率**＝有效獨家÷PR 數（**主指標**；按定義是「每支平均件數」、可大於 1）、**誤報率**＝誤報÷發現（噪音成本）。
- **主目錄禁區（隱私線，任何模式無例外）**：不得在主目錄工作——`data/store.db` 有真實餘額、IB flexToken、證件號，Grok 讀到的內容會送往 xAI 伺服器。獲授權進樹時一律 worktree（結構上讀不到 store.db，同 Codex 紀律）。
