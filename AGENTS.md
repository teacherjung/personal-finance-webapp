...
| **機構名正規化與祖父比對形**（Stage 4，2026-08-22） | 機構名進去重鍵／機構戳／定存鍵三處；入口唯一＝`stmtBank`→`canonicalBank` 正規短名（台新任何寫法壓回「台新」＝去重鍵走內建範本的祖父格式），既有資料**不改**、比對時兩邊正規化（`sameBank`／`canonRef`／`canonCdKey`）；只剝通用後綴＋別名表、不猜同義詞——完整契約 → [契約：收支記帳與匯入](docs/contracts/income-expense.md#機構名正規化與祖父比對形) |
