...
| **顯示標記 `applyDisplayLabels(name, {desc, subcategory})`**（使用者定 2026-07-18） | **只加在顯示名（交易 `note`）、絕不進 `storeKey`**——storeKey 是學習用身分鑰匙，混入標記會讓既有學習對不上。標記需要 `cleanStore` 看不到的上下文，所以由「知道上下文的三處」呼叫：`conformTxs`（匯入預覽）、`importRows`（寫入 note）、`normalizeBranches`（既有資料整理，原文從 `stmtRef` 第 4 段取回）。①**FP 外送**：帳單原文符合 `FP_PREFIX`（`FP`＋分隔符；要求有分隔符，免誤傷本來就叫 FPxxx 的店）→ 顯示成「**主體（FP）**」——**不留分店**（外送是線上點的、出餐分店是雜訊；使用者定 2026-07-18：`12MINI（桃園龜山復興一店）（FP）`→`12MINI（FP）`），尾端分店括號會被摘掉、外幣註記保留。②**停車**：**子類＝`停車費`** → 包成「停車費（原店名）」——**用分類、非店名字面**，才涵蓋得到嘟嘟房/阜爾運通/台灣普客二四 這種名字沒有「停車」二字的停車場（使用者定：一律套用）。**例外白名單 `PARK_WRAP_EXEMPT`**（使用者定 2026-07-18）：「儲值」不是在某停車場繳費、包成「停車費（eTag自動儲值）」語意不對 → 名字命中白名單（目前＝`eTag自動儲值`）維持原名不包；新的由使用者提出後加一條。兩者皆冪等；順序＝FP 先、停車包外層。`renameStoreDisplay` 的 `autoName` 也要套同一組標記，否則「打開編輯窗直接儲存」會把帶標記的現值誤記成自訂名 |**`branchNormalize`**：把「主體＋分店」統一成全形括號 `主體（分店）`（`統一超商-百福`→`統一超商（百福）`、`IKEA-新莊`→`IKEA（新莊）`）——**有分隔符（-／－…）者，分店限「純中文起頭」才切**（故 `ABC-MART`/`LADY-M` 不會被誤切）；**無分隔符的連鎖需列進 `BRANCH_CHAINS` 白名單**（目前＝`誠品生活`，因電腦無法自行判斷主體邊界）；冪等（已是 `…（分店）` 不再包）、結尾外幣註記 `（USD/9.99）` 先摘再接回。**`applyBrandCanon`**：品牌正規化表 `BRAND_CANON`（{prefix,brand}）——把「主體＋分店」拆開重組成標準品牌名，有分店才加括號（目前＝`台亞`/`台亞加油站`任何寫法→`台亞加油站`，`台亞林口第二交流道南站`→`台亞加油站（林口第二交流道南站）`；`台灣普客二四`/`Times Parking` 任何寫法→`台灣普客二四`——2026-07-18 使用者定改回中文名、**反轉**先前的→Times Parking；分店判斷前會先修公司字尾殘尾；比 BRAND_RENAME 強、用於「連鎖名要換且要切分店」）。**`promoteGenericBody`**（雜訊主體拆殼，2026-07-18）：白名單主體（`聯信`＝收單方、`停車場`＝類別詞，絕不會是真店名）＋括號 → 拆出括號內容當店名、修殘尾、再過一次品牌正規化——治舊規則產出的 `聯信（Times Parking股份有）`→`台灣普客二四`、`停車場（俥亭停車）`→`俥亭停車`；括號內單獨 `Times`＝普客二四舊縮寫。**`applyBrandRename`**：品牌顯示簡稱字串取代表 `BRAND_RENAME`（目前＝`全家便利商店`→`全家商店`，保留分店；純字串替換、用於「只換名不切分店」；使用者提出後加一條）。既有資料整理＝設定頁「店名格式整理」（**可重複執行**——每次新增整理規則後再跑一次即可套到舊資料）→ `POST /api/statement/normalize-branches`（note＝先 `stripDisplayLabels` 拆掉顯示標記→`normalizeStoreDisplay`→`applyDisplayLabels` 重上標記——不拆殼的話「停車費（…）」巢狀括號治不到；`dryRun` 預覽 note before→after；套用時 storeKey **有帳單原文者用 `cleanStore(orig)` 重算**（規則升級才套得全）、學習表 key 跟著 storeKey 舊→新對照搬家、學過的 `name` 值也用同規則治一次（回 `learnedNamesFixed`，UI toast 會念出來），save 自動 .bak）。台新 PDF/XLSX 都常截斷店名（`LOUISA COFFE`/`台灣國際開`），救不回被截字（盡力而為）。無公開店名對照表可匯入。**新增銀行＝加 `parseXxx()` 並補進 `parsePdfAuto`。`parseStatementPdf` 為 `parseStatement` 別名（相容） |
