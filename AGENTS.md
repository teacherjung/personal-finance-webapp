...
⚠️ **表外的具名例外（這兩件不隨實作者對調，用角色寫、不寫人名）**：**不論誰實作，「發射 Grok 複審後掃」由起得了掃描器的那一方做；「把草稿 `gh pr ready` 轉正式」由發射掃描的那一方做**（William 2026-09-11 裁「甲」＝改成角色寫法、現況照舊，落點＝https://github.com/teacherjung/personal-finance-webapp/pull/601#issuecomment-5632426208；**本專案現況＝兩件都是 Claude**，換專案時要改的是「誰起得了掃描器」這個事實，不是這兩句）。⚠️ **兩件的理由不同等級**（Grok #596 掃後拆開）：**掃**＝技術限制（本檔「Grok 的邊界」那三件扣起來：外層沙箱、第二層套不上、金絲雀 fail-closed——方向無關誰實作）；**轉正式**＝跟著發射者走、少一次交接（William 2026-09-10 裁「維持你轉。」，落點＝#592；他選「一律同一方」是為了少一次交接，**不是**掃描器起不來；2026-09-11 改成角色寫法時這一點照舊）。⚠️ **這兩件都不是改檔／commit／push**，所以**坐在 ① 審查時做這兩件不違反 ①**。⚠️ `REVIEW-AND-MERGE.md`「實作模式」第三層只寫「即使 Codex 實作」那個方向；發射者自己實作時由發射者轉正式是既有預設，這裡的「不論誰實作」比那一節寬一截、照實記。那一節自己也寫著「**不可以整節把人名對調**」。⚠️ **這一行與上表 2026-09-11 起都被逐字釘住**（`test/collab-invariant-docs.test.js` 的 `MODES_TABLE`；改一個字就紅）。⚠️ **逐字釘只保證文字沒被動，不保證這兩件在實務上被遵守**——沒有任何機器在看「是誰發射掃描、是誰轉正式」，也沒有機器在判「誰起得了掃描器」（那要用本檔「Grok 的邊界」那種實測證明、不是自報），那一半靠角色紀律。
...
- **複審後掃（常設；William 2026-08-18 拍板**改序**——本條原名「預審參謀」＝送 Codex 前先掃，2026-08-16 轉常設、2026-08-18 改為複審後掃）**：每支 PR，**不論實作者是誰**（William 2026-09-09 **對話中**裁「Grok 掃要對稱」，原話逐字「a. Grok掃要對稱」，⚖️ 落點＝#588 裡他的裁示留言 https://github.com/teacherjung/personal-finance-webapp/pull/588#issuecomment-5602749004 ；此前本條只掃 Claude 實作的支），**獨立審查者給出「通過」之後、`gh pr ready` 轉正式之前**，把該版本的 diff（**`base..head` 兩顆都寫死 SHA**）抽給 Grok 掃一遍——**發射＝起得了掃描器的那一方**（William 2026-09-11 裁「甲」＝改成角色寫法、現況照舊；**本專案現況＝Claude**：Claude 實作的支＝Codex 通過後 Claude 掃；Codex 實作的支＝Claude 複審通過後自己掃。為什麼現況是 Claude——**三件事扣起來**（Grok #588 掃後改寫、Codex r3 再補上中間那件；原本拿單支 PR 的跳過題數當理由，那既弱又會漂）：①發審查的指令列**帶著外層沙箱**（可執行的那一行在 `REVIEW-AND-MERGE.md`「使用方式」節＝`-s workspace-write`，**本檔沒有第二份指令列**；Codex 自己的預設也是這個模式）；②**這個外層環境實測套不上第二層**——不是推論：Codex #588 r3 在自己的暫存盒子裡跑掃描器的沙箱能力探測，退 71、`sandbox_apply: Operation not permitted`；③掃描器的金絲雀**套不上沙箱就整支不掃**（fail-closed，不是「跳過幾題還能掃」）。三件扣起來＝照現行方式起的 Codex 起不了這支掃描器，所以「起得了的那一方」今天只有 Claude。〔旁證，不是理由：#587 r1〜r8 八輪都記錄那批要沙箱的考題被跳過（2026-09-09 只查這一支）。〕⚠️ 沒試過不帶外層沙箱起 Codex、也**不為了掃這樣起**——要換「誰起得了」的現況＝先用同樣的實測證明、再**先問 William**（那等於動 Grok 的角色邊界，本節開頭那條）；⚠️ 「誰發射」**沒有任何考題或閘在看**，跟「掃有沒有真的發生」同一級——**每支只掃這一次**（第一次「通過」之後）：修完再審、再通過**不重掃**。本條要的是「複審漏了什麼」的證據，不是替最終 head 背書。
