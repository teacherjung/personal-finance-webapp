...
| **匯入對帳閘**（`lib/statement-reconcile.js`，P0 2026-08-11） | 強＝銀行餘額鏈＋真末筆對概要（首筆未驗＝誠實計數）、中＝摘要等式擋下＋明細對總額**影子不擋**、弱＝沒數字可對；不一致＝整份 400、缺數字＝skip 降級放行；卡閘只在預覽（importRows 不重解析）；★6：AI 路線（P1）弱閘不准匯入——完整契約 → [契約：收支記帳與匯入](docs/contracts/income-expense.md#匯入對帳閘) |
