...
②`CODEX-REVIEW.md`「固定維度」那份下限清單存在的理由就是這件事——
...
⇒ 複審**不論作者的重點清單寫了什麼**，這幾條一律要跑（清單在 `CODEX-REVIEW.md`「固定維度」節）。
