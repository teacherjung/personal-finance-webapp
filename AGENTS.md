...
| 模式 | 能做什麼 | 不能做什麼 | 誰啟動 |
|---|---|---|---|
| **①常態審查**（預設＝**對某一支 PR 而言、你不是它的實作者時**；⚠️ Grok #596 掃後補、Codex r6 收緊：這一格管的是「當你在審那一支」的行為，**不是你的常設座位**——**不受 ① 約束的只有兩種**：**另一件已獲 William 指派的實作任務**（在自己的實作樹裡做，跟受審那一支無關），以及 `CLAUDE.md` 那條「發現 Codex 未 commit 的改動先審查再 commit」的交接。⚠️ **正在審 A 的那個 session 不可以動 A 的任何產出**——含 A 的回歸考題與文件、就算放在自己的實作樹也一樣——**也不可以自己切換成實作**；「B 另有指派」是另一件合法工作，不是 ① 的漏洞） | 讀、跑三關、提意見（附重現與 `檔案:行`） | **絕對唯讀：不改檔、不 commit、不 push**；在該 PR 的拋棄式審查樹工作、不 checkout 任何分支 | 常設（**PR 進行中每一輪＋每批合併後**）或 William 隨時 |
