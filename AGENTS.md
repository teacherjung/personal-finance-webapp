...
| `cards` | name, type（credit／debit／membership）, issuer, network, lastFour, level, memberId, statementDay, dueDay, annualFee, expiry, benefits, note, pdfPassword | 銀行匯入（bank-import）會**自動建**簽帳金融卡（type:'debit'；Stage 5b） | pdfPassword＝PII（身分證字號；讀寫端 `projectCard` 剝，只卡片編輯窗需要）。issuer＝**可選清單＋其他（自行輸入）**（清單＝`public/modules/card-issuers.js`，與 `issuerBank` 共用）；**刻意不進 `FIELD_SCHEMA` 枚舉**——封閉了清單以外的機構就填不進去 |
...
③連身分都讀不出＝經 **William 特准**的豁免宣告（**身分讀得出、但輪次或 sha 欄損壞＝僅同一身分可豁免**）——**壞留言一律原地保留、不再刪留言**；
