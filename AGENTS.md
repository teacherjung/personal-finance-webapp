...
- **留痕＝照 `templates/ruling-record.md`**（形狀是那支工具的解析器認的，這裡只寫本專案的值）：問的當下貼在這個問題所屬的 PR、貼出後不編輯（改題＝另貼一則新 ❓）；六段「要你裁示的」照常問他、並把 ❓ 的網址寫在那一點裡。他回了＝一題一則 `⚖️ William 裁示`，第一段逐字照範本的原話行（轉述者＝實際轉述的那一方：Claude 實作的支由 Claude、Codex 實作的支由 Codex 自己問、自己貼、如實署名），**原 ❓ 的網址寫在開頭那一段**（任何引用、程式碼、表格、HTML、圖片、縮排之前——William 2026-09-07 裁「a」，原話逐字「a」，落點＝https://github.com/teacherjung/personal-finance-webapp/pull/579#issuecomment-5570875993 ；範本的「開頭那一段」就是它的操作化）。撤回只准用在「已經沒有對象可以回答」（理由三選一照範本逐字），「我自己決定了」「等太久了」「我覺得不重要了」一律繼續等；他若直接貼一則 ⚖️，那一題就是已結——**他的話比我的撤回大**。⚠️ 舊制的 `⏳ 逾時暫定` 留言（2026-09-10 前）套件工具不認、也不再有新的；它們回答過的題已於 2026-09-17 照搬家第 12 題 d 用套件形狀重貼（#577），套件讀法不放寬。❓／⚖️／🚫 整則留言不得出現機器人記號（結論聯集閘讀整份原文，見 `templates/verdict-header.md`）。**❓ 未結且影響本支＝本支不合**（RULES A5），沒有出口、等他；不影響本支的（純進待辦型）照常。
