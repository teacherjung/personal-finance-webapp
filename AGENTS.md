...
| **AI 解析路線 P1b**（P1b-1 2026-08-12，★3 拍板＝Anthropic） | 雙檔分工（ai-parse 純模組／ai-transport 唯一外連檔＝入外連登記閘；服務層不可 import ai-transport、真引擎由 statement.js 路由組裝注入）；四道規矩＝HOSTED 停止線寫死→useAi AI 要求旗標（確認窗僅 aiAskBeforeSend 開啟時出現）→settings.aiApiKey 機密欄→★6 AI 只收強閘；模型階梯 Sonnet→閘紅升 Opus 一次（裁示⑥）＋接地檢查與合計欄交叉驗證（裁示⑧＝P2-2a）；引擎交原始答案、服務層自驗收；**確認票**＝preview 發票、apply 憑票寫入同一份答案不重跑模型（AI 非確定性，r4#1）；**前端（P1b-2）**＝認不出版面＝預設**直接送**（`aiAskBeforeSend` 開關可改回每次問；2026-08-13 拍板，取代「每次跳同意窗」）（不記住）、`useAi` 嚴格布林、票只活在單次預覽閉包、AI 入口只認 `bank_unrecognized`（對帳閘紅無 code 走不到）、設定頁鑰匙欄不回顯——完整契約 → [契約：收支記帳與匯入](docs/contracts/income-expense.md#ai-解析路線-p1b) |
...
| `cards` | name, type（credit／debit／membership）, issuer, network, lastFour, level, memberId, statementDay, dueDay, annualFee, expiry, benefits, note, pdfPassword | 銀行匯入（bank-import）會**自動建**簽帳金融卡（type:'debit'；Stage 5b） | pdfPassword＝PII（身分證字號；讀寫端 `projectCard` 剝，只卡片編輯窗需要） |
...
| 工讀生×2（教學線，人類；2026-08-17 入列） | 教學影片的**製作與營運**：詞彙短片量產線（圖卡／配音合成／剪輯／品檢／YouTube 上架／頻道數據）＋EPxx 正片線的剪輯與上架＋生產看板（Notion 財金詞彙庫）維護（完整邊界與治理＝`docs/教學影片/製作流程與分工.md` v4「治理」節，此處不重抄） | 一切程式實作、正式複審與合併；**不進 repo、無 GitHub 權限**（與 repo 的唯一接點＝詞彙牆次階段由 Claude／Codex 從看板同步 metadata）；不接觸真實財務資料（`data/store.db`）與券商帳號憑證；語音合成與分身生成有閘（判準全文＝該檔「治理」節，**此處刻意不重抄**——重抄過一次就漂成較鬆的一份，`Codex #487 r2 High②`）；分身素材僅 EPxx |
...
- **身分與管道**：William 機器裝有 Grok CLI（xAI）。實測它**在本 repo 目錄啟動時**會自動讀本檔與 CLAUDE.md；但 Grok 的正式工作通道都在 repo 外（材料制），**站外 session 看不到規則書**——所以對它的約束不靠它讀規則，靠**發稿者控制材料**：材料檔的指示需載明該次適用的界線（審查／查核性質、無行動權限；Codex r1 抓到「一體適用」原寫法在站外通道是空話）。專案對它維持「未信任」預設。
- **常態角色＝教學線三職**（操作面在 `docs/教學影片/製作流程與分工.md`）：①**查核記者**＝腳本名詞／數字／案例查核；②**選題雷達**＝每集開工時跑一次（事件觸發、不做定期推播），X 輿情只回答「散戶最近在誤解什麼」供選題——**不得產出持股輿情、行情判斷或任何進入投資決策的資訊**（錢的絕對邊界規則 5 的延伸：教學與投資決策隔離，William 2026-08-15 拍板）；③**讀者測試員**＝就地解釋等文案定稿前扮零基礎讀者、回報哪句看不懂。三職都**不需 repo 權限**：材料（腳本、文案）由發稿者抽出來給它。
