...
- **值不值得改**：**審查回饋不無條件全改**。只有三種情況才回頭改——①**考題其實沒在守它宣稱的東西**（假綠）②**註解／題名說了撐不住的保證** ③**真的產線 bug**。其餘一律寫進待辦，**不再為它多開一輪審查**。
  ...
  ⚠️ **審查磨太久的兩道剎車（一條兩款）**（William 2026-09-11 裁，原話逐字「如果沒有錯，就幫我併成一條兩款，內容不動。」——「如果沒有錯」指他先確認的兩件：自評是第 7 輪過後才貼（規則原文＝第 8 輪起、每 7 輪一則）；自問表是同一件事的第三輪、跟整支審到第幾輪無關——兩件都對；落點＝https://github.com/teacherjung/personal-finance-webapp/pull/599#issuecomment-5630627683）：**第一款＝整支第 8 輪起：在磨的那一方在 PR 上公開留一則自評**；**第二款＝同一件事的第三輪：動手改之前私下答五題，寫進提交說明或逐條處置**。兩款正文照併列前的兩段原文搬入；動到的字只有這幾處：各款前面加一個款名標籤；第二款裡四處指向第一款的字（原寫「七輪自評那條」三處、「那條」一處）；第二款指向程序那份的那句（原寫「新增一列」，併列後是「那一列的第二款」）；第一款裡「固定維度那一條只看…」補上「的第一款」、尾段「它只看有沒有貼」補上「（那一列的第一款）」（併列後程序那份那一列也裝著第二款的核對，不補就把整列說窄了——後者 Codex #599 r1 抓到）。尾句照舊、一字不動。
