...
- **程式線＝常設「複審後掃」（材料制、不進樹）＋進樹一律關門**〔⚠️ 2026-08-19 更正：原標題寫「程式線＝預設關門」，那是 2026-08-16 轉常設**之前**的狀態，與本節開頭「程式線常設『複審後掃』」直接打架。關門的是**進樹**，不是**掃**〕：未經 William 指名，Grok 不在本 repo 讀寫檔案、跑指令、開分支。指名時**同樣走材料制**：發稿者把指定範圍的檔案內容打包給它、在 repo 外目錄啟動，Grok **不進任何樹、不跑 repo 程式**（Codex r1：git worktree 只是另一個 checkout，對「會跑指令的未信任 CLI」不構成隔離保證）。要讓 Grok 進樹＝先在本節補上**已驗證的沙箱條款**、William 拍板，才開門。
