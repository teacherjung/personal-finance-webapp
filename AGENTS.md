...
- **同時最多 2 支**（William 2026-09-03 拍板）：**同時開著（open）的 PR，全部作者加起來最多 2 支；草稿也算 open。** 理由＝2026-09-03 流程體檢：塞車不是「一步一 PR」造成的，是「同時在飛的支數」造成的——跨 PR 試合併閘要把受審支和**每一支** open PR 各合一次、各跑一套三關，成本隨支數**平方**成長（6 支 open＝每合併一支多跑 5 套全卷）；而且大家都往同一批熱點檔案加字（體檢當時最近 50 次合併：`docs/contracts/income-expense.md` 27 次、本檔 20 次、`test/contract-split.test.js` 15 次、`docs/contracts/README.md` 15 次），支數越多撞行越多、每次 rebase＝head 變＝複審結論作廢重拿。配套三句都是既有規矩，這裡只把它們接到支數上：①要開第 3 支＝**等**，不是開——開 PR 前先 `gh pr list --state open` 數一次，**含自己這支不得超過 2**；②等審查的空檔可以在**自己的分支**上把下一件事做到「準備好」（commit 照推），但**不開 PR**——就是下方「重疊 PR 讓道」的晚開做法，射程從「有重疊才晚開」擴到「額滿就晚開」；③會改的檔案與在飛那兩支的預約有重疊的，照「重疊 PR 讓道」在持有人 PR 掛名。⚠️ **刻意不做機械閘、先靠人守**：`scripts/` 沒有對應的閘、COLLAB-MAP 閘表也沒有這一列——本 repo 加一道閘的門檻很高（自報 `MERGE_GATE`、閘表對帳、`test/collab-invariant-docs.test.js` 反查），這是工作習慣，等實際數據證明靠人守不住再加；在那之前「數一次」是每個開 PR 的人自己的事，漏數不會有東西擋你。
