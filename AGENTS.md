...
- **複審後掃（常設；William 2026-08-18 拍板**改序**——本條原名「預審參謀」＝送 Codex 前先掃，2026-08-16 轉常設、2026-08-18 改為複審後掃）**：Claude 實作的每支 PR，**Codex 給出「通過」之後、請 Codex 執行合併之前**，把該版本的 diff（**`base..head` 兩顆都寫死 SHA**）抽給 Grok 掃一遍——**每支只掃這一次**（第一次「通過」之後）：修完再審、再通過**不重掃**。本條要的是「複審漏了什麼」的證據，不是替最終 head 背書。
    - **切換規則（改序當下在飛的支；#464／#466／#470 三次實測：舊 session 不會自己改序）**：①本條生效前已按舊制在**送審前**掃過的支——那一遍照原樣留著、**不必重掃**，但**不得充當本條的證據**（標明「舊制預掃」）：舊制那一遍正好長成本條的反例（先掃 `X`、Codex 之後才對同一顆 `X` 留通過，欄位全部吻合），拿它當「通過之後撈到」就是拿假金標準。②舊 session 若仍在送審前掃：不作廢、同樣標「舊制預掃」，要不要在通過後再補一遍由發射者判斷。③「每支只掃一次」禁的是**同一支重複跑本條這一遍**，不是禁止舊制那一遍存在。
  ...
  - **掃不成就記一行**：圍欄失效、驗屍非 0、CLI 不可用＝在固定小標下逐字寫「未執行：<原因>」即可，**不擋合併、不入任何計數**。⚠️ **本條沒有任何機械保證，全靠自律**——三個免跑理由都是自報、無人複查、不擋合併；要求記那一行，只為了讓事後 grep 得出哪幾支跑了、哪幾支沒跑。（舊版「停跑不是逃生門」那句有東西撐著＝漏跑入計數、不可洗銷、連 3 支重裁；本版把撐著它的全刪了，所以那句話一併刪掉，不留誇大。）
