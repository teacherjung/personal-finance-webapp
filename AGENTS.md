...
## 驗證與三關（技術；2026-09-17 自「協作流程」節逐字搬出，只搬不改字）
...
## PR 分級與契約（技術；2026-09-17 自「協作流程」節逐字搬出，只搬不改字）
...
⚠️ **上面那兩件的現況（2026-09-11 記；這一行刻意不釘：改它不必同步釘本——但程序那份、開工檔、PR 範本裡的現況句沒有從這裡取值，換現況時要一併核對）**：兩件都是 Claude——理由＝下面「Grok 的邊界」那三件扣起來（照現行方式起的 Codex 起不了掃描器）；Codex 實作的支裡發射審查的是 Codex、掃描發射者仍是 Claude；要換現況＝想換的那一方自己去量、貼出可重跑的結果，再問 William。
