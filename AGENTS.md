...
> - ⚠️ 誠實劃界：`.claude/` 那組只約束 **Claude Code**；Codex CLI 不讀 `.claude/`。約束 Codex 以本節條文（AGENTS.md 是 Codex 每次開工必讀）＋審查制度為主，上一條那道 hook 是**信任之後才加上的一層**、不是替代品。考題證明的是「條文與設定沒被靜靜退掉」，證明不了任何 AI 執行期必然守規。**2026-09-02 v6 加了兩層在家族網前面**（PR 待填；Grok 2026-09-01 兩遍掃描＋設計調研查出五條既有漏認）：①**宣告合法字元集**——工具名必須逐字符合 `[A-Za-z0-9_.-]`（1~200 字），不合一律擋。v5 只擋「非 JSON／缺欄位／型別錯」，實測空字串、前後空白、全形、零寬字元、`/` `:` 空白 `+` 分隔都會被當合法名放進家族網。②**已宣告的動錢連接器採白名單制**——名單外一律擋，**不管工具叫什麼名字**；名單內照常再走一次家族網（雙保險）。這一層關掉「靠名字猜」永遠關不掉的那一類：券商最常見的 `market_order`／`limit_order` 這種單側命名（家族網的動詞×名詞文法接不到），以及任何還沒見過的新工具名。⚠️ **誠實劃界**：連接器身分是那串 UUID，**重連換 UUID 這一層就失效、退回家族網**——那時要回來更新名單（規則 4 的通報義務接住這件事）；而**未宣告的連接器**仍只有家族網那張「靠名字猜」的網，它補不完（支付／鏈上／DeFi 各有整批用語），那是已知且刻意的射程限制。考題＝`test/money-boundary.test.js` 與 `test/codex-money-hook.test.js` 的 v6 兩題（探針共用 `test/helpers/money-family-probes.js`）。正則機械層自 2026-08-04 起**擴編為家族攔截**（William 指示「所有轉帳相關詞都進攔截器」；`place_order`／`transfer_funds` 之類同族名現已在網內）：動詞×名詞鎖＋出入金關鍵詞＋換匯動詞，**大小寫與 `_`/`-`/`.` 分隔符不敏感**（MCP 名字規格允許變體——Codex #404 r1 引規格抓到只認小寫底線的洞）；唯讀動詞前綴（封閉名單）放行；取捨方向＝**寧可誤殺、不可漏擋**（誤攔的代價是不便、漏攔的代價是錢；真誤攔＝報 William 裁決）。即便如此仍列舉不完所有未來名字——規則 1 的語意（「任何現在或未來…的工具」）＋規則 4 的通報義務仍是最後防線（發現新錢類工具＝先停手通報，由 William 決定是否再擴網）。
