...
- **複審後掃（常設；William 2026-08-18 拍板**改序**——本條原名「預審參謀」＝送 Codex 前先掃，2026-08-16 轉常設、2026-08-18 改為複審後掃）**：Claude 實作的每支 PR，**Codex 給出「通過」之後、`gh pr ready` 轉正式之前**，把該版本的 diff（**`base..head` 兩顆都寫死 SHA**）抽給 Grok 掃一遍——**每支只掃這一次**（第一次「通過」之後）：修完再審、再通過**不重掃**。本條要的是「複審漏了什麼」的證據，不是替最終 head 背書。
  ...
  - **撈到東西怎麼辦**：Claude 逐條驗證。判定要修者修掉——**修完 head 就變了、原本的「通過」自動失效**（複審結論閘綁 sha，機器本來就會要求對新 head 重新給結論）：**所以「修完不會拿舊結論合併」這件事機器本來就守著**（複審結論閘綁 sha，已複驗；⚠️ 2026-09-08 起**只剩這一道**——協作欄位閘從此不管版本，那一欄已拿掉，William 裁、落點＝https://github.com/teacherjung/personal-finance-webapp/pull/582#issuecomment-5578833211 。而複審結論閘 2026-09-08 起有一個**雲端「看得見」版**——每次開 PR／推 commit／改說明都自動跑，但**不在必要檢查裡、紅了不擋合併**：漏跑合併步驟時平台不會擋你，只會留下痕跡）：本條因此**不需要、也不會新增任何閘**。⚠️ **但那道閘證明不了「這一遍掃到底有沒有發生」**——那一塊刻意不設閘、靠自律。判定誤報者記一行、不改程式。修不修照「審查回饋處置」（例外照舊問 William），**判定要修且屬本支 diff 者修在這支；範圍外的一律寫待辦**（不在合併前最後一刻讓已通過的 PR 長出沒被審過的新內容）。
