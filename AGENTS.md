...
- **預審參謀（常設；William 2026-08-16 拍板——試行 10 支期滿、獨立稽核修正三率後轉常設）**：Claude 實作的每支 PR，**送 Codex 正式審之前**把 diff 抽給 Grok 掃一遍。**呼叫紀律**：repo 外目錄餵材料檔（邊界前綴＋指示＋diff）、**不在任何 repo 目錄啟動它的 CLI**、**必鎖工具**（不鎖會越界＝#467 實測：自行搜家目錄、`cd` 進主目錄跑 git；逐字旗標與版本釘抄 REVIEW-AND-MERGE「怎麼執行」節前置檢查行——**CLI 版本不同＝當未跑**、人工重驗工具清單後更新）；材料以**已 commit、將上 GitHub 的內容**為限（與送 Codex 同一暴露等級），機密與真實資料不得入材料。發現由 Claude **逐條驗證**；修不修照「審查回饋處置」（例外照舊問 William），**判定要修且屬本支 diff 者＝送審前修掉**。**Grok 意見不算正式複審、不進任何合併閘、不產生結論字串**（合併機器零改動）。每支 PR 描述附**輕量證據五件**（比試行版少掉最重的兩樣＝凍結自審清單與五格成績單；一行式項目全保留）：①**受掃版本與配方聲明**（精確 base／head SHA——不可用 `origin/main` 這類會移動的名稱——＋鎖工具旗標＋grok 版本；無配方聲明的掃描＝當未跑）②**材料檔 SHA-256**（一行，防事後換稿）③**掃描時序**（一行：送掃在哪一輪之前；事後補跑照實標）④**Grok 回覆原文** ⑤**逐條判定**（每條標屬實／誤報，判定要修者標修畢——誤報率由此可算）。**主指標＝防漏網，兩級記帳**（發射者在請 Codex 執行合併**之前**對照 Codex 全輪原文、回填 PR 描述——不再追「有效獨家」總數，它量的是 Grok 贏過發稿者自審，量不出對產線的邊際價值）：Grok 的驗真發現若同族議題 Codex 全程未自行提出＝標「**Codex 未觸及**」（可觀察的下界代理；Codex 讀得到預審原文，此格量的是「讀過提示仍未展開」、非盲測——誠實劃界）；若另有 #463 型硬證據（Codex 明文判無問題、被點名後對照實證撤回）＝標「**硬防漏網（實證）**」。**收回絆線（任一成立＝提請 William 重裁去留）**：(a) 連續 10 支入樣 PR 兩級標記皆掛零 (b) 近 10 支加總誤報率超過兩成（發現數為 0＝該項不適用、不觸發）(c) **連續 3 支漏跑**（漏跑＝記明原因後可發射，但入此計數——停跑不是逃生門）。（試行版的五格成績單／七件套／三率機制已由本版取代；試行最終帳——經 2026-08-16 獨立稽核修正：驗真率 86–92%、誤報率 8–11%、有效獨家 1.1–1.9/支，樣本含一漏跑、一漏帳、兩證據不齊，明細在畢業 PR。）
