...
③連身分都讀不出＝經 **William 特准**的豁免宣告（**身分讀得出、但重述接不了＝僅同一身分可豁免**——「接不了」含①metadata 讀不出②**metadata 讀得出、但那一行引不動**〔引文白名單／隱形字元／sha 歧義〕，`#543` 補上第②種之前，那一型的兩條救濟路互斥＝PR 永久鎖死）——**壞留言一律原地保留、不再刪留言**；
