...
9. **突變測試的判準分兩型——用錯型會把真考題誤判成假的，也會把假考題放過**
   ...
   **要先去掉註解、不可只認得一種寫法**（踩過：`${VAR}` 展開成空字串仍顯示「通過」、
   ...
   第 1／2／8 列（單向指標；那份是審查者實際照做的固定維度）。
...
兩個固定關卡：**合併前一句話檢查**——PR 說明要回答「這支若完全失敗，最糟會失去什麼？」；**合併後五分鐘檢查**——**級別與動作依 `node scripts/acceptance-tier.js <編號>` 印的（RULES H6；切換日暫仍用這支腳本，見「本專案協作附則」合併段）；本檔不抄清單、不用級名摘要）：那一步算出的累積動作裡凡是要重啟的，就由 William 重啟 App、以實際操作完成最核心的一條流程（＝每支 PR 附的「怎麼驗收」三句）；沒有重啟動作的就照它印的做。（William 2026-09-06 體檢第 5 題裁示分級，落點＝#573 裡他的留言。）
...
| PROJECT.md | 做到哪、下一步、待裁決事項（**「誰在做」＝看 open PR 清單〔讓道＝RULES D3〕，不在本檔**；**逐筆的待裁／逾時暫定紀錄也不在本檔＝該題所屬 PR 的留言**，見「問法與逾時預設」） | 每**階段收官** |
...
- 修不修＝在磨的那一方判（RULES F6）；**停下來問 William 的三種情況＝RULES F8**（在磨的那一方不確定／動到判準或金額口徑／動到架構），兩個詞哪些算＝下方界線表。磨太久＝F9（第 8 輪起每 7 輪一則公開自評，範本 `templates/self-review.md`）與 F10（同一件事連續第三輪先答五題，範本 `templates/third-round-self-check.md`）。⚠️ **讓檢查變鬆一律等他**：放寬、關掉、豁免任何一道閘、弱化會進三關／CI 的考題通過條件、或關掉重開變更以脫離閘的阻擋——都是「停下來回報」，不是選項題。

**審查回饋處置（本專案的留痕形狀；規矩正本＝RULES F6、F8、I1〜I3，本顆只寫舊工具 `scripts/pending-rulings.js` 換掉之前要認的形狀與本專案的值）**：
...
- **問法＝一句白話問題＋最多三個選項＋我建議的預設**（RULES I1）。⚠️ **沒有時限、也沒有逾時預設——問了就等他**（William 2026-09-10 裁，原話逐字「a＝拿掉時限，問了就等我」，落點＝https://github.com/teacherjung/personal-finance-webapp/pull/595#issuecomment-5617637892）。
- **留痕（第 6 步換套件工具之前照這個形狀；前例＝#571／#573／#576）**：問的當下貼一則留言到這個問題所屬的 PR，第一行固定 `## ❓ 待裁（YYYY-MM-DD）：〈一句白話問題〉`，內文＝選項／我建議的預設／類別聲明；GitHub 的建立時間就是發問時間；**❓ 貼出後不可編輯**（改題＝另貼一則新 ❓）；六段「要你裁示的」照常問他、並把 ❓ 的網址寫在那一點裡。結局兩選一，外加一個只讀不寫的舊類：①他回了＝貼 `## ⚖️ William 裁示（YYYY-MM-DD）：〈標題〉`，內文第一段**逐字照下面那一行填**（只換角括號裡的字、前後不再加任何字——解析器只認這個形狀，角色包粗體或多一個字都會判成形狀不合；Codex 實作的支由 Codex 自己問、自己貼、如實署名）；②**（已停用，只讀不寫）**`## ⏳ 逾時暫定（YYYY-MM-DD）：〈同一句問題〉`——2026-09-10 起不准再貼新的；舊的仍然有效、仍然算數，工具的解析器照舊認得（拿掉它，早就處理完的題目會整批冒回「還沒回」）；③題目已經沒有對象可以回答＝貼 `## 🚫 撤回（YYYY-MM-DD）：〈同一句問題〉`，內文＝`撤回理由：` 加上**三種之一**（`題目依附的東西沒了`／`問題本身問錯了`／`跟另一則 ❓ 重複`），類別後面**必須**接一個全形括號的說明、而且括號裡要有字（寫具體依據）／自報一句 `<撤回者> 撤回、William 未回；他隨時可以要我重問`（⚠️ **只換角括號裡的角色、其餘逐字**）／❓ 留言的連結。撤回只准用在「已經沒有對象可以回答」；「我自己決定了」「等太久了」「我覺得不重要了」一律繼續等。撤回過的題目照樣印在開工清單上（自成一段、附理由），他隨時可以叫發問的那一方重問；他若直接貼一則 ⚖️（網址一樣要寫在最外層才配得上），那一題就是已結——**他的話比我的撤回大**。⚠️ **引用 ❓ 網址必須寫在留言最外層才算數**——William 2026-09-07 裁，原話逐字「a」，落點＝https://github.com/teacherjung/personal-finance-webapp/pull/579#issuecomment-5570875993 。他裁的就是這一句；頂層那一行算（行首有 `- ` 清單記號的那一行本身也算），放進引言、縮排區塊、圍欄或 HTML 註解都不算。❓／⏳／⚖️／🚫 整則留言不得出現 🤖（結論聯集閘掃整則，見 `templates/verdict-header.md`）。**❓ 未結且影響本支＝本支不合**，沒有出口、等他——這一條贏過合併的常設授權；不影響本支的（純進待辦型）照常。
  **⚖️ 原話欄樣板（逐字照填）**：
  原話（對話中，<轉述者> 轉述）：**「逐字」**
