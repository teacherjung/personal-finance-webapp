...
> - **Grok 補註（2026-08-14 實測）**：Grok CLI **會**載入使用者層 `~/.claude/settings.json` 的 permissions（上述兩支下單工具的 deny 都在載入之列）；PreToolUse hook 那層是否被它執行**未驗證**；其 CLI 未配置任何 MCP 連接器＝手上沒有錢類工具。約束 Grok＝**材料制：界線隨每份材料送達**（它的正式通道都在站外、讀不到規則書；「自動讀本檔」只在 repo 目錄啟動時成立——只當額外防線、不當保證）＋三方協作框架「Grok 的邊界」節（教學線特約、程式線預設關門）。
...
| 工讀生×2（教學線，人類；2026-08-17 入列） | 教學影片的**製作與營運**：詞彙短片量產線（圖卡／配音合成／剪輯／品檢／YouTube 上架／頻道數據）＋EPxx 正片線的剪輯與上架＋生產看板（Notion 財金詞彙庫）維護（完整邊界與治理＝`docs/教學影片/製作流程與分工.md` v4「治理」節，此處不重抄） | 一切程式實作、正式複審與合併；**不進 repo、無 GitHub 權限**（與 repo 的唯一接點＝詞彙牆次階段由 Claude／Codex 從看板同步 metadata）；不接觸真實財務資料（`data/store.db`）與券商帳號憑證；語音合成僅限已審核腳本、分身素材僅 EPxx |
...
- **程式線＝預設關門＋唯讀顧問（個案）**：未經 William 指名，Grok 不在本 repo 讀寫檔案、跑指令、開分支。指名時**同樣走材料制**：發稿者把指定範圍的檔案內容打包給它、在 repo 外目錄啟動，Grok **不進任何樹、不跑 repo 程式**（Codex r1：git worktree 只是另一個 checkout，對「會跑指令的未信任 CLI」不構成隔離保證）。要讓 Grok 進樹＝先在本節補上**已驗證的沙箱條款**、William 拍板，才開門。
- **複審後掃（常設；William 2026-08-18 拍板**改序**——本條原名「預審參謀」＝送 Codex 前先掃，2026-08-16 轉常設、2026-08-18 改為複審後掃）**：Claude 實作的每支 PR，**Codex 給出「通過」之後、請 Codex 執行合併之前**，把該版本的 diff（**`base..head` 兩顆都寫死 SHA**）抽給 Grok 掃一遍——**每支只掃這一次**（第一次「通過」之後）：修完再審、再通過**不重掃**。本條要的是「複審漏了什麼」的證據，不是替最終 head 背書。
  ...
  - **PR 描述固定用小標 `### Grok 複審後掃`**（釘住字串才 grep 得出哪幾支跑了——#483 被記成零分的根因就是紀錄格式沒被釘住），內容二選一：①下列各項（刻意不寫件數——寫死的數字自己會漂）；②逐字「**未執行：<原因>**」。
    ...
    - **掃描時序**（一行）：對應**哪一則「通過」留言**（貼留言網址）＋掃描時間。⚠️ **自報、無機械保證**——留著它不是因為它擋得住造假，而是因為沒有它連「聲稱的先後」都不在紀錄上（#486 r1 High：head SHA 對得上≠先後對得上）。**缺這一行＝當未跑**（與「版本不同＝當未跑」「驗屍非 0＝該掃作廢」同級：整條推論就靠先後，先後沒記＝這一遍不成立）。
