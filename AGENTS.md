...
> - Claude Code 權限層已封鎖（`.claude/settings.json`，進版控）：`permissions.deny` 精確點名兩支工具全名，加上 `PreToolUse` deny hook（**家族攔截網**，唯一正本＝`.claude/settings.json` 的 **PreToolUse 指令本體**（python 結構化解析；matcher 只是 `^mcp__` 粗篩）、這裡刻意不重抄以免長出會漂的副本——**家族網那一層**只認工具名、不認連接器 UUID，連接器重連換了 UUID 照樣擋（⚠️ v6 新加的白名單那一層**認 UUID**，換了就失效——見下一則）；唯讀動詞前綴 `get_`/`list_`/`search_` 等放行，所以不誤傷 `get_order_instructions` 等查詢工具）。William 機器的 user 層 `~/.claude/settings.json` 另有同款封鎖（不在 repo）。
> - 考題＝`test/money-boundary.test.js`：斷言本節條文與 repo 設定存在、hook 正則行為精確（含換 UUID 情境）、兩層互相涵蓋。
> - **Codex 側（2026-09-01 起，PR #536）**：`.codex/hooks.json` 是同款封鎖的專案層副本（matcher 與指令本體與 `.claude/settings.json` 逐位相同，由 `test/codex-money-hook.test.js` 的身分互鎖釘住；家族矩陣直接跑在該副本的指令上）。⚠️ **它是條件式的**：Codex 的 hook 要 William 在 Codex 介面（`/hooks`）對**每個檔案各按一次「信任」**才會執行，信任狀態按雜湊存在 `~/.codex/config.toml` 的 `[hooks.state]`——**不在 repo**，改到 matcher 或指令會讓雜湊變、Codex 標為 Modified 並停止執行、要重新按。**未信任時 `codex exec` 一聲不吭地跳過**（2026-09-01 本機實測；帶 `--dangerously-bypass-hook-trust` 時同一份檔案攔截成功）。⚠️ 那次實驗是**單一探針**，證明的只有「hook 有被執行、deny 輸出的形狀被 Codex 認得」——證明不了整張家族網、也證明不了放行路徑。所以「repo 裡有這個檔案」證明不了「Codex 那邊擋得住」。
> - ⚠️ 誠實劃界：`.claude/` 那組只約束 **Claude Code**；Codex CLI 不讀 `.claude/`。約束 Codex 以本節條文（AGENTS.md 是 Codex 每次開工必讀）＋審查制度為主，上一條那道 hook 是**信任之後才加上的一層**、不是替代品。考題證明的是「條文與設定沒被靜靜退掉」，證明不了任何 AI 執行期必然守規。**2026-09-02 v6 加了兩層在家族網前面**（PR #540；Grok 2026-09-01 兩遍掃描＋設計調研查出五條既有漏認）：①**宣告合法字元集**——工具名必須逐字符合 `[A-Za-z0-9_.-]`（1~200 字，正則用 `\Z` 不用 `$`——python 的 `$` 會吃掉結尾換行，`Codex #540 r1` 實測），不合一律擋。⚠️ **射程止於 matcher**：hook 的 matcher 是 `^mcp__`，**不命中就整個 hook 不執行**——前導空白、全形整串這類形狀是被 matcher 篩掉、不是被這一層擋下（它們也不是有效的 MCP 工具名）。v5 只擋「非 JSON／缺欄位／型別錯」，實測空字串、前後空白、全形、零寬字元、`/` `:` 空白 `+` 分隔都會被當合法名放進家族網。②**已宣告的動錢連接器採白名單制**——名單外一律擋，**不管工具叫什麼名字**（掃工具名的**每一段** `__`，與家族網的候選切法對齊——`Grok #540 掃`：原本只認第一段，`mcp__prefix__<uuid>__…` 這種多段前綴整層不開火）；名單內照常再走一次家族網（雙保險）。名單是**一組精確的工具身分**（**這裡刻意不寫幾個**——寫死的數字自己會漂；考題直接比對指令裡的 `ALLOW` 與探針清單**集合相等**，多列或少列都轉紅——⚠️ 那個比對用 python AST 抽字面值，**只看得到原始碼裡直接寫出 `ALLOW` 的綁定**（15 種形式已固化成負向考題）；**執行期用算出來的名稱改寫**（`globals()`／`setattr`／`exec`）AST 看不到，那一層靠審查制度守——`Codex #540 r8` 逼出的誠實收窄），比對前**不做任何正規化**（`Codex #540 r1` 實測：原本先收分隔符再轉小寫，於是 `GET_ACCOUNT_BALANCES`／`get-account-balances` 這些不在名單上的名字都通過了＝把名單擴張成等價類）。⚠️ 代價：**這個連接器未來新增的任何工具都會先被擋**，包括無害的唯讀查詢，直到有人把它加進名單——那正好接上規則 4 的「先通報、不試用」。這一層關掉「靠名字猜」永遠關不掉的那一類：券商最常見的 `market_order`／`limit_order` 這種單側命名（家族網的動詞×名詞文法接不到），以及任何還沒見過的新工具名。⚠️ **誠實劃界**：連接器身分是那串 UUID，**重連換 UUID 這一層就失效、退回家族網**——那時要回來更新名單（規則 4 的通報義務接住這件事）；而**未宣告的連接器**仍只有家族網那張「靠名字猜」的網，它補不完（支付／鏈上／DeFi 各有整批用語），那是已知且刻意的射程限制。考題＝`test/money-boundary.test.js` 與 `test/codex-money-hook.test.js` 的 v6 那幾題（**這裡刻意不寫幾題**——寫死的數字自己會漂；探針共用 `test/helpers/money-family-probes.js`，那裡按「證明了什麼」分成三批：matcher∧handler 成對、僅 handler 的 fail-closed、matcher 射程外）。正則機械層自 2026-08-04 起**擴編為家族攔截**（William 指示「所有轉帳相關詞都進攔截器」；`place_order`／`transfer_funds` 之類同族名現已在網內）：動詞×名詞鎖＋出入金關鍵詞＋換匯動詞，**大小寫與 `_`/`-`/`.` 分隔符不敏感**（MCP 名字規格允許變體——Codex #404 r1 引規格抓到只認小寫底線的洞）；唯讀動詞前綴（封閉名單）放行；取捨方向＝**寧可誤殺、不可漏擋**（誤攔的代價是不便、漏攔的代價是錢；真誤攔＝報 William 裁決）。即便如此仍列舉不完所有未來名字——規則 1 的語意（「任何現在或未來…的工具」）＋規則 4 的通報義務仍是最後防線（發現新錢類工具＝先停手通報，由 William 決定是否再擴網）。
> - **Grok 補註（2026-08-14 實測）**：Grok CLI **會**載入使用者層 `~/.claude/settings.json` 的 permissions（上述兩支下單工具的 deny 都在載入之列）；PreToolUse hook 那層是否被它執行**未驗證**；其 CLI 未配置任何 MCP 連接器＝手上沒有錢類工具。約束 Grok＝**材料制：界線隨每份材料送達**（它的正式通道都在站外、讀不到規則書；「自動讀本檔」只在 repo 目錄啟動時成立，而呼叫紀律明文禁止在**任何** repo 目錄啟動它的 CLI——**照規則走這一層一次都不會發生，所以不列為防線**〔2026-08-19 更正：原寫「只當額外防線」，是撐不住的保證〕）＋三方協作框架「Grok 的邊界」節（教學線特約、程式線＝**複審後掃常設**、只在 OS 沙箱裡的盒子工作——盒子是已 commit 原始碼的副本，不是任何一棵樹）。
...
- **一個工作目錄只服務一個角色**（Codex 提議、使用者定 2026-07-19；2026-08-02 從「寫死三個目錄」改成「寫死角色與不變量」——實測當時共有 16 棵 worktree（Codex 的實作樹在 `/private/tmp/`、每支 PR 一棵審查樹），把數量寫死等於文件一開始就是錯的，同「不寫死頁數」的道理。下表是**各角色的不變量**（角色數會長，不寫死），**實作樹＝常設兩棵（下表）、審查樹＝拋棄式每 PR 一棵**（William 2026-08-04 拍板統一「實作常設、審查拋棄」），拋棄樹的數量與清單不寫死。起因：審查當下 Claude 在同一個目錄裡 rebase／切分支十幾次，Codex 正在讀的樹在腳下移動，看到新舊混雜的程式碼）：
  ...
  - ✅ **順帶補強鐵則 1**：`data/store.db`（真實餘額、IBKR flexToken、`pdfPassword`＝身分證字號）只存在主目錄，實作與審查 worktree 的 `data/` 只有 `seed.json`——「不要讀 store.db」從君子協定變成**樹內根本沒有那個檔＝防誤觸**。⚠️ 這不是對「會跑指令的未信任程序」的隔離保證：worktree 只是另一個 checkout、循路徑仍摸得回主目錄——所以 Grok 一律不進樹（見「Grok 的邊界」節）；Codex／Claude 的樹紀律靠角色授權＋審查制度，不是把這道防誤觸當安全牆。
...
- **同時最多 2 支**（William 2026-09-03 拍板）：**同時開著（open）的 PR，全部作者加起來最多 2 支；草稿也算 open。** ⚠️ **兩層要分開讀**：**上限取 `2` 是 William 2026-09-03 的流程裁示**；下面的證據只說明**為什麼需要一個低上限**，**不宣稱能由資料推出門檻剛好是 2**。依據分**三層**寫，不要混讀——**「塞車主因是同時在飛的支數、不是一步一 PR」是體檢當天的裁示結論，不是下面這組資料量到的**（下面的資料沒有量支數、沒有量同一行衝突次數、沒有量 rebase 次數，也沒有比較「一步一 PR」與別的切法）：①**數學（不依賴觀測）**：跨 PR 試合併閘要把受審支和**每一支** base=main 的 open PR 各合一次、各跑一套三關。**單獨一支當次跑 `n−1` 套＝線性**（6 支 open＝每合併一支多跑 5 套全卷）；**但一整批 `n` 支各走一次，兩兩組合的總試跑數＝`n(n−1)/2`＝平方級**——會平方成長的是批次總量，不是單支當次。②**觀測（單一樣本）**：在下述固定基準與口徑下，**變更集中在少數幾份文件**。③**推論（不是觀測）**：其他條件不變時，並行的支數越多、配對越多，撞在同一行的機會越高；撞到的下游是連鎖的——被迫 rebase ⇒ head 變 ⇒ 複審結論作廢重拿。**這一層沒有量過實際撞行次數。**觀測要連基準與口徑一起寫才算數：以體檢當天的 main（`d26fb49`）為基準、逐 commit 數「最近 50 個 commit 裡改到該檔的有幾個」，**彙總全庫檔案後改動次數最高的四檔**是 `docs/contracts/income-expense.md` 22 次、本檔 18 次、`test/contract-split.test.js` 13 次、`docs/contracts/README.md` 13 次（後兩者並列；第五名 11 次）——口徑：`git log <基準> -50 --format='%H'`，每顆用 `git show --name-only` 看檔名有沒有出現。⚠️ **此結果不外推到其他基準或視窗**，換了連名次都會翻：同基準改看最近 20 顆＝3／7／3／3、本檔變第一；基準改成 `d26fb49~5`、仍看 50 顆＝26／21／14／15、`docs/contracts/README.md` 反超 `test/contract-split.test.js`。配套三句：①要開第 3 支＝**等**，不是開——開 PR 前先 `gh pr list --state open` 數一次，**含自己這支不得超過 2**；②等名額的空檔可以在**自己的分支**上把下一件事做到「準備好」（commit 照推），但**不開 PR**——做法沿用下方「重疊 PR 讓道」的**晚開**（那套做法是既有的），**但觸發條件是本條新增的**：讓道的晚開由「與在途預約重疊」觸發、要在持有人 PR 掛名並盯**該支**終局；**名額已滿但零重疊時沒有持有人可掛名、也沒有特定的一支要盯**，觸發解除的條件就只是「名額空出來」（任一支 open PR 終局）。⚠️ 這種晚開**不持有任何正式預約**（正式預約的唯一即時來源＝open PR 清單，見下方「共享檔案預約」）——所以撞車風險由晚開者自負，**開 PR 前要重查一次預約**（open PR 清單＋在飛那幾支上的讓道留言，兩處都要看；⚠️ 同樣因額滿而晚開、零重疊的人**不在這兩處**——彼此看不見，撞上就靠開 PR 那一刻的重查與「先開者取得預約」收）；③會改的檔案與在飛那兩支的預約有重疊的，照「重疊 PR 讓道」在持有人 PR 掛名，**並等那一支終局**——⚠️ **額滿與重疊同時觸發時，以讓道的等待為準**：名額空出來的是別支、持有人那支還在飛＝仍不能開（否則跨 PR 試合併閘會把你和持有人合起來跑，正好是 #526 擋住 #519 那型）。⚠️ **刻意不做機械閘、先靠人守**：`scripts/` 沒有對應的閘、COLLAB-MAP 閘表也沒有這一列——現有的堆疊閘只驗 base 與上下游、跨 PR 試合併閘只逐支試合併，**兩者都不會因為支數太多而拒絕你建第 3 支**；本 repo 加一道閘的門檻很高（自報 `MERGE_GATE`、閘表對帳、`test/collab-invariant-docs.test.js` 反查），這是工作習慣，等實際數據證明靠人守不住再加；在那之前「數一次」是每個開 PR 的人自己的事，漏數不會有東西擋你。
...
- **合併的決策與執行是兩件事，分開講**（2026-07-30 對齊；此前「使用者是最終合併者」「使用者合併」「PR 由 William 合併」三句散在三份文件、字面上都已與實務不符——本條是唯一規則來源）：
  - **決策（要不要合、何時合）＝永遠 William**（角色表「合併裁決」）。任何人不得自行決定合併——「審查通過」只是門檻之一，不是合併指令。
    - **常設授權（William 2026-08-13 口頭拍板、當時只記在 Claude 記憶；2026-09-05 他在協作系統體檢第 1 題**確認整條**——含 08-13 那句與對稱適用，落點＝#571 裡他的留言）**：Claude 實作的支，Codex 留下合規「通過」、且 PR 說明的「### Grok 複審後掃」小標已填（逐條判定完、或逐字「未執行：<原因>」——未執行不擋合併，這裡也一樣）＝**視同 William 於送審時已預先決定「要合、而且閘綠了就合」**，直接請 Codex 依程序合併、**不必再問**；Codex 實作的支對稱——Claude 複審通過、**且同一個小標已填**（口徑與上一句同一套：逐條判定完、或逐字「未執行：<原因>」，未執行不擋合併；那一支的掃由 Claude 發射、也由 Claude 填小標）——⚠️ **這半句是 Claude 的操作化、現在生效，不是 William 的話**：他 2026-09-09 裁的只有「掃要對稱」，把「小標已填」接成合併前提是 Claude 推的，也**不在 09-05 那次「確認整條」的範圍**；他要收回隨時可以後由 Claude 依程序合併，同樣不必再問。這**不是**把決策權移轉：上一條一字不動，「何時合」也是他預先決定的（閘綠了就是時機），他**隨時可收回**本授權、也可對個案說「先不要合」；本條只把「每支都問」改成「預設同意、例外才問」。本授權**不改變**「審查回饋處置」的停問規則（那裡的清單是唯一真相，這裡刻意不複述）；「錢的絕對邊界」**完全不受本授權影響**（那是絕對禁令，不是問過就能解除的例外）。本授權**也不改變**「標準全流程」那句「William 實測驗收→Squash merge」：高風險／新功能支若依既有流程要先實測驗收，仍照那句、驗收前不合（本條沒有替他把驗收搬到合併後）；**合併後**的驗收分級＝`REVIEW-AND-MERGE.md` 合併步驟「回報合併結果與驗收分級」那一步（William 2026-09-06 體檢第 5 題裁，落點＝#573）。〔為什麼要寫進來：這條 2026-08-13 起只存在 Claude 的記憶，Codex 讀本檔字面會以為每支都要等 William；2026-09-05 協作系統體檢查出後入冊。〕
  - **執行（按下合併鍵）核心原則＝「實作者不按自己的合併鍵」**（William 2026-07-30 定，對稱授權）：**由審查者執行**（何時可以按＝上一子項的常設授權）——Claude 實作、Codex 審過 → **Codex 執行**（2026-07-27 常設授權）；Codex 實作、Claude 審過 → **Claude 執行**（2026-07-30 對稱常設授權）。這是「不可自審」在執行面的延伸：合併程序裡「確認審查結論無阻擋」那一關由實作者自己判讀＝利益衝突最容易滲進來的地方。⚠️ 這裡刻意**不寫第幾關**——步驟編號會變（2026-08-02 新增協作欄位閘之後就整個往後推一格），寫死編號的敘述注定過期。
...
- **沒用到的程式直接刪，不要留「以後可能會用」**（William 2026-08-03 定）。
  ...
  裡的 22KB **未追蹤**檔；而那個資料夾放的是**會擋合併的閘**（幾道以腳本自報的 MERGE_GATE 為準），下一個人會以為它是活的。
...
**Codex 審查的觸發方式（William 常設授權）**：**Claude 直接用本機 `codex` CLI 自動跑審查，不必先問**——涵蓋**PR 進行中的每一輪複審**（r1、r2、r3…；2026-08-03 擴充）**與每批 PR 合併進 `main` 後**的例行審查（2026-07-27 原始授權）。完整指令、沙箱參數、副作用檢查與回報規則見 `REVIEW-AND-MERGE.md` 開頭。要點：只在該 PR 的拋棄式審查樹跑（`/private/tmp/codex-review-pr<N>`，發射者備樹；樹內沒有 `data/store.db`＝防誤觸，非檔案系統隔離）、沙箱要開網路否則端點測試跑不了、審查樹由發射者備與收、**審查者不得自建其他 worktree**、**給 William 的回報規則＝`REVIEW-AND-MERGE.md` 開頭「回報」那條**（本段不複述）。**修不修＝在磨的那一方（＝這一支的實作者）判斷**（⚠️ **唯一正本＝下方「審查回饋處置」那一節，本段只指路、不另養一份角色判準**；2026-09-10 之前這裡寫死 Claude——Codex #594 r7 Medium 指出：那不是歷史引文，是**現行**指示，Codex 實作／Claude 審查時會把同一個決定交給兩個人。沿革：William 2026-08-14 裁示取代原本的「修不修仍由 William 決定／不含依審查結果自動動工」——那句已過期，47 輪的 PR 不可能每輪都問），**但有例外要停下來問——例外清單只寫在下方「審查回饋處置」，本段不複述**。授權**仍不含**：依審查結果自行開新題目、以及按合併鍵（見「實作者不按自己的合併鍵」）。**追加（2026-07-27）：合併也由 Codex 代 William 執行**——**程序一律照 `REVIEW-AND-MERGE.md` 的合併步驟（步數以那份為準）**。⚠️ **本檔不重述那幾步**（Codex #379 r1 High②／r2 High②：這裡原本留著一份舊摘要，少了協作欄位閘與 trailer——照它執行就剛好跳過新加的關卡。同一種漂移前後抓到五處）。**但下列不可跳過的守門要在這裡點名得出來**（`test/merge-procedure-docs.test.js` 與 `test/collab-invariant-docs.test.js` 各自盯著）：`scripts/check-pr-collab-fields.js`（協作欄位）、`scripts/check-review-verdicts.js`（複審結論取聯集）、`scripts/check-ci-really-ran.js`（真考卷：required checks 在合併頭上真跑且 success——skipped 不算（含重跑舊草稿場次蓋出來的）、auto-merge 必須關）、`scripts/check-pr-merge-gate.js`（堆疊）、`scripts/check-cross-pr-merge.js`（跨 PR 試合併）、合併訊息的 `Reviewed-By:` ／ `Merged-By:` trailer。**摘要會落後，名字不會**——這就是「指標＋守門名字」與「重述步驟」的差別。任一關卡不成立就停下來回報、不得合併，且 Codex 合併前不可自行改碼。
...
- **值不值得改**：**審查回饋不無條件全改**。只有三種情況才回頭改——①**考題其實沒在守它宣稱的東西**（假綠）②**註解／題名說了撐不住的保證** ③**真的產線 bug**。其餘一律寫進待辦，**不再為它多開一輪審查**。
  ...
  ⚠️ **第三輪自問表（William 2026-09-10 裁，原話逐字「2. 甲」，落點＝https://github.com/teacherjung/personal-finance-webapp/pull/596#issuecomment-5621280390 ；他先前原話逐字「我贊成乙。只是我們要怎麼立規矩幫助判斷：審核是不是軍備競賽？何時該換個方法？」「我們是不是該列出一些問題，當超過3輪的時候就問自己一遍，及時糾偏。」）**：**同一道檢查（或同一句話）連續第三輪被審查者抓到，動手改之前先答五題**——①我這一輪是在「多補一種寫法」，還是「換一個問題來回答」？補寫法＝**先停下來重新判斷做法**。②我的檢查是不是在**重做別人已經做好的東西**（畫面怎麼呈現、某語言怎麼讀、某服務怎麼判斷）？是＝永遠追不上，換一個自己量得到的東西。③我想證明的是「**所有**情況都不會…」嗎？有限取樣證明不了「所有」——直接量那個東西本身，或把量到的範圍寫死、其餘照實說不保證。④審查者這一輪要我知道的，是**一件新的事**，還是只是「再想一種」？只是再想一種＝同一族，**先停下來重新判斷做法**。⑤我**真正在乎的是什麼**？我的檢查量的是不是那個？⚠️ **「停」停的是這種補法，不是這個問題**（Codex #597 r1 Medium 抓到，附歷史反例 #454 r20：上一輪宣告了四個函式、行內路徑卻只查三個，**補齊那一個是正當處置**，不是跑步機）：**審查者提的有效反例不會因為「同族」而失效**——既定承諾範圍內的假綠、撐不住的保證、真的產線 bug，一律照上面「值不值得改」那三種判斷。①④判成「同族」的**理由要寫進逐條處置**讓審查者核，**他可以反駁並維持阻擋**。答完只有**四種動作**，選一個：**直接量事實**（⚠️ **含「做法本身可行、只是漏了已承諾集合裡的一項」——補齊、並提出能閉合的證據**）／**改規則書的寫法讓檢查變簡單**／**整段逐字釘住**／**把守得住的寫死、其餘照實說**。四個都不適用＝問 William 要不要繼續。⚠️ **留痕＝寫進下一顆修正的說明**（「第三輪自問：答了第②題，所以換做法」），**不另貼留言**。⚠️ **與上面那條是兩件事**：這一條管第三輪、私下答、寫進 commit 訊息；上面那條管第八輪起、公開貼在 PR 上。⚠️ **它不新增機械閘**：**沒有機器依本表計數或檢查答案**，漏答、漏留痕本身不構成審查阻擋（同上面那條）。⚠️ **但不要讀成「不會擋人」**（Codex #597 r1 Low 收窄）：**既有**的複審結論閘本來就會讀結論標頭那個輪次值（拿來比誰新、判同輪相反結論）；而「四個都不適用＝問 William」會**進既有的待裁程序**——問了就等他，那一題若影響本支，**合併也要等**。〔⚠️ **下面是事後推論，不是「一定會叫停」的保證**（Codex #597 r1 Low 收窄，它逐支去讀了留言核對）：#454（審到 r23）第①題在 r10 前後**可能**叫停——但 r10 本身是從畫面文字換到另一條通道，照①④我**也可能**判成「新的一件事」；#578（**取樣 r1〜r7，整支到 r16**）第③題在 r2 已有症狀；#596（**取樣 r1〜r4，整支到 r8**）第②⑤題在 r2 已有症狀。⚠️ **本表第三輪才觸發**，所以「第二輪就會叫停」是說錯的——那幾輪只是**症狀已經看得出來**。⚠️ **真正的限制**：新問題若不符合「同一道／連續第三輪」，本表**不強制啟動**；**不是**「這些問題幫不上忙」（#596 r7 那次封閉例外清單，事後看①⑤正好問得到）。⚠️ 另外更正一筆歸因：#596 r8 的那條 Low 是**前一輪的同步尾項沒做完**，不是「修正又引入一個新問題」。〕
...
⚠️ **架構那一列刻意寫成「補集＋兜底」，不是列舉**（William 2026-08-14 第二次裁定）。
   ...
   ——漏問的代價是我替他拍了他沒授權的板，多問的代價只是他多回一句。（**沒回＝等他**，沒有時限、也沒有逾時預設——見「問法與逾時預設」那顆；⚠️ 2026-09-10 之前這裡寫「時限內沒回＝照建議預設暫定、可翻案」。）
...
⚠️ **「純顯示格式也算」的理由**（起草時的推論，不是 William 的原話；規則本身才是他拍板的）：
   使用者是照**畫面上的數字**做決定的。
⚠️ 這張表是**下限、不是窮舉**。落在表外又說不準的，走**第 1 條**（不確定就問）——列舉補不完是本專案認過的病型，所以這裡不追求窮舉，只把已知會分錯的四個灰帶釘住。
...
⚠️ **表外的具名例外（這兩件刻意不對稱，不可以照上表對調人名）**：**不論誰實作，「發射 Grok 複審後掃」與「把草稿 `gh pr ready` 轉正式」兩件固定由 Claude 做**。⚠️ **兩件的理由不同等級**（Grok #596 掃後拆開）：**掃**＝技術限制（本檔「Grok 的邊界」那三件扣起來：外層沙箱、第二層套不上、金絲雀 fail-closed——方向無關誰實作）；**轉正式**＝William 的交接偏好（裁「維持你轉。」，落點＝#592；他選「一律 Claude」是為了少一次交接，**不是**掃描器起不來）。⚠️ **這兩件都不是改檔／commit／push**，所以**坐在 ① 審查時做這兩件不違反 ①**。⚠️ `REVIEW-AND-MERGE.md`「實作模式」第三層只寫「即使 Codex 實作」那個方向；Claude 自己實作時由 Claude 轉正式是既有預設，這裡的「不論誰實作」比那一節寬一截、照實記。那一節自己也寫著「**不可以整節把人名對調**」。⚠️ **這一行與上表 2026-09-11 起都被逐字釘住**（`test/collab-invariant-docs.test.js` 的 `MODES_TABLE`；改一個字就紅）。⚠️ **逐字釘只保證文字沒被動，不保證這兩件在實務上被遵守**——沒有任何機器在看「是誰發射掃描、是誰轉正式」，那一半靠角色紀律。

**Grok 的邊界（William 2026-08-15 拍板、2026-08-16 轉常設、2026-08-18 改序、2026-09-09 改對稱；管道實測 2026-08-14＝Grok CLI 可用，先立規矩再開門）**——Grok **不是第四方程式協作者**：教學線特約＋程式線常設「複審後掃」，角色表那列的完整邊界就是本節：

- **身分與管道**：William 機器裝有 Grok CLI（xAI）。實測它**在本 repo 目錄啟動時**會自動讀本檔與 CLAUDE.md；但 Grok 的正式工作通道都在 repo 外（材料制），**站外 session 看不到規則書**——所以對它的約束不靠它讀規則，靠**發稿者控制材料**：材料檔的指示需載明該次適用的界線（審查／查核性質、無行動權限；Codex r1 抓到「一體適用」原寫法在站外通道是空話）。⚠️ **該段界線的逐字正本＝教學影片 repo `AGENTS.md`「Grok 材料檔標準邊界前綴」節**（`../teaching-videos/AGENTS.md`）——**每份材料逐字照抄那四行，本 repo 刻意不另存副本**（抄一份就是第二份會漂的模板）。〔沿革：那節原本住 `docs/教學影片/製作流程與分工.md`，2026-08-25 隨分家搬走；本 repo 的程式線掃描與下方第三職都還要用它，所以留這個指標。`Codex #514 r7 中②`〕。專案對它維持「未信任」預設。
- **常態角色＝教學線三職**（⚠️ **操作面分兩處，別整包指過去**（`Grok 掃 #5`）：①查核記者與②選題雷達服務的是影片腳本＝操作面在 教學影片 repo（`../teaching-videos/`，GitHub `teacherjung/teaching-videos`）的 `AGENTS.md`；**③讀者測試員管的是本 repo 的 app 就地解釋文案，產線在這裡、不在對面**——它與工讀生的讀者測試站射程不同、互不取代，見 `docs/learning-hub-contract.md` 第三節）：①**查核記者**＝腳本名詞／數字／案例查核；②**選題雷達**＝每集開工時跑一次（事件觸發、不做定期推播），X 輿情只回答「散戶最近在誤解什麼」供選題——**不得產出持股輿情、行情判斷或任何進入投資決策的資訊**（錢的絕對邊界規則 5 的延伸：教學與投資決策隔離，William 2026-08-15 拍板）；③**讀者測試員**＝就地解釋等文案定稿前扮零基礎讀者、回報哪句看不懂。三職都**不需 repo 權限**：材料（腳本、文案）由發稿者抽出來給它。
- **程式線＝常設「複審後掃」（材料制、不進樹）＋進樹一律關門**〔⚠️ 2026-08-19 更正：原標題把程式線寫成「預設關門」，那是 2026-08-16 轉常設**之前**的狀態，與本節開頭「程式線常設『複審後掃』」直接打架。關門的是**進樹**，不是**掃**〕：未經 William 指名，Grok 不在本 repo 讀寫檔案、跑指令、開分支。指名時**同樣走沙箱制**（呼叫紀律那一條）：Grok **不進任何樹**（Codex r1：git worktree 只是另一個 checkout，對「會跑指令的未信任 CLI」不構成隔離保證），它工作的地方是 OS 沙箱裡的**盒子**——已 commit 原始碼的副本，在裡面跑 repo 程式是准的。〔2026-08-22 之前這裡寫「要讓 Grok 進樹＝先補上已驗證的沙箱條款、William 拍板，才開門」——沙箱條款已驗證（金絲雀＋突變）、William 已拍板，但開的不是「進樹」的門，是「盒子」的門；樹仍然不進。〕
- **複審後掃（常設；William 2026-08-18 拍板**改序**——本條原名「預審參謀」＝送 Codex 前先掃，2026-08-16 轉常設、2026-08-18 改為複審後掃）**：每支 PR，**不論實作者是誰**（William 2026-09-09 **對話中**裁「Grok 掃要對稱」，原話逐字「a. Grok掃要對稱」，⚖️ 落點＝#588 裡他的裁示留言 https://github.com/teacherjung/personal-finance-webapp/pull/588#issuecomment-5602749004 ；此前本條只掃 Claude 實作的支），**獨立審查者給出「通過」之後、`gh pr ready` 轉正式之前**，把該版本的 diff（**`base..head` 兩顆都寫死 SHA**）抽給 Grok 掃一遍——**發射一律由 Claude 做**（Claude 實作的支＝Codex 通過後 Claude 掃；Codex 實作的支＝Claude 複審通過後自己掃。理由是**三件事扣起來**（Grok #588 掃後改寫、Codex r3 再補上中間那件；原本拿單支 PR 的跳過題數當理由，那既弱又會漂）：①發審查的指令列**帶著外層沙箱**（可執行的那一行在 `REVIEW-AND-MERGE.md`「使用方式」節＝`-s workspace-write`，**本檔沒有第二份指令列**；Codex 自己的預設也是這個模式）；②**這個外層環境實測套不上第二層**——不是推論：Codex #588 r3 在自己的暫存盒子裡跑掃描器的沙箱能力探測，退 71、`sandbox_apply: Operation not permitted`；③掃描器的金絲雀**套不上沙箱就整支不掃**（fail-closed，不是「跳過幾題還能掃」）。三件扣起來＝照現行方式起的 Codex 起不了這支掃描器。〔旁證，不是理由：#587 r1〜r8 八輪都記錄那批要沙箱的考題被跳過（2026-09-09 只查這一支）。〕⚠️ 沒試過不帶外層沙箱起 Codex、也**不為了掃這樣起**——要改由 Codex 發射＝**先問 William**（那等於動 Grok 的角色邊界，本節開頭那條）；⚠️ 「誰發射」**沒有任何考題或閘在看**，跟「掃有沒有真的發生」同一級——**每支只掃這一次**（第一次「通過」之後）：修完再審、再通過**不重掃**。本條要的是「複審漏了什麼」的證據，不是替最終 head 背書。
    - **切換規則（改序當下在飛的支；#464／#466／#470 三次實測：舊 session 不會自己改序）**：①本條生效前已按舊制在**送審前**掃過的支——那一遍照原樣留著、**不必重掃**，但**不得充當本條的證據**（標明「舊制預掃」）：舊制那一遍正好長成本條的反例（先掃 `X`、Codex 之後才對同一顆 `X` 留通過，欄位全部吻合），拿它當「通過之後撈到」就是拿假金標準。②**例外的射程是有限集合、有截止**：只適用「**本條上線（本支合併）當下已經 open、且已按舊制掃過**」的 PR——那是可列舉的一批。本支合併**之後**才開的 PR 一律走新制；舊 session 若仍照舊習慣在送審前掃，那一遍同樣標「舊制預掃」、不得充當本條證據，**且不免除本條的複審後掃**（例外不隨 session 壽命延長，否則永遠關不掉）。③「每支只掃一次」禁的是**同一支重複跑本條這一遍**，不是禁止舊制那一遍存在。
  - **為什麼是這個順序（這是本條存在的主要理由，不要改回去；以下以「Codex 審 Claude 的支」為例——Codex 實作的支，把段內的「Codex」讀成留下「通過」的那位獨立審查者）**：掃在前面時，Grok 挑出的問題會在送審前就修掉——Codex 永遠看不到出問題的版本，於是「這條 Codex 自己會不會抓到」**結構上量不到**，只能問他本人（自評；而且他已讀過預審原文＝非盲測）。掃在後面，Codex 的「通過」已經留在紀錄上：**之後撈到的每一條屬實發現，就是正式複審漏掉的**——不必自評、不必記帳、不必算率。⚠️ **誠實劃界（#486 r1 High）**：這個推論的前提是「掃真的發生在通過之後」，而**相同的 head SHA 只能證明掃的是同一版、證明不了先後**（反例：先掃 `X`、Codex 之後才對同一顆 `X` 留下通過，欄位一樣全部吻合）。時序**靠操作者遵守＋記那一行時序，沒有機械保證**——本條刻意不為此新增合併閘。整段歷史目前僅有的一次硬證據（#463：Codex r7 明文「無須另補題」→ Grok 點名→ r8 對照實證後撤回自己的判斷）正是這個順序意外發生的。代價照實記：失去「先掃掉低級錯誤、省 Codex 一點時間」那個好處——但那個好處本來就是量不出來的那一半。
  - **撈到東西怎麼辦**：**逐條判定＝這一支的實作者**（同本檔「審查回饋處置」的誰拍板那一條；⚠️ 2026-09-10 之前這裡寫死「Claude 逐條驗證」——掃**是** Claude 發射的，所以由 Claude 把結果整理進 PR 說明的固定小標，但**判定與修改回到實作者**，兩件事不同）。判定要修者修掉——**修完 head 就變了、原本的「通過」自動失效**（複審結論閘綁 sha，機器本來就會要求對新 head 重新給結論）：**所以「修完不會拿舊結論合併」這件事機器本來就守著**（複審結論閘綁 sha，已複驗；⚠️ 2026-09-08 起**只剩這一道**——協作欄位閘從此不管版本，那一欄已拿掉，William 裁、落點＝https://github.com/teacherjung/personal-finance-webapp/pull/582#issuecomment-5578833211 。而複審結論閘**沒有 CI 版本**：漏跑不會有東西擋你）：本條因此**不需要、也不會新增任何閘**。⚠️ **但那道閘證明不了「這一遍掃到底有沒有發生」**——那一塊刻意不設閘、靠自律。判定誤報者記一行、不改程式。修不修照「審查回饋處置」（例外照舊問 William），**判定要修且屬本支 diff 者修在這支；範圍外的一律寫待辦**（不在合併前最後一刻讓已通過的 PR 長出沒被審過的新內容）。
  - **呼叫紀律（2026-08-22 起＝沙箱制；William 裁示 B＋中間路）**：一律用 `node scripts/grok-scan.js --base <sha> --head <sha> --prompt <指示檔>` 跑，**不准手動啟動它的 CLI**——那支腳本把整條流程串成一支，讓「忘了某一步」變成做不到：①**建盒子**（`git archive` 出 head 的已 commit 原始碼＋APFS clone 的 `node_modules`；沒有 `data/store.db`、沒有 `.env`——材料仍以**已 commit、將上 GitHub 的內容**為限，與送 Codex 同一暴露等級）②**金絲雀**（`scripts/grok-sandbox-canary.js`：在盒子外放假機密、叫 `cat`／`ls`／`curl` 去讀，**任一隻活著＝沙箱是假的、不掃**，fail-closed）③**轉送器**（`scripts/grok-relay.js`：localhost **每掃隨機 port** → xAI 那一個寫死的位址，目的地不從請求取；**只轉 ALLOWED_REQUESTS 表上的（白名單自 grok 1.0.3 記錄型 proxy 實測抄來、1.0.13 沒重抄；打出表外＝403＋掃描退 2，由考題釘） method／path、只認本掃隨機的假 token**——盒內 `auth.json` 的 key 是假值，真 token 在沙箱外由轉送器換上，從未進過盒子）④在 **`scripts/grok-sandbox.sb`** 沙箱裡跑 grok：`(deny default)`＋Apple 的 `system.sb` 再逐項放行——系統與工具鏈唯讀、盒子讀寫、**真 `~/.grok` 完全不進沙箱**（盒內的 grok 家是白名單複製的副本；登入狀態放沙箱專用的 `~/.grok-sandbox-auth`）；網路**只准連轉送器那一個 port、沒有 DNS**（會起本機伺服器再連自己的整合型考題在盒子裡跑不了，這是不讓它連到理財 app 4321 的代價；DNS 那條是 2026-08-23 沙箱制第一次正式掃描由 Grok 抓到的——r1–r14 一直放行 mDNSResponder socket，盒內能解析任意網域＝查詢名就是外送通道，Codex 十四輪沒看到）；`HOME`／`TMPDIR`／`GROK_HOME` 全指進盒子 ⑤**驗屍**（`scripts/audit-grok-scan.js` 數足跡＋查破口線索）⑥**盒子在所有出口清掉**，只留去機密的結果包在 `~/.grok-scan-results/`——**掃成功**＝`launch.json`＋sessions；**判事故（退 1）**＝`launch.json`＋`incident.json`、**不含 sessions**。`incident.json` 是**事故指紋包**：只有數字、雜湊、族別與遮蔽過的上下文視窗，**不含任何一族的命中值本身**（暗號族與 DLP 族連雜湊都不寫——它們的值由構造已知，寫衍生物不增加判斷力卻多留一份可查表的表示）。⚠️ 上下文視窗會把命中周圍的一小段（前後各約 200 字）留在本機 0600 檔裡（William 2026-08-31 裁示）。**與視窗相交的已知機密與其他形狀命中會整段換成佔位符——即使它伸出視窗外**（先切窗再替換只是把邊界往外移，新邊界上照樣留半截）；但**認不得的擋不住**——真破口時那一小段就是盒外讀到的東西。〔為什麼要有它：#516／#530 兩次事故靠**重算**判成假警報，#534 連重算的材料都沒有（四條命中裡兩條在 grok 的回覆裡，而回覆不可重現）——舊寫法在寫結果包**之前**就 return 了。〕**Grok 在盒子裡跑指令是准的**（William 2026-08-20 拍板「允許跑指令、作廢制廢止」——當時條文沒跟上，#494 照裁示做、#491 照過期條文做，兩邊都沒錯，錯的是這裡沒更新）：在盒子裡它可以 `cat`、`grep`、跑 `node --test`——盒子裡只有：已 commit 的原始碼副本、`node_modules` 副本、grok 執行檔（盒內副本驗過 sha256）、**重建的 `auth.json`**（只有 grok -p 需要的 7 欄＋假 key；email／姓名／refresh_token 等不進去）、空的 sessions/。沒有真實資料、沒有歷史 session、沒有真憑證；`config.toml`／`agent_id` 也不帶（先前未隔離的 Grok 可能已在那裡埋東西，而 -p 模式實測不需要）。⚠️ **為什麼從鎖工具改成沙箱**：2026-08-22 全量稽核 `~/.grok/sessions`，53 個 session 有 **52 個**留有工具足跡——鎖工具旗標的失效率不是「偶爾」，是常態；它進過主目錄讀原始碼跑 git、`ls` 過 Desktop／Documents（檔名已送 xAI）、`curl` 過外網（能 GET 就能 POST）；**碰 `store.db`／flexToken／`.env`／金鑰：0 次**——不是圍欄擋的，是它自己沒去。「請它不要做」擋不住，改成「作業系統不讓它做」。⚠️ **誠實劃界**：`sandbox-exec` 是 Apple 標為過時但 macOS 15.7 仍可用的工具，它消失這道圍欄就沒了；沙箱網路規則只認 localhost，放不了特定網域，所以靠轉送器；轉送器擋不住「把資料 POST 給 xAI」（那跟送 prompt 同一條路），真正的保護是它讀不到可以送的東西；金絲雀只證明列出的形狀擋得住、不證明沒有別的洞；**setsid 擋不住**（Seatbelt 的 syscall 過濾對它無效；對 setpgid 有效但 grok 的工具靠 setpgid 起子行程、擋了就一條指令都跑不了，所以也不擋——實測），所以「程序群組已全部收束」不是保證，保證的是離開來的程序**無害**（它的沙箱只准連本掃的隨機 port、只認本掃的假 token、只能讀寫已刪掉的盒子路徑；父程序讀 sessions 是單趟＋上限）；資源上限是 ulimit（單檔／程序數／CPU）＋父程序讀取上限，**沒有總磁碟配額**；**盒子裡放什麼仍是我們的責任**。失效條件三個不變：**CLI 版本不同＝當未跑**（轉送器的目的地是從 1.0.3 執行檔 `strings` 出來的、1.0.13 於 2026-09-05 重驗相同（紀錄在 PR #564），升版要重驗）、**金絲雀非 0＝不掃**（取代舊的「驗屍非 0＝該掃作廢」：驗屍現在記足跡、查破口，**破口＝沙箱破了＝事故**，不只是作廢）、**缺掃描時序一行＝當未跑**。
  - **PR 描述固定用小標 `### Grok 複審後掃`**（釘住字串才 grep 得出哪幾支跑了——#483 被記成零分的根因就是紀錄格式沒被釘住），內容二選一：①下列各項（刻意不寫件數——寫死的數字自己會漂）；②逐字「**未執行：<原因>**」。
    - **受掃版本與配方聲明**：精確 `base..head`（兩顆都寫死 SHA、不可用 `origin/main` 這類會移動的名稱）＋**`scripts/grok-scan.js` 印出的配方聲明整行照抄**（它含 grok 版本、沙箱設定檔、轉送器、盒子路徑、發射紀錄 `launch.json` 的路徑、掃描起訖）——同一台機器上的另一方（Codex／William）可以拿**結果包**（`~/.grok-scan-results/<head>-<時間>/` 裡的 `launch.json` 與 `sessions/`——盒子掃完即清、真 `~/.grok/sessions` 不會有這次的日誌）自己重跑驗屍複查，這是全條最能被複查的一格（GitHub 上的讀者重跑不了）。〔2026-08-22 前這裡寫「鎖工具旗標＋驗屍退出碼與 session id」——沙箱制下旗標不再是圍欄、驗屍不再作廢，改記腳本的配方聲明。〕記的是**當時掃的那顆 head，之後修過不回填**——它記的是「掃了哪一版」，不是「目前是哪一版」。⚠️ 誠實劃界：修過之後，**真正被合併的那顆 head 未必被 Grok 看過**；接住它的是複審重跑，不是本條。
    - **Grok 回覆原文**。
    - **逐條判定**（三選一）：**屬實·已修**（附觸發了第幾輪複審）／**屬實·進待辦**（附待辦去處）／**誤報**。〔為什麼要三個值：只有「屬實／誤報」時，照「值不值得改」該進待辦的那一條只能被漏記或被標成誤報——而懲罰亂標誤報的絆線已一併廢止，被污染的正好是新制唯一剩下的數字。〕
    - **掃描時序**（一行）：對應**哪一則「通過」留言**（貼留言網址）＋掃描時間。⚠️ **必須是掃的當下那一則**——撈到東西修完之後又出現新的通過，這一行**不改指**（改指最新那則：欄位仍齊、SHA 仍對得上當時的 head、三個失效字串仍在，但時序是假的；而「缺這一行＝當未跑」擋得住缺，擋不住填錯指向——這一格沒有機械保證）。⚠️ **自報、無機械保證**——留著它不是因為它擋得住造假，而是因為沒有它連「聲稱的先後」都不在紀錄上（#486 r1 High：head SHA 對得上≠先後對得上）。**缺這一行＝當未跑**（與「版本不同＝當未跑」「金絲雀非 0＝不掃」同級：整條推論就靠先後，先後沒記＝這一遍不成立）。
    〔2026-08-18 刪掉的一項與理由：材料檔 SHA-256——條款自己就寫明「無可重建前像＝擋不了換稿」，留著只是儀式。**掃描時序原本也要刪，理由是「head SHA 對得上＝時序自證」——那句話被 #486 r1 推翻（它只證明版本一致），所以這一項留下並降級為誠實劃界。**〕
  - **不記帳（William 2026-08-18 拍板改序時，選項說明逐字含「記帳整個消失」；廢止哪幾項由 Claude 依此推導列舉——被廢止的那套是他 2026-08-16 拍板的，落款照實分層，不冒名）**：廢止兩級記帳、尾輪自評問句、三條收回絆線與一切率。理由＝那整套是為了補償「量不到」而發明的，改序後量得到就不需要；而它實際生產的是**假帳**——#483（Grok 三條屬實、Codex 白紙黑字自評「不會抓到」）記成零分、#479 同一段裡「記 2 分」與「掛零」兩句互相矛盾。要重裁 Grok 的去留，**逐支翻 `### Grok 複審後掃` 小標下的逐條判定**即可（人工，但有固定字串可抓——沒有固定小標時這件事實際上做不到）；**不另設跨 PR 計數器**——這個 repo 每一道閘都單支自足、沒有跨 PR 累計狀態的先例，養一台計數器就是養第二本假帳。
  - **Grok 意見不算正式複審、不進任何合併閘、不產生結論字串**（合併機器零改動）。
  - **凡是本節判成「當未跑／不掃」的，一律同一個收口**（刻意不列舉理由、不寫數量——理由會長，寫死就會漏）：在固定小標下逐字寫「**未執行：<原因>**」，**不擋合併、不入任何計數**；要不要重掃是選項、不是義務。⚠️ **2026-08-22 起「作廢」這個字不再出現在新掃描裡**：沙箱制下驗屍不作廢（在盒子裡跑指令是准的），要嘛金絲雀不過＝**沒掃**（記「未執行」）、要嘛掃了＝**算數**；驗屍查到破口＝**事故**，另外處理（停掃、回報 William；判真假就看那一遍留下的 `incident.json`）。〔舊制留下 10 支 PR 裡 9 支記「未執行」、其中至少 2 支實際撈到真問題——紀錄又在說謊，這正是改沙箱的理由之一；那些舊紀錄保留原樣當歷史。〕⚠️ 〔舊制遺留的處置，只適用 2026-08-22 前已作廢的掃描；新制沒有「作廢」〕作廢掃描的內容仍可人工驗證後採用，但**依定義列為 Claude 自審、不計 Grok 出勤**，且**不得寫在固定小標底下**（那一區是日後人工翻表的統計區，混進作廢掃描的發現＝製造新的假金標準）——另立標題放。⚠️ **本條沒有任何機械保證，全靠自律**——免跑理由一律自報、無人複查、不擋合併；要求記那一行，只為了讓事後 grep 得出哪幾支跑了、哪幾支沒跑。（舊版「停跑不是逃生門」那句有東西撐著＝漏跑入計數、不可洗銷、連 3 支重裁；本版把撐著它的全刪了，所以那句話一併刪掉，不留誇大。）
- **主目錄禁區（隱私線，任何模式無例外）**：不得在主目錄工作——`data/store.db` 有真實餘額、IB flexToken、證件號，Grok 讀到的內容會送往 xAI 伺服器。現行條款下 Grok 本來就**不進任何樹**（上兩條皆材料制）；本條是兜底鐵線——連「在主目錄啟動它的 CLI」都算違規。
- **全域段落一體適用＋先規矩後開門**：錢的絕對邊界、鐵則、協作唯一不變量寫明「適用所有 AI」，Grok 含在內。要讓 Grok 承接本節以外的任何工作＝先在本節補上角色與邊界、William 拍板，才開門。
...
⚠️ **「來源」是機械身分，不是描述文字**（2026-08-14 PR #453 實際踩到）：閘認「誰是同一位審查者」
＝**角色＋來源**，而來源**只摺疊空白**——多一個 `codex` 前綴、換成全形括號、多一個逗號，
同一個審查工具就被拆成**兩位**，於是它自己的阻擋自己撤銷不掉。⇒ **每個審查工具固定一個逐字不變的
來源字串**；標準字串表、發審查提示要逐字給哪些機械字串（含**角色**——也是機械查表，2026-08-18
#479 發射提示把角色寫成「獨立審查者」＝兩則真審查被判讀不出身分）、以及漂掉之後的補救（**用補發，不可以編輯
舊留言**——改審查紀錄會洗掉稽核軌跡）＝`REVIEW-AND-MERGE.md`「發審查提示」節（單一真相，此處不重抄）。
...
**③重述＝壞標頭救濟的入口**（2026-08-06 William 裁決；起因＝發射提示沒列出三個合規結論字串，
五支 PR 的歷史留言裡都有「要求修改」「通過（無阻擋）」這類壞標頭 ⇒ 永久阻擋、補新留言也清不掉。
**2026-08-17 起為三級救濟階梯之首**：①四欄讀得出＝重述 ②只有 sha 欄空白＝缺 sha 例外（重述行自報版本）
③連身分都讀不出＝經 **William 特准**的豁免宣告（**身分讀得出、但重述接不了＝僅同一身分可豁免**——「接不了」含①metadata 讀不出②**metadata 讀得出、但那一行引不動**〔引文白名單／隱形字元／sha 歧義〕，`#543` 補上第②種之前，那一型的兩條救濟路互斥＝PR 永久鎖死。⚠️ **長度不在判準裡**（William 2026-09-02 裁示）：壞行長到連救濟留言都送不出時本閘沒有處方，那是平台限制，真的遇到就關掉重開 PR）——**壞留言一律原地保留、不再刪留言**；
階梯與完整規則的單一真相＝閘腳本的 RESTATE／QUOTED_HEAD_NOSHA／EXEMPT 三節，本檔只指路）。
...
對目前 head 的「通過」（機械執法與完整規則＝`scripts/check-review-verdicts.js` 的 RESTATE／
QUOTED_HEAD_NOSHA／EXEMPT 三節，單一真相；豁免宣告同樣**只會中和、不會產生**）。
...
| ③ 審修法 | **A** | ＝發現者，**不是**提案者 |
| ④ 實作 | **一律由 William 明確指派**；他**習慣**指派 Claude（Codex 被指派時＝模式③）（William 2026-09-10 裁「我希望全部都踩對稱方式」，落點＝https://github.com/teacherjung/personal-finance-webapp/pull/594#issuecomment-5615230593：**兩邊的實作都是他明確指派**，差別只在他**習慣**指派 Claude——「預設 Claude」是把習慣寫成規則） | 審查者不可改自己要審的碼 |
...
兩個固定關卡：**合併前一句話檢查**——PR 說明要回答「這支若完全失敗，最糟會失去什麼？」；**合併後五分鐘檢查**——**級別與動作依 `REVIEW-AND-MERGE.md` 合併步驟「回報合併結果與驗收分級」那一步**（那一步再指到分級腳本＝真正的正本；本檔不抄清單、不用級名摘要）：那一步算出的累積動作裡凡是要重啟的，就由 William 重啟 App、以實際操作完成最核心的一條流程（＝每支 PR 附的「怎麼驗收」三句）；沒有重啟動作的就照它印的做。（William 2026-09-06 體檢第 5 題裁示分級，落點＝#573 裡他的留言。）

**共享檔案預約（2026-07-31 起＝Draft PR，人工預約表退役）**：**開工第一步＝先開 Draft PR**（哪怕只有一個開工 commit；**例外只有一種做法＝晚開，但觸發條件有兩個**——①**與在途預約重疊**：先在持有人 PR 掛名、對方**終局**（合併，或 closed 且依下段分流判準＝放棄）後才開，見下方「重疊 PR 讓道」；②**名額已滿**（上方「同時最多 2 支」）：零重疊時沒有持有人可掛名，等名額空出來再開；**兩個觸發都中＝照①等持有人那支終局，不是等任一名額**。⚠️ 兩種晚開都**不持有正式預約**——正式預約的唯一即時來源就是下面說的 open PR 清單），**且 PR 說明開工時就要列出「預計修改的共享檔案／區域」**（還沒 commit 到的也要列——open PR 的 files 只看得到已改的，宣告才蓋得住整個工作範圍）——「誰在做什麼」的**正式預約**唯一即時來源就是 GitHub 的 open PR 清單（`gh pr list`＋各 PR 說明；**唯一的例外弱宣告＝讓道留言**，掛在持有人 PR 上——見下方「重疊 PR 讓道」，查預約時兩處都要看），不再人工維護第二張表（人工表實證會過期：曾經同時開著五支 PR、表上只寫一支）。**工作中途要碰開工時沒宣告的共享檔案＝先更新自己 PR 的說明、並重查其他 open PR 的宣告（**含讓道留言**：新要碰的檔案若在別支 open PR 的預約裡，去那支的留言區看有沒有讓道掛名——「兩處都要看」在這裡也適用）**；撞到別人已宣告的範圍就停下協調（插隊條件除外），不可先改再說。規則不變：同一檔案同一時間只有一位持有人（＝該 PR 的實作者）；其他人可讀可審、不直接修改；發現會造成**金額算錯／資料遺失／機密外洩／頁面或核心路由崩潰**的問題可立即插隊——插隊問題由**持有人**修，發現者提供重現條件並複審。
**重疊 PR 讓道（2026-08-29 #526 擋住 #519 的教訓；William 2026-08-29 排定入冊＝候選 8-18）**：要動的檔案與**在途 PR 已宣告的預約**重疊時，**不要開著自己的 PR 等**——跨 PR 試合併閘把受審支和**每一支 base=main 的 open PR**（堆疊支不在射程＝該閘 `--base main` 與 baseRefName 檢查；現行 `--limit 100`＝只查前 100 支，本 repo 同時 open 通常個位數、遠低於上限）**真的合起來跑三關**（文字衝突與「零檔案重疊、合起來測試紅」**都擋**——後者正是它的立閘事故型 #384/#385），**但不讀欄位裡的意圖**：在說明寫「不會搶」沒有機械效力，開著的 PR 就是閘眼中的衝突源，先預約的支反而被後開的擋住（#526 未送審就擋住了已審完六輪、走到合併最後一步的 #519，靠限時協調暫關才解）。**讓道＝上面「停下協調」的協調結果之一，不是繞過協調**；且它是「同一檔案同一時間只有一位持有人」的**明定例外——平行草稿**：讓道者只在**自己的分支**上動重疊檔案（不動 main、不動持有人的分支＝沒碰持有人的所有權），**以持有人合併後的版本為準**、自己 rebase 收衝突、風險自負（Codex 實作模式的「不碰他人預約檔案」條件同受此例外——REVIEW-AND-MERGE.md 該條有指回）。做法二選一：①**晚開**——先在**持有人的 open PR** 留言宣告「讓道中：本人在做 Y、會動到重疊檔 X，待本支終局後開 PR」，然後工作照做、commit 照推，等對方**終局**（合併＝rebase 其結果；確認放棄＝改以 origin/main 為準）、自己 rebase 完再開 PR。⚠️ 那則留言是讓道期間的**弱宣告**（人查得到、閘與 `gh pr list` 讀不到——沒開的 PR 不在「誰在做什麼」的唯一即時來源上），所以對方合併後**盡快開 PR 恢復正式宣告**。②**開了暫關**——closed 的 PR 閘不看（`scripts/check-cross-pr-merge.js` 現行只撈 `--state open`）；⚠️ **暫關不得刪 head 分支**（`gh pr close` 不帶 `--delete-branch`）——分支還在才 reopen 得回來，號碼與討論都保留；讓道留言**自己的 PR 與持有人的 PR 各留一則**——自己那則要逐字含「**本支讓道中、上游＝#N**」＋誰負責重開（分流判準認的就是這個字樣；只寫在 closed PR＝之後沒有任何流程會再讀到它，所以持有人那則不可省）。**持有人 PR 的終局讓道者要自己盯**——merged 與 closed 都是訊號，但 **closed ≠ 一律放棄**（暫關讓道的 PR 也是 closed）：先讀那支 PR 的留言分流，**只認寫明主體的宣告**（同帳號協作＝作者欄分不出人，主體要寫在字面上）：有「**本支讓道中、上游＝#N**」＝這支自己在讓道，視同讓道者、盯 #N 的終局；**明示放棄（「本支放棄」）覆蓋較早的讓道宣告、多則以最新為準**；兩者皆無＝放棄＝**預約釋放**，讓道者改以 origin/main 為準、照常開 PR。⚠️ **下游讓道者的掛名留言（「讓道中：本人在做…」）是替他們自己的未來 PR 掛名，不是這支 PR 的狀態**——認錯主體就會把「持有人放棄」誤判成「持有人在讓道」、去追不存在的上游。（closed PR 上的留言仍讀得到，「先掛名者先行」照舊。）**重開或晚開後開 PR 之前，重查兩處**：①open PR 清單的宣告 ②**原持有人 PR 上的讓道留言**——讓道者都不在 open 清單上，互相只能在那裡看見；發現不只一位＝照留言時間序**先掛名者先行**（或當場協調分工），後行者繼續讓道。〔刻意沒有「把衝突 hunk 改到不撞」這個選項：hunk 不撞仍可能「合起來測試紅」被擋，那正是這道閘存在的理由。〕
...
| PROJECT.md | 做到哪、下一步、待裁決事項（**「誰在做」＝看 open PR 清單〔讓道弱宣告例外見「重疊 PR 讓道」段〕，不在本檔**；**逐筆的待裁／逾時暫定紀錄也不在本檔＝該題所屬 PR 的留言**，見「問法與逾時預設」） | 每**階段收官** |
| 施工計畫 | 這個功能準備怎麼做 | **開工前定稿**（只有高風險與新功能需要） |
| Notion | 給 William 的白話原理與開發紀錄 | **複審收官後** |
| PR 說明 | **必填欄位**（清單的單一真相＝`scripts/check-pr-collab-fields.js` 的 `REQUIRED_FIELDS`；**與 PR 模板之間**有考題逐行逐字對帳。⚠️ **這張表上的名字沒有被那道對帳守著**，是給人讀的複本、可能自己漂——以正本為準）：實作者／獨立審查者／預計修改的共享檔案（＝預約）／這支若完全失敗最糟失去什麼；再加「實際改了什麼＋驗收法」 | 每支 PR。⚠️ **模板在 `.github/pull_request_template.md`、由 `scripts/check-pr-collab-fields.js` 機械把關**——2026-08-02 實測連續三支（#374/#375/#376）漏填，證明只靠記憶維持不住 |

不要求每支搬家 PR 同時修改多套文件。

**一句話總綱**：**William 逐支指派誰建造，另一方負責挑戰**，William 決定與驗收（⚠️ 2026-09-10 之前這裡寫「Claude 負責建造，Codex 負責挑戰」——那是把**習慣**寫成**分工**，而擺在整份文件最像結論的位置；他**習慣**指派 Claude，但兩邊的實作都是他明確指派，正本＝「審查回饋處置」與 `REVIEW-AND-MERGE.md`「實作模式」節）；共享檔案一次只交給一人（唯一例外＝「重疊 PR 讓道」的平行草稿——同款釘法：例外只在預約段定義，這半句不複述內容）。（⚠️ 總綱這句的「決定」**以上方角色分工表 William 那一列為準**——這裡刻意不複述、也不寫「哪幾項」，因為那一列會長：2026-08-14 就多了「審查回饋的例外」。**一般的審查回饋修不修不在他身上**，由**在磨的那一方（＝這一支的實作者）**判斷。摘要句比表格鬆是本檔反覆出現的漂移型態，這裡就地釘住。）高風險工作小步前進，每一步有考題、有證據、有退路。文件說明規則，契約連接模組，PR 隔離變更。
