...
| Grok | 教學線「外部記者」三職：腳本查核／每集開工時的選題雷達／就地解釋文案的零基礎讀者測試；程式線＝**預審參謀（常設，2026-08-16 畢業）**＋**唯讀顧問（個案）**（完整邊界＝「Grok 的邊界」節） | 一切實作、正式複審與合併執行；**沙箱條款落地前不進任何 repo 樹**（主目錄／實作樹／審查樹——一律材料制，見該節）；X 輿情不得用於投資決策線 |
...
**Grok 的邊界（William 2026-08-15 拍板、2026-08-16 預審轉常設；管道實測 2026-08-14＝Grok CLI 可用，先立規矩再開門）**——Grok **不是第四方程式協作者**：教學線特約＋程式線常設預審參謀，角色表那列的完整邊界就是本節：
...
- **預審參謀（常設；William 2026-08-16 拍板——試行 10 支期滿、獨立稽核修正三率後轉常設）**：Claude 實作的每支 PR，**送 Codex 正式審之前**把 diff 抽給 Grok 掃一遍。**呼叫紀律**：repo 外目錄餵材料檔（邊界前綴＋指示＋diff）、**不在任何 repo 目錄啟動它的 CLI**、**必鎖工具**（不鎖會越界＝#467 實測：自行搜家目錄、`cd` 進主目錄跑 git；逐字旗標與版本釘抄 REVIEW-AND-MERGE「怎麼執行」節前置檢查行——**CLI 版本不同＝當未跑**、人工重驗工具清單後更新；**掃後必驗屍**＝每次掃描後跑 `node scripts/audit-grok-scan.js --workspace <掃描 cwd>` 機械稽核該次 session 日誌，**退出碼非 0＝該掃作廢**、照本條款記漏跑或鎖工具重掃——2026-08-17 實測鎖工具旗標同版本會非決定性靜默失效，旗標只當第一層，**制度靠驗屍不靠運氣**（William 拍板）；材料以**已 commit、將上 GitHub 的內容**為限（與送 Codex 同一暴露等級），機密與真實資料不得入材料。發現由 Claude **逐條驗證**；修不修照「審查回饋處置」（例外照舊問 William），**判定要修且屬本支 diff 者＝送審前修掉**。**Grok 意見不算正式複審、不進任何合併閘、不產生結論字串**（合併機器零改動）。每支 PR 描述附**輕量證據五件**（比試行版少掉最重的兩樣＝凍結自審清單與五格成績單；一行式項目全保留）：①**受掃版本與配方聲明**（精確 base／head SHA——不可用 `origin/main` 這類會移動的名稱——＋鎖工具旗標＋grok 版本＋**驗屍結果**（audit-grok-scan 退出碼與 session id）；無配方聲明或驗屍非 0 的掃描＝當未跑）②**材料檔 SHA-256＋重建規格**（一行雜湊＋一句規格「材料＝附錄指示全文＋標定 base..head 的 git diff 原樣串接」——讀者可自行重算（#467 審查實測可行）；雜湊無可重建前像＝擋不了換稿，只剩誠實劃界；配方聲明仍屬自報、同為誠實劃界）③**掃描時序**（一行：送掃在哪一輪之前；事後補跑照實標）④**Grok 回覆原文** ⑤**逐條判定**（每條標屬實／誤報，判定要修者標修畢——誤報率由此可算）。**主指標＝防漏網，兩級記帳（尾輪自評制，William 2026-08-16 裁）**——不再追「有效獨家」總數（它量的是 Grok 贏過發稿者自審，量不出對產線的邊際價值）；也不用「Codex 未觸及」（Codex 審的是已修 head、本來就不會重述已修掉的問題＝系統性過計，#475 r1 抓到）：發射者在**最後一輪**審查提示多問一句「PR 描述附錄的預審發現，哪些你在**未修版**程式上也會自行抓到？」，Codex 逐條答會／不會／不確定——答「**不會**」者標「**防漏網（Codex 自評）**」；另有 #463 型對照實證（Codex 明文判無問題、被點名後實證撤回）＝標「**硬防漏網（實證）**」。誠實劃界：自評非盲測（Codex 已讀過預審原文），但方向正確——自己的覆蓋範圍自己最清楚。發射者在請 Codex 執行合併**前**把兩級標記回填 PR 描述、**並順帶重算下列三條絆線**記「絆線檢查：未觸發／觸發哪條」；William 本人直接合併的支＝由**下一支入樣 PR 的發射者**補算補記。**收回絆線（任一成立＝提請 William 重裁去留）**：(a) 連續 10 支入樣 PR 兩級標記皆掛零 (b) 近 10 支加總誤報率超過兩成（發現數為 0＝該項不適用、不觸發）(c) **連續 3 支漏跑**（漏跑＝記明原因後可發射，但入此計數且**不可事後洗銷**——停跑不是逃生門）。（試行版的五格成績單／七件套／三率機制已由本版取代；試行最終帳＝畢業 PR 的十支可重算台帳，**嚴格口徑**（僅證據乾淨樣本入分子、分母恆 10）：驗真率 94.7%、誤報率 5.3%、有效獨家 1.1/支——樣本含一漏跑、一漏帳、兩證據不齊、一遲掃，其他採計組合 1.4–2.5/支皆可自台帳重算，全數高於原裁決門檻 0.8。）
