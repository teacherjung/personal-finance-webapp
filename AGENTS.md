...
- **值不值得改**：**審查回饋不無條件全改**。只有三種情況才回頭改——①**考題其實沒在守它宣稱的東西**（假綠）②**註解／題名說了撐不住的保證** ③**真的產線 bug**。其餘一律寫進待辦，**不再為它多開一輪審查**。
  ...
  ⚠️ **但這條只管「還要不要繼續」**，不影響例外①：**單一條建議該不該照著改，不確定時照樣要問 William**。（Codex #457 r1 阻擋③：原句寫成無條件禁止詢問，跟例外①互相打架——兩句管的其實是不同的問題。）

**三個例外的界線（William 2026-08-14 裁定）**：原本只有「判準／金額口徑／架構」三個詞，Codex #457 r1 阻擋②指出**「不確定就問」擋不住 AI 很有把握地分錯類**——分錯的當下不會覺得自己不確定。所以界線寫出來：
...
| **架構** | 模組邊界、資料流向、欄位歸屬——**新增或移動邊界** | **純搬家**：行為不變、沒有新邊界（搬 helper、拆檔） |

⚠️ **「純顯示格式也算」是刻意的**：使用者**看錯**數字的後果，跟**算錯**數字一樣。
...
**一句話總綱**：Claude 負責建造，Codex 負責挑戰，William 負責決定與驗收；共享檔案一次只交給一人。（⚠️ 總綱這句的「決定」＝上方角色分工表那四項：**產品決定、需求優先序、畫面驗收、合併裁決**。**不含**「審查回饋修不修」——見「審查回饋處置」。摘要句比表格鬆是本檔反覆出現的漂移型態，這裡就地釘住。）高風險工作小步前進，每一步有考題、有證據、有退路。文件說明規則，契約連接模組，PR 隔離變更。
