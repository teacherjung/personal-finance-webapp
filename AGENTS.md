...
- **值不值得改**：**審查回饋不無條件全改**。只有三種情況才回頭改——①**考題其實沒在守它宣稱的東西**（假綠）②**註解／題名說了撐不住的保證** ③**真的產線 bug**。其餘一律寫進待辦，**不再為它多開一輪審查**。
  ...
  ⚠️ 反向的停損也有：**輪數本身不是問題**，磨到有價值可以繼續；但**超過 7 輪就要自己回頭問「是不是方向走偏了」**，而且**要在該 PR 留一則公開的自評留言**（第一行固定「🧭 自評｜r<輪次>」；內容＝為什麼還要繼續、方向有沒有偏、還剩什麼沒收、預計再幾輪）——**第 8 輪起、每 7 輪一則**：**應貼的輪次＝除以 7 餘 1 且 ≥ 8 的那些**（8、15、22、29…）。⚠️ **格子固定、不隨實際貼文移動**：漏貼晚補、或自願多貼，都**不順延**下一格；「輪次」＝結論留言標頭那個 `r` 值（William 2026-09-10 裁 a，原話逐字「a＝每再超過七輪就再留一則（第 8、15、22 輪；我建議）」，落點＝https://github.com/teacherjung/personal-finance-webapp/pull/590#issuecomment-5605696747 ；⚠️ **餘數寫法、往後無限延、以及「不順延」都是 Claude 的操作化**——他的原話只到那三個序數；本句原本還有一句「一則只交代到貼它的那一輪為止」，Grok #590 掃後判定那是**修辭**（沒有可違反的行為、沒有下游），而且它自帶第二套起算點、與上面的固定格子打架，已刪）⚠️ **這條不是剎車，照實說**：**沒有任何東西要求後一則與前一則有實質差異**——把上一則改個輪次再貼一次，字面上仍然合規；而且「輪數本身不是問題」仍在上面那半句，所以**每 7 輪貼一則、輪數照樣長到三十輪，全程都可以合規**。它保證的只有一件事：「有沒有停下來想」這件事在紀錄上**定期出現**——William 2026-09-09 裁（原話逐字）「要，而且必要，我們避免耗費資源做沒有意義的審查」，落點＝#589 裡他的裁示留言 https://github.com/teacherjung/personal-finance-webapp/pull/589#issuecomment-5603974572 ；沒留＝違反本條——**後果只有紀錄上看得出來**（沒有機器擋、審查者也不會因此下阻擋結論；Grok #589 掃後改口：原句「沒留＝當沒問過自己」是空話，沒有任何下游）。⚠️ 這一則**不是問他、不等他回**：**這一個問題（還要不要繼續磨）仍由 Claude 自己下判斷、不可以丟回給 William**——他明講過「為什麼一直過不了」不是問他、是問自己；留言只是讓他事後看得到我想過、想了什麼。⚠️ 沒有任何閘在數輪數或看這則留言，全靠自律；**唯一在看的是審查者**：`REVIEW-AND-MERGE.md`「固定維度」那節有一條，要它在受審輪次 ≥ 8 時逐一核對每個應貼輪次有沒有對應的自評留言、缺了就列成一條發現（William 2026-09-10 裁「丙 加進發任務單的固定清單。現在做」，落點＝__URL__；⚠️ 落在**固定維度**而不是每輪客製那份，是 Claude 的操作化：客製那份要我自己每輪寫進去，會漏）。⚠️ 它**只看有沒有貼**，看不出內容是不是照抄上一則；缺貼也**不是阻擋理由**〔2026-09-09〜10 的沿革：原句寫「請審查者順手核」，Grok #589 掃後改口成「固定核對清單目前沒有這一項」——那兩句先後都是真的，現在這一項真的加進去了〕。
...
| 角色 | 主要責任 | 不負責 |
|---|---|---|
| William | 產品決定、需求優先序、畫面驗收、合併裁決；**審查回饋的例外**（清單刻意不複述在這裡——見「審查回饋處置」） | 不需判斷程式實作細節；**一般的審查回饋修不修不必問他**（2026-08-14 起由 Claude 判斷） |
| Claude | 主要實作、考題、PR、自審、技術文件更新；**複審 Codex 實作的 PR 並代執行合併**（2026-07-30 對稱常設授權）；**判斷審查回饋修不修**（2026-08-14，例外見「審查回饋處置」） | 不自行推翻已拍板的產品規則；**不複審、不放行自己實作的支**；**「審查回饋處置」列的例外一律不自行拍板**——那些即使是 Codex 提的也要問 William（清單刻意不複述在這裡：漏抄一條就等於多給自己一塊權限；問了他沒回怎麼辦＝該節「問法與逾時預設」那顆——那是暫定、不是拍板） |
...
**一句話總綱**：Claude 負責建造，Codex 負責挑戰，William 負責決定與驗收；共享檔案一次只交給一人（唯一例外＝「重疊 PR 讓道」的平行草稿——同款釘法：例外只在預約段定義，這半句不複述內容）。（⚠️ 總綱這句的「決定」**以上方角色分工表 William 那一列為準**——這裡刻意不複述、也不寫「哪幾項」，因為那一列會長：2026-08-14 就多了「審查回饋的例外」。**一般的審查回饋修不修不在他身上**，由 Claude 判斷。摘要句比表格鬆是本檔反覆出現的漂移型態，這裡就地釘住。）高風險工作小步前進，每一步有考題、有證據、有退路。文件說明規則，契約連接模組，PR 隔離變更。
