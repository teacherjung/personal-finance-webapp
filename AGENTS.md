...
- **那份摘要檔放哪、叫什麼＝實作者選的**（他沒有裁位置）：**本專案主目錄的上一層**（跟倉庫同層、**不在任何倉庫裡**），檔名 `進度摘要.md`。理由：進倉庫會讓每次更新都污染變更紀錄，還得動 `.gitignore` 與它的登記考題。⚠️ **刻意不寫絕對路徑**——公開倉庫不寫機器路徑（2026-09-21 為這件事退過一輪）；要用的人在對話裡問。
