...
- **預審參謀（常設；William 2026-08-16 拍板——試行 10 支期滿、獨立稽核修正三率後轉常設）**：Claude 實作的每支 PR，**送 Codex 正式審之前**把 diff 抽給 Grok 掃一遍。**呼叫紀律**：repo 外目錄餵材料檔（邊界前綴＋指示＋diff）、**不在任何 repo 目錄啟動它的 CLI**、**必鎖工具**（不鎖會越界＝#467 實測：自行搜家目錄、`cd` 進主目錄跑 git；逐字旗標與版本釘抄 REVIEW-AND-MERGE「怎麼執行」節前置檢查行——**CLI 版本不同＝當未跑**、人工重驗工具清單後更新）；材料以**已 commit、將上 GitHub 的內容**為限（與送 Codex 同一暴露等級），機密與真實資料不得入材料。發現由 Claude **逐條驗證**；修不修照「審查回饋處置」（例外照舊問 William），**判定要修且屬本支 diff 者＝送審前修掉**。**Grok 意見不算正式複審、不進任何合併閘、不產生結論字串**（合併機器零改動）。每支 PR 描述附**三件證據**：①**受掃版本與配方聲明**（精確 base／head SHA——不可用 `origin/main` 這類會移動的名稱——＋鎖工具旗標＋grok 版本；無配方聲明的掃描＝當未跑）②**Grok 回覆原文** ③**逐條判定**（每條標屬實／誤報，判定要修者標修畢——誤報率由此可算）。**主指標＝硬防漏網**：發現經後續正式審查覆核、確認是「Codex 自己沒抓到的真缺陷」（型如 #463 護欄假綠：r5 通過、r7 判無須補題、r8 被點名後對照組實證撤回）＝在該 PR 描述標「**硬防漏網**」一筆——第二雙眼睛的存在理由就是這格，不再追「有效獨家」總數（它量的是 Grok 贏過發稿者自審，量不出對整條產線的邊際價值）。**收回絆線**：連續 10 支硬防漏網掛零**且**誤報率超過兩成＝提請 William 重裁去留。（試行版的五格成績單／七件套／三率機制已由本版取代；試行最終帳——經 2026-08-16 獨立稽核修正：驗真率 86–92%、誤報率 8–11%、有效獨家 1.1–1.9/支，樣本含一漏跑、一漏帳、兩證據不齊，明細在畢業 PR。）
