...
- **值不值得改**：**審查回饋不無條件全改**。只有三種情況才回頭改——①**考題其實沒在守它宣稱的東西**（假綠）②**註解／題名說了撐不住的保證** ③**真的產線 bug**。其餘一律寫進待辦，**不再為它多開一輪審查**。
  ...
  ⚠️ **但這條只管「還要不要繼續」**，不影響例外①：**單一條建議該不該照著改，不確定時照樣要問 William**。兩句管的是不同的問題。

**三個例外的界線（William 2026-08-14 裁定）**：只有「判準／金額口徑／架構」三個詞是不夠的——**「不確定就問」擋不住 AI 很有把握地分錯類**（分錯的當下不會覺得自己不確定）。所以界線寫出來：
...
| **架構** | 模組邊界、資料流向、欄位歸屬——**新增或移動邊界**（把 helper 抽成**新模組**＝新增邊界，算） | **既有模組邊界內的純搬移**：行為不變、**不產生新的模組邊界**（同檔搬函式、同模組內拆區塊） |

⚠️ **架構那一列的兩欄要互斥**：判準是**有沒有新的模組邊界**，不是「有沒有搬東西」。
   「搬 helper」本身兩邊都可能落——抽成新檔＝算，留在原模組內移位置＝不算。
⚠️ **「純顯示格式也算」的理由**（起草時的推論，不是 William 的原話；規則本身才是他拍板的）：
   使用者是照**畫面上的數字**做決定的。
⚠️ 這張表是**下限、不是窮舉**。落在表外又說不準的，走例外①（不確定就問）——列舉補不完是本專案認過的病型，所以這裡不追求窮舉，只把已知會分錯的四個灰帶釘住。

**角色分工（含「不負責」邊界）**：

| 角色 | 主要責任 | 不負責 |
|---|---|---|
| William | 產品決定、需求優先序、畫面驗收、合併裁決；**審查回饋裡的三個例外**（判準／金額口徑／架構——界線見「審查回饋處置」） | 不需判斷程式實作細節；**一般的審查回饋修不修不必問他**（2026-08-14 起由 Claude 判斷） |
| Claude | 主要實作、考題、PR、自審、技術文件更新；**複審 Codex 實作的 PR 並代執行合併**（2026-07-30 對稱常設授權）；**判斷審查回饋修不修**（2026-08-14，例外見「審查回饋處置」） | 不自行推翻已拍板的產品規則；**不複審、不放行自己實作的支**；**不自行拍板判準／金額口徑／架構的改動**——那三類即使是 Codex 提的也要問 William |
...
**一句話總綱**：Claude 負責建造，Codex 負責挑戰，William 負責決定與驗收；共享檔案一次只交給一人。（⚠️ 總綱這句的「決定」**以上方角色分工表 William 那一列為準**——這裡刻意不複述、也不寫「哪幾項」，因為那一列會長：2026-08-14 就多了「審查回饋裡的三個例外」。**一般的審查回饋修不修不在他身上**，由 Claude 判斷。摘要句比表格鬆是本檔反覆出現的漂移型態，這裡就地釘住。）高風險工作小步前進，每一步有考題、有證據、有退路。文件說明規則，契約連接模組，PR 隔離變更。
