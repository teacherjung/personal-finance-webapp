...
| **不可逆整批操作前的真備份 `backupNow(tag)`**（Codex r3#7） | 與啟動備份不同檔：每 tag 一顆、重複執行覆蓋；**備份做不成就擋下操作＋讓使用者確認繼續**（唯一閘門＝`backupBeforeIrreversible`，回傳值不可丟）；新增不可逆整批操作時一併加 tag——完整契約 → [契約：資料與儲存](docs/contracts/data-storage.md#不可逆整批操作前的真備份-backupnow) |
