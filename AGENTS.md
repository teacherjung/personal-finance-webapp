...
- **同時最多 2 支**（William 2026-09-03 拍板）：**同時開著（open）的 PR，全部作者加起來最多 2 支；草稿也算 open。** 理由＝2026-09-03 流程體檢：塞車不是「一步一 PR」造成的，是「同時在飛的支數」造成的——跨 PR 試合併閘要把受審支和**每一支** open PR 各合一次、各跑一套三關，成本隨支數**平方**成長（6 支 open＝每合併一支多跑 5 套全卷）；而且大家都往同一批熱點檔案加字，支數越多撞行越多，而撞行的下游是連鎖的：撞到＝被迫 rebase ⇒ head 變 ⇒ 複審結論作廢重拿。熱點是量出來的，**但要連基準與口徑一起寫才算數**：以體檢當天的 main（`d26fb49`）為基準、逐 commit 數「最近 50 個 commit 裡改到該檔的有幾個」，`docs/contracts/income-expense.md` 22 次、本檔 18 次、`test/contract-split.test.js` 13 次、`docs/contracts/README.md` 13 次（口徑：`git log <基準> -50 --format='%H'`，每顆用 `git show --name-only` 看檔名有沒有出現；後兩者並列 13，不是嚴格名次）。⚠️ **換基準或換視窗，連名次都會翻**——實測：同基準改看最近 20 顆＝3／7／3／3，本檔變第一；基準改成 `d26fb49~5`、仍看 50 顆＝26／21／14／15，`docs/contracts/README.md` 反超 `test/contract-split.test.js`。所以這裡**不宣稱名次穩定、也不宣稱這組數字以外的任何事**，只宣稱一件在上述基準與口徑下量得到的觀測：協作與契約這幾份文件反覆出現在熱點前列。配套三句：①要開第 3 支＝**等**，不是開——開 PR 前先 `gh pr list --state open` 數一次，**含自己這支不得超過 2**；②等名額的空檔可以在**自己的分支**上把下一件事做到「準備好」（commit 照推），但**不開 PR**——做法沿用下方「重疊 PR 讓道」的**晚開**（那套做法是既有的），**但觸發條件是本條新增的**：讓道的晚開由「與在途預約重疊」觸發、要在持有人 PR 掛名並盯**該支**終局；**名額已滿但零重疊時沒有持有人可掛名、也沒有特定的一支要盯**，觸發解除的條件就只是「名額空出來」（任一支 open PR 終局）。⚠️ 這種晚開**不持有任何正式預約**（正式預約的唯一即時來源＝open PR 清單，見下方「共享檔案預約」）——所以撞車風險由晚開者自負，**開 PR 前要重查一次預約**（open PR 清單＋在飛那幾支上的讓道留言，兩處都要看）；③會改的檔案與在飛那兩支的預約有重疊的，照「重疊 PR 讓道」在持有人 PR 掛名。⚠️ **刻意不做機械閘、先靠人守**：`scripts/` 沒有對應的閘、COLLAB-MAP 閘表也沒有這一列——現有的堆疊閘只驗 base 與上下游、跨 PR 試合併閘只逐支試合併，**兩者都不會因為支數太多而拒絕你建第 3 支**；本 repo 加一道閘的門檻很高（自報 `MERGE_GATE`、閘表對帳、`test/collab-invariant-docs.test.js` 反查），這是工作習慣，等實際數據證明靠人守不住再加；在那之前「數一次」是每個開 PR 的人自己的事，漏數不會有東西擋你。
