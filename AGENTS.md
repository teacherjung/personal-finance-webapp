...
| 帳單「自動學習」店名＋分類（`db.learnedCategories`＝{ `storeKey`(cleanStore後原名) → {category?,subcategory?,name?} }） | 匯入預覽先過 `applyLearned`：**key＝`t.store`(=cleanStore(desc)，穩定不隨改名變)**，命中則覆蓋分類、並把顯示 `store` 換成學過的 `name`，且回傳 `storeKey` 給匯入端存起來。**匯入的交易會存 `storeKey` 欄（穩定 key）＋`note`(顯示名，可被改名覆蓋)**。學習時機：①匯入時使用者選的分類與 `categorize(原始desc)` **不同才記**（category）②`PUT /api/transactions/:id` 且 `source==='stmt'`：記 category＋**若 `note`≠`storeKey`(改了顯示名)記 `name`，改回自動名則清除 `name`**（key＝`storeKey`，舊資料無則退用 note；**手動記帳不學**）。`GET /api/learned`＋`POST /api/learned/delete`{key}；設定頁「帳單店名／分類學習」卡（原店名/顯示為/分類）可檢視/刪除。**設定頁「店名對照表」可直接編輯顯示名**（2026-07-18）＝`POST /api/statement/rename-store`{**orig**,name}：**以「帳單原文」為準**（同原文的各月份交易整批改 note＋以原文為 key 記學習）——銀行截斷的店名會讓不同分店共用同一個 storeKey（`FP-12MINI (桃`/`(新` → 都是 `12MINI`），以 storeKey 為準會連動改名（使用者踩到），原文級才能各自取名。學習口徑：新名＝`cleanStore(orig)` 自動名→清除自訂；否則記 `name`；服務費不學。**`applyLearned` 對應有「原文級覆蓋」：`learned[t.desc]` 優先於 `learned[storeKey]`**（未來匯入同原文沿用細名）。新 db 欄＝`learnedCategories`{}（已進 `emptyDb`＋`seed.json`）。使用者拍板：**自動記住店名＋分類**。**改 `stmtRef`/`cleanStore` 會動到 storeKey 穩定性，注意。**。**「國外交易服務費」不學**（說明帶當筆金額、key 永不再命中，只會讓學習表膨脹；其分類本就由 `finalize` 繼承前一筆）——判準用 `lib/statement.js` 的 `isServiceFee`（finalize 與 learning 共用，勿各寫一份） |
