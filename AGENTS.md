...
| **配方快取 P2**（P2-1 純模組 2026-08-15；格式 A 拍板） | 配方＝純資料規則卡：零正則（字面文字＋枚舉）、零帳單內容機械驗證（NFKC＋squash 後數字總量 ≥4 拒收、禁遮罩星號與分段直線）、幾何機關在引擎不在配方（bank-statement.js 同源泛化＋等價考題）、出生驗收＝重現 AI 答案才存；失敗碼 `recipe_parse_failed`（≠bank_unrecognized）；版本不留舊版、佇列留 P3；儲存/接線＝P2-2、產配方＝P2-3——完整契約 → [契約：收支記帳與匯入](docs/contracts/income-expense.md#配方快取-p2) |
