...
- **階段 C 多人上線（進行中，C1–C5 ✅、C6 首次部署及對抗驗收 ✅）**：雙模式開關（LOCAL／HOSTED）、帳號系統與 auth gate、租戶隔離（RLS＋CAS）、機密 envelope 加密與雲端匯出剝機密、速率限制與資源上限**都已上線**（細節見同步點清單）；**尚未完成＝C6 的後續項（per-user 配額、Cloudflare、上線監控等）與 C7**（真實資料上線＋DNS）。分階段裁決、威脅模型與各項現況見 `docs/多人上線-施工計畫.md`（狀態以它為準）。
