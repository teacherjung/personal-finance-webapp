...
| **不可逆整批操作「刻意沒有」操作前備份**（William 2026-08-08 裁決） | 店名規則與開 app 自動整理**不做** `pre-rules`／`pre-normalize` 備份、不擋、不問；畫面只寫「儲存後無法復原」，不承諾自動還原檔。救援＝每日滾動備份 30 天＋使用者自己按的「匯出備份」。⚠️ **看到「不可逆卻沒備份」想補的人先讀裁決**（`lib/services/backup.js` 檔中註解）——那是拿掉的、不是漏掉的，`test/vault-and-backup-integrity.test.js` 有一題釘著——完整契約 → [契約：資料與儲存](docs/contracts/data-storage.md#不可逆整批操作前的真備份-backupnow) |
