...
//   ⓪先確認禁區清單（settings.json 的 forbidden.deny）裡有一個刻意放的無害測試工具：它要是 Codex 連得到的伺服器上真的存在的 mcp__ 工具，就算真的執行了也不會動到禁區、不會造成任何改變；原本還有別的攔截組並存時，挑一個只有這份清單擋、別組不擋的名字。清單上還沒有就先加、合併，再從①開始——沒有就停下來，不可以拿任何別的工具代替。
